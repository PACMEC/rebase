...
/** Sets up the paCMec Environment. */
...
if ( is_array( get_site_option( 'illegal_names' ) ) && isset( $_GET['new'] ) && in_array( $_GET['new'], get_site_option( 'illegal_names' ), true ) ) {
	mt_redirect( network_home_url() );
	die();
}

/**
 * Prints signup_header via mt_head
 *
 * @since MU (3.0.0)
 */
...
add_action( 'mt_head', 'do_signup_header' );

if ( ! is_multisite() ) {
	mt_redirect( mt_registration_url() );
	die();
}

if ( ! is_main_site() ) {
	mt_redirect( network_site_url( 'mt-signup.php' ) );
	die();
}

// Fix for page title.
$mt_query->is_404 = false;
 ...
 * @since MU (3.0.0)
 */
function mtmu_signup_stylesheet() {
	?>
	<style type="text/css">
...
add_action( 'mt_head', 'mtmu_signup_stylesheet' );
get_header( 'mt-signup' );
...
<div class="mu_register mt-signup-container" role="main">
<?php
/**
 ...
 * @param MT_Error|string $errors     A MT_Error object containing existing errors. Defaults to empty string.
 */
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param MT_Error $errors A MT_Error object possibly containing 'blogname' or 'blog_title' errors.
	 */
...
function validate_blog_form() {
	...
	return mtmu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'], $user );
}
...
function show_user_form( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param MT_Error $errors A MT_Error object containing 'user_name' or 'user_email' errors.
	 */
...
function validate_user_form() {
	return mtmu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
}
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type MT_Error $errors     A MT_Error object possibly containing 'blogname' or 'blog_title' errors.
	 * }
	 */
	...
	<form id="setupform" method="post" action="mt-signup.php">
 ...
 * @global MT_Error        $errors     Existing errors in the global scope.
...
function validate_another_blog_signup() {
	...
	if ( ! empty( $_POST['MTLANG'] ) ) {

		$languages = signup_get_available_languages();

		if ( in_array( $_POST['MTLANG'], $languages, true ) ) {
			$language = mt_unslash( sanitize_text_field( $_POST['MTLANG'] ) );

			if ( $language ) {
				$blog_meta_defaults['MTLANG'] = $language;
			}
		}
	}
	...
	$blog_id = mtmu_create_blog( $domain, $path, $blog_title, $current_user->ID, $meta, get_current_network_id() );
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array(), $blog_id = 0 ) {
	...
	if ( $blog_id ) {
		...
		$login_url = 'http://' . $domain . $path . 'mt-login.php';
	}
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 *     @type MT_Error $errors     A MT_Error object with possible errors relevant to the sign-up user.
	 * }
	 */
	...
	<form id="setupform" method="post" action="mt-signup.php" novalidate="novalidate">
		<input type="hidden" name="stage" value="validate-user-signup" />
		<?php
		/** This action is documented in mt-signup.php */
...
function validate_user_signup() {
	...
	/** This filter is documented in mt-signup.php */
	mtmu_signup_user( $user_name, $user_email, apply_filters( 'add_signup_meta', array() ) );
...
function signup_blog( $user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type MT_Error $errors     A MT_Error object with possible errors relevant to new site creation variables.
	 * }
	 */
...
function validate_blog_signup() {
	...
	$user_result = mtmu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
	...
	$result     = mtmu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'] );
	$domain     = $result['domain'];
	$path       = $result['path'];
	$blogname   = $result['blogname'];
	$blog_title = $result['blog_title'];
	...
	mtmu_signup_blog( $domain, $path, $blog_title, $user_name, $user_email, $meta );
 ...
 */
$active_signup = apply_filters( 'mtmu_active_signup', $active_signup );
...
if ( 'none' === $active_signup ) {
	...
	$login_url = mt_login_url( network_site_url( 'mt-signup.php' ) );
...
<?php
/**
 * Fires after the sign-up forms, before mt_footer.
 *
 * @since 3.0.0
 */
