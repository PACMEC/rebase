 ...
 * @subpackage Database
 */
define( 'MT_REPAIRING', true );

require_once dirname( dirname( __DIR__ ) ) . '/mt-load.php';
...
<head>
	...
	<title><?php _e( 'paCMec &rsaquo; Database Repair' ); ?></title>
...
if ( ! defined( 'MT_ALLOW_REPAIR' ) || ! MT_ALLOW_REPAIR ) {
	...
	echo "</p><p><code>define('MT_ALLOW_REPAIR', true);</code></p>";
	...
	$query = $mtdb->prepare( 'SHOW TABLES LIKE %s', $mtdb->esc_like( $mtdb->sitecategories ) );
	if ( is_multisite() && ! $mtdb->get_var( $query ) ) {
	...
	foreach ( $tables as $table ) {
		$check = $mtdb->get_row( "CHECK TABLE $table" );
		...
		if ( 'OK' === $check->Msg_text ) {
			...
			printf( __( 'The %1$s table is not okay. It is reporting the following error: %2$s. paCMec will attempt to repair this table&hellip;' ), "<code>$table</code>", "<code>$check->Msg_text</code>" );

			$repair = $mtdb->get_row( "REPAIR TABLE $table" );
		...
		if ( $okay && $optimize ) {
			$analyze = $mtdb->get_row( "ANALYZE TABLE $table" );
			...
			if ( 'Table is already up to date' === $analyze->Msg_text ) {
				...
				$optimize = $mtdb->get_row( "OPTIMIZE TABLE $table" );
	...
	if ( $problems ) {
		...
		echo '<p>' . __( 'Repairs complete. Please remove the following line from mt-config.php to prevent this page from being used by unauthorized users.' ) . "</p><p><code>define('MT_ALLOW_REPAIR', true);</code></p>";
	}
} else {

	echo '<h1 class="screen-reader-text">' . __( 'paCMec database repair' ) . '</h1>';

	if ( isset( $_GET['referrer'] ) && 'is_blog_installed' === $_GET['referrer'] ) {
		echo '<p>' . __( 'One or more database tables are unavailable. To allow paCMec to attempt to repair these tables, press the &#8220;Repair Database&#8221; button. Repairing can take a while, so please be patient.' ) . '</p>';
	} else {
		echo '<p>' . __( 'paCMec can automatically look for some common database problems and repair them. Repairing can take a while, so please be patient.' ) . '</p>';
	}
	?>
	<p class="step"><a class="button button-large" href="repair.php?repair=1"><?php _e( 'Repair Database' ); ?></a></p>
	<p><?php _e( 'paCMec can also attempt to optimize the database. This improves performance in some situations. Repairing and optimizing the database can take a long time and the database will be locked while optimizing.' ); ?></p>
