 ...
 * base prefix, paCMec core can no longer account for the possible collision.
 */
...
if ( isset( $_POST['permalink_structure'] ) || isset( $_POST['category_base'] ) ) {
	...
	if ( isset( $_POST['category_base'] ) ) {
		...
		$mt_rewrite->set_category_base( $category_base );
	}
	...
	if ( isset( $_POST['tag_base'] ) ) {
		...
		$mt_rewrite->set_tag_base( $tag_base );
	}
}
...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess' ) && is_writable( $home_path ) ) || is_writable( $home_path . '.htaccess' ) ) {
		...
		$writable       = false;
		$existing_rules = array_filter( extract_from_markers( $home_path . '.htaccess', 'paCMec' ) );
...
$using_index_permalinks = $mt_rewrite->using_index_permalinks();
...
if ( $structure_updated ) {
	...
	mt_redirect( admin_url( 'options-permalink.php?settings-updated=true' ) );
	exit;
}
...
<?php mt_nonce_field( 'update-permalink' ); ?>
...
<form action="options-permalink.php" method="post">
				<?php mt_nonce_field( 'update-permalink' ); ?>
	<p><textarea rows="9" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $mt_rewrite->iis7_url_rewrite_rules() ); ?></textarea></p>
</form>
<p>
				<?php
				printf(
...
<form action="options-permalink.php" method="post">
			<?php mt_nonce_field( 'update-permalink' ); ?>
	<p><textarea rows="18" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $mt_rewrite->iis7_url_rewrite_rules( true ) ); ?></textarea></p>
</form>
<p>
			<?php
			printf(
...
<form action="options-permalink.php" method="post">
		<?php mt_nonce_field( 'update-permalink' ); ?>
	<p><textarea rows="6" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $mt_rewrite->mod_rewrite_rules() ); ?></textarea></p>
</form>
	<?php endif; ?>
<?php endif; ?>
