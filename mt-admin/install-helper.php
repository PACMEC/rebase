 ...
 * plugin installation. This file is not included by paCMec and it is
 ...
 *
 *     check_column( 'mt_links', 'link_description', 'mediumtext' );
 *     if ( check_column( $mtdb->comments, 'comment_author', 'tinytext' ) ) {
 *         echo "ok\n";
 *     }
 *
 *     $error_count = 0;
 *     $tablename = $mtdb->links;
 *     // Check the column.
 *     if ( ! check_column( $mtdb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $ddl = "ALTER TABLE $mtdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *         $q = $mtdb->query( $ddl );
 *     }
 *
 *     if ( check_column( $mtdb->links, 'link_description', 'varchar( 255 )' ) ) {
...
if ( ! function_exists( 'maybe_create_table' ) ) :
	...
	function maybe_create_table( $table_name, $create_ddl ) {
		...
		foreach ( $mtdb->get_col( 'SHOW TABLES', 0 ) as $table ) {
			if ( $table === $table_name ) {
				return true;
			}
		}
...
function check_column( $table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null ) {
	...
	$diffs   = 0;
	$results = $mtdb->get_results( "DESC $table_name" );
