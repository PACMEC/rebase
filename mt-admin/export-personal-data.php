...
if ( ! current_user_can( 'export_others_personal_data' ) ) {
	mt_die( __( 'Sorry, you are not allowed to export personal data on this site.' ) );
}
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p>' . __( '<strong>Note: As this tool only gathers data from paCMec and participating plugins, you may need to do more to comply with export requests. You should also send the requestor any data collected by or stored with any 3rd party services used by your business or site.</strong>' ) . '</p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Personal data that is collected by paCMec and is displayed in the export files include:' ) . '</p>' .
...
$requests_table = _get_list_table( 'MT_Privacy_Data_Export_Requests_List_Table', $_list_table_args );
...
<div class="wrap nosubsub">
	...
	<form action="<?php echo esc_url( admin_url( 'export-personal-data.php' ) ); ?>" method="post" class="mt-privacy-request-form">
