...
if ( ! current_user_can( 'upload_files' ) ) {
	mt_die( __( 'Sorry, you are not allowed to upload files.' ), 403 );
}

mt_enqueue_script( 'plupload-handlers' );
mt_enqueue_script( 'image-edit' );
mt_enqueue_script( 'set-post-thumbnail' );
mt_enqueue_style( 'imgareaselect' );
mt_enqueue_script( 'media-gallery' );

header( 'Content-Type: ' . get_option( 'html_type' ) . '; charset=' . get_option( 'blog_charset' ) );

// IDs should be integers.
$ID      = isset( $ID ) ? (int) $ID : 0; // phpcs:ignore paCMec.NamingConventions.ValidVariableName
$post_id = isset( $post_id ) ? (int) $post_id : 0;

// Require an ID for the edit screen.
if ( isset( $action ) && 'edit' === $action && ! $ID ) { // phpcs:ignore paCMec.NamingConventions.ValidVariableName
	mt_die(
