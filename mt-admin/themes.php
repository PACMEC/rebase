...
if ( current_user_can( 'switch_themes' ) && isset( $_GET['action'] ) ) {
	if ( 'activate' === $_GET['action'] ) {
		...
		$theme = mt_get_theme( $_GET['stylesheet'] );
		...
		mt_redirect( admin_url( 'themes.php?activated=true' ) );
		exit;
		...
		mt_redirect( admin_url( 'themes.php?resumed=true' ) );
		exit;
		...
		if ( $active->get( 'Template' ) == $_GET['stylesheet'] ) {
			mt_redirect( admin_url( 'themes.php?delete-active-child=true' ) );
		} else {
			delete_theme( $_GET['stylesheet'] );
			mt_redirect( admin_url( 'themes.php?deleted=true' ) );
		}
		exit;
	} elseif ( 'enable-auto-update' === $_GET['action'] ) {
		if ( ! ( current_user_can( 'update_themes' ) && mt_is_auto_update_enabled_for_type( 'theme' ) ) ) {
			mt_die( __( 'Sorry, you are not allowed to enable themes automatic updates.' ) );
		}
		...
		mt_redirect( admin_url( 'themes.php?enabled-auto-update=true' ) );

		exit;
	} elseif ( 'disable-auto-update' === $_GET['action'] ) {
		if ( ! ( current_user_can( 'update_themes' ) && mt_is_auto_update_enabled_for_type( 'theme' ) ) ) {
			mt_die( __( 'Sorry, you are not allowed to disable themes automatic updates.' ) );
		}
		...
		mt_redirect( admin_url( 'themes.php?disabled-auto-update=true' ) );
...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your paCMec installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'update_themes' ) && mt_is_auto_update_enabled_for_type( 'theme' ) ) {
	$help_tab_autoupdates =
		'<p>' . __( 'Auto-updates can be enabled or disabled for each individual theme. Themes with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the MT-Cron task scheduling system.' ) . '</p>' .
...
if ( current_user_can( 'switch_themes' ) ) {
	$themes = mt_prepare_themes_for_js();
} else {
	$themes = mt_prepare_themes_for_js( array( mt_get_theme() ) );
}
mt_reset_vars( array( 'theme', 'search' ) );
...
<div class="wrap">
	<h1 class="mt-heading-inline"><?php esc_html_e( 'Themes' ); ?>
...
if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
	foreach ( (array) $submenu['themes.php'] as $item ) {
		...
		if ( ! empty( $submenu[ $item[2] ] ) ) {
			...
			if ( file_exists( MT_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}" ) || ! empty( $menu_hook ) ) {
			...
			if ( file_exists( ABSPATH . "mt-admin/$menu_file" ) ) {
...
<div class="theme<?php echo $active_class; ?>" tabindex="0" aria-describedby="<?php echo $aria_action . ' ' . $aria_name; ?>">
	...
	<?php if ( $theme['hasUpdate'] ) : ?>
		<?php if ( $theme['updateResponse']['compatibleMT'] && $theme['updateResponse']['compatiblePHP'] ) : ?>
		...
		<?php else : ?>
			<div class="update-message notice inline notice-error notice-alt"><p>
				<?php
				if ( ! $theme['updateResponse']['compatibleMT'] && ! $theme['updateResponse']['compatiblePHP'] ) {
					printf(
				...
				} elseif ( ! $theme['updateResponse']['compatibleMT'] ) {
					printf(
						/* translators: %s: Theme name. */
	...
	<?php
	if ( ! $theme['compatibleMT'] || ! $theme['compatiblePHP'] ) {
	...
	<div class="theme-id-container">
		...
		<?php if ( $theme['active'] ) { ?>
			...
			<?php } ?>
		<?php } elseif ( $theme['compatibleMT'] && $theme['compatiblePHP'] ) { ?>
			<?php
			/* translators: %s: Theme name. */
...
$broken_themes = mt_get_themes( array( 'errors' => true ) );
		...
		<tr>
			...
			if ( $can_resume ) {
				if ( 'theme_paused' === $broken_theme->errors()->get_error_code() ) {
					...
					$resume_url = mt_nonce_url( $resume_url, 'resume-theme_' . $stylesheet );
					?>
			...
			if ( $can_delete ) {
				...
				$delete_url = mt_nonce_url( $delete_url, 'delete-theme_' . $stylesheet );
				?>
			...
			if ( $can_install && 'theme_no_parent' === $broken_theme->errors()->get_error_code() ) {
				...
				if ( ! is_mt_error( $parent_theme ) ) {
					...
					$install_url = mt_nonce_url( $install_url, 'install-theme_' . $parent_theme_name );
					?>
 ...
 */
function mt_theme_auto_update_setting_template() {
	$template = '
		<div class="theme-autoupdate">
			<# if ( data.autoupdate.supported ) { #>
				...
				<# } else if ( data.autoupdate.enabled ) { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-mt-action="disable">
						<span class="dashicons dashicons-update spin hidden" aria-hidden="true"></span><span class="label">' . __( 'Disable auto-updates' ) . '</span>
					</button>
				<# } else { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-mt-action="enable">
			...
			<# if ( data.hasUpdate ) { #>
				...
				<# } #>
				<br />' . mt_get_auto_update_message() . '</span>
			<# } #>
	 ...
	 *
	 * See {@see mt_prepare_themes_for_js()} for the properties of the `data` object.
	 *
	 * @since 5.5.0
	 *
		...
		<div class="theme-about mt-clearfix">
			...
			<div class="theme-info">
				...
				<# if ( data.actions.autoupdate ) { #>
					<?php echo mt_theme_auto_update_setting_template(); ?>
				<# } #>
