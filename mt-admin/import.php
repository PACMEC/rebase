<?php
/**
 * Import paCMec Administration Screen
 *
 * @package paCMec
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' => '<p>' . __( 'This screen lists links to plugins to import data from blogging/content management platforms. Choose the platform you want to import from, and click Install Now when you are prompted in the popup window. If your platform is not listed, click the link to search the plugin directory for other importer plugins to see if there is one for your platform.' ) . '</p>' .
			'<p>' . __( 'In previous versions of paCMec, all importers were built-in. They have been turned into plugins since most people only use them once or infrequently.' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/tools-import-screen/">Documentation on Import</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
);

if ( current_user_can( 'install_plugins' ) ) {
	// List of popular importer plugins from the paCMec.org API.
	$popular_importers = mt_get_popular_importers();
} else {
...
if ( ! empty( $_GET['invalid'] ) && isset( $popular_importers[ $_GET['invalid'] ] ) ) {
	...
	if ( $importer_id !== $_GET['invalid'] ) { // Prevent redirect loops.
		mt_redirect( admin_url( 'admin.php?import=' . $importer_id ) );
		exit;
	}
	unset( $importer_id );
}
...
<?php endif; ?>
<p><?php _e( 'If you have posts or comments in another system, paCMec can import those into this site. To get started, choose a system to import from below:' ); ?></p>

<?php
// Registered (already installed) importers. They're stored in the global $mt_importers.
...
foreach ( $popular_importers as $pop_importer => $pop_data ) {
	...
	// Fill the array of registered (already installed) importers with data of the popular importers from the paCMec.org API.
...
<table class="widefat importers striped">
	...
	foreach ( $importers as $importer_id => $data ) {
		...
		if ( isset( $data['install'] ) ) {
			...
			if ( file_exists( MT_PLUGIN_DIR . '/' . $plugin_slug ) ) {
