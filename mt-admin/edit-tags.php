...
if ( ! in_array( $tax->name, get_taxonomies( array( 'show_ui' => true ) ), true ) ) {
	mt_die( __( 'Sorry, you are not allowed to edit terms in this taxonomy.' ) );
}
...
/**
 * $post_type is set when the MT_Terms_List_Table instance is created
 *
 * @global string $post_type
 */
global $post_type;

$mt_list_table = _get_list_table( 'MT_Terms_List_Table' );
...
if ( ! $referer ) { // For POST requests.
	$referer = mt_unslash( $_SERVER['REQUEST_URI'] );
}
$referer = remove_query_arg( array( '_mt_http_referer', '_mtnonce', 'error', 'message', 'paged' ), $referer );
switch ( $mt_list_table->current_action() ) {

	case 'add-tag':
		check_admin_referer( 'add-tag', '_mtnonce_add-tag' );
		...
		$ret = mt_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
	...
	case 'delete':
		...
		mt_delete_term( $tag_ID, $taxonomy );
	...
	case 'edit':
		...
		mt_redirect( esc_url_raw( get_edit_term_link( $term_id, $taxonomy, $post_type ) ) );
		exit;
	...
	case 'editedtag':
		...
		$ret = mt_update_term( $tag_ID, $taxonomy, $_POST );
		...
		break;
	default:
		if ( ! $mt_list_table->current_action() || ! isset( $_REQUEST['delete_tags'] ) ) {
			break;
		}
		check_admin_referer( 'bulk-tags' );
		...
		$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $mt_list_table->current_action(), $tags ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
		break;
}

if ( ! $location && ! empty( $_REQUEST['_mt_http_referer'] ) ) {
	$location = remove_query_arg( array( '_mt_http_referer', '_mtnonce' ), mt_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( $location ) {
	 ...
	 */
	mt_redirect( apply_filters( 'redirect_term_location', $location, $tax ) );
	exit;
}

$mt_list_table->prepare_items();
$total_pages = $mt_list_table->get_pagination_arg( 'total_pages' );

if ( $pagenum > $total_pages && $total_pages > 0 ) {
	mt_redirect( add_query_arg( 'paged', $total_pages ) );
	exit;
}

mt_enqueue_script( 'admin-tags' );
if ( current_user_can( $tax->cap->edit_terms ) ) {
	mt_enqueue_script( 'inline-edit-tax' );
}
...
if ( 'category' === $taxonomy || 'link_category' === $taxonomy || 'post_tag' === $taxonomy ) {
	...
	if ( 'category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/posts-categories-screen/">Documentation on Categories</a>' ) . '</p>';
	} elseif ( 'link_category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://codex.managertechnology.com.co/pacmec/Links_Link_Categories_Screen">Documentation on Link Categories</a>' ) . '</p>';
	} else {
		$help .= '<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/posts-tags-screen/">Documentation on Tags</a>' ) . '</p>';
	}

	$help .= '<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>';
...
require_once ABSPATH . 'mt-admin/includes/edit-tag-messages.php';

$class = ( isset( $_REQUEST['error'] ) ) ? 'error' : 'updated';

if ( is_plugin_active( 'mtcat2tag-importer/mtcat2tag-importer.php' ) ) {
	$import_link = admin_url( 'admin.php?import=mtcat2tag' );
} else {
...
<form class="search-form mt-clearfix" method="get">
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />

<?php $mt_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>
...
if ( $can_edit_terms ) {
	?>
<div id="col-container" class="mt-clearfix">
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />
	<?php mt_nonce_field( 'add-tag', '_mtnonce_add-tag' ); ?>
...
<div class="form-field term-parent-wrap">
	<label for="parent"><?php echo esc_html( $tax->labels->parent_item ); ?></label>
		...
		mt_dropdown_categories( $dropdown_args );
		?>
 ...
 */
do_action( "after-{$taxonomy}-table", $taxonomy );  // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
