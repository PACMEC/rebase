...
if ( ! current_user_can( 'erase_others_personal_data' ) || ! current_user_can( 'delete_users' ) ) {
	mt_die( __( 'Sorry, you are not allowed to erase personal data on this site.' ) );
}
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p>' . __( 'The tool associates data stored in paCMec by a supplied email address, including profile data and comments.' ) . '</p>' .
					'<p>' . __( '<strong>Note: As this tool only gathers data from paCMec and participating plugins, you may need to do more to comply with erasure requests. You should also delete any data collected by or stored with any 3rd party services used by your business or site.</strong>' ) . '</p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Personal data that is collected by paCMec and is deleted or anonymized include:' ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Many plugins may collect or store personal data either in the paCMec database or remotely. Any Erase Personal Data request should delete data from plugins as well.' ) . '</p>' .
					'<p>' . __( 'Plugin authors can <a href="https://developer.managertechnology.com.co/pacmec/plugins/privacy/adding-the-personal-data-eraser-to-your-plugin/" target=_blank"> learn more about how to add support for the Personal Data Eraser to a plugin here</a>.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/tools-erase-personal-data-screen/">Documentation on Erase Personal Data</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
);

// Handle list table actions.
_mt_personal_data_handle_actions();

// Cleans up failed and expired requests before displaying the list table.
_mt_personal_data_cleanup_requests();

mt_enqueue_script( 'privacy-tools' );
...
$requests_table = _get_list_table( 'MT_Privacy_Data_Removal_Requests_List_Table', $_list_table_args );
...
<div class="wrap nosubsub">
	...
	<form action="<?php echo esc_url( admin_url( 'erase-personal-data.php' ) ); ?>" method="post" class="mt-privacy-request-form">
		<h2><?php esc_html_e( 'Add Data Erasure Request' ); ?></h2>
		<div class="mt-privacy-request-form-field">
		...
		<?php mt_nonce_field( 'personal-data-request' ); ?>
	...
	<form class="search-form mt-clearfix">
