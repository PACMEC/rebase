...
switch ( $action ) {
	...
	case 'editcomment':
		...
		require ABSPATH . 'mt-admin/edit-form-comment.php';
	...
	case 'spam':
		...
		if ( ! $comment ) {
			mt_redirect( admin_url( 'edit-comments.php?error=1' ) );
			die();
		}

		if ( ! current_user_can( 'edit_comment', $comment->comment_ID ) ) {
			mt_redirect( admin_url( 'edit-comments.php?error=2' ) );
			die();
		}

		// No need to re-approve/re-trash/re-spam a comment.
		if ( str_replace( '1', 'approve', $comment->comment_approved ) === $action ) {
			mt_redirect( admin_url( 'edit-comments.php?same=' . $comment_id ) );
			die();
		}
...
<tr>
	...
	case 'unapprovecomment':
		...
		if ( mt_get_referer() && ! $noredir && false === strpos( mt_get_referer(), 'comment.php' ) ) {
			$redir = mt_get_referer();
		} elseif ( mt_get_original_referer() && ! $noredir ) {
			$redir = mt_get_original_referer();
		...
		switch ( $action ) {
			case 'deletecomment':
				mt_delete_comment( $comment );
				$redir = add_query_arg( array( 'deleted' => '1' ), $redir );
				break;
			case 'trashcomment':
				mt_trash_comment( $comment );
				$redir = add_query_arg(
					array(
						'trashed' => '1',
						'ids'     => $comment_id,
				...
				break;
			case 'untrashcomment':
				mt_untrash_comment( $comment );
				$redir = add_query_arg( array( 'untrashed' => '1' ), $redir );
				break;
			case 'spamcomment':
				mt_spam_comment( $comment );
				$redir = add_query_arg(
					array(
						'spammed' => '1',
						'ids'     => $comment_id,
				...
				break;
			case 'unspamcomment':
				mt_unspam_comment( $comment );
				$redir = add_query_arg( array( 'unspammed' => '1' ), $redir );
				break;
			case 'approvecomment':
				mt_set_comment_status( $comment, 'approve' );
				$redir = add_query_arg( array( 'approved' => 1 ), $redir );
				break;
			case 'unapprovecomment':
				mt_set_comment_status( $comment, 'hold' );
