...
if ( ! current_user_can( 'activate_plugins' ) ) {
	mt_die( __( 'Sorry, you are not allowed to manage plugins for this site.' ) );
}

$mt_list_table = _get_list_table( 'MT_Plugins_List_Table' );
$pagenum       = $mt_list_table->get_pagenum();

$action = $mt_list_table->current_action();

$plugin = isset( $_REQUEST['plugin'] ) ? mt_unslash( $_REQUEST['plugin'] ) : '';
$s      = isset( $_REQUEST['s'] ) ? urlencode( mt_unslash( $_REQUEST['s'] ) ) : '';
	...
	switch ( $action ) {
		case 'activate':
			if ( ! current_user_can( 'activate_plugin', $plugin ) ) {
				mt_die( __( 'Sorry, you are not allowed to activate this plugin.' ) );
			}

			if ( is_multisite() && ! is_network_admin() && is_network_only_plugin( $plugin ) ) {
				mt_redirect( self_admin_url( "plugins.php?plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			if ( isset( $_GET['from'] ) && 'import' === $_GET['from'] ) {
				...
				mt_redirect( self_admin_url( 'import.php?import=' . str_replace( '-importer', '', dirname( $plugin ) ) ) );
			} elseif ( isset( $_GET['from'] ) && 'press-this' === $_GET['from'] ) {
				mt_redirect( self_admin_url( 'press-this.php' ) );
			} else {
				// Overrides the ?error=true one above.
				mt_redirect( self_admin_url( "plugins.php?activate=true&plugin_status=$status&paged=$page&s=$s" ) );
			}
			exit;

		case 'activate-selected':
			if ( ! current_user_can( 'activate_plugins' ) ) {
				mt_die( __( 'Sorry, you are not allowed to activate plugins for this site.' ) );
			}

			check_admin_referer( 'bulk-plugins' );

			$plugins = isset( $_POST['checked'] ) ? (array) mt_unslash( $_POST['checked'] ) : array();
			...
			mt_redirect( self_admin_url( "plugins.php?activate-multi=true&plugin_status=$status&paged=$page&s=$s" ) );
			exit;
		...
		case 'update-selected':
			...
			if ( isset( $_GET['plugins'] ) ) {
				$plugins = explode( ',', mt_unslash( $_GET['plugins'] ) );
			} elseif ( isset( $_POST['checked'] ) ) {
				$plugins = (array) mt_unslash( $_POST['checked'] );
			} else {
				$plugins = array();
			}
		...
		case 'error_scrape':
			...
			/** This action is documented in mt-admin/includes/plugin.php */
			do_action( "activate_{$plugin}" );
			exit;

		case 'deactivate':
			if ( ! current_user_can( 'deactivate_plugin', $plugin ) ) {
				mt_die( __( 'Sorry, you are not allowed to deactivate this plugin.' ) );
			}
			...
			if ( headers_sent() ) {
				...
				mt_redirect( self_admin_url( "plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s" ) );
			}
			exit;

		case 'deactivate-selected':
			if ( ! current_user_can( 'deactivate_plugins' ) ) {
				mt_die( __( 'Sorry, you are not allowed to deactivate plugins for this site.' ) );
			}
			...
			mt_redirect( self_admin_url( "plugins.php?deactivate-multi=true&plugin_status=$status&paged=$page&s=$s" ) );
			exit;

		case 'delete-selected':
			if ( ! current_user_can( 'delete_plugins' ) ) {
				mt_die( __( 'Sorry, you are not allowed to delete plugins for this site.' ) );
			}

			check_admin_referer( 'bulk-plugins' );

			// $_POST = from the plugin form; $_GET = from the FTP details screen.
			$plugins = isset( $_REQUEST['checked'] ) ? (array) mt_unslash( $_REQUEST['checked'] ) : array();
			if ( empty( $plugins ) ) {
				mt_redirect( self_admin_url( "plugins.php?plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			if ( empty( $plugins ) ) {
				mt_redirect( self_admin_url( "plugins.php?error=true&main=true&plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			if ( ! isset( $_REQUEST['verify-delete'] ) ) {
				...
				foreach ( (array) $plugins as $plugin ) {
					...
					if ( '.' === $plugin_slug ) {
						$data = get_plugin_data( MT_PLUGIN_DIR . '/' . $plugin );
						if ( $data ) {
				...
				<form method="post" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" style="display:inline;">
					...
					?>
					<?php mt_nonce_field( 'bulk-plugins' ); ?>
			...
			mt_redirect( self_admin_url( "plugins.php?deleted=$plugins_to_delete&plugin_status=$status&paged=$page&s=$s" ) );
			exit;
		...
		case 'resume':
			...
			if ( ! current_user_can( 'resume_plugin', $plugin ) ) {
				mt_die( __( 'Sorry, you are not allowed to resume this plugin.' ) );
			}
			...
			mt_redirect( self_admin_url( "plugins.php?resume=true&plugin_status=$status&paged=$page&s=$s" ) );
			exit;
		case 'enable-auto-update':
		case 'disable-auto-update':
		case 'enable-auto-update-selected':
		case 'disable-auto-update-selected':
			if ( ! current_user_can( 'update_plugins' ) || ! mt_is_auto_update_enabled_for_type( 'plugin' ) ) {
				mt_die( __( 'Sorry, you are not allowed to manage plugins automatic updates.' ) );
			}

			if ( is_multisite() && ! is_network_admin() ) {
				mt_die( __( 'Please connect to your network admin to manage plugins automatic updates.' ) );
			}
			...
			if ( 'enable-auto-update' === $action ) {
				...
				if ( $new_auto_updates == $auto_updates ) { // phpcs:ignore paCMec.PHP.StrictComparisons.LooseComparison
					mt_redirect( $redirect );
					exit;
				}
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				...
				$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $action, $plugins ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
				mt_safe_redirect( $sendback );
				exit;
			}
			break;
	}
...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( 'Overview' ),
		'content' =>
				'<p>' . __( 'Plugins extend and expand the functionality of paCMec. Once a plugin is installed, you may activate it or deactivate it here.' ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				'<p>' . __( 'Most of the time, plugins play nicely with the core of paCMec and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.' ) . '</p>' .
				'<p>' . sprintf(
					/* translators: %s: MT_PLUGIN_DIR constant value. */
					__( 'If something goes wrong with a plugin and you can&#8217;t use paCMec, delete or rename that file in the %s directory and it will be automatically deactivated.' ),
					'<code>' . MT_PLUGIN_DIR . '</code>'
				) . '</p>',
	)
);
...
if ( current_user_can( 'update_plugins' ) && mt_is_auto_update_enabled_for_type( 'plugin' ) ) {
	get_current_screen()->add_help_tab(
		array(
			...
			'content' =>
					'<p>' . __( 'Auto-updates can be enabled or disabled for each individual plugin. Plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the MT-Cron task scheduling system.' ) . '</p>' .
					'<p>' . __( 'Auto-updates are only available for plugins recognized by paCMec.org, or that include a compatible update system.' ) . '</p>' .
					'<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override paCMec scheduling.' ) . '</p>',
		)
	);
...
if ( isset( $_GET['error'] ) ) :
	...
	if ( ! isset( $_GET['main'] ) && ! isset( $_GET['charsout'] )
		&& isset( $_GET['_error_nonce'] ) && mt_verify_nonce( $_GET['_error_nonce'], 'plugin-activation-error_' . $plugin )
	) {
		$iframe_url = add_query_arg(
			array(
				'action'   => 'error_scrape',
				'plugin'   => urlencode( $plugin ),
				'_mtnonce' => urlencode( $_GET['_error_nonce'] ),
			),
...
<?php $mt_list_table->views(); ?>

<form class="search-form search-plugins" method="get">
<?php $mt_list_table->search_box( __( 'Search Installed Plugins' ), 'plugin' ); ?>
</form>
...
<?php
mt_print_request_filesystem_credentials_modal();
mt_print_admin_notice_templates();
mt_print_update_row_templates();
