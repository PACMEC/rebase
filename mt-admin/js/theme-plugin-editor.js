/**
 * @output mt-admin/js/theme-plugin-editor.js
 */
...
mt.themePluginEditor = (function( $ ) {
	'use strict';
	var component, TreeLinks,
		__ = mt.i18n.__, _n = mt.i18n._n, sprintf = mt.i18n.sprintf;
	...
	component.init = function init( form, settings ) {
		...
		component.noticeTemplate = mt.template( 'mt-file-editor-notice' );
		...
		if ( false !== component.codeEditor ) {
			/*
			 * Defer adding notices until after DOM ready as workaround for MT Admin injecting
	...
	component.showWarning = function() {
		...
		// Hide all the #mtwrap content from assistive technologies.
		$( '#mtwrap' ).attr( 'aria-hidden', 'true' );
		...
		setTimeout( function() {
			mt.a11y.speak( mt.sanitize.stripTags( rawMessage.replace( /\s+/g, ' ' ) ), 'assertive' );
		}, 1000 );
	};
	...
	component.submit = function( event ) {
		...
		request = mt.ajax.post( 'edit-theme-plugin-file', data );
	...
	component.initCodeEditor = function initCodeEditor() {
		...
		editor = mt.codeEditor.initialize( $( '#newcontent' ), codeEditorSettings );
 ...
 * @type {object}
 */
mt.themePluginEditor.l10n = mt.themePluginEditor.l10n || {
	saveAlert: '',
	saveError: '',
	lintError: {
		alternative: 'mt.i18n',
...
mt.themePluginEditor.l10n = window.mt.deprecateL10nObject( 'mt.themePluginEditor.l10n', mt.themePluginEditor.l10n, '5.5.0' );
