 ...
 *
 * @output mt-admin/js/tags-suggest.js
 */
( function( $ ) {
	...
	var tempID = 0;
	var separator = mt.i18n._x( ',', 'tag delimiter' ) || ',';
	 ...
	 *
	 * Example: `$( element ).mtTagsSuggest( options )`.
	 *
	 * The taxonomy can be passed in a `data-mt-taxonomy` attribute on the element or
	 ...
	 */
	$.fn.mtTagsSuggest = function( options ) {
		var cache;
		var last;
		var $element = $( this );
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-mt-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'mt-tags-autocomplete-' + ui.item.id );
			...
			select: function( event, ui ) {
				...
				if ( $.ui.keyCode.TAB === event.keyCode ) {
					...
					window.mt.a11y.speak( mt.i18n.__( 'Term selected.' ), 'assertive' );
					event.preventDefault();
		...
		$element.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="mt-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
