/**
 * @output mt-admin/js/user-profile.js
 */
...
(function($) {
	...
	function bindToggleButton() {
		$toggleButton = $pass1Row.find('.mt-hide-pw');
	...
	function bindPasswordRestLink() {
		$( '#generate-reset-link' ).on( 'click', function() {
			var $this  = $(this),
				...
				var resetAction =  mt.ajax.post( 'send-password-reset', data );
	...
	function bindPasswordForm() {
		...
		$submitButton = $( '#submit, #mt-submit' ).on( 'click', function () {
			updateLock = false;
		});
		...
		$passwordWrapper = $pass1Row.find( '.mt-pwd' );
		$generateButton  = $pass1Row.find( 'button.mt-generate-pw' );
		...
		$generateButton.on( 'click', function () {
			...
			mt.ajax.post( 'generate-password' )
				.done( function( data ) {
					$pass1.data( 'pw', data );
				} );
		} );

		$cancelButton = $pass1Row.find( 'button.mt-cancel-pw' );
	...
	function check_pass_strength() {
		...
		strength = mt.passwordStrength.meter( pass1, mt.passwordStrength.userInputDisallowedList(), pass1 );
	...
	$(document).ready( function() {
		var $colorpicker, $stylesheet, user_id, current_user_id,
			...
			greeting     = $( '#mt-admin-bar-my-account' ).find( '.display-name' );
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// Repaint icons.
				if ( typeof mt !== 'undefined' && mt.svgPainter ) {
					try {
						colors = $.parseJSON( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						mt.svgPainter.setColors( colors );
						mt.svgPainter.paint();
					}
				}
	...
	$( document ).ready( function() {
		if ( $( '.reset-pass-submit' ).length ) {
			$( '.reset-pass-submit button.mt-generate-pw' ).trigger( 'click' );
		}
	});
