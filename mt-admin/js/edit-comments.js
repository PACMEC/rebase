 ...
 * @since 2.1.0
 * @output mt-admin/js/edit-comments.js
 */

/* global adminCommentsSettings, thousandsSeparator, list_args, QTags, ajaxurl, mtAjax */
...
window.setCommentsList = function() {
	 ...
	 * @param {Object} settings Settings for the mtList object.
	 *
	 * @return {void}
	 */
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			mtListsData = $( settings.target ).attr( 'data-mt-lists' );
		...
		if ( mtListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( mtListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = mtListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_mtnonce=' + settings.data._ajax_nonce);
			a.attr('data-mt-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	delAfter = function( r, settings ) {
		var total_items_i18n, total, animated, animatedCallback,
			...
			/*
			 * As `mtList` toggles only the `unapproved` class, the approved comment
		...
		theList.get(0).mtList.add( theExtraList.children( ':eq(0):not(.no-items)' ).remove().clone() );
		...
		animatedCallback = function() {
			if ( ! $( '#the-comment-list tr:visible' ).length ) {
				theList.get(0).mtList.add( theExtraList.find( '.no-items' ).clone() );
			}
		};
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // See MT_Comments_List_Table::prepare_items() in class-mt-comments-list-table.php.
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).mtList.add( response.rows );
			}
		});
	};
	 ...
	 * @global
	 */
	window.theExtraList = $('#the-extra-comment-list').mtList( { alt: '', delColor: 'none', addColor: 'none' } );
	 ...
	 * @global
	 */
	window.theList = $('#the-comment-list').mtList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.on('mtListDelEnd', function(e, s){
			var mtListsData = $(s.target).attr('data-mt-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( mtListsData.indexOf(':trash=1') != -1 || mtListsData.indexOf(':spam=1') != -1 )
...
window.commentReply = {
	...
	show : function(xml) {
		...
		r = mtAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': mtAjax.broken});
			return false;
		}
...
$(document).ready(function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		 ...
		 */
		toggle_all = function() {
			$('#cb-select-all-1').data( 'mt-toggle', 1 ).trigger( 'click' ).removeData( 'mt-toggle' );
		};
