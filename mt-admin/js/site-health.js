/**
 * Interactions used by the Site Health modules in paCMec.
 *
 * @output mt-admin/js/site-health.js
 */

/* global ajaxurl, ClipboardJS, SiteHealth, mt */
...
jQuery( document ).ready( function( $ ) {
	...
	var __ = mt.i18n.__,
		...
		pathsSizesSection = $( '#health-check-accordion-block-mt-paths-sizes' ),
		successTimeout;
	...
	clipboard.on( 'success', function( e ) {
		...
		mt.a11y.speak( __( 'Site information has been copied to your clipboard.' ) );
	} );
	...
	function appendIssue( issue ) {
		var template = mt.template( 'health-check-issue' ),
	...
	function recalculateProgression() {
		...
		if ( 80 <= val && 0 === parseInt( SiteHealth.site_status.issues.critical, 0 ) ) {
			...
			mt.a11y.speak( __( 'All site health tests have finished running. Your site is looking good, and the results are now available on the page.' ) );
		} else {
			$wrapper.addClass( 'orange' ).removeClass( 'green' );

			$progressLabel.text( __( 'Should be improved' ) );
			mt.a11y.speak( __( 'All site health tests have finished running. There are items that should be addressed, and the results are now available on the page.' ) );
		}
		...
		if ( ! isDebugTab ) {
			$.post(
				ajaxurl,
				{
					...
					'_mtnonce': SiteHealth.nonce.site_status_result,
		...
		if ( 1 <= SiteHealth.site_status.async.length ) {
			$.each( SiteHealth.site_status.async, function() {
				var data = {
					...
					'_mtnonce': SiteHealth.nonce.site_status
				};
				...
				if ( 'undefined' !== typeof( this.has_rest ) && this.has_rest ) {
					mt.apiRequest( {
						url: mt.url.addQueryArgs( this.test, { _locale: 'user' } ),
						headers: this.headers
					} )
						.done( function( response ) {
							/** This filter is documented in mt-admin/includes/class-mt-site-health.php */
							appendIssue( mt.hooks.applyFilters( 'site_status_test_result', response ) );
						} )
					...
					$.post(
						...
						/** This filter is documented in mt-admin/includes/class-mt-site-health.php */
						appendIssue( mt.hooks.applyFilters( 'site_status_test_result', response.data ) );
					} ).fail( function( response ) {
						var description;
	...
	function addFailedSiteHealthCheckNotice( url, description ) {
		...
		/** This filter is documented in mt-admin/includes/class-mt-site-health.php */
		appendIssue( mt.hooks.applyFilters( 'site_status_test_result', issue ) );
	}
	...
	function getDirectorySizes() {
		...
		var timeout = window.setTimeout( function() {
			mt.a11y.speak( __( 'Please wait...' ) );
		}, 3000 );

		mt.apiRequest( {
			path: '/mt-site-health/v1/directory-sizes'
		} ).done( function( response ) {
			updateDirSizes( response || {} );
		} ).always( function() {
			var delay = ( new Date().getTime() ) - timestamp;

			$( '.health-check-mt-paths-sizes.spinner' ).css( 'visibility', 'hidden' );
			recalculateProgression();
			...
			if ( delay > 3000 ) {
				...
				window.setTimeout( function() {
					mt.a11y.speak( __( 'All site health tests have finished running.' ) );
				}, delay );
			} else {
