 ...
 *
 * @since 2.7.0
 * @output mt-admin/js/inline-edit-post.js
 */
	...
	setBulk : function(){
		...
		$( 'tbody th.check-column input[type="checkbox"]' ).each( function() {
			...
			if ( $(this).prop('checked') ) {
				...
				theTitle = $('#inline_'+id+' .post_title').html() || mt.i18n.__( '(no title)' );
				te += '<div id="ttle'+id+'"><a id="_'+id+'" class="ntdelbutton" title="'+ mt.i18n.__( 'Remove From Bulk Edit' ) +'">X</a>'+theTitle+'</div>';
			}
		});
		...
		if ( 'post' === type ) {
			$( 'tr.inline-editor textarea[data-mt-taxonomy]' ).each( function ( i, element ) {
				/*
	...
	edit : function(id) {
		...
		$('.tags_input', rowData).each(function(){
			var terms = $(this),
				...
				comma = mt.i18n._x( ',', 'tag delimiter' ).trim();
					...
					if ( -1 !== r.indexOf( '<tr' ) ) {
						...
						$( inlineEditPost.what + id ).hide().fadeIn( 400, function() {
							...
							$( this ).find( '.editinline' )
								...
								.trigger( 'focus' );
							mt.a11y.speak( mt.i18n.__( 'Changes saved.' ) );
						});
					} else {
						r = r.replace( /<.[^<>]*?>/g, '' );
						$errorNotice.removeClass( 'hidden' );
						$error.html( r );
						mt.a11y.speak( $error.text() );
					}
				} else {
					$errorNotice.removeClass( 'hidden' );
					$error.text( mt.i18n.__( 'Error while saving the changes.' ) );
					mt.a11y.speak( mt.i18n.__( 'Error while saving the changes.' ) );
				}
			},
		'html');
...
$( document ).on( 'heartbeat-tick.mt-check-locked-posts', function( e, data ) {
	var locked = data['mt-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('mt-locked') ) {
				...
				row.addClass('mt-locked');
			}
		} else if ( row.hasClass('mt-locked') ) {
			row.removeClass( 'mt-locked' ).find( '.locked-info span' ).empty();
		}
	});
}).on( 'heartbeat-send.mt-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['mt-check-locked-posts'] = check;
	}
}).ready( function() {

	// Set the heartbeat interval to 15 seconds.
	if ( typeof mt !== 'undefined' && mt.heartbeat ) {
