...
/* global ajaxurl, _mtMediaGridSettings, showNotice, findPosts */
...
( function( $ ){
	window.findPosts = {
		...
		send: function() {
			...
			$.ajax( ajaxurl, {
				...
				if ( ! x.success ) {
					$( '#find-posts-response' ).text( mt.i18n.__( 'An error has occurred. Please reload the page and try again.' ) );
				}
	...
	$( document ).ready( function() {
		var settings, $mediaGridWrap = $( '#mt-media-grid' );

		// Opens a manage media frame into the grid.
		if ( $mediaGridWrap.length && window.mt && window.mt.media ) {
			settings = _mtMediaGridSettings;

			var frame = window.mt.media({
				frame: 'manage',
			...
			$mediaGridWrap.trigger( 'mt-media-grid-ready', frame );
		}
