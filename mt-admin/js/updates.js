/**
 * Functions for ajaxified updates, deletions and installs inside the paCMec admin.
 *
 * @version 4.2.0
 * @output mt-admin/js/updates.js
 */
 ...
 * @param {object}  mt                                       MT object.
 * @param {object}  settings                                 MT Updates settings.
...
(function( $, mt, settings ) {
	...
	/**
	 * The MT Updates object.
	 *
	 * @since 4.2.0
	 *
	 * @namespace mt.updates
	 */
	mt.updates = {};
	...
	mt.updates.l10n = window.mt.deprecateL10nObject( 'mt.updates.l10n', mt.updates.l10n, '5.5.0' );
	 ...
	 * @type {string}
	 */
	mt.updates.ajaxNonce = settings.ajax_nonce;
	 ...
	 * @type {bool}
	 */
	mt.updates.shouldRequestFilesystemCredentials = false;
	 ...
	 *                                                             Default 'false'.
	 */
	mt.updates.filesystemCredentials = {
		ftp:       {
			host:           '',
			username:       '',
			password:       '',
	 ...
	 * @type {function}
	 */
	mt.updates.adminNotice = mt.template( 'mt-updates-admin-notice' );
	 ...
	 * @type {jQuery}
	 */
	mt.updates.$elToReturnFocusToFromCredentialsModal = undefined;
	 ...
	 *
	 */
	mt.updates.addAdminNotice = function( data ) {
		var $notice = $( data.selector ),
			$headerEnd = $( '.mt-header-end' ),
			$adminNotice;

		delete data.selector;
		$adminNotice = mt.updates.adminNotice( data );
		...
		$document.trigger( 'mt-updates-notice-added' );
	};

	/**
	 * Handles Ajax requests to paCMec.
	 *
	 * @since 4.6.0
	 *
	 ...
	 */
	mt.updates.ajax = function( action, data ) {
		var options = {};

		if ( mt.updates.ajaxLocked ) {
			mt.updates.queue.push( {
				action: action,
				data:   data
			} );
		...
		options.data = _.extend( data, {
			action:          action,
			_ajax_nonce:     mt.updates.ajaxNonce,
			_fs_nonce:       mt.updates.filesystemCredentials.fsNonce,
			username:        mt.updates.filesystemCredentials.ftp.username,
			password:        mt.updates.filesystemCredentials.ftp.password,
			hostname:        mt.updates.filesystemCredentials.ftp.hostname,
			connection_type: mt.updates.filesystemCredentials.ftp.connectionType,
			public_key:      mt.updates.filesystemCredentials.ssh.publicKey,
			private_key:     mt.updates.filesystemCredentials.ssh.privateKey
		} );

		return mt.ajax.send( options ).always( mt.updates.ajaxAlways );
	};
	 ...
	 */
	mt.updates.ajaxAlways = function( response ) {
		if ( ! response.errorCode || 'unable_to_connect_to_filesystem' !== response.errorCode ) {
			mt.updates.ajaxLocked = false;
			mt.updates.queueChecker();
		}
		...
		if ( 'undefined' !== typeof response.debug && window.console && window.console.log ) {
			_.map( response.debug, function( message ) {
				...
				window.console.log( mt.sanitize.stripTagsAndEncodeText( message ) );
			} );
		}
	};
	 ...
	 * @since 4.7.0
	 */
	mt.updates.refreshCount = function() {
		var $adminBarUpdates              = $( '#mt-admin-bar-updates' ),
	 ...
	 *                      Can be 'plugin', 'theme'.
	 */
	mt.updates.decrementCount = function( type ) {
	 ...
	 * @param {updatePluginSuccess=} args.success Optional. Success callback. Default: mt.updates.updatePluginSuccess
	 * @param {updatePluginError=}   args.error   Optional. Error callback. Default: mt.updates.updatePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	mt.updates.updatePlugin = function( args ) {
		var $updateRow, $card, $message, message,
			$adminBarUpdates = $( '#mt-admin-bar-updates' );

		args = _.extend( {
			success: mt.updates.updatePluginSuccess,
			error: mt.updates.updatePluginError
		}, args );
		...
		$document.trigger( 'mt-plugin-updating', args );

		return mt.updates.ajax( 'update-plugin', args );
	};
	 ...
	 */
	mt.updates.updatePluginSuccess = function( response ) {
		...
		mt.a11y.speak( __( 'Update completed successfully.' ) );

		mt.updates.decrementCount( 'plugin' );

		$document.trigger( 'mt-plugin-update-success', response );
	};
	 ...
	 */
	mt.updates.updatePluginError = function( response ) {
		var $card, $message, errorMessage,
			$adminBarUpdates = $( '#mt-admin-bar-updates' );

		if ( ! mt.updates.isValidResponse( response, 'update' ) ) {
			return;
		}

		if ( mt.updates.maybeHandleCredentialError( response, 'update-plugin' ) ) {
			return;
		}
		...
		mt.a11y.speak( errorMessage, 'assertive' );

		$document.trigger( 'mt-plugin-update-error', response );
	};
	 ...
	 *
	 * @param {Object}                args         Arguments.
	 * @param {string}                args.slug    Plugin identifier in the paCMec.org Plugin repository.
	 * @param {installPluginSuccess=} args.success Optional. Success callback. Default: mt.updates.installPluginSuccess
	 * @param {installPluginError=}   args.error   Optional. Error callback. Default: mt.updates.installPluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	mt.updates.installPlugin = function( args ) {
		var $card    = $( '.plugin-card-' + args.slug ),
			$message = $card.find( '.install-now' );

		args = _.extend( {
			success: mt.updates.installPluginSuccess,
			error: mt.updates.installPluginError
		}, args );
		...
		mt.a11y.speak( __( 'Installing... please wait.' ) );

		// Remove previous error messages, if any.
		$card.removeClass( 'plugin-card-install-failed' ).find( '.notice.notice-error' ).remove();

		$document.trigger( 'mt-plugin-installing', args );

		return mt.updates.ajax( 'install-plugin', args );
	};
	 ...
	 */
	mt.updates.installPluginSuccess = function( response ) {
		...
		mt.a11y.speak( __( 'Installation completed successfully.' ) );

		$document.trigger( 'mt-plugin-install-success', response );
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	mt.updates.installPluginError = function( response ) {
		var $card   = $( '.plugin-card-' + response.slug ),
			$button = $card.find( '.install-now' ),
			errorMessage;

		if ( ! mt.updates.isValidResponse( response, 'install' ) ) {
			return;
		}

		if ( mt.updates.maybeHandleCredentialError( response, 'install-plugin' ) ) {
			return;
		}
		...
		$document.trigger( 'mt-plugin-install-error', response );
	};
	 ...
	 * @param {string} response.activateUrl URL to activate the just installed plugin.
	 */
	mt.updates.installImporterSuccess = function( response ) {
		mt.updates.addAdminNotice( {
			id:        'install-success',
		...
		$document.trigger( 'mt-importer-install-success', response );
	};
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	mt.updates.installImporterError = function( response ) {
		...
		$document.trigger( 'mt-importer-install-error', response );
	};
	 ...
	 * @param {deletePluginSuccess=} args.success Optional. Success callback. Default: mt.updates.deletePluginSuccess
	 * @param {deletePluginError=}   args.error   Optional. Error callback. Default: mt.updates.deletePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	mt.updates.deletePlugin = function( args ) {
		var $link = $( '[data-plugin="' + args.plugin + '"]' ).find( '.row-actions a.delete' );

		args = _.extend( {
			success: mt.updates.deletePluginSuccess,
			error: mt.updates.deletePluginError
		}, args );
		...
		mt.a11y.speak( __( 'Deleting...' ) );

		$document.trigger( 'mt-plugin-deleting', args );

		return mt.updates.ajax( 'delete-plugin', args );
	};
	 ...
	 */
	mt.updates.deletePluginSuccess = function( response ) {
		...
		$( '[data-plugin="' + response.plugin + '"]' ).css( { backgroundColor: '#faafaa' } ).fadeOut( 350, function() {
			var $form            = $( '#bulk-action-form' ),
				...
				pluginDeletedRow = mt.template( 'item-deleted-row' ),
				/**
		...
		mt.a11y.speak( _x( 'Deleted!', 'plugin' ) );

		$document.trigger( 'mt-plugin-delete-success', response );
	};
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	mt.updates.deletePluginError = function( response ) {
		var $plugin, $pluginUpdateRow,
			pluginUpdateRow  = mt.template( 'item-update-row' ),
			noticeContent    = mt.updates.adminNotice( {
		...
		if ( ! mt.updates.isValidResponse( response, 'delete' ) ) {
			return;
		}

		if ( mt.updates.maybeHandleCredentialError( response, 'delete-plugin' ) ) {
			return;
		}
		...
		$document.trigger( 'mt-plugin-delete-error', response );
	};
	 ...
	 * @param {updateThemeSuccess=} args.success Optional. Success callback. Default: mt.updates.updateThemeSuccess
	 * @param {updateThemeError=}   args.error   Optional. Error callback. Default: mt.updates.updateThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	mt.updates.updateTheme = function( args ) {
		var $notice;

		args = _.extend( {
			success: mt.updates.updateThemeSuccess,
			error: mt.updates.updateThemeError
		}, args );
		...
		mt.a11y.speak( __( 'Updating... please wait.' ) );
		$notice.text( __( 'Updating...' ) );

		$document.trigger( 'mt-theme-updating', args );

		return mt.updates.ajax( 'update-theme', args );
	};
	 ...
	 */
	mt.updates.updateThemeSuccess = function( response ) {
		...
		if ( 'customize' === pagenow ) {
			...
			$notice = $( '.theme-info .notice' ).add( mt.customize.control( 'installed_theme_' + response.slug ).container.find( '.theme' ).find( '.update-message' ) );
		...
		mt.updates.addAdminNotice( _.extend( { selector: $notice }, updatedMessage ) );
		mt.a11y.speak( __( 'Update completed successfully.' ) );

		mt.updates.decrementCount( 'theme' );

		$document.trigger( 'mt-theme-update-success', response );

		// Show updated message after modal re-rendered.
		if ( isModalOpen && 'customize' !== pagenow ) {
			$( '.theme-info .theme-author' ).after( mt.updates.adminNotice( updatedMessage ) );
		}
	};
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	mt.updates.updateThemeError = function( response ) {
		...
		if ( mt.updates.maybeHandleCredentialError( response, 'update-theme' ) ) {
			return;
		}

		if ( 'customize' === pagenow ) {
			$theme = mt.customize.control( 'installed_theme_' + response.slug ).container.find( '.theme' );
		}
		...
		mt.a11y.speak( errorMessage );

		$document.trigger( 'mt-theme-update-error', response );
	};
	 ...
	 * @param {Object}               args
	 * @param {string}               args.slug    Theme stylesheet.
	 * @param {installThemeSuccess=} args.success Optional. Success callback. Default: mt.updates.installThemeSuccess
	 * @param {installThemeError=}   args.error   Optional. Error callback. Default: mt.updates.installThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	mt.updates.installTheme = function( args ) {
		var $message = $( '.theme-install[data-slug="' + args.slug + '"]' );

		args = _.extend( {
			success: mt.updates.installThemeSuccess,
			error: mt.updates.installThemeError
		}, args );
		...
		$document.trigger( 'mt-theme-installing', args );

		return mt.updates.ajax( 'install-theme', args );
	};
	 ...
	 */
	mt.updates.installThemeSuccess = function( response ) {
		var $card = $( '.mt-full-overlay-header, [data-slug=' + response.slug + ']' ),
			$message;

		$document.trigger( 'mt-theme-install-success', response );
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	mt.updates.installThemeError = function( response ) {
		var $card, $button,
			errorMessage = sprintf(
				/* translators: %s: Error string for a failed installation. */
				__( 'Installation failed: %s' ),
			...
			$message     = mt.updates.adminNotice( {
				className: 'update-message notice-error notice-alt',
				message:   errorMessage
			} );
		...
		if ( mt.updates.maybeHandleCredentialError( response, 'install-theme' ) ) {
			return;
		}
		...
		if ( 'customize' === pagenow ) {
			...
			mt.customize.notifications.remove( 'theme_installing' );
		} else {
		...
		$document.trigger( 'mt-theme-install-error', response );
	};
	 ...
	 * @param {Object}              args
	 * @param {string}              args.slug    Theme stylesheet.
	 * @param {deleteThemeSuccess=} args.success Optional. Success callback. Default: mt.updates.deleteThemeSuccess
	 * @param {deleteThemeError=}   args.error   Optional. Error callback. Default: mt.updates.deleteThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	mt.updates.deleteTheme = function( args ) {
		var $button;
		...
		args = _.extend( {
			success: mt.updates.deleteThemeSuccess,
			error: mt.updates.deleteThemeError
		}, args );
		...
		$document.trigger( 'mt-theme-deleting', args );

		return mt.updates.ajax( 'delete-theme', args );
	};
	 ...
	 */
	mt.updates.deleteThemeSuccess = function( response ) {
		...
		if ( 'themes-network' === pagenow ) {
			...
			$themeRows.css( { backgroundColor: '#faafaa' } ).fadeOut( 350, function() {
				var $views     = $( '.subsubsub' ),
					...
					deletedRow = mt.template( 'item-deleted-row' );
		...
		mt.a11y.speak( _x( 'Deleted!', 'theme' ) );

		$document.trigger( 'mt-theme-delete-success', response );
	};
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	mt.updates.deleteThemeError = function( response ) {
		var $themeRow    = $( 'tr.inactive[data-slug="' + response.slug + '"]' ),
			$button      = $( '.theme-actions .delete-theme' ),
			updateRow    = mt.template( 'item-update-row' ),
		...
		if ( mt.updates.maybeHandleCredentialError( response, 'delete-theme' ) ) {
			return;
		}
		...
		$document.trigger( 'mt-theme-delete-error', response );
	};
	 ...
	 */
	mt.updates._addCallbacks = function( data, action ) {
		if ( 'import' === pagenow && 'install-plugin' === action ) {
			data.success = mt.updates.installImporterSuccess;
			data.error   = mt.updates.installImporterError;
		}
	 ...
	 */
	mt.updates.queueChecker = function() {
		var job;

		if ( mt.updates.ajaxLocked || ! mt.updates.queue.length ) {
			return;
		}

		job = mt.updates.queue.shift();

		// Handle a queue job.
		switch ( job.action ) {
			case 'install-plugin':
				mt.updates.installPlugin( job.data );
				break;

			case 'update-plugin':
				mt.updates.updatePlugin( job.data );
				break;

			case 'delete-plugin':
				mt.updates.deletePlugin( job.data );
				break;

			case 'install-theme':
				mt.updates.installTheme( job.data );
				break;

			case 'update-theme':
				mt.updates.updateTheme( job.data );
				break;

			case 'delete-theme':
				mt.updates.deleteTheme( job.data );
				break;
	 ...
	 */
	mt.updates.requestFilesystemCredentials = function( event ) {
		if ( false === mt.updates.filesystemCredentials.available ) {
			/*
			 * After exiting the credentials request modal,
			 * return the focus to the element triggering the request.
			 */
			if ( event && ! mt.updates.$elToReturnFocusToFromCredentialsModal ) {
				mt.updates.$elToReturnFocusToFromCredentialsModal = $( event.target );
			}

			mt.updates.ajaxLocked = true;
			mt.updates.requestForCredentialsModalOpen();
		}
	};
	 ...
	 * @param {Event=} event Optional. Event interface.
	 */
	mt.updates.maybeRequestFilesystemCredentials = function( event ) {
		if ( mt.updates.shouldRequestFilesystemCredentials && ! mt.updates.ajaxLocked ) {
			mt.updates.requestFilesystemCredentials( event );
		}
	};
	 ...
	 */
	mt.updates.keydown = function( event ) {
		if ( 27 === event.keyCode ) {
			mt.updates.requestForCredentialsModalCancel();
	 ...
	 * @since 4.2.0
	 */
	mt.updates.requestForCredentialsModalOpen = function() {
		var $modal = $( '#request-filesystem-credentials-dialog' );

		$( 'body' ).addClass( 'modal-open' );
		$modal.show();
		$modal.find( 'input:enabled:first' ).focus();
		$modal.on( 'keydown', mt.updates.keydown );
	};
	 ...
	 * @since 4.2.0
	 */
	mt.updates.requestForCredentialsModalClose = function() {
		$( '#request-filesystem-credentials-dialog' ).hide();
		$( 'body' ).removeClass( 'modal-open' );

		if ( mt.updates.$elToReturnFocusToFromCredentialsModal ) {
			mt.updates.$elToReturnFocusToFromCredentialsModal.focus();
		}
	};
	 ...
	 */
	mt.updates.requestForCredentialsModalCancel = function() {
		...
		_.each( mt.updates.queue, function( job ) {
		...
		mt.updates.requestForCredentialsModalClose();
	};
	 ...
	 */
	mt.updates.showErrorInCredentialsForm = function( message ) {
	 ...
	 * @param {string} action   The type of request to perform.
	 */
	mt.updates.credentialError = function( response, action ) {

		// Restore callbacks.
		response = mt.updates._addCallbacks( response, action );

		mt.updates.queue.unshift( {
			action: action,
		...
		mt.updates.filesystemCredentials.available = false;
		mt.updates.showErrorInCredentialsForm( response.errorMessage );
		mt.updates.requestFilesystemCredentials();
	};
	 ...
	 */
	mt.updates.maybeHandleCredentialError = function( response, action ) {
		if ( mt.updates.shouldRequestFilesystemCredentials && response.errorCode && 'unable_to_connect_to_filesystem' === response.errorCode ) {
			mt.updates.credentialError( response, action );
			return true;
		}
	 ...
	 *                                                'update' or 'install'.
	 */
	mt.updates.isValidResponse = function( response, action ) {
	 ...
	 * @since 4.2.0
	 */
	mt.updates.beforeunload = function() {
		if ( mt.updates.ajaxLocked ) {
	...
	$( function() {
		var $pluginFilter        = $( '#plugin-filter' ),
			...
			$pluginSearch        = $( '.plugins-php .mt-filter-search' ),
			$pluginInstallSearch = $( '.plugin-install-php .mt-filter-search' );

		settings = _.extend( settings, window._mtUpdatesItemCounts || {} );

		if ( settings.totals ) {
			mt.updates.refreshCount();
		}
		 ...
		 *
		 * @see {mt_print_request_filesystem_credentials_modal() in PHP}
		 */
		mt.updates.shouldRequestFilesystemCredentials = $filesystemModal.length > 0;
		...
		$filesystemModal.on( 'submit', 'form', function( event ) {
			...
			mt.updates.filesystemCredentials.ftp.hostname       = $( '#hostname' ).val();
			mt.updates.filesystemCredentials.ftp.username       = $( '#username' ).val();
			mt.updates.filesystemCredentials.ftp.password       = $( '#password' ).val();
			mt.updates.filesystemCredentials.ftp.connectionType = $( 'input[name="connection_type"]:checked' ).val();
			mt.updates.filesystemCredentials.ssh.publicKey      = $( '#public_key' ).val();
			mt.updates.filesystemCredentials.ssh.privateKey     = $( '#private_key' ).val();
			mt.updates.filesystemCredentials.fsNonce            = $( '#_fs_nonce' ).val();
			mt.updates.filesystemCredentials.available          = true;
		 ...
		 * @since 4.2.0
		 */
		$filesystemModal.on( 'click', '[data-js-action="close"], .notification-dialog-background', mt.updates.requestForCredentialsModalCancel );
		...
		$document.on( 'credential-modal-cancel', function( event, job ) {
			...
			mt.a11y.speak( __( 'Update canceled.' ) );
		} );
		...
		$bulkActionForm.on( 'click', '[data-plugin] .update-link', function( event ) {
			...
			mt.updates.$elToReturnFocusToFromCredentialsModal = $pluginRow.find( '.check-column input' );
			mt.updates.updatePlugin( {
		...
		$document.on( 'click', '.importer-item .install-now', function( event ) {
			...
			mt.updates.installPlugin( {
				slug:    $button.data( 'slug' ),
				pagenow: pagenow,
				success: mt.updates.installImporterSuccess,
				error:   mt.updates.installImporterError
			} );
		} );
		...
		$document.on( 'click', '.themes-php.network-admin .update-link', function( event ) {
			...
			mt.updates.$elToReturnFocusToFromCredentialsModal = $themeRow.find( '.check-column input' );
		...
		$bulkActionForm.on( 'click', '[type="submit"]:not([name="clear-recent-list"])', function( event ) {
			...
			if ( ! itemsSelected.length ) {
				...
				return mt.updates.addAdminNotice( {
			...
			$document.trigger( 'mt-' + type + '-bulk-' + bulkAction, itemsSelected );
			...
			$document.on( 'mt-plugin-update-success mt-plugin-update-error mt-theme-update-success mt-theme-update-error', function( event, response ) {
				var $itemRow = $( '[data-slug="' + response.slug + '"]' ),
					$bulkActionNotice, itemName;

				if ( 'mt-' + response.update + '-update-success' === event.type ) {
					success++;
				} else {
				...
				mt.updates.adminNotice = mt.template( 'mt-bulk-updates-admin-notice' );
				...
				if ( error > 0 && ! mt.updates.queue.length ) {
					$( 'html, body' ).animate( { scrollTop: 0 } );
				}
			} );

			// Reset admin notice template after #bulk-action-notice was added.
			$document.on( 'mt-updates-notice-added', function() {
				mt.updates.adminNotice = mt.template( 'mt-updates-admin-notice' );
			} );
		...
		$pluginInstallSearch.on( 'keyup input', _.debounce( function( event, eventtype ) {
			...
			if ( mt.updates.searchTerm === data.s && 'typechange' !== eventtype ) {
				return;
			} else {
				$pluginFilter.empty();
				mt.updates.searchTerm = data.s;
			}
			...
			if ( ! $searchTab.length ) {
				...
				$( '.mt-filter .filter-links .current' )
					.removeClass( 'current' )
			...
			if ( 'undefined' !== typeof mt.updates.searchRequest ) {
				mt.updates.searchRequest.abort();
			}
			$( 'body' ).addClass( 'loading-content' );

			mt.updates.searchRequest = mt.ajax.post( 'search-install-plugins', data ).done( function( response ) {
				$( 'body' ).removeClass( 'loading-content' );
				$pluginFilter.append( response.items );
				delete mt.updates.searchRequest;

				if ( 0 === response.count ) {
					mt.a11y.speak( __( 'You do not appear to have any plugins available at this time.' ) );
				} else {
					mt.a11y.speak(
						sprintf(
		...
		$pluginSearch.on( 'keyup input', _.debounce( function( event ) {
			...
			if ( mt.updates.searchTerm === data.s ) {
				return;
			} else {
				mt.updates.searchTerm = data.s;
			}
			...
			mt.updates.searchRequest = mt.ajax.post( 'search-plugins', data ).done( function( response ) {
				...
				if ( 0 === response.count ) {
					mt.a11y.speak( __( 'No plugins found. Try a different search.'  ) );
				} else {
					mt.a11y.speak(
						sprintf(
							/* translators: %s: Number of plugins. */
		...
		$document.on( 'submit', '.search-plugins', function( event ) {
			...
			$( 'input.mt-filter-search' ).trigger( 'input' );
		} );
		...
		$( window ).on( 'message', function( event ) {
			...
			switch ( message.action ) {
				...
				// Called from `mt-admin/includes/class-mt-upgrader-skins.php`.
				case 'decrementUpdateCount':
					/** @property {string} message.upgradeType */
					mt.updates.decrementCount( message.upgradeType );
					break;
				...
				case 'update-plugin':
					...
					message.data = mt.updates._addCallbacks( message.data, message.action );

					mt.updates.queue.push( message );
					mt.updates.queueChecker();
					break;
			}
		} );
		 ...
		 * @since 4.2.0
		 */
		$( window ).on( 'beforeunload', mt.updates.beforeunload );
		...
		$document.on( 'click keyup', '.column-auto-updates .toggle-auto-update, .theme-overlay .toggle-auto-update', function( event ) {
			var data, asset, type, $parent,
				$toggler = $( this ),
				action = $toggler.attr( 'data-mt-action' ),
			...
			$.post( window.ajaxurl, data )
				.done( function( response ) {
					...
					if ( ! response.success ) {
						// if MT returns 0 for response (which can happen in a few cases),
					...
					if ( 'enable' === action ) {
						...
						$toggler.attr( 'data-mt-action', 'enable' );

						$label.text( __( 'Enable auto-updates' ) );
						$parent.find( '.auto-update-time' ).addClass( 'hidden' );
						mt.a11y.speak( __( 'Auto-updates disabled' ) );
					}

					$document.trigger( 'mt-auto-update-setting-changed', { state: action, type: type, asset: asset } );
				} )
				.fail( function() {
					...
					mt.a11y.speak( __( 'The request could not be completed.' ), 'assertive' );
				} )
				.always( function() {
...
})( jQuery, window.mt, window._mtUpdatesSettings );
