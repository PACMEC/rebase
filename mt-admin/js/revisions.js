 ...
 *
 * @output mt-admin/js/revisions.js
 */
...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.mt.revisions.
	 */
	revisions = mt.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back end.
	revisions.settings = window._mtRevisionsSettings || {};
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			mt.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				mt.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = mt.ajax.send( options ),
	...
	/**
	 * mt.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see mt.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = mt.Backbone.View.extend({
		className: 'revisions',
		template: mt.template('revisions-frame'),
		...
		render: function() {
			mt.Backbone.View.prototype.render.apply( this, arguments );

			$('html').css( 'overflow-y', 'scroll' );
			$('#mtbody-content .wrap').append( this.el );
	...
	revisions.view.Controls = mt.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.mt.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.mt.revisions.pinning click.mt.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.mt.revisions.pinning');
	...
	// The tickmarks view.
	revisions.view.Tickmarks = mt.Backbone.View.extend({
	...
	// The metabox view.
	revisions.view.Metabox = mt.Backbone.View.extend({
	...
	revisions.view.Meta = mt.Backbone.View.extend({
		template: mt.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = mt.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: mt.template('revisions-checkbox'),
	...
	revisions.view.Tooltip = mt.Backbone.View.extend({
	...
	revisions.view.Buttons = mt.Backbone.View.extend({
		className: 'revisions-buttons',
		template: mt.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = mt.Backbone.View.extend({
		className: 'mt-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.mt.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.mt.revisions');
	...
	revisions.view.Diff = mt.Backbone.View.extend({
		className: 'revisions-diff',
		template:  mt.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// mt_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// mt_localize_script does not allow for top-level booleans so do a comparator here.
