/**
 * @output mt-admin/js/auth-app.js
 */
...
( function( $, authApp ) {
	...
	$approveBtn.click( function( e ) {
		 ...
		 */
		request = mt.hooks.applyFilters( 'mt_application_passwords_approve_app_request', request, context );

		mt.apiRequest( {
			path: '/mt/v2/users/me/application-passwords?_locale=user',
			method: 'POST',
			data: request
			 ...
			 *
			 * In most cases, this should be used in combination with the {@see 'mt_authorize_application_password_form_approved_no_js'}
			 ...
			 */
			mt.hooks.doAction( 'mt_application_passwords_approve_app_request_success', response, textStatus, jqXHR );
			...
			if ( raw ) {
				...
				message = mt.i18n.sprintf(
					/* translators: %s: Application name. */
					'<label for="new-application-password-value">' + mt.i18n.__( 'Your new password for %s is:' ) + '</label>',
					'<strong></strong>'
				...
				$notice = $( '<div></div>' )
					...
					.append( '<p>' + mt.i18n.__( 'Be sure to save this in a safe location. You will not be able to retrieve it.' ) + '</p>' );
			 ...
			 * @param {jqXHR}       jqXHR       The underlying jqXHR object that made the request.
			 */
			mt.hooks.doAction( 'mt_application_passwords_approve_app_request_error', error, textStatus, errorThrown, jqXHR );
		} );
	} );
	...
	$rejectBtn.click( function( e ) {
		 ...
		 * @param {string} context.rejectUrl  The URL the user will be redirected to after rejecting the request.
		 */
		mt.hooks.doAction( 'mt_application_passwords_reject_app', context );
