/**
 * @output mt-admin/js/dashboard.js
 */

/* global pagenow, ajaxurl, postboxes, mtActiveEditor:true, ajaxWidgets */
...
jQuery(document).ready( function($) {
	var welcomePanel = $( '#welcome-panel' ),
		welcomePanelHide = $('#mt_welcome_panel-hide'),
		updateWelcomePanel;
	...
	$('.welcome-panel-close, .welcome-panel-dismiss a', welcomePanel).on( 'click', function(e) {
		...
		$('#mt_welcome_panel-hide').prop('checked', false);
	});
	...
	$( '.meta-box-sortables' ).sortable( 'option', 'containment', '#mtwrap' );
...
jQuery( function( $ ) {
	...
	var communityEventsData = window.communityEventsData,
		dateI18n = mt.date.dateI18n,
		format = mt.date.format,
	 ...
	 *
	 * @memberOf mt
	 * @namespace mt.communityEvents
	 */
	app = window.mt.communityEvents = /** @lends mt.communityEvents */{
		initialized: false,
		model: null,

		/**
		 * Initializes the mt.communityEvents object.
		 *
		 * @since 4.8.0
		 *
		 * @return {void}
		...
		getEvents: function( requestParams ) {
			...
			requestParams._mtnonce = communityEventsData.nonce;
			...
			mt.ajax.post( 'get-community-events', requestParams )
				.always( function() {
		 ...
		 * @since 4.8.0
		 *
		 * @param {Object} templateParams The various parameters that will get passed to mt.template.
		...
		renderEventsTemplate: function( templateParams, initiatedBy ) {
			...
			if ( templateParams.location.ip ) {
				...
				if ( templateParams.events.length ) {
					template = mt.template( 'community-events-event-list' );
					$results.html( template( templateParams ) );
				} else {
					template = mt.template( 'community-events-no-upcoming-events' );
					$results.html( template( templateParams ) );
				}
				...
				template = mt.template( 'community-events-attend-event-near' );
				...
				template = mt.template( 'community-events-could-not-locate' );
				 ...
				 */
				mt.a11y.speak( __( 'An error occurred. Please try again.' ) );
		 ...
		 * @param {string} timeFormat A time format acceptable by `mt.date.dateI18n()`.
		 *
		 * @returns {Array}
		 */
		...
		/**
		 * Returns the user's local/browser time zone, in a form suitable for `mt.date.i18n()`.
		 *
		 * @since 5.5.2
		 *
		 ...
		 * @param {string} timeZone    A time zone string or offset which is parsable by `mt.date.i18n()`.
		 *
		 * @returns {string}
		 */
...
window.communityEventsData.l10n = window.mt.deprecateL10nObject( 'communityEventsData.l10n', window.communityEventsData.l10n, '5.6.0' );
