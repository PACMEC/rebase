...
( function( $ ) {
		...
		get : function(total, num) {
			...
			$.post(
				...
				function(r) {
					...
					$('#the-comment-list').append('<tr><td colspan="2">'+mtAjax.broken+'</td></tr>');
				}
			);
	 ...
	 * @global
	 */
	window.MTSetThumbnailHTML = function(html){
	 ...
	 * @global
	 */
	window.MTSetThumbnailID = function(id){
	 ...
	 * @global
	 */
	window.MTRemoveThumbnail = function(nonce){
		$.post(ajaxurl, {
	...
	$(document).on( 'heartbeat-send.refresh-lock', function( e, data ) {
		...
		data['mt-refresh-post-lock'] = send;
		...
		if ( data['mt-refresh-post-lock'] ) {
			...
			if ( received.lock_error ) {
				...
				if ( wrap.length && ! wrap.is(':visible') ) {
					...
					wrap.find('.mt-tab-first').trigger( 'focus' );
				}
...
(function($) {
	...
	$(document).on( 'heartbeat-send.mt-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#mt-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_mtnonce').val() ) {
				data['mt-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.mt-refresh-nonces', function( e, data ) {
		var nonces = data['mt-refresh-post-nonces'];
...
jQuery(document).ready( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('mt-tab-first') && e.shiftKey ) {
			$(this).find('.mt-tab-last').trigger( 'focus' );
			e.preventDefault();
		// [Tab] on last tab cycles back to first tab.
		} else if ( target.hasClass('mt-tab-last') && ! e.shiftKey ) {
			$(this).find('.mt-tab-first').trigger( 'focus' );
			e.preventDefault();
		}
	}).filter(':visible').find('.mt-tab-first').trigger( 'focus' );

	// Set the heartbeat interval to 15 seconds if post lock dialogs are enabled.
	if ( mt.heartbeat && $('#post-lock-dialog').length ) {
		mt.heartbeat.interval( 15 );
	}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#mt-preview'),
			target = $this.attr('target') || 'mt-preview',
		...
		if ( mt.autosave ) {
			mt.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! mt.heartbeat || ! mt.heartbeat.hasConnectionError() ) {
	...
	$( window ).on( 'beforeunload.edit-post', function( event ) {
		...
		if ( mt.autosave ) {
			changed = mt.autosave.server.postChanged();
		} else if ( editor ) {
		...
		var data = {
			action: 'mt-remove-post-lock',
			_mtnonce: $('#_mtnonce').val(),
			post_ID: postID,
	...
	$('.categorydiv').each( function(){
		...
		// @todo Move to jQuery 1.3+, support for multiple hierarchical taxonomies, see mt-lists.js.
		...
		$('#' + taxonomy + '-add-toggle').on( 'click', function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'mt-hidden-children' );
	...
	if ( $('#submitdiv').length ) {
		...
		updateText = function() {
			...
			$('#post-status-display').text(
				...
				mt.sanitize.stripTagsAndEncodeText( $('option:selected', postStatus).text() )
			);
		...
		buttons.children( '.save' ).on( 'click', function() {
			...
			$.post(
				...
				function(data) {
					...
					mt.a11y.speak( __( 'Permalink saved' ) );
				}
			);
		});
	 ...
	 * @return {void}
	 */
	window.mttitlehint = function( id ) {
		id = id || 'title';
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'mt-editor-expand' ) ) {
				return;
			}
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#mt-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.mt-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.mt-editor-resize', dragging )
				.on( 'mouseup.mt-editor-resize mouseleave.mt-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.mt-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.mt-autosave', function( event ) {
		// Key [S] has code 83.
		if ( event.which === 83 ) {
			...
			mt.autosave && mt.autosave.server.triggerSave();
			event.preventDefault();
		}
	});
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'mt-post-new-reload=true';
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#mt-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
...
} )( jQuery, new mt.utils.WordCounter() );
