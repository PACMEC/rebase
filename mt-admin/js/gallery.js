/**
 * @output mt-admin/js/gallery.js
 */

/* global unescape, getUserSetting, setUserSetting, mtgallery, tinymce */
...
jQuery(window).unload( function () { window.tinymce = window.tinyMCE = window.mtgallery = null; } ); // Cleanup.
...
window.mtgallery = {
	...
	setup : function() {
		...
		if ( t.el.nodeName !== 'IMG' || ! ed.dom.hasClass(t.el, 'mtGallery') ) {
			if ( ( g = ed.dom.select('img.mtGallery') ) && g[0] ) {
				t.el = g[0];
			} else {
