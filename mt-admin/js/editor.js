...
( function( $, mt ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'mt-switch-editor' ) ) {
						id = target.attr( 'data-mt-editor-id' );
		...
		function switchEditor( id, mode ) {
			...
			if ( 'tmce' === mode || 'tinymce' === mode ) {
				...
				if ( editor ) {
					keepSelection = editor.getParam( 'mt_keep_scroll_position' );
				} else {
					keepSelection = window.tinyMCEPreInit.mceInit[ id ] &&
									window.tinyMCEPreInit.mceInit[ id ].mt_keep_scroll_position;
				}
				...
				if ( editor ) {
					...
					if ( editor.getParam( 'mt_keep_scroll_position' ) ) {
		...
		function getShortCodePositionsInText( content ) {
			...
			var shortcodeDetailsRegexp = mt.shortcode.regexp( allShortcodes.join( '|' ) ),
		 ...
		 * from the window height, to get the proper viemtort window, that the user sees.
		 *
		 * @param {Object} editor TinyMCE editor instance.
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#mt-content-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#mt-content-editor-tools` and
			 * subtracting the height. This gives the scroll position where the top of the editor tools aligns with
			 * the top of the viemtort (under the Master Bar)
			 */
			var adjustedScroll;
			if ( editor.settings.mt_autoresize_on ) {
		...
		/**
		 * Replaces <p> tags with two line breaks. "Opposite" of mtautop().
		 *
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<mt-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<mt-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				html = html.replace( /<mt-temp-br([^>]*)>/g, '<br$1>' );
			}

			// Restore preserved tags.
			if ( preserve.length ) {
				html = html.replace( /<mt-preserve>/g, function() {
		 ...
		 *
		 * Similar to `mtautop()` in formatting.php.
		 *
		 * @since 2.5.0
		 *
		 * @memberof switchEditors
		...
		function autop( text ) {
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<mt-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<mt-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		mt.editor.autop = mtautop;
		mt.editor.removep = pre_mtautop;

		exports = {
			go: switchEditor,
			mtautop: mtautop,
			pre_mtautop: pre_mtautop,
			_mt_Autop: autop,
			_mt_Nop: removep
		};
	...
	/**
	 * Initialize TinyMCE and/or Quicktags. For use with mt_enqueue_editor() (PHP).
	 *
	 * Intended for use with an existing textarea that will become the Text editor tab.
	 * The editor width will be the width of the textarea container, height will be adjustable.
	 *
	 * Settings for both TinyMCE and Quicktags can be passed on initialization, and are "filtered"
	 * with custom jQuery events on the document element, mt-before-tinymce-init and mt-before-quicktags-init.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * }
	 */
	mt.editor.initialize = function( id, settings ) {
		var init;
		var defaults;

		if ( ! $ || ! id || ! mt.editor.getDefaultSettings ) {
			return;
		}

		defaults = mt.editor.getDefaultSettings();
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'mt-core-ui mt-editor-wrap tmce-active',
					id: 'mt-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="mt-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-mt-editor-id': id
				} );

			var $editorTools = $( '<div class="mt-editor-tools">' );

			if ( settings.mediaButtons ) {
				var buttonText = 'Add Media';

				if ( window._mtMediaViewsL10n && window._mtMediaViewsL10n.addMedia ) {
					buttonText = window._mtMediaViewsL10n.addMedia;
				}

				var $addMediaButton = $( '<button type="button" class="button insert-media add_media">' );

				$addMediaButton.append( '<span class="mt-media-buttons-icon"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="mt-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="mt-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'mt-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'mt-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'mt-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'mt-before-quicktags-init', init );
			window.quicktags( init );

			if ( ! window.mtActiveEditor ) {
				window.mtActiveEditor = init.id;
			}
		}
	};
	 ...
	 *
	 * Intended for use with editors that were initialized with mt.editor.initialize().
	 *
	 * @since 4.8.0
	 *
	 * @param {string} id The HTML id of the editor textarea.
	 */
	mt.editor.remove = function( id ) {
	 ...
	 */
	mt.editor.getContent = function( id ) {
		var editor;
