/**
 * @output mt-admin/js/theme.js
 */

/* global _mtThemeSettings, confirm, tb_position */
window.mt = window.mt || {};
...
// Setup app structure.
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: mt.template });
...
themes.view.Appearance = mt.Backbone.View.extend({

	el: '#mtbody-content .wrap .theme-browser',
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="mt-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el )
...
themes.Collection = Backbone.Collection.extend({
	...
	// Send request to api.managertechnology.com.co/pacmec/themes.
	apiCall: function( request, paginated ) {
		return mt.ajax.send( 'query-themes', {
			data: {
				// Request data.
				request: _.extend({
					per_page: 100
...
themes.view.Theme = mt.Backbone.View.extend({
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'mt-theme-install-success', function( event, response ) {
			if ( _this.model.get( 'id' ) === response.slug ) {
				_this.model.set( { 'installed': true } );
			}
		} );

		mt.updates.installTheme( {
	...
	updateTheme: function( event ) {
		...
		$( document ).on( 'mt-theme-update-success', function( event, response ) {
...
themes.view.Details = mt.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.mt-themes', function( event ) {
	...
	autoupdateState: function() {
		...
		callback = function( event, data ) {
			...
			if ( _this.model.get( 'id' ) === data.asset ) {
				...
				$( document ).off( 'mt-auto-update-setting-changed', callback );
			}
		};

		// Triggered in updates.js
		$( document ).on( 'mt-auto-update-setting-changed', callback );
	},
	...
	deleteTheme: function( event ) {
		...
		if ( ! window.confirm( mt.themes.data.settings.confirmDelete ) ) {
			return;
		}

		mt.updates.maybeRequestFilesystemCredentials( event );

		$( document ).one( 'mt-theme-delete-success', function( event, response ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'mt-full-overlay expanded',
	...
	tooglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.mt-full-overlay-footer .devices' );
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'mt-theme-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );
...
themes.view.Themes = mt.Backbone.View.extend({

	className: 'themes mt-clearfix',
		...
		if ( this.model.get( 'hasUpdate' ) ) {
			...
			if ( $card.find( '.updating-message' ).length ) {
				...
				$modal.find( '.notice-warning' )
					...
					.find( 'p' ).text( mt.updates.l10n.updating );
	...
	announceSearchResults: function( count ) {
		if ( 0 === count ) {
			mt.a11y.speak( l10n.noThemesFound );
		} else {
			mt.a11y.speak( l10n.themesFound.replace( '%d', count ) );
		}
	}
});

// Search input view controller.
themes.view.Search = mt.Backbone.View.extend({

	tagName: 'input',
	className: 'mt-filter-search',
	id: 'mt-filter-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.mt-filter-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.mt-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: function( value ) {
		...
		// Get the themes by sending Ajax POST request to api.managertechnology.com.co/pacmec/themes
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	events: {
		...
		'keyup #mtorg-username-input': 'saveUsername'
	},
	...
	applyFilters: function( event ) {
		...
		if ( ! tags ) {
			mt.a11y.speak( l10n.selectFeatureFilter );
			return;
		}
	...
	// Save the user's paCMec.org username and get his favorite themes.
	saveUsername: function ( event ) {
		var username = $( '#mtorg-username-input' ).val(),
			nonce = $( '#mtorg-username-nonce' ).val(),
		...
		return mt.ajax.send( 'save-mtorg-username', {
			data: {
				_mtnonce: nonce,
				username: username
			},
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `mt.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.mt-filter-search' ).trigger( 'focus' ).trigger( 'keyup' );
		});
...
$( document ).ready(function() {
	...
	$( document.body ).on( 'click', '.load-customize', function() {
		...
		urlParser.search = $.param( _.extend(
			mt.customize.utils.parseQueryString( urlParser.search.substr( 1 ) ),
			{
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _mtThemeSettings.settings.confirmDelete );
	});
});
...
jQuery(document).ready( function($) {
	window.tb_position = function() {
		...
		if ( $('#mtadminbar').length ) {
			adminbar_height = parseInt( $('#mtadminbar').css('height'), 10 );
		}
