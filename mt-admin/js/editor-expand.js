/**
 * @output mt-admin/js/editor-expand.js
 */
...
( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#mt-content-wrap' ),
			$tools = $( '#mt-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.mt.editor && window.mt.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			// Set the minimum height to the initial viemtort height.
			...
			function mceGetCursorOffset() {
				...
				/*
				 * If editor.mt.getView and the selection node from the editor selection
				 * are defined, use this as a view for the offset.
				 */
				if ( editor.mt && editor.mt.getView && ( view = editor.mt.getView( node ) ) ) {
			 ...
			 * the window when the cursor moves out of the viemtort to a mtview.
			 * Setting a buffer > 0 will prevent the browser default.
			 * Some browsers will scroll to the middle,
			 * others to the top/bottom of the *window* when moving the cursor out of the viemtort.
			 *
			 * @since 4.1.0
			 *
			 ...
			 * Executes the mtAutoResize on the active editor.
			 *
			 * @since 4.0.0
			 *
			 * @return {void}
			...
			function mceShow() {
				...
				setTimeout( function() {
					editor.execCommand( 'mtAutoResize' );
					adjust();
				}, 300 );
			}
			...
			mceBind = function() {
				...
				editor.on( 'mt-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent mt-autoresize mt-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'mt-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent mt-autoresize mt-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'mt-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .mt-editor-container.
			...
			if ( $postboxContainer.width() < 300 && heights.windowWidth > 600 &&
				...
				// Check if the editor is taller than the viemtort.
				...
				if ( ( heights.sideSortablesHeight + pinnedToolsTop + sidebarBottom ) > heights.windowHeight || fixedSideTop || fixedSideBottom ) {
					...
					// If the sidebar container is smaller than the viemtort, then pin/unpin the top when scrolling.
		...
		function on() {
			...
			$wrap.addClass( 'mt-editor-expand' );
			 ...
			 */
			$document.on( 'mt-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'mt-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
						mceEditor.execCommand( 'mtAutoResize' );
					} else {
						textEditorResize();
					}
				});
			...
			if ( mceEditor ) {
				mceEditor.settings.mt_autoresize_on = true;
				mceEditor.execCommand( 'mtAutoResizeOn' );
		...
		function off() {
			...
			$wrap.removeClass( 'mt-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.mt_autoresize_on = false;
				mceEditor.execCommand( 'mtAutoResizeOff' );
	...
	$( function() {
		...
		$document.on( 'tinymce-editor-setup.focus', function( event, editor ) {
			...
			editor.addCommand( 'mtToggleDFW', toggle );
			editor.addShortcut( 'access+w', '', 'mtToggleDFW' );
		} );
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'mt-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'mt-autoresize', recalcEditorRect );
				};
