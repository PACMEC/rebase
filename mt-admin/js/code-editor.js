/**
 * @output mt-admin/js/code-editor.js
 */

if ( 'undefined' === typeof window.mt ) {
	/**
	 * @namespace mt
	 */
	window.mt = {};
}
if ( 'undefined' === typeof window.mt.codeEditor ) {
	/**
	 * @namespace mt.codeEditor
	 */
	window.mt.codeEditor = {};
}
...
( function( $, mt ) {
	...
	/**
	 * @typedef {object} mt.codeEditor~CodeEditorInstance
	 ...
	 */
	mt.codeEditor.initialize = function initialize( textarea, settings ) {
		...
		instanceSettings = $.extend( {}, mt.codeEditor.defaultSettings, settings );
		instanceSettings.codemirror = $.extend( {}, instanceSettings.codemirror );

		codemirror = mt.CodeMirror.fromTextArea( $textarea[0], instanceSettings.codemirror );
		...
		if ( codemirror.showHint ) {
			codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
				...
				innerMode = mt.CodeMirror.innerMode( codemirror.getMode(), token.state ).mode.name;
