/**
 * @output mt-admin/js/customize-widgets.js
 */

/* global _mtCustomizeWidgetsSettings */
(function( mt, $ ){
	...
	/**
	 * @namespace mt.customize.Widgets
	 */
	api.Widgets = api.Widgets || {};
	api.Widgets.savedWidgetIds = {};

	// Link settings.
	api.Widgets.data = _mtCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;

	/**
	 * mt.customize.Widgets.WidgetModel
	 *
	 * A single widget model.
	 *
	 * @class    mt.customize.Widgets.WidgetModel
	 * @augments Backbone.Model
	 */
	api.Widgets.WidgetModel = Backbone.Model.extend(/** @lends mt.customize.Widgets.WidgetModel.prototype */{
		id: null,
		temp_id: null,
		classname: null,
		control_tpl: null,
	...
	/**
	 * mt.customize.Widgets.WidgetCollection
	 *
	 * Collection for widget models.
	 *
	 * @class    mt.customize.Widgets.WidgetCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.WidgetCollection = Backbone.Collection.extend(/** @lends mt.customize.Widgets.WidgetCollection.prototype */{
	...
	/**
	 * mt.customize.Widgets.SidebarModel
	 *
	 * A single sidebar model.
	 *
	 * @class    mt.customize.Widgets.SidebarModel
	 * @augments Backbone.Model
	 */
	api.Widgets.SidebarModel = Backbone.Model.extend(/** @lends mt.customize.Widgets.SidebarModel.prototype */{
		after_title: null,
		after_widget: null,
		before_title: null,
		before_widget: null,
	...
	/**
	 * mt.customize.Widgets.SidebarCollection
	 *
	 * Collection for sidebar models.
	 *
	 * @class    mt.customize.Widgets.SidebarCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.SidebarCollection = Backbone.Collection.extend(/** @lends mt.customize.Widgets.SidebarCollection.prototype */{
	...
	api.Widgets.AvailableWidgetsPanelView = mt.Backbone.View.extend(/** @lends mt.customize.Widgets.AvailableWidgetsPanelView.prototype */{
		 ...
		 *
		 * @constructs mt.customize.Widgets.AvailableWidgetsPanelView
	 ...
	 *
	 * @alias    mt.customize.Widgets.formSyncHandlers
	 */
	...
	api.Widgets.WidgetControl = api.Control.extend(/** @lends mt.customize.Widgets.WidgetControl.prototype */{
		...
		/**
		 * mt.customize.Widgets.WidgetControl
		 *
		 * Customizer control for widgets.
		 * Note that 'widget_form' must match the MT_Widget_Form_Customize_Control::$type
		 *
		 * @since 4.1.0
		 *
		 * @constructs mt.customize.Widgets.WidgetControl
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.mt-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						mt.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						mt.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // mt_widget_control() outputs the button as "Delete".
		...
		/**
		 * @return {mt.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			jqxhr = $.post( mt.ajax.settings.url, data );
	...
	/**
	 * mt.customize.Widgets.WidgetsPanel
	 *
	 * Customizer panel containing the widget area sections.
	 *
	 * @since 4.4.0
	 *
	 * @class    mt.customize.Widgets.WidgetsPanel
	 * @augments mt.customize.Panel
	 */
	api.Widgets.WidgetsPanel = api.Panel.extend(/** @lends mt.customize.Widgets.WigetsPanel.prototype */{
		...
		ready: function () {
			...
			panel.deferred.embedded.done(function() {
				...
				// @todo This should use the Notifications API introduced to panels. See <https://core.trac.managertechnology.com.co/pacmec/ticket/38794>.
	...
	/**
	 * mt.customize.Widgets.SidebarSection
	 *
	 * Customizer section representing a widget area widget
	 *
	 * @since 4.1.0
	 *
	 * @class    mt.customize.Widgets.SidebarSection
	 * @augments mt.customize.Section
	 */
	api.Widgets.SidebarSection = api.Section.extend(/** @lends mt.customize.Widgets.SidebarSection.prototype */{
	...
	/**
	 * mt.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the MT_Widget_Area_Customize_Control::$type
	 *
	 * @since 3.9.0
	 *
	 * @class    mt.customize.Widgets.SidebarControl
	 * @augments mt.customize.Control
	 */
	api.Widgets.SidebarControl = api.Control.extend(/** @lends mt.customize.Widgets.SidebarControl.prototype */{
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[mt_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[mt_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[mt_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( showOrHide ) {
				...
				mt.a11y.speak( l10n.reorderModeOn );
				...
				mt.a11y.speak( l10n.reorderModeOff );
		 ...
		 *
		 * @since 3.9.0
		 * @return {mt.customize.controlConstructor.widget_form[]}
		 */
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo This can use widgetIdToSettingId(), then pass into mt.customize.control( x ).getSidebarWidgetsControl().
	 ...
	 *
	 * @param {mt.customize.Section|mt.customize.Panel|mt.customize.Control} focusConstruct - The object to initially focus.
	 * @param {mt.customize.Section|mt.customize.Panel|mt.customize.Control} returnConstruct - The object to return focus.
	 */
