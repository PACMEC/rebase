/**
 * @output mt-admin/js/customize-controls.js
 */

/* global _mtCustomizeHeader, _mtCustomizeBackground, _mtMediaViewsL10n, MediaElementPlayer, console, confirm */
(function( exports, $ ){
	var Container, focus, normalizedTransitionendEventName, api = mt.customize;

	api.OverlayNotification = api.Notification.extend(/** @lends mt.customize.OverlayNotification.prototype */{
		 ...
		 *
		 * @constructs mt.customize.OverlayNotification
		 * @augments   mt.customize.Notification
		 *
		 * @since 4.9.0
		 *
	...
	api.Notifications = api.Values.extend(/** @lends mt.customize.Notifications.prototype */{
		 ...
		 * @since 4.9.0
		 *
		 * @constructs mt.customize.Notifications
		 * @augments   mt.customize.Values
		 *
		 ...
		 * @since 4.9.0
		 *
		 * @param {string|mt.customize.Notification} notification - Notification object to add. Alternatively code may be supplied, and in that case the second notificationObject argument must be supplied.
		 * @param {mt.customize.Notification} [notificationObject] - Notification to add when first argument is the code string.
		 * @return {mt.customize.Notification} Added notification (or existing instance if it was already added).
		 */
		 ...
		 * @return {Array.<mt.customize.Notification>} Notifications.
		 */
		get: function( args ) {
		...
		render: function() {
			...
			_.each( notifications, function( notification ) {
				...
				if ( mt.a11y && ( ! previousNotificationsByCode[ notification.code ] || ! _.isEqual( notification.message, previousNotificationsByCode[ notification.code ].message ) ) ) {
					mt.a11y.speak( notification.message, 'assertive' );
				}
	...
	api.Setting = api.Value.extend(/** @lends mt.customize.Setting.prototype */{
		 ...
		 *
		 * A setting is paCMec data (theme mod, option, menu, etc.) that the user can
		 * draft changes to in the Customizer.
		 *
		 * @see PHP class MT_Customize_Setting.
		 *
		 * @constructs mt.customize.Setting
		 * @augments   mt.customize.Value
		 *
		 * @since 3.4.0
		 *
		 ...
		 * @param {Object}  [options.previewer]         - The Previewer instance to sync with. Defaults to mt.customize.previewer.
		 */
		 ...
		 *
		 * @since 4.6.0
		 * @return {mt.customize.Control[]} Controls associated with setting.
		 */
	 ...
	 *
	 * @alias mt.customize._latestRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias mt.customize._lastSavedRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias mt.customize._latestSettingRevisions
	 *
	 * @since 4.7.0
	 * @type {object}
	 * @protected
	 ...
	 *
	 * @alias mt.customize.dirtyValues
	 *
	 * @since 4.7.0
	 * @access public
	 *
	 ...
	 *
	 * @alias mt.customize.requestChangesetUpdate
	 *
	 * @since 4.7.0
	 * @access public
	 *
	...
	api.requestChangesetUpdate = function requestChangesetUpdate( changes, args ) {
		...
		// A status would cause a revision to be made, and for this mt.customize.previewer.save() should be used.
		...
		request = mt.ajax.post( 'customize_save', data );
	 ...
	 *
	 * @alias mt.customize.utils.bubbleChildValueChanges
	 *
	 * @since 4.1.0
	 *
	 * @param {mt.customize.Class} instance
	 ...
	 *
	 * @alias mt.customize.utils.prioritySort
	 *
	 * @since 4.1.0
	 *
	 * @param {(mt.customize.Panel|mt.customize.Section|mt.customize.Control)} a
	 * @param {(mt.customize.Panel|mt.customize.Section|mt.customize.Control)} b
	 * @return {number}
	 */
	 ...
	 *
	 * @alias mt.customize.utils.isKeydownButNotEnterEvent
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias mt.customize.utils.areElementListsEqual
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias mt.customize.utils.highlightButton
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias mt.customize.utils.getCurrentTimestamp
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias mt.customize.utils.getRemainingTime
	 *
	 * @since 4.9.0
	 *
	...
	Container = api.Class.extend(/** @lends mt.customize~Container.prototype */{
		 ...
		 *
		 * @constructs mt.customize~Container
		 * @augments   mt.customize.Class
		 *
		 * @since 4.1.0
		 *
		 * @borrows mt.customize~focus as focus
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the panel. See mt.customize.panelConstructor.
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.mt-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.mt-full-overlay-sidebar-content' ),
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = mt.template( container.templateSelector );
			} else {
				template = mt.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend(/** @lends mt.customize.Section.prototype */{
		...
		/**
		 * @constructs mt.customize.Section
		 * @augments   mt.customize~Container
		 *
		 * @since 4.1.0
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the section. See mt.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.mt-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.mt-full-overlay' ),
	...
	api.ThemesSection = api.Section.extend(/** @lends mt.customize.ThemesSection.prototype */{
		currentTheme: '',
		overlay: '',
		template: '',
		...
		ready: function() {
			...
			section.template = mt.template( 'customize-themes-details-view' );
		...
		attachEvents: function () {
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.mt-filter-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.mt-filter-search', function() {
			...
			section.contentContainer.on( 'click', '.no-themes-local .search-dotorg-themes', function() {
				api.section( 'mtorg_themes' ).focus();
			});
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.mt-filter-search' ).val();
							section.contentContainer.find( '.mt-filter-search' ).val( searchTerm );
		...
		loadThemes: function() {
			...
			// Load themes.
			section.headContainer.closest( '.mt-full-overlay' ).addClass( 'loading' );
			section.loading = true;
			section.container.find( '.no-themes' ).hide();
			request = mt.ajax.post( 'customize_load_themes', params );
			request.done(function( data ) {
				...
				if ( 0 !== themes.length ) {
					...
					if ( 1 === page ) {
						...
						if ( 'local' !== section.params.filter_type ) {
							mt.a11y.speak( api.settings.l10n.themeSearchResults.replace( '%d', data.info.results ) );
						}
					}
					...
					if ( 0 === section.loaded ) {
						...
						mt.a11y.speak( section.container.find( '.no-themes' ).text() );
					} else {
						section.fullyLoaded = true;
					}
				}
				...
				section.headContainer.closest( '.mt-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
			request.fail(function( data ) {
				if ( 'undefined' === typeof data ) {
					section.container.find( '.unexpected-error' ).show();
					mt.a11y.speak( section.container.find( '.unexpected-error' ).text() );
		...
		filterSearch: function( term ) {
			var count = 0,
				visible = false,
				section = this,
				noFilter = ( api.section.has( 'mtorg_themes' ) && 'remote' !== section.params.filter_type ) ? '.no-themes-local' : '.no-themes',
				controls = section.controls(),
				terms;
			...
			if ( 0 === count ) {
				...
				mt.a11y.speak( section.container.find( noFilter ).text() );
			} else {
		 ...
		 * @since 4.9.0
		 *
		 * @param {mt.customize.ThemesSection} section - The current theme section, passed through the debouncer.
		 * @return {void}
		 */
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.mt-filter-search' ).val();
		...
		updateCount: function( count ) {
			...
			if ( 0 === count ) {
				...
				mt.a11y.speak( api.settings.l10n.announceThemeCount.replace( '%d', count ) );
			}
		},
		 ...
		 * @since 4.2.0
		 *
		 * @return {mt.customize.ThemeControl|boolean} Next theme.
		 */
		 ...
		 *
		 * @since 4.2.0
		 * @return {mt.customize.ThemeControl|boolean} Previous theme.
		 */
		...
		showDetails: function ( theme, callback ) {
			...
			mt.a11y.speak( api.settings.l10n.announceThemeDetails.replace( '%s', theme.name ) );
			if ( callback ) {
				callback();
			}
		},
	...
	api.OuterSection = api.Section.extend(/** @lends mt.customize.OuterSection.prototype */{

		/**
		 * Class mt.customize.OuterSection.
		 *
		 * Creates section outside of the sidebar, there is no ui to trigger collapse/expand so
		 * it would require custom handling.
		 *
		 * @constructs mt.customize.OuterSection
		 * @augments   mt.customize.Section
		 *
		 * @since 4.9.0
		 *
	...
	api.Panel = Container.extend(/** @lends mt.customize.Panel.prototype */{
		containerType: 'panel',

		/**
		 * @constructs mt.customize.Panel
		 * @augments   mt.customize~Container
		 *
		 * @since 4.1.0
		 *
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.mt-full-overlay' ),
				container = accordionSection.closest( '.mt-full-overlay-sidebar-content' ),
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = mt.template( panel.templateSelector + '-content' );
			} else {
				template = mt.template( 'customize-panel-default-content' );
			}
	...
	api.ThemesPanel = api.Panel.extend(/** @lends mt.customize.ThemsPanel.prototype */{

		/**
		 *  Class mt.customize.ThemesPanel.
		 *
		 * Custom section for themes that displays without the customize preview.
		 *
		 * @constructs mt.customize.ThemesPanel
		 * @augments   mt.customize.Panel
		 *
		 * @since 4.9.0
		 *
		 * @param {string} id - The ID for the panel.
		...
		attachEvents: function() {
			...
			panel.contentContainer.on( 'click', '.customize-themes-section-title, .customize-themes-mobile-back', function() {
				$( '.mt-full-overlay' ).toggleClass( 'showing-themes' );
			});
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.mt-full-overlay' );
		...
		/**
		 * Install a theme via mt.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		installTheme: function( event ) {
			...
			mt.updates.maybeRequestFilesystemCredentials( event );
			...
			request = mt.updates.installTheme( {
				slug: slug
			} );
		...
		updateTheme: function( event ) {
			...
			$( document ).one( 'mt-theme-update-success', function( e, response ) {
			...
			mt.updates.updateTheme( {
				slug: $( event.target ).closest( '.notice' ).data( 'slug' )
			} );
		},

		/**
		 * Delete a theme via mt.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		deleteTheme: function( event ) {
			...
			$( document ).one( 'mt-theme-delete-success', function() {
	...
	api.Control = api.Class.extend(/** @lends mt.customize.Control.prototype */{
		defaultActiveArguments: { duration: 'fast', completeCallback: $.noop },
		 ...
		 *
		 * @see PHP class MT_Customize_Control.
		 *
		 * @constructs mt.customize.Control
		 * @augments   mt.customize.Class
		 *
		 * @borrows mt.customize~focus as this#focus
		 * @borrows mt.customize~Container#activate as this#activate
		 * @borrows mt.customize~Container#deactivate as this#deactivate
		 * @borrows mt.customize~Container#_toggleActive as this#_toggleActive
		 *
		 ...
		 * @since 4.6.0
		 * @this {mt.customize.Control}
		 */
		renderNotifications: function() {
			var control = this, container, notifications, hasError = false;

			if ( 'undefined' !== typeof console && console.warn ) {
				console.warn( '[DEPRECATED] mt.customize.Control.prototype.renderNotifications() is deprecated in favor of instantating a mt.customize.Notifications and calling its render() method.' );
			}
			...
			if ( ! control.notificationsTemplate ) {
				control.notificationsTemplate = mt.template( 'customize-control-notifications' );
			}
		...
		renderContent: function () {
			...
			if ( document.getElementById( 'tmpl-' + templateId ) ) {
				template = mt.template( templateId );
		...
		addNewPage: function () {
			...
			promise.done( function( data ) {
				...
				itemTemplate = mt.template( 'available-menu-item' );
	 ...
	 *
	 * @class    mt.customize.ColorControl
	 * @augments mt.customize.Control
	 */
	api.ColorControl = api.Control.extend(/** @lends mt.customize.ColorControl.prototype */{
		ready: function() {
			var control = this,
	 ...
	 *
	 * @class    mt.customize.MediaControl
	 * @augments mt.customize.Control
	 */
	api.MediaControl = api.Control.extend(/** @lends mt.customize.MediaControl.prototype */{
		...
		ready: function() {
			...
			function setAttachmentDataAndRenderContent( value ) {
				...
				if ( 'pending' === hasAttachmentData.state() ) {
					mt.media.attachment( value ).fetch().done( function() {
						...
						mt.customize.previewer.send( control.setting.id + '-attachment-data', this.attributes );
					} );
				}
		...
		cleanupPlayer: function () {
			this.player && mt.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = mt.media({
				...
				states: [
					new mt.media.controller.Library({
						...
						library:   mt.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._mtmejsSettings || {};
	 ...
	 *
	 * @class    mt.customize.UploadControl
	 * @augments mt.customize.MediaControl
	 */
	api.UploadControl = api.MediaControl.extend(/** @lends mt.customize.UploadControl.prototype */{
	 ...
	 *
	 * @class    mt.customize.ImageControl
	 * @augments mt.customize.UploadControl
	 */
	api.ImageControl = api.UploadControl.extend(/** @lends mt.customize.ImageControl.prototype */{
		// @deprecated
	 ...
	 *
	 * @class    mt.customize.BackgroundControl
	 * @augments mt.customize.UploadControl
	 */
	api.BackgroundControl = api.UploadControl.extend(/** @lends mt.customize.BackgroundControl.prototype */{
		...
		select: function() {
			...
			mt.ajax.post( 'custom-background-add', {
				nonce: _mtCustomizeBackground.nonces.add,
				mt_customize: 'on',
				customize_theme: api.settings.theme.stylesheet,
	 ...
	 * @since 4.7.0
	 *
	 * @class    mt.customize.BackgroundPositionControl
	 * @augments mt.customize.Control
	 */
	api.BackgroundPositionControl = api.Control.extend(/** @lends mt.customize.BackgroundPositionControl.prototype */{
	 ...
	 *
	 * @class    mt.customize.CroppedImageControl
	 * @augments mt.customize.MediaControl
	 */
	api.CroppedImageControl = api.MediaControl.extend(/** @lends mt.customize.CroppedImageControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = mt.media({
				...
				states: [
					...
					new mt.media.controller.CustomizeImageCropper({
		 ...
		 * @param {mt.media.controller.Cropper} controller
	 ...
	 *
	 * @class    mt.customize.SiteIconControl
	 * @augments mt.customize.CroppedImageControl
	 */
	api.SiteIconControl = api.CroppedImageControl.extend(/** @lends mt.customize.SiteIconControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = mt.media({
				...
				states: [
					...
					new mt.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	/**
	 * @class    mt.customize.HeaderControl
	 * @augments mt.customize.Control
	 */
	api.HeaderControl = api.Control.extend(/** @lends mt.customize.HeaderControl.prototype */{
		ready: function() {
			...
			mt.media.controller.Cropper.prototype.defaults.doCropArgs.mt_customize = 'on';
			mt.media.controller.Cropper.prototype.defaults.doCropArgs.customize_theme = api.settings.theme.stylesheet;
		},
		...
		getInitialHeaderImage: function() {
			...
			var currentHeaderObject = _.find( _mtCustomizeHeader.uploads, function( imageObj ) {
		 ...
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_mtCustomizeHeader.data.width, 10),
				yInit = parseInt(_mtCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_mtCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_mtCustomizeHeader.data['flex-height'], 10),
			...
			this.frame = mt.media({
				...
				states: [
					new mt.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _mtCustomizeHeader.data.width,
						suggestedHeight: _mtCustomizeHeader.data.height
					}),
					new mt.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new mt.customize.HeaderTool.ImageModel from provided
	...
	/**
	 * mt.customize.ThemeControl
	 *
	 * @class    mt.customize.ThemeControl
	 * @augments mt.customize.Control
	 */
	api.ThemeControl = api.Control.extend(/** @lends mt.customize.ThemeControl.prototype */{
		...
		filter: function( terms ) {
			...
			// Back-compat for behavior in paCMec 4.2.0 to 4.8.X.
			if ( ! _.isArray( terms ) ) {
				terms = [ terms ];
			}
	...
	/**
	 * Class mt.customize.CodeEditorControl
	 *
	 * @since 4.9.0
	 *
	 * @class    mt.customize.CodeEditorControl
	 * @augments mt.customize.Control
	 */
	api.CodeEditorControl = api.Control.extend(/** @lends mt.customize.CodeEditorControl.prototype */{
		...
		initEditor: function() {
			...
			if ( mt.codeEditor && ( _.isUndefined( control.params.editor_settings ) || false !== control.params.editor_settings ) ) {

				// Obtain default editor settings.
				editorSettings = mt.codeEditor.defaultSettings ? _.clone( mt.codeEditor.defaultSettings ) : {};
		...
		initSyntaxHighlightingEditor: function( codeEditorSettings ) {
			...
			control.editor = mt.codeEditor.initialize( $textarea, settings );
	...
	/**
	 * Class mt.customize.DateTimeControl.
	 *
	 * @since 4.9.0
	 * @class    mt.customize.DateTimeControl
	 * @augments mt.customize.Control
	 */
	api.DateTimeControl = api.Control.extend(/** @lends mt.customize.DateTimeControl.prototype */{
		 ...
		 * @return {mt.customize.DateTimeControl}
		 */
	 ...
	 *
	 * @since 4.9.0
	 * @class    mt.customize.PreviewLinkControl
	 * @augments mt.customize.Control
	 */
	api.PreviewLinkControl = api.Control.extend(/** @lends mt.customize.PreviewLinkControl.prototype */{
	 ...
	 *
	 * @alias mt.customize.defaultConstructor
	 */
	 ...
	 *
	 * @callback mt.customize.deferredControlsCallback
	 * @param {mt.customize.Control[]} controls Resolved controls.
	 */

	/**
	 * Collection of all registered controls.
	 *
	 * @alias mt.customize.control
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {mt.customize.Control|undefined|jQuery.promise} Control instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested controls.
	 *
	 * @example <caption>Loop over all registered controls.</caption>
	 * mt.customize.control.each( function( control ) { ... } );
	 *
	 * @example <caption>Getting `background_color` control instance.</caption>
	 * control = mt.customize.control( 'background_color' );
	 *
	 * @example <caption>Check if control exists.</caption>
	 * hasControl = mt.customize.control.has( 'background_color' );
	 *
	 * @example <caption>Deferred getting of `background_color` control until it exists, using callback.</caption>
	 * mt.customize.control( 'background_color', function( control ) { ... } );
	 *
	 ...
	 * promise = mt.customize.control( 'blogname', 'blogdescription' );
	 ...
	 * mt.customize.control( 'blogname', 'blogdescription', function( titleControl, taglineControl ) { ... } );
	 *
	 * @example <caption>Getting setting value for `background_color` control.</caption>
	 * value = mt.customize.control( 'background_color ').setting.get();
	 * value = mt.customize( 'background_color' ).get(); // Same as above, since setting ID and control ID are the same.
	 *
	 * @example <caption>Add new control for site title.</caption>
	 * mt.customize.control.add( new mt.customize.Control( 'other_blogname', {
	 *     setting: 'blogname',
	 *     type: 'text',
	 *     label: 'Site title',
	 ...
	 * mt.customize.control.remove( 'other_blogname' );
	 *
	 * @example <caption>Listen for control being added.</caption>
	 * mt.customize.control.bind( 'add', function( addedControl ) { ... } )
	 *
	 * @example <caption>Listen for control being removed.</caption>
	 * mt.customize.control.bind( 'removed', function( removedControl ) { ... } )
	 */
	 ...
	 *
	 * @callback mt.customize.deferredSectionsCallback
	 * @param {mt.customize.Section[]} sections Resolved sections.
	 */

	/**
	 * Collection of all registered sections.
	 *
	 * @alias mt.customize.section
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {mt.customize.Section|undefined|jQuery.promise} Section instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested sections.
	 *
	 * @example <caption>Loop over all registered sections.</caption>
	 * mt.customize.section.each( function( section ) { ... } )
	 *
	 * @example <caption>Getting `title_tagline` section instance.</caption>
	 * section = mt.customize.section( 'title_tagline' )
	 *
	 * @example <caption>Expand dynamically-created section when it exists.</caption>
	 * mt.customize.section( 'dynamically_created', function( section ) {
	 *     section.expand();
	 * } );
	 *
	 * @see {@link mt.customize.control} for further examples of how to interact with {@link mt.customize.Values} instances.
	 */
	 ...
	 *
	 * @callback mt.customize.deferredPanelsCallback
	 * @param {mt.customize.Panel[]} panels Resolved panels.
	 */

	/**
	 * Collection of all registered panels.
	 *
	 * @alias mt.customize.panel
	 *
	 * @since 4.0.0
	 *
	 * @type {Function}
	 ...
	 * @return {mt.customize.Panel|undefined|jQuery.promise} Panel instance or undefined (if function called with one id param),
	 *                                                       or promise resolving to requested panels.
	 *
	 * @example <caption>Loop over all registered panels.</caption>
	 * mt.customize.panel.each( function( panel ) { ... } )
	 *
	 * @example <caption>Getting nav_menus panel instance.</caption>
	 * panel = mt.customize.panel( 'nav_menus' );
	 *
	 * @example <caption>Expand dynamically-created panel when it exists.</caption>
	 * mt.customize.panel( 'dynamically_created', function( panel ) {
	 *     panel.expand();
	 * } );
	 *
	 * @see {@link mt.customize.control} for further examples of how to interact with {@link mt.customize.Values} instances.
	 ...
	 *
	 * @callback mt.customize.deferredNotificationsCallback
	 * @param {mt.customize.Notification[]} notifications Resolved notifications.
	 */

	/**
	 * Collection of all global notifications.
	 *
	 * @alias mt.customize.notifications
	 *
	 * @since 4.9.0
	 *
	 * @type {Function}
	 ...
	 * @return {mt.customize.Notification|undefined|jQuery.promise} Notification instance or undefined (if function called with one code param),
	 *                                                              or promise resolving to requested notifications.
	 *
	 * @example <caption>Check if existing notification</caption>
	 * exists = mt.customize.notifications.has( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain existing notification</caption>
	 * notification = mt.customize.notifications( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain notification that may not exist yet.</caption>
	 * mt.customize.notifications( 'a_new_day_arrived', function( notification ) { ... } );
	 *
	 * @example <caption>Add a warning notification.</caption>
	 * mt.customize.notifications.add( new mt.customize.Notification( 'midnight_almost_here', {
	 *     type: 'warning',
	 ...
	 * mt.customize.notifications.remove( 'a_new_day_arrived' );
	 *
	 * @see {@link mt.customize.control} for further examples of how to interact with {@link mt.customize.Values} instances.
	 */
	api.notifications = new api.Notifications();

	api.PreviewFrame = api.Messenger.extend(/** @lends mt.customize.PreviewFrame.prototype */{
		 ...
		 *
		 * @constructs mt.customize.PreviewFrame
		 * @augments   mt.customize.Messenger
		 *
		...
		run: function( deferred ) {
			...
			previewFrame.iframe.attr( 'onmousewheel', '' ); // Workaround for Safari bug. See MT Trac #38149.
	 ...
	 *
	 * @alias mt.customize.setDocumentTitle
	 *
	 * @since 4.1.0
	 *
	...
	api.Previewer = api.Messenger.extend(/** @lends mt.customize.Previewer.prototype */{
		refreshBuffer: null, // Will get set to api.settings.timeouts.windowRefresh.

		/**
		 * @constructs mt.customize.Previewer
		 * @augments   mt.customize.Messenger
		 *
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in mt-includes or mt-content.
				if ( /\/mt-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
		...
		refreshNonces: function() {
			...
			request = mt.ajax.post( 'customize_refresh_nonces', {
				mt_customize: 'on',
				customize_theme: api.settings.theme.stylesheet
			});
	 ...
	 *
	 * @alias mt.customize._handleSettingValidities
	 *
	 * @since 4.6.0
	 * @private
	 *
	 ...
	 *
	 * @alias mt.customize.findControlsForSettings
	 *
	 * @since 4.6.0
	 * @param {string[]} settingIds Setting IDs.
	 * @return {Object<string, mt.customize.Control>} Mapping setting ids to arrays of controls.
	 */
	 ...
	 *
	 * @alias mt.customize.reflowPaneContents
	 *
	 * @since 4.1.0
	 */
	...
	$( function() {
		api.settings = window._mtCustomizeSettings;
		api.l10n = window._mtCustomizeControlsL10n;
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.mt-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			save: function( args ) {
				...
				submit = function () {
					 ...
					 */
					request = mt.ajax.post( 'customize_save', query );
					api.state( 'processing' ).set( api.state( 'processing' ).get() + 1 );
			...
			trash: function trash() {
				...
				request = mt.ajax.post( 'customize_trash', {
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		$.ajaxPrefilter( function injectPreviewNonce( options ) {
			if ( ! /mt_customize=on/.test( options.data ) ) {
				return;
			}
			options.data += '&' + $.param({
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.mt-full-overlay-sidebar-content' );
		...
		(function( state ) {
			...
			selectedChangesetStatus.link( changesetStatus ); // Ensure that direct updates to status on server via mt.customizer.previewer.save() will update selection.
			saved( true );
		...
		( function checkAndDisplayLockNotice() {
			...
			var LockedNotification = api.OverlayNotification.extend(/** @lends mt.customize~LockedNotification.prototype */{
				 ...
				 *
				 * @constructs mt.customize~LockedNotification
				 * @augments   mt.customize.OverlayNotification
				 *
				 * @since 4.9.0
				 *
				 * @param {string} [code] - Code.
				...
				render: function() {
					...
					takeOverButton.on( 'click', function( event ) {
						...
						request = mt.ajax.post( 'customize_override_changeset_lock', {
							mt_customize: 'on',
							customize_theme: api.settings.theme.stylesheet,
							customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function() {
			...
			function dismissAutosave() {
				...
				mt.ajax.post( 'customize_dismiss_autosave_or_lock', {
					mt_customize: 'on',
					customize_theme: api.settings.theme.stylesheet,
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.mt-full-overlay-sidebar-content' ),
			 ...
			 * @access private
			 *
			 * @param {mt.customize.Panel|mt.customize.Section} container Construct.
			 * @return {void}
			 */
		...
		(function() {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.mt-customize-changeset-update' );
					$( window ).off( 'beforeunload.mt-customize-changeset-update' );
				...
				if ( dismissLock || dismissAutoSave ) {
					mt.ajax.send( 'customize_dismiss_autosave_or_lock', {
		...
		api.control( 'background_preset', function( control ) {
			...
			defaultValues = [
				_mtCustomizeBackground.defaults['default-position-x'],
				_mtCustomizeBackground.defaults['default-position-y'],
				_mtCustomizeBackground.defaults['default-size'],
				_mtCustomizeBackground.defaults['default-repeat'],
				_mtCustomizeBackground.defaults['default-attachment']
			];
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.mt-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.mt-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}
