/**
 * @output mt-admin/js/customize-nav-menus.js
 */

/* global _mtCustomizeNavMenusSettings, mtNavMenu, console */
( function( api, mt, $ ) {
	'use strict';

	/**
	 * Set up mtNavMenu for drag and drop.
	 */
	mtNavMenu.originalInit = mtNavMenu.init;
	mtNavMenu.options.menuItemDepthPerLevel = 20;
	mtNavMenu.options.sortableItems         = '> .customize-control-nav_menu_item';
	mtNavMenu.options.targetTolerance       = 10;
	mtNavMenu.init = function() {
		this.jQueryExtensions();
	};

	/**
	 * @namespace mt.customize.Menus
	 */
	api.Menus = api.Menus || {};
	...
	if ( 'undefined' !== typeof _mtCustomizeNavMenusSettings ) {
		$.extend( api.Menus.data, _mtCustomizeNavMenusSettings );
	}
	 ...
	 *
	 * @alias mt.customize.Menus.generatePlaceholderAutoIncrementId
	 *
	 * @return {number}
	 */
	...
	/**
	 * mt.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's MT_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @class    mt.customize.Menus.AvailableItemModel
	...
	/**
	 * mt.customize.Menus.AvailableItemCollection
	 *
	 * Collection for available menu item models.
	 *
	 * @class    mt.customize.Menus.AvailableItemCollection
	 * @augments Backbone.Collection
	 */
	api.Menus.AvailableItemCollection = Backbone.Collection.extend(/** @lends mt.customize.Menus.AvailableItemCollection.prototype */{
	 ...
	 *
	 * @since 4.7.0
	 * @alias mt.customize.Menus.insertAutoDraftPost
	 *
	...
	api.Menus.insertAutoDraftPost = function insertAutoDraftPost( params ) {
		...
		request = mt.ajax.post( 'customize-nav-menus-insert-auto-draft', {
	...
	api.Menus.AvailableMenuItemsPanelView = mt.Backbone.View.extend(/** @lends mt.customize.Menus.AvailableMenuItemsPanelView.prototype */{
		...
		/**
		 * mt.customize.Menus.AvailableMenuItemsPanelView
		 *
		 * View class for the available menu items panel.
		 *
		 * @constructs mt.customize.Menus.AvailableMenuItemsPanelView
		...
		doSearch: function( page ) {
			...
			if ( page < 0 ) {
				...
				mt.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = mt.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					mt.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					mt.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( itemTypes, deprecated ) {
			...
			request = mt.ajax.post( 'load-available-menu-items-customizer', params );
		...
		submitNew: function( container ) {
			...
			promise.done( function( data ) {
				...
				itemElement = $( mt.template( 'available-menu-item' )( availableItem.attributes ) );
	...
	/**
	 * mt.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the MT_Customize_Menu_Panel::$type.
	 *
	 * @class    mt.customize.Menus.MenusPanel
	 * @augments mt.customize.Panel
	 */
	api.Menus.MenusPanel = api.Panel.extend(/** @lends mt.customize.Menus.MenusPanel.prototype */{
		...
		ready: function() {
			...
			api.section( 'menu_locations', function( section ) {
				section.headContainer.prepend(
					mt.template( 'nav-menu-locations-header' )( api.Menus.data )
				);
			} );
		},
		...
		saveManageColumnsState: _.debounce( function() {
			...
			panel._updateHiddenColumnsRequest = mt.ajax.post( 'hidden-columns', {
				hidden: panel.hidden(),
	...
	/**
	 * mt.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the MT_Customize_Menu_Section::$type.
	 *
	 * @class    mt.customize.Menus.MenuSection
	 * @augments mt.customize.Section
	 */
	api.Menus.MenuSection = api.Section.extend(/** @lends mt.customize.Menus.MenuSection.prototype */{
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				mtNavMenu.menuList = section.contentContainer;
				mtNavMenu.targetList = mtNavMenu.menuList;

				// Add attributes needed by mtNavMenu.
				$( '#menu-to-edit' ).removeAttr( 'id' );
				mtNavMenu.menuList.attr( 'id', 'menu-to-edit' ).addClass( 'menu' );
				...
				args.completeCallback = function() {
					if ( 'resolved' !== section.deferred.initSortables.state() ) {
						mtNavMenu.initSortables(); // Depends on menu-to-edit ID being set above.
						section.deferred.initSortables.resolve( mtNavMenu.menuList ); // Now MenuControl can extend the sortable.

						// @todo Note that mt.customize.reflowPaneContents() is debounced,
	 ...
	 * @return {mt.customize.Menus.MenuSection} Added nav menu.
	 */
	...
	/**
	 * mt.customize.Menus.NewMenuSection
	 *
	 * Customizer section for new menus.
	 *
	 * @class    mt.customize.Menus.NewMenuSection
	 * @augments mt.customize.Section
	 */
	api.Menus.NewMenuSection = api.Section.extend(/** @lends mt.customize.Menus.NewMenuSection.prototype */{
		...
		attachEvents: function() {
			...
			section.headContainer.find( '.accordion-section-title' ).replaceWith(
				mt.template( 'nav-menu-create-menu-section-title' )
			);
			 ...
			 *
			 * @since 4.9.0
			 * @param {mt.customize.Setting} setting - Added setting.
			 * @return {void}
			 */
			 ...
			 *
			 * @since 4.9.0
			 * @param {mt.customize.Setting} setting - Removed setting.
			 * @return {void}
			 */
		...
		submit: function() {
			...
			mt.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * mt.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the MT_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @class    mt.customize.Menus.MenuLocationControl
	 * @augments mt.customize.Control
	 */
	api.Menus.MenuLocationControl = api.Control.extend(/** @lends mt.customize.Menus.MenuLocationControl.prototype */{
		initialize: function( id, options ) {
			var control = this,
	...
	api.Menus.MenuItemControl = api.Control.extend(/** @lends mt.customize.Menus.MenuItemControl.prototype */{

		/**
		 * mt.customize.Menus.MenuItemControl
		 *
		 * Customizer control for menu items.
		 * Note that 'menu_item' must match the MT_Customize_Menu_Item_Control::$type.
		 *
		 * @constructs mt.customize.Menus.MenuItemControl
		 * @augments   mt.customize.Control
		 *
		 * @inheritDoc
		 */
		...
		_setupReorderUI: function() {
			...
			template = mt.template( 'menu-item-reorder-nav' );
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					mt.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {mt.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			mt.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			mt.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			mt.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			mt.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * mt.customize.Menus.MenuNameControl
	 *
	 * Customizer control for a nav menu's name.
	 *
	 * @class    mt.customize.Menus.MenuNameControl
	 * @augments mt.customize.Control
	 */
	api.Menus.MenuNameControl = api.Control.extend(/** @lends mt.customize.Menus.MenuNameControl.prototype */{
	...
	/**
	 * mt.customize.Menus.MenuLocationsControl
	 *
	 * Customizer control for a nav menu's locations.
	 *
	 * @since 4.9.0
	 * @class    mt.customize.Menus.MenuLocationsControl
	 * @augments mt.customize.Control
	 */
	api.Menus.MenuLocationsControl = api.Control.extend(/** @lends mt.customize.Menus.MenuLocationsControl.prototype */{
	...
	/**
	 * mt.customize.Menus.MenuAutoAddControl
	 *
	 * Customizer control for a nav menu's auto add.
	 *
	 * @class    mt.customize.Menus.MenuAutoAddControl
	 * @augments mt.customize.Control
	 */
	api.Menus.MenuAutoAddControl = api.Control.extend(/** @lends mt.customize.Menus.MenuAutoAddControl.prototype */{
	...
	/**
	 * mt.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the MT_Menu_Customize_Control::$type
	 *
	 * @class    mt.customize.Menus.MenuControl
	 * @augments mt.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend(/** @lends mt.customize.Menus.MenuControl.prototype */{
		/**
		 * Set up the control.
		 */
		ready: function() {
		 ...
		 *
		 * Notice that the UI aspects here are handled by mtNavMenu.initSortables()
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that mtNavMenu will be setting this .menu-item-data-parent-id input's value.
		...
		_handleDeletion: function() {
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						mt.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				mt.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				mt.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {mt.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @return {mt.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			mt.a11y.speak( api.Menus.data.l10n.itemAdded );
		 ...
		 * @since 4.9.0
		 *
		 * @param {mt.customize.controlConstructor.nav_menu_item[]} optionalMenuItemControls
		 */
	...
	/**
	 * Extends mt.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends mt.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends mt.customize.sectionConstructor with section constructor for menu.
	 */
	 ...
	 *
	 * @alias mt.customize.Menus.applySavedData
	 *
	 * @param {Object} data
			...
			if ( 'inserted' === update.status ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in MT_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @alias mt.customize.Menus.focusMenuItemControl
	 *
	 ...
	 *
	 * @alias mt.customize.Menus.getMenuControl
	 *
	 * @param menuId
	 * @return {mt.customize.controlConstructor.menus[]}
	 */
	 ...
	 *
	 * @alias mt.customize.Menus.getMenuItemControl
	 *
	 * @param {string} menuItemId
	 * @return {Object|null}
	 */
	...
	/**
	 * @alias mt.customize.Menus~menuItemIdToSettingId
	 *
	 * @param {string} menuItemId
	 */
	 ...
	 *
	 * @alias mt.customize.Menus~displayNavMenuName
	 *
	 * @param {string} name
	 * @return {string}
	 */
	function displayNavMenuName( name ) {
		name = name || '';
		name = mt.sanitize.stripTagsAndEncodeText( name ); // Remove any potential tags from name.
		name = name.toString().trim();
		return name || api.Menus.data.l10n.unnamed;
	}

})( mt.customize, mt, jQuery );
