/**
 * paCMec Administration Navigation Menu
 ...
 * @output mt-admin/js/nav-menu.js
 */

/* global menus, postboxes, columns, isRtl, ajaxurl, mtNavMenu */
...
(function($) {
	...
	/**
	 * Contains all the functions to handle paCMec navigation menus administration.
	 *
	 * @namespace mtNavMenu
	 */
	api = window.mtNavMenu = {
		...
		initPreviewing : function() {
			...
			$( '#menu-to-edit' ).on( 'change input', '.edit-menu-item-title', function(e) {
				...
				if ( title ) {
					...
					titleEl.text( mt.i18n._x( '(no label)', 'missing menu item navigation label' ) ).addClass( 'no-title' );
				}
			} );
		},
		...
		attachUnsavedChangesListener : function() {
			...
			if ( 0 !== $('#menu-to-edit').length || 0 !== $('.menu-location-menus select').length ) {
				window.onbeforeunload = function(){
					if ( api.menusChanged )
						return mt.i18n.__( 'The changes you made will be lost if you navigate away from this page.' );
				};
			} else {
		...
		eventOnClickMenuDelete : function() {
			// Delete warning AYS.
			if ( window.confirm( mt.i18n.__( 'You are about to permanently delete this menu.\n\'Cancel\' to stop, \'OK\' to delete.' ) ) ) {
				window.onbeforeunload = null;
				return true;
			}
			return false;
		...
		processQuickSearchQueryResponse : function(resp, req, panel) {
			...
			if( ! $items.length ) {
				$('.categorychecklist', panel).html( '<li><p>' + mt.i18n.__( 'No results found.' ) + '</p></li>' );
