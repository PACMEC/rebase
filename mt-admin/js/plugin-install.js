 ...
 *
 * @output mt-admin/js/plugin-install.js
 */
...
jQuery( document ).ready( function( $ ) {
	 ...
	 * @todo Consider to implement a paCMec general utility for this and don't use jQuery UI.
	 */
	function handleTabbables() {
		...
		$firstAndLast.off( 'keydown.mt-plugin-details' );
		// Attach again the keydown event on the first and last focusable elements.
		$firstAndLast.on( 'keydown.mt-plugin-details', function( event ) {
	...
	$( '.wrap' ).on( 'click', '.thickbox.open-plugin-details-modal', function( e ) {
		...
		tbWindow
			.attr({
				'role': 'dialog',
				'aria-label': mt.i18n.__( 'Plugin details' )
			})
