/**
 * @output mt-admin/js/widgets/media-video-widget.js
 */
...
(function( component ) {
	 ...
	 *
	 * @class    mt.mediaWidgets.controlConstructors~VideoDetailsMediaFrame
	 * @augments mt.media.view.MediaFrame.VideoDetails
	 *
	 * @private
	 */
	VideoDetailsMediaFrame = mt.media.view.MediaFrame.VideoDetails.extend(/** @lends mt.mediaWidgets.controlConstructors~VideoDetailsMediaFrame.prototype */{
		...
		createStates: function createStates() {
			this.states.add([
				new mt.media.controller.VideoDetails({
					media: this.media
				}),

				new mt.media.controller.MediaLibrary({
					type: 'video',
					id: 'add-video-source',
					title: mt.media.view.l10n.videoAddSourceTitle,
				...
				new mt.media.controller.MediaLibrary({
					type: 'text',
					id: 'add-track',
					title: mt.media.view.l10n.videoAddTrackTitle,
					toolbar: 'add-track',
					media: this.media,
					menu: 'video-details'
				})
	 ...
	 * Video widget model.
	 *
	 * See MT_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    mt.mediaWidgets.modelConstructors.media_video
	 ...
	 * See MT_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    mt.mediaWidgets.controlConstructors.media_video
	 * @augments mt.mediaWidgets.MediaWidgetControl
	 */
	VideoWidgetControl = component.MediaWidgetControl.extend(/** @lends mt.mediaWidgets.controlConstructors.media_video.prototype */{
		...
		fetchEmbed: function fetchEmbed() {
			...
			control.fetchEmbedDfd = mt.apiRequest({
		...
		renderPreview: function renderPreview() {
			...
			if ( mime && attachmentId ) {
				if ( ! _.contains( _.values( mt.media.view.settings.embedMimes ), mime ) ) {
				...
				if ( matches ) {
					if ( ! _.contains( _.keys( mt.media.view.settings.embedMimes ), matches[1] ) ) {
						error = 'unsupported_file_type';
					}
				} else {
					isOEmbed = true;
			...
			previewTemplate = mt.template( 'mt-media-widget-video-preview' );
