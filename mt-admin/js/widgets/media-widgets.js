/**
 * @output mt-admin/js/widgets/media-widgets.js
 */

/* eslint consistent-this: [ "error", "control" ] */

/**
 * @namespace mt.mediaWidgets
 * @memberOf  mt
 */
mt.mediaWidgets = ( function( $ ) {
	'use strict';
	 ...
	 *
	 * @memberOf mt.mediaWidgets
	 *
	 * @type {Object.<string, mt.mediaWidgets.MediaWidgetModel>}
	 */
	...
	component.PersistentDisplaySettingsLibrary = mt.media.controller.Library.extend(/** @lends mt.mediaWidgets.PersistentDisplaySettingsLibrary.prototype */{

		/**
		 * Library which persists the customized display settings across selections.
		 *
		 * @constructs mt.mediaWidgets.PersistentDisplaySettingsLibrary
		...
		initialize: function initialize( options ) {
			...
			mt.media.controller.Library.prototype.initialize.call( this, options );
		},
		...
		display: function getDisplaySettingsModel( model ) {
			...
			display = mt.media.controller.Library.prototype.display.call( this, model );
	 ...
	 *
	 * @class    mt.mediaWidgets.MediaEmbedView
	 * @augments mt.media.view.Embed
	 */
	component.MediaEmbedView = mt.media.view.Embed.extend(/** @lends mt.mediaWidgets.MediaEmbedView.prototype */{
		...
		initialize: function( options ) {
			...
			mt.media.view.Embed.prototype.initialize.call( view, options );
		 ...
		 * Refresh embed view.
		 *
		 * Forked override of {mt.media.view.Embed#refresh()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
		refresh: function refresh() {
			/**
			 * @class mt.mediaWidgets~Constructor
			 */
			var Constructor;
			...
			if ( 'image' === this.controller.options.mimeType ) {
				...
				Constructor = mt.media.view.EmbedLink.extend(/** @lends mt.mediaWidgets~Constructor.prototype */{
					...
					updateoEmbed: function() {
						...
						mt.media.view.EmbedLink.prototype.updateoEmbed.call( embedLinkView );
					},
					...
					fetch: function() {
						...
						if ( matches ) {
							fileExt = matches[1];
							if ( ! mt.media.view.settings.embedMimes[ fileExt ] ) {
								embedLinkView.renderFail();
							} else if ( 0 !== mt.media.view.settings.embedMimes[ fileExt ].indexOf( embedLinkView.controller.options.mimeType ) ) {
								embedLinkView.renderFail();
							} else {
						...
						embedLinkView.dfd = mt.apiRequest({
	 ...
	 *
	 * @class    mt.mediaWidgets.MediaFrameSelect
	 * @augments mt.media.view.MediaFrame.Post
	 */
	component.MediaFrameSelect = mt.media.view.MediaFrame.Post.extend(/** @lends mt.mediaWidgets.MediaFrameSelect.prototype */{
		...
		createStates: function createStates() {
			...
			_.each( mt.media.view.settings.embedMimes, function( embedMime ) {
			...
			this.states.add([
				...
				new mt.media.controller.EditImage({ model: this.options.editImage }),

				// Embed states.
				new mt.media.controller.Embed({
		 ...
		 * Main insert toolbar.
		 *
		 * Forked override of {mt.media.view.MediaFrame.Post#mainInsertToolbar()} to override text.
		 *
		 * @param {mt.Backbone.View} view - Toolbar view.
		 * @this {mt.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Main embed toolbar.
		 *
		 * Forked override of {mt.media.view.MediaFrame.Post#mainEmbedToolbar()} to override text.
		 *
		 * @param {mt.Backbone.View} toolbar - Toolbar view.
		 * @this {mt.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Embed content.
		 *
		 * Forked override of {mt.media.view.MediaFrame.Post#embedContent()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
	...
	component.MediaWidgetControl = Backbone.View.extend(/** @lends mt.mediaWidgets.MediaWidgetControl.prototype */{

		/**
		 * Translation strings.
		 *
		 * The mapping of translation strings is handled by media widget subclasses,
		 * exported from PHP to JS such as is done in MT_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {Object}
		 */
		l10n: {
		 ...
		 * such as is done in MT_Widget_Media_Image::enqueue_admin_scripts(). If not,
		 ...
		 * such as is done in MT_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {string}
		 */
		mime_type: '',
		 ...
		 *
		 * @constructs mt.mediaWidgets.MediaWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.selectedAttachment = new mt.media.model.Attachment();
			 ...
			 * from the start, without having to sync with hidden fields. See <https://core.trac.managertechnology.com.co/pacmec/ticket/33507>.
			 */
			...
			control.displaySettings = new Backbone.Model( _.pick(
				...
				_.keys( mt.media.view.settings.defaultProps )
			) );
		},
		...
		updateSelectedAttachment: function updateSelectedAttachment() {
			...
			if ( 0 === control.model.get( 'attachment_id' ) ) {
				...
				attachment = new mt.media.model.Attachment({
		...
		template: function template() {
			...
			return mt.template( 'widget-media-' + control.id_base + '-control' );
		},
		...
		selectMedia: function selectMedia() {
			...
			selection = new mt.media.model.Selection( selectionModels, { multiple: false } );
			...
			mt.media.frame = mediaFrame; // See mt.media().
			...
			// Disable syncing of attachment changes back to server (except for deletions). See <https://core.trac.managertechnology.com.co/pacmec/ticket/40403>.
			defaultSync = mt.media.model.Attachment.prototype.sync;
			mt.media.model.Attachment.prototype.sync = function( method ) {
				if ( 'delete' === method ) {
			...
			mediaFrame.on( 'close', function onClose() {
				mt.media.model.Attachment.prototype.sync = defaultSync;
			});
		 ...
		 *
		 * @param {mt.media.view.MediaFrame.Select} mediaFrame - Select frame.
		 * @return {Object} Props.
		 */
		getModelPropsFromMediaFrame: function getModelPropsFromMediaFrame( mediaFrame ) {
			...
			_.each( mt.media.view.settings.embedExts, function( ext ) {
	 ...
	 * Media widget model.
	 *
	 * @class    mt.mediaWidgets.MediaWidgetModel
	 * @augments Backbone.Model
	 */
	component.MediaWidgetModel = Backbone.Model.extend(/** @lends mt.mediaWidgets.MediaWidgetModel.prototype */{
		 ...
		 * @return {mt.mediaWidgets.MediaWidgetModel} This model.
		 */
	 ...
	 * @memberOf mt.mediaWidgets
	 *
	 * @type {Object.<string, mt.mediaWidgets.MediaWidgetControl>}
	 */
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		 ...
		 * and replaced with the rendered output of MT_Widget::form() which is
	 ...
	 * When paCMec enqueues this script, it should have an inline script
	 * attached which calls mt.mediaWidgets.init().
	 *
	 * @memberOf mt.mediaWidgets
	 *
	 * @return {void}
