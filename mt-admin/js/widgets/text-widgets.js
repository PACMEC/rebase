/**
 * @output mt-admin/js/widgets/text-widgets.js
 */
...
/**
 * @namespace mt.textWidgets
 */
mt.textWidgets = ( function( $ ) {
	'use strict';
	...
	component.TextWidgetControl = Backbone.View.extend(/** @lends mt.textWidgets.TextWidgetControl.prototype */{
		 ...
		 * Text widget control.
		 *
		 * @constructs mt.textWidgets.TextWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.$el.html( mt.template( 'widget-text-control-fields' ) );

			control.customHtmlWidgetPointer = control.$el.find( '.mt-pointer.custom-html-widget-pointer' );
			...
			control.pasteHtmlPointer = control.$el.find( '.mt-pointer.paste-html-pointer' );
		...
		dismissPointers: function dismissPointers( pointers ) {
			_.each( pointers, function( pointer ) {
				mt.ajax.post( 'dismiss-mt-pointer', {
					pointer: pointer
				});
		...
		openAvailableWidgetsPanel: function openAvailableWidgetsPanel() {
			...
			mt.customize.section.each( function( section ) {
				if ( section.extended( mt.customize.Widgets.SidebarSection ) && section.expanded() ) {
					sidebarControl = mt.customize.control( 'sidebars_widgets[' + section.params.sidebarId + ']' );
				}
			});
			if ( ! sidebarControl ) {
				return;
			...
			setTimeout( function() { // Timeout to prevent click event from causing panel to immediately collapse.
				mt.customize.Widgets.availableWidgetsPanel.open( sidebarControl );
				mt.customize.Widgets.availableWidgetsPanel.$search.val( 'HTML' ).trigger( 'keyup' );
			});
		},
		...
		updateFields: function updateFields() {
			...
			if ( control.fields.text.is( ':visible' ) ) {
				...
				control.editor.setContent( mt.editor.autop( syncInput.val() ) );
			}
		},
		...
		initializeEditor: function initializeEditor() {
			...
			triggerChangeIfDirty = function() {
				var updateWidgetBuffer = 300; // See mt.customize.Widgets.WidgetControl._setupUpdateUI() which uses 250ms for updateWidgetDebounced.
			...
			function buildEditor() {
				...
				// Add or enable the `mtview` plugin.
				$( document ).one( 'mt-before-tinymce-init.text-widget-init', function( event, init ) {
					// If somebody has removed all plugins, they must have a good reason.
					// Keep it that way.
					if ( ! init.plugins ) {
						return;
					} else if ( ! /\bmtview\b/.test( init.plugins ) ) {
						init.plugins += ',mtview';
					}
				} );
				...
				showPointerElement = function( pointerElement ) {
					...
					mt.a11y.speak( pointerElement.find( 'h3, p' ).map( function() {
						return $( this ).text();
					} ).get().join( '\n\n' ) );
				};
	 ...
	 *
	 * @memberOf mt.textWidgets
	 *
	 * @type {Object.<string, mt.textWidgets.TextWidgetControl>}
	 */
	 ...
	 * attached which calls mt.textWidgets.init().
	 *
	 * @memberOf mt.textWidgets
	 *
	 * @return {void}
