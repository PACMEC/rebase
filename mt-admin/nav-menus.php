<?php
/**
 * paCMec Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) ) {
	mt_die( __( 'Your theme does not support navigation menus or widgets.' ) );
}
...
switch ( $action ) {
	case 'add-menu-item':
		...
		if ( isset( $_REQUEST['nav-menu-locations'] ) ) {
			...
			mt_save_nav_menu_items( $nav_menu_selected_id, $_REQUEST['menu-item'] );
		}
	...
	case 'move-down-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			$menus = isset( $_REQUEST['menu'] ) ? array( (int) $_REQUEST['menu'] ) : mt_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );

			if ( ! is_mt_error( $menus ) && ! empty( $menus[0] ) ) {
				$menu_id            = (int) $menus[0];
				$ordered_menu_items = mt_get_nav_menu_items( $menu_id );
				$menu_item_data     = (array) mt_setup_nav_menu_item( get_post( $menu_item_id ) );
				...
				if ( isset( $orders_to_dbids[ $dbids_to_orders[ $menu_item_id ] + 1 ] ) ) {
					...
					$next_item_data = (array) mt_setup_nav_menu_item( get_post( $next_item_id ) );
					...
					if ( ! empty( $menu_item_data['menu_item_parent'] )
						...
						$parent_object = mt_setup_nav_menu_item( get_post( $parent_db_id ) );

						if ( ! is_mt_error( $parent_object ) ) {
						...
						mt_update_post( $menu_item_data );
						mt_update_post( $next_item_data );
					}
	...
	case 'move-up-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			if ( isset( $_REQUEST['menu'] ) ) {
				...
				$menus = mt_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
			}
	...
	case 'delete-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) && mt_delete_post( $menu_item_id, true ) ) {
	...
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			$deletion = mt_delete_nav_menu( $nav_menu_selected_id );
		} else {
	...
	case 'delete_menus':
		...
		foreach ( $_REQUEST['delete_menus'] as $menu_id_to_delete ) {
			...
			$deletion = mt_delete_nav_menu( $menu_id_to_delete );
	...
	case 'update':
		...
		if ( 0 === $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				$_nav_menu_selected_id = mt_update_nav_menu_object( 0, array( 'menu-name' => $new_menu_title ) );

				if ( is_mt_error( $_nav_menu_selected_id ) ) {
					$messages[] = '<div id="message" class="error notice is-dismissible"><p>' . $_nav_menu_selected_id->get_error_message() . '</p></div>';
				} else {
					$_menu_object            = mt_get_nav_menu_object( $_nav_menu_selected_id );
					$nav_menu_selected_id    = $_nav_menu_selected_id;
					$nav_menu_selected_title = $_menu_object->name;

					if ( isset( $_REQUEST['menu-item'] ) ) {
						mt_save_nav_menu_items( $nav_menu_selected_id, absint( $_REQUEST['menu-item'] ) );
					}
					...
					if ( isset( $_REQUEST['zero-menu-state'] ) || ! empty( $_POST['auto-add-pages'] ) ) {
						...
						mt_nav_menu_update_menu_items( $nav_menu_selected_id, $nav_menu_selected_title );
					}
					...
					mt_redirect( admin_url( 'nav-menus.php?menu=' . $_nav_menu_selected_id ) );
					exit;
				}
			} else {
			...
			$_menu_object = mt_get_nav_menu_object( $nav_menu_selected_id );
			...
			if ( ! is_mt_error( $_menu_object ) ) {
				$_nav_menu_selected_id = mt_update_nav_menu_object( $nav_menu_selected_id, array( 'menu-name' => $menu_title ) );
			...
			// Update menu items.
			if ( ! is_mt_error( $_menu_object ) ) {
				$messages = array_merge( $messages, mt_nav_menu_update_menu_items( $_nav_menu_selected_id, $nav_menu_selected_title ) );

				// If the menu ID changed, redirect to the new URL.
				if ( $nav_menu_selected_id !== $_nav_menu_selected_id ) {
					mt_redirect( admin_url( 'nav-menus.php?menu=' . (int) $_nav_menu_selected_id ) );
					exit;
				}
			}
		}
	...
	case 'locations':
		if ( ! $num_locations ) {
			mt_redirect( admin_url( 'nav-menus.php' ) );
			exit;
		}
...
$page_count = mt_count_posts( 'page' );
...
if ( 0 === $menu_count && ! $add_new_screen && ! $one_theme_location_no_menus ) {
	mt_redirect( admin_url( 'nav-menus.php?action=edit&menu=0' ) );
}
...
if ( ! $nav_menu_selected_title && is_nav_menu( $nav_menu_selected_id ) ) {
	$_menu_object            = mt_get_nav_menu_object( $nav_menu_selected_id );
	$nav_menu_selected_title = ! is_mt_error( $_menu_object ) ? $_menu_object->name : '';
}

// Generate truncated menu names.
foreach ( (array) $nav_menus as $key => $_nav_menu ) {
	$nav_menus[ $key ]->truncated_name = mt_html_excerpt( $_nav_menu->name, 40, '&hellip;' );
}
 ...
 *
 * @global int $_mt_nav_menu_max_depth
 */
global $_mt_nav_menu_max_depth;
$_mt_nav_menu_max_depth = 0;

// Calling mt_get_nav_menu_to_edit generates $_mt_nav_menu_max_depth.
if ( is_nav_menu( $nav_menu_selected_id ) ) {
	$menu_items  = mt_get_nav_menu_items( $nav_menu_selected_id, array( 'post_status' => 'any' ) );
	$edit_markup = mt_get_nav_menu_to_edit( $nav_menu_selected_id );
}
 ...
 * @return string
 */
function mt_nav_menu_max_depth( $classes ) {
	global $_mt_nav_menu_max_depth;
	return "$classes menu-max-depth-$_mt_nav_menu_max_depth";
}

add_filter( 'admin_body_class', 'mt_nav_menu_max_depth' );

mt_nav_menu_setup();
mt_initial_nav_menu_meta_boxes();
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/appearance-menus-screen/">Documentation on Menus</a>' ) . '</p>' .
...
?>
<div class="wrap">
	<h1 class="mt-heading-inline"><?php echo esc_html( __( 'Menus' ) ); ?></h1>
	<?php
			...
			esc_url(
				add_query_arg(
					array(
						...
						'return' => urlencode( remove_query_arg( mt_removable_query_args(), mt_unslash( $_SERVER['REQUEST_URI'] ) ) ),
					),
	...
	<nav class="nav-tab-wrapper mt-clearfix" aria-label="<?php esc_attr_e( 'Secondary menu' ); ?>">
	...
	<div id="menu-locations-wrap">
		<form method="post" action="<?php echo esc_url( add_query_arg( array( 'action' => 'locations' ), admin_url( 'nav-menus.php' ) ) ); ?>">
							...
							<select name="menu-locations[<?php echo $_location; ?>]" id="locations-<?php echo $_location; ?>">
								...
								foreach ( $nav_menus as $menu ) :
									...
									<option <?php echo $data_orig; ?> <?php selected( $selected ); ?> value="<?php echo $menu->term_id; ?>">
										<?php echo mt_html_excerpt( $menu->name, 40, '&hellip;' ); ?>
									</option>
								<?php endforeach; ?>
							</select>
			...
			<p class="button-controls mt-clearfix"><?php submit_button( __( 'Save Changes' ), 'primary left', 'nav-menu-locations', false ); ?></p>
			<?php mt_nonce_field( 'save-menu-locations' ); ?>
				...
				<?php foreach ( (array) $nav_menus as $_nav_menu ) : ?>
					<option value="<?php echo esc_attr( $_nav_menu->term_id ); ?>" <?php selected( $_nav_menu->term_id, $nav_menu_selected_id ); ?>>
						...
						if ( ! empty( $menu_locations ) && in_array( $_nav_menu->term_id, $menu_locations, true ) ) {
							 ...
							 */
							$locations_listed_per_menu = absint( apply_filters( 'mt_nav_locations_listed_per_menu', 3 ) );
	...
	<div id="nav-menus-frame" class="mt-clearfix">
	<div id="menu-settings-column" class="metabox-holder<?php echo $metabox_holder_disabled_class; ?>">
		...
		<form id="nav-menu-meta" class="nav-menu-meta" method="post" enctype="multipart/form-data">
			...
			<?php mt_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?>
		...
		<div id="menu-management">
			<form id="update-nav-menu" method="post" enctype="multipart/form-data">
				...
				<div class="menu-edit">
					...
					mt_nonce_field( 'update-nav_menu', 'update-nav-menu-nonce' );
					...
					<div id="nav-menu-header">
						<div class="major-publishing-actions mt-clearfix">
					...
					<div id="post-body">
						<div id="post-body-content" class="mt-clearfix">
							<?php if ( ! $add_new_screen ) : ?>
								...
								<?php
								if ( isset( $edit_markup ) && ! is_mt_error( $edit_markup ) ) {
									echo $edit_markup;
								} else {
									?>
												...
												<?php if ( ! empty( $menu_locations[ $location ] ) && $menu_locations[ $location ] !== $nav_menu_selected_id ) : ?>
													...
													<?php
														printf(
															...
															mt_get_nav_menu_object( $menu_locations[ $location ] )->name
														);
													?>
													</span>
												<?php endif; ?>
