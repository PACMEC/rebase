...
if ( isset( $_POST['action'] ) && 'authorize_application_password' === $_POST['action'] ) {
	...
	if ( isset( $_POST['reject'] ) ) {
		...
		$created = MT_Application_Passwords::create_new_application_password(
			get_current_user_id(),
			array(
				'name'   => $app_name,
				'app_id' => $app_id,
			...
			if ( $success_url ) {
				$redirect = add_query_arg(
					array(
						...
						'user_login' => urlencode( mt_get_current_user()->user_login ),
	...
	if ( $redirect ) {
		// Explicitly not using mt_safe_redirect b/c sends to arbitrary domain.
...
$is_valid = mt_is_authorize_application_password_request_valid( $request, $user );
		...
		<?php if ( $new_password ) : ?>
			<div class="notice notice-success notice-alt below-h2">
				<p class="application-password-display">
					...
					<input id="new-application-password-value" type="text" class="code" readonly="readonly" value="<?php esc_attr( MT_Application_Passwords::chunk_password( $new_password ) ); ?>" />
				</p>
			 ...
			 *
			 * In most cases, this should be used in combination with the {@see 'mt_application_passwords_approve_app_request_success'}
			 ...
			 */
			do_action( 'mt_authorize_application_password_form_approved_no_js', $new_password, $request, $user );
			?>
		<?php else : ?>
			<form action="<?php echo esc_url( admin_url( 'authorize-application.php' ) ); ?>" method="post" class="form-wrap">
				<?php mt_nonce_field( 'authorize_application_password' ); ?>
				...
				<div class="form-field">
					...
					<input type="text" id="app_name" name="app_name" value="<?php echo esc_attr( $app_name ); ?>" placeholder="<?php esc_attr_e( 'paCMec App on My Phone' ); ?>" required />
				</div>
				 ...
				 * @param MT_User $user The user authorizing the application.
				 */
				do_action( 'mt_authorize_application_password_form', $request, $user );
				?>
				...
				<p class="description" id="description-reject">
					...
					if ( $reject_url ) {
						...
						_e( 'You will be returned to the paCMec Dashboard, and no changes will be made.' );
					}
					?>
				</p>
			</form>
