...
if ( ! current_user_can( 'upload_files' ) ) {
	mt_die( __( 'Sorry, you are not allowed to upload files.' ) );
}
...
if ( 'grid' === $mode ) {
	mt_enqueue_media();
	mt_enqueue_script( 'media-grid' );
	mt_enqueue_script( 'media' );

	remove_action( 'admin_head', 'mt_admin_canonical_url' );

	$q = $_GET;
	// Let JS handle this.
	unset( $q['s'] );
	$vars   = mt_edit_attachments_query_vars( $q );
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/media-library-screen/">Documentation on Media Library</a>' ) . '</p>' .
	...
	?>
	<div class="wrap" id="mt-media-grid" data-search="<?php _admin_search_query(); ?>">
...
$mt_list_table = _get_list_table( 'MT_Media_List_Table' );
...
if ( $doaction ) {
	...
	if ( 'delete_all' === $doaction ) {
		$post_ids = $mtdb->get_col( "SELECT ID FROM $mtdb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'detach':
			mt_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			mt_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id_del ) {
				...
				if ( ! mt_delete_attachment( $post_id_del ) ) {
		...
		default:
			...
			$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $doaction, $post_ids ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
	}
...
if ( ! empty( $_GET['trashed'] ) && absint( $_GET['trashed'] ) ) {
	...
	$message               .= ' <a href="' . esc_url( mt_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bulk-media' ) ) . '">' . __( 'Undo' ) . '</a>';
...
$messages[4] = __( 'Media file moved to the Trash.' ) . ' <a href="' . esc_url( mt_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bulk-media' ) ) . '">' . __( 'Undo' ) . '</a>';
