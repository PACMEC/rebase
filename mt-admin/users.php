...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/users-screen/">Documentation on Managing Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/roles-and-capabilities/">Descriptions of Roles and Capabilities</a>' ) . '</p>' .
...
if ( empty( $_REQUEST ) ) {
	$referer = '<input type="hidden" name="mt_http_referer" value="' . esc_attr( mt_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset( $_REQUEST['mt_http_referer'] ) ) {
	$redirect = remove_query_arg( array( 'mt_http_referer', 'updated', 'delete_count' ), mt_unslash( $_REQUEST['mt_http_referer'] ) );
	$referer  = '<input type="hidden" name="mt_http_referer" value="' . esc_attr( $redirect ) . '" />';
} else {
	$redirect = 'users.php';
	$referer  = '';
}
...
switch ( $mt_list_table->current_action() ) {
	...
	case 'promote':
		...
		if ( ! current_user_can( 'promote_users' ) ) {
			mt_die( __( 'Sorry, you are not allowed to edit this user.' ), 403 );
		}
		...
		foreach ( $userids as $id ) {
			...
			if ( $id == $current_user->ID && ! $mt_roles->role_objects[ $role ]->has_cap( 'promote_users' )
		...
		mt_redirect( add_query_arg( 'update', $update, $redirect ) );
		exit;

	case 'dodelete':
		if ( is_multisite() ) {
			mt_die( __( 'User deletion is not allowed from this screen.' ), 400 );
		}
		...
		if ( empty( $_REQUEST['delete_option'] ) ) {
			...
			$url = str_replace( '&amp;', '&', mt_nonce_url( $url, 'bulk-users' ) );
			mt_redirect( $url );
			exit;
		}

		if ( ! current_user_can( 'delete_users' ) ) {
			mt_die( __( 'Sorry, you are not allowed to delete users.' ), 403 );
		}
		...
		foreach ( $userids as $id ) {
			if ( ! current_user_can( 'delete_user', $id ) ) {
				mt_die( __( 'Sorry, you are not allowed to delete that user.' ), 403 );
			}
			...
			switch ( $_REQUEST['delete_option'] ) {
				case 'delete':
					...
					break;
				case 'reassign':
					mt_delete_user( $id, $_REQUEST['reassign_user'] );
					break;
			}
			++$delete_count;
		}
	...
	case 'resetpassword':
		...
		if ( ! current_user_can( 'edit_users' ) ) {
			$errors = new MT_Error( 'edit_users', __( 'Sorry, you are not allowed to edit users.' ) );
		}
		if ( empty( $_REQUEST['users'] ) ) {
			mt_redirect( $redirect );
			exit();
	...
	case 'delete':
		...
		if ( ! current_user_can( 'delete_users' ) ) {
			$errors = new MT_Error( 'edit_users', __( 'Sorry, you are not allowed to delete users.' ) );
		}
		...
		if ( $userids && ! $users_have_content ) {
			if ( $mtdb->get_var( "SELECT ID FROM {$mtdb->posts} WHERE post_author IN( " . implode( ',', $userids ) . ' ) LIMIT 1' ) ) {
				$users_have_content = true;
			} elseif ( $mtdb->get_var( "SELECT link_id FROM {$mtdb->links} WHERE link_owner IN( " . implode( ',', $userids ) . ' ) LIMIT 1' ) ) {
				$users_have_content = true;
			}
		}
	...
	<form method="post" name="updateusers" id="updateusers">
		<?php mt_nonce_field( 'delete-users' ); ?>
		<?php echo $referer; ?>
...
<ul>
		...
		<ul style="list-style:none;">
			 ...
			 *
			 * @param MT_User $current_user MT_User object for the current user.
			 * @param int[]   $userids      Array of IDs for users being deleted.
			 */
	...
	case 'doremove':
		...
		if ( ! is_multisite() ) {
			mt_die( __( 'You can&#8217;t remove users.' ), 400 );
		}
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			mt_die( __( 'Sorry, you are not allowed to remove users.' ), 403 );
		}
	...
	case 'remove':
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			$error = new MT_Error( 'edit_users', __( 'Sorry, you are not allowed to remove users.' ) );
		}
		...
		?>
	<form method="post" name="updateusers" id="updateusers">
		<?php mt_nonce_field( 'remove-users' ); ?>
		<?php echo $referer; ?>
...
<ul>
	...
	default:
		if ( ! empty( $_GET['_mt_http_referer'] ) ) {
			mt_redirect( remove_query_arg( array( '_mt_http_referer', '_mtnonce' ), mt_unslash( $_SERVER['REQUEST_URI'] ) ) );
			exit;
		}

		if ( $mt_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
			$screen   = get_current_screen()->id;
			$sendback = mt_get_referer();
			$userids  = $_REQUEST['users'];

			/** This action is documented in mt-admin/edit.php */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $mt_list_table->current_action(), $userids ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
					...
					if ( $user_id && current_user_can( 'edit_user', $user_id ) ) {
						$message .= sprintf(
							...
							esc_url(
								add_query_arg(
									'mt_http_referer',
									urlencode( mt_unslash( $_SERVER['REQUEST_URI'] ) ),
...
<form method="get">
		...
		<?php $mt_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
