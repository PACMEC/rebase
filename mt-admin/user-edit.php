...
mt_reset_vars( array( 'action', 'user_id', 'mt_http_referer' ) );
...
if ( mt_is_application_passwords_available_for_user( $user_id ) ) {
	mt_enqueue_script( 'application-passwords' );
}
...
$profile_help = '<p>' . __( 'Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using paCMec.' ) . '</p>' .
	'<p>' . __( 'You can change your password, turn on keyboard shortcuts, change the color scheme of your paCMec administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.' ) . '</p>' .
	'<p>' . __( 'You can select the language you wish to use while using the paCMec administration screen without affecting the language site visitors see.' ) . '</p>' .
...
if ( is_multisite()
	...
	mt_die( __( 'Sorry, you are not allowed to edit this user.' ) );
}
...
if ( IS_PROFILE_PAGE && isset( $_GET['newuseremail'] ) && $current_user->ID ) {
	...
	if ( $new_email && hash_equals( $new_email['hash'], $_GET['newuseremail'] ) ) {
		...
		if ( is_multisite() && $mtdb->get_var( $mtdb->prepare( "SELECT user_login FROM {$mtdb->signups} WHERE user_login = %s", $current_user->user_login ) ) ) {
			$mtdb->query( $mtdb->prepare( "UPDATE {$mtdb->signups} SET user_email = %s WHERE user_login = %s", $user->user_email, $current_user->user_login ) );
		}
		mt_update_user( $user );
		delete_user_meta( $current_user->ID, '_new_email' );
		mt_redirect( add_query_arg( array( 'updated' => 'true' ), self_admin_url( 'profile.php' ) ) );
		die();
	} else {
		mt_redirect( add_query_arg( array( 'error' => 'new-email' ), self_admin_url( 'profile.php' ) ) );
	}
...
switch ( $action ) {
	case 'update':
		...
		if ( is_multisite() ) {
			...
			if ( $user->user_login && isset( $_POST['email'] ) && is_email( $_POST['email'] ) && $mtdb->get_var( $mtdb->prepare( "SELECT user_login FROM {$mtdb->signups} WHERE user_login = %s", $user->user_login ) ) ) {
				$mtdb->query( $mtdb->prepare( "UPDATE {$mtdb->signups} SET user_email = %s WHERE user_login = %s", $_POST['email'], $user_login ) );
			}
		}
		...
		if ( ! is_mt_error( $errors ) ) {
			...
			if ( $mt_http_referer ) {
				$redirect = add_query_arg( 'mt_http_referer', urlencode( $mt_http_referer ), $redirect );
			}
	...
	default:
		...
		$sessions = MT_Session_Tokens::get_instance( $profileuser->ID );
...
<div id="message" class="updated notice is-dismissible">
	...
	<?php endif; ?>
			<?php if ( $mt_http_referer && false === strpos( $mt_http_referer, 'user-new.php' ) && ! IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( mt_validate_redirect( esc_url_raw( $mt_http_referer ), self_admin_url( 'users.php' ) ) ); ?>"><?php _e( '&larr; Go to Users' ); ?></a></p>
	<?php endif; ?>
</div>
		<?php endif; ?>
...
<div class="notice notice-error">
	...
	<?php endif; ?>
		...
		<?php endif; ?>
		<?php if ( isset( $errors ) && is_mt_error( $errors ) ) : ?>
...
<form id="your-profile" action="<?php echo esc_url( self_admin_url( IS_PROFILE_PAGE ? 'profile.php' : 'user-edit.php' ) ); ?>" method="post" novalidate="novalidate"
		...
		?>
	>
		<?php mt_nonce_field( 'update-user_' . $user_id ); ?>
		<?php if ( $mt_http_referer ) : ?>
	<input type="hidden" name="mt_http_referer" value="<?php echo esc_url( $mt_http_referer ); ?>" />
		<?php endif; ?>
<p>
...
<table class="form-table" role="presentation">
	...
	<tr class="user-syntax-highlighting-wrap">
		...
		<?php if ( count( $_mt_admin_css_colors ) > 1 && has_action( 'admin_color_scheme_picker' ) ) : ?>
	<tr class="user-admin-color-wrap">
		...
		<?php endif; // End if count ( $_mt_admin_css_colors ) > 1 ?>
	...
	<tr class="user-language-wrap">
		 ...
		 * @since 2.7.0
		 *
		 * @param MT_User $profileuser The current MT_User object.
		 */
...
<tr class="user-display-name-wrap">
	...
	<tr class="user-email-wrap">
		...
		<p>
			...
			printf(
				' <a href="%1$s">%2$s</a>',
				esc_url( mt_nonce_url( self_admin_url( 'profile.php?dismiss=' . $current_user->ID . '_new_email' ), 'dismiss-' . $current_user->ID . '_new_email' ) ),
				__( 'Cancel' )
			);
			?>
		</p>
	...
	<td><input type="url" name="url" id="url" value="<?php echo esc_attr( $profileuser->user_url ); ?>" class="regular-text code" /></td>
		...
		<?php
		foreach ( mt_get_user_contact_methods( $profileuser ) as $name => $desc ) {
			?>
...
<?php endif; ?>
	...
	<tr class="user-sessions-wrap hide-if-no-js">
		 ...
		 * @param MT_User $profileuser User object for the current user to edit.
		 */
...
<tr id="password" class="user-pass1-wrap">
	...
	<td>
		...
		<button type="button" class="button mt-generate-pw hide-if-no-js" aria-expanded="false"><?php _e( 'Set New Password' ); ?></button>
		<div class="mt-pwd hide-if-js">
			<span class="password-input-wrapper">
				<input type="password" name="pass1" id="pass1" class="regular-text" value="" autocomplete="off" data-pw="<?php echo esc_attr( mt_generate_password( 24 ) ); ?>" aria-describedby="pass-strength-result" />
			</span>
			<button type="button" class="button mt-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
				<span class="dashicons dashicons-hidden" aria-hidden="true"></span>
				<span class="text"><?php _e( 'Hide' ); ?></span>
			</button>
			<button type="button" class="button mt-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Cancel password change' ); ?>">
...
<?php endif; ?>
		...
		<?php if ( mt_is_application_passwords_available_for_user( $user_id ) ) : ?>
	<div class="application-passwords hide-if-no-js" id="application-passwords-section">
		...
		<p><?php _e( 'Application passwords allow authentication via non-interactive systems, such as XML-RPC or the REST API, without providing your actual password. Application passwords can be easily revoked. They cannot be used for traditional logins to your website.' ); ?></p>
			...
			if ( ! mt_is_site_protected_by_basic_auth( 'front' ) ) {
				?>
			<div class="create-application-password form-wrap">
				<div class="form-field">
					<label for="new_application_password_name"><?php _e( 'New Application Password Name' ); ?></label>
					<input type="text" size="30" id="new_application_password_name" name="new_application_password_name" placeholder="<?php esc_attr_e( 'paCMec App on My Phone' ); ?>" class="input" aria-required="true" aria-describedby="new_application_password_name_desc" />
				 ...
				 * @param MT_User $profileuser The current MT_User object.
				 */
				do_action( 'mt_create_application_password_form', $profileuser );
				?>
		...
		<div class="application-passwords-list-table-wrapper">
			<?php
			$application_passwords_list_table = _get_list_table( 'MT_Application_Passwords_List_Table', array( 'screen' => 'application-passwords-user' ) );
