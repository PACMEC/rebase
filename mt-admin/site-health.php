...
if ( ! current_user_can( 'view_site_health_checks' ) ) {
	mt_die( __( 'Sorry, you are not allowed to access site health information.' ), '', 403 );
}
...
if ( 'update_https' === $action ) {
	check_admin_referer( 'mt_update_https' );

	if ( ! current_user_can( 'update_https' ) ) {
		mt_die( __( 'Sorry, you are not allowed to update this site to HTTPS.' ), 403 );
	}

	if ( ! mt_is_https_supported() ) {
		mt_die( __( 'It looks like HTTPS is not supported for your website at this point.' ) );
	}

	$result = mt_update_urls_to_https();

	mt_redirect( add_query_arg( 'https_updated', (int) $result, mt_get_referer() ) );
	exit;
}
...
$health_check_site_status->check_mt_version_check_exists();
...
<div class="health-check-body hide-if-no-js">
	...
	<div class="site-status-has-issues">
		...
		<p><?php _e( 'The site health check shows critical information about your paCMec configuration and items that require your attention.' ); ?></p>
