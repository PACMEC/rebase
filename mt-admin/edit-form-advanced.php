...
mt_enqueue_script( 'post' );

$_mt_editor_expand   = false;
...
if ( post_type_supports( $post_type, 'editor' )
	 ...
	 */
	if ( apply_filters( 'mt_editor_expand', true, $post_type ) ) {
		mt_enqueue_script( 'editor-expand' );
		$_content_editor_dfw = true;
		$_mt_editor_expand   = ( 'on' === get_user_setting( 'editor_expand', 'on' ) );
	}
}
...
if ( (int) get_option( 'page_for_posts' ) === $post_ID && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_mt_posts_page_notice' );
...
if ( $thumbnail_support ) {
	add_thickbox();
	mt_enqueue_media( array( 'post' => $post_ID ) );
}
...
$messages['post']       = array(
	...
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Post restored to revision from %s.' ), mt_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page']       = array(
	...
	4  => __( 'Page updated.' ),
	/* translators: %s: Date and time of the revision. */
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Page restored to revision from %s.' ), mt_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _mt_post_revision_fields( $post ) as $autosave_field => $_autosave_field ) {
...
if ( 'post' === $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
			'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/pages-add-new-screen/">Documentation on Adding New Pages</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/pages-screen/">Documentation on Editing Pages</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
	);
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/edit-media/">Documentation on Edit Media</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
	);
}
...
if ( 'post' === $post_type || 'page' === $post_type ) {
	...
	$inserting_media .= '<p>' . __( 'You can also embed media from many popular websites including Twitter, YouTube, Flickr and others by pasting the media URL on its own line into the content of your post/page. <a href="https://managertechnology.com.co/pacmec/support/article/embeds/">Learn more about embeds</a>.' ) . '</p>';
...
if ( 'post' === $post_type ) {
	...
	if ( current_theme_supports( 'post-formats' ) && post_type_supports( 'post', 'post-formats' ) ) {
		$publish_box .= '<li>' . __( '<strong>Format</strong> &mdash; Post Formats designate how your theme will display a specific post. For example, you could have a <em>standard</em> blog post with a title and paragraphs, or a short <em>aside</em> that omits the title and contains a short text blurb. Your theme could enable all or some of 10 possible formats. <a href="https://managertechnology.com.co/pacmec/support/article/post-formats/#supported-formats">Learn more about each post format</a>.' ) . '</li>';
	}
	...
	$discussion_settings  = '<p>' . __( '<strong>Send Trackbacks</strong> &mdash; Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other paCMec sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.' ) . '</p>';
...
?>
>
<?php mt_nonce_field( $nonce_action ); ?>
...
<div class="inside">
	...
	if ( $viewable ) :
		...
		if ( has_filter( 'pre_get_shortlink' ) || has_filter( 'get_shortlink' ) ) {
			$shortlink = mt_get_shortlink( $post->ID, 'post' );
...
if ( post_type_supports( $post_type, 'editor' ) ) {
	$_mt_editor_expand_class = '';
	if ( $_mt_editor_expand ) {
		$_mt_editor_expand_class = ' mt-editor-expand';
	}
	?>
<div id="postdivrich" class="postarea<?php echo $_mt_editor_expand_class; ?>">
	...
	mt_editor(
		...
		array(
			...
			'tinymce'             => array(
				'resize'                  => false,
				'mt_autoresize_on'        => $_mt_editor_expand,
				'add_unload_trigger'      => false,
				'mt_keep_scroll_position' => ! $is_IE,
			),
		)
	);
	?>
<table id="post-status-info"><tbody><tr>
	<td id="mt-word-count" class="hide-if-no-js">
	<?php
	printf(
...
<?php if ( ! mt_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>
