...
if ( ! current_user_can( 'view_site_health_checks' ) ) {
	mt_die( __( 'Sorry, you are not allowed to access the debug data.' ), '', 403 );
}
...
$health_check_site_status = MT_Site_Health::get_instance();
...
<div class="health-check-body health-check-debug-tab hide-if-no-js">
	...
	MT_Debug_Data::check_for_updates();

	$info = MT_Debug_Data::debug_data();
	...
	<p>
		<?php
			...
			printf( __( 'This page can show you every detail about the configuration of your paCMec website. For any improvements that could be made, see the <a href="%s">Site Health Status</a> page.' ), esc_url( admin_url( 'site-health.php' ) ) );
		?>
	</p>
	<p>
	...
	<div class="site-health-copy-buttons">
		<div class="copy-button-wrapper">
			<button type="button" class="button copy-button" data-clipboard-text="<?php echo esc_attr( MT_Debug_Data::format( $info, 'debug' ) ); ?>">
		...
		foreach ( $info as $section => $details ) {
			...
			<h3 class="health-check-accordion-heading">
				<button aria-expanded="false" class="health-check-accordion-trigger" aria-controls="health-check-accordion-block-<?php echo esc_attr( $section ); ?>" type="button">
					...
					if ( 'mt-paths-sizes' === $section ) {
						?>
						<span class="health-check-mt-paths-sizes spinner"></span>
						<?php
					}
