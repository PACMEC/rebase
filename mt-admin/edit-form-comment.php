...
<?php mt_nonce_field( 'update-comment_' . $comment->comment_ID ); ?>
<div class="wrap">
...
<?php
if ( 'approved' === mt_get_comment_status( $comment ) && $comment->comment_post_ID > 0 ) :
...
<?php
	 ...
	 * @param MT_Comment $comment Current comment object.
	 */
...
<?php echo "<a class='submitdelete deletion' href='" . mt_nonce_url( 'comment.php?action=' . ( ! EMPTY_TRASH_DAYS ? 'deletecomment' : 'trashcomment' ) . "&amp;c=$comment->comment_ID&amp;_mt_original_http_referer=" . urlencode( mt_get_referer() ), 'delete-comment_' . $comment->comment_ID ) . "'>" . ( ! EMPTY_TRASH_DAYS ? __( 'Delete Permanently' ) : __( 'Move to Trash' ) ) . "</a>\n"; ?>
</div>
