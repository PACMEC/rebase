...
/** paCMec Administration Widgets API */
require_once ABSPATH . 'mt-admin/includes/widgets.php';
...
if ( 'on' === $widgets_access ) {
	add_filter( 'admin_body_class', 'mt_widgets_access_body_class' );
} else {
	mt_enqueue_script( 'admin-widgets' );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/appearance-widgets-screen/">Documentation on Widgets</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
);

if ( ! current_theme_supports( 'widgets' ) ) {
	mt_die( __( 'The theme you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change. For information on making your theme widget-aware, please <a href="https://developer.managertechnology.com.co/pacmec/themes/functionality/widgets/">follow these instructions</a>.' ) );
}
...
if ( empty( $sidebars_widgets ) ) {
	$sidebars_widgets = mt_get_widget_defaults();
}

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'mt_inactive_widgets' === $sidebar_id ) {
		continue;
	}
...
if ( isset( $_POST['savewidget'] ) || isset( $_POST['removewidget'] ) ) {
	...
	if ( isset( $_POST['removewidget'] ) && $_POST['removewidget'] ) {
		...
		if ( ! in_array( $widget_id, $sidebar, true ) ) {
			mt_redirect( admin_url( 'widgets.php?error=0' ) );
			exit;
		}
	...
	mt_redirect( admin_url( 'widgets.php?message=0' ) );
	exit;
}

// Remove inactive widgets without JS.
if ( isset( $_POST['removeinactivewidgets'] ) ) {
	check_admin_referer( 'remove-inactive-widgets', '_mtnonce_remove_inactive_widgets' );
...
if ( isset( $_GET['editwidget'] ) && $_GET['editwidget'] ) {
	...
	if ( isset( $_GET['addnew'] ) ) {
		...
		$keys    = array_keys( $mt_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset( $_GET['base'] ) && isset( $_GET['num'] ) ) { // Multi-widget.
			// Copy minimal info from an existing instance of this widget to a new instance.
			foreach ( $mt_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					...
					$mt_registered_widget_controls[ $control['id'] ] = $control;
					break;
				}
			}
		}
	...
	if ( isset( $mt_registered_widget_controls[ $widget_id ] ) && ! isset( $control ) ) {
		$control          = $mt_registered_widget_controls[ $widget_id ];
		$control_callback = $control['callback'];
	} elseif ( ! isset( $mt_registered_widget_controls[ $widget_id ] ) && isset( $mt_registered_widgets[ $widget_id ] ) ) {
		$name = esc_html( strip_tags( $mt_registered_widgets[ $widget_id ]['name'] ) );
	}
	...
	if ( ! isset( $sidebar ) ) {
		$sidebar = isset( $_GET['sidebar'] ) ? $_GET['sidebar'] : 'mt_inactive_widgets';
	}
	...
	<?php
	foreach ( $mt_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr( $sbname ) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'mt_inactive_widgets' === $sbname || 'orphaned_widgets' === substr( $sbname, 0, 16 ) ) {
			echo '&nbsp;';
		} else {
	...
	<div class="widget-control-actions">
		...
		<div class="alignright">
			...
			<?php mt_nonce_field( "save-delete-widget-$widget_id" ); ?>
		</div>
		<br class="clear" />
	</div>
...
$nonce = mt_create_nonce( 'widgets-access' );
?>
<div class="widget-access-link">
	<a id="access-on" href="widgets.php?widgets-access=on&_mtnonce=<?php echo urlencode( $nonce ); ?>"><?php _e( 'Enable accessibility mode' ); ?></a><a id="access-off" href="widgets.php?widgets-access=off&_mtnonce=<?php echo urlencode( $nonce ); ?>"><?php _e( 'Disable accessibility mode' ); ?></a>
</div>
...
foreach ( $mt_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( false !== strpos( $registered_sidebar['class'], 'inactive-sidebar' ) || 'orphaned_widgets' === substr( $sidebar, 0, 16 ) ) {
		...
		$is_inactive_widgets = 'mt_inactive_widgets' === $registered_sidebar['id'];
		?>
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php mt_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				...
				<div class="remove-inactive-widgets">
					<form action="" method="post">
						<p>
							...
							if ( empty( $sidebars_widgets['mt_inactive_widgets'] ) ) {
								$attributes['disabled'] = '';
							}
						...
						<?php mt_nonce_field( 'remove-inactive-widgets', '_mtnonce_remove_inactive_widgets' ); ?>
					</form>
				</div>
				<?php } ?>
			</div>
...
<div id="widgets-right" class="mt-clearfix<?php echo $single_sidebar_class; ?>">
...
foreach ( $theme_sidebars as $sidebar => $registered_sidebar ) {
	...
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		...
		mt_list_widget_controls( $sidebar, $registered_sidebar['name'] );
		?>
	</div>
	<?php
...
<form method="post">
<?php mt_nonce_field( 'save-sidebar-widgets', '_mtnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
