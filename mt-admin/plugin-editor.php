...
if ( is_multisite() && ! is_network_admin() ) {
	mt_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit;
}

if ( ! current_user_can( 'edit_plugins' ) ) {
	mt_die( __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
}
...
if ( isset( $_REQUEST['file'] ) ) {
	$file = mt_unslash( $_REQUEST['file'] );
}

if ( isset( $_REQUEST['plugin'] ) ) {
	$plugin = mt_unslash( $_REQUEST['plugin'] );
}
...
if ( 'POST' === $_SERVER['REQUEST_METHOD'] ) {
	$r = mt_edit_theme_plugin_file( mt_unslash( $_POST ) );
	if ( is_mt_error( $r ) ) {
		$edit_error = $r;
		if ( check_ajax_referer( 'edit-plugin_' . $file, 'nonce', false ) && isset( $_POST['newcontent'] ) ) {
			$posted_content = mt_unslash( $_POST['newcontent'] );
		}
	} else {
		mt_redirect(
			add_query_arg(
...
if ( ! is_file( $real_file ) ) {
	mt_die( sprintf( '<p>%s</p>', __( 'File does not exist! Please double check the name and try again.' ) ) );
} else {
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $ext, $editable_extensions, true ) ) {
			mt_die( sprintf( '<p>%s</p>', __( 'Files of this type are not editable.' ) ) );
		}
	}
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/plugins-editor-screen/">Documentation on Editing Plugins</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://developer.managertechnology.com.co/pacmec/plugins/">Documentation on Writing Plugins</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
);

$settings = array(
	'codeEditor' => mt_enqueue_code_editor( array( 'file' => $real_file ) ),
);
mt_enqueue_script( 'mt-theme-plugin-editor' );
mt_add_inline_script( 'mt-theme-plugin-editor', sprintf( 'jQuery( function( $ ) { mt.themePluginEditor.init( $( "#template" ), %s ); } )', mt_json_encode( $settings ) ) );
mt_add_inline_script( 'mt-theme-plugin-editor', sprintf( 'mt.themePluginEditor.themeOrPlugin = "plugin";' ) );

require_once ABSPATH . 'mt-admin/admin-header.php';

update_recently_edited( MT_PLUGIN_DIR . '/' . $file );
...
if ( '.php' === substr( $real_file, strrpos( $real_file, '.' ) ) ) {
	$functions = mt_doc_link_parse( $content );
...
<?php elseif ( is_mt_error( $edit_error ) ) : ?>
...
<div id="templateside">
	...
	<li role="treeitem" tabindex="-1" aria-expanded="true" aria-level="1" aria-posinset="1" aria-setsize="1">
		<ul role="group">
			<?php mt_print_plugin_file_tree( mt_make_plugin_file_tree( $plugin_editable_files ) ); ?>
		</ul>
	</ul>
</div>

<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php mt_nonce_field( 'edit-plugin_' . $file, 'nonce' ); ?>
	<div>
	...
	<?php mt_print_file_editor_templates(); ?>
</form>
<br class="clear" />
</div>
<?php
$dismissed_pointers = explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_mt_pointers', true ) );
if ( ! in_array( 'plugin_editor_notice', $dismissed_pointers, true ) ) :
	// Get a back URL.
	$referer = mt_get_referer();

	$excluded_referer_basenames = array( 'plugin-editor.php', 'mt-login.php' );
		...
		<div class="notification-dialog">
			<div class="file-editor-warning-content">
				<div class="file-editor-warning-message">
					...
					<p><?php _e( 'You appear to be making direct edits to your plugin in the paCMec dashboard. We recommend that you don&#8217;t! Editing plugins directly may introduce incompatibilities that break your site and your changes may be lost in future updates.' ); ?></p>
