 ...
 */
global $mt_scripts, $mt_customize;

if ( $mt_customize->changeset_post_id() ) {
	$changeset_post = get_post( $mt_customize->changeset_post_id() );
	...
	if ( $missed_schedule ) {
		/*
		 * Note that an Ajax request spawns here instead of just calling `mt_publish_post( $changeset_post->ID )`.
		 *
		 * Because MT_Customize_Manager is not instantiated for customize.php with the `settings_previewed=false`
		 ...
		 */
		$nonces       = $mt_customize->get_nonces();
		$request_args = array(
			'nonce'                      => $nonces['save'],
			'customize_changeset_uuid'   => $mt_customize->changeset_uuid(),
			'mt_customize'               => 'on',
			'customize_changeset_status' => 'publish',
		);
		ob_start();
		?>
		<?php mt_print_scripts( array( 'mt-util' ) ); ?>
		<script>
			mt.ajax.post( 'customize_save', <?php echo mt_json_encode( $request_args ); ?> );
		</script>
		<?php
		$script = ob_get_clean();
...
mt_reset_vars( array( 'url', 'return', 'autofocus' ) );
if ( ! empty( $url ) ) {
	$mt_customize->set_preview_url( mt_unslash( $url ) );
}
if ( ! empty( $return ) ) {
	$mt_customize->set_return_url( mt_unslash( $return ) );
}
if ( ! empty( $autofocus ) && is_array( $autofocus ) ) {
	$mt_customize->set_autofocus( mt_unslash( $autofocus ) );
}

$registered             = $mt_scripts->registered;
$mt_scripts             = new MT_Scripts;
$mt_scripts->registered = $registered;

add_action( 'customize_controls_print_scripts', 'print_head_scripts', 20 );
add_action( 'customize_controls_print_footer_scripts', '_mt_footer_scripts' );
...
mt_enqueue_script( 'customize-controls' );
mt_enqueue_style( 'customize-controls' );
...
$body_class = 'mt-core-ui mt-customizer js';

if ( mt_is_mobile() ) :
	$body_class .= ' mobile';
	add_filter( 'admin_viemtort_meta', '_customizer_mobile_viemtort_meta' );
endif;

if ( $mt_customize->is_ios() ) {
	$body_class .= ' ios';
}
...
$admin_title = sprintf( $mt_customize->get_document_title_template(), __( 'Loading&hellip;' ) );

?>
<title><?php echo esc_html( $admin_title ); ?></title>

<script type="text/javascript">
var ajaxurl = <?php echo mt_json_encode( admin_url( 'admin-ajax.php', 'relative' ) ); ?>,
	pagenow = 'customize';
</script>
...
<div class="mt-full-overlay expanded">
	<form id="customize-controls" class="wrap mt-full-overlay-sidebar">
		<div id="customize-header-actions" class="mt-full-overlay-header">
			<?php
			$compatible_mt  = is_mt_version_compatible( $mt_customize->theme()->get( 'RequiresMT' ) );
			$compatible_php = is_php_version_compatible( $mt_customize->theme()->get( 'RequiresPHP' ) );
			?>
			<?php if ( $compatible_mt && $compatible_php ) : ?>
				<?php $save_text = $mt_customize->is_theme_active() ? __( 'Publish' ) : __( 'Activate &amp; Publish' ); ?>
			...
			<a class="customize-controls-close" href="<?php echo esc_url( $mt_customize->get_return_url() ); ?>">
		...
		<div id="widgets-right" class="mt-clearfix"><!-- For Widget Customizer, many widgets try to look for instances under div#widgets-right, so we have to add that ID to a container div in the Customizer for compat -->
			<div id="customize-notifications-area" class="customize-control-notifications-container">
				<ul></ul>
			</div>
			<div class="mt-full-overlay-sidebar-content" tabindex="-1">
		...
		<div id="customize-footer-actions" class="mt-full-overlay-footer">
			...
			<?php $previewable_devices = $mt_customize->get_previewable_devices(); ?>
	...
	<div id="customize-preview" class="mt-full-overlay-main"></div>
	<?php
