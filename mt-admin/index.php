...
/** Load paCMec dashboard API */
require_once ABSPATH . 'mt-admin/includes/dashboard.php';

mt_dashboard_setup();

mt_enqueue_script( 'dashboard' );
...
$help = '<p>' . __( 'Welcome to your paCMec Dashboard! This is the screen you will see when you log in to your site, and gives you access to all the site management features of paCMec. You can get help for any screen by clicking the Help tab above the screen title.' ) . '</p>';
...
$help  = '<p>' . __( 'The left-hand navigation menu provides links to all of the paCMec administration screens, with submenu items displayed on hover. You can minimize this menu to a narrow icon strip by clicking on the Collapse Menu arrow at the bottom.' ) . '</p>';
$help .= '<p>' . __( 'Links in the Toolbar at the top of the screen connect your dashboard and the front end of your site, and provide access to your profile and helpful paCMec information.' ) . '</p>';
...
if ( current_user_can( 'edit_posts' ) ) {
	$help .= '<p>' . __( '<strong>At a Glance</strong> &mdash; Displays a summary of the content on your site and identifies which theme and version of paCMec you are using.' ) . '</p>';
}
...
if ( has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) :
	...
	$hide = ( 0 === $option || ( 2 === $option && mt_get_current_user()->user_email !== get_option( 'admin_email' ) ) );
	if ( $hide ) {
		$classes .= ' hidden';
	}
	?>
	...
	<div id="welcome-panel" class="<?php echo esc_attr( $classes ); ?>">
		<?php mt_nonce_field( 'welcome-panel-nonce', 'welcomepanelnonce', false ); ?>
		 ...
		 *
		 *     remove_action( 'welcome_panel', 'mt_welcome_panel' );
		 *
		 * @since 3.5.0
		 */
...
<?php
mt_print_community_events_templates();
