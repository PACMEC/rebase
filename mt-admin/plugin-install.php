...
if ( is_multisite() && ! is_network_admin() ) {
	mt_redirect( network_admin_url( 'plugin-install.php' ) );
	exit;
}

$mt_list_table = _get_list_table( 'MT_Plugin_Install_List_Table' );
$pagenum       = $mt_list_table->get_pagenum();

if ( ! empty( $_REQUEST['_mt_http_referer'] ) ) {
	$location = remove_query_arg( '_mt_http_referer', mt_unslash( $_SERVER['REQUEST_URI'] ) );
 ...
 */
if ( 'upload' !== $tab ) {
	/** This action is documented in mt-admin/plugin-install.php */
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				'<p>' . __( 'If you know what you&#8217;re looking for, Search is your best bet. The Search screen has options to search the paCMec Plugin Directory for a particular Term, Author, or Tag. You can also search the directory by selecting popular tags. Tags in larger type mean more plugins have been labeled with that tag.' ) . '</p>' .
				...
				'<p>' . __( 'You can also browse a user&#8217;s favorite plugins, by using the Favorites link above the plugins list and entering their paCMec.org username.' ) . '</p>' .
