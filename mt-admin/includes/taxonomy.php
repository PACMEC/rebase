 ...
 * @return int|MT_Error
 */
function mt_create_category( $cat_name, $parent = 0 ) {
	$id = category_exists( $cat_name, $parent );
	if ( $id ) {
		return $id;
	}

	return mt_insert_category(
		array(
			'cat_name'        => $cat_name,
 ...
 */
function mt_create_categories( $categories, $post_id = '' ) {
	$cat_ids = array();
	foreach ( $categories as $category ) {
		...
		if ( $id ) {
			...
			$id = mt_create_category( $category );
			if ( $id ) {
				$cat_ids[] = $id;
			}
		}
	...
	if ( $post_id ) {
		mt_set_post_categories( $post_id, $cat_ids );
	}
 ...
 *
 * @since 2.0.0
 * @since 2.5.0 $mt_error parameter was added.
 ...
 * }
 * @param bool  $mt_error Optional. Default false.
 * @return int|object The ID number of the new or updated Category on success. Zero or a MT_Error on failure,
 *                    depending on param $mt_error.
 */
function mt_insert_category( $catarr, $mt_error = false ) {
	$cat_defaults = array(
		'cat_ID'               => 0,
		'taxonomy'             => 'category',
		'cat_name'             => '',
	...
	$catarr       = mt_parse_args( $catarr, $cat_defaults );

	if ( '' === trim( $catarr['cat_name'] ) ) {
		if ( ! $mt_error ) {
			return 0;
		} else {
			return new MT_Error( 'cat_name', __( 'You did not enter a category name.' ) );
		}
	}
	...
	if ( $update ) {
		$catarr['cat_ID'] = mt_update_term( $catarr['cat_ID'], $catarr['taxonomy'], $args );
	} else {
		$catarr['cat_ID'] = mt_insert_term( $catarr['cat_name'], $catarr['taxonomy'], $args );
	}

	if ( is_mt_error( $catarr['cat_ID'] ) ) {
		if ( $mt_error ) {
...
/**
 * Aliases mt_insert_category() with minimal args.
 *
 ...
 */
function mt_update_category( $catarr ) {
	...
	$category = mt_slash( $category );

	// Merge old and new fields with new fields overwriting old ones.
	$catarr = array_merge( $category, $catarr );

	return mt_insert_category( $catarr );
}
 ...
 */
function mt_create_tag( $tag_name ) {
	return mt_create_term( $tag_name, 'post_tag' );
}
...
function get_terms_to_edit( $post_id, $taxonomy = 'post_tag' ) {
	...
	if ( false === $terms ) {
		$terms = mt_get_object_terms( $post_id, $taxonomy );
		mt_cache_add( $post_id, mt_list_pluck( $terms, 'term_id' ), $taxonomy . '_relationships' );
	}
 ...
 * @return array|MT_Error
 */
function mt_create_term( $tag_name, $taxonomy = 'post_tag' ) {
	$id = term_exists( $tag_name, $taxonomy );
	if ( $id ) {
		return $id;
	}

	return mt_insert_term( $tag_name, $taxonomy );
}
