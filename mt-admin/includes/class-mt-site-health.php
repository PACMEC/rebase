<?php
/**
 * Class for looking up a site's health based on a user's paCMec environment.
 *
 * @package paCMec
...
class MT_Site_Health {
	...
	public function __construct() {
		...
		// Save memory limit before it's affected by mt_raise_memory_limit( 'admin' ).
		$this->php_memory_limit = ini_get( 'memory_limit' );

		$this->timeout_late_cron   = 0;
		$this->timeout_missed_cron = - 5 * MINUTE_IN_SECONDS;

		if ( defined( 'DISABLE_MT_CRON' ) && DISABLE_MT_CRON ) {
		...
		add_action( 'mt_site_health_scheduled_check', array( $this, 'mt_cron_scheduled_check' ) );
	}

	/**
	 * Return an instance of the MT_Site_Health class, or create one if none exist yet.
	 *
	 * @since 5.4.0
	 *
	 * @return MT_Site_Health|null
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new MT_Site_Health();
		}
	...
	public function enqueue_scripts() {
		...
		$health_check_js_variables = array(
			'screen'      => $screen->id,
			'nonce'       => array(
				'site_status'        => mt_create_nonce( 'health-check-site-status' ),
				'site_status_result' => mt_create_nonce( 'health-check-site-status-result' ),
			),
			'site_status' => array(
				'direct' => array(),
				'async'  => array(),
		...
		if ( 'site-health' === $screen->id && ! isset( $_GET['tab'] ) ) {
			$tests = MT_Site_Health::get_tests();
		...
		mt_localize_script( 'site-health', 'SiteHealth', $health_check_js_variables );
	}
	...
	private function prepare_sql_data() {
		...
		if ( $mtdb->use_mysqli ) {
			// phpcs:ignore paCMec.DB.RestrictedFunctions.mysql_mysqli_get_server_info
			$mysql_server_type = mysqli_get_server_info( $mtdb->dbh );
		} else {
			// phpcs:ignore paCMec.DB.RestrictedFunctions.mysql_mysql_get_server_info,PHPCompatibility.Extensions.RemovedExtensions.mysql_DeprecatedRemoved
			$mysql_server_type = mysql_get_server_info( $mtdb->dbh );
		}

		$this->mysql_server_version = $mtdb->get_var( 'SELECT VERSION()' );
	...
	/**
	 * Test if `mt_version_check` is blocked.
	 *
	 * It's possible to block updates with the `mt_version_check` filter, but this can't be checked
	 ...
	 * @since 5.2.0
	 */
	public function check_mt_version_check_exists() {
		if ( ! is_admin() || ! is_user_logged_in() || ! current_user_can( 'update_core' ) || ! isset( $_GET['health-check-test-mt_version_check'] ) ) {
			return;
		}

		echo ( has_filter( 'mt_version_check', 'mt_version_check' ) ? 'yes' : 'no' );

		die();
	}

	/**
	 * Tests for paCMec version and outputs it.
	 *
	...
	public function get_test_pacmec_version() {
		...
		if ( ! is_array( $core_updates ) ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: Your current version of paCMec. */
				__( 'paCMec version %s' ),
			...
			$result['description'] = sprintf(
				'<p>%s</p>',
				__( 'We were unable to check if any new versions of paCMec are available.' )
			);
			...
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					...
					$result['label'] = sprintf(
						/* translators: %s: The latest version of paCMec available. */
						__( 'paCMec update available (%s)' ),
						$update->version
					);
					...
					$result['actions'] = sprintf(
						...
						__( 'Install the latest version of paCMec' )
					);
					...
					if ( $current_major !== $new_major ) {
						...
						$result['description'] = sprintf(
							'<p>%s</p>',
							__( 'A new version of paCMec is available.' )
						);
					} else {
					...
					$result['status'] = 'good';
					$result['label']  = sprintf(
						/* translators: %s: The current version of paCMec installed on this site. */
						__( 'Your version of paCMec (%s) is up to date' ),
						$core_current_version
					);

					$result['description'] = sprintf(
						'<p>%s</p>',
						__( 'You are currently running the latest version of paCMec available, keep it up!' )
					);
				}
			}
		}
	...
	public function get_test_theme_version() {
		...
		$all_themes   = mt_get_themes();
		$active_theme = mt_get_theme();

		// If MT_DEFAULT_THEME doesn't exist, fall back to the latest core default theme.
		$default_theme = mt_get_theme( MT_DEFAULT_THEME );
		if ( ! $default_theme->exists() ) {
			$default_theme = MT_Theme::get_core_default_theme();
		}
			...
			if ( $active_theme->parent() ) {
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							/* translators: 1: The default theme for paCMec. 2: The currently active theme. 3: The active theme's parent theme. */
							__( 'To enhance your site&#8217;s security, we recommend you remove any themes you&#8217;re not using. You should keep %1$s, the default paCMec theme, %2$s, your current theme, and %3$s, its parent theme.' ),
							$default_theme ? $default_theme->name : MT_DEFAULT_THEME,
							$active_theme->name,
							$active_theme->parent()->name
						)
					);
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						'<p>%s %s</p>',
						sprintf(
							/* translators: 1: The amount of inactive themes. 2: The default theme for paCMec. 3: The currently active theme. */
							_n(
								'Your site has %1$d inactive theme, other than %2$s, the default paCMec theme, and %3$s, your active theme.',
								'Your site has %1$d inactive themes, other than %2$s, the default paCMec theme, and %3$s, your active theme.',
								$themes_inactive
							),
							$themes_inactive,
							$default_theme ? $default_theme->name : MT_DEFAULT_THEME,
		...
		if ( ! $has_default_theme ) {
			...
			$result['description'] .= sprintf(
				'<p>%s</p>',
				__( 'Your site does not have any default theme. Default themes are used by paCMec automatically if anything is wrong with your chosen theme.' )
			);
		}
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'PHP is the programming language used to build and maintain paCMec. Newer versions of PHP are created with increased performance in mind, so you may see a positive effect on your site&#8217;s performance. The minimum recommended version of PHP is %s.' ),
	...
	public function get_test_php_default_timezone() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'PHP default timezone was configured by paCMec on loading. This is necessary for correct calculations of dates and times.' )
			),
			'actions'     => '',
		...
		if ( 'UTC' !== date_default_timezone_get() ) {
			...
			$result['description'] = sprintf(
				...
				sprintf(
					...
					__( 'PHP default timezone was changed after paCMec loading by a %s function call. This interferes with correct calculations of dates and times.' ),
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The SQL server is a required piece of software for the database paCMec uses to store all your site&#8217;s content and settings.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s <span class="screen-reader-text">%s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
		...
		$db_dropin = file_exists( MT_CONTENT_DIR . '/db.php' );
		...
		if ( ! $this->mysql_min_version_check ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'paCMec requires %1$s version %2$s or higher. Contact your web hosting company to correct this.' ),
					( $this->is_mariadb ? 'MariaDB' : 'MySQL' ),
	...
	public function get_test_utf8mb4_support() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'UTF8MB4 is the character set paCMec prefers for database storage because it safely supports the widest set of characters and encodings, including Emoji, enabling better support for non-English languages.' )
			),
			'actions'     => '',
		...
		if ( ! $this->is_mariadb ) {
			if ( version_compare( $this->mysql_server_version, '5.5.3', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						__( 'paCMec&#8217; utf8mb4 support requires MySQL version %s or greater. Please contact your server administrator.' ),
						'5.5.3'
					)
				);
			} else {
			...
			if ( version_compare( $this->mysql_server_version, '5.5.0', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						/* translators: %s: Version number. */
						__( 'paCMec&#8217; utf8mb4 support requires MariaDB version %s or greater. Please contact your server administrator.' ),
						'5.5.0'
					)
				);
			} else {
		...
		if ( $mtdb->use_mysqli ) {
			// phpcs:ignore paCMec.DB.RestrictedFunctions.mysql_mysqli_get_client_info
			$mysql_client_version = mysqli_get_client_info();
		} else {
			// phpcs:ignore paCMec.DB.RestrictedFunctions.mysql_mysql_get_client_info,PHPCompatibility.Extensions.RemovedExtensions.mysql_DeprecatedRemoved
			...
			if ( version_compare( $mysql_client_version, '5.0.9', '<' ) ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						__( 'paCMec&#8217; utf8mb4 support requires MySQL client library (%1$s) version %2$s or newer. Please contact your server administrator.' ),
						'mysqlnd',
						'5.0.9'
					)
				);
	...
	/**
	 * Test if the site can communicate with paCMec.org.
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	...
	public function get_test_dotorg_communication() {
		$result = array(
			'label'       => __( 'Can communicate with paCMec.org' ),
			'status'      => '',
			'badge'       => array(
				'label' => __( 'Security' ),
				'color' => 'blue',
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Communicating with the paCMec servers is used to check for new versions, and to both install and update paCMec core, themes or plugins.' )
			),
			'actions'     => '',
		...
		if ( ! is_mt_error( $mt_dotorg ) ) {
			...
			$result['label'] = __( 'Could not reach paCMec.org' );
	 ...
	 *
	 * When MT_DEBUG is enabled, errors and information may be disclosed to site visitors,
	...
	public function get_test_is_in_debug_mode() {
		...
		if ( defined( 'MT_DEBUG' ) && MT_DEBUG ) {
			if ( defined( 'MT_DEBUG_LOG' ) && MT_DEBUG_LOG ) {
				...
				$result['description'] .= sprintf(
					...
					sprintf(
						...
						'<code>MT_DEBUG_LOG</code>'
					)
				);
			}

			if ( defined( 'MT_DEBUG_DISPLAY' ) && MT_DEBUG_DISPLAY ) {
				...
				$result['description'] .= sprintf(
					'<p>%s</p>',
					sprintf(
						/* translators: 1: MT_DEBUG_DISPLAY, 2: MT_DEBUG */
	 ...
	 *
	 * @since 5.2.0
	 * @since 5.7.0 Updated to rely on {@see mt_is_using_https()} and {@see mt_is_https_supported()}.
	 *
	 * @return array The test results.
	 */
	public function get_test_https_status() {
		...
		$default_update_url = mt_get_default_update_https_url();
		...
		if ( ! mt_is_using_https() ) {
			...
			if ( mt_is_site_url_using_https() ) {
				...
				if ( is_ssl() ) {
					$result['description'] = sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: URL to Settings > General > paCMec Address, 2: URL to Settings > General > Site Address. */
							__( 'You are accessing this website using HTTPS, but your <a href="%1$s">paCMec Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS by default.' ),
					...
					$result['description'] = sprintf(
						...
						sprintf(
							/* translators: 1: URL to Settings > General > paCMec Address, 2: URL to Settings > General > Site Address. */
							__( 'Your <a href="%1$s">paCMec Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS.' ),
							esc_url( admin_url( 'options-general.php' ) . '#siteurl' ),
							esc_url( admin_url( 'options-general.php' ) . '#home' )
						)
					);
			...
			if ( mt_is_https_supported() ) {
				$result['description'] .= sprintf(
					'<p>%s</p>',
					__( 'HTTPS is already supported for your website.' )
				);

				if ( defined( 'MT_HOME' ) || defined( 'MT_SITEURL' ) ) {
					$result['description'] .= sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: mt-config.php, 2: MT_HOME, 3: MT_SITEURL */
							__( 'However, your paCMec Address is currently controlled by a PHP constant and therefore cannot be updated. You need to edit your %1$s and remove or update the definitions of %2$s and %3$s.' ),
							'<code>mt-config.php</code>',
							'<code>MT_HOME</code>',
							'<code>MT_SITEURL</code>'
						)
					);
				} elseif ( current_user_can( 'update_https' ) ) {
					$default_direct_update_url = add_query_arg( 'action', 'update_https', mt_nonce_url( admin_url( 'site-health.php' ), 'mt_update_https' ) );
					$direct_update_url         = mt_get_direct_update_https_url();
				...
				$update_url = mt_get_update_https_url();
			...
			if ( ! empty( $https_detection_errors['ssl_verification_failed'] ) ) {
				$result['description'] = sprintf(
					'<p>%s</p>',
					sprintf(
						/* translators: %s: URL to Settings > General > paCMec Address. */
						__( 'Your <a href="%s">paCMec Address</a> is set up to use HTTPS, but the SSL certificate appears to be invalid.' ),
						esc_url( admin_url( 'options-general.php' ) . '#siteurl' )
					)
				);
			} else {
				$result['description'] = sprintf(
					...
					sprintf(
						/* translators: %s: URL to Settings > General > paCMec Address. */
						__( 'Your <a href="%s">paCMec Address</a> is set up to use HTTPS, but your website appears to be unavailable when using an HTTPS connection.' ),
						esc_url( admin_url( 'options-general.php' ) . '#siteurl' )
					)
				);
			}
	...
	public function get_test_ssl_support() {
		...
		$supports_https = mt_http_supports( array( 'ssl' ) );
	 ...
	 *
	 * If scheduled events are not running, this may indicate something with MT_Cron is not working
	...
	public function get_test_scheduled_events() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Scheduled events are what periodically looks for updates to plugins, themes and paCMec itself. It is also what makes sure scheduled posts are published on time. It may also be used by various plugins to make sure that planned actions are executed.' )
			),
			'actions'     => '',
			'test'        => 'scheduled_events',
		);

		$this->mt_schedule_test_init();

		if ( is_mt_error( $this->has_missed_cron() ) ) {
			$result['status'] = 'critical';
	...
	/**
	 * Test if paCMec can run automated background updates.
	 *
	 * Background updates in paCMec are primarily used for minor releases and security updates.
	...
	public function get_test_background_updates() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Background updates ensure that paCMec can auto-update if a security update is released for the version you are currently using.' )
			),
			'actions'     => '',
			'test'        => 'background_updates',
		);

		if ( ! class_exists( 'MT_Site_Health_Auto_Updates' ) ) {
			require_once ABSPATH . 'mt-admin/includes/class-mt-site-health-auto-updates.php';
		}

		// Run the auto-update tests in a separate class,
		// as there are many considerations to be made.
		$automatic_updates = new MT_Site_Health_Auto_Updates();
	 ...
	 *
	 * A loopback is when paCMec queries itself, for example to start a new MT_Cron instance,
	...
	public function get_test_http_requests() {
		...
		if ( defined( 'MT_HTTP_BLOCK_EXTERNAL' ) && MT_HTTP_BLOCK_EXTERNAL ) {
			$blocked = true;
		}

		if ( defined( 'MT_ACCESSIBLE_HOSTS' ) ) {
			$hosts = explode( ',', MT_ACCESSIBLE_HOSTS );
		}
		...
		if ( $blocked && 0 === count( $hosts ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					'<code>MT_HTTP_BLOCK_EXTERNAL</code>'
				)
			);
		}
	...
	public function get_test_rest_availability() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The REST API is one way paCMec, and other applications, communicate with the server. One example is the block editor screen, which relies on this to display, and save, your posts and pages.' )
			),
			'actions'     => '',
			'test'        => 'rest_availability',
		);

		$cookies = mt_unslash( $_COOKIE );
		$timeout = 10;
		$headers = array(
			'Cache-Control' => 'no-cache',
			'X-MT-Nonce'    => mt_create_nonce( 'mt_rest' ),
		);
		/** This filter is documented in mt-includes/class-mt-http-streams.php */
		$sslverify = apply_filters( 'https_local_ssl_verify', false );

		// Include Basic auth in loopback requests.
		if ( isset( $_SERVER['PHP_AUTH_USER'] ) && isset( $_SERVER['PHP_AUTH_PW'] ) ) {
			$headers['Authorization'] = 'Basic ' . base64_encode( mt_unslash( $_SERVER['PHP_AUTH_USER'] ) . ':' . mt_unslash( $_SERVER['PHP_AUTH_PW'] ) );
		}

		$url = rest_url( 'mt/v2/types/post' );
		...
		$r = mt_remote_get( $url, compact( 'cookies', 'headers', 'timeout', 'sslverify' ) );
		...
		if ( is_mt_error( $r ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					sprintf(
						/* translators: 1: The paCMec error message. 2: The paCMec error code. */
						__( 'Error: %1$s (%2$s)' ),
						$r->get_error_message(),
						$r->get_error_code()
					)
		...
		} elseif ( 200 !== mt_remote_retrieve_response_code( $r ) ) {
			$result['status'] = 'recommended';
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					mt_remote_retrieve_response_code( $r ),
					esc_html( mt_remote_retrieve_body( $r ) )
				)
			);
		} else {
			$json = json_decode( mt_remote_retrieve_body( $r ), true );
	...
	public function get_test_file_uploads() {
		...
		if ( mt_convert_hr_to_bytes( $post_max_size ) < mt_convert_hr_to_bytes( $upload_max_filesize ) ) {
			$result['label'] = sprintf(
			...
			if ( 0 === mt_convert_hr_to_bytes( $post_max_size ) ) {
				$result['description'] = sprintf(
					'<p>%s</p>',
					sprintf(
						/* translators: 1: post_max_size, 2: upload_max_filesize */
	...
	public static function get_tests() {
		$tests = array(
			...
			'async'  => array(
				'dotorg_communication' => array(
					'label'             => __( 'Communication with paCMec.org' ),
					'test'              => rest_url( 'mt-site-health/v1/tests/dotorg-communication' ),
					'has_rest'          => true,
					'async_direct_test' => array( MT_Site_Health::get_instance(), 'get_test_dotorg_communication' ),
				),
				'background_updates'   => array(
					'label'             => __( 'Background updates' ),
					'test'              => rest_url( 'mt-site-health/v1/tests/background-updates' ),
					'has_rest'          => true,
					'async_direct_test' => array( MT_Site_Health::get_instance(), 'get_test_background_updates' ),
				),
				'loopback_requests'    => array(
					'label'             => __( 'Loopback request' ),
					'test'              => rest_url( 'mt-site-health/v1/tests/loopback-requests' ),
					'has_rest'          => true,
					'async_direct_test' => array( MT_Site_Health::get_instance(), 'get_test_loopback_requests' ),
				),
				'https_status'         => array(
					'label'             => __( 'HTTPS status' ),
					'test'              => rest_url( 'mt-site-health/v1/tests/https-status' ),
					'has_rest'          => true,
					'async_direct_test' => array( MT_Site_Health::get_instance(), 'get_test_https_status' ),
				),
				'authorization_header' => array(
					'label'     => __( 'Authorization header' ),
					'test'      => rest_url( 'mt-site-health/v1/tests/authorization-header' ),
					'has_rest'  => true,
		 ...
		 * both the paCMec Hosting Team, but also web standards in general.
		 *
		 ...
		 * to complete should run asynchronously, to avoid extended loading periods within mt-admin.
		 *
		 * @since 5.2.0
	...
	/**
	 * Initiate the MT_Cron schedule test cases.
	 *
	 * @since 5.2.0
	 */
	private function mt_schedule_test_init() {
		$this->schedules = mt_get_schedules();
		$this->get_cron_tasks();
	}
	...
	private function get_cron_tasks() {
		...
		if ( empty( $cron_tasks ) ) {
			$this->crons = new MT_Error( 'no_tasks', __( 'No scheduled events exist on this site.' ) );
			return;
		}
	 ...
	 *
	 * If the list of crons is an instance of MT_Error, returns the instance instead of a boolean value.
	 *
	 * @since 5.2.0
	 *
	 * @return bool|MT_Error True if a cron was missed, false if not. MT_Error if the cron is set to that.
	 */
	public function has_missed_cron() {
		if ( is_mt_error( $this->crons ) ) {
			return $this->crons;
		}
	 ...
	 * @since 5.3.0
	 *
	 * @return bool|MT_Error True if a cron is late, false if not. MT_Error if the cron is set to that.
	 */
	...
	function detect_plugin_theme_auto_update_issues() {
		...
		$test_plugins_enabled = mt_is_auto_update_forced_for_item( 'plugin', true, $mock_plugin );
		$test_themes_enabled  = mt_is_auto_update_forced_for_item( 'theme', true, $mock_theme );

		$ui_enabled_for_plugins = mt_is_auto_update_enabled_for_type( 'plugin' );
		$ui_enabled_for_themes  = mt_is_auto_update_enabled_for_type( 'theme' );
	 ...
	 *
	 * Loopbacks are what paCMec uses to communicate with itself to start up MT_Cron, scheduled posts,
	...
	function can_perform_loopback() {
		...
		if ( 200 !== mt_remote_retrieve_response_code( $r ) ) {
			return (object) array(
				'status'  => 'recommended',
				'message' => sprintf(
	...
	public function maybe_create_scheduled_event() {
		if ( ! mt_next_scheduled( 'mt_site_health_scheduled_check' ) && ! mt_installing() ) {
			mt_schedule_event( time() + DAY_IN_SECONDS, 'weekly', 'mt_site_health_scheduled_check' );
		}
	}
	 ...
	 * @since 5.4.0
	 */
	public function mt_cron_scheduled_check() {
		// Bootstrap mt-admin, as MT_Cron doesn't do this for us.
		require_once trailingslashit( ABSPATH ) . 'mt-admin/includes/admin.php';
		...
		foreach ( $tests['async'] as $test ) {
			...
			if ( is_string( $test['test'] ) ) {
				...
				if ( isset( $test['has_rest'] ) && $test['has_rest'] ) {
					$result_fetch = mt_remote_get(
						$test['test'],
						array(
							'body' => array(
								'_mtnonce' => mt_create_nonce( 'mt_rest' ),
							),
						)
					);
				} else {
					$result_fetch = mt_remote_post(
						admin_url( 'admin-ajax.php' ),
						array(
							'body' => array(
								'action'   => $test['test'],
								'_mtnonce' => mt_create_nonce( 'health-check-site-status' ),
							),
						)
					);
				}

				if ( ! is_mt_error( $result_fetch ) && 200 === mt_remote_retrieve_response_code( $result_fetch ) ) {
					$result = json_decode( mt_remote_retrieve_body( $result_fetch ), true );
				} else {
					$result = false;
				}
		...
		set_transient( 'health-check-site-status-result', mt_json_encode( $site_status ) );
	}
	...
	public function is_development_environment() {
		return in_array( mt_get_environment_type(), array( 'development', 'local' ), true );
	}
