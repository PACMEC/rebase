<?php
/**
 * Template paCMec Administration API.
 *
...
require_once ABSPATH . 'mt-admin/includes/class-walker-category-checklist.php';

/** MT_Internal_Pointers class */
require_once ABSPATH . 'mt-admin/includes/class-mt-internal-pointers.php';
 ...
 */
function mt_category_checklist( $post_id = 0, $descendants_and_self = 0, $selected_cats = false, $popular_cats = false, $walker = null, $checked_ontop = true ) {
	mt_terms_checklist(
		$post_id,
		array(
			'taxonomy'             => 'category',
 ...
 *
 * Taxonomy-independent version of mt_category_checklist().
 *
 * @since 3.0.0
 ...
 */
function mt_terms_checklist( $post_id = 0, $args = array() ) {
	$defaults = array(
	 ...
	 */
	$params = apply_filters( 'mt_terms_checklist_args', $args, $post_id );

	$parsed_args = mt_parse_args( $params, $defaults );
	...
	if ( is_array( $parsed_args['selected_cats'] ) ) {
		...
		$args['selected_cats'] = mt_get_object_terms( $post_id, $taxonomy, array_merge( $args, array( 'fields' => 'ids' ) ) );
	} else {
 ...
 */
function mt_popular_terms_checklist( $taxonomy, $default = 0, $number = 10, $echo = true ) {
	$post = get_post();

	if ( $post && $post->ID ) {
		$checked_terms = mt_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );
	} else {
		$checked_terms = array();
	}
	...
	foreach ( (array) $terms as $term ) {
		...
		<li id="<?php echo $id; ?>" class="popular-category">
			<label class="selectit">
				...
				<?php
				/** This filter is documented in mt-includes/category-template.php */
 ...
 * @param int $link_id
 */
function mt_link_category_checklist( $link_id = 0 ) {
	$default = 1;

	$checked_categories = array();

	if ( $link_id ) {
		$checked_categories = mt_get_link_cats( $link_id );
...
<div class="hidden" id="inline_' . $post->ID . '">
	...
	foreach ( $taxonomy_names as $taxonomy_name ) {
		...
		if ( $taxonomy->hierarchical && $taxonomy->show_ui ) {
			...
			if ( false === $terms ) {
				$terms = mt_get_object_terms( $post->ID, $taxonomy_name );
				mt_cache_add( $post->ID, mt_list_pluck( $terms, 'term_id' ), $taxonomy_name . '_relationships' );
			}
			$term_ids = empty( $terms ) ? array() : mt_list_pluck( $terms, 'term_id' );
	 ...
	 * @since 4.9.8
	 *
	 * @param MT_Post      $post             The current post object.
	 * @param MT_Post_Type $post_type_object The current post's post type object.
	 */
 ...
 * @since 2.7.0
 *
 * @global MT_List_Table $mt_list_table
 *
 * @param int    $position
 * @param bool   $checkbox
 * @param string $mode
 ...
 */
function mt_comment_reply( $position = 1, $checkbox = false, $mode = 'single', $table_row = true ) {
	global $mt_list_table;
	/**
	...
	if ( ! $mt_list_table ) {
		if ( 'single' === $mode ) {
			$mt_list_table = _get_list_table( 'MT_Post_Comments_List_Table' );
		} else {
			$mt_list_table = _get_list_table( 'MT_Comments_List_Table' );
		}
	}
...
<form method="get">
	<?php if ( $table_row ) : ?>
<table style="display:none;"><tbody id="com-reply"><tr id="replyrow" class="inline-edit-row" style="display:none;"><td colspan="<?php echo $mt_list_table->get_column_count(); ?>" class="colspanchange">
<?php else : ?>
...
<?php endif; ?>
	...
	<?php
		mt_nonce_field( 'replyto-comment', '_ajax_nonce-replyto-comment', false );
	if ( current_user_can( 'unfiltered_html' ) ) {
		mt_nonce_field( 'unfiltered-html-comment', '_mt_unfiltered_html_comment', false );
	}
	?>
	</fieldset>
	<?php if ( $table_row ) : ?>
 ...
 * @since 2.9.0
 */
function mt_comment_trashnotice() {
	?>
...
<table id="list-table" style="display: none;">
	...
	<tbody id="the-list" data-mt-lists="list:meta">
	<tr><td></td></tr>
	</tbody>
...
function _list_meta_row( $entry, &$count ) {
	...
	if ( ! $update_nonce ) {
		$update_nonce = mt_create_nonce( 'add-meta' );
	}
	...
	$delete_nonce = mt_create_nonce( 'delete-meta_' . $entry['meta_id'] );

	$r .= "\n\t<tr id='meta-{$entry['meta_id']}'>";
	$r .= "\n\t\t<td class='left'><label class='screen-reader-text' for='meta-{$entry['meta_id']}-key'>" . __( 'Key' ) . "</label><input name='meta[{$entry['meta_id']}][key]' id='meta-{$entry['meta_id']}-key' type='text' size='20' value='{$entry['meta_key']}' />";

	$r .= "\n\t\t<div class='submit'>";
	$r .= get_submit_button( __( 'Delete' ), 'deletemeta small', "deletemeta[{$entry['meta_id']}]", false, array( 'data-mt-lists' => "delete:the-list:meta-{$entry['meta_id']}::_ajax_nonce=$delete_nonce" ) );
	$r .= "\n\t\t";
	$r .= get_submit_button( __( 'Update' ), 'updatemeta small', "meta-{$entry['meta_id']}-submit", false, array( 'data-mt-lists' => "add:the-list:meta-{$entry['meta_id']}::_ajax_nonce-add-meta=$update_nonce" ) );
	$r .= '</div>';
	$r .= mt_nonce_field( 'change-meta', '_ajax_nonce', false, false );
	$r .= '</td>';
 ...
 *
 * @param MT_Post $post Optional. The post being edited.
 */
...
<div class="submit">
	...
	submit_button(
		...
		array(
			'id'            => 'newmeta-submit',
			'data-mt-lists' => 'add:the-list:newmeta',
		)
	);
	?>
</div>
	<?php mt_nonce_field( 'add-meta', '_ajax_nonce-add-meta', false ); ?>
</td></tr>
</tbody>
</table>
	<?php
...
function touch_time( $edit = 1, $for_post = 1, $tab_index = 0, $multi = 0 ) {
	...
	for ( $i = 1; $i < 13; $i = $i + 1 ) {
		...
		$monthtext = $mt_locale->get_month_abbrev( $mt_locale->get_month( $i ) );
...
function parent_dropdown( $default = 0, $parent = 0, $level = 0, $post = null ) {
	...
	$post  = get_post( $post );
	$items = $mtdb->get_results( $mtdb->prepare( "SELECT ID, post_parent, post_title FROM $mtdb->posts WHERE post_parent = %d AND post_type = 'page' ORDER BY menu_order", $parent ) );
 ...
 */
function mt_dropdown_roles( $selected = '' ) {
	$r = '';
 ...
 */
function mt_import_upload_form( $action ) {
	 ...
	 * @since 2.3.0
	 *
	 * @see mt_max_upload_size()
	 *
	 * @param int $max_upload_size Allowed upload size. Default 1 MB.
	 */
	$bytes      = apply_filters( 'import_upload_size_limit', mt_max_upload_size() );
	...
	if ( ! empty( $upload_dir['error'] ) ) :
		...
		?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" class="mt-upload-form" action="<?php echo esc_url( mt_nonce_url( $action, 'import-upload' ) ); ?>">
<p>
		<?php
		printf(
 ...
 * @param string|array|MT_Screen $screen        Optional. The screen or screens on which to show the box
 *                                              (such as a post type, 'link', or 'comment'). Accepts a single
 *                                              screen ID, MT_Screen object, or array of screen IDs. Default
...
function add_meta_box( $id, $title, $callback, $screen = null, $context = 'advanced', $priority = 'default', $callback_args = null ) {
	...
	if ( ! isset( $mt_meta_boxes ) ) {
		$mt_meta_boxes = array();
	}
	if ( ! isset( $mt_meta_boxes[ $page ] ) ) {
		$mt_meta_boxes[ $page ] = array();
	}
	if ( ! isset( $mt_meta_boxes[ $page ][ $context ] ) ) {
		$mt_meta_boxes[ $page ][ $context ] = array();
	}

	foreach ( array_keys( $mt_meta_boxes[ $page ] ) as $a_context ) {
		foreach ( array( 'high', 'core', 'default', 'low' ) as $a_priority ) {
			if ( ! isset( $mt_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ] ) ) {
				continue;
			}

			// If a core box was previously removed, don't add.
			if ( ( 'core' === $priority || 'sorted' === $priority )
				&& false === $mt_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]
			) {
				return;
			}
			...
			if ( 'core' === $priority ) {
				...
				if ( 'default' === $a_priority ) {
					$mt_meta_boxes[ $page ][ $a_context ]['core'][ $id ] = $mt_meta_boxes[ $page ][ $a_context ]['default'][ $id ];
					unset( $mt_meta_boxes[ $page ][ $a_context ]['default'][ $id ] );
				}
				return;
			}
			...
			if ( empty( $priority ) ) {
				...
				$title         = $mt_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]['title'];
				$callback      = $mt_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]['callback'];
				$callback_args = $mt_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]['args'];
			}

			// An ID can be in only one priority and one context.
			if ( $priority !== $a_priority || $context !== $a_context ) {
				unset( $mt_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ] );
			}
		}
	}
	...
	if ( ! isset( $mt_meta_boxes[ $page ][ $context ][ $priority ] ) ) {
		$mt_meta_boxes[ $page ][ $context ][ $priority ] = array();
	}

	$mt_meta_boxes[ $page ][ $context ][ $priority ][ $id ] = array(
		'id'       => $id,
		'title'    => $title,
		'callback' => $callback,
		'args'     => $callback_args,
...
function do_block_editor_incompatible_meta_box( $object, $box ) {
	...
	if ( empty( $plugins['classic-editor/classic-editor.php'] ) ) {
		...
		if ( current_user_can( 'activate_plugins' ) ) {
			$activate_url = mt_nonce_url( self_admin_url( 'plugins.php?action=activate&plugin=classic-editor/classic-editor.php' ), 'activate-plugin_classic-editor/classic-editor.php' );
			echo '<p>';
			/* translators: %s: A link to activate the Classic Editor plugin. */
			printf( __( 'Please activate the <a href="%s">Classic Editor plugin</a> to use this meta box.' ), esc_url( $activate_url ) );
			echo '</p>';
		}
	} elseif ( $object instanceof MT_Post ) {
...
function _get_plugin_from_callback( $callback ) {
	...
	if ( ! $reflection->isInternal() ) {
		...
		$filename   = mt_normalize_path( $reflection->getFileName() );
		$plugin_dir = mt_normalize_path( MT_PLUGIN_DIR );
 ...
 * @global array $mt_meta_boxes
 *
 * @param string|MT_Screen $screen  The screen identifier. If you have used add_menu_page() or
 *                                  add_submenu_page() to create a new screen (and hence screen_id)
 *                                  make sure your menu slug conforms to the limits of sanitize_key()
 *                                  otherwise the 'screen' menu may not correctly render on your page.
 ...
 *                                  example a `MT_Post` or `MT_Comment` object.
...
function do_meta_boxes( $screen, $context, $object ) {
	...
	if ( isset( $mt_meta_boxes[ $page ][ $context ] ) ) {
		foreach ( array( 'high', 'sorted', 'core', 'default', 'low' ) as $priority ) {
			if ( isset( $mt_meta_boxes[ $page ][ $context ][ $priority ] ) ) {
				foreach ( (array) $mt_meta_boxes[ $page ][ $context ][ $priority ] as $box ) {
					...
					if ( MT_DEBUG && ! $block_compatible && 'edit' === $screen->parent_base && ! $screen->is_block_editor() && ! isset( $_GET['meta-box-loader'] ) ) {
 ...
 *
 * @param string                 $id      Meta box ID (used in the 'id' attribute for the meta box).
 * @param string|array|MT_Screen $screen  The screen or screens on which the meta box is shown (such as a
 *                                        post type, 'link', or 'comment'). Accepts a single screen ID,
 *                                        MT_Screen object, or array of screen IDs.
...
function remove_meta_box( $id, $screen, $context ) {
	...
	foreach ( array( 'high', 'core', 'default', 'low' ) as $priority ) {
		$mt_meta_boxes[ $page ][ $context ][ $priority ][ $id ] = false;
	}
}
 ...
 * @since 3.6.0
 *
 * @uses global $mt_meta_boxes Used to retrieve registered meta boxes.
 *
...
function do_accordion_sections( $screen, $context, $object ) {
	...
	if ( isset( $mt_meta_boxes[ $page ][ $context ] ) ) {
		foreach ( array( 'high', 'core', 'default', 'low' ) as $priority ) {
			if ( isset( $mt_meta_boxes[ $page ][ $context ][ $priority ] ) ) {
				foreach ( $mt_meta_boxes[ $page ][ $context ][ $priority ] as $box ) {
					if ( false === $box || ! $box['title'] ) {
						continue;
					}
 ...
 * @since 2.7.0
 *
 * @global array $mt_settings_sections Storage array of all settings sections added to admin pages.
 *
...
function add_settings_section( $id, $title, $callback, $page ) {
	...
	$mt_settings_sections[ $page ][ $id ] = array(
		'id'       => $id,
		'title'    => $title,
		'callback' => $callback,
	);
 ...
 *
 * @global array $mt_settings_fields Storage array of settings fields and info about their pages/sections.
 *
...
function add_settings_field( $id, $title, $callback, $page, $section = 'default', $args = array() ) {
	...
	$mt_settings_fields[ $page ][ $section ][ $id ] = array(
		'id'       => $id,
		'title'    => $title,
		'callback' => $callback,
		'args'     => $args,
...
function do_settings_sections( $page ) {
	global $mt_settings_sections, $mt_settings_fields;

	if ( ! isset( $mt_settings_sections[ $page ] ) ) {
		return;
	}

	foreach ( (array) $mt_settings_sections[ $page ] as $section ) {
		if ( $section['title'] ) {
		...
		if ( ! isset( $mt_settings_fields ) || ! isset( $mt_settings_fields[ $page ] ) || ! isset( $mt_settings_fields[ $page ][ $section['id'] ] ) ) {
			continue;
		}
 ...
 *
 * @global array $mt_settings_fields Storage array of settings fields and their pages/sections.
 *
 * @since 2.7.0
 *
...
function do_settings_fields( $page, $section ) {
	...
	if ( ! isset( $mt_settings_fields[ $page ][ $section ] ) ) {
		return;
	}

	foreach ( (array) $mt_settings_fields[ $page ][ $section ] as $field ) {
		$class = '';
 ...
 *
 * @global array $mt_settings_errors Storage array of errors registered during this pageload
 *
...
function add_settings_error( $setting, $code, $message, $type = 'error' ) {
	global $mt_settings_errors;

	$mt_settings_errors[] = array(
		'setting' => $setting,
		'code'    => $code,
		'message' => $message,
		'type'    => $type,
 ...
 *
 * Checks the $mt_settings_errors array for any errors declared during the current
...
function get_settings_errors( $setting = '', $sanitize = false ) {
	...
	if ( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] && get_transient( 'settings_errors' ) ) {
		$mt_settings_errors = array_merge( (array) $mt_settings_errors, get_transient( 'settings_errors' ) );
		delete_transient( 'settings_errors' );
	}

	// Check global in case errors have been added on this pageload.
	if ( empty( $mt_settings_errors ) ) {
		return array();
	}
	...
	if ( $setting ) {
		...
		foreach ( (array) $mt_settings_errors as $key => $details ) {
			if ( $setting === $details['setting'] ) {
				$setting_errors[] = $mt_settings_errors[ $key ];
			}
		}
	...
	<div id="find-posts" class="find-box" style="display: none;">
		...
		<div class="find-box-inside">
			<div class="find-box-search">
				...
				<?php mt_nonce_field( 'find-posts', '_ajax_nonce', false ); ?>
...
function _admin_search_query() {
	echo isset( $_REQUEST['s'] ) ? esc_attr( mt_unslash( $_REQUEST['s'] ) ) : '';
}
...
function iframe_header( $title = '', $deprecated = false ) {
	...
	global $hook_suffix, $admin_body_class, $mt_locale;
	...
	?>
<title><?php bloginfo( 'name' ); ?> &rsaquo; <?php echo $title; ?> &#8212; <?php _e( 'paCMec' ); ?></title>
	<?php
...
var ajaxurl = '<?php echo esc_js( admin_url( 'admin-ajax.php', 'relative' ) ); ?>',
	...
	/** This filter is documented in mt-admin/admin-header.php */
	$admin_body_classes = apply_filters( 'admin_body_class', '' );
	$admin_body_classes = ltrim( $admin_body_classes . ' ' . $admin_body_class );
	?>
<body <?php echo $admin_body_id; ?>class="mt-admin mt-core-ui no-js iframe <?php echo $admin_body_classes; ?>">
<script type="text/javascript">
(function(){
...
function iframe_footer() {
	...
	/** This action is documented in mt-admin/admin-footer.php */
	do_action( "admin_print_footer_scripts-{$hook_suffix}" ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
 ...
 * @see get_post_states()
 *
 * @param MT_Post $post The post to retrieve states for.
...
function get_post_states( $post ) {
	...
	if ( (int) get_option( 'mt_page_for_privacy_policy' ) === $post->ID ) {
 ...
 *
 * @param MT_Post $post The attachment post to retrieve states for.
 * @param bool    $echo Optional. Whether to echo the post states as an HTML string. Default true.
 ...
 * @since 5.6.0
 *
 * @param MT_Post $post The attachment to retrieve states for.
...
function get_media_states( $post ) {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		$meta_header = get_post_meta( $post->ID, '_mt_attachment_is_custom_header', true );

		if ( is_random_header_image() ) {
			if ( ! isset( $header_images ) ) {
				$header_images = mt_list_pluck( get_uploaded_header_images(), 'attachment_id' );
			}
			...
			if ( ! empty( $meta_header ) && $meta_header === $stylesheet && mt_get_attachment_url( $post->ID ) !== $header_image ) {
				$media_states[] = __( 'Header Image' );
			}

			// Display "Current Header Image" if the image is currently the header image.
			if ( $header_image && mt_get_attachment_url( $post->ID ) === $header_image ) {
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		$meta_background = get_post_meta( $post->ID, '_mt_attachment_is_custom_background', true );

		if ( ! empty( $meta_background ) && $meta_background === $stylesheet ) {
			$media_states[] = __( 'Background Image' );

			$background_image = get_background_image();
			if ( $background_image && mt_get_attachment_url( $post->ID ) === $background_image ) {
	 ...
	 * @param MT_Post  $post         The current attachment object.
	 */
...
function compression_test() {
	?>
	<script type="text/javascript">
	var compressionNonce = <?php echo mt_json_encode( mt_create_nonce( 'update_can_compress_scripts' ) ); ?>;
	var testCompression = {
		get : function(test) {
			var x;
			...
			if (x) {
				...
				x.open('GET', ajaxurl + '?action=mt-compression-test&test='+test+'&_ajax_nonce='+compressionNonce+'&'+(new Date()).getTime(), true);
				x.send('');
			}
		},
 ...
 * @global bool $is_IE
 */
function _mt_admin_html_begin() {
	global $is_IE;

	$admin_html_class = ( is_admin_bar_showing() ) ? 'mt-toolbar' : '';
 ...
 * @return MT_Screen Screen object.
 */
function convert_to_screen( $hook_name ) {
	if ( ! class_exists( 'MT_Screen' ) ) {
		_doing_it_wrong(
			'convert_to_screen(), add_meta_box()',
			sprintf(
				/* translators: 1: mt-admin/includes/template.php, 2: add_meta_box(), 3: add_meta_boxes */
				__( 'Likely direct inclusion of %1$s in order to use %2$s. This is very wrong. Hook the %2$s call into the %3$s action instead.' ),
				'<code>mt-admin/includes/template.php</code>',
	...
	return MT_Screen::get( $hook_name );
}
 ...
 */
function mt_star_rating( $args = array() ) {
	$defaults    = array(
		'rating' => 0,
		'type'   => 'rating',
		'number' => 0,
 ...
 * @since 4.2.0
 */
function _mt_posts_page_notice() {
