<?php
/**
 * List Table API: MT_Users_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_Users_List_Table extends MT_List_Table {
	...
	public function prepare_items() {
		...
		$usersearch = isset( $_REQUEST['s'] ) ? mt_unslash( trim( $_REQUEST['s'] ) ) : '';
		...
		if ( 'none' === $role ) {
			$args = array(
				...
				'include' => mt_get_users_with_no_role( $this->site_id ),
				'search'  => $usersearch,
		 ...
		 * @since 4.4.0
		 *
		 * @param array $args Arguments passed to MT_User_Query to retrieve items for the current
		 *                    users list table.
		 */
		$args = apply_filters( 'users_list_table_query_args', $args );

		// Query the user IDs for this page.
		$mt_user_search = new MT_User_Query( $args );

		$this->items = $mt_user_search->get_results();

		$this->set_pagination_args(
			array(
				'total_items' => $mt_user_search->get_total(),
	...
	protected function get_views() {
		...
		foreach ( $mt_roles->get_names() as $this_role => $name ) {
	 ...
	 *
	 * @param MT_User $user_object The current user object.
	...
	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
		if ( ! ( $user_object instanceof MT_User ) ) {
		...
		if ( current_user_can( 'list_users' ) ) {
			...
			$edit_link = esc_url( add_query_arg( 'mt_http_referer', urlencode( mt_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_object->ID ) ) );
			...
			if ( ! is_multisite() && get_current_user_id() != $user_object->ID && current_user_can( 'delete_user', $user_object->ID ) ) {
				$actions['delete'] = "<a class='submitdelete' href='" . mt_nonce_url( "users.php?action=delete&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Delete' ) . '</a>';
			}
			if ( is_multisite() && current_user_can( 'remove_user', $user_object->ID ) ) {
				$actions['remove'] = "<a class='submitdelete' href='" . mt_nonce_url( $url . "action=remove&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Remove' ) . '</a>';
			}
			...
			if ( get_current_user_id() !== $user_object->ID && current_user_can( 'edit_user', $user_object->ID ) ) {
				$actions['resetpassword'] = "<a class='resetpassword' href='" . mt_nonce_url( "users.php?action=resetpassword&amp;users=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Send password reset' ) . '</a>';
			}
			 ...
			 * @param MT_User  $user_object MT_User object for the currently listed user.
			 */
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			$data = 'data-colname="' . mt_strip_all_tags( $column_display_name ) . '"';
	 ...
	 * @since 4.4.0
	 *
	 * @param MT_User $user_object The MT_User object.
	...
	protected function get_role_list( $user_object ) {
		...
		foreach ( $user_object->roles as $role ) {
			if ( isset( $mt_roles->role_names[ $role ] ) ) {
				$role_list[ $role ] = translate_user_role( $mt_roles->role_names[ $role ] );
			}
		}
		 ...
		 * @param MT_User  $user_object A MT_User object.
		 */
