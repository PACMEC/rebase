...
$mt_file_descriptions = array(
	...
	// Deprecated files.
	'mt-layout.css'         => __( 'Stylesheet' ),
	'mt-comments.php'       => __( 'Comments Template' ),
	'mt-comments-popup.php' => __( 'Popup Comments Template' ),
	'comments-popup.php'    => __( 'Popup Comments' ),
);

/**
 * Gets the description for standard paCMec theme files.
 *
 * @since 1.5.0
 *
 * @global array $mt_file_descriptions Theme file descriptions.
 * @global array $allowed_files        List of allowed files.
 *
 * @param string $file Filesystem path or filename.
 * @return string Description of file from $mt_file_descriptions or basename of $file if description doesn't exist.
 *                Appends 'Page Template' to basename of $file if the file is a page template.
 */
function get_file_description( $file ) {
	global $mt_file_descriptions, $allowed_files;

	$dirname   = pathinfo( $file, PATHINFO_DIRNAME );
	$file_path = $allowed_files[ $file ];

	if ( isset( $mt_file_descriptions[ basename( $file ) ] ) && '.' === $dirname ) {
		return $mt_file_descriptions[ basename( $file ) ];
...
/**
 * Gets the absolute filesystem path to the root of the paCMec installation.
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the paCMec installation.
 */
function get_home_path() {
	...
	if ( ! empty( $home ) && 0 !== strcasecmp( $home, $siteurl ) ) {
		$mt_path_rel_to_home = str_ireplace( $home, '', $siteurl ); /* $siteurl - $home */
		$pos                 = strripos( str_replace( '\\', '/', $_SERVER['SCRIPT_FILENAME'] ), trailingslashit( $mt_path_rel_to_home ) );
 ...
 */
function mt_get_plugin_file_editable_extensions( $plugin ) {
 ...
 * @since 4.9.0
 *
 * @param MT_Theme $theme Theme object.
 * @return string[] Array of editable file extensions.
 */
function mt_get_theme_file_editable_extensions( $theme ) {
	 ...
	 * @param MT_Theme $theme         The current theme object.
	 */
	$file_types = apply_filters( 'mt_theme_editor_filetypes', $default_types, $theme );
 ...
 * @since 4.9.0
 */
function mt_print_file_editor_templates() {
	?>
	<script type="text/html" id="tmpl-mt-file-editor-notice">
 ...
 * }
 * @return true|MT_Error True on success or `MT_Error` on failure.
 */
function mt_edit_theme_plugin_file( $args ) {
	if ( empty( $args['file'] ) ) {
		return new MT_Error( 'missing_file' );
	}
	$file = $args['file'];
	if ( 0 !== validate_file( $file ) ) {
		return new MT_Error( 'bad_file' );
	}

	if ( ! isset( $args['newcontent'] ) ) {
		return new MT_Error( 'missing_content' );
	}
	$content = $args['newcontent'];

	if ( ! isset( $args['nonce'] ) ) {
		return new MT_Error( 'missing_nonce' );
	}
	...
	if ( ! empty( $args['plugin'] ) ) {
		...
		if ( ! current_user_can( 'edit_plugins' ) ) {
			return new MT_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
		}

		if ( ! mt_verify_nonce( $args['nonce'], 'edit-plugin_' . $file ) ) {
			return new MT_Error( 'nonce_failure' );
		}

		if ( ! array_key_exists( $plugin, get_plugins() ) ) {
			return new MT_Error( 'invalid_plugin' );
		}

		if ( 0 !== validate_file( $file, get_plugin_files( $plugin ) ) ) {
			return new MT_Error( 'bad_plugin_file_path', __( 'Sorry, that file cannot be edited.' ) );
		}

		$editable_extensions = mt_get_plugin_file_editable_extensions( $plugin );

		$real_file = MT_PLUGIN_DIR . '/' . $file;
		...
		if ( 0 !== validate_file( $stylesheet ) ) {
			return new MT_Error( 'bad_theme_path' );
		}

		if ( ! current_user_can( 'edit_themes' ) ) {
			return new MT_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit templates for this site.' ) );
		}

		$theme = mt_get_theme( $stylesheet );
		if ( ! $theme->exists() ) {
			return new MT_Error( 'non_existent_theme', __( 'The requested theme does not exist.' ) );
		}

		if ( ! mt_verify_nonce( $args['nonce'], 'edit-theme_' . $stylesheet . '_' . $file ) ) {
			return new MT_Error( 'nonce_failure' );
		}
		...
		$editable_extensions = mt_get_theme_file_editable_extensions( $theme );
		...
		if ( 0 !== validate_file( $file, array_keys( $allowed_files ) ) ) {
			return new MT_Error( 'disallowed_theme_file', __( 'Sorry, that file cannot be edited.' ) );
		}
		...
		return new MT_Error( 'missing_theme_or_plugin' );
	}

	// Ensure file is real.
	if ( ! is_file( $real_file ) ) {
		return new MT_Error( 'file_does_not_exist', __( 'File does not exist! Please double check the name and try again.' ) );
	}
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $extension, $editable_extensions, true ) ) {
			return new MT_Error( 'illegal_file_type', __( 'Files of this type are not editable.' ) );
		}
	}
	...
	if ( ! is_writable( $real_file ) ) {
		return new MT_Error( 'file_not_writable' );
	}
	...
	fclose( $f );
	if ( false === $written ) {
		return new MT_Error( 'unable_to_write', __( 'Unable to write to file.' ) );
	}

	mt_opcache_invalidate( $real_file, true );
	...
	if ( $is_active && 'php' === $extension ) {
		...
		$scrape_params = array(
			'mt_scrape_key'   => $scrape_key,
			'mt_scrape_nonce' => $scrape_nonce,
		);
		$headers       = array(
		...
		$needle_start = "###### mt_scraping_result_start:$scrape_key ######";
		$needle_end   = "###### mt_scraping_result_end:$scrape_key ######";
		...
		$body                   = mt_remote_retrieve_body( $r );
		...
		if ( true !== $result ) {
			...
			return new MT_Error( 'php_error', $message, $result );
		}
	}

	if ( $theme instanceof MT_Theme ) {
		$theme->cache_delete();
	}
 ...
 */
function mt_tempnam( $filename = '', $dir = '' ) {
	if ( empty( $dir ) ) {
		$dir = get_temp_dir();
	}
	...
	if ( ! $temp_filename ) {
		return mt_tempnam( dirname( $filename ), $dir );
	}

	// Suffix some random data to avoid filename conflicts.
	$temp_filename .= '-' . mt_generate_password( 6, false );
	$temp_filename .= '.tmp';
	$temp_filename  = $dir . mt_unique_filename( $dir, $temp_filename );

	$fp = @fopen( $temp_filename, 'x' );
	if ( ! $fp && is_writable( $dir ) && file_exists( $temp_filename ) ) {
		return mt_tempnam( $filename, $dir );
	}
	if ( $fp ) {
		fclose( $fp );
	}
...
function validate_file_to_edit( $file, $allowed_files = array() ) {
	...
	switch ( $code ) {
		case 1:
			mt_die( __( 'Sorry, that file cannot be edited.' ) );

			// case 2 :
			// mt_die( __('Sorry, can&#8217;t call files with their real path.' ));
 ...
 */
function _mt_handle_upload( &$file, $overrides, $time, $action ) {
	// The default error handler.
	if ( ! function_exists( 'mt_handle_upload_error' ) ) {
		function mt_handle_upload_error( &$file, $message ) {
			return array( 'error' => $message );
		}
	}

	/**
	 * Filters the data for a file before it is uploaded to paCMec.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 * Possible filter names include:
	 *
	 *  - `mt_handle_sideload_prefilter`
	 *  - `mt_handle_upload_prefilter`
	 *
	 * @since 2.9.0 as 'mt_handle_upload_prefilter'.
	...
	/**
	 * Filters the override parameters for a file before it is uploaded to paCMec.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 * Possible filter names include:
	 *
	 *  - `mt_handle_sideload_overrides`
	 *  - `mt_handle_upload_overrides`
	 *
	 * @since 5.7.0
	 *
	 * @param array|false $overrides An array of override parameters for this file. Boolean false if none are
	 *                               provided. @see _mt_handle_upload().
	 * @param string[]    $file      An array of data for the file. Reference to a single element of `$_FILES`.
	 */
	$overrides = apply_filters( "{$action}_overrides", $overrides, $file );

	// You may define your own function and pass the name in $overrides['upload_error_handler'].
	$upload_error_handler = 'mt_handle_upload_error';
	if ( isset( $overrides['upload_error_handler'] ) ) {
		$upload_error_handler = $overrides['upload_error_handler'];
	}

	// You may have had one or more 'mt_handle_upload_prefilter' functions error out the file. Handle that gracefully.
	...
	$test_uploaded_file = 'mt_handle_upload' === $action ? is_uploaded_file( $file['tmp_name'] ) : @is_readable( $file['tmp_name'] );
	if ( ! $test_uploaded_file ) {
		return call_user_func_array( $upload_error_handler, array( &$file, __( 'Specified file failed upload test.' ) ) );
	}

	$test_file_size = 'mt_handle_upload' === $action ? $file['size'] : filesize( $file['tmp_name'] );
	...
	if ( $test_type ) {
		$mt_filetype     = mt_check_filetype_and_ext( $file['tmp_name'], $file['name'], $mimes );
		$ext             = empty( $mt_filetype['ext'] ) ? '' : $mt_filetype['ext'];
		$type            = empty( $mt_filetype['type'] ) ? '' : $mt_filetype['type'];
		$proper_filename = empty( $mt_filetype['proper_filename'] ) ? '' : $mt_filetype['proper_filename'];

		// Check to see if mt_check_filetype_and_ext() determined the filename was incorrect.
		if ( $proper_filename ) {
	 ...
	 * overriding this one.
	 */
	$uploads = mt_upload_dir( $time );
	if ( ! ( $uploads && false === $uploads['error'] ) ) {
		return call_user_func_array( $upload_error_handler, array( &$file, $uploads['error'] ) );
	}

	$filename = mt_unique_filename( $uploads['path'], $file['name'], $unique_filename_callback );
	...
	return apply_filters(
		...
		'mt_handle_sideload' === $action ? 'sideload' : 'upload'
	);
}

/**
 * Wrapper for _mt_handle_upload().
 *
 * Passes the {@see 'mt_handle_upload'} action.
 *
 * @since 2.0.0
 *
 * @see _mt_handle_upload()
 *
 ...
 *               or `array( 'error' => $message )`.
 */
function mt_handle_upload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'mt_handle_upload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}

	return _mt_handle_upload( $file, $overrides, $time, $action );
}

/**
 * Wrapper for _mt_handle_upload().
 *
 * Passes the {@see 'mt_handle_sideload'} action.
 *
 * @since 2.6.0
 *
 * @see _mt_handle_upload()
 ...
 *               or `array( 'error' => $message )`.
 */
function mt_handle_sideload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'mt_handle_sideload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}
...
/**
 * Downloads a URL to a local temporary file using the paCMec HTTP API.
 *
 ...
 *                                       Default false.
 * @return string|MT_Error Filename on success, MT_Error on failure.
 */
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	// WARNING: The file is not automatically deleted, the script must unlink() the file.
	if ( ! $url ) {
		return new MT_Error( 'http_no_url', __( 'Invalid URL Provided.' ) );
	}

	$url_filename = basename( parse_url( $url, PHP_URL_PATH ) );

	$tmpfname = mt_tempnam( $url_filename );
	if ( ! $tmpfname ) {
		return new MT_Error( 'http_no_file', __( 'Could not create Temporary file.' ) );
	}
	...
	$response_code = mt_remote_retrieve_response_code( $response );
	...
	if ( 200 != $response_code ) {
		...
		unlink( $tmpfname );
		return new MT_Error( 'http_404', trim( mt_remote_retrieve_response_message( $response ) ), $data );
	}

	$content_md5 = mt_remote_retrieve_header( $response, 'content-md5' );
	if ( $content_md5 ) {
		$md5_check = verify_file_md5( $tmpfname, $content_md5 );
		if ( is_mt_error( $md5_check ) ) {
			unlink( $tmpfname );
			return $md5_check;
		}
	}
	...
	if ( $signature_verification ) {
		$signature = mt_remote_retrieve_header( $response, 'x-content-signature' );
		if ( ! $signature ) {
			// Retrieve signatures from a file if the header wasn't included.
			// paCMec.org stores signatures at $package_url.sig.
			 ...
			 */
			$signature_url = apply_filters( 'mt_signature_url', $signature_url, $url );

			if ( $signature_url ) {
				$signature_request = mt_safe_remote_get(
					$signature_url,
					array(
				...
				if ( ! is_mt_error( $signature_request ) && 200 === mt_remote_retrieve_response_code( $signature_request ) ) {
					$signature = explode( "\n", mt_remote_retrieve_body( $signature_request ) );
				}
			}
		}
	...
	if ( is_mt_error( $signature_verification ) ) {
		if (
			/**
			 ...
			 */
			apply_filters( 'mt_signature_softfail', true, $url )
		) {
 ...
 * @return bool|MT_Error True on success, false when the MD5 format is unknown/unexpected,
 *                       MT_Error on failure.
 */
 ...
 * @return bool|MT_Error True on success, false if verification not attempted,
 *                       or MT_Error describing an error condition.
 */
function verify_file_signature( $filename, $signatures, $filename_for_errors = false ) {
	if ( ! $filename_for_errors ) {
		$filename_for_errors = mt_basename( $filename );
	}
	...
	if ( ! extension_loaded( 'sodium' ) && ! ParagonIE_Sodium_Compat::polyfill_is_fast() ) {
		...
		// Allow for an old version of Sodium_Compat being loaded before the bundled paCMec one.
		if ( method_exists( 'ParagonIE_Sodium_Compat', 'runtime_speed_test' ) ) {
			// Run `ParagonIE_Sodium_Compat::runtime_speed_test()` in optimized integer mode, as that's what paCMec utilises during signing verifications.
			// phpcs:disable paCMec.NamingConventions.ValidVariableName
...
/**
 * Retrieves the list of signing keys trusted by paCMec.
 *
 * @since 5.2.0
 *
 * @return string[] Array of base64-encoded signing keys.
 */
function mt_trusted_keys() {
	$trusted_keys = array();

	if ( time() < 1617235200 ) {
		// paCMec.org Key #1 - This key is only valid before April 1st, 2021.
	 ...
	 */
	return apply_filters( 'mt_trusted_keys', $trusted_keys );
}

/**
 * Unzips a specified ZIP file to a location on the filesystem via the paCMec
 * Filesystem Abstraction.
 *
 * Assumes that MT_Filesystem() has already been called and set up. Does not extract
...
function unzip_file( $file, $to ) {
	...
	if ( ! $mt_filesystem || ! is_object( $mt_filesystem ) ) {
		return new MT_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
	}
	...
	if ( ! $mt_filesystem->is_dir( $to ) ) { // Only do parents if no children exist.
 ...
 *
 * Assumes that MT_Filesystem() has already been called and set up.
 *
 * @since 3.0.0
 * @access private
 *
...
function _unzip_file_ziparchive( $file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen ) {
		return new MT_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), array( 'ziparchive_error' => $zopen ) );
	}
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		$info = $z->statIndex( $i );
		if ( ! $info ) {
			return new MT_Error( 'stat_failed_ziparchive', __( 'Could not retrieve file from archive.' ) );
		}
	 ...
	 */
	if ( mt_doing_cron() ) {
		$available_space = @disk_free_space( MT_CONTENT_DIR );
		if ( $available_space && ( $uncompressed_size * 2.1 ) > $available_space ) {
			return new MT_Error( 'disk_full_unzip_file', __( 'Could not copy files. You may have run out of disk space.' ), compact( 'uncompressed_size', 'available_space' ) );
		}
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $mt_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $mt_filesystem->is_dir( $_dir ) ) {
			return new MT_Error( 'mkdir_failed_ziparchive', __( 'Could not create directory.' ), substr( $_dir, strlen( $to ) ) );
		}
	}
	unset( $needed_dirs );
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents ) {
			return new MT_Error( 'extract_failed_ziparchive', __( 'Could not extract file from archive.' ), $info['name'] );
		}

		if ( ! $mt_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE ) ) {
			return new MT_Error( 'copy_failed_ziparchive', __( 'Could not copy file.' ), $info['name'] );
		}
	}
...
function _unzip_file_pclzip( $file, $to, $needed_dirs = array() ) {
	...
	require_once ABSPATH . 'mt-admin/includes/class-pclzip.php';
	...
	if ( ! is_array( $archive_files ) ) {
		return new MT_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), $archive->errorInfo( true ) );
	}

	if ( 0 === count( $archive_files ) ) {
		return new MT_Error( 'empty_archive_pclzip', __( 'Empty archive.' ) );
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		// Only check to see if the dir exists upon creation failure. Less I/O this way.
		if ( ! $mt_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $mt_filesystem->is_dir( $_dir ) ) {
			return new MT_Error( 'mkdir_failed_pclzip', __( 'Could not create directory.' ), substr( $_dir, strlen( $to ) ) );
		}
	}
	unset( $needed_dirs );
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $mt_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE ) ) {
			return new MT_Error( 'copy_failed_pclzip', __( 'Could not copy file.' ), $file['filename'] );
		}
	}
	return true;
}
...
/**
 * Copies a directory from one location to another via the paCMec Filesystem
 * Abstraction.
 *
...
function copy_dir( $from, $to, $skip_list = array() ) {
	...
	if ( false === $dirlist ) {
		return new MT_Error( 'dirlist_failed_copy_dir', __( 'Directory listing failed.' ), basename( $to ) );
	}
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' === $fileinfo['type'] ) {
			if ( ! $mt_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
				...
				if ( ! $mt_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
					return new MT_Error( 'copy_failed_copy_dir', __( 'Could not copy file.' ), $to . $filename );
				}
			}
			...
			if ( ! $mt_filesystem->is_dir( $to . $filename ) ) {
				if ( ! $mt_filesystem->mkdir( $to . $filename, FS_CHMOD_DIR ) ) {
					return new MT_Error( 'mkdir_failed_copy_dir', __( 'Could not create directory.' ), $to . $filename );
				}
			}
...
/**
 * Initializes and connects the paCMec Filesystem Abstraction classes.
 *
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force paCMec to use them by returning
 ...
 *                                                   directly to the `MT_Filesystem_*()` classes.
 *                                                   Default false.
 ...
 */
function MT_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $mt_filesystem;

	require_once ABSPATH . 'mt-admin/includes/class-mt-filesystem-base.php';
	...
	if ( ! class_exists( "MT_Filesystem_$method" ) ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . 'mt-admin/includes/class-mt-filesystem-' . $method . '.php', $method );
	...
	$method = "MT_Filesystem_$method";

	$mt_filesystem = new $method( $args );
 ...
 * @since 2.5.0
 *
 * @global callable $_mt_filesystem_direct_method
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $context ) {
		$context = MT_CONTENT_DIR;
	}

	// If the directory doesn't exist (mt-content/languages) then use the parent directory as we'll create it.
	if ( MT_LANG_DIR == $context && ! is_dir( $context ) ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the paCMec files, and that of newly created files.
			$mt_file_owner   = false;
			$temp_file_owner = false;
			if ( function_exists( 'fileowner' ) ) {
				$mt_file_owner   = @fileowner( __FILE__ );
				$temp_file_owner = @fileowner( $temp_file_name );
			}

			if ( false !== $mt_file_owner && $mt_file_owner === $temp_file_owner ) {
				/*
				 * paCMec is creating files as the same owner as the paCMec files,
				 * this means it's safe to modify & create new files via PHP.
				 */
				$method                                  = 'direct';
				$GLOBALS['_mt_filesystem_direct_method'] = 'file_owner';
				 ...
				 */
				$method                                  = 'direct';
				$GLOBALS['_mt_filesystem_direct_method'] = 'relaxed_ownership';
			}
 ...
 * @param bool|MT_Error $error                        Optional. Whether the current request has failed
...
function request_filesystem_credentials( $form_post, $type = '', $error = false, $context = '', $extra_fields = null, $allow_relaxed_file_ownership = false ) {
	 ...
	 * @param bool|MT_Error $error                        Whether the current request has failed to connect,
	 *                                                    or an error object.
	...
	$submitted_form = mt_unslash( $_POST );

	// Verify nonce, or unset submitted form field values on failure.
	if ( ! isset( $_POST['_fs_nonce'] ) || ! mt_verify_nonce( $_POST['_fs_nonce'], 'filesystem-credentials' ) ) {
		unset(
...
<p id="request-filesystem-credentials-desc">
	...
	_e( 'To perform the requested action, paCMec needs to access your web server.' );
	echo ' ';
...
<p id="auth-keys-desc"><?php _e( 'Enter the location on the server where the public and private keys are located. If a passphrase is needed, enter that in the password field above.' ); ?></p>
	...
	<p class="request-filesystem-credentials-action-buttons">
		<?php mt_nonce_field( 'filesystem-credentials', '_fs_nonce', false, true ); ?>
 ...
 * @since 4.2.0
 */
function mt_print_request_filesystem_credentials_modal() {
 ...
 */
function mt_opcache_invalidate( $filepath, $force = false ) {
	static $can_invalidate = null;

	/*
	 * Check to see if paCMec is able to run `opcache_invalidate()` or not, and cache the value.
	 *
	 ...
	 * @since 5.5.0
	 *
	 * @param bool   $will_invalidate Whether paCMec will invalidate `$filepath`. Default true.
	 * @param string $filepath        The path to the PHP file to invalidate.
	 */
	if ( apply_filters( 'mt_opcache_invalidate_file', true, $filepath ) ) {
