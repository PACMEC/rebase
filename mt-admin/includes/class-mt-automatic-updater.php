<?php
/**
 * Upgrade API: MT_Automatic_Updater class
 *
 * @package paCMec
 * @subpackage Upgrader
 * @since 4.6.0
 ...
 *
 * @since 3.7.0
 * @since 4.6.0 Moved to its own file from mt-admin/includes/class-mt-upgrader.php.
 */
class MT_Automatic_Updater {
	...
	public function is_disabled() {
		...
		if ( ! mt_is_file_mod_allowed( 'automatic_updater' ) ) {
			return true;
		}
		...
		// More fine grained control can be done through the MT_AUTO_UPDATE_CORE constant and filters.
	 ...
	 * mt-content directory, for example). The underlying assumption is that if you are
	...
	public function should_update( $type, $item, $context ) {
		// Used to see if MT_Filesystem is set up to allow unattended updates.
		...
		if ( 'core' === $type ) {
			...
			if ( ! $update && mt_is_auto_update_enabled_for_type( $type ) ) {
		...
		if ( 'core' === $type ) {
			...
			if ( file_exists( MT_CONTENT_DIR . '/db.php' ) && empty( $mtdb->is_mysql ) ) {
				...
				$mysql_compat = version_compare( $mtdb->db_version(), $item->mysql_version, '>=' );
			}
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * from paCMec.org sets a particular flag. This allows some discretion
		 * in if and when to notify.
		 *
		 * This filter is only evaluated once per release. If the same email address
		 * was already notified of the same new version, paCMec won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			...
			case 'plugin':
				...
				$context  = MT_PLUGIN_DIR; // We don't support custom Plugin directories, or updates for MTMU_PLUGIN_DIR.
				break;
			case 'theme':
			...
			case 'translation':
				...
				$context  = MT_CONTENT_DIR; // MT_LANG_DIR;
				break;
		}
		...
		switch ( $type ) {
			case 'core':
				...
				$skin->feedback( __( 'Updating to paCMec %s' ), $item->version );
				/* translators: %s: paCMec version. */
				$item_name = sprintf( __( 'paCMec %s' ), $item->version );
				break;
			case 'theme':
				$upgrader_item = $item->theme;
				$theme         = mt_get_theme( $upgrader_item );
			...
			case 'translation':
				...
				/* translators: %s: Project name (plugin, theme, or paCMec). */
				$item_name = sprintf( __( 'Translations for %s' ), $language_item_name );
				/* translators: 1: Project name (plugin, theme, or paCMec), 2: Language. */
		...
		if ( false === $upgrade_result ) {
			$upgrade_result = new MT_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
		}

		if ( 'core' === $type ) {
			if ( is_mt_error( $upgrade_result )
			...
			if ( is_mt_error( $upgrade_result ) ) {
				...
				$skin->feedback( __( 'paCMec updated successfully.' ) );
			}
		}
	...
	public function run() {
		...
		if ( ! MT_Upgrader::create_lock( 'auto_updater' ) ) {
			return;
		}

		// Don't automatically run these things, as we'll handle it ourselves.
		remove_action( 'upgrader_process_complete', array( 'Language_Pack_Upgrader', 'async_upgrade' ), 20 );
		remove_action( 'upgrader_process_complete', 'mt_version_check' );
		remove_action( 'upgrader_process_complete', 'mt_update_plugins' );
		remove_action( 'upgrader_process_complete', 'mt_update_themes' );

		// Next, plugins.
		mt_update_plugins(); // Check for plugin updates.
		...
		mt_update_themes();  // Check for theme updates.
		...
		mt_version_check(); // Check for core updates.
		...
		mt_update_themes( $theme_stats ); // Check for theme updates.
		...
		mt_update_plugins( $plugin_stats ); // Check for plugin updates.

		// Finally, process any new translations.
		$language_updates = mt_get_translation_updates();
		if ( $language_updates ) {
		...
		MT_Upgrader::release_lock( 'auto_updater' );
	}
	...
	protected function after_core_update( $update_result ) {
		...
		// Any of these MT_Error codes are critical failures, as in they occurred after we started to copy core files.
		// We should not try to perform a background update again until there is a successful one-click update performed by the user.
		$critical = false;
		if ( 'disk_full' === $error_code || false !== strpos( $error_code, '__copy_dir' ) ) {
			$critical = true;
		} elseif ( 'rollback_was_required' === $error_code && is_mt_error( $result->get_error_data()->rollback ) ) {
		...
		/*
		 * Any other MT_Error code (like download_failed or files_not_writable) occurs before
		 ...
		 * the issue could actually be on paCMec.org's side.) If that one fails, then email.
		 */
		$send               = true;
		$transient_failures = array( 'incompatible_archive', 'download_failed', 'insane_distro', 'locked' );
		if ( in_array( $error_code, $transient_failures, true ) && ! get_site_option( 'auto_core_update_failed' ) ) {
			mt_schedule_single_event( time() + HOUR_IN_SECONDS, 'mt_maybe_auto_update' );
			$send = false;
		}
	 ...
	 * @param mixed  $result      Optional. The result for the core update. Can be MT_Error.
	 */
	protected function send_email( $type, $core_update, $result = null ) {
		 ...
		 * @param object $core_update The update offer that was attempted.
		 * @param mixed  $result      The result for the core update. Can be MT_Error.
		 */
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: paCMec version. */
				$subject = __( '[%1$s] Your site has updated to paCMec %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: paCMec version. */
				$subject = __( '[%1$s] paCMec %2$s is available. Please update!' );
				break;
		...
		// If the auto-update is not to the latest version, say that the current version of MT is available instead.
		$version = 'success' === $type ? $core_update->current : $next_user_core_update->current;
		$subject = sprintf( $subject, mt_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $version );
		...
		switch ( $type ) {
			case 'success':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: paCMec version. */
					__( 'Howdy! Your site at %1$s has been updated automatically to paCMec %2$s.' ),
					home_url(),
					$core_update->current
				);
				$body .= "\n\n";
				...
				/* translators: %s: paCMec version. */
				$body .= sprintf( __( 'For more on version %s, see the About paCMec screen:' ), $about_version );
				$body .= "\n" . admin_url( 'about.php' );

				if ( $newer_version_available ) {
					/* translators: %s: paCMec latest version. */
					$body .= "\n\n" . sprintf( __( 'paCMec %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'manual':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: paCMec version. */
					__( 'Please update your site at %1$s to paCMec %2$s.' ),
					home_url(),
			...
			case 'critical':
				if ( $newer_version_available ) {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: paCMec version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update paCMec to version %2$s.' ),
						home_url(),
						$core_update->current
					);
				} else {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: paCMec latest version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of paCMec, %2$s.' ),
						home_url(),
						$core_update->current
					);
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf(
				...
				__( 'The paCMec team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ),
		...
		if ( 'critical' === $type && is_mt_error( $result ) ) {
			$body .= "\n***\n\n";
			/* translators: %s: paCMec version. */
			...
			// Loop through all errors (the main MT_Error, the update result, the rollback result) for code, data, etc.
		 ...
		 *
		 * @param array $email {
		 *     Array of email arguments that will be passed to mt_mail().
		 *
		 *     @type string $to      The email recipient. An array of emails
		 *                            can be returned, as handled by mt_mail().
		...
		mt_mail( $email['to'], mt_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	}
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		$body    = implode( "\n", $body );
		$to      = get_site_option( 'admin_email' );
		$subject = sprintf( $subject, mt_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ) );
		$headers = '';
		...
		$result = mt_mail( $email['to'], mt_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	...
	protected function send_debug_email() {
		...
		$body[] = sprintf( __( 'paCMec site: %s' ), network_home_url( '/' ) );

		// Core.
		if ( isset( $this->update_results['core'] ) ) {
			$result = $this->update_results['core'][0];
			if ( $result->result && ! is_mt_error( $result->result ) ) {
				/* translators: %s: paCMec version. */
				$body[] = sprintf( __( 'SUCCESS: paCMec was successfully updated to %s' ), $result->name );
			} else {
				/* translators: %s: paCMec version. */
				$body[] = sprintf( __( 'FAILED: paCMec failed to update to %s' ), $result->name );
				$failures++;
			}
			$body[] = '';
		}
		...
		foreach ( array( 'plugin', 'theme', 'translation' ) as $type ) {
			...
			$success_items = mt_list_filter( $this->update_results[ $type ], array( 'result' => true ) );
			if ( $success_items ) {
				$messages = array(
				...
				$body[] = $messages[ $type ];
				foreach ( mt_list_pluck( $success_items, 'name' ) as $name ) {
			...
			if ( $success_items != $this->update_results[ $type ] ) {
				...
				foreach ( $this->update_results[ $type ] as $item ) {
					if ( ! $item->result || is_mt_error( $item->result ) ) {
						/* translators: %s: Name of plugin / theme / translation. */
		...
		$site_title = mt_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES );
		if ( $failures ) {
			$body[] = trim(
				__(
					"BETA TESTING?
