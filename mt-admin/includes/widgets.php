<?php
/**
 * paCMec Widgets Administration API
 *
 * @package paCMec
...
function mt_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset( $mt_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) ) {
		...
		$sidebar_args = mt_list_widget_controls_dynamic_sidebar( $control_args );

		mt_widget_control( ...$sidebar_args );
	}
}
 ...
 */
function mt_list_widget_controls( $sidebar, $sidebar_name = '' ) {
	add_filter( 'dynamic_sidebar_params', 'mt_list_widget_controls_dynamic_sidebar' );

	$description = mt_sidebar_description( $sidebar );
 ...
 * @return array
 */
function mt_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $mt_registered_widgets[ $widget_id ]['callback'] ) ) {
		$mt_registered_widgets[ $widget_id ]['_callback'] = $mt_registered_widgets[ $widget_id ]['callback'];
		$mt_registered_widgets[ $widget_id ]['callback']  = 'mt_widget_control';
	}
 ...
 * @return array
 */
function mt_widget_control( $sidebar_args ) {
	global $mt_registered_widgets, $mt_registered_widget_controls, $sidebars_widgets;

	$widget_id  = $sidebar_args['widget_id'];
	$sidebar_id = isset( $sidebar_args['id'] ) ? $sidebar_args['id'] : false;
	$key        = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[ $sidebar_id ], true ) : '-1'; // Position of widget in sidebar.
	$control    = isset( $mt_registered_widget_controls[ $widget_id ] ) ? $mt_registered_widget_controls[ $widget_id ] : array();
	$widget     = $mt_registered_widgets[ $widget_id ];
	...
	$mt_registered_widgets[ $widget_id ]['callback'] = $mt_registered_widgets[ $widget_id ]['_callback'];
	unset( $mt_registered_widgets[ $widget_id ]['_callback'] );
	...
	<?php
	$widget_description = mt_widget_description( $widget_id );
 ...
 * @return string
 */
function mt_widgets_access_body_class( $classes ) {
