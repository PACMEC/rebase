<?php
/**
 * List Table API: MT_MS_Users_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_MS_Users_List_Table extends MT_List_Table {
	/**
	 * @return bool
	 */
	...
	public function prepare_items() {
		 ...
		 */
		if ( ! $usersearch && mt_is_large_network( 'users' ) ) {
		...
		/** This filter is documented in mt-admin/includes/class-mt-users-list-table.php */
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters( 'mtmu_users_columns', $users_columns );
	}
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_User $user The current MT_User object.
	 */
	...
	public function column_username( $user ) {
		...
		if ( current_user_can( 'edit_user', $user->ID ) ) {
			$edit_link = esc_url( add_query_arg( 'mt_http_referer', urlencode( mt_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_User $user        User being acted upon.
	...
	protected function handle_row_actions( $user, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins, true ) ) {
			$actions['delete'] = '<a href="' . esc_url( network_admin_url( add_query_arg( '_mt_http_referer', urlencode( mt_unslash( $_SERVER['REQUEST_URI'] ) ), mt_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}
