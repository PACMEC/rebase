<?php
/**
 * paCMec Dashboard Widget Administration Screen API
 *
 * @package paCMec
 ...
 * @global callable[] $mt_dashboard_control_callbacks
 */
function mt_dashboard_setup() {
	global $mt_registered_widgets, $mt_registered_widget_controls, $mt_dashboard_control_callbacks;
	$mt_dashboard_control_callbacks = array();
	$screen                         = get_current_screen();

	/* Register Widgets and Controls */

	$response = mt_check_browser_version();

	if ( $response && $response['upgrade'] ) {
		add_filter( 'postbox_classes_dashboard_dashboard_browser_nag', 'dashboard_browser_nag_class' );
		if ( $response['insecure'] ) {
			mt_add_dashboard_widget( 'dashboard_browser_nag', __( 'You are using an insecure browser!' ), 'mt_dashboard_browser_nag' );
		} else {
			mt_add_dashboard_widget( 'dashboard_browser_nag', __( 'Your browser is out of date!' ), 'mt_dashboard_browser_nag' );
		}
	}
	...
	if ( $response && isset( $response['is_acceptable'] ) && ! $response['is_acceptable'] && current_user_can( 'update_php' ) ) {
		...
		mt_add_dashboard_widget( 'dashboard_php_nag', __( 'PHP Update Recommended' ), 'mt_dashboard_php_nag' );
	}
	...
	if ( current_user_can( 'view_site_health_checks' ) && ! is_network_admin() ) {
		...
		mt_enqueue_style( 'site-health' );
		mt_enqueue_script( 'site-health' );

		mt_add_dashboard_widget( 'dashboard_site_health', __( 'Site Health Status' ), 'mt_dashboard_site_health' );
	}

	// Right Now.
	if ( is_blog_admin() && current_user_can( 'edit_posts' ) ) {
		mt_add_dashboard_widget( 'dashboard_right_now', __( 'At a Glance' ), 'mt_dashboard_right_now' );
	}

	if ( is_network_admin() ) {
		mt_add_dashboard_widget( 'network_dashboard_right_now', __( 'Right Now' ), 'mt_network_dashboard_right_now' );
	}

	// Activity Widget.
	if ( is_blog_admin() ) {
		mt_add_dashboard_widget( 'dashboard_activity', __( 'Activity' ), 'mt_dashboard_site_activity' );
	}
	...
	if ( is_blog_admin() && current_user_can( get_post_type_object( 'post' )->cap->create_posts ) ) {
		...
		mt_add_dashboard_widget( 'dashboard_quick_press', $quick_draft_title, 'mt_dashboard_quick_press' );
	}

	// paCMec Events and News.
	mt_add_dashboard_widget( 'dashboard_primary', __( 'paCMec Events and News' ), 'mt_dashboard_events_news' );
	...
	if ( is_network_admin() ) {
		 ...
		 * @since 3.1.0
		 */
		do_action( 'mt_network_dashboard_setup' );
		 ...
		 */
		$dashboard_widgets = apply_filters( 'mt_network_dashboard_widgets', array() );
		 ...
		 * @since 3.1.0
		 */
		do_action( 'mt_user_dashboard_setup' );
		 ...
		 * @param string[] $dashboard_widgets An array of dashboard widget IDs.
		 */
		$dashboard_widgets = apply_filters( 'mt_user_dashboard_widgets', array() );
	} else {
		 ...
		 * @param string[] $dashboard_widgets An array of dashboard widget IDs.
		 */
		$dashboard_widgets = apply_filters( 'mt_dashboard_widgets', array() );
	}

	foreach ( $dashboard_widgets as $widget_id ) {
		$name = empty( $mt_registered_widgets[ $widget_id ]['all_link'] ) ? $mt_registered_widgets[ $widget_id ]['name'] : $mt_registered_widgets[ $widget_id ]['name'] . " <a href='{$mt_registered_widgets[$widget_id]['all_link']}' class='edit-box open-box'>" . __( 'View all' ) . '</a>';
		mt_add_dashboard_widget( $widget_id, $name, $mt_registered_widgets[ $widget_id ]['callback'], $mt_registered_widget_controls[ $widget_id ]['callback'] );
	}

	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget_id'] ) ) {
		check_admin_referer( 'edit-dashboard-widget_' . $_POST['widget_id'], 'dashboard-widget-nonce' );
		ob_start(); // Hack - but the same hack mt-admin/widgets.php uses.
		mt_dashboard_trigger_widget_control( $_POST['widget_id'] );
		ob_end_clean();
		mt_redirect( remove_query_arg( 'edit' ) );
		exit;
	}

	/** This action is documented in mt-admin/includes/meta-boxes.php */
 ...
 */
function mt_add_dashboard_widget( $widget_id, $widget_name, $callback, $control_callback = null, $callback_args = null, $context = 'normal', $priority = 'core' ) {
	$screen = get_current_screen();
	global $mt_dashboard_control_callbacks;
	...
	if ( $control_callback && current_user_can( 'edit_dashboard' ) && is_callable( $control_callback ) ) {
		$mt_dashboard_control_callbacks[ $widget_id ] = $control_callback;
		if ( isset( $_GET['edit'] ) && $widget_id == $_GET['edit'] ) {
			list($url)    = explode( '#', add_query_arg( 'edit', false ), 2 );
			$widget_name .= ' <span class="postbox-title-action"><a href="' . esc_url( $url ) . '">' . __( 'Cancel' ) . '</a></span>';
			$callback     = '_mt_dashboard_control_callback';
		} else {
 ...
 */
function _mt_dashboard_control_callback( $dashboard, $meta_box ) {
	echo '<form method="post" class="dashboard-widget-control-form mt-clearfix">';
	mt_dashboard_trigger_widget_control( $meta_box['id'] );
	mt_nonce_field( 'edit-dashboard-widget_' . $meta_box['id'], 'dashboard-widget-nonce' );
 ...
 * @since 2.7.0
 */
function mt_dashboard_right_now() {
	?>
	<div class="main">
	<ul>
	<?php
	...
	foreach ( array( 'post', 'page' ) as $post_type ) {
		$num_posts = mt_count_posts( $post_type );
 ...
 * @since 3.1.0
 */
function mt_network_dashboard_right_now() {
	$actions = array();
 ...
 */
function mt_dashboard_quick_press( $error_msg = false ) {
	global $post_ID;
 ...
 * @since 2.7.0
 *
 * @param MT_Post[]|false $drafts Optional. Array of posts to display. Default false.
 */
function mt_dashboard_recent_drafts( $drafts = false ) {
	if ( ! $drafts ) {
		$query_args = array(
			'post_type'      => 'post',
			'post_status'    => 'draft',
	...
	foreach ( $drafts as $draft ) {
		...
		$the_content = mt_trim_words( $draft->post_content, $draft_length );
		if ( $the_content ) {
 ...
 *
 * @param MT_Comment $comment   The current comment.
 * @param bool       $show_date Optional. Whether to display the date.
 */
function _mt_dashboard_recent_comments_row( &$comment, $show_date = true ) {
	...
	if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) {
		...
		$del_nonce     = esc_html( '_mtnonce=' . mt_create_nonce( "delete-comment_$comment->comment_ID" ) );
		$approve_nonce = esc_html( '_mtnonce=' . mt_create_nonce( "approve-comment_$comment->comment_ID" ) );
		...
		$actions['approve'] = sprintf(
			'<a href="%s" data-mt-lists="%s" class="vim-a aria-button-if-js" aria-label="%s">%s</a>',
			$approve_url,
		...
		$actions['unapprove'] = sprintf(
			'<a href="%s" data-mt-lists="%s" class="vim-u aria-button-if-js" aria-label="%s">%s</a>',
			$unapprove_url,
		...
		$actions['spam'] = sprintf(
			'<a href="%s" data-mt-lists="%s" class="vim-s vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
			$spam_url,
		...
		if ( ! EMPTY_TRASH_DAYS ) {
			$actions['delete'] = sprintf(
				'<a href="%s" data-mt-lists="%s" class="delete vim-d vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
				$delete_url,
		 ...
		 *                            'Delete', and 'Trash'.
		 * @param MT_Comment $comment The comment object.
		 */
	...
	?>
		...
		<li id="comment-<?php echo $comment->comment_ID; ?>" <?php comment_class( array( 'comment-item', mt_get_comment_status( $comment ) ), $comment ); ?>>
 ...
 * @since 3.8.0
 */
function mt_dashboard_site_activity() {

	echo '<div id="activity-widget">';

	$future_posts = mt_dashboard_recent_posts(
		array(
			'max'    => 5,
			'status' => 'future',
			'order'  => 'ASC',
	...
	$recent_posts = mt_dashboard_recent_posts(
		array(
			'max'    => 5,
			'status' => 'publish',
			'order'  => 'DESC',
	...
	$recent_comments = mt_dashboard_recent_comments();
 ...
 */
function mt_dashboard_recent_posts( $args ) {
	$query_args = array(
		'post_type'      => 'post',
	 ...
	 * @since 4.2.0
	 *
	 * @param array $query_args The arguments passed to MT_Query to produce the list of posts.
	 */
	$query_args = apply_filters( 'dashboard_recent_posts_query_args', $query_args );
	$posts      = new MT_Query( $query_args );
 ...
 */
function mt_dashboard_recent_comments( $total_items = 5 ) {
	...
	if ( $comments ) {
		...
		echo '<ul id="the-comment-list" data-mt-lists="list:comment">';
		...
		if ( current_user_can( 'edit_posts' ) ) {
			...
			_get_list_table( 'MT_Comments_List_Table' )->views();
		}

		mt_comment_reply( -1, false, 'dashboard', false );
		mt_comment_trashnotice();
 ...
 */
function mt_dashboard_rss_output( $widget_id ) {
	$widgets = get_option( 'dashboard_widget_options' );
	echo '<div class="rss-widget">';
	mt_widget_rss_output( $widgets[ $widget_id ] );
	echo '</div>';
}
 ...
 * which is later replaced by Ajax call (see top of /mt-admin/index.php)
 *
 * @since 2.5.0
 ...
 */
function mt_dashboard_cached_rss_widget( $widget_id, $callback, $check_urls = array(), ...$args ) {
 ...
 */
function mt_dashboard_trigger_widget_control( $widget_control_id = false ) {
	global $mt_dashboard_control_callbacks;

	if ( is_scalar( $widget_control_id ) && $widget_control_id && isset( $mt_dashboard_control_callbacks[ $widget_control_id ] ) && is_callable( $mt_dashboard_control_callbacks[ $widget_control_id ] ) ) {
		call_user_func(
			$mt_dashboard_control_callbacks[ $widget_control_id ],
			'',
			array(
				'id'       => $widget_control_id,
				'callback' => $mt_dashboard_control_callbacks[ $widget_control_id ],
			)
		);
	}
}
 ...
 *
 * Sets up $args to be used as input to mt_widget_rss_form(). Handles POST data
 ...
 */
function mt_dashboard_rss_control( $widget_id, $form_inputs = array() ) {
	...
	$number = 1; // Hack to use mt_widget_rss_form().

	$widget_options[ $widget_id ]['number'] = $number;

	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget-rss'][ $number ] ) ) {
		$_POST['widget-rss'][ $number ]         = mt_unslash( $_POST['widget-rss'][ $number ] );
		$widget_options[ $widget_id ]           = mt_widget_rss_process( $_POST['widget-rss'][ $number ] );
		$widget_options[ $widget_id ]['number'] = $number;
	...
	mt_widget_rss_form( $widget_options[ $widget_id ], $form_inputs );
}
 ...
 * @since 4.8.0
 */
function mt_dashboard_events_news() {
	mt_print_community_events_markup();
 ...
 * @since 4.8.0
 */
function mt_print_community_events_markup() {
	?>
	 ...
	 * won't be ready until mt.communityEvents.renderEventsTemplate() has run.
	 */
	?>
 ...
 * @since 4.8.0
 */
function mt_print_community_events_templates() {
	?>
	...
	<script id="tmpl-community-events-event-list" type="text/template">
		<# _.each( data.events, function( event ) { #>
			<li class="event event-{{ event.type }} mt-clearfix">
...
/**
 * 'paCMec Events and News' dashboard widget.
 *
 * @since 2.7.0
 * @since 4.8.0 Removed popular plugins feed.
 */
function mt_dashboard_primary() {
	$feeds = array(
		'news'   => array(

			/**
			 * Filters the primary link URL for the 'paCMec Events and News' dashboard widget.
			 *
			 * @since 2.5.0
			 *
			 ...
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'paCMec Blog' ) ),
			'items'        => 2,
			'show_summary' => 0,
			'show_author'  => 0,
			'show_date'    => 0,
		...
		'planet' => array(
			...
			/**
			 * Filters the secondary link URL for the 'paCMec Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 ...
			 */
			'title'        => apply_filters( 'dashboard_secondary_title', __( 'Other paCMec News' ) ),

			/**
			 * Filters the number of secondary link items for the 'paCMec Events and News' dashboard widget.
			 *
			 * @since 4.4.0
			 *
	...
	mt_dashboard_cached_rss_widget( 'dashboard_primary', 'mt_dashboard_primary_output', $feeds );
}

/**
 * Displays the paCMec events and news feeds.
 *
 * @since 3.8.0
 * @since 4.8.0 Removed popular plugins feed.
 *
 ...
 */
function mt_dashboard_primary_output( $widget_id, $feeds ) {
	foreach ( $feeds as $type => $args ) {
		$args['type'] = $type;
		echo '<div class="rss-widget">';
			mt_widget_rss_output( $args['url'], $args );
		echo '</div>';
	}
}
 ...
 * @since 3.2.0
 */
function mt_dashboard_browser_nag() {
	$notice   = '';
	$response = mt_check_browser_version();
	...
	if ( $response ) {
		if ( $response['insecure'] ) {
			$msg = sprintf(
				...
				__( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best paCMec experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an old version of %s. For the best paCMec experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
	 ...
	 * @param array  $response An array containing web browser information. See `mt_check_browser_version()`.
	 */
	echo apply_filters( 'browse-happy-notice', $notice, $response ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function mt_check_browser_version() {
	...
	if ( false === $response ) {
		 ...
		 */
		$response = json_decode( mt_remote_retrieve_body( $response ), true );
...
function mt_dashboard_php_nag() {
	...
	mt_update_php_annotation();
	mt_direct_php_update_button();
}
 ...
 * @since 5.4.0
 */
function mt_dashboard_site_health() {
 ...
 * @since 2.5.0
 */
function mt_dashboard_empty() {}

/**
 * Displays a welcome panel to introduce users to paCMec.
 *
 * @since 3.3.0
 */
function mt_welcome_panel() {
	?>
	<div class="welcome-panel-content">
	<h2><?php _e( 'Welcome to paCMec!' ); ?></h2>
	<p class="about-description"><?php _e( 'We&#8217;ve assembled some links to get you started:' ); ?></p>
	<div class="welcome-panel-column-container">
	<div class="welcome-panel-column">
		<?php if ( current_user_can( 'customize' ) ) : ?>
			<h3><?php _e( 'Get Started' ); ?></h3>
			<a class="button button-primary button-hero load-customize hide-if-no-customize" href="<?php echo mt_customize_url(); ?>"><?php _e( 'Customize Your Site' ); ?></a>
		<?php endif; ?>
		<a class="button button-primary button-hero hide-if-customize" href="<?php echo admin_url( 'themes.php' ); ?>"><?php _e( 'Customize Your Site' ); ?></a>
		<?php if ( current_user_can( 'install_themes' ) || ( current_user_can( 'switch_themes' ) && count( mt_get_themes( array( 'allowed' => true ) ) ) > 1 ) ) : ?>
