 ...
 * @see MT_Upgrader
 */
class Plugin_Upgrader extends MT_Upgrader {
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|MT_Error True if the installation was successful, false or a MT_Error otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( $parsed_args['clear_update_cache'] ) {
			// Clear cache so mt_update_plugins() knows about the new plugin.
			add_action( 'upgrader_process_complete', 'mt_clean_plugins_cache', 9, 0 );
		}
		...
		remove_action( 'upgrader_process_complete', 'mt_clean_plugins_cache', 9 );
		remove_filter( 'upgrader_source_selection', array( $this, 'check_package' ) );

		if ( ! $this->result || is_mt_error( $this->result ) ) {
			return $this->result;
		}
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|MT_Error True if the upgrade was successful, false or a MT_Error object otherwise.
	 */
	...
	public function bulk_upgrade( $plugins, $args = array() ) {
		...
		foreach ( $results as $plugin => $result ) {
			...
			if ( ! $result || is_mt_error( $result ) || ! isset( $past_failure_emails[ $plugin ] ) ) {
				continue;
			}
	 ...
	 * @return string|MT_Error The source as passed, or a MT_Error object on failure.
	 */
	public function check_package( $source ) {
		global $mt_filesystem, $mt_version;
		...
		$working_directory = str_replace( $mt_filesystem->mt_content_dir(), trailingslashit( MT_CONTENT_DIR ), $source );
		...
		if ( empty( $this->new_plugin_data ) ) {
			return new MT_Error( 'incompatible_archive_no_plugins', $this->strings['incompatible_archive'], __( 'No valid plugins were found.' ) );
		}

		$requires_php = isset( $info['RequiresPHP'] ) ? $info['RequiresPHP'] : null;
		$requires_mt  = isset( $info['RequiresMT'] ) ? $info['RequiresMT'] : null;
		...
		if ( ! is_php_version_compatible( $requires_php ) ) {
			...
			return new MT_Error( 'incompatible_php_required_version', $this->strings['incompatible_archive'], $error );
		}

		if ( ! is_mt_version_compatible( $requires_mt ) ) {
			$error = sprintf(
				/* translators: 1: Current paCMec version, 2: Version required by the uploaded plugin. */
				__( 'Your paCMec version is %1$s, however the uploaded plugin requires %2$s.' ),
				$mt_version,
				$requires_mt
			);

			return new MT_Error( 'incompatible_mt_required_version', $this->strings['incompatible_archive'], $error );
		}
	 ...
	 *
	 * @param bool|MT_Error $return Upgrade offer return.
	 * @param array         $plugin Plugin package arguments.
	 * @return bool|MT_Error The passed in $return param or MT_Error.
	 */
	public function deactivate_plugin_before_upgrade( $return, $plugin ) {

		if ( is_mt_error( $return ) ) { // Bypass.
			return $return;
		}
	 ...
	 * @global MT_Filesystem_Base $mt_filesystem paCMec filesystem subclass.
	 *
	 * @param bool|MT_Error $removed            Whether the destination was cleared.
	 *                                          True on success, MT_Error on failure.
	 * @param string        $local_destination  The local package destination.
	 * @param string        $remote_destination The remote package destination.
	 * @param array         $plugin             Extra arguments passed to hooked filters.
	 * @return bool|MT_Error
	 */
	public function delete_old_plugin( $removed, $local_destination, $remote_destination, $plugin ) {
		...
		if ( ! $mt_filesystem->exists( $this_plugin_dir ) ) { // If it's already vanished.
			return $removed;
		}
		...
		if ( strpos( $plugin, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			...
			$deleted = $mt_filesystem->delete( $plugins_dir . $plugin );
		}
