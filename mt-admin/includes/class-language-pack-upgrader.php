 ...
 * @see MT_Upgrader
 */
class Language_Pack_Upgrader extends MT_Upgrader {
	 ...
	 * @since 3.7.0
	 *
	 * @param false|MT_Upgrader $upgrader Optional. MT_Upgrader instance or false. If `$upgrader` is
	...
	public static function async_upgrade( $upgrader = false ) {
		 ...
		 */
		$check_vcs = new MT_Automatic_Updater;
		if ( $check_vcs->is_vcs_checkout( MT_CONTENT_DIR ) ) {
			return;
		}
	 ...
	 * @return array|bool|MT_Error The result of the upgrade, or a MT_Error object instead.
	 */
	 ...
	 *
	 * @param object[] $language_updates Optional. Array of language packs to update. @see mt_get_translation_updates().
	 *                                   Default empty array.
	 * @param array    $args {
	 ...
	 *                                    Default true.
	 * }
	 * @return array|bool|MT_Error Will return an array of results, or true if there are no updates,
	 *                             false or MT_Error for initial errors.
	 */
	public function bulk_upgrade( $language_updates = array(), $args = array() ) {
		...
		// Remove any existing upgrade filters from the plugin/theme upgraders #MT29425 & #MT29230.
		...
		$res = $this->fs_connect( array( MT_CONTENT_DIR, MT_LANG_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		/*
		 * The filesystem's mkdir() is not recursive. Make sure MT_LANG_DIR exists,
		 * as we then may need to create a /plugins or /themes directory inside of it.
		 */
		$remote_destination = $mt_filesystem->find_folder( MT_LANG_DIR );
		if ( ! $mt_filesystem->exists( $remote_destination ) ) {
			if ( ! $mt_filesystem->mkdir( $remote_destination, FS_CHMOD_DIR ) ) {
				return new MT_Error( 'mkdir_failed_lang_dir', $this->strings['mkdir_failed'], $remote_destination );
			}
		}
	 ...
	 * @global MT_Filesystem_Base $mt_filesystem paCMec filesystem subclass.
	 *
	 * @param string|MT_Error $source        The path to the downloaded package source.
	...
	public function check_package( $source, $remote_source ) {
		...
		$files = $mt_filesystem->dirlist( $remote_source );
	...
	public function get_name_for_update( $update ) {
		switch ( $update->type ) {
			case 'core':
				return 'paCMec'; // Not translated.

			case 'theme':
				$theme = mt_get_theme( $update->slug );
				if ( $theme->exists() ) {
	...
	public function clear_destination( $remote_destination ) {
		...
		$language_directory = MT_LANG_DIR . '/'; // Local path for use with glob().
		...
		$files = array_filter( $files, array( $mt_filesystem, 'exists' ) );
		...
		foreach ( $files as $file ) {
			if ( ! $mt_filesystem->is_writable( $file ) ) {
				// Attempt to alter permissions to allow writes and try again.
				$mt_filesystem->chmod( $file, FS_CHMOD_FILE );
				if ( ! $mt_filesystem->is_writable( $file ) ) {
