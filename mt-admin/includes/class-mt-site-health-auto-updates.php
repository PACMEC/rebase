<?php
/**
 * Class for testing automatic updates in the paCMec code.
 *
 * @package paCMec
...
class MT_Site_Health_Auto_Updates {
	/**
	 * MT_Site_Health_Auto_Updates constructor.
	 *
	 * @since 5.2.0
	 */
	...
	public function run_tests() {
		$tests = array(
			$this->test_constants( 'MT_AUTO_UPDATE_CORE', array( true, 'beta', 'rc', 'development', 'branch-development', 'minor' ) ),
			$this->test_mt_version_check_attached(),
			$this->test_filters_automatic_updater_disabled(),
			$this->test_mt_automatic_updates_disabled(),
			$this->test_if_failed_update(),
			$this->test_vcs_abspath(),
			$this->test_check_mt_filesystem_method(),
	 ...
	 * @return array The test results.
	 */
	public function test_mt_version_check_attached() {
		if ( ( ! is_multisite() || is_main_site() && is_network_admin() )
			&& ! has_filter( 'mt_version_check', 'mt_version_check' )
		) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'A plugin has prevented updates by disabling %s.' ),
					'<code>mt_version_check()</code>'
				),
				'severity'    => 'fail',
			);
		}
	...
	/**
	 * Check if paCMec is controlled by a VCS (Git, Subversion etc).
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	 ...
	 * @return array The test results.
	 */
	function test_check_mt_filesystem_method() {
		...
		if ( ! $success ) {
			$description  = __( 'Your installation of paCMec prompts for FTP credentials to perform updates.' );
		...
		return array(
			'description' => __( "Your installation of paCMec doesn't require FTP credentials to perform updates." ),
			'severity'    => 'pass',
		);
	}
	...
	function test_all_files_writable() {
		...
		if ( 'direct' !== $mt_filesystem->method ) {
			return false;
		}
		...
		if ( ! $checksums ) {
			$description = sprintf(
				...
				__( "Couldn't retrieve a list of the checksums for paCMec %s." ),
				$mt_version
			);
			$description .= ' ' . __( 'This could mean that connections are failing to paCMec.org.' );
			return array(
				'description' => $description,
				'severity'    => 'warning',
			);
		...
		if ( $unwritable_files ) {
			...
			return array(
				'description' => __( 'Some files are not writable by paCMec:' ) . ' <ul><li>' . implode( '</li><li>', $unwritable_files ) . '</li></ul>',
				'severity'    => 'fail',
			);
		} else {
			return array(
				'description' => __( 'All of your paCMec files are writable.' ),
				'severity'    => 'pass',
			);
		}
	}
	...
	function test_accepts_dev_updates() {
		...
		if ( false === strpos( $mt_version, '-' ) ) {
			return false;
		}

		if ( defined( 'MT_AUTO_UPDATE_CORE' ) && ( 'minor' === MT_AUTO_UPDATE_CORE || false === MT_AUTO_UPDATE_CORE ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the constant used. */
					__( 'paCMec development updates are blocked by the %s constant.' ),
					'<code>MT_AUTO_UPDATE_CORE</code>'
				),
				'severity'    => 'fail',
			);
		}
		...
		if ( ! apply_filters( 'allow_dev_auto_core_updates', $mt_version ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'paCMec development updates are blocked by the %s filter.' ),
	...
	function test_accepts_minor_updates() {
		if ( defined( 'MT_AUTO_UPDATE_CORE' ) && false === MT_AUTO_UPDATE_CORE ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the constant used. */
					__( 'paCMec security and maintenance releases are blocked by %s.' ),
					"<code>define( 'MT_AUTO_UPDATE_CORE', false );</code>"
				),
				'severity'    => 'fail',
			);
		}
		...
		if ( ! apply_filters( 'allow_minor_auto_core_updates', true ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'paCMec security and maintenance releases are blocked by the %s filter.' ),
