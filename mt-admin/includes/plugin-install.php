<?php
/**
 * paCMec Plugin Install Administration API
 *
 * @package paCMec
...
/**
 * Retrieves plugin installer pages from the paCMec.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the paCMec.org
 ...
 *         @type bool $requires          Whether to return the required paCMec version. Default true.
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the paCMec.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current paCMec.org Plugin Installation API request.
	 *
	...
	if ( false === $res ) {
		...
		if ( is_mt_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new MT_Error( 'plugins_api_failed', $res->error );
			}
		}
	} elseif ( ! is_mt_error( $res ) ) {
		$res->external = true;
	}
...
/**
 * Retrieve popular paCMec plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	...
	if ( is_mt_error( $api_tags ) ) {
		echo $api_tags->get_error_message();
	} else {
		// Set up the tags in a way which can be interpreted by mt_generate_tag_cloud().
		$tags = array();
...
function install_search_form( $deprecated = true ) {
	$type = isset( $_REQUEST['type'] ) ? mt_unslash( $_REQUEST['type'] ) : 'term';
	$term = isset( $_REQUEST['s'] ) ? mt_unslash( $_REQUEST['s'] ) : '';
	?>
	<form class="search-form search-plugins" method="get">
		...
		<input type="search" name="s" id="search-plugins" value="<?php echo esc_attr( $term ); ?>" class="mt-filter-search" placeholder="<?php esc_attr_e( 'Search plugins...' ); ?>" />
...
<div class="upload-plugin">
	...
	<form method="post" enctype="multipart/form-data" class="mt-upload-form" action="<?php echo self_admin_url( 'update.php?action=upload-plugin' ); ?>">
		<?php mt_nonce_field( 'plugin-upload' ); ?>
...
function install_plugins_favorites_form() {
	...
	$action = 'save_mtorg_username_' . get_current_user_id();
	?>
	<p><?php _e( 'If you have marked plugins as favorites on paCMec.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your paCMec.org username:' ); ?></label>
			<input type="search" id="user" name="user" value="<?php echo esc_attr( $user ); ?>" />
			<input type="submit" class="button" value="<?php esc_attr_e( 'Get Favorites' ); ?>" />
			<input type="hidden" id="mtorg-username-nonce" name="_mtnonce" value="<?php echo esc_attr( mt_create_nonce( $action ) ); ?>" />
		</p>
	</form>
	<?php
}
...
function display_plugins_table() {
	...
	switch ( current_filter() ) {
		case 'install_plugins_favorites':
			if ( empty( $_GET['user'] ) && ! get_user_option( 'mtorg_favorites' ) ) {
				return;
			}
			break;
...
function install_plugin_install_status( $api, $loop = false ) {
		...
		foreach ( (array) $update_plugins->response as $file => $plugin ) {
			if ( $plugin->slug === $api->slug ) {
				...
				if ( current_user_can( 'update_plugins' ) ) {
					$url = mt_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' . $update_file ), 'upgrade-plugin_' . $update_file );
				}
				break;
			}
		}
	...
	if ( 'install' === $status ) {
		if ( is_dir( MT_PLUGIN_DIR . '/' . $api->slug ) ) {
			$installed_plugin = get_plugins( '/' . $api->slug );
			if ( empty( $installed_plugin ) ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$url = mt_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=' . $api->slug ), 'install-plugin_' . $api->slug );
				}
			} else {
	...
	if ( isset( $_GET['from'] ) ) {
		$url .= '&amp;from=' . urlencode( mt_unslash( $_GET['from'] ) );
	}
...
function install_plugin_information() {
	...
	$api = plugins_api(
		'plugin_information',
		array(
			'slug' => mt_unslash( $_REQUEST['plugin'] ),
		)
	);
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$api->sections[ $section_name ] = mt_kses( $content, $plugins_allowedtags );
	}

	foreach ( array( 'version', 'author', 'requires', 'tested', 'homepage', 'downloaded', 'slug' ) as $key ) {
		if ( isset( $api->$key ) ) {
			$api->$key = mt_kses( $api->$key, $plugins_allowedtags );
		}
	}
	...
	$section = isset( $_REQUEST['section'] ) ? mt_unslash( $_REQUEST['section'] ) : 'description';
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires paCMec Version:' ); ?></strong>
					<?php
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on paCMec.org or write your own!' ); ?></p>
			<?php
	...
	$requires_mt  = isset( $api->requires ) ? $api->requires : null;

	$compatible_php = is_php_version_compatible( $requires_php );
	$compatible_mt  = is_mt_version_compatible( $requires_mt );
	$tested_mt      = ( empty( $api->tested ) || version_compare( get_bloginfo( 'version' ), $api->tested, '<=' ) );
	...
	if ( ! $tested_mt ) {
		...
		_e( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of paCMec.' );
		echo '</p></div>';
	} elseif ( ! $compatible_mt ) {
		echo '<div class="notice notice-error notice-alt"><p>';
		_e( '<strong>Error:</strong> This plugin <strong>requires a newer version of paCMec</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			printf(
				/* translators: %s: URL to paCMec Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update paCMec</a>.' ),
	...
	if ( ! empty( $api->download_link ) && ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) ) {
		...
		switch ( $status['status'] ) {
			case 'install':
				if ( $status['url'] ) {
					if ( $compatible_php && $compatible_mt ) {
