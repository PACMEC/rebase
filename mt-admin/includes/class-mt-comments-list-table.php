<?php
/**
 * List Table API: MT_Comments_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_Comments_List_Table extends MT_List_Table {
	...
	public function prepare_items() {
		...
		if ( is_array( $_comments ) ) {
			...
			$_comment_post_ids = array_unique( mt_list_pluck( $_comments, 'comment_post_ID' ) );
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_comments = ( $post_id ) ? mt_count_comments( $post_id ) : mt_count_comments();
		...
		foreach ( $stati as $status => $label ) {
			...
			if ( !empty( $_REQUEST['s'] ) )
				$link = add_query_arg( 's', esc_attr( mt_unslash( $_REQUEST['s'] ) ), $link );
			*/
	...
	protected function extra_tablenav( $which ) {
		...
		if ( ( 'spam' === $comment_status || 'trash' === $comment_status ) && current_user_can( 'moderate_comments' ) && $has_items ) {
			mt_nonce_field( 'bulk-destroy', '_destroy_nonce' );
	...
	public function display() {
		...
		?>
<table class="mt-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
	<thead>
	<tr>
	...
	<tbody id="the-comment-list" data-mt-lists="list:comment">
		<?php $this->display_rows_or_placeholder(); ?>
	</tbody>

	<tbody id="the-extra-comment-list" data-mt-lists="list:comment" style="display: none;">
		<?php
			/*
	...
	public function single_row( $item ) {
		...
		$the_comment_class = mt_get_comment_status( $comment );
	...
	protected function handle_row_actions( $comment, $column_name, $primary ) {
		...
		$the_comment_status = mt_get_comment_status( $comment );
		...
		if ( $comment_status && 'all' !== $comment_status ) {
			if ( 'approved' === $the_comment_status ) {
				$actions['unapprove'] = sprintf(
					'<a href="%s" data-mt-lists="%s" class="vim-u vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
					$unapprove_url,
				...
				$actions['approve'] = sprintf(
					'<a href="%s" data-mt-lists="%s" class="vim-a vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
					$approve_url,
		...
		if ( 'spam' !== $the_comment_status ) {
			...
			$actions['unspam'] = sprintf(
				'<a href="%s" data-mt-lists="%s" class="vim-z vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
				$unspam_url,
		...
		/** This filter is documented in mt-admin/includes/dashboard.php */
		...
		foreach ( $actions as $action => $link ) {
			...
			if ( ( 'reply' === $action || 'quickedit' === $action ) && ! mt_doing_ajax() ) {
				$action .= ' hide-if-no-js';
			} elseif ( ( 'untrash' === $action && 'trash' === $the_comment_status )
				|| ( 'unspam' === $action && 'spam' === $the_comment_status )
			) {
				if ( '1' == get_comment_meta( $comment->comment_ID, '_mt_trash_meta_status', true ) ) {
					$action .= ' approve';
				} else {
					$action .= ' unapprove';
				}
	...
	public function column_comment( $comment ) {
		...
		if ( $this->user_can ) {
			/** This filter is documented in mt-admin/includes/comment.php */
	...
	public function column_author( $comment ) {
		...
		if ( strlen( $author_url_display ) > 50 ) {
			$author_url_display = mt_html_excerpt( $author_url_display, 49, '&hellip;' );
		}
	...
	public function column_date( $comment ) {
		...
		if ( 'approved' === mt_get_comment_status( $comment ) && ! empty( $comment->comment_post_ID ) ) {
			printf(
				'<a href="%s">%s</a>',
	...
	public function column_response( $comment ) {
		...
		if ( 'attachment' === $post->post_type ) {
			$thumb = mt_get_attachment_image( $post->ID, array( 80, 60 ), true );
			if ( $thumb ) {
				echo $thumb;
			}
		}
