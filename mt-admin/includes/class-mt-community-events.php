...
class MT_Community_Events {
	...
	/**
	 * Constructor for MT_Community_Events.
	 *
	 * @since 4.8.0
	 *
	 * @param int        $user_id       MT user ID.
	 ...
	 * directly, it would also need to make a second request to MT in order to
	 * pass the data for caching. Having MT make the request also introduces
	 ...
	 *              in `mt.communityEvents.populateDynamicEventFields()` now.
	 *
	 * @param string $location_search Optional. City name to help determine the location.
	 *                                e.g., "Seattle". Default empty string.
	 * @param string $timezone        Optional. Timezone to help determine the location.
	 *                                Default empty string.
	 * @return array|MT_Error A MT_Error on failure; an array with location and events on
	 *                        success.
	 */
	public function get_events( $location_search = '', $timezone = '' ) {
		...
		$response       = mt_remote_get( $api_url, $request_args );
		$response_code  = mt_remote_retrieve_response_code( $response );
		$response_body  = json_decode( mt_remote_retrieve_body( $response ), true );
		$response_error = null;

		if ( is_mt_error( $response ) ) {
			$response_error = $response;
		} elseif ( 200 !== $response_code ) {
			$response_error = new MT_Error(
				'api-error',
	...
	protected function get_request_args( $search = '', $timezone = '' ) {
		...
		// Wrap the args in an array compatible with the second parameter of `mt_remote_get()`.
		return array(
			'body' => $args,
		);
	}
	...
	public static function get_unsafe_client_ip() {
		...
		$anon_ip = mt_privacy_anonymize_ip( $client_ip, true );
			...
			foreach ( $response_body['events'] as $key => $event ) {
				...
				if ( isset( $event['end_date'] ) ) {
					...
					if ( 'meetup' !== $event['type'] && $formatted_end_date !== $formatted_date ) {
						...
						$formatted_date = mt_maybe_decline_date( $formatted_date, 'F j, Y' );
					}
				}
	...
	protected function trim_events( array $events ) {
		...
		$trimmed_event_types = mt_list_pluck( $trimmed_events, 'type' );
