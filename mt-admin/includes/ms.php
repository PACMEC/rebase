...
function check_upload_size( $file ) {
	...
	if ( '0' != $file['error'] && ! isset( $_POST['html-upload'] ) && ! mt_doing_ajax() ) {
		mt_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );
	}
 ...
 *
 * @since 3.0.0
 * @since 5.1.0 Use mt_delete_site() internally to delete the site row from the database.
 *
 ...
 */
function mtmu_delete_blog( $blog_id, $drop = false ) {
	global $mtdb;
	...
	// If ms_files_rewriting is enabled and upload_path is empty, mt_upload_dir is not reliable.
	if ( $drop && get_site_option( 'ms_files_rewriting' ) && empty( $upload_path ) ) {
		$drop = false;
	}

	if ( $drop ) {
		mt_delete_site( $blog_id );
	} else {
		/** This action is documented in mt-includes/ms-blogs.php */
 ...
 * @since 3.0.0
 *
 * @todo Merge with mt_delete_user()?
 *
 * @global mtdb $mtdb paCMec database abstraction object.
 *
...
function mtmu_delete_user( $id ) {
	 ...
	 * @param MT_User $user MT_User object of the user about to be deleted from the network.
	 */
	do_action( 'mtmu_delete_user', $id, $user );
	...
	/** This action is documented in mt-admin/includes/user.php */
 ...
 * @since 3.0.0
 *
 * @param MT_Term|array $term     The term.
 * @param string        $taxonomy The taxonomy for `$term`. Should be 'category' or 'post_tag', as these are
 *                                the only taxonomies which are processed by this function; anything else
 *                                will be returned untouched.
 * @return MT_Term|array Returns `$term`, after filtering the 'slug' field with `sanitize_title()`
...
function _access_denied_splash() {
	...
	if ( mt_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) ) {
		return;
	}
...
function site_admin_notice() {
	...
	if ( get_site_option( 'mtmu_upgrade_site' ) != $mt_db_version ) {
...
function confirm_delete_users( $users ) {
	...
	<?php
	mt_nonce_field( 'ms-users-delete' );
	...
	<?php
	/** This action is documented in mt-admin/users.php */
...
function network_edit_site_nav( $args = array() ) {
	...
	// All done!
	echo '<nav class="nav-tab-wrapper mt-clearfix" aria-label="' . esc_attr__( 'Secondary menu' ) . '">';
...
function get_site_screen_help_tab_args() {
	return array(
		...
		'content' =>
			...
			'<p>' . __( '<strong>Settings</strong> &mdash; This page shows a list of all settings associated with this site. Some are created by paCMec and others are created by plugins you activate. Note that some fields are grayed out and say Serialized Data. You cannot modify these values due to the way the setting is stored in the database.' ) . '</p>',
	);
}
