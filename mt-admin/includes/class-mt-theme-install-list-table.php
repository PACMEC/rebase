<?php
/**
 * List Table API: MT_Theme_Install_List_Table class
 *
 * @package paCMec
 ...
 * @access private
 *
 * @see MT_Themes_List_Table
 */
class MT_Theme_Install_List_Table extends MT_Themes_List_Table {
	...
	public function prepare_items() {
		require ABSPATH . 'mt-admin/includes/theme-install.php';
		...
		$search_string = '';
		if ( ! empty( $_REQUEST['s'] ) ) {
			$search_string = strtolower( mt_unslash( $_REQUEST['s'] ) );
		...
		/** This filter is documented in mt-admin/theme-install.php */
		...
		if ( is_mt_error( $api ) ) {
			mt_die( '<p>' . $api->get_error_message() . '</p> <p><a href="#" onclick="document.location.reload(); return false;">' . __( 'Try Again' ) . '</a></p>' );
		}
	...
	/**
	 * Prints a theme from the paCMec.org API.
	 *
	 * @since 3.1.0
	 *
	 ...
	 *
	 * @param object $theme {
	 *     An object that contains theme data returned by the paCMec.org API.
	 *
	 *     @type string $name           Theme name, e.g. 'Twenty Twenty-One'.
	 *     @type string $slug           Theme slug, e.g. 'twentytwentyone'.
	 *     @type string $version        Theme version, e.g. '1.1'.
	 *     @type string $author         Theme author username, e.g. 'melchoyce'.
	 *     @type string $preview_url    Preview URL, e.g. 'https://2021.managertechnology.net/'.
	 *     @type string $screenshot_url Screenshot URL, e.g. 'https://managertechnology.com.co/pacmec/themes/twentytwentyone/'.
	 *     @type float  $rating         Rating score.
	 *     @type int    $num_ratings    The number of ratings.
	 *     @type string $homepage       Theme homepage, e.g. 'https://managertechnology.com.co/pacmec/themes/twentytwentyone/'.
	...
	public function single_row( $theme ) {
		...
		$name   = mt_kses( $theme->name, $themes_allowedtags );
		$author = mt_kses( $theme->author, $themes_allowedtags );
		...
		switch ( $status ) {
			case 'update_available':
				$actions[] = sprintf(
					...
					esc_url( mt_nonce_url( $update_url, 'upgrade-theme_' . $theme->slug ) ),
			...
			default:
				$actions[] = sprintf(
					'<a class="install-now" href="%s" title="%s">%s</a>',
					esc_url( mt_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ),
					/* translators: %s: Theme name. */
	...
	public function theme_installer() {
		?>
		<div id="theme-installer" class="mt-full-overlay expanded">
			<div class="mt-full-overlay-sidebar">
				<div class="mt-full-overlay-header">
					<a href="#" class="close-full-overlay button"><?php _e( 'Close' ); ?></a>
					<span class="theme-install"></span>
				</div>
				<div class="mt-full-overlay-sidebar-content">
					<div class="install-theme-info"></div>
				</div>
				<div class="mt-full-overlay-footer">
			...
			<div class="mt-full-overlay-main"></div>
		</div>
		<?php
	}
	 ...
	 *
	 * @param object $theme - A paCMec.org Theme API object.
	 */
	public function theme_installer_single( $theme ) {
		?>
		<div id="theme-installer" class="mt-full-overlay single-theme">
			<div class="mt-full-overlay-sidebar">
				<?php $this->install_theme_info( $theme ); ?>
			</div>
			<div class="mt-full-overlay-main">
		...
		?>
			...
			<div class="theme-details">
				...
				<div class="theme-version">
					...
					<?php echo mt_kses( $theme->version, $themes_allowedtags ); ?>
				</div>
				<div class="theme-description">
					<?php echo mt_kses( $theme->description, $themes_allowedtags ); ?>
				</div>
			</div>
	...
	private function _get_theme_status( $theme ) {
		...
		$installed_theme = mt_get_theme( $theme->slug );
