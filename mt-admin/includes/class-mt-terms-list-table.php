<?php
/**
 * List Table API: MT_Terms_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_Terms_List_Table extends MT_List_Table {
	...
	public function prepare_items() {
		...
		$search = ! empty( $_REQUEST['s'] ) ? trim( mt_unslash( $_REQUEST['s'] ) ) : '';
		...
		if ( ! empty( $_REQUEST['orderby'] ) ) {
			$args['orderby'] = trim( mt_unslash( $_REQUEST['orderby'] ) );
		}

		if ( ! empty( $_REQUEST['order'] ) ) {
			$args['order'] = trim( mt_unslash( $_REQUEST['order'] ) );
		}
	...
	public function column_name( $tag ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see MT_Terms_List_Table::column_name()
		 *
		...
		$uri = mt_doing_ajax() ? mt_get_referer() : $_SERVER['REQUEST_URI'];
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_Term $tag         Tag being acted upon.
	...
	protected function handle_row_actions( $tag, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_term', $tag->term_id ) ) {
			$actions['delete'] = sprintf(
				...
				mt_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ),
				/* translators: %s: Taxonomy term name. */
	...
	public function inline_edit() {
		...
		<table style="display: none"><tbody id="inlineedit">
			...
			<div class="inline-edit-save submit">
				...
				<?php mt_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>
