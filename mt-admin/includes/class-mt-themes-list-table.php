<?php
/**
 * List Table API: MT_Themes_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_Themes_List_Table extends MT_List_Table {
	...
	public function prepare_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			$this->search_terms = array_unique( array_filter( array_map( 'trim', explode( ',', strtolower( mt_unslash( $_REQUEST['s'] ) ) ) ) ) );
		}
		...
		if ( is_multisite() ) {
			...
			if ( current_user_can( 'install_themes' ) ) {
				printf(
					...
					__( 'You only have one theme installed right now. Live a little! You can choose from over 1,000 free themes in the paCMec Theme Directory at any time: just click on the <a href="%s">Install Themes</a> tab above.' ),
	...
	public function display_rows() {
		...
		foreach ( $themes as $theme ) :
			...
			$activate_link = mt_nonce_url( 'themes.php?action=activate&amp;template=' . urlencode( $template ) . '&amp;stylesheet=' . urlencode( $stylesheet ), 'switch-theme_' . $stylesheet );
			...
			if ( ! is_multisite() && current_user_can( 'delete_themes' ) ) {
				$actions['delete'] = sprintf(
					...
					mt_nonce_url( 'themes.php?action=delete&amp;stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet ),
					/* translators: %s: Theme name. */
			...
			/** This filter is documented in mt-admin/includes/class-mt-ms-themes-list-table.php */
			...
			<a href="<?php echo mt_customize_url( $stylesheet ); ?>" class="screenshot load-customize hide-if-no-customize">
				<?php if ( $screenshot ) : ?>
					<img src="<?php echo esc_url( $screenshot ); ?>" alt="" />
				<?php endif; ?>
			</a>
	...
	public function _js_vars( $extra_args = array() ) {
		$search_string = isset( $_REQUEST['s'] ) ? esc_attr( mt_unslash( $_REQUEST['s'] ) ) : '';
