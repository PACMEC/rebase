<?php
/**
 * MT_Privacy_Policy_Content class.
 *
 * @package paCMec
...
final class MT_Privacy_Policy_Content {
	 ...
	 *
	 * Intended for use from `mt_add_privacy_policy_content()`.
	 *
	 * @since 4.9.6
	 *
	...
	public static function text_change_check() {
		...
		$old = (array) get_post_meta( $policy_page_id, '_mt_suggested_privacy_policy_content' );
		...
		$cached = get_option( '_mt_suggested_policy_text_has_changed' );
		...
		if ( $new != $old ) {
			...
			add_action( 'admin_notices', array( 'MT_Privacy_Policy_Content', 'policy_text_changed_notice' ) );
			$state = 'changed';
		} else {
			$state = 'not-changed';
		}
		...
		if ( $cached !== $state ) {
			update_option( '_mt_suggested_policy_text_has_changed', $state );
		}
	...
	public static function _policy_page_updated( $post_id ) {
		...
		if ( $update_cache ) {
			delete_post_meta( $policy_page_id, '_mt_suggested_privacy_policy_content' );
			// Update the cache.
			foreach ( $done as $data ) {
				add_post_meta( $policy_page_id, '_mt_suggested_privacy_policy_content', $data );
			}
		}
	}
	 ...
	 *
	 * @param MT_Post|null $post The currently edited post. Default null.
	 */
	public static function notice( $post = null ) {
		...
		if ( get_current_screen()->is_block_editor() ) {
			...
			mt_add_inline_script(
				'mt-notices',
				sprintf(
					'mt.data.dispatch( "core/notices" ).createWarningNotice( "%s", { actions: [ %s ], isDismissible: false } )',
					$message,
					mt_json_encode( $action )
				),
				'after'
			);
		} else {
			?>
			<div class="notice notice-warning inline mt-pp-notice">
				<p>
				<?php
				echo $message;
				printf(
	...
	public static function get_default_content( $description = false, $blocks = true ) {
		...
		if ( $description ) {
			$strings[] = '<div class="mt-suggested-text">';
		}
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default paCMec does not collect any personal data about visitors, and only collects the data shown on the User Profile screen from registered users. However some of your plugins may collect personal data. You should add the relevant information below.' ) . '</p>';
		}
		...
		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should note what information is captured through comments. We have noted the data which paCMec collects by default.' ) . '</p>';
		} else {
		...
		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should list the cookies your web site uses, including those set by your plugins, social media, and analytics. We have provided the cookies which paCMec installs by default.' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default paCMec does not collect any analytics data. However, many web hosting accounts collect some anonymous analytics data. You may also have installed a paCMec plugin that provides analytics services. In that case, add information from that plugin here.' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default paCMec does not share any personal data with anyone.' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
		...
		if ( $blocks ) {
			foreach ( $strings as $key => $string ) {
				if ( 0 === strpos( $string, '<p>' ) ) {
					$strings[ $key ] = '<!-- mt:paragraph -->' . $string . '<!-- /mt:paragraph -->';
				}

				if ( 0 === strpos( $string, '<h2>' ) ) {
					$strings[ $key ] = '<!-- mt:heading -->' . $string . '<!-- /mt:heading -->';
				}
			}
		}
		 ...
		 * @deprecated 5.7.0 Use mt_add_privacy_policy_content() instead.
		 *
		...
		return apply_filters_deprecated(
			'mt_get_default_privacy_policy_content',
			array( $content, $strings, $description, $blocks ),
			'5.7.0',
			'mt_add_privacy_policy_content()'
		);
	}
	...
	public static function add_suggested_content() {
		...
		mt_add_privacy_policy_content( __( 'paCMec' ), $content );
	}
}
