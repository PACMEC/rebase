<?php
/**
 * List Table API: MT_Posts_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_Posts_List_Table extends MT_List_Table {
	 ...
	 * @since 3.1.0
	 *
	 * @see MT_List_Table::__construct() for more information on default arguments.
	 *
	...
	public function __construct( $args = array() ) {
		...
		$this->user_posts_count = (int) $mtdb->get_var(
			$mtdb->prepare(
				"
			SELECT COUNT( 1 )
			FROM $mtdb->posts
		...
		if ( 'post' === $post_type && $sticky_posts ) {
			...
			$this->sticky_posts_count = $mtdb->get_var( $mtdb->prepare( "SELECT COUNT( 1 ) FROM $mtdb->posts WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND ID IN ($sticky_posts)", $post_type ) );
		}
	}
	...
	public function prepare_items() {
		global $mode, $avail_post_stati, $mt_query, $per_page;
		...
		// Is going to call mt().
		$avail_post_stati = mt_edit_posts_query();

		$this->set_hierarchical_display( is_post_type_hierarchical( $this->screen->post_type ) && 'menu_order title' === $mt_query->query['orderby'] );
		...
		if ( $this->hierarchical_display ) {
			$total_items = $mt_query->post_count;
		} elseif ( $mt_query->found_posts || $this->get_pagenum() === 1 ) {
			$total_items = $mt_query->found_posts;
		} else {
			$post_counts = (array) mt_count_posts( $post_type, 'readable' );
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_posts    = mt_count_posts( $post_type, 'readable' );
	...
	protected function categories_dropdown( $post_type ) {
		...
		if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
			...
			mt_dropdown_categories( $dropdown_options );
		}
	}
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			 ...
			 *                          'top' or 'bottom' for MT_Posts_List_Table,
			 *                          'bar' for MT_Media_List_Table.
			 */
	...
	public function get_columns() {
		...
		$taxonomies = mt_filter_object_list( $taxonomies, array( 'show_admin_column' => true ), 'and', 'name' );
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		global $mt_query, $per_page;

		if ( empty( $posts ) ) {
			$posts = $mt_query->posts;
		}
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent == $page->ID ) {
					$page->post_parent = 0;
					$mtdb->update( $mtdb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_Post $post The current MT_Post object.
	 */
	public function column_cb( $post ) {
		 ...
		 * @param MT_Post $post The current MT_Post object.
		 */
		if ( apply_filters( 'mt_list_table_show_post_checkbox', $show, $post ) ) :
			?>
	...
	public function column_title( $post ) {
		...
		if ( $can_edit_post && 'trash' !== $post->post_status ) {
			$lock_holder = mt_check_post_lock( $post->ID );
	...
	public function column_default( $post, $column_name ) {
		...
		if ( $taxonomy ) {
			...
			if ( is_array( $terms ) ) {
				 ...
				 * @param MT_Term[] $terms      Array of term objects appearing in the post row.
				 */
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_Post $post        Post being acted upon.
	...
	protected function handle_row_actions( $post, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			if ( 'trash' === $post->post_status ) {
				$actions['untrash'] = sprintf(
					'<a href="%s" aria-label="%s">%s</a>',
					mt_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ),
					/* translators: %s: Post title. */
		...
		if ( 'mt_block' === $post->post_type ) {
			$actions['export'] = sprintf(
				'<button type="button" class="mt-list-reusable-blocks__export button-link" data-id="%s" aria-label="%s">%s</button>',
				$post->ID,
				/* translators: %s: Post title. */
	...
	public function inline_edit() {
		...
		while ( $bulk < 2 ) :
			...
			<fieldset class="inline-edit-col-left">
				...
				if ( post_type_supports( $screen->post_type, 'author' ) ) :
					...
					if ( current_user_can( $post_type_object->cap->edit_others_posts ) ) :
						 ...
						 * @since 5.6.0
						 *
						 * @see mt_dropdown_users()
						 *
						 * @param array $users_opt An array of arguments passed to mt_dropdown_users().
						 * @param bool  $bulk      A flag to denote if it's a bulk action.
						 */
						$users_opt = apply_filters( 'quick_edit_dropdown_authors_args', $users_opt, $bulk );

						$authors = mt_dropdown_users( $users_opt );
						if ( $authors ) :
				...
				<?php if ( ! $bulk && $can_publish ) : ?>
					...
					<div class="inline-edit-group mt-clearfix">
				...
				<fieldset class="inline-edit-col-center inline-edit-categories">
					...
					<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
						...
						<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name ); ?>-checklist">
							<?php mt_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ); ?>
						</ul>
				...
				<?php if ( post_type_supports( $screen->post_type, 'page-attributes' ) ) : ?>
					...
					<?php if ( $post_type_object->hierarchical ) : ?>
						...
						<label>
							 ...
							 *
							 * @param array $dropdown_args An array of arguments passed to mt_dropdown_pages().
							 * @param bool  $bulk          A flag to denote if it's a bulk action.
							 */
							$dropdown_args = apply_filters( 'quick_edit_dropdown_pages_args', $dropdown_args, $bulk );

							mt_dropdown_pages( $dropdown_args );
							?>
						</label>
					...
					<?php foreach ( $flat_taxonomies as $taxonomy ) : ?>
						...
						<?php if ( current_user_can( $taxonomy->cap->assign_terms ) ) : ?>
							...
							<label class="inline-edit-tags">
								...
								<textarea data-mt-taxonomy="<?php echo $taxonomy_name; ?>" cols="22" rows="1" name="tax_input[<?php echo $taxonomy_name; ?>]" class="tax_input_<?php echo $taxonomy_name; ?>"></textarea>
							</label>
			...
			<div class="submit inline-edit-save">
				...
				<?php if ( ! $bulk ) : ?>
					<?php mt_nonce_field( 'inlineeditnonce', '_inline_edit', false ); ?>
