<?php
/**
 * List Table API: MT_Plugins_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_Plugins_List_Table extends MT_List_Table {
	/**
	...
	public function __construct( $args = array() ) {
		...
		if ( isset( $_REQUEST['s'] ) ) {
			$_SERVER['REQUEST_URI'] = add_query_arg( 's', mt_unslash( $_REQUEST['s'] ) );
		}

		$page = $this->get_pagenum();

		$this->show_autoupdates = mt_is_auto_update_enabled_for_type( 'plugin' )
	...
	public function prepare_items() {
		...
		mt_reset_vars( array( 'orderby', 'order' ) );
		...
		if ( ! is_multisite() || ( $screen->in_admin( 'network' ) && current_user_can( 'manage_network_plugins' ) ) ) {
			...
			/** This action is documented in mt-admin/includes/class-mt-plugins-list-table.php */
		...
		foreach ( (array) $plugins['all'] as $plugin_file => $plugin_data ) {
			...
			$filter_payload = (object) mt_parse_args( $plugin_data, $filter_payload );

			$auto_update_forced = mt_is_auto_update_forced_for_item( 'plugin', null, $filter_payload );
	...
	public function no_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			$s = esc_html( mt_unslash( $_REQUEST['s'] ) );
			...
			if ( ! is_multisite() && current_user_can( 'install_plugins' ) ) {
				echo ' <a href="' . esc_url( admin_url( 'plugin-install.php?tab=search&s=' . urlencode( $s ) ) ) . '">' . __( 'Search for plugins in the paCMec Plugin Directory.' ) . '</a>';
			}
	...
	public function search_box( $text, $input_id ) {
		...
		<p class="search-box">
			...
			<input type="search" id="<?php echo esc_attr( $input_id ); ?>" class="mt-filter-search" name="s" value="<?php _admin_search_query(); ?>" placeholder="<?php esc_attr_e( 'Search installed plugins...' ); ?>"/>
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'recently_activated' === $status ) {
			...
			echo '<p>' . sprintf(
				...
				'<code>' . str_replace( ABSPATH, '/', MTMU_PLUGIN_DIR ) . '</code>'
			) . '</p>';
		...
		if ( 'mustuse' === $context ) {
			...
			if ( true === ( $dropins[ $plugin_file ][1] ) ) { // Doesn't require a constant.
				...
				$description = '<p><strong>' . $dropins[ $plugin_file ][0] . ' <span class="error-message">' . __( 'Inactive:' ) . '</span></strong> ' .
					sprintf(
						/* translators: 1: Drop-in constant name, 2: mt-config.php */
				...
				if ( $is_active ) {
					if ( current_user_can( 'manage_network_plugins' ) ) {
						$actions['deactivate'] = sprintf(
							...
							mt_nonce_url( 'plugins.php?action=deactivate&amp;plugin=' . urlencode( $plugin_file ) . '&amp;plugin_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'deactivate-plugin_' . $plugin_file ),
					...
					if ( current_user_can( 'manage_network_plugins' ) ) {
						$actions['activate'] = sprintf(
							...
							mt_nonce_url( 'plugins.php?action=activate&amp;plugin=' . urlencode( $plugin_file ) . '&amp;plugin_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'activate-plugin_' . $plugin_file ),
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
					...
					if ( current_user_can( 'delete_plugins' ) && ! is_plugin_active( $plugin_file ) ) {
						$actions['delete'] = sprintf(
							...
							mt_nonce_url( 'plugins.php?action=delete-selected&amp;checked[]=' . urlencode( $plugin_file ) . '&amp;plugin_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'bulk-plugins' ),
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
				...
				if ( $restrict_network_active ) {
					...
					if ( current_user_can( 'resume_plugin', $plugin_file ) && is_plugin_paused( $plugin_file ) ) {
						$actions['resume'] = sprintf(
							...
							mt_nonce_url( 'plugins.php?action=resume&amp;plugin=' . urlencode( $plugin_file ) . '&amp;plugin_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'resume-plugin_' . $plugin_file ),
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
			...
			switch ( $column_name ) {
				...
				case 'description':
					...
					if ( $paused ) {
						...
						$error = mt_get_plugin_error( $plugin_file );

						if ( false !== $error ) {
							printf( '<div class="error-display"><p>%s</p></div>', mt_get_extension_error_description( $error ) );
						}
					}
				...
				case 'auto-updates':
					...
					if ( 'unavailable' === $action ) {
						...
						$html[] = sprintf(
							'<a href="%s" class="toggle-auto-update aria-button-if-js" data-mt-action="%s">',
							mt_nonce_url( $url, 'updates' ),
							$action
						);
