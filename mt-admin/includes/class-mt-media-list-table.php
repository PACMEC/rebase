<?php
/**
 * List Table API: MT_Media_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_Media_List_Table extends MT_List_Table {
	/**
	...
	public function prepare_items() {
		global $mode, $mt_query, $post_mime_types, $avail_post_mime_types;
		...
		list( $post_mime_types, $avail_post_mime_types ) = mt_edit_attachments_query( $_REQUEST );
		...
		$this->set_pagination_args(
			array(
				...
				'total_pages' => $mt_query->max_num_pages,
				'per_page'    => $mt_query->query_vars['posts_per_page'],
			)
		);
	}
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			$selected = selected(
				$filter && 0 === strpos( $filter, 'post_mime_type:' ) &&
					mt_match_mime_types( $mime_type, str_replace( 'post_mime_type:', '', $filter ) ),
				true,
				false
			);
	...
	protected function extra_tablenav( $which ) {
		...
		<div class="actions">
			...
			/** This action is documented in mt-admin/includes/class-mt-posts-list-table.php */
	...
	public function column_title( $post ) {
		...
		$thumb      = mt_get_attachment_image( $post->ID, array( 60, 60 ), true, array( 'alt' => '' ) );
		$link_start = '';
		$link_end   = '';
		...
		<p class="filename">
			...
			echo esc_html( mt_basename( $file ) );
			?>
		</p>
		<?php
	}
			...
			if ( $user_can_edit ) :
				$detach_url = add_query_arg(
					array(
						...
						'media[]'        => $post->ID,
						'_mtnonce'       => mt_create_nonce( 'bulk-' . $this->_args['plural'] ),
					),
					'upload.php'
				);
				printf(
	...
	public function display_rows() {
		...
		$post_ids = mt_list_pluck( $mt_query->posts, 'ID' );
		reset( $mt_query->posts );
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = sprintf(
						...
						mt_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-post_' . $post->ID ),
						/* translators: %s: Attachment title. */
					...
					$actions['delete'] = sprintf(
						...
						mt_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-post_' . $post->ID ),
						$delete_ays,
						/* translators: %s: Attachment title. */
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash ) {
					$actions['untrash'] = sprintf(
						'<a href="%s" class="submitdelete aria-button-if-js" aria-label="%s">%s</a>',
						mt_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-post_' . $post->ID ),
						/* translators: %s: Attachment title. */
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_Post $post        Attachment being acted upon.
	 * @param string  $column_name Current column name.
