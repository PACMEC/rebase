 ...
 * @return array|MT_Error Array of post data on success, MT_Error on failure.
 */
function _mt_translate_postdata( $update = false, $post_data = null ) {
	...
	if ( $update && ! current_user_can( 'edit_post', $post_data['ID'] ) ) {
		if ( 'page' === $post_data['post_type'] ) {
			return new MT_Error( 'edit_others_pages', __( 'Sorry, you are not allowed to edit pages as this user.' ) );
		} else {
			return new MT_Error( 'edit_others_posts', __( 'Sorry, you are not allowed to edit posts as this user.' ) );
		}
	} elseif ( ! $update && ! current_user_can( $ptype->cap->create_posts ) ) {
		if ( 'page' === $post_data['post_type'] ) {
			return new MT_Error( 'edit_others_pages', __( 'Sorry, you are not allowed to create pages as this user.' ) );
		} else {
			return new MT_Error( 'edit_others_posts', __( 'Sorry, you are not allowed to create posts as this user.' ) );
		}
	}
	...
	if ( ! empty( $post_data['edit_date'] ) ) {
		...
		$valid_date             = mt_checkdate( $mm, $jj, $aa, $post_data['post_date'] );
		if ( ! $valid_date ) {
 ...
 * @return array|MT_Error Array of post data on success, MT_Error on failure.
 */
function _mt_get_allowed_postdata( $post_data = null ) {
	if ( empty( $post_data ) ) {
		$post_data = $_POST;
	}
...
function edit_post( $post_data = null ) {
	...
	if ( post_type_supports( $ptype->name, 'revisions' ) ) {
		$revisions = mt_get_post_revisions(
			$post_ID,
			array(
				'order'          => 'ASC',
				'posts_per_page' => 1,
		...
		if ( $revisions && _mt_get_post_revision_version( $revision ) < 1 ) {
			_mt_upgrade_revisions_of_post( $post, mt_get_post_revisions( $post_ID ) );
		}
	}
	...
	$post_data = _mt_translate_postdata( true, $post_data );
	if ( is_mt_error( $post_data ) ) {
		mt_die( $post_data->get_error_message() );
	}
	$translated = _mt_get_allowed_postdata( $post_data );
	...
	foreach ( $format_meta_urls as $format_meta_url ) {
		...
		if ( isset( $post_data[ $keyed ] ) ) {
			update_post_meta( $post_ID, $keyed, mt_slash( esc_url_raw( mt_unslash( $post_data[ $keyed ] ) ) ) );
		}
	}
	...
	foreach ( $format_keys as $key ) {
		...
		if ( isset( $post_data[ $keyed ] ) ) {
			if ( current_user_can( 'unfiltered_html' ) ) {
				...
				update_post_meta( $post_ID, $keyed, mt_filter_post_kses( $post_data[ $keyed ] ) );
			}
		}
	}

	if ( 'attachment' === $post_data['post_type'] && preg_match( '#^(audio|video)/#', $post_data['post_mime_type'] ) ) {
		$id3data = mt_get_attachment_metadata( $post_ID );
		if ( ! is_array( $id3data ) ) {
			$id3data = array();
		}

		foreach ( mt_get_attachment_id3_keys( $post, 'edit' ) as $key => $label ) {
			if ( isset( $post_data[ 'id3_' . $key ] ) ) {
				$id3data[ $key ] = sanitize_text_field( mt_unslash( $post_data[ 'id3_' . $key ] ) );
			}
		}
		mt_update_attachment_metadata( $post_ID, $id3data );
	}
	...
	if ( 'attachment' === $post_data['post_type'] ) {
		if ( isset( $post_data['_mt_attachment_image_alt'] ) ) {
			$image_alt = mt_unslash( $post_data['_mt_attachment_image_alt'] );

			if ( get_post_meta( $post_ID, '_mt_attachment_image_alt', true ) !== $image_alt ) {
				$image_alt = mt_strip_all_tags( $image_alt, true );

				// update_post_meta() expects slashed.
				update_post_meta( $post_ID, '_mt_attachment_image_alt', mt_slash( $image_alt ) );
			}
		}
	...
	$success = mt_update_post( $translated );

	// If the save failed, see if we can sanity check the main fields and try again.
	if ( ! $success && is_callable( array( $mtdb, 'strip_invalid_text_for_column' ) ) ) {
		$fields = array( 'post_title', 'post_content', 'post_excerpt' );

		foreach ( $fields as $field ) {
			if ( isset( $translated[ $field ] ) ) {
				$translated[ $field ] = $mtdb->strip_invalid_text_for_column( $mtdb->posts, $field, $translated[ $field ] );
			}
		}

		mt_update_post( $translated );
	}
...
function bulk_edit_posts( $post_data = null ) {
	...
	if ( ! current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' === $ptype->name ) {
			mt_die( __( 'Sorry, you are not allowed to edit pages.' ) );
		} else {
			mt_die( __( 'Sorry, you are not allowed to edit posts.' ) );
		}
	}
	...
	if ( isset( $post_data['post_parent'] ) && (int) $post_data['post_parent'] ) {
		...
		$pages    = $mtdb->get_results( "SELECT ID, post_parent FROM $mtdb->posts WHERE post_type = 'page'" );
		$children = array();
	...
	foreach ( $post_IDs as $post_ID ) {
		...
		foreach ( $tax_names as $tax_name ) {
			...
			if ( $taxonomy_obj->hierarchical ) {
				$current_terms = (array) mt_get_object_terms( $post_ID, $tax_name, array( 'fields' => 'ids' ) );
			} else {
				$current_terms = (array) mt_get_object_terms( $post_ID, $tax_name, array( 'fields' => 'names' ) );
			}
		...
		if ( isset( $new_cats ) && in_array( 'category', $tax_names, true ) ) {
			$cats                       = (array) mt_get_post_categories( $post_ID );
		...
		$post_data = _mt_get_allowed_postdata( $post_data );

		if ( isset( $shared_post_data['post_format'] ) ) {
			set_post_format( $post_ID, $shared_post_data['post_format'] );
		}

		// Prevent mt_insert_post() from overwriting post format with the old data.
		unset( $post_data['tax_input']['post_format'] );

		$updated[] = mt_update_post( $post_data );
 ...
 * @return MT_Post Post object containing all the default post data as attributes
 */
function get_default_post_to_edit( $post_type = 'post', $create_in_db = false ) {
	$post_title = '';
	if ( ! empty( $_REQUEST['post_title'] ) ) {
		$post_title = esc_html( mt_unslash( $_REQUEST['post_title'] ) );
	}

	$post_content = '';
	if ( ! empty( $_REQUEST['content'] ) ) {
		$post_content = esc_html( mt_unslash( $_REQUEST['content'] ) );
	}

	$post_excerpt = '';
	if ( ! empty( $_REQUEST['excerpt'] ) ) {
		$post_excerpt = esc_html( mt_unslash( $_REQUEST['excerpt'] ) );
	}

	if ( $create_in_db ) {
		$post_id = mt_insert_post(
			array(
		...
		mt_after_insert_post( $post, false, null );

		// Schedule auto-draft cleanup.
		if ( ! mt_next_scheduled( 'mt_scheduled_auto_draft_delete' ) ) {
			mt_schedule_event( time(), 'daily', 'mt_scheduled_auto_draft_delete' );
		}
	} else {
		$post                 = new stdClass;
		$post->ID             = 0;
...
function post_exists( $title, $content = '', $date = '', $type = '' ) {
	...
	$post_title   = mt_unslash( sanitize_post_field( 'post_title', $title, 0, 'db' ) );
	$post_content = mt_unslash( sanitize_post_field( 'post_content', $content, 0, 'db' ) );
	$post_date    = mt_unslash( sanitize_post_field( 'post_date', $date, 0, 'db' ) );
	$post_type    = mt_unslash( sanitize_post_field( 'post_type', $type, 0, 'db' ) );

	$query = "SELECT ID FROM $mtdb->posts WHERE 1=1";
	$args  = array();
	...
	if ( ! empty( $args ) ) {
		return (int) $mtdb->get_var( $mtdb->prepare( $query, $args ) );
	}
 ...
 * @since 2.1.0
 *
 * @global MT_User $current_user
 *
 * @return int|MT_Error Post ID on success, MT_Error on failure.
 */
function mt_write_post() {
	...
	if ( ! current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' === $ptype->name ) {
			return new MT_Error( 'edit_pages', __( 'Sorry, you are not allowed to create pages on this site.' ) );
		} else {
			return new MT_Error( 'edit_posts', __( 'Sorry, you are not allowed to create posts or drafts on this site.' ) );
		}
	}
	...
	$translated = _mt_translate_postdata( false );
	if ( is_mt_error( $translated ) ) {
		return $translated;
	}
	$translated = _mt_get_allowed_postdata( $translated );

	// Create the post.
	$post_ID = mt_insert_post( $translated );
...
/**
 * Calls mt_write_post() and handles the errors.
 *
 * @since 2.0.0
 *
 ...
 */
function write_post() {
	$result = mt_write_post();
	if ( is_mt_error( $result ) ) {
		mt_die( $result->get_error_message() );
	} else {
		return $result;
	}
}
...
function add_meta( $post_ID ) {
	...
	$metakeyselect = isset( $_POST['metakeyselect'] ) ? mt_unslash( trim( $_POST['metakeyselect'] ) ) : '';
	$metakeyinput  = isset( $_POST['metakeyinput'] ) ? mt_unslash( trim( $_POST['metakeyinput'] ) ) : '';
...
function has_meta( $postid ) {
	...
	return $mtdb->get_results(
		$mtdb->prepare(
			"SELECT meta_key, meta_value, meta_id, post_id
			FROM $mtdb->postmeta WHERE post_id = %d
 ...
 * @return void|int|MT_Error Void if nothing fixed. 0 or MT_Error on update failure. The post ID on update success.
 */
function _fix_attachment_links( $post ) {
	...
	foreach ( $link_matches[1] as $key => $value ) {
		if ( ! strpos( $value, '?attachment_id=' ) || ! strpos( $value, 'mt-att-' )
			|| ! preg_match( '/href=(["\'])[^"\']*\?attachment_id=(\d+)[^"\']*\\1/', $value, $url_match )
			|| ! preg_match( '/rel=["\'][^"\']*mt-att-(\d+)/', $value, $rel_match ) ) {
				continue;
		}
...
function get_available_post_statuses( $type = 'post' ) {
	$stati = mt_count_posts( $type );

	return array_keys( get_object_vars( $stati ) );
}

/**
 * Run the mt query to fetch the posts for listing on the edit posts page
 *
 * @since 2.5.0
 *
 ...
 * @return array
 */
function mt_edit_posts_query( $q = false ) {
	if ( false === $q ) {
		$q = $_GET;
	}
 ...
 */
function mt_edit_attachments_query_vars( $q = false ) {
	if ( false === $q ) {
		$q = $_GET;
	}
	$q['m']         = isset( $q['m'] ) ? (int) $q['m'] : 0;
...
/**
 * Executes a query for attachments. An array of MT_Query arguments
 ...
 * @return array
 */
function mt_edit_attachments_query( $q = false ) {
	mt( mt_edit_attachments_query_vars( $q ) );
 ...
 * @since 2.5.0
 *
 * @param int|MT_Post $id    Post ID or post object.
...
function get_sample_permalink( $id, $title = null, $name = null ) {
	...
	$post->post_name = mt_unique_post_slug( $post->post_name, $post->ID, $post->post_status, $post->post_type, $post->post_parent );
	...
	if ( $ptype->hierarchical ) {
		...
		/** This filter is documented in mt-admin/edit-tag-form.php */
...
function get_sample_permalink_html( $id, $new_title = null, $new_slug = null ) {
	...
	if ( current_user_can( 'read_post', $post->ID ) ) {
		if ( 'draft' === $post->post_status || empty( $post->post_name ) ) {
			...
			$preview_target = " target='mt-preview-{$post->ID}'";
		} else {
 ...
 * @param int|MT_Post $post         Optional. The post ID or object associated with the thumbnail, defaults to global $post.
 * @return string The post thumbnail HTML.
 */
function _mt_post_thumbnail_html( $thumbnail_id = null, $post = null ) {
	...
	if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
		$size = isset( $_mt_additional_image_sizes['post-thumbnail'] ) ? 'post-thumbnail' : array( 266, 266 );
		 ...
		 * @param MT_Post      $post         The post object associated with the thumbnail.
		 */
		$size = apply_filters( 'admin_post_thumbnail_size', $size, $thumbnail_id, $post );

		$thumbnail_html = mt_get_attachment_image( $thumbnail_id, $size );
 ...
 * @since 2.5.0
 *
 * @param int|MT_Post $post_id ID or object of the post to check for editing.
 * @return int|false ID of the user with lock. False if the post does not exist, post is not locked,
 *                   the user with lock does not exist, or the post is locked by current user.
 */
function mt_check_post_lock( $post_id ) {
	$post = get_post( $post_id );
	if ( ! $post ) {
		return false;
	}
	...
	/** This filter is documented in mt-admin/includes/ajax-actions.php */
	$time_window = apply_filters( 'mt_check_post_lock_window', 150 );
 ...
 * @since 2.5.0
 *
 * @param int|MT_Post $post_id ID or object of the post being edited.
 * @return array|false Array of the lock time and user ID. False if the post does not exist, or
 *                     there is no current user.
 */
function mt_set_post_lock( $post_id ) {
	$post = get_post( $post_id );
	if ( ! $post ) {
		return false;
	}
...
function _admin_notice_post_locked() {
	...
	if ( $user ) {
		 ...
		 * @param MT_Post $post    Post object.
		 * @param MT_User $user    The user with the lock for the post.
		 */
	...
	if ( $locked ) {
		...
		if ( get_post_type_object( $post->post_type )->public ) {
			if ( 'publish' === $post->post_status || $user->ID != $post->post_author ) {
				...
				$nonce                       = mt_create_nonce( 'post_preview_' . $post->ID );
		...
		$tab_last = $override ? '' : ' mt-tab-last';

		?>
		<div class="post-locked-message">
		<div class="post-locked-avatar"><?php echo get_avatar( $user->ID, 64 ); ?></div>
		<p class="currently-editing mt-tab-first" tabindex="0">
		<?php
		if ( $override ) {
		...
		if ( $override ) {
			?>
	<a class="button button-primary mt-tab-last" href="<?php echo esc_url( add_query_arg( 'get-post-lock', '1', mt_nonce_url( get_edit_post_link( $post->ID, 'url' ), 'lock-post_' . $post->ID ) ) ); ?>"><?php _e( 'Take over' ); ?></a>
			<?php
		}
		...
		<div class="post-taken-over">
			...
			<p class="mt-tab-first" tabindex="0">
			...
			?>
			<p><a class="button button-primary mt-tab-last" href="<?php echo esc_url( $sendback ); ?>"><?php echo $sendback_text; ?></a></p>
		</div>
		<?php
	}
 ...
 * @return int|MT_Error The autosave revision ID. MT_Error or 0 on error.
 */
function mt_create_post_autosave( $post_data ) {
	...
	$old_autosave = mt_get_post_autosave( $post_id, $post_author );
	if ( $old_autosave ) {
		$new_autosave                = _mt_post_revision_data( $post_data, true );
		...
		foreach ( array_intersect( array_keys( $new_autosave ), array_keys( _mt_post_revision_fields( $post ) ) ) as $field ) {
		...
		if ( ! $autosave_is_different ) {
			mt_delete_post_revision( $old_autosave->ID );
			return 0;
		}
		 ...
		 */
		do_action( 'mt_creating_autosave', $new_autosave );

		return mt_update_post( $new_autosave );
	}

	// _mt_put_post_revision() expects unescaped.
	$post_data = mt_unslash( $post_data );

	// Otherwise create the new autosave as a special post revision.
	return _mt_put_post_revision( $post_data, true );
}
...
function post_preview() {
	...
	if ( ! mt_check_post_lock( $post->ID ) && get_current_user_id() == $post->post_author
		...
		$saved_post_id = mt_create_post_autosave( $post->ID );
	}

	if ( is_mt_error( $saved_post_id ) ) {
		mt_die( $saved_post_id->get_error_message() );
	}
	...
	if ( $is_autosave && $saved_post_id ) {
		$query_args['preview_id']    = $post->ID;
		$query_args['preview_nonce'] = mt_create_nonce( 'post_preview_' . $post->ID );
 ...
 * @return mixed The value 0 or MT_Error on failure. The saved post ID on success.
 *               The ID can be the draft post_id or the autosave revision post_id.
 */
function mt_autosave( $post_data ) {
	// Back-compat.
	...
	if ( false === mt_verify_nonce( $post_data['_mtnonce'], 'update-post_' . $post_id ) ) {
		return new MT_Error( 'invalid_nonce', __( 'Error while saving.' ) );
	}

	$post = get_post( $post_id );

	if ( ! current_user_can( 'edit_post', $post->ID ) ) {
		return new MT_Error( 'edit_posts', __( 'Sorry, you are not allowed to edit this item.' ) );
	}
...
function redirect_post( $post_id = '' ) {
	if ( isset( $_POST['save'] ) || isset( $_POST['publish'] ) ) {
		...
		$location = add_query_arg( 'message', 2, mt_get_referer() );
		$location = explode( '#', $location );
		$location = $location[0] . '#postcustom';
	} elseif ( isset( $_POST['deletemeta'] ) && $_POST['deletemeta'] ) {
		$location = add_query_arg( 'message', 3, mt_get_referer() );
		$location = explode( '#', $location );
		$location = $location[0] . '#postcustom';
	} else {
	 ...
	 */
	mt_redirect( apply_filters( 'redirect_post_location', $location, $post_id ) );
	exit;
}
 ...
 * @since 5.0.0
 *
 * @param int|MT_Post $post Post ID or MT_Post object.
...
function use_block_editor_for_post( $post ) {
	 ...
	 * @param MT_Post $post             The post being checked.
	 */
...
function get_block_categories( $post ) {
	 ...
	 * @param MT_Post $post               Post being loaded.
	 */
...
function the_block_editor_meta_boxes() {
	global $post, $current_screen, $mt_meta_boxes;

	// Handle meta box state.
	$_original_meta_boxes = $mt_meta_boxes;
	 ...
	 * @since 5.0.0
	 *
	 * @param array $mt_meta_boxes Global meta box state.
	 */
	$mt_meta_boxes = apply_filters( 'filter_block_editor_meta_boxes', $mt_meta_boxes );
	...
	<form id="toggle-custom-fields-form" method="post" action="<?php echo esc_attr( admin_url( 'post.php' ) ); ?>">
		<?php mt_nonce_field( 'toggle-custom-fields', 'toggle-custom-fields-nonce' ); ?>
	...
	foreach ( $locations as $location ) {
		...
		if ( ! isset( $mt_meta_boxes[ $current_screen->id ][ $location ] ) ) {
			continue;
		}

		foreach ( $priorities as $priority ) {
			if ( ! isset( $mt_meta_boxes[ $current_screen->id ][ $location ][ $priority ] ) ) {
				continue;
			}

			$meta_boxes = (array) $mt_meta_boxes[ $current_screen->id ][ $location ][ $priority ];
	 ...
	 * editor instance.
	 */
	$script = 'window._mtLoadBlockEditor.then( function() {
		mt.data.dispatch( \'core/edit-post\' ).setAvailableMetaBoxesPerLocation( ' . mt_json_encode( $meta_boxes_per_location ) . ' );
	} );';

	mt_add_inline_script( 'mt-edit-post', $script );

	/**
	 * When `mt-edit-post` is output in the `<head>`, the inline script needs to be manually printed. Otherwise,
	 * meta boxes will not display because inline scripts for `mt-edit-post` will not be printed again after this point.
	 */
	if ( mt_script_is( 'mt-edit-post', 'done' ) ) {
	...
	if ( $enable_custom_fields ) {
		...
		mt_enqueue_script( 'mt-lists' );
		mt_add_inline_script( 'mt-lists', $script );
	}

	// Reset meta box data.
	$mt_meta_boxes = $_original_meta_boxes;
}
...
function the_block_editor_meta_box_post_form_hidden_fields( $post ) {
	...
	$classic_elements = mt_html_split( $classic_output );
	$hidden_inputs    = '';
	...
	if ( 'draft' !== get_post_status( $post ) ) {
		mt_original_referer_field( true, 'previous' );
	}
	echo $form_extra;
	mt_nonce_field( 'meta-box-order', 'meta-box-order-nonce', false );
	mt_nonce_field( 'closedpostboxes', 'closedpostboxesnonce', false );
	// Permalink title nonce.
	mt_nonce_field( 'samplepermalink', 'samplepermalinknonce', false );
	 ...
	 * @since 5.0.0
	 *
	 * @param MT_Post $post The post that is being edited.
	 */
