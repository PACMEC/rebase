...
/**
 * Loads the MT image-editing interface.
 *
 * @since 2.9.0
 *
 ...
 *                              Default false.
 */
function mt_image_editor( $post_id, $msg = false ) {
	$nonce     = mt_create_nonce( "image_editor-$post_id" );
	...
	$backup_sizes = get_post_meta( $post_id, '_mt_attachment_backup_sizes', true );
	$can_restore  = false;
	if ( ! empty( $backup_sizes ) && isset( $backup_sizes['full-orig'], $meta['file'] ) ) {
		$can_restore = mt_basename( $meta['file'] ) !== $backup_sizes['full-orig']['file'];
	}
	...
	?>
	<div class="imgedit-wrap mt-clearfix">
	<div id="imgedit-panel-<?php echo $post_id; ?>">

	<div class="imgedit-panel-content mt-clearfix">
		<?php echo $note; ?>
		<div class="imgedit-menu mt-clearfix">
	...
	<?php
	if ( $thumb && $sub_sizes ) {
		$thumb_img = mt_constrain_dimensions( $thumb['width'], $thumb['height'], 160, 120 );
		?>
...
/**
 * Streams image in MT_Image_Editor to browser.
 *
 * @since 2.9.0
 *
 * @param MT_Image_Editor $image         The image editor instance.
 * @param string          $mime_type     The mime type of the image.
 * @param int             $attachment_id The image's attachment post ID.
 * @return bool True on success, false on failure.
 */
function mt_stream_image( $image, $mime_type, $attachment_id ) {
	if ( $image instanceof MT_Image_Editor ) {

		/**
		 * Filters the MT_Image_Editor instance for the image to be streamed to the browser.
		 *
		 * @since 3.5.0
		 *
		 * @param MT_Image_Editor $image         The image editor instance.
		...
		if ( is_mt_error( $image->stream( $mime_type ) ) ) {
			return false;
		}

		return true;
	} else {
		/* translators: 1: $image, 2: MT_Image_Editor */
		_deprecated_argument( __FUNCTION__, '3.5.0', sprintf( __( '%1$s needs to be a %2$s object.' ), '$image', 'MT_Image_Editor' ) );
 ...
 * @return bool True on success, false on failure.
 */
function mt_save_image_file( $filename, $image, $mime_type, $post_id ) {
	if ( $image instanceof MT_Image_Editor ) {

		/** This filter is documented in mt-admin/includes/image-edit.php */
		 ...
		 * @param int             $post_id   Attachment post ID.
		 */
		$saved = apply_filters( 'mt_save_image_editor_file', null, $filename, $image, $mime_type, $post_id );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use {@see 'mt_save_image_editor_file'} instead.
		 *
		...
		switch ( $mime_type ) {
			case 'image/jpeg':
				/** This filter is documented in mt-includes/class-mt-image-editor.php */
 ...
 *
 * @since 2.9.0
 * @deprecated 3.5.0 Use MT_Image_Editor::rotate()
 * @see MT_Image_Editor::rotate()
 *
 * @ignore
...
function _rotate_image_resource( $img, $angle ) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'MT_Image_Editor::rotate()' );
 ...
 *
 * @since 2.9.0
 * @deprecated 3.5.0 Use MT_Image_Editor::flip()
 * @see MT_Image_Editor::flip()
 *
 * @ignore
...
function _flip_image_resource( $img, $horz, $vert ) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'MT_Image_Editor::flip()' );

	$w   = imagesx( $img );
	$h   = imagesy( $img );
	$dst = mt_imagecreatetruecolor( $w, $h );
 ...
 * @since 2.9.0
 *
 * @param MT_Image_Editor $image   MT_Image_Editor instance.
 * @param array           $changes Array of change operations.
 * @return MT_Image_Editor MT_Image_Editor instance with changes applied.
 */
function image_edit_apply_changes( $image, $changes ) {
	...
	if ( $image instanceof MT_Image_Editor ) {
		...
		/**
		 * Filters the MT_Image_Editor instance before applying changes to the image.
		 *
		 * @since 3.5.0
		 *
		 * @param MT_Image_Editor $image   MT_Image_Editor instance.
		 * @param array           $changes Array of change operations.
		 */
		$image = apply_filters( 'mt_image_editor_before_change', $image, $changes );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use {@see 'mt_image_editor_before_change'} instead.
		 *
		 * @param resource|GdImage $image   GD image resource or GdImage instance.
		 * @param array            $changes Array of change operations.
		 */
		$image = apply_filters_deprecated( 'image_edit_before_change', array( $image, $changes ), '3.5.0', 'mt_image_editor_before_change' );
	}
...
function stream_preview_image( $post_id ) {
	...
	$img = mt_get_image_editor( _load_image_to_edit_path( $post_id ) );

	if ( is_mt_error( $img ) ) {
		return false;
	}

	$changes = ! empty( $_REQUEST['history'] ) ? json_decode( mt_unslash( $_REQUEST['history'] ) ) : null;
	if ( $changes ) {
	...
	if ( is_mt_error( $img->resize( $w2, $h2 ) ) ) {
		return false;
	}

	return mt_stream_image( $img, $post->post_mime_type, $post_id );
}
 ...
 */
function mt_restore_image( $post_id ) {
	$meta             = mt_get_attachment_metadata( $post_id );
	...
	if ( isset( $backup_sizes['full-orig'] ) && is_array( $backup_sizes['full-orig'] ) ) {
		...
		$meta['file']   = _mt_relative_upload_path( $restored_file );
	...
	if ( ! mt_update_attachment_metadata( $post_id, $meta ) ||
		( $old_backup_sizes !== $backup_sizes && ! update_post_meta( $post_id, '_mt_attachment_backup_sizes', $backup_sizes ) ) ) {
 ...
 * @return stdClass
 */
function mt_save_image( $post_id ) {
	...
	$img = mt_get_image_editor( _load_image_to_edit_path( $post_id, 'full' ) );
	if ( is_mt_error( $img ) ) {
	...
	if ( $scale && $fwidth > 0 && $fheight > 0 ) {
		...
		$changes = json_decode( mt_unslash( $_REQUEST['history'] ) );
		if ( $changes ) {
			$img = image_edit_apply_changes( $img, $changes );
		}
	} else {
	...
	if ( ! mt_save_image_file( $new_path, $img, $post->post_mime_type, $post_id ) ) {
	...
	if ( 'nothumb' === $target || 'all' === $target || 'full' === $target || $scaled ) {
		...
		$meta['file'] = _mt_relative_upload_path( $new_path );
	...
	if ( isset( $sizes ) ) {
		...
		foreach ( $sizes as $size ) {
			...
			if ( isset( $_mt_additional_image_sizes[ $size ] ) ) {
				$width  = (int) $_mt_additional_image_sizes[ $size ]['width'];
				$height = (int) $_mt_additional_image_sizes[ $size ]['height'];
				$crop   = ( $nocrop ) ? false : $_mt_additional_image_sizes[ $size ]['crop'];
			} else {
	...
	if ( $success ) {
		mt_update_attachment_metadata( $post_id, $meta );
		update_post_meta( $post_id, '_mt_attachment_backup_sizes', $backup_sizes );

		if ( 'thumbnail' === $target || 'all' === $target || 'full' === $target ) {
			// Check if it's an image edit from attachment edit screen.
			if ( ! empty( $_REQUEST['context'] ) && 'edit-attachment' === $_REQUEST['context'] ) {
				$thumb_url         = mt_get_attachment_image_src( $post_id, array( 900, 600 ), true );
