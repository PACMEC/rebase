...
class Custom_Background {
	...
	public function __construct( $admin_header_callback = '', $admin_image_div_callback = '' ) {
		...
		add_action( 'mt_ajax_custom-background-add', array( $this, 'ajax_background_add' ) );

		// Unused since 3.5.0.
		add_action( 'mt_ajax_set-background-image', array( $this, 'mt_set_background_image' ) );
	}
	...
	public function admin_load() {
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="https://codex.managertechnology.com.co/pacmec/Appearance_Background_Screen">Documentation on Custom Background</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
		);

		mt_enqueue_media();
		mt_enqueue_script( 'custom-background' );
	...
	public function take_action() {
		...
		if ( isset( $_POST['reset-background'] ) ) {
			check_admin_referer( 'custom-background-reset', '_mtnonce-custom-background-reset' );
		...
		if ( isset( $_POST['remove-background'] ) ) {
			...
			check_admin_referer( 'custom-background-remove', '_mtnonce-custom-background-remove' );
			set_theme_mod( 'background_image', '' );
			set_theme_mod( 'background_image_thumb', '' );
			$this->updated = true;
			mt_safe_redirect( $_POST['_mt_http_referer'] );
			return;
		}
...
<td>
<form method="post">
			<?php mt_nonce_field( 'custom-background-remove', '_mtnonce-custom-background-remove' ); ?>
...
<td>
<form method="post">
			<?php mt_nonce_field( 'custom-background-reset', '_mtnonce-custom-background-reset' ); ?>
...
<td><form enctype="multipart/form-data" id="upload-form" class="mt-upload-form" method="post">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
			<?php mt_nonce_field( 'custom-background-upload', '_mtnonce-custom-background-upload' ); ?>
...
<input type="text" name="background-color" id="background-color" value="#<?php echo esc_attr( get_background_color() ); ?>"<?php echo $default_color; ?>>
		...
		<?php mt_nonce_field( 'custom-background' ); ?>
	...
	public function handle_upload() {
		...
		check_admin_referer( 'custom-background-upload', '_mtnonce-custom-background-upload' );
		...
		// Save the data.
		$id = mt_insert_attachment( $object, $file );

		// Add the metadata.
		mt_update_attachment_metadata( $id, mt_generate_attachment_metadata( $id, $file ) );
		update_post_meta( $id, '_mt_attachment_is_custom_background', get_option( 'stylesheet' ) );

		set_theme_mod( 'background_image', esc_url_raw( $url ) );

		$thumbnail = mt_get_attachment_image_src( $id, 'thumbnail' );
		set_theme_mod( 'background_image_thumb', esc_url_raw( $thumbnail[0] ) );

		/** This action is documented in mt-admin/includes/class-custom-image-header.php */
		do_action( 'mt_create_file_in_uploads', $file, $id ); // For replication.
		$this->updated = true;
	}
	...
	public function ajax_background_add() {
		...
		update_post_meta( $attachment_id, '_mt_attachment_is_custom_background', get_stylesheet() );
	 ...
	 * @deprecated 3.5.0
	 */
	public function mt_set_background_image() {
		...
		update_post_meta( $attachment_id, '_mt_attachment_is_custom_background', get_option( 'stylesheet' ) );

		$url       = mt_get_attachment_image_src( $attachment_id, $size );
		$thumbnail = mt_get_attachment_image_src( $attachment_id, 'thumbnail' );
