<?php
/**
 * paCMec Network Administration API.
 *
 * @package paCMec
...
function network_domain_check() {
	...
	$sql = $mtdb->prepare( 'SHOW TABLES LIKE %s', $mtdb->esc_like( $mtdb->site ) );
	if ( $mtdb->get_var( $sql ) ) {
		return $mtdb->get_var( "SELECT domain FROM $mtdb->site ORDER BY id ASC LIMIT 1" );
	}
	return false;
}
...
function allow_subdirectory_install() {
	...
	$post = $mtdb->get_row( "SELECT ID FROM $mtdb->posts WHERE post_date < DATE_SUB(NOW(), INTERVAL 1 MONTH) AND post_status = 'publish'" );
	if ( empty( $post ) ) {
		return true;
	}
 ...
 *
 * @param false|MT_Error $errors Optional. Error object. Default false.
 */
function network_step1( $errors = false ) {
	...
	mt_nonce_field( 'install-network-1' );
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of paCMec sites. We will create configuration files in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) :
		...
		<p><?php _e( 'Please choose whether you would like sites in your paCMec network to use sub-domains or sub-directories.' ); ?>
	...
	if ( MT_CONTENT_DIR !== ABSPATH . 'mt-content' && ( allow_subdirectory_install() || ! allow_subdomain_install() ) ) {
		echo '<div class="error inline"><p><strong>' . __( 'Warning:' ) . '</strong> ' . __( 'Subdirectory networks may not be fully compatible with custom mt-content directories.' ) . '</p></div>';
	}
	...
	if ( $is_www ) :
			...
			<tr>
				...
				<?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your paCMec network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( ! allow_subdomain_install() ) : ?>
			<tr>
				...
				<td>
				<?php
					_e( 'Because your installation is in a directory, the sites in your paCMec network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td>
				<?php
				_e( 'Because your installation is not new, the sites in your paCMec network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	$mt_siteurl_subdir = preg_replace( '#^' . preg_quote( $home_path, '#' ) . '#', '', $abspath_fix );
	$rewrite_base      = ! empty( $mt_siteurl_subdir ) ? ltrim( trailingslashit( $mt_siteurl_subdir ), '/' ) : '';

	$location_of_mt_config = $abspath_fix;
	if ( ! file_exists( ABSPATH . 'mt-config.php' ) && file_exists( dirname( ABSPATH ) . '/mt-config.php' ) ) {
		$location_of_mt_config = dirname( $abspath_fix );
	}
	$location_of_mt_config = trailingslashit( $location_of_mt_config );
	...
	<p><?php _e( 'The original configuration steps are shown here for reference.' ); ?></p>
			...
			$subdomain_install = (bool) $mtdb->get_var( "SELECT meta_value FROM $mtdb->sitemeta WHERE site_id = 1 AND meta_key = 'subdomain_install'" );
			?>
	<div class="error"><p><strong><?php _e( 'Warning:' ); ?></strong> <?php _e( 'An existing paCMec network was detected.' ); ?></p></div>
	...
	if ( $_POST || ! is_multisite() ) {
		...
		if ( file_exists( $home_path . '.htaccess' ) ) {
			...
			printf(
				/* translators: 1: mt-config.php, 2: .htaccess */
			...
			echo '<strong>' . __( 'Caution:' ) . '</strong> ';
			printf(
				/* translators: 1: mt-config.php, 2: web.config */
				__( 'We recommend you back up your existing %1$s and %2$s files.' ),
				'<code>mt-config.php</code>',
	...
	<ol>
		...
		<?php
		printf(
			/* translators: 1: mt-config.php, 2: Location of mt-config file, 3: Translated version of "That's all, stop editing! Happy publishing." */
			__( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:' ),
			'<code>mt-config.php</code>',
			'<code>' . $location_of_mt_config . '</code>',
			/*
...
define('BLOG_ID_CURRENT_SITE', 1);
		...
		if ( ! empty( $keys_salts ) ) {
			...
			if ( is_mt_error( $from_api ) ) {
				...
				$from_api = explode( "\n", mt_remote_retrieve_body( $from_api ) );
				foreach ( $keys_salts as $c => $v ) {
...
<configuration>
    <system.webServer>
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$web_config_file .= '
                <rule name="paCMec Rule for Files" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'files/(.+)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . MTINC . '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
		}
			$web_config_file .= '
                <rule name="paCMec Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'mt-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'mt-admin/" redirectType="Permanent" />
                </rule>
                <rule name="paCMec Rule 3" stopProcessing="true">
                ...
                <rule name="paCMec Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(mt-(content|admin|includes).*)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:1}" />
                </rule>
                <rule name="paCMec Rule 5" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:2}" />
                </rule>
                <rule name="paCMec Rule 6" stopProcessing="true">
                    <match url="." ignoreCase="false" />
...
';
			...
			printf(
				...
				__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other paCMec rules:' ),
				'<code>web.config</code>',
				'<code>' . $home_path . '</code>'
			);
		echo '</p>';
		if ( ! $subdomain_install && MT_CONTENT_DIR !== ABSPATH . 'mt-content' ) {
			echo '<p><strong>' . __( 'Warning:' ) . ' ' . __( 'Subdirectory networks may not be fully compatible with custom mt-content directories.' ) . '</strong></p>';
		}
		?>
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$ms_files_rewriting  = "\n# uploaded files\nRewriteRule ^";
			$ms_files_rewriting .= $subdir_match . "files/(.+) {$rewrite_base}" . MTINC . "/ms-files.php?file={$subdir_replacement_12} [L]" . "\n";
		}
...
{$ms_files_rewriting}
# add a trailing slash to /mt-admin
RewriteRule ^{$subdir_match}mt-admin$ {$subdir_replacement_01}mt-admin/ [R=301,L]

RewriteCond %{REQUEST_FILENAME} -f [OR]
RewriteCond %{REQUEST_FILENAME} -d
RewriteRule ^ - [L]
RewriteRule ^{$subdir_match}(mt-(content|admin|includes).*) {$rewrite_base}{$subdir_replacement_12} [L]
...
EOF;
	...
	if ( ! is_multisite() ) {
		?>
		<p><?php _e( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.' ); ?> <a href="<?php echo esc_url( mt_login_url() ); ?>"><?php _e( 'Log In' ); ?></a></p>
		<?php
	}
}
