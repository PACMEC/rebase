<?php
/**
 * Class for providing debug data based on a users paCMec environment.
 *
 * @package paCMec
...
class MT_Debug_Data {
	...
	static function debug_data() {
		...
		$environment_type       = mt_get_environment_type();
		...
		if ( is_array( $core_updates ) ) {
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					/* translators: %s: Latest paCMec version number. */
		...
		if ( ! $is_multisite ) {
			$info['mt-paths-sizes'] = array(
				'label'  => __( 'Directories and Sizes' ),
				'fields' => array(),
			);
		}

		$info['mt-dropins'] = array(
			'label'       => __( 'Drop-ins' ),
			'show_count'  => true,
			'description' => sprintf(
				/* translators: %s: mt-content directory name. */
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance paCMec features in ways that are not possible for traditional plugins.' ),
				'<code>' . str_replace( ABSPATH, '', MT_CONTENT_DIR ) . '</code>'
			),
			'fields'      => array(),
		);

		$info['mt-active-theme'] = array(
			'label'  => __( 'Active Theme' ),
			'fields' => array(),
		);

		$info['mt-parent-theme'] = array(
			'label'  => __( 'Parent Theme' ),
			'fields' => array(),
		);

		$info['mt-themes-inactive'] = array(
			'label'      => __( 'Inactive Themes' ),
			'show_count' => true,
			'fields'     => array(),
		);

		$info['mt-mu-plugins'] = array(
			'label'      => __( 'Must Use Plugins' ),
			'show_count' => true,
			'fields'     => array(),
		);

		$info['mt-plugins-active'] = array(
			'label'      => __( 'Active Plugins' ),
			'show_count' => true,
			'fields'     => array(),
		);

		$info['mt-plugins-inactive'] = array(
		...
		$info['mt-database'] = array(
			'label'  => __( 'Database' ),
			'fields' => array(),
		);

		// Check if MT_DEBUG_LOG is set.
		$mt_debug_log_value = __( 'Disabled' );

		if ( is_string( MT_DEBUG_LOG ) ) {
			$mt_debug_log_value = MT_DEBUG_LOG;
		} elseif ( MT_DEBUG_LOG ) {
			$mt_debug_log_value = __( 'Enabled' );
		}
		...
		// Check MT_LOCAL_DEV.
		if ( defined( 'MT_LOCAL_DEV' ) ) {
			$mt_local_dev       = MT_LOCAL_DEV ? __( 'Enabled' ) : __( 'Disabled' );
			$mt_local_dev_debug = MT_LOCAL_DEV ? 'true' : 'false';
		} else {
			$mt_local_dev       = __( 'Undefined' );
			$mt_local_dev_debug = 'undefined';
		}

		$info['mt-constants'] = array(
			'label'       => __( 'paCMec Constants' ),
			'description' => __( 'These settings alter where and how parts of paCMec are loaded.' ),
			'fields'      => array(
				'ABSPATH'             => array(
					'label'   => 'ABSPATH',
					'value'   => ABSPATH,
				...
				'MT_HOME'             => array(
					'label' => 'MT_HOME',
					'value' => ( defined( 'MT_HOME' ) ? MT_HOME : __( 'Undefined' ) ),
					'debug' => ( defined( 'MT_HOME' ) ? MT_HOME : 'undefined' ),
				),
				'MT_SITEURL'          => array(
					'label' => 'MT_SITEURL',
					'value' => ( defined( 'MT_SITEURL' ) ? MT_SITEURL : __( 'Undefined' ) ),
					'debug' => ( defined( 'MT_SITEURL' ) ? MT_SITEURL : 'undefined' ),
				),
				'MT_CONTENT_DIR'      => array(
					'label' => 'MT_CONTENT_DIR',
					'value' => MT_CONTENT_DIR,
				),
				'MT_PLUGIN_DIR'       => array(
					'label' => 'MT_PLUGIN_DIR',
					'value' => MT_PLUGIN_DIR,
				),
				'MT_MEMORY_LIMIT'     => array(
					'label' => 'MT_MEMORY_LIMIT',
					'value' => MT_MEMORY_LIMIT,
				),
				'MT_MAX_MEMORY_LIMIT' => array(
					'label' => 'MT_MAX_MEMORY_LIMIT',
					'value' => MT_MAX_MEMORY_LIMIT,
				),
				'MT_DEBUG'            => array(
					'label' => 'MT_DEBUG',
					'value' => MT_DEBUG ? __( 'Enabled' ) : __( 'Disabled' ),
					'debug' => MT_DEBUG,
				),
				'MT_DEBUG_DISPLAY'    => array(
					'label' => 'MT_DEBUG_DISPLAY',
					'value' => MT_DEBUG_DISPLAY ? __( 'Enabled' ) : __( 'Disabled' ),
					'debug' => MT_DEBUG_DISPLAY,
				),
				'MT_DEBUG_LOG'        => array(
					'label' => 'MT_DEBUG_LOG',
					'value' => $mt_debug_log_value,
					'debug' => MT_DEBUG_LOG,
				),
				'SCRIPT_DEBUG'        => array(
					'label' => 'SCRIPT_DEBUG',
				...
				'MT_CACHE'            => array(
					'label' => 'MT_CACHE',
					'value' => MT_CACHE ? __( 'Enabled' ) : __( 'Disabled' ),
					'debug' => MT_CACHE,
				),
		...
		$is_writable_abspath            = mt_is_writable( ABSPATH );
		$is_writable_mt_content_dir     = mt_is_writable( MT_CONTENT_DIR );
		$is_writable_upload_dir         = mt_is_writable( $upload_dir['basedir'] );
		$is_writable_mt_plugin_dir      = mt_is_writable( MT_PLUGIN_DIR );
		$is_writable_template_directory = mt_is_writable( get_theme_root( get_template() ) );

		$info['mt-filesystem'] = array(
			...
			'fields'      => array(
				...
				'mt-content' => array(
					'label' => __( 'The mt-content directory' ),
					'value' => ( $is_writable_mt_content_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
					'debug' => ( $is_writable_mt_content_dir ? 'writable' : 'not writable' ),
				),
				'uploads'    => array(
				...
				'plugins'    => array(
					...
					'value' => ( $is_writable_mt_plugin_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
					'debug' => ( $is_writable_mt_plugin_dir ? 'writable' : 'not writable' ),
				),
				'themes'     => array(
		...
		if ( is_multisite() ) {
			$network_query = new MT_Network_Query();
			...
			$info['mt-core']['fields']['user_count'] = array(
				'label' => __( 'User count' ),
				'value' => get_user_count(),
			);

			$info['mt-core']['fields']['site_count'] = array(
				'label' => __( 'Site count' ),
				'value' => $site_count,
			);

			$info['mt-core']['fields']['network_count'] = array(
		...
		if ( ! is_mt_error( $mt_dotorg ) ) {
			$info['mt-core']['fields']['dotorg_communication'] = array(
				...
				'value' => __( 'paCMec.org is reachable' ),
				'debug' => 'true',
			);
		} else {
			$info['mt-core']['fields']['dotorg_communication'] = array(
		...
		foreach ( $dropins as $dropin_key => $dropin ) {
			$info['mt-dropins']['fields'][ sanitize_text_field( $dropin_key ) ] = array(
				'label' => $dropin_key,
		...
		$info['mt-media']['fields']['image_editor'] = array(
			'label' => __( 'Active editor' ),
			'value' => _mt_image_editor_choose(),
		);
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			$effective           = min( mt_convert_hr_to_bytes( $post_max_size ), mt_convert_hr_to_bytes( $upload_max_filesize ) );

			// Add info in Media section.
			$info['mt-media']['fields']['file_uploads']        = array(
				'label' => __( 'File uploads' ),
				'value' => empty( ini_get( 'file_uploads' ) ) ? __( 'Disabled' ) : __( 'Enabled' ),
				'debug' => 'File uploads is turned off',
			);
			$info['mt-media']['fields']['post_max_size']       = array(
				'label' => __( 'Max size of post data allowed' ),
				'value' => $post_max_size,
			);
			$info['mt-media']['fields']['upload_max_filesize'] = array(
				'label' => __( 'Max size of an uploaded file' ),
				'value' => $upload_max_filesize,
			);
			$info['mt-media']['fields']['max_effective_size']  = array(
				'label' => __( 'Max effective file size' ),
				'value' => size_format( $effective ),
			);
			$info['mt-media']['fields']['max_file_uploads']    = array(
		...
		if ( 'MT_Image_Editor_Imagick' === _mt_image_editor_choose() && isset( $imagick ) && $imagick instanceof Imagick ) {
			$limits = array(
			...
			$info['mt-media']['fields']['imagick_limits'] = array(
		...
		$info['mt-media']['fields']['gd_version'] = array(
			'label' => __( 'GD version' ),
		...
		$info['mt-media']['fields']['ghostscript_version'] = array(
		...
		$info['mt-server']['fields']['server_architecture'] = array(
		...
		$info['mt-server']['fields']['httpd_software']      = array(
			'label' => __( 'Web server' ),
		...
		$info['mt-server']['fields']['php_version']         = array(
			'label' => __( 'PHP version' ),
			'value' => $php_version,
			'debug' => $php_version_debug,
		);
		$info['mt-server']['fields']['php_sapi']            = array(
			'label' => __( 'PHP SAPI' ),
		...
		if ( ! function_exists( 'ini_get' ) ) {
			$info['mt-server']['fields']['ini_get'] = array(
			...
			$info['mt-server']['fields']['max_input_variables'] = array(
				'label' => __( 'PHP max input variables' ),
				'value' => ini_get( 'max_input_vars' ),
			);
			$info['mt-server']['fields']['time_limit']          = array(
				'label' => __( 'PHP time limit' ),
				'value' => ini_get( 'max_execution_time' ),
			);

			if ( MT_Site_Health::get_instance()->php_memory_limit !== ini_get( 'memory_limit' ) ) {
				$info['mt-server']['fields']['memory_limit']       = array(
					'label' => __( 'PHP memory limit' ),
					'value' => MT_Site_Health::get_instance()->php_memory_limit,
				);
				$info['mt-server']['fields']['admin_memory_limit'] = array(
			...
			$info['mt-server']['fields']['max_input_time']      = array(
				'label' => __( 'Max input time' ),
				'value' => ini_get( 'max_input_time' ),
			);
			$info['mt-server']['fields']['upload_max_filesize'] = array(
				'label' => __( 'Upload max filesize' ),
				'value' => ini_get( 'upload_max_filesize' ),
			);
			$info['mt-server']['fields']['php_post_max_size']   = array(
		...
		if ( function_exists( 'curl_version' ) ) {
			...
			$info['mt-server']['fields']['curl_version'] = array(
		...
		$info['mt-server']['fields']['suhosin'] = array(
		...
		$info['mt-server']['fields']['imagick_availability'] = array(
		...
		$info['mt-server']['fields']['pretty_permalinks'] = array(
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core paCMec rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN paCMec[\s\S]+?# END paCMec/si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core paCMec features.' ), '.htaccess' );
			}

			$info['mt-server']['fields']['htaccess_extra_rules'] = array(
		...
		if ( is_resource( $mtdb->dbh ) ) {
			// Old mysql extension.
			$extension = 'mysql';
		} elseif ( is_object( $mtdb->dbh ) ) {
			// mysqli or PDO.
			$extension = get_class( $mtdb->dbh );
		} else {
			// Unknown sql extension.
			$extension = null;
		}

		$server = $mtdb->get_var( 'SELECT VERSION()' );

		if ( isset( $mtdb->use_mysqli ) && $mtdb->use_mysqli ) {
			$client_version = $mtdb->dbh->client_info;
		} else {
		...
		$info['mt-database']['fields']['extension'] = array(
			'label' => __( 'Extension' ),
			'value' => $extension,
		);

		$info['mt-database']['fields']['server_version'] = array(
			'label' => __( 'Server version' ),
			'value' => $server,
		);

		$info['mt-database']['fields']['client_version'] = array(
			'label' => __( 'Client version' ),
			'value' => $client_version,
		);

		$info['mt-database']['fields']['database_user'] = array(
			'label'   => __( 'Database username' ),
			'value'   => $mtdb->dbuser,
			'private' => true,
		);

		$info['mt-database']['fields']['database_host'] = array(
			'label'   => __( 'Database host' ),
			'value'   => $mtdb->dbhost,
			'private' => true,
		);

		$info['mt-database']['fields']['database_name'] = array(
			'label'   => __( 'Database name' ),
			'value'   => $mtdb->dbname,
			'private' => true,
		);

		$info['mt-database']['fields']['database_prefix'] = array(
			'label'   => __( 'Table prefix' ),
			'value'   => $mtdb->prefix,
			'private' => true,
		);

		$info['mt-database']['fields']['database_charset'] = array(
			'label'   => __( 'Database charset' ),
			'value'   => $mtdb->charset,
			'private' => true,
		);

		$info['mt-database']['fields']['database_collate'] = array(
		...
		foreach ( $mu_plugins as $plugin_path => $plugin ) {
			...
			$info['mt-mu-plugins']['fields'][ sanitize_text_field( $plugin['Name'] ) ] = array(
				'label' => $plugin['Name'],
		...
		$auto_updates_enabled = mt_is_auto_update_enabled_for_type( 'plugin' );
		...
		foreach ( $plugins as $plugin_path => $plugin ) {
			$plugin_part = ( is_plugin_active( $plugin_path ) ) ? 'mt-plugins-active' : 'mt-plugins-inactive';
			...
			if ( $auto_updates_enabled ) {
				if ( isset( $transient->response[ $plugin_path ] ) ) {
					...
					$item = mt_parse_args( $plugin, $item );
				}

				$auto_update_forced = mt_is_auto_update_forced_for_item( 'plugin', null, (object) $item );
		...
		if ( ! empty( $_mt_theme_features ) ) {
			foreach ( $_mt_theme_features as $feature => $options ) {
		...
		$auto_updates         = array();
		$auto_updates_enabled = mt_is_auto_update_enabled_for_type( 'theme' );
		if ( $auto_updates_enabled ) {
		...
		$info['mt-active-theme']['fields'] = array(
			'name'           => array(
				'label' => __( 'Name' ),
				'value' => sprintf(
					/* translators: 1: Theme name. 2: Theme slug. */
			...
			'author'         => array(
				'label' => __( 'Author' ),
				'value' => mt_kses( $active_theme->author, array() ),
			),
		...
		if ( $auto_updates_enabled ) {
			...
			$auto_update_forced = mt_is_auto_update_forced_for_item( 'theme', null, (object) $item );
			...
			/** This filter is documented in mt-admin/includes/class-mt-debug-data.php */
			$auto_updates_string = apply_filters( 'theme_auto_update_debug_string', $auto_updates_string, $active_theme, $enabled );

			$info['mt-active-theme']['fields']['auto_update'] = array(
		...
		if ( $parent_theme ) {
			...
			$info['mt-parent-theme']['fields'] = array(
				'name'           => array(
					'label' => __( 'Name' ),
					'value' => sprintf(
						/* translators: 1: Theme name. 2: Theme slug. */
				...
				'author'         => array(
					'label' => __( 'Author' ),
					'value' => mt_kses( $parent_theme->author, array() ),
				),
				'author_website' => array(
					'label' => __( 'Author website' ),
			...
			if ( $auto_updates_enabled ) {
				...
				$info['mt-parent-theme']['fields']['auto_update'] = array(
		...
		foreach ( $all_themes as $theme_slug => $theme ) {
			...
			// Sanitize.
			$theme_author = mt_kses( $theme_author, array() );
			...
			if ( $auto_updates_enabled ) {
				 ...
				 *
				 * @param string   $auto_updates_string The string output for the auto-updates column.
				 * @param MT_Theme $theme               An object of theme data.
				 * @param bool     $enabled             Whether auto-updates are enabled for this item.
				 */
			...
			$info['mt-themes-inactive']['fields'][ sanitize_text_field( $theme->name ) ] = array(
				'label' => sprintf(
					/* translators: 1: Theme name. 2: Theme slug. */
					__( '%1$s (%2$s)' ),
					$theme->name,
		...
		if ( defined( 'MTMU_PLUGIN_DIR' ) && is_dir( MTMU_PLUGIN_DIR ) ) {
			$is_writable_mtmu_plugin_dir = mt_is_writable( MTMU_PLUGIN_DIR );

			$info['mt-filesystem']['fields']['mu-plugins'] = array(
				'label' => __( 'The must use plugins directory' ),
				'value' => ( $is_writable_mtmu_plugin_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_mtmu_plugin_dir ? 'writable' : 'not writable' ),
			);
		}
		 ...
		 *
		 * Array keys for sections added by core are all prefixed with `mt-`, plugins and themes should use their own slug as
	 ...
	 * @since 5.2.0
	 *
	 * @param array  $info_array Information gathered from the `MT_Debug_Data::debug_data` function.
	...
	public static function get_database_size() {
		global $mtdb;
		$size = 0;
		$rows = $mtdb->get_results( 'SHOW TABLE STATUS', ARRAY_A );
