 ...
 * @since 2.0.0
 *
 * @return int|MT_Error Value 0 or MT_Error on failure. The link ID on success.
 */
function add_link() {
	return edit_link();
}
...
function edit_link( $link_id = 0 ) {
	...
	if ( ! empty( $link_id ) ) {
		...
		return mt_update_link( $_POST );
	} else {
		return mt_insert_link( $_POST );
	}
}
...
function get_default_link_to_edit() {
	...
	if ( isset( $_GET['linkurl'] ) ) {
		$link->link_url = esc_url( mt_unslash( $_GET['linkurl'] ) );
	} else {
		$link->link_url = '';
	}

	if ( isset( $_GET['name'] ) ) {
		$link->link_name = esc_attr( mt_unslash( $_GET['name'] ) );
	} else {
		$link->link_name = '';
	}
 ...
 */
function mt_delete_link( $link_id ) {
	global $mtdb;
	/**
	...
	mt_delete_object_term_relationships( $link_id, 'link_category' );

	$mtdb->delete( $mtdb->links, array( 'link_id' => $link_id ) );
 ...
 */
function mt_get_link_cats( $link_id = 0 ) {
	$cats = mt_get_object_terms( $link_id, 'link_category', array( 'fields' => 'ids' ) );
 ...
 * }
 * @param bool  $mt_error Optional. Whether to return a MT_Error object on failure. Default false.
 * @return int|MT_Error Value 0 or MT_Error on failure. The link ID on success.
 */
function mt_insert_link( $linkdata, $mt_error = false ) {
	global $mtdb;
	...
	$parsed_args = mt_parse_args( $linkdata, $defaults );
	$parsed_args = mt_unslash( sanitize_bookmark( $parsed_args, 'db' ) );
	...
	if ( $update ) {
		if ( false === $mtdb->update( $mtdb->links, compact( 'link_url', 'link_name', 'link_image', 'link_target', 'link_description', 'link_visible', 'link_owner', 'link_rating', 'link_rel', 'link_notes', 'link_rss' ), compact( 'link_id' ) ) ) {
			if ( $mt_error ) {
				return new MT_Error( 'db_update_error', __( 'Could not update link in the database.' ), $mtdb->last_error );
			} else {
				return 0;
			}
		}
	} else {
		if ( false === $mtdb->insert( $mtdb->links, compact( 'link_url', 'link_name', 'link_image', 'link_target', 'link_description', 'link_visible', 'link_owner', 'link_rating', 'link_rel', 'link_notes', 'link_rss' ) ) ) {
			if ( $mt_error ) {
				return new MT_Error( 'db_insert_error', __( 'Could not insert link into the database.' ), $mtdb->last_error );
			} else {
				return 0;
			}
		}
		$link_id = (int) $mtdb->insert_id;
	}

	mt_set_link_cats( $link_id, $link_category );
 ...
 */
function mt_set_link_cats( $link_id = 0, $link_categories = array() ) {
	...
	mt_set_object_terms( $link_id, $link_categories, 'link_category' );
 ...
 * @since 2.0.0
 *
 * @param array $linkdata Link data to update. See mt_insert_link() for accepted arguments.
 * @return int|MT_Error Value 0 or MT_Error on failure. The updated link ID on success.
 */
function mt_update_link( $linkdata ) {
	...
	return mt_insert_link( $linkdata );
}

/**
 * Outputs the 'disabled' message for the paCMec Link Manager.
 *
 * @since 3.5.0
 * @access private
 *
 ...
 */
function mt_link_manager_disabled_message() {
	global $pagenow;
	...
	if ( $really_can_manage_links && current_user_can( 'install_plugins' ) ) {
		...
		mt_die( sprintf( __( 'If you are looking to use the link manager, please install the <a href="%s">Link Manager</a> plugin.' ), $link ) );
	}
