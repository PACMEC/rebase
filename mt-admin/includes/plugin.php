 ...
 *     Requires at least: Optional. Specify the minimum required paCMec version.
 ...
 *     @type string $RequiresMT  Minimum required version of paCMec.
...
function _get_plugin_data_markup_translate( $plugin_file, $plugin_data, $markup = true, $translate = true ) {
	...
	// Sanitize the plugin filename to a MT_PLUGIN_DIR relative path.
	...
	if ( $translate ) {
		...
		if ( $textdomain ) {
			foreach ( array( 'Name', 'PluginURI', 'Description', 'Author', 'AuthorURI', 'Version' ) as $field ) {
				// phpcs:ignore paCMec.MT.I18n.LowLevelTranslationFunction,paCMec.MT.I18n.NonSingularStringLiteralText,paCMec.MT.I18n.NonSingularStringLiteralDomain
	...
	$plugin_data['Name']   = mt_kses( $plugin_data['Name'], $allowed_tags_in_links );
	$plugin_data['Author'] = mt_kses( $plugin_data['Author'], $allowed_tags );

	$plugin_data['Description'] = mt_kses( $plugin_data['Description'], $allowed_tags );
	$plugin_data['Version']     = mt_kses( $plugin_data['Version'], $allowed_tags );
	...
	if ( $markup ) {
		...
		$plugin_data['Description'] = mttexturize( $plugin_data['Description'] );
...
function get_plugin_files( $plugin ) {
	$plugin_file = MT_PLUGIN_DIR . '/' . $plugin;
	$dir         = dirname( $plugin_file );

	$plugin_files = array( plugin_basename( $plugin_file ) );

	if ( is_dir( $dir ) && MT_PLUGIN_DIR !== $dir ) {
 ...
 *
 * paCMec only supports plugin files in the base plugins directory
 * (mt-content/plugins) and in one directory above the plugins directory
 * (mt-content/plugins/my-plugin). The file it looks for has the plugin data
...
function get_plugins( $plugin_folder = '' ) {
	...
	$cache_plugins = mt_cache_get( 'plugins', 'plugins' );
	if ( ! $cache_plugins ) {
		$cache_plugins = array();
	}
	...
	$mt_plugins  = array();
	$plugin_root = MT_PLUGIN_DIR;
	if ( ! empty( $plugin_folder ) ) {
		$plugin_root .= $plugin_folder;
	}

	// Files in mt-content/plugins directory.
	...
	foreach ( $plugin_files as $plugin_file ) {
		...
		$mt_plugins[ plugin_basename( $plugin_file ) ] = $plugin_data;
	}

	uasort( $mt_plugins, '_sort_uname_callback' );

	$cache_plugins[ $plugin_folder ] = $mt_plugins;
	mt_cache_set( 'plugins', $cache_plugins, 'plugins' );
 ...
 *
 * paCMec only includes mu-plugin files in the base mu-plugins directory (mt-content/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	...
	// Files in mt-content/mu-plugins directory.
	$plugins_dir = @opendir( MTMU_PLUGIN_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
			if ( '.php' === substr( $file, -4 ) ) {
				$plugin_files[] = $file;
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( MTMU_PLUGIN_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( MTMU_PLUGIN_DIR . "/$plugin_file", false, false );

		if ( empty( $plugin_data['Name'] ) ) {
			$plugin_data['Name'] = $plugin_file;
		}

		$mt_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $mt_plugins['index.php'] ) && filesize( MTMU_PLUGIN_DIR . '/index.php' ) <= 30 ) {
		// Silence is golden.
		unset( $mt_plugins['index.php'] );
	}
...
/**
 * Check the mt-content directory and retrieve all drop-ins with any plugin data.
 *
 * @since 3.0.0
...
function get_dropins() {
	...
	// Files in mt-content directory.
	$plugins_dir = @opendir( MT_CONTENT_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( MT_CONTENT_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( MT_CONTENT_DIR . "/$plugin_file", false, false );
...
/**
 * Returns drop-ins that paCMec uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php'      => array( __( 'Advanced caching plugin.' ), 'MT_CACHE' ),  // MT_CACHE
 ...
 *
 * @since 2.5.0
 * @since 5.2.0 Test for paCMec version and PHP version compatibility.
 *
 * @param string $plugin       Path to the plugin file relative to the plugins directory.
 ...
 * @return null|MT_Error Null on success, MT_Error on invalid file.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( $network_wide && ! isset( $current[ $plugin ] )
		...
		if ( ! empty( $redirect ) ) {
			...
			mt_redirect( add_query_arg( '_error_nonce', mt_create_nonce( 'plugin-activation-error_' . $plugin ), $redirect ) );
		}
		...
		mt_register_plugin_realpath( MT_PLUGIN_DIR . '/' . $plugin );
		$_mt_plugin_file = $plugin;
		include_once MT_PLUGIN_DIR . '/' . $plugin;
		$plugin = $_mt_plugin_file; // Avoid stomping of the $plugin variable in a plugin.
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new MT_Error( 'unexpected_output', __( 'The plugin generated unexpected output.' ), $output );
		}
		ob_end_clean();
	}
...
function deactivate_plugins( $plugins, $silent = false, $network_wide = null ) {
	...
	foreach ( (array) $plugins as $plugin ) {
		...
		if ( $do_blog && mt_is_recovery_mode() ) {
			list( $extension ) = explode( '/', $plugin );
			mt_paused_plugins()->delete( $extension );
		}
 ...
 *
 * When MT_Error is returned, it does not mean that one of the plugins had
 ...
 * @return bool|MT_Error True when finished or MT_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ! empty( $errors ) ) {
		return new MT_Error( 'plugins_invalid', __( 'One of the plugins is invalid.' ), $errors );
	}
 ...
 * @return bool|null|MT_Error True on success, false if `$plugins` is empty, `MT_Error` on failure.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	$url = mt_nonce_url( 'plugins.php?action=delete-selected&verify-delete=1&' . implode( '&', $checked ), 'bulk-plugins' );
	...
	if ( is_mt_error( $mt_filesystem->errors ) && $mt_filesystem->errors->has_errors() ) {
		return new MT_Error( 'fs_error', __( 'Filesystem error.' ), $mt_filesystem->errors );
	}

	// Get the base plugin folder.
	$plugins_dir = $mt_filesystem->mt_plugins_dir();
	if ( empty( $plugins_dir ) ) {
		return new MT_Error( 'fs_no_plugins_dir', __( 'Unable to locate paCMec plugin directory.' ) );
	}

	$plugins_dir = trailingslashit( $plugins_dir );

	$plugin_translations = mt_get_installed_translations( 'plugins' );
	...
	foreach ( $plugins as $plugin_file ) {
		...
		if ( strpos( $plugin_file, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			$deleted = $mt_filesystem->delete( $this_plugin_dir, true );
		} else {
			$deleted = $mt_filesystem->delete( $plugins_dir . $plugin_file );
		}
		...
		if ( '.' !== $plugin_slug && ! empty( $plugin_translations[ $plugin_slug ] ) ) {
			...
			foreach ( $translations as $translation => $data ) {
				$mt_filesystem->delete( MT_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.po' );
				$mt_filesystem->delete( MT_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.mo' );

				$json_translation_files = glob( MT_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '-*.json' );
				if ( $json_translation_files ) {
					array_map( array( $mt_filesystem, 'delete' ), $json_translation_files );
				}
			}
		}
	}
	...
	if ( ! empty( $errors ) ) {
		...
		return new MT_Error( 'could_not_remove_plugin', sprintf( $message, implode( ', ', $errors ) ) );
	}
 ...
 *
 * @since 2.5.0
 * @return MT_Error[] Array of plugin errors keyed by plugin file name.
 */
 ...
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 * @return int|MT_Error 0 on success, MT_Error on failure.
 */
function validate_plugin( $plugin ) {
	if ( validate_file( $plugin ) ) {
		return new MT_Error( 'plugin_invalid', __( 'Invalid plugin path.' ) );
	}
	if ( ! file_exists( MT_PLUGIN_DIR . '/' . $plugin ) ) {
		return new MT_Error( 'plugin_not_found', __( 'Plugin file does not exist.' ) );
	}

	$installed_plugins = get_plugins();
	if ( ! isset( $installed_plugins[ $plugin ] ) ) {
		return new MT_Error( 'no_plugin_header', __( 'The plugin does not have a valid header.' ) );
	}
	return 0;
}

/**
 * Validates the plugin requirements for paCMec version and PHP version.
 *
...
function validate_plugin_requirements( $plugin ) {
	$plugin_headers = get_plugin_data( MT_PLUGIN_DIR . '/' . $plugin );

	$requirements = array(
		'requires'     => ! empty( $plugin_headers['RequiresMT'] ) ? $plugin_headers['RequiresMT'] : '',
		'requires_php' => ! empty( $plugin_headers['RequiresPHP'] ) ? $plugin_headers['RequiresPHP'] : '',
	);

	$readme_file = MT_PLUGIN_DIR . '/' . dirname( $plugin ) . '/readme.txt';
	...
	if ( ! $compatible_mt && ! $compatible_php ) {
		return new MT_Error(
			'plugin_mt_php_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current paCMec version, 2: Current PHP version, 3: Plugin name, 4: Required paCMec version, 5: Required PHP version. */
				_x( '<strong>Error:</strong> Current versions of paCMec (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires paCMec %4$s and PHP %5$s.', 'plugin' ),
		...
		return new MT_Error(
			'plugin_mt_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current paCMec version, 2: Plugin name, 3: Required paCMec version. */
				_x( '<strong>Error:</strong> Current paCMec version (%1$s) does not meet minimum requirements for %2$s. The plugin requires paCMec %3$s.', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
...
function is_uninstallable_plugin( $plugin ) {
	...
	if ( isset( $uninstallable_plugins[ $file ] ) || file_exists( MT_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		return true;
	}
...
function uninstall_plugin( $plugin ) {
	...
	if ( file_exists( MT_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		...
		define( 'MT_UNINSTALL_PLUGIN', $file );

		mt_register_plugin_realpath( MT_PLUGIN_DIR . '/' . $file );
		include_once MT_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php';
	...
	if ( isset( $uninstallable_plugins[ $file ] ) ) {
		...
		mt_register_plugin_realpath( MT_PLUGIN_DIR . '/' . $file );
		include_once MT_PLUGIN_DIR . '/' . $file;
 ...
 *                             * Pass 'none' to leave div.mt-menu-image empty so an icon can be added via CSS.
 ...
 * @global array $menu
 * @global array $_mt_real_parent_file
 * @global bool  $_mt_submenu_nopriv
 * @global array $_registered_pages
 * @global array $_parent_pages
 *
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '', $position = null ) {
	global $submenu, $menu, $_mt_real_parent_file, $_mt_submenu_nopriv,
		$_registered_pages, $_parent_pages;

	$menu_slug   = plugin_basename( $menu_slug );
	$parent_slug = plugin_basename( $parent_slug );

	if ( isset( $_mt_real_parent_file[ $parent_slug ] ) ) {
		$parent_slug = $_mt_real_parent_file[ $parent_slug ];
	}

	if ( ! current_user_can( $capability ) ) {
		$_mt_submenu_nopriv[ $parent_slug ][ $menu_slug ] = true;
		return false;
	}
	 ...
	 * See mt-admin/admin.php for redirect from edit.php to tools.php.
	 */
 ...
 * @global array  $_mt_real_parent_file
 * @global array  $_mt_menu_nopriv
 * @global array  $_mt_submenu_nopriv
 *
 * @param string $parent The slug name for the parent menu (or the file name of a standard
 *                       paCMec admin page). Default empty string.
 * @return string The parent file of the current admin page.
 */
function get_admin_page_parent( $parent = '' ) {
	global $parent_file, $menu, $submenu, $pagenow, $typenow,
		$plugin_page, $_mt_real_parent_file, $_mt_menu_nopriv, $_mt_submenu_nopriv;

	if ( ! empty( $parent ) && 'admin.php' !== $parent ) {
		if ( isset( $_mt_real_parent_file[ $parent ] ) ) {
			$parent = $_mt_real_parent_file[ $parent ];
		}
	...
	if ( 'admin.php' === $pagenow && isset( $plugin_page ) ) {
		foreach ( (array) $menu as $parent_menu ) {
			if ( $parent_menu[2] === $plugin_page ) {
				...
				if ( isset( $_mt_real_parent_file[ $parent_file ] ) ) {
					$parent_file = $_mt_real_parent_file[ $parent_file ];
				}

				return $parent_file;
			}
		}
		if ( isset( $_mt_menu_nopriv[ $plugin_page ] ) ) {
			$parent_file = $plugin_page;
	...
	if ( isset( $plugin_page ) && isset( $_mt_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
		$parent_file = $pagenow;
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_mt_menu_nopriv, $_mt_submenu_nopriv,
		$plugin_page, $_registered_pages;

	$parent = get_admin_page_parent();

	if ( ! isset( $plugin_page ) && isset( $_mt_submenu_nopriv[ $parent ][ $pagenow ] ) ) {
		return false;
	}

	if ( isset( $plugin_page ) ) {
		if ( isset( $_mt_submenu_nopriv[ $parent ][ $plugin_page ] ) ) {
			return false;
		}
	...
	if ( empty( $parent ) ) {
		if ( isset( $_mt_menu_nopriv[ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $_mt_submenu_nopriv[ $pagenow ][ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_mt_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_mt_menu_nopriv[ $plugin_page ] ) ) {
			return false;
		}

		foreach ( array_keys( $_mt_submenu_nopriv ) as $key ) {
			if ( isset( $_mt_submenu_nopriv[ $key ][ $pagenow ] ) ) {
				return false;
			}
			if ( isset( $plugin_page ) && isset( $_mt_submenu_nopriv[ $key ][ $plugin_page ] ) ) {
				return false;
			}
		}
	...
	if ( isset( $plugin_page ) && $plugin_page === $parent && isset( $_mt_menu_nopriv[ $plugin_page ] ) ) {
		return false;
	}
...
function settings_fields( $option_group ) {
	...
	mt_nonce_field( "$option_group-options" );
}
 ...
 */
function mt_clean_plugins_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_plugins' );
	}
	mt_cache_delete( 'plugins', 'plugins' );
}
 ...
 *
 * @since 3.0.0
 * @since 4.4.0 Function was moved into the `mt-admin/includes/plugin.php` file.
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 */
 ...
 */
function mt_add_privacy_policy_content( $plugin_name, $policy_text ) {
	if ( ! is_admin() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: %s: admin_init */
				__( 'The suggested privacy policy content should be added only in mt-admin by using the %s (or later) action.' ),
	...
	MT_Privacy_Policy_Content::add( $plugin_name, $policy_text );
}
 ...
 */
function mt_get_plugin_error( $plugin ) {
	if ( ! isset( $GLOBALS['_paused_plugins'] ) ) {
		return false;
	}
 ...
 * @return bool|MT_Error True on success, false if `$plugin` was not paused,
 *                       `MT_Error` on failure.
 */
function resume_plugin( $plugin, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		mt_redirect(
			add_query_arg(
				'_error_nonce',
				mt_create_nonce( 'plugin-resume-error_' . $plugin ),
				$redirect
			)
		);
	...
	$result = mt_paused_plugins()->delete( $extension );
