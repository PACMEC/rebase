<?php
/**
 * List Table API: MT_MS_Themes_List_Table class
 *
 * @package paCMec
 ...
 * @see MT_List_Table
 */
class MT_MS_Themes_List_Table extends MT_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->show_autoupdates = mt_is_auto_update_enabled_for_type( 'theme' ) &&
	...
	public function prepare_items() {
		...
		mt_reset_vars( array( 'orderby', 'order', 's' ) );

		$themes = array(
			/**
			 * Filters the full array of MT_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 3.1.0
			 *
			 * @param MT_Theme[] $all Array of MT_Theme objects to display in the list table.
			 */
			'all'      => apply_filters( 'all_themes', mt_get_themes() ),
			'search'   => array(),
			'enabled'  => array(),
			'disabled' => array(),
			'upgrade'  => array(),
			'broken'   => $this->is_site_themes ? array() : mt_get_themes( array( 'errors' => true ) ),
		);
		...
		foreach ( (array) $themes['all'] as $key => $theme ) {
			...
			$auto_update_forced = mt_is_auto_update_forced_for_item( 'theme', null, $filter_payload );
		...
		MT_Theme::sort_by_name( $this->items );
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_Theme $theme The current MT_Theme object.
	 */
	...
	public function column_name( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() ) {
				...
				$actions['enable'] = sprintf(
					...
					esc_url( mt_nonce_url( $url, 'enable-theme_' . $stylesheet ) ),
					esc_attr( $aria_label ),
			...
			$actions['disable'] = sprintf(
				...
				esc_url( mt_nonce_url( $url, 'disable-theme_' . $stylesheet ) ),
				esc_attr( $aria_label ),
		...
		if ( ! $allowed && ! $this->is_site_themes
			...
			$actions['delete'] = sprintf(
				...
				esc_url( mt_nonce_url( $url, 'bulk-themes' ) ),
				esc_attr( $aria_label ),
				__( 'Delete' )
			);
		}
	...
	public function column_default( $theme, $column_name ) {
		 ...
		 * @param string   $stylesheet  Directory name of the theme.
		 * @param MT_Theme $theme       Current MT_Theme object.
		 */
	 ...
	 * @since 4.3.0
	 *
	 * @param MT_Theme $item The current MT_Theme object.
	 */
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes ) {
			remove_action( "after_theme_row_$stylesheet", 'mt_theme_update_row' );
		}
