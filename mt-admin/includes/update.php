<?php
/**
 * paCMec Administration Update API
 *
 * @package paCMec
...
/**
 * Gets the best available (and enabled) Auto-Update for paCMec core.
 *
...
/**
 * Gets and caches the checksums for the given version of paCMec.
 *
 * @since 3.7.0
 *
...
function get_core_checksums( $version, $locale ) {
	...
	if ( $ssl && is_mt_error( $response ) ) {
		...
		$response = mt_remote_get( $http_url, $options );
	}
...
/**
 * Finds the available update for paCMec core.
 *
 * @since 2.7.0
 *
...
function core_update_footer( $msg = '' ) {
	...
	if ( $is_development_version ) {
		return sprintf(
			/* translators: 1: paCMec version number, 2: URL to paCMec Updates screen. */
...
function update_nag() {
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: 1: URL to paCMec release notes, 2: New paCMec version, 3: URL to network admin, 4: Accessibility text. */
			__( '<a href="%1$s">paCMec %2$s</a> is available! <a href="%3$s" aria-label="%4$s">Please update now</a>.' ),
			$version_url,
			$cur->current,
			network_admin_url( 'update-core.php' ),
			esc_attr__( 'Please update paCMec now' )
		);
	} else {
		$msg = sprintf(
			/* translators: 1: URL to paCMec release notes, 2: New paCMec version. */
...
/**
 * Displays paCMec version and active theme in the 'At a Glance' dashboard widget.
 *
 * @since 2.5.0
 */
function update_right_now_message() {
	$theme_name = mt_get_theme();
	...
	if ( current_user_can( 'update_core' ) ) {
		...
		if ( isset( $cur->response ) && 'upgrade' === $cur->response ) {
			$msg .= sprintf(
				'<a href="%s" class="button" aria-describedby="mt-version">%s</a> ',
				network_admin_url( 'update-core.php' ),
				/* translators: %s: paCMec version number, or 'Latest' string. */
	...
	$content = __( 'paCMec %1$s running %2$s theme.' );
	...
	$msg .= sprintf( '<span id="mt-version">' . $content . '</span>', get_bloginfo( 'version', 'display' ), $theme_name );

	echo "<p id='mt-version-message'>$msg</p>";
}
 ...
 * @since 2.9.0
 */
function mt_plugin_update_rows() {
	...
	if ( isset( $plugins->response ) && is_array( $plugins->response ) ) {
		...
		foreach ( $plugins as $plugin_file ) {
			add_action( "after_plugin_row_{$plugin_file}", 'mt_plugin_update_row', 10, 2 );
		}
	}
}
 ...
 * @return void|false
 */
function mt_plugin_update_row( $file, $plugin_data ) {
	$current = get_site_transient( 'update_plugins' );
	...
	$plugin_name = mt_kses( $plugin_data['Name'], $plugins_allowedtags );
	...
	if ( is_network_admin() || ! is_multisite() ) {
		...
		printf(
			...
			esc_attr( $file ),
			esc_attr( $mt_list_table->get_column_count() ),
			$notice_type
		);
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			...
			if ( $compatible_php ) {
				printf(
					...
					esc_attr( $response->new_version ),
					mt_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' ) . $file, 'upgrade-plugin_' . $file ),
					sprintf(
				...
				mt_update_php_annotation( '<br><em>', '</em>' );
			}
		}
		 ...
		 * }
		 */
		do_action( "in_plugin_update_message-{$file}", $plugin_data, $response ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
...
function get_theme_updates() {
	...
	foreach ( $current->response as $stylesheet => $data ) {
		$update_themes[ $stylesheet ]         = mt_get_theme( $stylesheet );
...
function mt_theme_update_rows() {
	...
	if ( isset( $themes->response ) && is_array( $themes->response ) ) {
		...
		foreach ( $themes as $theme ) {
			add_action( "after_theme_row_{$theme}", 'mt_theme_update_row', 10, 2 );
		}
	}
}
 ...
 * @return void|false
 */
function mt_theme_update_row( $theme_key, $theme ) {
	$current = get_site_transient( 'update_themes' );
	...
	/** @var MT_MS_Themes_List_Table $mt_list_table */
	$mt_list_table = _get_list_table( 'MT_MS_Themes_List_Table' );

	$active = $theme->is_allowed( 'network' ) ? ' active' : '';

	$requires_mt  = isset( $response['requires'] ) ? $response['requires'] : null;
	$requires_php = isset( $response['requires_php'] ) ? $response['requires_php'] : null;
	...
	printf(
		...
		$mt_list_table->get_column_count()
	);
	...
	if ( $compatible_mt && $compatible_php ) {
		if ( ! current_user_can( 'update_themes' ) ) {
			...
			printf(
				...
				$response['new_version'],
				mt_nonce_url( self_admin_url( 'update.php?action=upgrade-theme&theme=' ) . $theme_key, 'upgrade-theme_' . $theme_key ),
				sprintf(
					'class="update-link" aria-label="%s"',
	 ...
	 * the theme slug as found in the paCMec.org themes repository.
	 *
	 * @since 3.1.0
	 *
	 * @param MT_Theme $theme    The MT_Theme object.
	 * @param array    $response {
	 ...
	 * }
	 */
	do_action( "in_theme_update_message-{$theme_key}", $theme, $response ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
...
function maintenance_nag() {
	...
	if ( ! $nag ) {
		...
		if ( isset( $failed['attempted'] ) && version_compare( $failed['attempted'], $mt_version, $comparison ) ) {
			$nag = true;
		}
	}
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: %s: URL to paCMec Updates screen. */
			__( 'An automated paCMec update has failed to complete - <a href="%s">please attempt the update again now</a>.' ),
			'update-core.php'
		);
	} else {
		$msg = __( 'An automated paCMec update has failed to complete! Please notify the site administrator.' );
	}
 ...
 * @since 4.6.0
 */
function mt_print_admin_notice_templates() {
	?>
	<script id="tmpl-mt-updates-admin-notice" type="text/html">
		<div <# if ( data.id ) { #>id="{{ data.id }}"<# } #> class="notice {{ data.className }}"><p>{{{ data.message }}}</p></div>
	</script>
	<script id="tmpl-mt-bulk-updates-admin-notice" type="text/html">
 ...
 * @since 4.6.0
 */
function mt_print_update_row_templates() {
	?>
 ...
 */
function mt_is_auto_update_enabled_for_type( $type ) {
	if ( ! class_exists( 'MT_Automatic_Updater' ) ) {
 ...
 */
function mt_is_auto_update_forced_for_item( $type, $update, $item ) {
	/** This filter is documented in mt-admin/includes/class-mt-automatic-updater.php */
 ...
 */
function mt_get_auto_update_message() {
	$next_update_time = mt_next_scheduled( 'mt_version_check' );

	// Check if the event exists.
	if ( false === $next_update_time ) {
		$message = __( 'Automatic update not scheduled. There may be a problem with MT-Cron.' );
	} else {
		...
		if ( $overdue ) {
			$message = sprintf(
				/* translators: %s: Duration that MT-Cron has been overdue. */
				__( 'Automatic update overdue by %s. There may be a problem with MT-Cron.' ),
				$time_to_next_update
			);
		} else {
			$message = sprintf(
