 ...
 * @see MT_Upgrader
 */
class Theme_Upgrader extends MT_Upgrader {
	 ...
	 * }
	 *
	 * @return bool|MT_Error True if the installation was successful, false or a MT_Error object otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( $parsed_args['clear_update_cache'] ) {
			// Clear cache so mt_update_themes() knows about the new theme.
			add_action( 'upgrader_process_complete', 'mt_clean_themes_cache', 9, 0 );
		}
		...
		remove_action( 'upgrader_process_complete', 'mt_clean_themes_cache', 9 );
		...
		mt_clean_themes_cache( $parsed_args['clear_update_cache'] );

		if ( $parsed_args['overwrite_package'] ) {
			/** This action is documented in mt-admin/includes/class-plugin-upgrader.php */
	...
	public function bulk_upgrade( $themes, $args = array() ) {
		...
		$res = $this->fs_connect( array( MT_CONTENT_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		foreach ( $results as $theme => $result ) {
			...
			if ( ! $result || is_mt_error( $result ) || ! isset( $past_failure_emails[ $theme ] ) ) {
				continue;
			}
	...
	public function check_package( $source ) {
		...
		if ( ! is_mt_version_compatible( $requires_mt ) ) {
			$error = sprintf(
				/* translators: 1: Current paCMec version, 2: Version required by the uploaded theme. */
				__( 'Your paCMec version is %1$s, however the uploaded theme requires %2$s.' ),
				$mt_version,
				$requires_mt
			);
	...
	public function delete_old_theme( $removed, $local_destination, $remote_destination, $theme ) {
		...
		$theme      = $theme['theme'];
		$themes_dir = trailingslashit( $mt_filesystem->mt_themes_dir( $theme ) );
		if ( $mt_filesystem->exists( $themes_dir . $theme ) ) {
			if ( ! $mt_filesystem->delete( $themes_dir . $theme, true ) ) {
				return false;
			}
		}
	...
	/**
	 * Get the MT_Theme object for a theme.
	 *
	 * @since 2.8.0
	 ...
	 * @return MT_Theme|false The theme's info object, or false `$theme` is not supplied
