<?php
/**
 * paCMec Administration Revisions API
 *
 * @package paCMec
 ...
 * @since 3.6.0
 *
 * @param MT_Post|int $post         The post object or post ID.
 * @param int         $compare_from The revision ID to compare from.
 * @param int         $compare_to   The revision ID to come to.
 * @return array|false Associative array of a post's revisioned fields and their diffs.
 *                    Or, false on failure.
 */
function mt_get_revision_ui_diff( $post, $compare_from, $compare_to ) {
	$post = get_post( $post );
	if ( ! $post ) {
		return false;
	}
	...
	foreach ( _mt_post_revision_fields( $post ) as $field => $name ) {
		/**
		 ...
		 * @param MT_Post $compare_from   The revision post object to compare to or from.
		 * @param string  $context        The context of whether the current revision is the old
		 *                                or the new one. Values are 'to' or 'from'.
		 */
		$content_from = $compare_from ? apply_filters( "_mt_post_revision_field_{$field}", $compare_from->$field, $field, $compare_from, 'from' ) : '';

		/** This filter is documented in mt-admin/includes/revision.php */
		$content_to = apply_filters( "_mt_post_revision_field_{$field}", $compare_to->$field, $field, $compare_to, 'to' );
		 ...
		 *
		 * Filters the options passed to mt_text_diff() when viewing a post revision.
		 *
		 * @since 4.1.0
		 *
		 * @param array   $args {
		 *     Associative array of options to pass to mt_text_diff().
		 *
		 *     @type bool $show_split_view True for split view (two columns), false for
		 *                                 un-split view (single column). Default true.
		 * }
		 * @param string  $field        The current revision field.
		 * @param MT_Post $compare_from The revision post to compare from.
		 * @param MT_Post $compare_to   The revision post to compare to.
		 */
		$args = apply_filters( 'revision_text_diff_options', $args, $field, $compare_from, $compare_to );

		$diff = mt_text_diff( $content_from, $content_to, $args );
	 ...
	 * @param MT_Post $compare_to   The revision post to compare to.
	 */
	return apply_filters( 'mt_get_revision_ui_diff', $return, $compare_from, $compare_to );
 ...
 */
function mt_prepare_revisions_for_js( $post, $selected_revision_id, $from = null ) {
	$post    = get_post( $post );
	$authors = array();
	$now_gmt = time();
	...
	cache_users( mt_list_pluck( $revisions, 'post_author' ) );
	...
	foreach ( $revisions as $revision ) {
		...
		$autosave = (bool) mt_is_post_autosave( $revision );
		 ...
		 *     @type string     $title      Title for the revision's parent MT_Post object.
		 ...
		 * }
		 * @param MT_Post $revision       The revision's MT_Post object.
		 * @param MT_Post $post           The revision's parent MT_Post object.
		 */
		$revisions[ $revision->ID ] = apply_filters( 'mt_prepare_revision_for_js', $revisions_data, $revision, $post );
	}
	...
	$diffs = array(
		array(
			...
			'fields' => mt_get_revision_ui_diff( $post->ID, $from, $selected_revision_id ),
		),
	);

	return array(
		'postId'         => $post->ID,
		'nonce'          => mt_create_nonce( 'revisions-ajax-nonce' ),
 ...
 */
function mt_print_revision_templates() {
	global $post;
		...
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					<input  <?php if ( mt_check_post_lock( $post->ID ) ) { ?>
						disabled="disabled"
					<?php } else { ?>
