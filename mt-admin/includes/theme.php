 ...
 * @return bool|null|MT_Error True on success, false if `$stylesheet` is empty, MT_Error on failure.
...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $redirect ) ) {
		$redirect = mt_nonce_url( 'themes.php?action=delete&stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet );
	}
	...
	$themes_dir = $mt_filesystem->mt_themes_dir();
	if ( empty( $themes_dir ) ) {
		return new MT_Error( 'fs_no_themes_dir', __( 'Unable to locate paCMec theme directory.' ) );
	}

	$themes_dir = trailingslashit( $themes_dir );
	$theme_dir  = trailingslashit( $themes_dir . $stylesheet );
	$deleted    = $mt_filesystem->delete( $theme_dir, true );
	...
	$theme_translations = mt_get_installed_translations( 'themes' );
	...
	if ( ! empty( $theme_translations[ $stylesheet ] ) ) {
		...
		foreach ( $translations as $translation => $data ) {
			$mt_filesystem->delete( MT_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.po' );
			$mt_filesystem->delete( MT_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.mo' );

			$json_translation_files = glob( MT_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '-*.json' );
	...
	if ( is_multisite() ) {
		MT_Theme::network_disable_theme( $stylesheet );
	}
 ...
 *
 * @param MT_Post|null $post      Optional. The post being edited, provided for context.
 * @param string       $post_type Optional. Post type to get the templates for. Default 'page'.
 * @return string[] Array of template file names keyed by the template header name.
 */
function get_page_templates( $post = null, $post_type = 'page' ) {
	return array_flip( mt_get_theme()->get_page_templates( $post, $post_type ) );
}
 ...
 *
 * @param MT_Theme $theme Theme data object.
 */
 ...
 * @since 3.8.0
 *
 * @param MT_Theme $theme MT_Theme object.
...
function get_theme_update_available( $theme ) {
	...
	if ( isset( $themes_update->response[ $stylesheet ] ) ) {
		...
		$details_url = add_query_arg(
			...
			$update['url']
		); // Theme browser inside MT? Replace this. Also, theme preview JS will override this on the available list.
		$update_url  = mt_nonce_url( admin_url( 'update.php?action=upgrade-theme&amp;theme=' . urlencode( $stylesheet ) ), 'upgrade-theme_' . $stylesheet );
...
/**
 * Retrieve list of paCMec theme features (aka theme tags).
 *
 * @since 3.1.0
 ...
 *
 * @param bool $api Optional. Whether try to fetch tags from the paCMec.org API. Defaults to true.
...
function get_theme_feature_list( $api = true ) {
	...
	$feature_list = get_site_transient( 'mtorg_theme_feature_list' );
	if ( ! $feature_list ) {
		set_site_transient( 'mtorg_theme_feature_list', array(), 3 * HOUR_IN_SECONDS );
	}

	if ( ! $feature_list ) {
		$feature_list = themes_api( 'feature_list', array() );
		if ( is_mt_error( $feature_list ) ) {
			return $features;
		}
	}
	...
	set_site_transient( 'mtorg_theme_feature_list', $feature_list, 3 * HOUR_IN_SECONDS );
	...
	$mtorg_features = array();

	// Loop over the mt.org canonical list and apply translations.
	foreach ( (array) $feature_list as $feature_category => $feature_items ) {
		if ( isset( $category_translations[ $feature_category ] ) ) {
			$feature_category = $category_translations[ $feature_category ];
		}

		$mtorg_features[ $feature_category ] = array();

		foreach ( $feature_items as $feature ) {
			if ( isset( $features[ $feature_category ][ $feature ] ) ) {
				$mtorg_features[ $feature_category ][ $feature ] = $features[ $feature_category ][ $feature ];
			} else {
				$mtorg_features[ $feature_category ][ $feature ] = $feature;
			}
		}
	}

	return $mtorg_features;
}

/**
 * Retrieves theme installer pages from the paCMec.org Themes API.
 *
 ...
 *
 * The second filter, {@see 'themes_api'}, allows a plugin to override the paCMec.org
 ...
 *         {@link https://developer.managertechnology.com.co/pacmec/reference/functions/themes_api/ function reference article}
...
function themes_api( $action, $args = array() ) {
	...
	if ( ! isset( $args->mt_version ) ) {
		$args->mt_version = substr( $mt_version, 0, 3 ); // x.y
	}

	/**
	 * Filters arguments used to query for installer pages from the paCMec.org Themes API.
	 *
	 * Important: An object MUST be returned to this filter.
	 *
	 * @since 2.8.0
	 *
	 * @param object $args   Arguments used to query for installer pages from the paCMec.org Themes API.
	...
	/**
	 * Filters whether to override the paCMec.org Themes API.
	 *
	 * Passing a non-false value will effectively short-circuit the paCMec.org API request.
	 *
	 ...
	 * @since 2.8.0
	 *
	 * @param false|object|array $override Whether to override the paCMec.org Themes API. Default false.
	 * @param string             $action   Requested action. Likely values are 'theme_information',
	 *                                    'feature_list', or 'query_themes'.
	...
	if ( ! $res ) {
		$url = 'http://api.managertechnology.com.co/pacmec/themes/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
		...
		$request   = mt_remote_get( $url, $http_args );
		...
		if ( $ssl && is_mt_error( $request ) ) {
			...
			$request = mt_remote_get( $http_url, $http_args );
		}
		...
		if ( is_mt_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new MT_Error( 'themes_api_failed', $res->error );
			}
		}
	...
	/**
	 * Filters the returned paCMec.org Themes API response.
	 *
	 * @since 2.8.0
	 *
	 * @param array|object|MT_Error $res    paCMec.org Themes API response.
	 * @param string                $action Requested action. Likely values are 'theme_information',
	 *                                      'feature_list', or 'query_themes'.
	 * @param object                $args   Arguments used to query for installer pages from the paCMec.org Themes API.
	 */
 ...
 * @since 3.8.0
 *
 * @param MT_Theme[] $themes Optional. Array of theme objects to prepare.
 *                           Defaults to all allowed themes.
 *
 * @return array An associative array of theme data, sorted by name.
 */
function mt_prepare_themes_for_js( $themes = null ) {
	$current_theme = get_stylesheet();

	/**
	 * Filters theme data before it is prepared for JavaScript.
	 *
	 * Passing a non-empty array will result in mt_prepare_themes_for_js() returning
	 * early with that value instead.
	 *
	 * @since 4.2.0
	 *
	 * @param array           $prepared_themes An associative array of theme data. Default empty array.
	 * @param MT_Theme[]|null $themes          An array of theme objects to prepare, if any.
	...
	if ( null === $themes ) {
		...
		if ( ! isset( $themes[ $current_theme ] ) ) {
			$themes[ $current_theme ] = mt_get_theme();
		}
	}
	...
	foreach ( $themes as $theme ) {
		...
		if ( current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
			$customize_action = esc_url(
				add_query_arg(
					array(
						'return' => urlencode( esc_url_raw( remove_query_arg( mt_removable_query_args(), mt_unslash( $_SERVER['REQUEST_URI'] ) ) ) ),
					),
		...
		$update_requires_mt  = isset( $updates[ $slug ]['requires'] ) ? $updates[ $slug ]['requires'] : null;
		...
		if ( isset( $updates[ $slug ] ) ) {
			...
			$auto_update_filter_payload = (object) array(
				...
				'url'          => '',
				'package'      => '',
				'requires'     => $theme->get( 'RequiresMT' ),
				'requires_php' => $theme->get( 'RequiresPHP' ),
			);
		}

		$auto_update_forced = mt_is_auto_update_forced_for_item( 'theme', null, $auto_update_filter_payload );
		...
		$prepared_themes[ $slug ] = array(
			...
			'compatibleMT'   => is_mt_version_compatible( $theme->get( 'RequiresMT' ) ),
			'compatiblePHP'  => is_php_version_compatible( $theme->get( 'RequiresPHP' ) ),
			'updateResponse' => array(
				'compatibleMT'  => is_mt_version_compatible( $update_requires_mt ),
			...
			'actions'        => array(
				'activate'   => current_user_can( 'switch_themes' ) ? mt_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . $encoded_slug ), 'switch-theme_' . $slug ) : null,
				'customize'  => $customize_action,
				'delete'     => ( ! is_multisite() && current_user_can( 'delete_themes' ) ) ? mt_nonce_url( admin_url( 'themes.php?action=delete&amp;stylesheet=' . $encoded_slug ), 'delete-theme_' . $slug ) : null,
				'autoupdate' => mt_is_auto_update_enabled_for_type( 'theme' ) && ! is_multisite() && current_user_can( 'update_themes' )
					? mt_nonce_url( admin_url( 'themes.php?action=' . $auto_update_action . '&amp;stylesheet=' . $encoded_slug ), 'updates' )
					: null,
			),
		);
	}
	 ...
	 */
	$prepared_themes = apply_filters( 'mt_prepare_themes_for_js', $prepared_themes );
...
function customize_themes_print_templates() {
	...
	<script type="text/html" id="tmpl-customize-themes-details-view">
		...
		<div class="theme-wrap mt-clearfix" role="document">
			...
			<div class="theme-about mt-clearfix">
				...
				<div class="theme-info">
					...
					<# if ( data.hasUpdate ) { #>
						<# if ( data.updateResponse.compatibleMT && data.updateResponse.compatiblePHP ) { #>
							...
							<div class="notice notice-error notice-alt notice-large" data-slug="{{ data.id }}">
								...
								<p>
									<# if ( ! data.updateResponse.compatibleMT && ! data.updateResponse.compatiblePHP ) { #>
										...
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it doesn&#8217;t work with your versions of paCMec and PHP.' ),
											'{{{ data.name }}}'
										);
										if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
											...
											mt_update_php_annotation( '</p><p><em>', '</em>' );
										...
										?>
									<# } else if ( ! data.updateResponse.compatibleMT ) { #>
										<?php
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it doesn&#8217;t work with your version of paCMec.' ),
											'{{{ data.name }}}'
										);
					...
					<# if ( ! data.compatibleMT || ! data.compatiblePHP ) { #>
						<div class="notice notice-error notice-alt notice-large"><p>
							<# if ( ! data.compatibleMT && ! data.compatiblePHP ) { #>
								<?php
								_e( 'This theme doesn&#8217;t work with your versions of paCMec and PHP.' );
								if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
									printf(
										/* translators: 1: URL to paCMec Updates screen, 2: URL to Update PHP page. */
										' ' . __( '<a href="%1$s">Please update paCMec</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
								...
								?>
							<# } else if ( ! data.compatibleMT ) { #>
								<?php
								_e( 'This theme doesn&#8217;t work with your version of paCMec.' );
								if ( current_user_can( 'update_core' ) ) {
									printf(
										/* translators: %s: URL to paCMec Updates screen. */
										' ' . __( '<a href="%s">Please update paCMec</a>.' ),
 ...
 */
function mt_get_theme_error( $theme ) {
	if ( ! isset( $GLOBALS['_paused_themes'] ) ) {
		return false;
	}
 ...
 * @return bool|MT_Error True on success, false if `$theme` was not paused,
 *                       `MT_Error` on failure.
 */
function resume_theme( $theme, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		...
		if ( ! empty( $functions_path ) ) {
			mt_redirect(
				add_query_arg(
					'_error_nonce',
					mt_create_nonce( 'theme-resume-error_' . $theme ),
					$redirect
				)
			);
	...
	$result = mt_paused_themes()->delete( $extension );
