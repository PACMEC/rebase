...
if ( ! current_user_can( 'manage_sites' ) ) {
	mt_die( __( 'Sorry, you are not allowed to edit this site.' ) );
}
...
if ( isset( $_REQUEST['action'] ) && 'update-site' === $_REQUEST['action'] && is_array( $_POST['option'] ) ) {
	...
	foreach ( (array) $_POST['option'] as $key => $val ) {
		...
		if ( 0 === $key || is_array( $val ) || in_array( $key, $skip_options, true ) ) {
			continue; // Avoids "0 is a protected MT option and may not be modified" error when editing blog options.
		}
	 ...
	 */
	do_action( 'mtmu_update_blog_options', $id );
...
<form method="post" action="site-settings.php?action=update-site">
	...
	<table class="form-table" role="presentation">
		...
		$query       = $mtdb->prepare(
			$sql,
			$mtdb->esc_like( '_' ) . '%',
			'%' . $mtdb->esc_like( 'user_roles' )
		);
		$options     = $mtdb->get_results( $query );
		 ...
		 */
		do_action( 'mtmueditblogaction', $id );
		?>
	</table>
	<?php submit_button(); ?>
</form>
