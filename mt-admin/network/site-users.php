...
if ( ! current_user_can( 'manage_sites' ) ) {
	mt_die( __( 'Sorry, you are not allowed to edit this site.' ), 403 );
}
...
$referer                = remove_query_arg( 'update', mt_get_referer() );
...
if ( ! $id ) {
	mt_die( __( 'Invalid site ID.' ) );
}

$details = get_site( $id );
if ( ! $details ) {
	mt_die( __( 'The requested site does not exist.' ) );
}

if ( ! can_edit_network( $details->site_id ) ) {
	mt_die( __( 'Sorry, you are not allowed to access this page.' ), 403 );
}
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'newuser':
			check_admin_referer( 'add-user', '_mtnonce_add-new-user' );
			$user = $_POST['user'];
			if ( ! is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				$update = 'err_new';
			} else {
				$password = mt_generate_password( 12, false );
				$user_id  = mtmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
		...
		default:
			...
			$referer = apply_filters( 'handle_network_bulk_actions-' . get_current_screen()->id, $referer, $action, $userids, $id ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores

			$update = $action;
			break;
	}

	mt_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit;
}
 ...
 */
if ( ! mt_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) ) {
...
/** This filter is documented in mt-admin/network/site-users.php */
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input type="text" class="regular-text mt-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
			...
			<?php
			switch_to_blog( $id );
			mt_dropdown_roles( get_option( 'default_role' ) );
			restore_current_blog();
			?>
			</select></td>
		</tr>
...
<form action="<?php echo network_admin_url( 'site-users.php?action=newuser' ); ?>" id="newuser" method="post">
	...
	<?php mt_nonce_field( 'add-user', '_mtnonce_add-new-user' ); ?>
