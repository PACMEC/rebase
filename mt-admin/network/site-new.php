...
if ( ! current_user_can( 'create_sites' ) ) {
	mt_die( __( 'Sorry, you are not allowed to add sites to this network.' ) );
}
...
if ( isset( $_REQUEST['action'] ) && 'add-site' === $_REQUEST['action'] ) {
	check_admin_referer( 'add-blog', '_mtnonce_add-blog' );

	if ( ! is_array( $_POST['blog'] ) ) {
		mt_die( __( 'Can&#8217;t create an empty site.' ) );
	}
		...
		if ( in_array( $domain, $subdirectory_reserved_names, true ) ) {
			mt_die(
				sprintf(
					...
					__( 'The following words are reserved for use by paCMec functions and cannot be used as blog names: %s' ),
	...
	if ( isset( $_POST['MTLANG'] ) ) {
		if ( '' === $_POST['MTLANG'] ) {
			...
			$language = mt_download_language_pack( mt_unslash( $_POST['MTLANG'] ) );
			if ( $language ) {
				$meta['MTLANG'] = $language;
			}
		}
	}

	if ( empty( $domain ) ) {
		mt_die( __( 'Missing or invalid site address.' ) );
	}

	if ( isset( $blog['email'] ) && '' === trim( $blog['email'] ) ) {
		mt_die( __( 'Missing email address.' ) );
	}

	$email = sanitize_email( $blog['email'] );
	if ( ! is_email( $email ) ) {
		mt_die( __( 'Invalid email address.' ) );
	}
	...
	if ( ! $user_id ) { // Create a new user with a random password.
		...
		if ( $user_id ) {
			mt_die( __( 'The domain or path entered conflicts with an existing username.' ) );
		}
		$password = mt_generate_password( 12, false );
		$user_id  = mtmu_create_user( $domain, $password, $email );
		if ( false === $user_id ) {
			mt_die( __( 'There was an error creating the user.' ) );
		}
	...
	$mtdb->hide_errors();
	$id = mtmu_create_blog( $newdomain, $path, $title, $user_id, $meta, get_current_network_id() );
	$mtdb->show_errors();
	...
	if ( ! is_mt_error( $id ) ) {
		...
		mtmu_new_site_admin_notification( $id, $user_id );
		mtmu_welcome_notification( $id, $user_id, $password, $title, array( 'public' => 1 ) );
		mt_redirect(
			add_query_arg(
				array(
					'update' => 'added',
...
<?php mt_nonce_field( 'add-blog', '_mtnonce_add-blog' ); ?>
	<table class="form-table" role="presentation">
		...
		<tr class="form-field form-required">
			...
			<td><input name="blog[email]" type="email" class="regular-text mt-suggest-user" id="admin-email" data-autocomplete-type="search" data-autocomplete-field="user_email" aria-describedby="site-admin-email" required /></td>
		</tr>
		<tr class="form-field">
