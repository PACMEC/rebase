...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/network-admin-settings-screen/">Documentation on Network Settings</a>' ) . '</p>' .
...
if ( $_POST ) {
	...
	if ( ! empty( $_POST['MTLANG'] ) && current_user_can( 'install_languages' ) && mt_can_install_language_pack() ) {
	...
	foreach ( $options as $option_name ) {
		...
		$value = mt_unslash( $_POST[ $option_name ] );
	 ...
	 * @since MU (3.0.0)
	 */
	do_action( 'update_mtmu_options' );

	mt_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit;
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php mt_nonce_field( 'siteoptions' ); ?>
				...
				<td>
					...
					if ( $new_admin_email && get_site_option( 'admin_email' ) !== $new_admin_email ) :
						...
						<?php
							...
							printf(
								' <a href="%1$s">%2$s</a>',
								esc_url( mt_nonce_url( network_admin_url( 'settings.php?dismiss=new_network_admin_email' ), 'dismiss_new_network_admin_email' ) ),
								__( 'Cancel' )
							);
						?>
						</p>
			...
			<tr>
				...
				<td>
					...
					if ( is_subdomain_install() ) {
						...
						printf(
							/* translators: 1: NOBLOGREDIRECT, 2: mt-config.php */
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ); ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table" role="presentation">
				<tr>
					<th><label for="MTLANG"><?php _e( 'Default Language' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
					<td>
						<?php
