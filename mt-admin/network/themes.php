...
if ( ! current_user_can( 'manage_network_themes' ) ) {
	mt_die( __( 'Sorry, you are not allowed to manage network themes.' ) );
}
...
$referer                = remove_query_arg( $temp_args, mt_get_referer() );

if ( $action ) {
	switch ( $action ) {
		case 'enable':
			check_admin_referer( 'enable-theme_' . $_GET['theme'] );
			MT_Theme::network_enable_theme( $_GET['theme'] );
			if ( false === strpos( $referer, '/network/themes.php' ) ) {
				mt_redirect( network_admin_url( 'themes.php?enabled=1' ) );
			} else {
				mt_safe_redirect( add_query_arg( 'enabled', 1, $referer ) );
			}
			exit;
		case 'disable':
			check_admin_referer( 'disable-theme_' . $_GET['theme'] );
			MT_Theme::network_disable_theme( $_GET['theme'] );
			mt_safe_redirect( add_query_arg( 'disabled', '1', $referer ) );
			exit;
		case 'enable-selected':
			check_admin_referer( 'bulk-themes' );
			$themes = isset( $_POST['checked'] ) ? (array) $_POST['checked'] : array();
			if ( empty( $themes ) ) {
				mt_safe_redirect( add_query_arg( 'error', 'none', $referer ) );
				exit;
			}
			MT_Theme::network_enable_theme( (array) $themes );
			mt_safe_redirect( add_query_arg( 'enabled', count( $themes ), $referer ) );
			exit;
		case 'disable-selected':
			...
			MT_Theme::network_disable_theme( (array) $themes );
			mt_safe_redirect( add_query_arg( 'disabled', count( $themes ), $referer ) );
			exit;
		...
		case 'delete-selected':
			if ( ! current_user_can( 'delete_themes' ) ) {
				mt_die( __( 'Sorry, you are not allowed to delete themes for this site.' ) );
			}
			...
			if ( empty( $themes ) ) {
				mt_safe_redirect( add_query_arg( 'error', 'main', $referer ) );
				exit;
			}

			$theme_info = array();
			foreach ( $themes as $key => $theme ) {
				$theme_info[ $theme ] = mt_get_theme( $theme );
			}
		...
		case 'disable-auto-update-selected':
			if ( ! ( current_user_can( 'update_themes' ) && mt_is_auto_update_enabled_for_type( 'theme' ) ) ) {
				mt_die( __( 'Sorry, you are not allowed to change themes automatic update settings.' ) );
			}
			...
			if ( 'enable-auto-update' === $action ) {
				...
				// Bulk enable/disable.
				$themes = (array) mt_unslash( $_POST['checked'] );
		...
		default:
			...
			/** This action is documented in mt-admin/network/site-themes.php */
			$referer = apply_filters( 'handle_network_bulk_actions-' . get_current_screen()->id, $referer, $action, $themes ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.managertechnology.com.co/pacmec/Network_Admin_Themes_Screen">Documentation on Network Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
mt_enqueue_script( 'theme-preview' );
...
<form method="get">
<?php $mt_list_table->search_box( __( 'Search Installed Themes' ), 'theme' ); ?>
</form>
