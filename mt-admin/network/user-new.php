...
if ( ! current_user_can( 'create_users' ) ) {
	mt_die( __( 'Sorry, you are not allowed to add users to this network.' ) );
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.managertechnology.com.co/pacmec/Network_Admin_Users_Screen">Documentation on Network Users</a>' ) . '</p>' .
...
if ( isset( $_REQUEST['action'] ) && 'add-user' === $_REQUEST['action'] ) {
	...
	if ( ! is_array( $_POST['user'] ) ) {
		mt_die( __( 'Cannot create an empty user.' ) );
	}

	$user = mt_unslash( $_POST['user'] );

	$user_details = mtmu_validate_user_signup( $user['username'], $user['email'] );

	if ( is_mt_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
		$add_user_errors = $user_details['errors'];
	} else {
		$password = mt_generate_password( 12, false );
		$user_id  = mtmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, sanitize_email( $user['email'] ) );

		if ( ! $user_id ) {
			$add_user_errors = new MT_Error( 'add_user_fail', __( 'Cannot add user.' ) );
		} else {
			/**
...
if ( isset( $add_user_errors ) && is_mt_error( $add_user_errors ) ) {
	?>
	<div class="error">
		<?php
...
<?php } ?>
	...
	mt_nonce_field( 'add-user', '_mtnonce_add-user' );
