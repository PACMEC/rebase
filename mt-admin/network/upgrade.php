...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( 'Overview' ),
		'content' =>
			'<p>' . __( 'Only use this screen once you have updated to a new version of paCMec through Updates/Available Updates (via the Network Administration navigation menu or the Toolbar). Clicking the Upgrade Network button will step through each site in the network, five at a time, and make sure any database updates are applied.' ) . '</p>' .
...
switch ( $action ) {
	case 'upgrade':
		...
		if ( $n < 5 ) {
			...
			update_site_option( 'mtmu_upgrade_site', $mt_db_version );
		}
		...
		foreach ( (array) $site_ids as $site_id ) {
			 ...
			 * @since MU (3.0.0)
			 *
			 * @param array|MT_Error $response The upgrade response array or MT_Error on failure.
			 */
			 ...
			 */
			do_action( 'mtmu_upgrade_site', $site_id );
		}
		echo '</ul>';
	...
	case 'show':
	default:
		if ( (int) get_site_option( 'mtmu_upgrade_site' ) !== $GLOBALS['mt_db_version'] ) :
			?>
		<h2><?php _e( 'Database Update Required' ); ?></h2>
		<p><?php _e( 'paCMec has been updated! Before we send you on your way, we need to individually upgrade the sites in your network.' ); ?></p>
		<?php endif; ?>
