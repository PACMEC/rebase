 ...
 *
 * Requires mt-admin/includes/revision.php.
 *
 * @package paCMec
 ...
 */
mt_reset_vars( array( 'revision', 'action', 'from', 'to' ) );
...
switch ( $action ) {
	case 'restore':
		...
		if ( ! mt_revisions_enabled( $post ) && ! mt_is_post_autosave( $revision ) ) {
		 ...
		 * Because mt_insert_post() and mt_transition_post_status() are called
		 * during the process, plugins can unexpectedly modify $post.
		 */
		$backup_global_post = clone $post;

		mt_restore_post_revision( $revision->ID );
...
mt_enqueue_script( 'revisions' );
mt_localize_script( 'revisions', '_mtRevisionsSettings', mt_prepare_revisions_for_js( $post, $revision_id, $from ) );
...
$revisions_sidebar .= '<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/revisions/">Revisions Management</a>' ) . '</p>';
$revisions_sidebar .= '<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>';
