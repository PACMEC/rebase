...
if ( 'attachment' === $typenow ) {
	if ( mt_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
$mt_list_table = _get_list_table( 'MT_Posts_List_Table' );
$pagenum       = $mt_list_table->get_pagenum();

// Back-compat for viewing comments of an entry.
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		mt_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'locked', 'ids' ), mt_get_referer() );
	if ( ! $sendback ) {
	...
	if ( 'delete_all' === $doaction ) {
		...
		if ( get_post_status_object( $post_status ) ) {
			$post_ids = $mtdb->get_col( $mtdb->prepare( "SELECT ID FROM $mtdb->posts WHERE post_type=%s AND post_status = %s", $post_type, $post_status ) );
		}
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		...
		case 'untrash':
			...
			if ( isset( $_GET['doaction'] ) && ( 'undo' === $_GET['doaction'] ) ) {
				add_filter( 'mt_untrash_post_status', 'mt_untrash_post_set_previous_status', 10, 3 );
			}
			...
			remove_filter( 'mt_untrash_post_status', 'mt_untrash_post_set_previous_status', 10, 3 );
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				if ( 'attachment' === $post_del->post_type ) {
					if ( ! mt_delete_attachment( $post_id ) ) {
						mt_die( __( 'Error in deleting the attachment.' ) );
					}
				} else {
					if ( ! mt_delete_post( $post_id ) ) {
		...
		default:
			 ...
			 */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $doaction, $post_ids ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
			break;
	}
	...
	mt_redirect( $sendback );
	exit;
} elseif ( ! empty( $_REQUEST['_mt_http_referer'] ) ) {
...
mt_enqueue_script( 'inline-edit-post' );
mt_enqueue_script( 'heartbeat' );

if ( 'mt_block' === $post_type ) {
	mt_enqueue_script( 'mt-list-reusable-blocks' );
	mt_enqueue_style( 'mt-list-reusable-blocks' );
}
...
if ( 'post' === $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/posts-screen/">Documentation on Managing Posts</a>' ) . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/pages-screen/">Documentation on Managing Pages</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/">Support</a>' ) . '</p>'
	);
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( 'trashed' === $message && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = '<a href="' . esc_url( mt_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", 'bulk-posts' ) ) . '">' . __( 'Undo' ) . '</a>';
	}
...
<?php $mt_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
