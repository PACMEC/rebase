...
if ( is_multisite() ) {
	add_filter( 'mtmu_signup_user_notification_email', 'admin_created_user_email' );
}

if ( isset( $_REQUEST['action'] ) && 'adduser' === $_REQUEST['action'] ) {
	check_admin_referer( 'add-user', '_mtnonce_add-user' );

	$user_details = null;
	$user_email   = mt_unslash( $_REQUEST['email'] );
	if ( false !== strpos( $user_email, '@' ) ) {
		...
		if ( current_user_can( 'manage_network_users' ) ) {
			...
			mt_redirect( add_query_arg( array( 'update' => 'enter_email' ), 'user-new.php' ) );
			die();
		}
	}

	if ( ! $user_details ) {
		mt_redirect( add_query_arg( array( 'update' => 'does_not_exist' ), 'user-new.php' ) );
		die();
	}
	...
	if ( null != $username && array_key_exists( $blog_id, get_blogs_of_user( $user_id ) ) ) {
		...
		if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
			...
			$newuser_key = mt_generate_password( 20, false );
			add_option(
...
%4$s'
			...
			$new_user_email['subject'] = sprintf(
				...
				mt_specialchars_decode( get_option( 'blogname' ) )
			);
			$new_user_email['message'] = sprintf(
				$message,
				get_option( 'blogname' ),
				home_url(),
				mt_specialchars_decode( translate_user_role( $role['name'] ) ),
	...
	check_admin_referer( 'create-user', '_mtnonce_create-user' );
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = mt_unslash( $_REQUEST['email'] );
		$user_details   = mtmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );
		if ( is_mt_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
			$add_user_errors = $user_details['errors'];
		} else {
			/** This filter is documented in mt-includes/user.php */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( mt_unslash( $_REQUEST['user_login'] ), true ) );
			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
				add_filter( 'mtmu_signup_user_notification', '__return_false' );  // Disable confirmation email.
				add_filter( 'mtmu_welcome_user_notification', '__return_false' ); // Disable welcome email.
			}
			mtmu_signup_user(
				$new_user_login,
				$new_user_email,
			...
			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
				$key      = $mtdb->get_var( $mtdb->prepare( "SELECT activation_key FROM {$mtdb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $new_user_email ) );
				$new_user = mtmu_activate_signup( $key );
				if ( is_mt_error( $new_user ) ) {
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/users-add-new-screen/">Documentation on Adding New Users</a>' ) . '</p>' .
...
if ( is_multisite() && current_user_can( 'promote_users' ) && ! mt_is_large_network( 'users' )
	...
	mt_enqueue_script( 'user-suggest' );
}
...
if ( isset( $_GET['update'] ) ) {
	...
	if ( is_multisite() ) {
		...
		if ( ( isset( $_GET['user_id'] ) ) ) {
			...
			if ( $user_id_new ) {
				$edit_link = esc_url( add_query_arg( 'mt_http_referer', urlencode( mt_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_id_new ) ) );
			}
		}
		...
		switch ( $_GET['update'] ) {
			...
			case 'addexisting':
				...
				break;
			case 'could_not_add':
				$add_user_errors = new MT_Error( 'could_not_add', __( 'That user could not be added to this site.' ) );
				break;
			case 'created_could_not_add':
				$add_user_errors = new MT_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) );
				break;
			case 'does_not_exist':
				$add_user_errors = new MT_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );
				break;
			case 'enter_email':
				$add_user_errors = new MT_Error( 'enter_email', __( 'Please enter a valid email address.' ) );
				break;
		}
	} else {
		if ( 'add' === $_GET['update'] ) {
...
<?php if ( isset( $add_user_errors ) && is_mt_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<input name="action" type="hidden" value="adduser" />
	<?php mt_nonce_field( 'add-user', '_mtnonce_add-user' ); ?>

<table class="form-table" role="presentation">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo $label; ?></label></th>
		<td><input name="email" type="<?php echo $type; ?>" id="adduser-email" class="mt-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e( 'Role' ); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php mt_dropdown_roles( get_option( 'default_role' ) ); ?>
			</select>
		</td>
	</tr>
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"
	<?php
	/** This action is documented in mt-admin/user-new.php */
	do_action( 'user_new_form_tag' );
	?>
>
<input name="action" type="hidden" value="createuser" />
	<?php mt_nonce_field( 'create-user', '_mtnonce_create-user' ); ?>
	<?php
	// Load up the passed data, else set to a default.
	$creating = isset( $_POST['createuser'] );

	$new_user_login             = $creating && isset( $_POST['user_login'] ) ? mt_unslash( $_POST['user_login'] ) : '';
	$new_user_firstname         = $creating && isset( $_POST['first_name'] ) ? mt_unslash( $_POST['first_name'] ) : '';
	$new_user_lastname          = $creating && isset( $_POST['last_name'] ) ? mt_unslash( $_POST['last_name'] ) : '';
	$new_user_email             = $creating && isset( $_POST['email'] ) ? mt_unslash( $_POST['email'] ) : '';
	$new_user_uri               = $creating && isset( $_POST['url'] ) ? mt_unslash( $_POST['url'] ) : '';
	$new_user_role              = $creating && isset( $_POST['role'] ) ? mt_unslash( $_POST['role'] ) : '';
	$new_user_send_notification = $creating && ! isset( $_POST['send_user_notification'] ) ? false : true;
	$new_user_ignore_pass       = $creating && isset( $_POST['noconfirmation'] ) ? mt_unslash( $_POST['noconfirmation'] ) : '';
...
<table class="form-table" role="presentation">
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button mt-generate-pw hide-if-no-js"><?php _e( 'Generate password' ); ?></button>
			<div class="mt-pwd">
				<?php $initial_password = mt_generate_password( 24 ); ?>
	...
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			mt_dropdown_roles( $new_user_role );
			?>
			</select>
		</td>
	</tr>
