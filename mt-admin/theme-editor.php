...
if ( is_multisite() && ! is_network_admin() ) {
	mt_redirect( network_admin_url( 'theme-editor.php' ) );
	exit;
}

if ( ! current_user_can( 'edit_themes' ) ) {
	mt_die( '<p>' . __( 'Sorry, you are not allowed to edit templates for this site.' ) . '</p>' );
}
...
if ( ! $theme->exists() ) {
	mt_die( __( 'The requested theme does not exist.' ) );
}

if ( $theme->errors() && 'theme_no_stylesheet' === $theme->errors()->get_error_code() ) {
	mt_die( __( 'The requested theme does not exist.' ) . ' ' . $theme->errors()->get_error_message() );
}

$allowed_files = array();
$style_files   = array();

$file_types = mt_get_theme_file_editable_extensions( $theme );
...
$settings = array(
	'codeEditor' => mt_enqueue_code_editor( compact( 'file' ) ),
);
...
mt_add_inline_script( 'mt-theme-plugin-editor', 'mt.themePluginEditor.themeOrPlugin = "theme";' );
...
<div class="alignright">
	<form action="theme-editor.php" method="get">
		...
		<?php
		foreach ( mt_get_themes( array( 'errors' => null ) ) as $a_stylesheet => $a_theme ) {
...
<div id="templateside">
	...
	<ul role="tree" aria-labelledby="theme-files-label">
		...
		<li role="treeitem" tabindex="-1" aria-expanded="true" aria-level="1" aria-posinset="1" aria-setsize="1">
			<ul role="group">
				<?php mt_print_theme_file_tree( mt_make_theme_file_tree( $allowed_files ) ); ?>
			</ul>
		</li>
	</ul>
</div>
...
if ( $error ) :
	...
	<form name="template" id="template" action="theme-editor.php" method="post">
		<?php mt_nonce_field( 'edit-theme_' . $stylesheet . '_' . $relative_file, 'nonce' ); ?>
		<div>
...
if ( ! in_array( 'theme_editor_notice', $dismissed_pointers, true ) ) :
	...
	$excluded_referer_basenames = array( 'theme-editor.php', 'mt-login.php' );
		...
		<div class="notification-dialog">
			<div class="file-editor-warning-content">
				<div class="file-editor-warning-message">
					...
					<p>
						<?php
						_e( 'You appear to be making direct edits to your theme in the paCMec dashboard. We recommend that you don&#8217;t! Editing your theme directly could break your site and your changes may be lost in future updates.' );
						?>
					</p>
						<?php
						if ( ! $theme->parent() ) {
