...
if ( $doaction ) {
	...
	if ( 'delete_all' === $doaction && ! empty( $_REQUEST['pagegen_timestamp'] ) ) {
		$comment_status = mt_unslash( $_REQUEST['comment_status'] );
		$delete_time    = mt_unslash( $_REQUEST['pagegen_timestamp'] );
		$comment_ids    = $mtdb->get_col( $mtdb->prepare( "SELECT comment_ID FROM $mtdb->comments WHERE comment_approved = %s AND %s > comment_date_gmt", $comment_status, $delete_time ) );
		$doaction       = 'delete';
	...
	$redirect_to = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'spammed', 'unspammed', 'approved', 'unapproved', 'ids' ), mt_get_referer() );
	...
	foreach ( $comment_ids as $comment_id ) { // Check the permissions on each.
		...
		switch ( $doaction ) {
			case 'approve':
				mt_set_comment_status( $comment_id, 'approve' );
				$approved++;
				break;
			case 'unapprove':
				mt_set_comment_status( $comment_id, 'hold' );
				$unapproved++;
				break;
			case 'spam':
				mt_spam_comment( $comment_id );
				$spammed++;
				break;
			case 'unspam':
				mt_unspam_comment( $comment_id );
				$unspammed++;
				break;
			case 'trash':
				mt_trash_comment( $comment_id );
				$trashed++;
				break;
			case 'untrash':
				mt_untrash_comment( $comment_id );
				$untrashed++;
				break;
			case 'delete':
				mt_delete_comment( $comment_id );
				$deleted++;
				break;
		}
	}
	...
	if ( ! in_array( $doaction, array( 'approve', 'unapprove', 'spam', 'unspam', 'trash', 'delete' ), true ) ) {
		...
		$redirect_to = apply_filters( "handle_bulk_actions-{$screen}", $redirect_to, $doaction, $comment_ids ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
	}
...
if ( $post_id ) {
	$comments_count      = mt_count_comments( $post_id );
	$draft_or_post_title = mt_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' );
	...
	$comments_count = mt_count_comments();
	if ( $comments_count->moderated > 0 ) {
		$title = sprintf(
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/comment-spam/">Documentation on Comment Spam</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://managertechnology.com.co/pacmec/support/article/keyboard-shortcuts/">Documentation on Keyboard Shortcuts</a>' ) . '</p>' .
...
if ( $post_id ) {
	printf(
		...
		sprintf(
			...
			mt_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' )
		)
	);
} else {
	_e( 'Comments' );
...
if ( isset( $_REQUEST['s'] ) && strlen( $_REQUEST['s'] ) ) {
	...
	printf(
		...
		'<strong>' . mt_html_excerpt( esc_html( mt_unslash( $_REQUEST['s'] ) ), 50, '&hellip;' ) . '</strong>'
	);
	echo '</span>';
}
?>
...
if ( isset( $_REQUEST['approved'] ) || isset( $_REQUEST['deleted'] ) || isset( $_REQUEST['trashed'] ) || isset( $_REQUEST['untrashed'] ) || isset( $_REQUEST['spammed'] ) || isset( $_REQUEST['unspammed'] ) || isset( $_REQUEST['same'] ) ) {
	...
	if ( $approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0 ) {
		...
		if ( $spammed > 0 ) {
			...
			/* translators: %s: Number of comments. */
			$messages[] = sprintf( _n( '%s comment marked as spam.', '%s comments marked as spam.', $spammed ), $spammed ) . ' <a href="' . esc_url( mt_nonce_url( "edit-comments.php?doaction=undo&action=unspam&ids=$ids", 'bulk-comments' ) ) . '">' . __( 'Undo' ) . '</a><br />';
		}
		...
		if ( $trashed > 0 ) {
			$ids = isset( $_REQUEST['ids'] ) ? $_REQUEST['ids'] : 0;
			/* translators: %s: Number of comments. */
			$messages[] = sprintf( _n( '%s comment moved to the Trash.', '%s comments moved to the Trash.', $trashed ), $trashed ) . ' <a href="' . esc_url( mt_nonce_url( "edit-comments.php?doaction=undo&action=untrash&ids=$ids", 'bulk-comments' ) ) . '">' . __( 'Undo' ) . '</a><br />';
		}
...
<?php $mt_list_table->search_box( __( 'Search Comments' ), 'comment' ); ?>
...
<input type="hidden" name="_total" value="<?php echo esc_attr( $mt_list_table->get_pagination_arg( 'total_items' ) ); ?>" />
<input type="hidden" name="_per_page" value="<?php echo esc_attr( $mt_list_table->get_pagination_arg( 'per_page' ) ); ?>" />
<input type="hidden" name="_page" value="<?php echo esc_attr( $mt_list_table->get_pagination_arg( 'page' ) ); ?>" />
...
<?php
mt_comment_reply( '-1', true, 'detail' );
