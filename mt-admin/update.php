...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'update-selected' === $action ) {
		if ( ! current_user_can( 'update_plugins' ) ) {
			mt_die( __( 'Sorry, you are not allowed to update plugins for this site.' ) );
		}
		...
		if ( ! isset( $_GET['failure'] ) && ! isset( $_GET['success'] ) ) {
			mt_redirect( admin_url( 'update.php?action=activate-plugin&failure=true&plugin=' . urlencode( $plugin ) . '&_mtnonce=' . $_GET['_mtnonce'] ) );
			activate_plugin( $plugin, '', ! empty( $_GET['networkwide'] ), true );
			mt_redirect( admin_url( 'update.php?action=activate-plugin&success=true&plugin=' . urlencode( $plugin ) . '&_mtnonce=' . $_GET['_mtnonce'] ) );
			die();
		}
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			mt_die( __( 'Sorry, you are not allowed to install plugins on this site.' ) );
		}

		include_once ABSPATH . 'mt-admin/includes/plugin-install.php'; // For plugins_api().
		...
		// Make sure the attachment still exists, or File_Upload_Upgrader will call mt_die()
		...
		mt_redirect( self_admin_url( 'plugin-install.php' ) );
		exit;
	} elseif ( 'upgrade-theme' === $action ) {

		if ( ! current_user_can( 'update_themes' ) ) {
			mt_die( __( 'Sorry, you are not allowed to update themes for this site.' ) );
		}
		...
		include_once ABSPATH . 'mt-admin/includes/class-mt-upgrader.php'; // For themes_api().
		...
		mt_redirect( self_admin_url( 'theme-install.php' ) );
		exit;
	} else {
		/**
		 ...
		 * provided in the request for mt-admin/update.php. Can be used to
		 * provide custom update functionality for themes and plugins.
		 *
		 * @since 2.8.0
		 */
		do_action( "update-custom_{$action}" ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
	}
}
