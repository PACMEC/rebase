 ...
 *
 * @link https://developer.pacmec.org/themes/basics/theme-functions/
 */

/**
 * TownHub only works in Pacmec 5.0 or later.
 */
if ( version_compare( $GLOBALS['mt_version'], '5.0', '<' ) ) {
...
function townhub_setup() {
	 ...
	 *
	 * @link https://developer.pacmec.org/themes/functionality/featured-images-post-thumbnails/
	 */
	...
	// This theme uses mt_nav_menu() in two locations.
	 ...
	 *
	 * See: https://codex.pacmec.org/Post_Formats
	 */
...
function townhub_resource_hints( $urls, $relation_type ) {
	if ( mt_style_is( 'townhub-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
add_filter( 'mt_resource_hints', 'townhub_resource_hints', 10, 2 );

/**
 * Register widget area.
 *
 * @link https://developer.pacmec.org/themes/functionality/sidebars/#registering-a-sidebar
 */
...
add_action( 'mt_head', 'townhub_pingback_header' );
...
function townhub_scripts() {
	...
	mt_enqueue_style( 'townhub-fonts', townhub_fonts_url(), array(), null );


	mt_enqueue_style( 'townhub-plugins', get_theme_file_uri( '/assets/css/plugins.css' ), array(  ), null );
    mt_style_add_data( 'townhub-plugins', 'rtl', 'replace' );
	// Theme stylesheet.
	mt_enqueue_style( 'townhub-style', get_stylesheet_uri() );
    mt_style_add_data( 'townhub-style', 'rtl', 'replace' );

    if( townhub_get_option('header_height', 80) != '80' ){
        mt_add_inline_style( 'townhub-style', townhub_headerstyle() );
    }
    

	mt_enqueue_style( 'townhub-color', get_theme_file_uri( '/assets/css/color.min.css' ), array(  ), null );
    ...
    if( townhub_get_option('use_custom_color', false) ){
        mt_add_inline_style( 'townhub-color', townhub_overridestyle() );
    }
    ...
    if (!empty($inline_custom_style)) {
        mt_add_inline_style('townhub-color', townhub_stripWhitespace($inline_custom_style) );
    }


    mt_enqueue_script("jquery-easing", get_theme_file_uri( '/assets/js/jquery.easing.min.js' ), array('jquery'), '1.4.0', true);
    mt_enqueue_script("jquery-appear", get_theme_file_uri( '/assets/js/jquery.appear.js' ) , array(), '0.3.6', true);
    mt_enqueue_script("scrollax", get_theme_file_uri( '/assets/js/Scrollax.js' ) , '1.0.0', true);
    mt_enqueue_script("jquery-countto", get_theme_file_uri( '/assets/js/jquery.countTo.js' ) , array(), null, true);
    mt_enqueue_script("single-page-nav", get_theme_file_uri( '/assets/js/navigation.js' ) , array(), null, true);


	mt_enqueue_script( 'townhub-scripts', get_theme_file_uri( '/assets/js/scripts.js' ), array( 'jquery', 'imagesloaded' ), null , true );
    mt_localize_script('townhub-scripts', '_townhub', array(
...
add_action( 'mt_enqueue_scripts', 'townhub_scripts' );
 ...
 *
 * This function is hooked into `tgmpa_register`, which is fired on the MT `init` action on priority 10.
 */
function townhub_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array('name' => esc_html__('Redux Framework','townhub'),
            ...
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/redux-framework/' ),
        ...
        array(
            ...
            'required' => true,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/elementor/' ),
             // If set, overrides default API URL and points to an external URL.
        ...
        array(
            ...
            'required' => true,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/contact-form-7/' ),
             // If set, overrides default API URL and points to an external URL.

            'function_to_check'         => 'mtcf7',
            'class_to_check'            => 'MTCF7'
        ), 
        ...
        array(
            ...
            'required' => true,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/support/plugin/cmb2'),
             // If set, overrides default API URL and points to an external URL.
        ...
        array(
            ...
            'required' => false,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url('https://pacmec.org/plugins/woocommerce/' ),
             // If set, overrides default API URL and points to an external URL.
        ...
        array(
            ...
            'required' => false,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/loco-translate/'),
             // If set, overrides default API URL and points to an external URL.
        ...
        array('name' => esc_html__('One Click Demo Import','townhub'),
            ...
            'required' => false,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/one-click-demo-import/'),
             // If set, overrides default API URL and points to an external URL.
        ...
        array('name' => esc_html__('Regenerate Thumbnails','townhub'),
            ...
            'required' => false,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/regenerate-thumbnails/' ),
             // If set, overrides default API URL and points to an external URL.
        ...
        array('name' => esc_html__('MT Facebook Login for Pacmec','townhub'),
             // The plugin name.
            'slug' => 'mt-facebook-login',
             // The plugin slug (typically the folder name).
            'required' => false,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/mt-facebook-login/' ),
             // If set, overrides default API URL and points to an external URL.
        ...
        array('name' => esc_html__('Pacmec Social Login (Facebook, Google, Twitter)','townhub'),
             // The plugin name.
            'slug' => 'miniorange-login-openid',
             // The plugin slug (typically the folder name).
            'required' => false,
             // If false, the plugin is only 'recommended' instead of required.
            'external_url' => esc_url(townhub_relative_protocol_url().'://pacmec.org/plugins/miniorange-login-openid/' ),
             // If set, overrides default API URL and points to an external URL.
