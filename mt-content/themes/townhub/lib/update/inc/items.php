...
class Items
{
     ...
     * @var array
     */
    private static $mt_plugins = array();
    ...
    /**
     * Get the list of Pacmec plugins
     *
     *
     * @param bool $flush Forces a cache flush. Default is 'false'.
     * @return array
     */
    public function mt_plugins( $flush = false ) {
        if ( empty( self::$mt_plugins ) || true === $flush ) {
            mt_cache_flush();
            self::$mt_plugins = get_plugins();
        }

        return self::$mt_plugins;
    }


    /**
     * Disables requests to the mt.org repository for premium themes.
     *
     *
    ...
    public function update_check( $request, $url ) {
        ...
        // string(97) "http://update.cththemes.net/mt-json/cthupdate/v1/themes/171.224.179.183||kak-v-dubae.ru||25019571" 
        // string(50) "https://api.pacmec.org/themes/update-check/1.1/" 
        // string(97) "http://update.cththemes.net/mt-json/cthupdate/v1/themes/171.224.179.183||kak-v-dubae.ru||25019571" 
        // string(180) "https://api.pacmec.org/core/version-check/1.7/?version=5.5.3&php=7.3.6&locale=en_US&mysql=5.7.27&local_package=ru_RU&blogs=1&users=1&multisite_enabled=0&initial_db_version=47018" 

        // Theme update request.
        if ( false !== strpos( $url, '//api.pacmec.org/themes/update-check/1.1/' ) ) {

            /**
             * Excluded theme slugs that should never ping the Pacmec API.
            ...
            $request['body']['themes'] = mt_json_encode( $data );
        }

        // Plugin update request.
        if ( false !== strpos( $url, '//api.pacmec.org/plugins/update-check/1.1/' ) ) {
            ...
            // Encode back into JSON and update the response.
            $request['body']['plugins'] = mt_json_encode( $data );
        }
    ...
    public function normalize( $string ) {
        return strtolower( html_entity_decode( mt_strip_all_tags( $string ) ) );
    }
    ...
    private function process_themes( $purchased ) {
        if ( is_mt_error( $purchased ) ) {
            $purchased = array();
        }

        $current   = mt_get_theme()->get_template();
        $active    = array();
        $installed = array();
        $install   = $purchased;

        if ( ! empty( $purchased ) ) {
            foreach ( mt_get_themes() as $theme ) {
    ...
    private function process_plugins( $purchased, $args = array() ) {
        ...
        if ( ! empty( $purchased ) ) {
            foreach ( self::mt_plugins( true ) as $slug => $plugin ) {
                foreach ( $install as $key => $value ) {
                    if ( $this->normalize( $value['name'] ) === $this->normalize( $plugin['Name'] ) && $this->normalize( $value['author'] ) === $this->normalize( $plugin['Author'] ) && file_exists( MT_PLUGIN_DIR . '/' . $slug ) ) {
