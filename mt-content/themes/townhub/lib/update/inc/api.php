...
class Api
{
    ...
    {
        $this->api_url = 'http://update.cththemes.net/mt-json/cthupdate/v1';
        // Envato API token.
    ...
    public function request($url, $args = array())
    {
        $defaults = array(
            ...
            'headers' => array(
                ...
                'User-Agent'    => 'CTHthemes - MT Update ' . cththemes_auto_update()->get_version(),
            ),
            'timeout' => 14,
        );
        ...
        if (empty($token)) {
            return new \MT_Error('api_token_error', __('An API token is required.', 'townhub'));
        }

        // Make an API request.
        $response = mt_remote_get(esc_url_raw($url), $args);
        // error example
        // string(130) "{"headers":{"Authorization":"Basic purchase-code","User-Agent":"CTHthemes - MT Update 1.0.0"},"timeout":14}" string(115) "{"errors":{"http_request_failed":["cURL error 28: Connection timed out after 10000 milliseconds"]},"error_data":[]}" string(130) "{"headers":{"Authorization":"Basic purchase-code","User-Agent":"CTHthemes - MT Update 1.0.0"},"timeout":14}" string(115) "{"errors":{"http_request_failed":["cURL error 28: Connection timed out after 10000 milliseconds"]},"error_data":[]}" array(4) { ["purchased"]=> array(0) { } ["active"]=> array(0) { } ["installed"]=> array(0) { } ["install"]=> array(0) { } } array(0) { } string(130) "{"headers":{"Authorization":"Basic purchase-code","User-Agent":"CTHthemes - MT Update 1.0.0"},"timeout":14}" string(115) "{"errors":{"http_request_failed":["cURL error 28: Connection timed out after 10000 milliseconds"]},"error_data":[]}" array(4) { ["purchased"]=> array(0) { } ["active"]=> array(0) { } ["installed"]=> array(0) { } ["install"]=> array(0) { } } array(0) { } string(130) "{"headers":{"Authorization":"Basic purchase-code","User-Agent":"CTHthemes - MT Update 1.0.0"},"timeout":14}" string(115) "{"errors":{"http_request_failed":["cURL error 28: Connection timed out after 10000 milliseconds"]},"error_data":[]}" array(4) { ["purchased"]=> array(0) { } ["active"]=> array(0) { } ["installed"]=> array(0) { } ["install"]=> array(0) { } } array(0) { } string(130) "{"headers":{"Authorization":"Basic purchase-code","User-Agent":"CTHthemes - MT Update 1.0.0"},"timeout":14}" string(115) "{"errors":{"http_request_failed":["cURL error 28: Connection timed out after 10001 milliseconds"]},"error_data":[]}" 

        // Check the response code.
        $response_code    = mt_remote_retrieve_response_code($response);
        $response_message = mt_remote_retrieve_response_message($response);
        ...
        if (!empty($response->errors) && isset($response->errors['http_request_failed'])) {
            ...
            return new \MT_Error('http_error', esc_html(current($response->errors['http_request_failed'])));
        }

        if (200 !== $response_code && !empty($response_message)) {
            return new \MT_Error($response_code, $response_message);
        } elseif (200 !== $response_code) {
            return new \MT_Error($response_code, __('An unknown API error occurred.', 'townhub'));
        } else {
            $return = json_decode(mt_remote_retrieve_body($response), true);
            if (null === $return) {
                return new \MT_Error('api_error', __('An unknown API error occurred.', 'townhub'));
            }
            return $return;
        }
    }
     ...
     * @param  array $args The arguments passed to `mt_remote_get`.
    ...
    public function download( $id, $type = 'update_theme', $args = array() ) {
        ...
        if ( is_mt_error( $response ) || empty( $response ) || ! empty( $response['error'] ) ) {
            return false;
        }
        ...
        // Missing a Pacmec theme and plugin, report an error.
        $option = cththemes_auto_update()->get_options();
    ...
    public function themes( $args = array() ) {
        ...
        if ( is_mt_error( $response ) || empty( $response ) || empty( $response['results'] ) ) {
            return $themes;
        }
