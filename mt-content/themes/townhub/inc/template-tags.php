...
if (!function_exists('townhub_post_nav')) {
    function townhub_post_nav() {
        ...
        if ( is_a( $prev_post, 'MT_Post' ) || is_a( $next_post, 'MT_Post' ) ) :
?>
<div class="post-nav-wrap fl-wrap">
    <?php
        if ( is_a( $prev_post, 'MT_Post' ) ) :
        ?>
...
if (!function_exists('townhub_pagination')) {
    function townhub_pagination(){
        ...
        the_posts_pagination( array(
            'prev_text' =>  mt_kses(__('<i class="fas fa-caret-left"></i><span>Prev</span>','townhub'),array('span'=>array('class'=>array(),),'i'=>array('class'=>array(),),) ) ,
            'next_text' =>  mt_kses(__('<span>Next</span><i class="fas fa-caret-right"></i>','townhub'),array('span'=>array('class'=>array(),),'i'=>array('class'=>array(),),) ) ,
...
if (!function_exists('townhub_custom_pagination')) {
    function townhub_custom_pagination($pages = '', $range = 2, $current_query = '', $sec_wrap = false) {
        ...
        if (1 < $pages) {
            ...
            if ($paged > 1) echo '<a href="' . get_pagenum_link($paged - 1) . '" class="prevposts-link">'.mt_kses(__('<i class="fa fa-chevron-left"></i>','townhub'),array('i'=>array('class'=>array(),),) ).'</a>';
            for ($i = 1; $i <= $pages; $i++) {
            ...
            if ($paged < $pages) echo '<a href="' . get_pagenum_link($paged + 1) . '" class="nextposts-link">'.mt_kses(__('<i class="fa fa-chevron-right"></i>','townhub'),array('i'=>array('class'=>array(),),) ).'</a>';
            echo '</div>';
        }
        ...
        if ( is_archive() && !is_tax() ) {
            ...
            if(empty($last_category) && !empty($custom_taxonomy)) {
                ...
                foreach ($custom_taxonomy_arr as $key => $custom_taxonomy_val) {
                    ...
                    if($taxonomy_terms && !($taxonomy_terms instanceof MT_Error) ){
 ...
 * https://codex.pacmec.org/Function_Reference/get_shortcode_regex
 */ 
