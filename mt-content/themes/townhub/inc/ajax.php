 ...
 *
 * Prevents TownHub from running on Pacmec versions prior to 4.7,
...
add_action('mt_ajax_nopriv_townhub_lm_folio', 'townhub_lm_folio_callback');
add_action('mt_ajax_townhub_lm_folio', 'townhub_lm_folio_callback');

function townhub_lm_folio_callback(){

    $output = array();
    $output['status'] = 'fail';
    if ( ! isset( $_POST['_lmnonce'] ) || ! mt_verify_nonce( $_POST['_lmnonce'], 'townhub_lm_folio' ) ) {
        ...
        $args = mt_parse_args( $_POST['mt_query'], $default_args );
...
add_action('mt_ajax_nopriv_townhub_lm_gal', 'townhub_lm_gal_callback');
add_action('mt_ajax_townhub_lm_gal', 'townhub_lm_gal_callback');
...
function townhub_lm_gal_callback(){
    ...
    $output['content'] = '';
    $output['is_remaining'] = 'no';
    if ( ! isset( $_POST['_lmnonce'] ) || ! mt_verify_nonce( $_POST['_lmnonce'], 'townhub_lm_gal' ) ) {
        // This nonce is not valid.
        $output['content'] = esc_html__('Sorry, your nonce did not verify.','townhub' );
        $output['is_remaining'] = 'no';
    } else {
            ...
            foreach ($images as $key => $img) {
                ...
                <div class="gallery-item <?php echo esc_attr( $item_filter );?>">
                    <div class="grid-item-holder">
                        <?php echo mt_get_attachment_image( $img, 'townhub-folio-one'); ?>
                        <a href="<?php echo esc_url( mt_get_attachment_url($img ) );?>" class="popup-image slider-zoom"><i class="fa fa-search"></i></a>
                        ...
                        <div class="thumb-info">
                            ...
                            <?php endif;?>
                            <?php if($show_img_desc == 'yes') echo mt_kses_post( $at_des ); ?>
                        </div>
                        <?php endif;?>
                    </div>
                </div>
