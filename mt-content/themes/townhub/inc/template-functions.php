...
/**
 * Return attachment image link by using mt_get_attachment_image_src function
 *
 */
function townhub_get_attachment_thumb_link( $id, $size = 'thumbnail' ){
    $image_attributes = mt_get_attachment_image_src( $id, $size, false );
    if ( $image_attributes ) {
...
add_filter( 'mt_calculate_image_sizes', 'townhub_content_image_sizes_attr', 10, 2 );
...
if(!function_exists('townhub_get_template_part')){
     ...
     * @author CTHthemes 
     * @ref http://www.zmastaa.com/2015/02/06/php-2/pacmec-passing-variables-get_template_part
     * @ref http://keithdevon.com/passing-variables-to-get_template_part-in-pacmec/
     */
...
add_filter('mt_list_categories', 'townhub_custom_category_count_widget');
