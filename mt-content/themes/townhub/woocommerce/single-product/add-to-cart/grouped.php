	...
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table">
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				$post               = $post_object; // MTCS: override ok.
				setup_postdata( $post );
				...
				foreach ( $grouped_product_columns as $column_id ) {
						...
						case 'quantity':
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								woocommerce_quantity_input( array(
									...
									'input_value' => isset( $_POST['quantity'][ $grouped_product_child->get_id() ] ) ? wc_stock_amount( wc_clean( mt_unslash( $_POST['quantity'][ $grouped_product_child->get_id() ] ) ) ) : 0, // MTCS: CSRF ok, input var okay, sanitization ok.
					...
					echo '<td class="woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . '">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</td>'; // MTCS: XSS ok.
			...
			$post = $previous_post; // MTCS: override ok.
			setup_postdata( $post );
			?>
		</tbody>
	</table>
