...
class Featured_Content {
	...
	/**
	 * Conditionally hook into paCMec.
	 *
	 * Theme must declare that they support this module by adding
	 * add_theme_support( 'featured-content' ); during after_setup_theme.
	 *
	 * If no theme support is found there is no need to hook into paCMec.
	...
	public static function init() {
		...
		add_action( 'mt_loaded', array( __CLASS__, 'mt_loaded' ) );
	}

	/**
	 * Hide "featured" tag from the front end.
	 *
	 * Has to run on mt_loaded so that the preview filters of the Customizer
	 * have a chance to alter the value.
	 *
	 * @since Twenty Fourteen 1.0
	 */
	public static function mt_loaded() {
	 ...
	 * @since Twenty Fourteen 1.0
	 *
	 * @param MT_Query $query MT_Query object.
	 * @return MT_Query Possibly-modified MT_Query.
	 */
	 ...
	 */
	public static function customize_register( $mt_customize ) {
	...
	public static function enqueue_scripts() {
		mt_enqueue_script( 'featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20131205', true );
	}
	...
	public static function get_setting( $key = 'all' ) {
		...
		$options = mt_parse_args( $saved, $defaults );
	...
	public static function validate_settings( $input ) {
		...
		if ( empty( $input['tag-name'] ) ) {
			...
			if ( $term ) {
				...
				$new_tag = mt_create_tag( $input['tag-name'] );

				if ( ! is_mt_error( $new_tag ) && isset( $new_tag['term_id'] ) ) {
