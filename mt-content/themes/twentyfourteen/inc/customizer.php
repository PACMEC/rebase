 ...
 */
function twentyfourteen_customize_register( $mt_customize ) {
	...
	$mt_customize->get_control( 'header_textcolor' )->label = __( 'Site Title Color', 'twentyfourteen' );

	// Rename the label to "Display Site Title & Tagline" in order to make this option extra clear.
	$mt_customize->get_control( 'display_header_text' )->label = __( 'Display Site Title &amp; Tagline', 'twentyfourteen' );

	// Add custom description to Colors and Background controls or sections.
	if ( property_exists( $mt_customize->get_control( 'background_color' ), 'description' ) ) {
		$mt_customize->get_control( 'background_color' )->description = __( 'May only be visible on wide screens.', 'twentyfourteen' );
		$mt_customize->get_control( 'background_image' )->description = __( 'May only be visible on wide screens.', 'twentyfourteen' );
	} else {
		$mt_customize->get_section( 'colors' )->description           = __( 'Background may only be visible on wide screens.', 'twentyfourteen' );
		$mt_customize->get_section( 'background_image' )->description = __( 'Background may only be visible on wide screens.', 'twentyfourteen' );
	}

	// Add the featured content section in case it's not already there.
	$mt_customize->add_section(
		'featured_content',
		array(
...
function twentyfourteen_customize_preview_js() {
	mt_enqueue_script( 'twentyfourteen_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20141015', true );
}
		...
		array(
			...
			'content' =>
				'<ul>' .
					...
					'<li>' . sprintf( __( 'Enhance your site design by using <a href="%s">Featured Images</a> for posts you&rsquo;d like to stand out (also known as post thumbnails). This allows you to associate an image with your post without inserting it. Twenty Fourteen uses featured images for posts and pages&mdash;above the title&mdash;and in the Featured Content area on the home page.', 'twentyfourteen' ), 'https://codex.managertechnology.com.co/pacmec/Post_Thumbnails#Setting_a_Post_Thumbnail' ) . '</li>' .
					/* translators: %s: Twenty Fourteen documentation URL. */
					'<li>' . sprintf( __( 'For an in-depth tutorial, and more tips and tricks, visit the <a href="%s">Twenty Fourteen documentation</a>.', 'twentyfourteen' ), 'https://codex.managertechnology.com.co/pacmec/Twenty_Fourteen' ) . '</li>' .
				'</ul>',
		)
	);
}
