 ...
 * for various features in paCMec, such as post thumbnails, navigation menus, and the like.
 *
 ...
 * </code>
 *
 * For more information on hooks, actions, and filters, see https://developer.managertechnology.com.co/pacmec/plugins/.
 *
 * @package paCMec
...
/* Tell paCMec to run twentyten_setup() when the 'after_setup_theme' hook is run. */
...
if ( ! function_exists( 'twentyten_setup' ) ) :
	...
	function twentyten_setup() {
		...
		add_theme_support(
			'custom-background',
			array(
				// Let paCMec know what our default background color is.
		...
		if ( ! function_exists( 'get_custom_header' ) ) {
			// This is all for compatibility with versions of paCMec prior to 3.4.
...
/**
 * Show a home link for our mt_nav_menu() fallback, mt_page_menu().
 *
 * To override this in a child theme, remove the filter and optionally add
 * your own function tied to the mt_page_menu_args filter hook.
 *
 * @since Twenty Ten 1.0
 *
 * @param array $args An optional array of arguments. @see mt_page_menu()
 */
...
add_filter( 'mt_page_menu_args', 'twentyten_page_menu_args' );
 ...
 * a simple filter call that tells paCMec to not use the default styles.
 *
 ...
 *
 * @since Twenty Ten 1.0
 * @deprecated Deprecated in Twenty Ten 1.2 for paCMec 3.1
 *
...
// Backward compatibility with paCMec 3.0.
if ( version_compare( $GLOBALS['mt_version'], '3.1', '<' ) ) {
 ...
 *
 * This function uses a filter (show_recent_comments_widget_style) new in paCMec 3.1
 * to remove the default style. Using Twenty Ten 1.2 in paCMec 3.0 will show the styles,
...
function twentyten_scripts_styles() {
	...
	mt_enqueue_style( 'twentyten-block-style', get_template_directory_uri() . '/blocks.css', array(), '20181218' );
}
add_action( 'mt_enqueue_scripts', 'twentyten_scripts_styles' );
...
function twentyten_block_editor_styles() {
	// Block styles.
	mt_enqueue_style( 'twentyten-block-editor-style', get_template_directory_uri() . '/editor-blocks.css', array(), '20201208' );
}
