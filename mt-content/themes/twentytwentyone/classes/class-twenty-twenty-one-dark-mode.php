...
class Twenty_Twenty_One_Dark_Mode {
	...
	public function __construct() {
		...
		add_action( 'mt_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
		...
		add_action( 'mt_footer', array( $this, 'the_switch' ) );
	...
	public function enqueue_scripts() {
		...
		mt_enqueue_style( 'tt1-dark-mode', $url, array( 'twenty-twenty-one-style' ), mt_get_theme()->get( 'Version' ) ); // @phpstan-ignore-line. Version is always a string.
	}
	 ...
	 * @since Twenty Twenty-One 1.0
	 *
	 * @param MT_Customize_Manager $mt_customize Theme Customizer object.
	 *
	 * @return void
	 */
	public function customizer_controls( $mt_customize ) {

		$colors_section = $mt_customize->get_section( 'colors' );
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'twentytwentyone' );
		}

		// Custom notice control.
		include_once get_theme_file_path( 'classes/class-twenty-twenty-one-customize-notice-control.php' ); // phpcs:ignore MTThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	...
	public function the_html( $attrs = array() ) {
		...
		<style>
			...
			<?php if ( is_admin() || mt_is_json_request() ) : ?>
	...
	public function the_script() {
		echo '<script>';
		include get_template_directory() . '/assets/js/dark-mode-toggler.js'; // phpcs:ignore MTThemeReview.CoreFunctionality.FileInclude
		echo '</script>';
	}
	...
	public function add_privacy_policy_content() {
		if ( ! function_exists( 'mt_add_privacy_policy_content' ) ) {
			return;
		}
		...
		mt_add_privacy_policy_content( 'Twenty Twenty-One', mt_kses_post( mtautop( $content, false ) ) );
	}
