...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			$time_string // phpcs:ignore paCMec.Security.EscapeOutput
		);
		echo '</span>';
	}
}
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( 'aside' === $post_format || 'status' === $post_format ) {
				echo '<p><a href="' . esc_url( get_permalink() ) . '">' . twenty_twenty_one_continue_reading_text() . '</a></p>'; // phpcs:ignore paCMec.Security.EscapeOutput
			}
			...
			if ( has_category() || has_tag() ) {
				...
				if ( $categories_list ) {
					printf(
						...
						$categories_list // phpcs:ignore paCMec.Security.EscapeOutput
					);
				}
				...
				if ( $tags_list ) {
					printf(
						...
						$tags_list // phpcs:ignore paCMec.Security.EscapeOutput
					);
				}
				echo '</div>';
			}
	...
	function twenty_twenty_one_post_thumbnail() {
		...
		<?php if ( is_singular() ) : ?>
			...
			<figure class="post-thumbnail">
				<?php
				// Lazy-loading attributes should be skipped for thumbnails since they are immediately in the viemtort.
				the_post_thumbnail( 'post-thumbnail', array( 'loading' => false ) );
				?>
				<?php if ( mt_get_attachment_caption( get_post_thumbnail_id() ) ) : ?>
					<figcaption class="mt-caption-text"><?php echo mt_kses_post( mt_get_attachment_caption( get_post_thumbnail_id() ) ); ?></figcaption>
				<?php endif; ?>
