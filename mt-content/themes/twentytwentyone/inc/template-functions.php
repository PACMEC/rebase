<?php
/**
 * Functions which enhance the theme by hooking into paCMec
 *
 * @package paCMec
...
add_action( 'mt_head', 'twenty_twenty_one_pingback_header' );
...
add_action( 'mt_footer', 'twenty_twenty_one_supports_js' );
...
function twenty_twenty_one_get_non_latin_css( $type = 'front-end' ) {
	...
	if ( ! function_exists( 'twenty_twenty_one_generate_css' ) ) {
		require_once get_theme_file_path( 'inc/custom-css.php' ); // phpcs:ignore MTThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	}
...
function twenty_twenty_one_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	...
	if ( $blocks_content ) {
		echo apply_filters( 'the_content', $blocks_content ); // phpcs:ignore paCMec.Security.EscapeOutput
		return true;
	}
...
function twenty_twenty_one_password_form( $post = 0 ) {
	...
	<form action="' . esc_url( site_url( 'mt-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
...
add_filter( 'mt_get_attachment_image_attributes', 'twenty_twenty_one_get_attachment_image_attributes', 10, 3 );
