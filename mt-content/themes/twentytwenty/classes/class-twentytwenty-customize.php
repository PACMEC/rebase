...
if ( ! class_exists( 'TwentyTwenty_Customize' ) ) {
	...
	class TwentyTwenty_Customize {
		 ...
		 */
		public static function register( $mt_customize ) {
			...
			$mt_customize->add_control(
				new MT_Customize_Color_Control(
					...
					array(
						...
						'mode'            => 'hue',
						'active_callback' => function() use ( $mt_customize ) {
							return ( 'custom' === $mt_customize->get_setting( 'accent_hue_active' )->value() );
						},
					)
				)
			);
			...
			$mt_customize->get_setting( 'background_color' )->transport = 'postMessage';
			...
			$mt_customize->add_setting(
				...
				array(
					'sanitize_callback' => 'mt_filter_nohtml_kses',
				)
			);
