/* global twentyTwentyBgColors, twentyTwentyColor, jQuery, mt, _ */
/**
...
( function() {
	...
	mt.customize.bind( 'ready', function() {
		...
		mt.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( twentyTwentyBgColors ).forEach( function( context ) {
					...
					if ( twentyTwentyBgColors[ context ].color ) {
						...
						backgroundColorValue = mt.customize( twentyTwentyBgColors[ context ].setting ).get();
					}
		...
		Object.keys( twentyTwentyBgColors ).forEach( function( context ) {
			mt.customize( twentyTwentyBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					twentyTwentySetAccessibleColorsValue( context, to, mt.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		twentyTwentySetRetineLogoVisibility( !! mt.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		mt.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
	...
	function twentyTwentySetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		...
		value = mt.customize( 'accent_accessible_colors' ).get();
		...
		// Change the value.
		mt.customize( 'accent_accessible_colors' ).set( value );

		// Small hack to save the option.
		mt.customize( 'accent_accessible_colors' )._dirty = true;
	}
	...
	function twentyTwentySetRetineLogoVisibility( visible ) {
		mt.customize.control( 'retina_logo' ).container.toggle( visible );
	}
}( jQuery ) );
