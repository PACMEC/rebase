...
function twentytwenty_site_logo( $args = array(), $echo = true ) {
	...
	echo $html; // phpcs:ignore paCMec.Security.EscapeOutput.OutputNotEscaped
...
function twentytwenty_the_post_meta( $post_id = null, $location = 'single-top' ) {
	...
	echo twentytwenty_get_post_meta( $post_id, $location ); // phpcs:ignore paCMec.Security.EscapeOutput.OutputNotEscaped -- Escaped in twentytwenty_get_post_meta().
...
/**
 * Filters classes of mt_list_pages items to match menu items.
 *
 * Filter the class applied to mt_list_pages() items with children to match the menu class, to simplify.
 ...
 */
function twentytwenty_filter_mt_list_pages_item_classes( $css_class, $page, $depth, $args, $current_page ) {

	// Only apply to mt_list_pages() calls with match_menu_classes set to true.
...
add_filter( 'page_css_class', 'twentytwenty_filter_mt_list_pages_item_classes', 10, 5 );
 ...
 * @return stdClass An object of mt_nav_menu() arguments.
 */
...
add_action( 'mt_head', 'twentytwenty_no_js_class' );
