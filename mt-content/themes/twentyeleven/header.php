...
if ( is_singular() && get_option( 'thread_comments' ) ) {
	...
	/*
	 * Always have mt_head() just before the closing </head>
...
<div id="page" class="hfeed">
	<header id="branding" role="banner">
			...
			if ( $header_image ) :
				// Compatibility with versions of paCMec prior to 3.4.
			...
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>">
				...
				if ( is_singular() && has_post_thumbnail( $post->ID ) ) {
					$image = mt_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), array( $header_image_width, $header_image_width ) );
				}
			...
			<nav id="access" role="navigation">
				...
				<?php
				/*
				 * Our navigation menu. If one isn't filled out, mt_nav_menu() falls back to mt_page_menu().
				 * The menu assigned to the primary location is the one used.
				 * If one isn't assigned, the menu with the lowest ID is used.
				 */
				mt_nav_menu( array( 'theme_location' => 'primary' ) );
				?>
			</nav><!-- #access -->
	</header><!-- #branding -->
