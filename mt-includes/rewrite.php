...
function add_rewrite_rule( $regex, $query, $after = 'bottom' ) {
	...
	$mt_rewrite->add_rule( $regex, $query, $after );
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$mt_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
...
function remove_rewrite_tag( $tag ) {
	global $mt_rewrite;
	$mt_rewrite->remove_rewrite_tag( $tag );
}
...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$mt_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see MT_Rewrite::remove_permastruct()
 * @global MT_Rewrite $mt_rewrite paCMec rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
function remove_permastruct( $name ) {
	...
	$mt_rewrite->remove_permastruct( $name );
}
...
function add_feed( $feedname, $function ) {
	...
	if ( ! in_array( $feedname, $mt_rewrite->feeds, true ) ) {
		$mt_rewrite->feeds[] = $feedname;
	}
...
function flush_rewrite_rules( $hard = true ) {
	...
	if ( is_callable( array( $mt_rewrite, 'flush_rules' ) ) ) {
		$mt_rewrite->flush_rules( $hard );
	}
}
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $mt_rewrite;
	$mt_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _mt_filter_taxonomy_base( $base ) {
	if ( ! empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to MT_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, mt_unique_post_slug() prevents the creation of post slugs
 ...
 *                          MT::parse_request(). Default empty array.
 * @return array Returns the original array of query vars, with date/post conflicts resolved.
 */
function mt_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' === get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof MT_Post ) {
	...
	if ( ! $mt_rewrite->using_index_permalinks() ) {
		$url = str_replace( $mt_rewrite->index . '/', '', $url );
	}
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( (string) $key, $mt->public_query_vars, true ) ) {
					$query[ $key ] = $value;
			...
			$query = mt_resolve_numeric_slug_conflicts( $query );
