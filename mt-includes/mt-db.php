<?php
/**
 * paCMec database access abstraction class
 *
 ...
 * By default, paCMec uses this class to instantiate the global $mtdb object, providing
 * access to the paCMec database.
 *
 * It is possible to replace this class with your own by setting the $mtdb global variable
...
class mtdb {
	 ...
	 *
	 * Default is to show errors if both MT_DEBUG and MT_DEBUG_DISPLAY evaluate to true.
	 *
	 * @since 0.71
	 * @var bool
	 */
	 ...
	 * paCMec table prefix
	 *
	 * You can set this to have multiple paCMec installations in a single database.
	...
	/**
	 * List of paCMec per-blog tables.
	 *
	 * @since 2.5.0
	 * @see mtdb::tables()
	 * @var array
	...
	/**
	 * List of deprecated paCMec tables.
	 *
	...
	/**
	 * paCMec Term Relationships table.
	 *
	 * @since 2.3.0
	 * @var string
	 */
	 ...
	 *
	 * Columns not listed here default to %s. Initialized during MT load.
	 ...
	 *
	 * Set in mtdb::db_connect() to true, by default. This is used when checking
	 * against the required MySQL version for paCMec. Normally, a replacement
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		if ( MT_DEBUG && MT_DEBUG_DISPLAY ) {
			$this->show_errors();
		}

		// Use ext/mysqli if it exists unless MT_USE_EXT_MYSQL is defined as true.
		if ( function_exists( 'mysqli_connect' ) ) {
			$this->use_mysqli = true;

			if ( defined( 'MT_USE_EXT_MYSQL' ) ) {
				$this->use_mysqli = ! MT_USE_EXT_MYSQL;
			}
		}
		...
		// mt-config.php creation will manually connect when ready.
	...
	/**
	 * Changes the current SQL mode, and ensures its paCMec compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the paCMec tables.
	 *
	 * @since 2.5.0
	 *
	 * @param string $prefix          Alphanumeric name for the new prefix.
	 * @param bool   $set_table_names Optional. Whether the table names, e.g. mtdb::$posts,
	 *                                should be updated or not. Default true.
	 * @return string|MT_Error Old prefix or MT_Error on error.
	 */
	public function set_prefix( $prefix, $set_table_names = true ) {

		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return new MT_Error( 'invalid_db_prefix', 'Invalid database prefix' );
		}
	...
	/**
	 * Returns an array of paCMec tables.
	 *
	 * Also allows for the CUSTOM_USER_TABLE and CUSTOM_USER_META_TABLE to override the paCMec users
	 ...
	 *
	 * @uses mtdb::$tables
	 * @uses mtdb::$old_tables
	 * @uses mtdb::$global_tables
	 * @uses mtdb::$ms_global_tables
	 *
	 ...
	 *
	 * Use esc_sql() or mtdb::prepare() instead.
	 *
	 * @since 2.8.0
	 * @deprecated 3.6.0 Use mtdb::prepare()
	 * @see mtdb::prepare()
	 * @see esc_sql()
	 *
	 * @param string $string
	...
	function _weak_escape( $string ) {
		if ( func_num_args() === 1 && function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __METHOD__, '3.6.0', 'mtdb::prepare() or esc_sql()' );
		}
	...
	function _real_escape( $string ) {
		...
		if ( $this->dbh ) {
			...
			if ( function_exists( '__' ) ) {
				/* translators: %s: Database access abstraction class, usually mtdb or a class extending mtdb. */
	 ...
	 *
	 * Examples:
	 *     $mtdb->prepare( "SELECT * FROM `table` WHERE `column` = %s AND `field` = %d OR `other_field` LIKE %s", array( 'foo', 1337, '%bar' ) );
	 *     $mtdb->prepare( "SELECT DATE_FORMAT(`field`, '%%c') FROM `table` WHERE `column` = %s", 'foo' );
	 *
	 * @since 2.3.0
	 ...
	 *
	 * Use this only before mtdb::prepare() or esc_sql(). Reversing the order is very bad for security.
	 *
	 * Example Prepared Statement:
	 *
	 *     $wild = '%';
	 *     $find = 'only 43% of planets';
	 *     $like = $wild . $mtdb->esc_like( $find ) . $wild;
	 *     $sql  = $mtdb->prepare( "SELECT * FROM $mtdb->posts WHERE post_content LIKE %s", $like );
	 *
	 * Example Escape Chain:
	 *
	 *     $sql  = esc_sql( $mtdb->esc_like( $input ) );
	 *
	 * @since 4.0.0
	 *
	 ...
	 *                Call mtdb::prepare() or mtdb::_real_escape() next.
	 */
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( __( 'paCMec database error %1$s for query %2$s made by %3$s' ), $str, $this->last_query, $caller );
		} else {
			/* translators: 1: Database error message, 2: SQL query. */
			$error_str = sprintf( __( 'paCMec database error %1$s for query %2$s' ), $str, $this->last_query );
		}
		...
		if ( is_multisite() ) {
			...
			printf(
				'<div id="error"><p class="mtdberror"><strong>%s</strong> [%s]<br /><code>%s</code></p></div>',
				__( 'paCMec database error:' ),
				$str,
				$query
			);
	 ...
	 * mtdb::hide_errors() should be used instead for hiding errors.
	 *
	 * @since 0.71
	 *
	 * @see mtdb::hide_errors()
	 *
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( $this->use_mysqli ) {
			...
			if ( $this->dbh->connect_errno ) {
				 ...
				 *  - MT_USE_EXT_MYSQL isn't set to false, and
				 *  - ext/mysql is loaded.
				 */
				...
				} elseif ( defined( 'MT_USE_EXT_MYSQL' ) && ! MT_USE_EXT_MYSQL ) {
					$attempt_fallback = false;
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: 1: mt-config.php, 2: Database host. */
	...
	public function check_connection( $allow_bail = true ) {
		...
		for ( $tries = 1; $tries <= $this->reconnect_retries; $tries++ ) {
			...
			if ( $this->reconnect_retries === $tries && MT_DEBUG ) {
				error_reporting( $error_reporting );
			}
		...
		// If template_redirect has already happened, it's too late for mt_die()/dead_db().
	...
	public function placeholder_escape() {
		...
		if ( ! $placeholder ) {
			...
			// Old MT installs may not have AUTH_SALT defined.
	 ...
	 *
	 * Examples:
	 *     mtdb::insert( 'table', array( 'column' => 'foo', 'field' => 'bar' ) )
	 *     mtdb::insert( 'table', array( 'column' => 'foo', 'field' => 1337 ), array( '%s', '%d' ) )
	 *
	 * @since 2.5.0
	 *
	 * @see mtdb::prepare()
	 * @see mtdb::$field_types
	 * @see mt_set_mtdb_vars()
	 *
	 ...
	 *
	 * Examples:
	 *     mtdb::replace( 'table', array( 'column' => 'foo', 'field' => 'bar' ) )
	 *     mtdb::replace( 'table', array( 'column' => 'foo', 'field' => 1337 ), array( '%s', '%d' ) )
	 *
	 * @since 3.0.0
	 *
	 * @see mtdb::prepare()
	 ...
	 *
	 * Examples:
	 *     mtdb::update( 'table', array( 'column' => 'foo', 'field' => 'bar' ), array( 'ID' => 1 ) )
	 *     mtdb::update( 'table', array( 'column' => 'foo', 'field' => 1337 ), array( 'ID' => 1 ), array( '%s', '%d' ), array( '%d' ) )
	 *
	 * @since 2.5.0
	 *
	 * @see mtdb::prepare()
	 ...
	 *
	 * Examples:
	 *     mtdb::delete( 'table', array( 'ID' => 1 ) )
	 *     mtdb::delete( 'table', array( 'ID' => 1 ), array( '%d' ) )
	 *
	 * @since 3.4.0
	 *
	 * @see mtdb::prepare()
	 ...
	 *
	 * This is a helper method for mtdb's CRUD methods, which take field/value pairs
	...
	/**
	 * Prepares arrays of value/format pairs as passed to mtdb CRUD methods.
	 *
	 * @since 4.2.0
	 *
	...
	/**
	 * Adds field charsets to field/value/format arrays generated by mtdb::process_field_formats().
	 *
	 * @since 4.2.0
	 *
	 * @param array  $data  As it comes from the mtdb::process_field_formats() method.
	 * @param string $table Table name.
	 ...
	 * @since 4.2.1
	 *
	 * @param array  $data  As it comes from the mtdb::process_field_charsets() method.
	 * @param string $table Table name.
	 ...
	 *
	 * @param string $table Table name.
	 * @return string|MT_Error Table character set, MT_Error object if it couldn't be found.
	 */
	protected function get_table_charset( $table ) {
		...
		if ( ! $results ) {
			return new MT_Error( 'mtdb_get_table_charset_failure' );
		}
	 ...
	 * @return string|false|MT_Error Column character set as a string. False if the column has
	 *                               no character set. MT_Error object if there was an error.
	 */
	 ...
	 * @param string $table  Table name.
	 * @param string $column Column name.
	 * @return array|false|MT_Error array( 'length' => (int), 'type' => 'byte' | 'char' ).
	 *                              False if the column has no length (for example, numeric column).
	 *                              MT_Error object if there was an error.
	 */
	 ...
	 * @return array|MT_Error The $data parameter, with invalid characters removed from each value.
	...
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
				return new MT_Error( 'mtdb_strip_invalid_text_failure' );
			}
	 ...
	 * @return string|MT_Error The converted query, or a MT_Error object if the conversion fails.
	 */
	 ...
	 * @return string|MT_Error The converted string, or a MT_Error object if the conversion fails.
	 */
	...
	protected function get_table_from_query( $query ) {
		...
		// SHOW TABLE STATUS and SHOW TABLES WHERE Name = 'mt_posts'
		if ( preg_match( '/^\s*SHOW\s+(?:TABLE\s+STATUS|(?:FULL\s+)?TABLES).+WHERE\s+Name\s*=\s*("|\')((?:[0-9a-zA-Z$_.-]|[\xC2-\xDF][\x80-\xBF])+)\\1/is', $query, $maybe ) ) {
			return $maybe[2];
		}

		/*
		 * SHOW TABLE STATUS LIKE and SHOW TABLES LIKE 'mt\_123\_%'
		 * This quoted LIKE operand seldom holds a full table name.
		 * It is usually a pattern for matching a prefix so we just
		 * strip the trailing % and unescape the _ to get 'mt_123_'
	 ...
	 *
	 * Will not die if mtdb::$show_errors is false.
	 *
	 * @since 1.5.0
	 *
	...
	public function bail( $message, $error_code = '500' ) {
		if ( $this->show_errors ) {
			...
			if ( class_exists( 'MT_Error', false ) ) {
				$this->error = new MT_Error( $error_code, $message );
			} else {
				$this->error = $message;
			}
	 ...
	 * @return void|MT_Error
	 */
	public function check_database_version() {
		global $mt_version, $required_mysql_version;
		// Make sure the server has the required MySQL version.
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: paCMec version number, 2: Minimum required MySQL version number. */
			return new MT_Error( 'database_version', sprintf( __( '<strong>Error</strong>: paCMec %1$s requires MySQL %2$s or higher' ), $mt_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when paCMec is generating the table scheme.
	 *
	 * Use `mtdb::has_cap( 'collation' )`.
	 *
	 * @since 2.5.0
	 * @deprecated 3.5.0 Use mtdb::has_cap()
	 *
	 * @return bool True if collation is supported, false if not.
	 */
	public function supports_collation() {
		_deprecated_function( __FUNCTION__, '3.5.0', 'mtdb::has_cap( \'collation\' )' );
	...
	/**
	 * Retrieves a comma-separated list of the names of the functions that called mtdb.
	 *
	 * @since 2.5.0
	 *
	...
	public function get_caller() {
		return mt_debug_backtrace_summary( __CLASS__ );
	}
