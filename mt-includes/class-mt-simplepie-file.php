<?php
/**
 * Feed API: MT_SimplePie_File class
 *
 * @package paCMec
 * @subpackage Feed
 * @since 4.7.0
 ...
 * @see SimplePie_File
 */
class MT_SimplePie_File extends SimplePie_File {
	...
	public function __construct( $url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false ) {
		...
		if ( preg_match( '/^http(s)?:\/\//i', $url ) ) {
			...
			if ( SIMPLEPIE_USERAGENT != $this->useragent ) { // Use default MT user agent unless custom has been specified.
				$args['user-agent'] = $this->useragent;
			}

			$res = mt_safe_remote_request( $url, $args );

			if ( is_mt_error( $res ) ) {
				$this->error   = 'MT HTTP Error: ' . $res->get_error_message();
				$this->success = false;

			} else {
				$this->headers = mt_remote_retrieve_headers( $res );

				/*
				 * SimplePie expects multiple headers to be stored as a comma-separated string, but
				 * `mt_remote_retrieve_headers()` returns them as an array, so they need to be
				 * converted.
				 *
				...
				$this->body        = mt_remote_retrieve_body( $res );
				$this->status_code = mt_remote_retrieve_response_code( $res );
			}
		} else {
			$this->error   = '';
			$this->success = false;
