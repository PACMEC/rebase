<?php
/**
 * Facilitates adding of the paCMec editor as used on the Write and Edit screens.
 *
 * @package paCMec
 * @since 3.3.0
 *
 * Private, not included by default. See mt_editor() in mt-includes/general-template.php.
 */

final class _MT_Editors {
	 ...
	 *
	 *     @type bool       $mtautop           Whether to use mtautop(). Default true.
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filters the mt_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 ...
		 */
		$settings = apply_filters( 'mt_editor_settings', $settings, $editor_id );
		...
		if ( self::$this_tinymce ) {
			if ( false !== strpos( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'mt_editor()', '3.9.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['mt_autoresize_on'] ) ) {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class   = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' mt-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : mt_default_editor();
				// 'html' is used for the "Text" editor tab.
				if ( 'html' !== $default_editor ) {
					$default_editor = 'tinymce';
				}

				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce" class="mt-switch-editor switch-tmce"' .
					' data-mt-editor-id="' . $editor_id_attr . '">' . _x( 'Visual', 'Name for the Visual editor tab' ) . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html" class="mt-switch-editor switch-html"' .
					' data-mt-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class   = 'mt-core-ui mt-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="mt-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			mt_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="mt-' . $editor_id_attr . '-editor-tools" class="mt-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( ! function_exists( 'media_buttons' ) ) {
					require ABSPATH . 'mt-admin/includes/media.php';
				}

				echo '<div id="mt-' . $editor_id_attr . '-media-buttons" class="mt-media-buttons">';
			...
			echo '<div class="mt-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		...
		$the_editor = apply_filters(
			'the_editor',
			'<div id="mt-' . $editor_id_attr . '-editor-container" class="mt-editor-container">' .
			$quicktags_toolbar .
	...
	public static function editor_settings( $editor_id, $set ) {
		if ( empty( self::$first_init ) ) {
			if ( is_admin() ) {
				...
				add_action( 'mt_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'mt_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
				add_action( 'mt_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * 'myplugin' => 'http://mysite.com/mt-content/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in paCMec should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 * @since 5.3.0 The `$editor_id` parameter was added.
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as mt_mce_translation(),
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = MT_CONTENT_DIR . $path . '/langs/';
				...
				if ( ! empty( $mce_external_plugins ) ) {
					$settings['external_plugins'] = mt_json_encode( $mce_external_plugins );
				}

				/** This filter is documented in mt-admin/includes/media.php */
				if ( apply_filters( 'disable_captions', '' ) ) {
					$settings['mteditimage_disable_captions'] = true;
				}
				 ...
				 * Plugins that use mt_editor() on the front-end can decide whether to add the theme stylesheet
	...
	public static function enqueue_scripts( $default_scripts = false ) {
		if ( $default_scripts || self::$has_tinymce ) {
			mt_enqueue_script( 'editor' );
		}

		if ( $default_scripts || self::$has_quicktags ) {
			mt_enqueue_script( 'quicktags' );
			mt_enqueue_style( 'buttons' );
		}

		if ( $default_scripts || in_array( 'mtlink', self::$plugins, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			mt_enqueue_script( 'mtlink' );
			mt_enqueue_script( 'jquery-ui-autocomplete' );
		}

		if ( self::$has_medialib ) {
			add_thickbox();
			mt_enqueue_script( 'media-upload' );
	...
	public static function enqueue_default_editor() {
		...
		if ( did_action( 'mt_enqueue_editor' ) ) {
			return;
		}

		self::enqueue_scripts( true );

		// Also add mt-includes/css/editor.css.
		mt_enqueue_style( 'editor-buttons' );
		...
		if ( is_admin() ) {
			...
			add_action( 'mt_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
			add_action( 'mt_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		}
	}
	...
	public static function print_default_editor_scripts() {
		...
		if ( $user_can_richedit ) {
			...
			/*
			 * In production all plugins are loaded (they are in mt-editor.js.gz).
			 * The 'mtview', 'mtdialogs', and 'media' TinyMCE plugins are not initialized by default.
			 * Can be added from js by using the 'mt-before-tinymce-init' event.
			 */
		...
		window.mt = window.mt || {};
		window.mt.editor = window.mt.editor || {};
		window.mt.editor.getDefaultSettings = function() {
			return {
		...
		$settings = array(
			...
			'formats'                      => '{' .
				'alignleft: [' .
					...
					'{selector: "img,table,dl.mt-caption", classes: "alignleft"}' .
				'],' .
				'aligncenter: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
					'{selector: "img,table,dl.mt-caption", classes: "aligncenter"}' .
				'],' .
				'alignright: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
					'{selector: "img,table,dl.mt-caption", classes: "alignright"}' .
				'],' .
			...
			'entity_encoding'              => 'raw',
			'keep_styles'                  => false,
			'cache_suffix'                 => 'mt-mce-' . $tinymce_version,
			'resize'                       => 'vertical',
			'menubar'                      => false,
			'branding'                     => false,
			...
			'mteditimage_html5_captions'   => true,
			'mt_lang_attr'                 => get_bloginfo( 'language' ),
			'mt_keep_scroll_position'      => false,
			'mt_shortcut_labels'           => mt_json_encode( $shortcut_labels ),
		);
	 ...
	 */
	public static function mt_mce_translation( $mce_locale = '', $json_only = false ) {
		if ( ! $mce_locale ) {
			$mce_locale = self::get_mce_locale();
		}
		 ...
		 */
		$mce_translation = apply_filters( 'mt_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return mt_json_encode( $mce_translation );
		}

		$baseurl = self::get_baseurl();

		return "tinymce.addI18n( '$mce_locale', " . mt_json_encode( $mce_translation ) . ");\n" .
	...
	public static function force_uncompressed_tinymce() {
		...
		$mt_scripts->remove( 'mt-tinymce' );
		mt_register_tinymce_scripts( $mt_scripts, true );
	}
	...
	public static function print_tinymce_scripts() {
		...
		mt_print_scripts( array( 'mt-tinymce' ) );

		echo "<script type='text/javascript'>\n" . self::mt_mce_translation() . "</script>\n";
	}
	...
	public static function editor_js() {
		 ...
		 */
		do_action( 'before_mt_tiny_mce', self::$mce_settings );
		?>
		...
		if ( $tmce_on ) {
			...
			if ( self::$ext_plugins ) {
				...
				echo "<script type='text/javascript' src='{$baseurl}/langs/mt-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'mt_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				if ( tinymce.Env.ie && tinymce.Env.ie < 11 ) {
					tinymce.$( '.mt-editor-wrap ' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
					return;
				}

				for ( id in tinyMCEPreInit.mceInit ) {
					init = tinyMCEPreInit.mceInit[id];
					$wrap = tinymce.$( '#mt-' + id + '-wrap' );

					if ( ( $wrap.hasClass( 'tmce-active' ) || ! tinyMCEPreInit.qtInit.hasOwnProperty( id ) ) && ! init.mt_skip_init ) {
						tinymce.init( init );

						if ( ! window.mtActiveEditor ) {
							window.mtActiveEditor = id;
						}
					}
				}
			}
		...
		if ( in_array( 'mtlink', self::$plugins, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			self::mt_link_dialog();
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_mt_tiny_mce', self::$mce_settings );
	}
	 ...
	 * @deprecated 4.3.0
	 */
	public static function mt_fullscreen_html() {
	 ...
	 */
	public static function mt_link_query( $args = array() ) {
		 ...
		 *
		 * @see MT_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of MT_Query arguments.
		 */
		$query = apply_filters( 'mt_link_query_args', $query );
		 ...
		 * @param array $query  An array of MT_Query arguments.
		 */
		$results = apply_filters( 'mt_link_query', $results, $query );
	 ...
	 * @since 3.1.0
	 */
	public static function mt_link_dialog() {
		// Run once.
		...
		// `display: none` is required here, see #MT27605.
		?>
		<div id="mt-link-backdrop" style="display: none"></div>
		<div id="mt-link-wrap" class="mt-core-ui" style="display: none" role="dialog" aria-labelledby="link-modal-title">
		<form id="mt-link" tabindex="-1">
		<?php mt_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ); ?></h1>
		<button type="button" id="mt-link-close"><span class="screen-reader-text"><?php _e( 'Close' ); ?></span></button>
		<div id="link-selector">
			<div id="link-options">
				<p class="howto" id="mtlink-enter-url"><?php _e( 'Enter the destination URL' ); ?></p>
				<div>
					<label><span><?php _e( 'URL' ); ?></span>
					<input id="mt-link-url" type="text" aria-describedby="mtlink-enter-url" /></label>
				</div>
				<div class="mt-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span>
					<input id="mt-link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="mt-link-target" /> <?php _e( 'Open link in a new tab' ); ?></label>
				</div>
			</div>
			<p class="howto" id="mtlink-link-existing-content"><?php _e( 'Or link to existing content' ); ?></p>
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						<span class="search-label"><?php _e( 'Search' ); ?></span>
						<input type="search" id="mt-link-search" class="link-search-field" autocomplete="off" aria-describedby="mtlink-link-existing-content" />
		...
		<div class="submitbox">
			<div id="mt-link-cancel">
				<button type="button" class="button"><?php _e( 'Cancel' ); ?></button>
			</div>
			<div id="mt-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="mt-link-submit" name="mt-link-submit">
			</div>
		</div>
		</form>
		</div>
