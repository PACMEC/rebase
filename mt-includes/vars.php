<?php
/**
 * Creates common globals for the rest of paCMec
 *
 * Sets $pagenow global which is the current page. Checks
 * for the browser to set which one is currently being used.
 *
 * Detects which user environment paCMec is being used on.
...
if ( is_admin() ) {
	// mt-admin pages are checked more carefully.
	if ( is_network_admin() ) {
		preg_match( '#/mt-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} elseif ( is_user_admin() ) {
		preg_match( '#/mt-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/mt-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( '.php' !== substr( $pagenow, -4, 4 ) ) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /mt-admin/themes/index.php (themes.php is queried).
		}
	}
} else {
...
function mt_is_mobile() {
	 ...
	 */
	return apply_filters( 'mt_is_mobile', $is_mobile );
}
