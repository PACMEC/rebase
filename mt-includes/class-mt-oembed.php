...
class MT_oEmbed {
	...
	public function __construct() {
		...
		$providers = array(
			...
			'#https?://(www\.)?issuu\.com/.+/docs/.+#i'    => array( 'https://issuu.com/oembed_mt', true ),
		 ...
		 *
		 * Since paCMec 4.4, oEmbed discovery is enabled for all users and allows embedding of sanitized
		...
		// Fix any embeds that contain new lines in the middle of the HTML which breaks mtautop().
	 ...
	 *
	 * The provider is added just-in-time when mt_oembed_add_provider() is called before
	 ...
	 *
	 * The provider is removed just-in-time when mt_oembed_remove_provider() is called before
	 * the {@see 'plugins_loaded'} hook.
	 *
	 ...
	 *                           See mt_oembed_get() for accepted arguments. Default empty.
	...
	public function discover( $url ) {
		...
		// Fetch URL content.
		$request = mt_safe_remote_get( $url, $args );
		$html    = mt_remote_retrieve_body( $request );
		if ( $html ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = mt_parse_args( $args, mt_embed_defaults( $url ) );
		...
		foreach ( array( 'json', 'xml' ) as $format ) {
			...
			if ( is_mt_error( $result ) && 'not-implemented' === $result->get_error_code() ) {
				continue;
			}
			return ( $result && ! is_mt_error( $result ) ) ? $result : false;
		}
		return false;
	}
	 ...
	 * @return object|false|MT_Error The result in the form of an object on success, false on failure.
	 */
	private function _fetch_with_format( $provider_url_with_args, $format ) {
		$provider_url_with_args = add_query_arg( 'format', $format, $provider_url_with_args );

		/** This filter is documented in mt-includes/class-mt-oembed.php */
		$args = apply_filters( 'oembed_remote_get_args', array(), $provider_url_with_args );

		$response = mt_safe_remote_get( $provider_url_with_args, $args );
		if ( 501 == mt_remote_retrieve_response_code( $response ) ) {
			return new MT_Error( 'not-implemented' );
		}
		$body = mt_remote_retrieve_body( $response );
		if ( ! $body ) {
			return false;
		}
	...
	/**
	 * Converts a data object from MT_oEmbed::fetch() and returns the HTML.
	 *
	 * @since 2.9.0
	 *
	 ...
	 * @param object $data Data object from MT_oEmbed::data2html()
