 ...
 *
 * These functions are meant to live inside of the paCMec loop.
 *
 * @package paCMec
 * @subpackage Template
 */
 ...
 *
 * @since 1.5.0
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to retrieve the author.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to print the author.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to get the author's email.
 *                                   Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to print the author's email.
 *                                   Default current comment.
 */
 ...
 * @param int|MT_Comment $comment  Optional. Comment ID or MT_Comment object. Default is the current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to get the author's link.
 *                                   Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to print the author's link.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to get the author's IP address.
 *                                   Default current comment.
 * @return string Comment author's IP address.
 */
function get_comment_author_IP( $comment_ID = 0 ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$comment = get_comment( $comment_ID );
	 ...
	 * @param MT_Comment $comment           The comment object.
	 */
	return apply_filters( 'get_comment_author_IP', $comment->comment_author_IP, $comment->comment_ID, $comment );  // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
}
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to print the author's IP address.
 *                                   Default current comment.
 */
function comment_author_IP( $comment_ID = 0 ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionNameInvalid
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to get the author's URL.
 *                                   Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or the ID of the comment for which to print the author's URL.
 *                                   Default current comment.
 */
 ...
 *                                 Default empty.
 * @param int|MT_Comment $comment  Optional. Comment ID or MT_Comment object.
 ...
 *
 * @since 2.7.0
 * @since 4.4.0 Added the ability for `$comment` to also accept a MT_Comment object.
 *
 * @param string|string[] $class    Optional. One or more classes to add to the class list.
 *                                  Default empty.
 * @param int|MT_Comment  $comment  Comment ID or MT_Comment object. Default current comment.
 * @param int|MT_Post     $post_id  Post ID or MT_Post object. Default current post.
 ...
 * @param int|MT_Comment  $comment_id Comment ID or MT_Comment object. Default current comment.
 * @param int|MT_Post     $post_id    Post ID or MT_Post object. Default current post.
...
function get_comment_class( $class = '', $comment_id = null, $post_id = null ) {
	 ...
	 * @param int         $comment_id The comment ID.
	 * @param MT_Comment  $comment    The comment object.
	 * @param int|MT_Post $post_id    The post ID or MT_Post object.
	 */
 ...
 * @param int|MT_Comment $comment_ID MT_Comment or ID of the comment for which to get the date.
 *                                   Default current comment.
 ...
 *
 * @param string         $format     Optional. PHP date format. Defaults to the 'date_format' option.
 * @param int|MT_Comment $comment_ID MT_Comment or ID of the comment for which to print the date.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID  MT_Comment or ID of the comment for which to get the excerpt.
 *                                    Default current comment.
...
function get_comment_excerpt( $comment_ID = 0 ) {
	...
	$excerpt = mt_trim_words( $comment_text, $comment_excerpt_length, '&hellip;' );
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID  MT_Comment or ID of the comment for which to print the excerpt.
 *                                    Default current comment.
 */
 ...
 */
function get_comment_ID() { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$comment = get_comment();
	 ...
	 * @param MT_Comment $comment    The comment object.
	 */
	return apply_filters( 'get_comment_ID', $comment->comment_ID, $comment );  // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
}
 ...
 * @since 0.71
 */
function comment_ID() { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionNameInvalid
	echo get_comment_ID();
}
 ...
 *
 * @since 1.5.0
 * @since 4.4.0 Added the ability for `$comment` to also accept a MT_Comment object. Added `$cpage` argument.
 *
 ...
 *
 * @param MT_Comment|int|null $comment Comment to retrieve. Default current comment.
 * @param array               $args {
...
function get_comment_link( $comment = null, $args = array() ) {
	global $mt_rewrite, $in_comment_loop;
	...
	if ( $cpage && get_option( 'page_comments' ) ) {
		if ( $mt_rewrite->using_permalinks() ) {
			if ( $cpage ) {
				$link = trailingslashit( $link ) . $mt_rewrite->comments_pagination_base . '-' . $cpage;
			}
	 ...
	 * @param MT_Comment $comment The current comment object.
...
function get_comments_link( $post_id = 0 ) {
	 ...
	 * @param int|MT_Post $post_id       Post ID or MT_Post object.
	 */
 ...
 * @since 1.5.0
 *
 * @param int|MT_Post $post_id Optional. Post ID or MT_Post object. Default is the global `$post`.
 ...
 *
 * @param int|MT_Comment $comment_ID MT_Comment or ID of the comment for which to get the text.
 *                                   Default current comment.
 ...
 * @see Walker_Comment::comment()
 *
 * @param int|MT_Comment $comment_ID MT_Comment or ID of the comment for which to print the text.
 *                                   Default current comment.
 * @param array          $args       Optional. An array of arguments. Default empty array.
 */
function comment_text( $comment_ID = 0, $args = array() ) {
	 ...
	 * @param MT_Comment|null $comment      The comment object. Null if not found.
	 * @param array           $args         An array of arguments.
	 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MT_Comment object.
 *
 * @param int|MT_Comment $comment_ID Optional. MT_Comment or ID of the comment for which to get the type.
 *                                   Default current comment.
...
function get_trackback_url() {
	if ( get_option( 'permalink_structure' ) ) {
		...
		$tb_url = get_option( 'siteurl' ) . '/mt-trackback.php?p=' . get_the_ID();
	}
...
function trackback_rdf( $deprecated = '' ) {
	...
	the_permalink();
	echo '"' . "\n";
	echo '    dc:title="' . str_replace( '--', '&#x2d;&#x2d;', mttexturize( strip_tags( get_the_title() ) ) ) . '"' . "\n";
 ...
 * @since 2.1.3
 */
function mt_comment_form_unfiltered_html_nonce() {
	$post    = get_post();
	$post_id = $post ? $post->ID : 0;

	if ( current_user_can( 'unfiltered_html' ) ) {
		mt_nonce_field( 'unfiltered-html-comment_' . $post_id, '_mt_unfiltered_html_comment_disabled', false );
		echo "<script>(function(){if(window===window.parent){document.getElementById('_mt_unfiltered_html_comment_disabled').name='_mt_unfiltered_html_comment';}})();</script>\n";
	}
}
 ...
 *
 * Uses the paCMec database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the paCMec process will be
...
function comments_template( $file = '/comments.php', $separate_comments = false ) {
	global $mt_query, $withcomments, $post, $mtdb, $id, $comment, $user_login, $user_identity, $overridden_cpage;
	...
	if ( get_option( 'page_comments' ) ) {
		...
		if ( $page ) {
			...
			$top_level_query = new MT_Comment_Query();
	...
	$comment_query = new MT_Comment_Query( $comment_args );
	 ...
	 * @param int   $post_ID  Post ID.
	 */
	$mt_query->comments = apply_filters( 'comments_array', $comments_flat, $post->ID );

	$comments                        = &$mt_query->comments;
	$mt_query->comment_count         = count( $mt_query->comments );
	$mt_query->max_num_comment_pages = $comment_query->max_num_pages;

	if ( $separate_comments ) {
		$mt_query->comments_by_type = separate_comments( $comments );
		$comments_by_type           = &$mt_query->comments_by_type;
	} else {
		$mt_query->comments_by_type = array();
	}

	$overridden_cpage = false;

	if ( '' == get_query_var( 'cpage' ) && $mt_query->max_num_comment_pages > 1 ) {
	...
	if ( file_exists( $include ) ) {
		...
		require ABSPATH . MTINC . '/theme-compat/comments.php';
	}
}
 ...
 * }
 * @param int|MT_Comment $comment Comment being replied to. Default current comment.
 * @param int|MT_Post    $post    Post ID or MT_Post object the comment is going to be displayed on.
...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	 ...
	 * @param MT_Comment $comment The object of the comment being replied to.
	 * @param MT_Post    $post    The MT_Post object.
	 */
	...
	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		$link = sprintf(
			...
			esc_url( mt_login_url( get_permalink() ) ),
			$args['login_text']
		);
	} else {
	 ...
	 * @param MT_Comment $comment The object of the comment being replied.
	 * @param MT_Post    $post    The MT_Post object.
	 */
 ...
 * }
 * @param int|MT_Post $post    Optional. Post ID or MT_Post object the comment is going to be displayed on.
 *                             Default current post.
 * @return string|false|null Link to show comment form, if successful. False, if comments are closed.
 */
function get_post_reply_link( $args = array(), $post = null ) {
	 ...
	 * @param int|MT_Post $post      The post ID or MT_Post object.
	 */
 ...
 * }
 * @param MT_Comment[] $comments Optional. Array of MT_Comment objects.
 * @return void|string Void if 'echo' argument is true, or no comments to list.
 *                     Otherwise, HTML list of comments.
 */
function mt_list_comments( $args = array(), $comments = null ) {
	global $mt_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	 ...
	 */
	$parsed_args = apply_filters( 'mt_list_comments_args', $parsed_args );
	...
	if ( null !== $comments ) {
		...
		/*
		 * If 'page' or 'per_page' has been passed, and does not match what's in $mt_query,
		 * perform a separate comment query and allow Walker_Comment to paginate.
		 */
		if ( $parsed_args['page'] || $parsed_args['per_page'] ) {
			$current_cpage = get_query_var( 'cpage' );
			if ( ! $current_cpage ) {
				$current_cpage = 'newest' === get_option( 'default_comments_page' ) ? 1 : $mt_query->max_num_comment_pages;
			}
			...
			// Otherwise, fall back on the comments from `$mt_query->comments`.
		} else {
			if ( empty( $mt_query->comments ) ) {
				return;
			}
			if ( 'all' !== $parsed_args['type'] ) {
				if ( empty( $mt_query->comments_by_type ) ) {
					$mt_query->comments_by_type = separate_comments( $mt_query->comments );
				}
				if ( empty( $mt_query->comments_by_type[ $parsed_args['type'] ] ) ) {
					return;
				}
				$_comments = $mt_query->comments_by_type[ $parsed_args['type'] ];
			} else {
				$_comments = $mt_query->comments;
			}

			if ( $mt_query->max_num_comment_pages ) {
	...
	mt_queue_comments_for_comment_meta_lazyload( $_comments );
 ...
 *     @type string $action               The comment form element action attribute. Default '/mt-comments-post.php'.
 ...
 * }
 * @param int|MT_Post $post_id Post ID or MT_Post object to generate the form for. Default current post.
 */
function comment_form( $args = array(), $post_id = null ) {
	...
	if ( has_action( 'set_comment_cookies', 'mt_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
		$consent = empty( $commenter['comment_author_email'] ) ? '' : ' checked="checked"';

		$fields['cookies'] = sprintf(
			'<p class="comment-form-cookies-consent">%s %s</p>',
			sprintf(
				'<input id="mt-comment-cookies-consent" name="mt-comment-cookies-consent" type="checkbox" value="yes"%s />',
				$consent
			),
			sprintf(
				'<label for="mt-comment-cookies-consent">%s</label>',
	...
	$defaults = array(
		...
		'must_log_in'          => sprintf(
			...
			sprintf(
				...
				mt_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ), $post_id ) )
			)
		),
		'logged_in_as'         => sprintf(
			...
			sprintf(
				...
				$user_identity,
				/** This filter is documented in mt-includes/link-template.php */
				mt_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ), $post_id ) )
			)
		),
		...
		'comment_notes_after'  => '',
		'action'               => site_url( '/mt-comments-post.php' ),
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
	 ...
	 */
	$args = mt_parse_args( $args, apply_filters( 'comment_form_defaults', $defaults ) );
