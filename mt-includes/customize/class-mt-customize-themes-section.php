<?php
/**
 * Customize API: MT_Customize_Themes_Section class
 *
 * @package paCMec
 ...
 */
class MT_Customize_Themes_Section extends MT_Customize_Section {
	 ...
	 *
	 * Defines the type of themes to load (installed, mtorg, etc.).
	 *
	 * @since 4.9.0
	 * @var string
	 */
			...
			<div class="customize-themes-section themes-section-{{ data.id }} control-section-content themes-php">
				...
				<div class="theme-browser rendered">
					...
					<p class="no-themes-local">
						<?php
						printf(
							/* translators: %s: "Search paCMec.org themes" button text. */
							__( 'No themes found. Try a different search, or %s.' ),
							sprintf( '<button type="button" class="button-link search-dotorg-themes">%s</button>', __( 'Search paCMec.org themes' ) )
						);
						?>
					</p>
					<p class="spinner"></p>
	...
	protected function filter_bar_content_template() {
		...
		<# if ( 'mtorg' === data.action ) { #>
			<div class="search-form">
				<label for="mt-filter-search-input-{{ data.id }}" class="screen-reader-text"><?php _e( 'Search themes&hellip;' ); ?></label>
				<input type="search" id="mt-filter-search-input-{{ data.id }}" placeholder="<?php esc_attr_e( 'Search themes&hellip;' ); ?>" aria-describedby="{{ data.id }}-live-search-desc" class="mt-filter-search">
		...
		<# } else { #>
			<div class="themes-filter-container">
				...
				<input type="search" id="{{ data.id }}-themes-filter" placeholder="<?php esc_attr_e( 'Search themes&hellip;' ); ?>" aria-describedby="{{ data.id }}-live-search-desc" class="mt-filter-search mt-filter-search-themes" />
				<div class="search-icon" aria-hidden="true"></div>
				<span id="{{ data.id }}-live-search-desc" class="screen-reader-text"><?php _e( 'The search results will be updated as you type.' ); ?></span>
			</div>
		<# } #>
