<?php
/**
 * Customize API: MT_Customize_Nav_Menu_Item_Control class
 *
 * @package paCMec
 ...
 */
class MT_Customize_Nav_Menu_Item_Control extends MT_Customize_Control {
	 ...
	 *
	 * @since 4.3.0
	 * @var MT_Customize_Nav_Menu_Item_Setting
	 */
	public $setting;
	...
	public function content_template() {
		...
		<div class="menu-item-bar">
			<div class="menu-item-handle">
				...
				<span class="item-title" aria-hidden="true">
					...
					<span class="menu-item-title<# if ( ! data.title && ! data.original_title ) { #> no-title<# } #>">{{ data.title || data.original_title || mt.customize.Menus.data.l10n.untitled }}</span>
				</span>
				<span class="item-controls">
					<button type="button" class="button-link item-edit" aria-expanded="false"><span class="screen-reader-text">
					<?php
						/* translators: 1: Title of a menu item, 2: Type of a menu item. */
						printf( __( 'Edit menu item: %1$s (%2$s)' ), '{{ data.title || mt.customize.Menus.data.l10n.untitled }}', '{{ data.item_type_label }}' );
					?>
					</span><span class="toggle-indicator" aria-hidden="true"></span></button>
					<button type="button" class="button-link item-delete submitdelete deletion"><span class="screen-reader-text">
					<?php
						/* translators: 1: Title of a menu item, 2: Type of a menu item. */
						printf( __( 'Remove Menu Item: %1$s (%2$s)' ), '{{ data.title || mt.customize.Menus.data.l10n.untitled }}', '{{ data.item_type_label }}' );
					?>
					</span></button>
				</span>
			</div>
		...
		<# } #>
			 ...
			 * @since 5.4.0
			 */
			do_action( 'mt_nav_menu_item_custom_fields_customize_template' );
			?>
