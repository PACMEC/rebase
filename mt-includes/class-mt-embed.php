...
class MT_Embed {
	...
	public function __construct() {
		// Hack to get the [embed] shortcode to run before mtautop().
	 ...
	 * an Ajax request that will call MT_Embed::cache_oembed().
	 */
...
<script type="text/javascript">
	 ...
	 *
	 * Do not use this function directly, use mt_embed_register_handler() instead.
	 *
	 ...
	 *
	 * Do not use this function directly, use mt_embed_unregister_handler() instead.
	 *
	...
	public function get_embed_handler_html( $attr, $url ) {
		$rawattr = $attr;
		$attr    = mt_parse_args( $attr, mt_embed_defaults( $url ) );
	 ...
	 * will be given to the MT_oEmbed class.
	 *
	 * @param array  $attr {
	 *     Shortcode attributes. Optional.
	 *
	...
	public function shortcode( $attr, $url = '' ) {
		...
		// Potentially set by MT_Embed::cache_oembed().
		...
		if ( $post_ID ) {
			...
			$has_kses = false !== has_filter( 'content_save_pre', 'mt_filter_post_kses' );
		...
		if ( $html ) {
			/** This filter is documented in mt-includes/class-mt-embed.php */
	...
	/**
	 * Passes any unlinked URLs that are on their own line to MT_Embed::shortcode() for potential embedding.
	 *
	 * @see MT_Embed::autoembed_callback()
	 *
	...
	public function autoembed( $content ) {
		...
		$content = mt_replace_in_html_tags( $content, array( "\n" => '<!-- mt-line-break -->' ) );
		...
		return str_replace( '<!-- mt-line-break -->', "\n", $content );
	}

	/**
	 * Callback function for MT_Embed::autoembed().
	 *
	...
	public function find_oembed_post_id( $cache_key ) {
		...
		$oembed_post_id = mt_cache_get( $cache_key, $cache_group );

		if ( $oembed_post_id && 'oembed_cache' === get_post_type( $oembed_post_id ) ) {
			return $oembed_post_id;
		}

		$oembed_post_query = new MT_Query(
			array(
				'post_type'              => 'oembed_cache',
				'post_status'            => 'publish',
				'name'                   => $cache_key,
		...
		if ( ! empty( $oembed_post_query->posts ) ) {
			...
			mt_cache_set( $cache_key, $oembed_post_id, $cache_group );
