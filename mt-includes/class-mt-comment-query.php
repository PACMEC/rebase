<?php
/**
 * Comment API: MT_Comment_Query class
 *
 * @package paCMec
 * @subpackage Comments
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see MT_Comment_Query::__construct() for accepted arguments.
 */
class MT_Comment_Query {
	 ...
	 *                                                   See MT_Meta_Query. Default empty.
	 ...
	 *                                                   stored in a `children` property on the `MT_Comment` object.
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value'                => '',
			'meta_query'                => '',
			'date_query'                => null, // See MT_Date_Query.
			'hierarchical'              => false,
			'cache_domain'              => 'core',
	 ...
	 *
	 * @since 4.2.0 Extracted from MT_Comment_Query::query().
	 *
	 * @param string|array $query MT_Comment_Query arguments. See MT_Comment_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$this->query_vars = mt_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.2.0
		 *
		 * @param MT_Comment_Query $this The MT_Comment_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_comment_query', array( &$this ) );
	}

	/**
	 * Sets up the paCMec query for retrieving comments.
	 *
	 * @since 3.1.0
	...
	public function get_comments() {
		 ...
		 * @since 3.1.0
		 *
		 * @param MT_Comment_Query $this Current instance of MT_Comment_Query (passed by reference).
		 */
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$this->meta_query_clauses = $this->meta_query->get_sql( 'comment', $mtdb->comments, 'comment_ID', $this );
		}
		 ...
		 *
		 * Return a non-null value to bypass paCMec' default comment queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of MT_Comment objects.
		 *
		 * Note that if the filter returns an array of comment data, it will be assigned
		 * to the `comments` property of the current MT_Comment_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_comments` and `max_num_pages` properties of the MT_Comment_Query object,
		 * passed to the filter by reference. If MT_Comment_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.3.0
		 * @since 5.6.0 The returned array of comment data is assigned to the `comments` property
		 *              of the current MT_Comment_Query instance.
		 *
		 * @param array|int|null   $comment_data Return an array of comment data to short-circuit MT's comment query,
		 *                                       the comment count as an integer if `$this->query_vars['count']` is set,
		 *                                       or null to allow MT to run its normal queries.
		 * @param MT_Comment_Query $query        The MT_Comment_Query instance, passed by reference.
		 */
		 ...
		 */
		$_args = mt_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) );
		unset( $_args['fields'] );

		$key          = md5( serialize( $_args ) );
		$last_changed = mt_cache_get_last_changed( 'comment' );

		$cache_key   = "get_comments:$key:$last_changed";
		$cache_value = mt_cache_get( $cache_key, 'comment' );
		if ( false === $cache_value ) {
			...
			mt_cache_add( $cache_key, $cache_value, 'comment' );
		} else {
		 ...
		 * @since 3.1.0
		 *
		 * @param MT_Comment[]     $_comments An array of comments.
		 * @param MT_Comment_Query $query     Current instance of MT_Comment_Query (passed by reference).
		 */
		$_comments = apply_filters_ref_array( 'the_comments', array( $_comments, &$this ) );

		// Convert to MT_Comment instances.
	...
	protected function get_comment_ids() {
		...
		$statuses       = mt_parse_list( $this->query_vars['status'] );
		...
		if ( ! in_array( 'any', $statuses, true ) ) {
			foreach ( $statuses as $status ) {
				switch ( $status ) {
					...
					default:
						$status_clauses[] = $mtdb->prepare( 'comment_approved = %s', $status );
						break;
				}
			}
		...
		if ( ! empty( $this->query_vars['include_unapproved'] ) ) {
			$include_unapproved = mt_parse_list( $this->query_vars['include_unapproved'] );
			...
			foreach ( $include_unapproved as $unapproved_identifier ) {
				...
				if ( is_numeric( $unapproved_identifier ) ) {
					$approved_clauses[] = $mtdb->prepare( "( user_id = %d AND comment_approved = '0' )", $unapproved_identifier );
				} else {
					// Otherwise we match against email addresses.
					if ( ! empty( $_GET['unapproved'] ) && ! empty( $_GET['moderation-hash'] ) ) {
						// Only include requested comment.
						$approved_clauses[] = $mtdb->prepare( "( comment_author_email = %s AND comment_approved = '0' AND comment_ID = %d )", $unapproved_identifier, (int) $_GET['unapproved'] );
					} else {
						// Include all of the author's unapproved comments.
						$approved_clauses[] = $mtdb->prepare( "( comment_author_email = %s AND comment_approved = '0' )", $unapproved_identifier );
					}
				}
			}
		}
		...
		if ( in_array( $this->query_vars['orderby'], array( 'none', array(), false ), true ) ) {
			...
			if ( empty( $orderby_array ) ) {
				$orderby_array[] = "$mtdb->comments.comment_date_gmt $order";
			}
			...
			if ( ! $found_orderby_comment_id ) {
				...
				$orderby_array[] = "$mtdb->comments.comment_ID $comment_id_order";
			}

			$orderby = implode( ', ', $orderby_array );
		} else {
			$orderby = "$mtdb->comments.comment_date_gmt $order";
		}
		...
		if ( $this->query_vars['count'] ) {
			...
			$fields = "$mtdb->comments.comment_ID";
		}

		$post_id = absint( $this->query_vars['post_id'] );
		if ( ! empty( $post_id ) ) {
			$this->sql_clauses['where']['post_id'] = $mtdb->prepare( 'comment_post_ID = %d', $post_id );
		}

		// Parse comment IDs for an IN clause.
		if ( ! empty( $this->query_vars['comment__in'] ) ) {
			$this->sql_clauses['where']['comment__in'] = "$mtdb->comments.comment_ID IN ( " . implode( ',', mt_parse_id_list( $this->query_vars['comment__in'] ) ) . ' )';
		}

		// Parse comment IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['comment__not_in'] ) ) {
			$this->sql_clauses['where']['comment__not_in'] = "$mtdb->comments.comment_ID NOT IN ( " . implode( ',', mt_parse_id_list( $this->query_vars['comment__not_in'] ) ) . ' )';
		}

		// Parse comment parent IDs for an IN clause.
		if ( ! empty( $this->query_vars['parent__in'] ) ) {
			$this->sql_clauses['where']['parent__in'] = 'comment_parent IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['parent__in'] ) ) . ' )';
		}

		// Parse comment parent IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['parent__not_in'] ) ) {
			$this->sql_clauses['where']['parent__not_in'] = 'comment_parent NOT IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['parent__not_in'] ) ) . ' )';
		}

		// Parse comment post IDs for an IN clause.
		if ( ! empty( $this->query_vars['post__in'] ) ) {
			$this->sql_clauses['where']['post__in'] = 'comment_post_ID IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['post__in'] ) ) . ' )';
		}

		// Parse comment post IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post__not_in'] ) ) {
			$this->sql_clauses['where']['post__not_in'] = 'comment_post_ID NOT IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['post__not_in'] ) ) . ' )';
		}

		if ( '' !== $this->query_vars['author_email'] ) {
			$this->sql_clauses['where']['author_email'] = $mtdb->prepare( 'comment_author_email = %s', $this->query_vars['author_email'] );
		}

		if ( '' !== $this->query_vars['author_url'] ) {
			$this->sql_clauses['where']['author_url'] = $mtdb->prepare( 'comment_author_url = %s', $this->query_vars['author_url'] );
		}

		if ( '' !== $this->query_vars['karma'] ) {
			$this->sql_clauses['where']['karma'] = $mtdb->prepare( 'comment_karma = %d', $this->query_vars['karma'] );
		}
		...
		foreach ( $raw_types as $operator => $_raw_types ) {
			...
			foreach ( $_raw_types as $type ) {
				switch ( $type ) {
					...
					default:
						$comment_types[ $operator ][] = $mtdb->prepare( '%s', $type );
						break;
				}
			}
		...
		if ( '' !== $parent ) {
			$this->sql_clauses['where']['parent'] = $mtdb->prepare( 'comment_parent = %d', $parent );
		}

		if ( is_array( $this->query_vars['user_id'] ) ) {
			$this->sql_clauses['where']['user_id'] = 'user_id IN (' . implode( ',', array_map( 'absint', $this->query_vars['user_id'] ) ) . ')';
		} elseif ( '' !== $this->query_vars['user_id'] ) {
			$this->sql_clauses['where']['user_id'] = $mtdb->prepare( 'user_id = %d', $this->query_vars['user_id'] );
		}
		...
		$plucked          = mt_array_slice_assoc( $this->query_vars, array( 'post_author', 'post_name', 'post_parent' ) );
		...
		if ( ! empty( $post_fields ) ) {
			...
			foreach ( $post_fields as $field_name => $field_value ) {
				...
				$this->sql_clauses['where'][ $field_name ] = $mtdb->prepare( " {$mtdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $field_value );
			}
		}
		...
		foreach ( array( 'post_status', 'post_type' ) as $field_name ) {
			...
			if ( ! empty( $this->query_vars[ $field_name ] ) ) {
				...
				// phpcs:ignore paCMec.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				$this->sql_clauses['where'][ $field_name ] = $mtdb->prepare( " {$mtdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $q_values );
			}
		}

		// Comment author IDs for an IN clause.
		if ( ! empty( $this->query_vars['author__in'] ) ) {
			$this->sql_clauses['where']['author__in'] = 'user_id IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['author__in'] ) ) . ' )';
		}

		// Comment author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['author__not_in'] ) ) {
			$this->sql_clauses['where']['author__not_in'] = 'user_id NOT IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['author__not_in'] ) ) . ' )';
		}

		// Post author IDs for an IN clause.
		if ( ! empty( $this->query_vars['post_author__in'] ) ) {
			$join_posts_table                              = true;
			$this->sql_clauses['where']['post_author__in'] = 'post_author IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['post_author__in'] ) ) . ' )';
		}

		// Post author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post_author__not_in'] ) ) {
			$join_posts_table                                  = true;
			$this->sql_clauses['where']['post_author__not_in'] = 'post_author NOT IN ( ' . implode( ',', mt_parse_id_list( $this->query_vars['post_author__not_in'] ) ) . ' )';
		}
		...
		if ( $join_posts_table ) {
			$join .= "JOIN $mtdb->posts ON $mtdb->posts.ID = $mtdb->comments.comment_post_ID";
		}
		...
		if ( ! empty( $this->meta_query_clauses ) ) {
			...
			if ( ! $this->query_vars['count'] ) {
				$groupby = "{$mtdb->comments}.comment_ID";
			}
		}

		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			$this->date_query                         = new MT_Date_Query( $this->query_vars['date_query'], 'comment_date' );
		...
		$this->sql_clauses['from']    = "FROM $mtdb->comments $join";
		...
		if ( $this->query_vars['count'] ) {
			return (int) $mtdb->get_var( $this->request );
		} else {
			$comment_ids = $mtdb->get_col( $this->request );
	...
	private function set_found_comments() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param MT_Comment_Query $comment_query        The `MT_Comment_Query` instance.
			 */
			$found_comments_query = apply_filters( 'found_comments_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_comments = (int) $mtdb->get_var( $found_comments_query );
		}
	}
	 ...
	 * @global mtdb $mtdb paCMec database abstraction object.
	 *
	 * @param MT_Comment[] $comments Array of top-level comments whose descendants should be filled in.
	 * @return array
	 */
	protected function fill_descendants( $comments ) {
		...
		$levels = array(
			0 => mt_list_pluck( $comments, 'comment_ID' ),
		);

		$key          = md5( serialize( mt_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );
		$last_changed = mt_cache_get_last_changed( 'comment' );
		...
		do {
			...
			foreach ( $_parent_ids as $parent_id ) {
				...
				$parent_child_ids = mt_cache_get( $cache_key, 'comment' );
			...
			if ( $uncached_parent_ids ) {
				...
				foreach ( $parent_map as $parent_id => $children ) {
					$cache_key = "get_comment_child_ids:$parent_id:$key:$last_changed";
					mt_cache_set( $cache_key, $children, 'comment' );
				}
			}
	...
	protected function get_search_sql( $string, $cols ) {
		...
		$like = '%' . $mtdb->esc_like( $string ) . '%';

		$searches = array();
		foreach ( $cols as $col ) {
			$searches[] = $mtdb->prepare( "$col LIKE %s", $like );
		}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $this->query_vars['meta_key'] === $orderby || 'meta_value' === $orderby ) {
			$parsed = "$mtdb->commentmeta.meta_value";
		} elseif ( 'meta_value_num' === $orderby ) {
			$parsed = "$mtdb->commentmeta.meta_value+0";
		} elseif ( 'comment__in' === $orderby ) {
			$comment__in = implode( ',', array_map( 'absint', $this->query_vars['comment__in'] ) );
			$parsed      = "FIELD( {$mtdb->comments}.comment_ID, $comment__in )";
