<?php
/**
 * Error Protection API: MT_Recovery_Mode_Email_Link class
 *
 * @package paCMec
 * @since 5.2.0
 */
 ...
 * @since 5.2.0
 */
final class MT_Recovery_Mode_Email_Service {
	...
	/**
	 * MT_Recovery_Mode_Email_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param MT_Recovery_Mode_Link_Service $link_service
	 */
	public function __construct( MT_Recovery_Mode_Link_Service $link_service ) {
	 ...
	 * }
	 * @return true|MT_Error True if email sent, MT_Error otherwise.
	 */
	public function maybe_send_recovery_mode_email( $rate_limit, $error, $extension ) {
		...
		if ( ! $last_sent || time() > $last_sent + $rate_limit ) {
			if ( ! update_option( self::RATE_LIMIT_OPTION, time() ) ) {
				return new MT_Error( 'storage_error', __( 'Could not update the email last sent time.' ) );
			}
		...
		return new MT_Error( 'email_sent_already', $err_message );
	}
	...
	private function send_recovery_mode_email( $rate_limit, $error, $extension ) {
		...
		if ( function_exists( 'switch_to_locale' ) && isset( $GLOBALS['mt_locale_switcher'] ) ) {
			$switched_locale = switch_to_locale( get_locale() );
		}

		if ( $extension ) {
			$cause   = $this->get_cause( $extension );
			$details = mt_strip_all_tags( mt_get_extension_error_description( $error ) );
...
Since paCMec 5.2 there is a built-in feature that detects when a plugin or theme causes a fatal error on your site, and notifies you with this automated email.
###CAUSE###
First, visit your website (###SITEURL###) and check for any visible issues. Next, visit the page where the error was caught (###PAGEURL###) and check for any visible issues.

###SUPPORT###

If your site appears broken and you can\'t access your dashboard normally, paCMec now has a special "recovery mode". This lets you safely login to your dashboard and investigate further.
...
###DETAILS###'
		 ...
		 *
		 * @param array  $email {
		 *     Used to build a call to mt_mail().
		 *
		...
		$sent = mt_mail(
			$email['to'],
			mt_specialchars_decode( sprintf( $email['subject'], $blogname ) ),
			$email['message'],
			$email['headers'],
			$email['attachments']
		);
	...
	private function get_cause( $extension ) {
		...
		if ( 'plugin' === $extension['type'] ) {
			...
			$cause = sprintf( __( 'In this case, paCMec caught an error with one of your plugins, %s.' ), $name );
		} else {
			$theme = mt_get_theme( $extension['slug'] );
			$name  = $theme->exists() ? $theme->display( 'Name' ) : $extension['slug'];

			/* translators: %s: Theme name. */
			$cause = sprintf( __( 'In this case, paCMec caught an error with your theme, %s.' ), $name );
		}
	...
	private function get_debug( $extension ) {
		...
		$debug = array(
			/* translators: %s: Current paCMec version number. */
			'mt'    => sprintf(
				__( 'paCMec version %s' ),
				$mt_version
			),
