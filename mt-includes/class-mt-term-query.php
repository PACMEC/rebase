...
/**
 * Taxonomy API: MT_Term_Query class.
 *
 * @package paCMec
 * @subpackage Taxonomy
 * @since 4.6.0
 ...
 * @since 4.6.0
 *
 * @see MT_Term_Query::__construct() for accepted arguments.
 */
class MT_Term_Query {
	 ...
	 *                                                - 'all' Returns an array of complete term objects (`MT_Term[]`).
	 *                                                - 'all_with_object_id' Returns an array of term objects
	 *                                                  with the 'object_id' param (`MT_Term[]`). Works only
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query MT_Term_Query arguments. See MT_Term_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$query = mt_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.6.0
		 *
		 * @param MT_Term_Query $this Current instance of MT_Term_Query.
		 */
	 ...
	 * MT_Term_Query::get_terms() for details.
	 *
	 * @since 4.6.0
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 * @return MT_Term[]|int[]|string[]|string Array of terms, or number of terms as numeric string
	 ...
	 *
	 * The following will result in an array of `MT_Term` objects being returned:
	 *
	 *   - 'all'
	 *   - 'all_with_object_id'
	 *
	 ...
	 *   - 'tt_ids'
	 *
	 * In all cases, a `MT_Error` object will be returned if an invalid taxonomy is used.
	 *
	 * @since 4.6.0
	 *
	...
	public function get_terms() {
		 ...
		 * @since 4.6.0
		 *
		 * @param MT_Term_Query $this Current instance of MT_Term_Query (passed by reference).
		 */
		...
		if ( ! empty( $include ) ) {
			$exclude      = '';
			$exclude_tree = '';
			$inclusions   = implode( ',', mt_parse_id_list( $include ) );
		}
		...
		if ( ! empty( $exclude_tree ) ) {
			$exclude_tree      = mt_parse_id_list( $exclude_tree );
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( mt_parse_id_list( $exclude ), $exclusions );
		}
		...
		if ( ! empty( $args['term_taxonomy_id'] ) ) {
			if ( is_array( $args['term_taxonomy_id'] ) ) {
				...
				$this->sql_clauses['where']['term_taxonomy_id'] = $mtdb->prepare( 'tt.term_taxonomy_id = %d', $args['term_taxonomy_id'] );
			}
		}

		if ( ! empty( $args['name__like'] ) ) {
			$this->sql_clauses['where']['name__like'] = $mtdb->prepare( 't.name LIKE %s', '%' . $mtdb->esc_like( $args['name__like'] ) . '%' );
		}

		if ( ! empty( $args['description__like'] ) ) {
			$this->sql_clauses['where']['description__like'] = $mtdb->prepare( 'tt.description LIKE %s', '%' . $mtdb->esc_like( $args['description__like'] ) . '%' );
		}
		...
		$join .= " INNER JOIN $mtdb->term_taxonomy AS tt ON t.term_id = tt.term_id";

		if ( ! empty( $this->query_vars['object_ids'] ) ) {
			$join .= " INNER JOIN {$mtdb->term_relationships} AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id";
		}
		...
		$this->sql_clauses['from']    = "FROM $mtdb->terms AS t $join";
		 ...
		 *
		 * Return a non-null value to bypass paCMec' default term queries.
		 *
		 * @since 5.3.0
		 *
		 * @param array|null    $terms Return an array of term data to short-circuit MT's term query,
		 *                             or null to allow MT queries to run normally.
		 * @param MT_Term_Query $query The MT_Term_Query instance, passed by reference.
		 */
		...
		$key          = md5( serialize( mt_array_slice_assoc( $args, array_keys( $this->query_var_defaults ) ) ) . serialize( $taxonomies ) . $this->request );
		...
		if ( 'count' === $_fields ) {
			$count = $mtdb->get_var( $this->request );
			mt_cache_set( $cache_key, $count, 'terms' );
			return $count;
		}

		$terms = $mtdb->get_results( $this->request );
		...
		if ( empty( $terms ) ) {
			mt_cache_add( $cache_key, array(), 'terms', DAY_IN_SECONDS );
			return array();
		}
		...
		mt_cache_add( $cache_key, $terms, 'terms', DAY_IN_SECONDS );
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', mt_parse_id_list( $this->query_vars['include'] ) );
	 ...
	 * @since 4.6.0
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to MT_Term_Query.
	...
	protected function get_search_sql( $string ) {
		...
		return $mtdb->prepare( '((t.name LIKE %s) OR (t.slug LIKE %s))', $like, $like );
	}
