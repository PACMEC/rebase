...
class MT_Block_Parser_Block {
	 ...
	 *
	 * @example "...Just <!-- mt:test /--> testing..." -> "Just testing..."
	 *
	 * @since 5.0.0
	 * @var string
	 */
...
class MT_Block_Parser_Frame {
	 ...
	 * @since 5.0.0
	 *
	 * @param MT_Block_Parser_Block $block              Full or partial block.
...
class MT_Block_Parser {
	 ...
	 *
	 * @example "Pre-text\n<!-- mt:paragraph -->This is inside a block!<!-- /mt:paragraph -->"
	 *
	 * @since 5.0.0
	 * @var string
	 */
		...
		switch ( $token_type ) {
			...
			case 'void-block':
				...
				if ( 0 === $stack_depth ) {
					...
					$this->output[] = (array) new MT_Block_Parser_Block( $block_name, $attrs, array(), '', array() );
				...
				$this->add_inner_block(
					new MT_Block_Parser_Block( $block_name, $attrs, array(), '', array() ),
					$start_offset,
					$token_length
				);
				$this->offset = $start_offset + $token_length;
	...
	function next_token() {
		 ...
		 * block opener and a block closer is the leading `/` before `mt:` (and
		 * a closer has no attributes). we can trap them both and process the
		 * match back in PHP to see which one it was.
		 */
		$has_match = preg_match(
			'/<!--\s+(?P<closer>\/)?mt:(?P<namespace>[a-z][a-z0-9_-]*\/)?(?P<name>[a-z][a-z0-9_-]*)\s+(?P<attrs>{(?:(?:[^}]+|}+(?=})|(?!}\s+\/?-->).)*+)?}\s+)?(?P<void>\/)?-->/s',
			$this->document,
			$matches,
			PREG_OFFSET_CAPTURE,
			$this->offset
	 ...
	 * @return MT_Block_Parser_Block freeform block object.
	 */
	function freeform( $innerHTML ) {
		return new MT_Block_Parser_Block( null, $this->empty_attrs, array(), $innerHTML, array( $innerHTML ) );
	}
	 ...
	 * @internal
	 * @since 5.0.0
	 * @param MT_Block_Parser_Block $block        The block to add to the output.
	 * @param int                   $token_start  Byte offset into the document where the first token for the block starts.
	 * @param int                   $token_length Byte length of entire block from start of opening token to end of closing token.
	 * @param int|null              $last_offset  Last byte offset into document if continuing form earlier output.
	 */
	function add_inner_block( MT_Block_Parser_Block $block, $token_start, $token_length, $last_offset = null ) {
