...
<feed
	...
	<?php
		/** This action is documented in mt-includes/feed-atom.php */
		do_action( 'atom_ns' );
...
>
	...
	if ( is_singular() ) {
		...
		printf( ent2ncr( __( 'Comments for %s' ) ), get_mt_title_rss() );
	}
	?>
	</title>
...
while ( have_comments() ) :
	...
	<entry>
		...
		if ( ! is_singular() ) {
			...
			/** This filter is documented in mt-includes/feed.php */
		...
		if ( 0 == $comment->comment_parent ) : // This comment is top-level.
			 ...
			 * but MT doesn't create them for comments (unlike posts).
