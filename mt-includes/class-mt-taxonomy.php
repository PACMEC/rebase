<?php
/**
 * Taxonomy API: MT_Taxonomy class
 *
 * @package paCMec
 * @subpackage Taxonomy
 * @since 4.7.0
...
final class MT_Taxonomy {
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see MT_Taxonomy::get_rest_controller()}.
	 *
	 * @since 5.5.0
	...
	/**
	 * Whether terms in this taxonomy should be sorted in the order they are provided to `mt_set_object_terms()`.
	 *
	...
	/**
	 * Array of arguments to automatically use inside `mt_get_object_terms()` for this taxonomy.
	 *
	 * @since 2.6.0
	 * @var array|null
	 */
	...
	public function set_props( $object_type, $args ) {
		...
		if ( ! empty( $args['default_term'] ) ) {
			...
			$args['default_term'] = mt_parse_args(
				$args['default_term'],
				array(
					'name'        => '',
					'slug'        => '',
	...
	public function add_hooks() {
		add_filter( 'mt_ajax_add-' . $this->name, '_mt_ajax_add_hierarchical_term' );
	}
	...
	public function remove_hooks() {
		remove_filter( 'mt_ajax_add-' . $this->name, '_mt_ajax_add_hierarchical_term' );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return MT_REST_Controller|null The controller instance, or null if the taxonomy
	...
	public function get_rest_controller() {
		...
		$class = $this->rest_controller_class ? $this->rest_controller_class : MT_REST_Terms_Controller::class;
