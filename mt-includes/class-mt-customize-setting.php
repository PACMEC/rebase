<?php
/**
 * paCMec Customize Setting classes
 *
 * @package paCMec
...
class MT_Customize_Setting {
	...
	public function preview() {
		...
		switch ( $this->type ) {
			...
			default:
				/**
				 * Fires when the MT_Customize_Setting::preview() method is called for settings
				 ...
				 * @since 3.4.0
				 *
				 * @param MT_Customize_Setting $this MT_Customize_Setting instance.
				 */
	 ...
	 * that `MT_Customize_Setting::_multidimensional_preview_filter()`
	 * is called for this setting.
	 *
	 * @since 4.4.0
	 *
	 * @see MT_Customize_Manager::set_post_value()
	 * @see MT_Customize_Setting::_multidimensional_preview_filter()
	 */
	 ...
	 * @since 4.4.0
	 *
	 * @see MT_Customize_Setting::$aggregated_multidimensionals
	...
	final public function save() {
		...
		/**
		 * Fires when the MT_Customize_Setting::save() method is called.
		 *
	 ...
	 * @return string|array|null|MT_Error Sanitized value, or `null`/`MT_Error` if invalid.
	 */
	 ...
	 * @return true|MT_Error True if the input was validated, otherwise MT_Error.
	 */
	public function validate( $value ) {
		...
		if ( is_null( $value ) ) {
			return new MT_Error( 'invalid_value', __( 'Invalid value.' ) );
		}

		$validity = new MT_Error();

		/**
		 * Validates a Customize setting value.
		 *
		 * Plugins should amend the `$validity` object via its `MT_Error::add()` method.
		 *
		 * The dynamic portion of the hook name, `$this->ID`, refers to the setting ID.
		 *
		 * @since 4.6.0
		 *
		 * @param MT_Error             $validity Filtered from `true` to `MT_Error` when invalid.
		 * @param mixed                $value    Value of the setting.
		 * @param MT_Customize_Setting $setting  MT_Customize_Setting instance.
		 */
		$validity = apply_filters( "customize_validate_{$this->id}", $validity, $value, $this );

		if ( is_mt_error( $validity ) && ! $validity->has_errors() ) {
			$validity = true;
		}
		return $validity;
	}
	...
	protected function get_root_value( $default = null ) {
		...
		if ( 'option' === $this->type ) {
			...
			/*
			 * Any MT_Customize_Setting subclass implementing aggregate multidimensional
	...
	protected function update( $value ) {
		...
		if ( 'option' === $this->type || 'theme_mod' === $this->type ) {
			...
			/**
			 * Fires when the MT_Customize_Setting::update() method is called for settings
			 * not handled as theme_mods or options.
			 *
	...
	public function value() {
		...
		if ( ! $is_core_type && ! $this->is_multidimensional_aggregated ) {
			 ...
			 * @param MT_Customize_Setting $setting The setting instance.
			 */
