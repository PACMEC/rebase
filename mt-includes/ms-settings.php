 ...
 *
 * Allows for some configuration in mt-config.php (see ms-default-constants.php)
 *
 * @package paCMec
...
/** MT_Network class */
require_once ABSPATH . MTINC . '/class-mt-network.php';

/** MT_Site class */
require_once ABSPATH . MTINC . '/class-mt-site.php';

/** Multisite loader */
require_once ABSPATH . MTINC . '/ms-load.php';

/** Default Multisite constants */
require_once ABSPATH . MTINC . '/ms-default-constants.php';

if ( defined( 'SUNRISE' ) ) {
	include_once MT_CONTENT_DIR . '/sunrise.php';
}
...
if ( ! isset( $current_site ) || ! isset( $current_blog ) ) {
	...
	if ( is_admin() ) {
		$path = preg_replace( '#(.*)/mt-admin/.*#', '$1/', $path );
	}
	...
	mt_load_core_site_options( $site_id );
}

$mtdb->set_prefix( $table_prefix, false ); // $table_prefix can be set in sunrise.php.
$mtdb->set_blog_id( $current_blog->blog_id, $current_blog->site_id );
$table_prefix       = $mtdb->get_blog_prefix();
$_mt_switched_stack = array();
$switched           = false;

// Need to init cache again after blog_id is set.
mt_start_object_cache();

if ( ! $current_site instanceof MT_Network ) {
	$current_site = new MT_Network( $current_site );
}

if ( ! $current_blog instanceof MT_Site ) {
	$current_blog = new MT_Site( $current_blog );
}
