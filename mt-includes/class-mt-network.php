 ...
 *
 * This class is most useful in paCMec multi-network installations where the
...
class MT_Network {
	 ...
	 * @return MT_Network|false The network's object if found. False if not.
	 */
	public static function get_instance( $network_id ) {
		...
		$_network = mt_cache_get( $network_id, 'networks' );

		if ( false === $_network ) {
			$_network = $mtdb->get_row( $mtdb->prepare( "SELECT * FROM {$mtdb->site} WHERE id = %d LIMIT 1", $network_id ) );

			if ( empty( $_network ) || is_mt_error( $_network ) ) {
				$_network = -1;
			}

			mt_cache_add( $network_id, $_network, 'networks' );
		}
		...
		return new MT_Network( $_network );
	}

	/**
	 * Create a new MT_Network object.
	 *
	 ...
	 * @since 4.4.0
	 *
	 * @param MT_Network|object $network A network object.
	 */
	...
	private function get_main_site_id() {
		 ...
		 * @param MT_Network $network      The network object for which the main site was detected.
		 */
		...
		if ( $site->domain === $this->domain && $site->path === $this->path ) {
			...
			$main_site_id = mt_cache_get( $cache_key, 'site-options' );
			if ( false === $main_site_id ) {
				...
				mt_cache_add( $cache_key, $main_site_id, 'site-options' );
			}
		}
	...
	public static function get_by_path( $domain = '', $path = '', $segments = null ) {
		...
		if ( mt_using_ext_object_cache() ) {
			$using_paths = mt_cache_get( 'networks_have_paths', 'site-options' );
			if ( false === $using_paths ) {
				...
				mt_cache_add( 'networks_have_paths', $using_paths, 'site-options' );
			}
		}
		 ...
		 * an object from mt_get_network().
		 *
		 * @since 3.9.0
		 *
		 * @param null|false|MT_Network $network  Network value to return by path. Default null
