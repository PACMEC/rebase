<?php
/**
 * Error Protection API: MT_Recovery_Mode_Link_Handler class
 *
 * @package paCMec
 * @since 5.2.0
 */
 ...
 * @since 5.2.0
 */
class MT_Recovery_Mode_Link_Service {
	...
	/**
	 * MT_Recovery_Mode_Link_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param MT_Recovery_Mode_Cookie_Service $cookie_service Service to handle setting the recovery mode cookie.
	 * @param MT_Recovery_Mode_Key_Service    $key_service    Service to handle generating recovery mode keys.
	 */
	public function __construct( MT_Recovery_Mode_Cookie_Service $cookie_service, MT_Recovery_Mode_Key_Service $key_service ) {
	...
	/**
	 * Enters recovery mode when the user hits mt-login.php with a valid recovery mode link.
	 *
	 * @since 5.2.0
	 *
	...
	public function handle_begin_link( $ttl ) {
		...
		$url = add_query_arg( 'action', self::LOGIN_ACTION_ENTERED, mt_login_url() );
		mt_redirect( $url );
		die;
	}
