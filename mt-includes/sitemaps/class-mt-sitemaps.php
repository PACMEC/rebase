...
class MT_Sitemaps {
	 ...
	 * @since 5.5.0
	 *
	 * @var MT_Sitemaps_Renderer
	 */
	public $renderer;

	/**
	 * MT_Sitemaps constructor.
	 *
	 * @since 5.5.0
	 */
	public function __construct() {
		$this->registry = new MT_Sitemaps_Registry();
		$this->renderer = new MT_Sitemaps_Renderer();
		$this->index    = new MT_Sitemaps_Index( $this->registry );
	}
	...
	public function sitemaps_enabled() {
		 ...
		 *
		 * @see MT_Sitemaps::register_rewrites()
		 *
		 * @since 5.5.0
		 *
		 ...
		 */
		return (bool) apply_filters( 'mt_sitemaps_enabled', $is_enabled );
	}
	...
	public function register_sitemaps() {
		$providers = array(
			'posts'      => new MT_Sitemaps_Posts(),
			'taxonomies' => new MT_Sitemaps_Taxonomies(),
			'users'      => new MT_Sitemaps_Users(),
		);

		/* @var MT_Sitemaps_Provider $provider */
	...
	public function register_rewrites() {
		...
		add_rewrite_rule( '^mt-sitemap\.xml$', 'index.php?sitemap=index', 'top' );

		// Register rewrites for the XSL stylesheet.
		add_rewrite_tag( '%sitemap-stylesheet%', '([^?]+)' );
		add_rewrite_rule( '^mt-sitemap\.xsl$', 'index.php?sitemap-stylesheet=sitemap', 'top' );
		add_rewrite_rule( '^mt-sitemap-index\.xsl$', 'index.php?sitemap-stylesheet=index', 'top' );

		// Register routes for providers.
		add_rewrite_rule(
			'^mt-sitemap-([a-z]+?)-([a-z\d_-]+?)-(\d+?)\.xml$',
			'index.php?sitemap=$matches[1]&sitemap-subtype=$matches[2]&paged=$matches[3]',
			'top'
		);
		add_rewrite_rule(
			'^mt-sitemap-([a-z]+?)-(\d+?)\.xml$',
	...
	public function render_sitemaps() {
		...
		if ( $stylesheet_type ) {
			$stylesheet = new MT_Sitemaps_Stylesheet();
	...
	/**
	 * Redirects a URL to the mt-sitemap.xml
	 *
	 * @since 5.5.0
	 *
	 * @param bool     $bypass Pass-through of the pre_handle_404 filter value.
	 * @param MT_Query $query  The MT_Query object.
	...
	public function redirect_sitemapxml( $bypass, $query ) {
		...
		if ( 'sitemap-xml' === $query->get( 'pagename' )
			...
			mt_safe_redirect( $this->index->get_index_url() );
			exit();
		}
