 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0.0', MTINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'paCMec/' . $GLOBALS['mt_version']);
...
/**
 * Retrieve URL headers and content using MT HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = mt_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_mt_error($resp) ) {
	...
	// Also note, MT_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( mt_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = mt_remote_retrieve_response_code( $resp );
	$response->response_code = mt_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = mt_remote_retrieve_body( $resp );
...
class RSSCache {
	...
	function __construct( $base = '', $age = '' ) {
		$this->BASE_CACHE = MT_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function mt_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with mt_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
