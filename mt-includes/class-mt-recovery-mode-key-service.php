<?php
/**
 * Error Protection API: MT_Recovery_Mode_Key_Service class
 *
 * @package paCMec
 * @since 5.2.0
 */
 ...
 * @since 5.2.0
 */
final class MT_Recovery_Mode_Key_Service {
	...
	public function generate_recovery_mode_token() {
		return mt_generate_password( 22, false );
	}
	...
	public function generate_and_store_recovery_mode_key( $token ) {
		...
		$key = mt_generate_password( 22, false );
		...
		$hashed = $mt_hasher->HashPassword( $key );
	...
	public function validate_recovery_mode_key( $token, $key, $ttl ) {
		...
		if ( ! isset( $records[ $token ] ) ) {
			return new MT_Error( 'token_not_found', __( 'Recovery Mode not initialized.' ) );
		}
		...
		if ( ! is_array( $record ) || ! isset( $record['hashed_key'], $record['created_at'] ) ) {
			return new MT_Error( 'invalid_recovery_key_format', __( 'Invalid recovery key format.' ) );
		}

		if ( ! mt_check_password( $key, $record['hashed_key'] ) ) {
			return new MT_Error( 'hash_mismatch', __( 'Invalid recovery key.' ) );
		}

		if ( time() > $record['created_at'] + $ttl ) {
			return new MT_Error( 'key_expired', __( 'Recovery key expired.' ) );
		}
