...
final class MT_Post_Type {
	 ...
	 * to use a font icon, e.g. 'dashicons-chart-pie'. Pass 'none' to leave div.mt-menu-image empty
	 ...
	 *
	 * Custom controllers must extend MT_REST_Controller.
	 *
	 * @since 4.7.4
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see MT_Post_Type::get_rest_controller()}.
	 *
	 * @since 5.3.0
	 * @var MT_REST_Controller $rest_controller
	 */
	...
	public function add_rewrite_rules() {
		...
		if ( false !== $this->query_var && $mt && is_post_type_viewable( $this ) ) {
			$mt->add_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite && ( is_admin() || get_option( 'permalink_structure' ) ) ) {
			...
			if ( $this->has_archive ) {
				...
				if ( $this->rewrite['with_front'] ) {
					$archive_slug = substr( $mt_rewrite->front, 1 ) . $archive_slug;
				} else {
					$archive_slug = $mt_rewrite->root . $archive_slug;
				}

				add_rewrite_rule( "{$archive_slug}/?$", "index.php?post_type=$this->name", 'top' );
				if ( $this->rewrite['feeds'] && $mt_rewrite->feeds ) {
					$feeds = '(' . trim( implode( '|', $mt_rewrite->feeds ) ) . ')';
				...
				if ( $this->rewrite['pages'] ) {
					add_rewrite_rule( "{$archive_slug}/{$mt_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$this->name" . '&paged=$matches[1]', 'top' );
				}
			}
	...
	public function remove_supports() {
		...
		unset( $_mt_post_type_features[ $this->name ] );
	}
	...
	public function remove_rewrite_rules() {
		global $mt, $mt_rewrite, $post_type_meta_caps;

		// Remove query var.
		if ( false !== $this->query_var ) {
			$mt->remove_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite ) {
			...
			foreach ( $mt_rewrite->extra_rules_top as $regex => $query ) {
				if ( false !== strpos( $query, "index.php?post_type=$this->name" ) ) {
					unset( $mt_rewrite->extra_rules_top[ $regex ] );
				}
			}
		}
	 ...
	 * @since 5.3.0
	 *
	 * @return MT_REST_Controller|null The controller instance, or null if the post type
	...
	public function get_rest_controller() {
		...
		$class = $this->rest_controller_class ? $this->rest_controller_class : MT_REST_Posts_Controller::class;

		if ( ! class_exists( $class ) ) {
			return null;
		}

		if ( ! is_subclass_of( $class, MT_REST_Controller::class ) ) {
			return null;
		}
