<?php
/**
 * REST API: MT_REST_Meta_Fields class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 */
abstract class MT_REST_Meta_Fields {
	 ...
	 * @param MT_REST_Request $request Current request object.
	 ...
	 * @param int   $object_id Object ID to fetch meta for.
	 * @return null|MT_Error Null on success, MT_Error object on failure.
	 */
		...
		foreach ( $fields as $meta_key => $args ) {
			...
			if ( is_null( $value ) || ( array() === $value && ! $args['single'] ) ) {
				...
				if ( $args['single'] ) {
					...
					if ( is_mt_error( rest_validate_value_from_schema( $current, $args['schema'] ) ) ) {
						return new MT_Error(
	 ...
	 * @return true|MT_Error True if meta field is deleted, MT_Error otherwise.
	 */
	protected function delete_meta_value( $object_id, $meta_key, $name ) {
		...
		if ( ! delete_metadata( $meta_type, $object_id, mt_slash( $meta_key ) ) ) {
			return new MT_Error(
				'rest_meta_database_error',
				__( 'Could not delete meta value from database.' ),
				array(
					'key'    => $name,
					'status' => MT_Http::INTERNAL_SERVER_ERROR,
				)
			);
		}
	 ...
	 * @return true|MT_Error True if meta fields are updated, MT_Error otherwise.
	 */
	protected function update_multi_meta_value( $object_id, $meta_key, $name, $values ) {
		...
		foreach ( $to_remove as $value ) {
			if ( ! delete_metadata( $meta_type, $object_id, mt_slash( $meta_key ), mt_slash( $value ) ) ) {
				return new MT_Error(
					'rest_meta_database_error',
					/* translators: %s: Custom field key. */
		...
		foreach ( $to_add as $value ) {
			if ( ! add_metadata( $meta_type, $object_id, mt_slash( $meta_key ), mt_slash( $value ) ) ) {
				return new MT_Error(
					'rest_meta_database_error',
					/* translators: %s: Custom field key. */
					sprintf( __( 'Could not update the meta value of %s in database.' ), $meta_key ),
	 ...
	 * @return true|MT_Error True if the meta field was updated, MT_Error otherwise.
	 */
	protected function update_meta_value( $object_id, $meta_key, $name, $value ) {
		...
		if ( ! update_metadata( $meta_type, $object_id, mt_slash( $meta_key ), mt_slash( $value ) ) ) {
			return new MT_Error(
				'rest_meta_database_error',
				/* translators: %s: Custom field key. */
				sprintf( __( 'Could not update the meta value of %s in database.' ), $meta_key ),
	...
	public static function prepare_value( $value, $request, $args ) {
		...
		if ( is_mt_error( rest_validate_value_from_schema( $value, $schema ) ) ) {
			return null;
		}
