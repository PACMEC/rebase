<?php
/**
 * REST API: MT_REST_Request class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.4.0
 ...
 *
 * Note: When using features provided by ArrayAccess, be aware that paCMec deliberately
 ...
 * MT_REST_Request::get_body_params(), MT_REST_Request::get_url_params(), etc.
 *
 * @since 4.4.0
 *
 * @link https://www.php.net/manual/en/class.arrayaccess.php
 */
class MT_REST_Request implements ArrayAccess {
	...
	public function is_json_content_type() {
		...
		return isset( $content_type['value'] ) && mt_is_json_media_type( $content_type['value'] );
	}

	/**
	 * Retrieves the parameter priority order.
	 *
	 * Used when checking parameters in MT_REST_Request::get_param().
	 *
	 * @since 4.4.0
	 *
	...
	protected function get_parameter_order() {
		 ...
		 *
		 * The order affects which parameters are checked when using MT_REST_Request::get_param()
		 * and family. This acts similarly to PHP's `request_order` setting.
		 *
		 * @since 4.4.0
		 *
		 * @param string[]        $order Array of types to check, in order of priority.
		 * @param MT_REST_Request $this  The request object.
		 */
	 ...
	 * @return true|MT_Error True if the JSON data was passed or no JSON data was provided, MT_Error if invalid JSON was passed.
	 */
	protected function parse_json_params() {
		...
		if ( null === $params && JSON_ERROR_NONE !== json_last_error() ) {
			...
			$error_data = array(
				'status'             => MT_Http::BAD_REQUEST,
				'json_error_code'    => json_last_error(),
				'json_error_message' => json_last_error_msg(),
			);

			return new MT_Error( 'rest_invalid_json', __( 'Invalid JSON body passed.' ), $error_data );
		}
	 ...
	 * @since 4.4.0
	 *
	 * @return true|MT_Error True if parameters were sanitized, MT_Error if an error occurred during sanitization.
	 */
	public function sanitize_params() {
		...
		foreach ( $order as $type ) {
			...
			foreach ( $this->params[ $type ] as $key => $value ) {
				...
				/** @var mixed|MT_Error $sanitized_value */
	 ...
	 * @since 4.4.0
	 *
	 * @return true|MT_Error True if there are no parameters to validate or if all pass validation,
	 *                       MT_Error if required parameters are missing.
	 */
	public function has_valid_params() {
		// If JSON data was passed, check for errors.
		$json_error = $this->parse_json_params();
		if ( is_mt_error( $json_error ) ) {
			return $json_error;
		}
		...
		if ( isset( $attributes['validate_callback'] ) ) {
			...
			if ( false === $valid_check ) {
				// A MT_Error instance is preferred, but false is supported for parity with the per-arg validate_callback.
				return new MT_Error( 'rest_invalid_params', __( 'Invalid parameters.' ), array( 'status' => 400 ) );
			}
		}
	...
	/**
	 * Retrieves a MT_REST_Request object from a full URL.
	 *
	 * @since 4.5.0
	 *
	 * @param string $url URL with protocol, domain, path and query args.
	 * @return MT_REST_Request|false MT_REST_Request object on success, false on failure.
	 */
	public static function from_url( $url ) {
		...
		if ( ! empty( $bits['query'] ) ) {
			mt_parse_str( $bits['query'], $query_params );
		}
		...
		$request = false;
		if ( ! empty( $route ) ) {
			$request = new MT_REST_Request( 'GET', $route );
		 ...
		 * @since 4.5.0
		 *
		 * @param MT_REST_Request|false $request Generated request object, or false if URL
		 *                                       could not be parsed.
