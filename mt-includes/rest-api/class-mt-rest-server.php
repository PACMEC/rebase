...
/**
 * Core class used to implement the paCMec REST API server.
 *
 * @since 4.4.0
 */
class MT_REST_Server {
	 ...
	 * @since 4.4.0
	 *
	 * @return MT_Error|null MT_Error indicates unsuccessful login, null indicates successful
	...
	public function check_authentication() {
		 ...
		 *
		 * This is used to pass a MT_Error from an authentication method back to
		 * the API.
		 *
		 ...
		 * errors.
		 *
		 * A MT_Error instance can be returned if an error occurs, and this should
		 ...
		 * @since 4.4.0
		 *
		 * @param MT_Error|null|true $errors MT_Error if authentication error, null if authentication
	 ...
	 *
	 * Note: This should only be used in MT_REST_Server::serve_request(), as it
	 * cannot handle MT_Error internally. All callbacks and other internal methods
	 * should instead return a MT_Error with the data set to an array that includes
	 * a 'status' key, with the value being the HTTP status to send.
	 *
	 * @since 4.4.0
	 *
	 * @param string $code    MT_Error-style code.
	 ...
	 *
	 * @global MT_User $current_user The currently authenticated user.
	 *
	...
	public function serve_request( $path = null ) {
		/* @var MT_User|null $current_user */
		global $current_user;

		if ( $current_user instanceof MT_User && ! $current_user->exists() ) {
			/*
		...
		$expose_headers = array( 'X-MT-Total', 'X-MT-TotalPages', 'Link' );
		...
		if ( $send_no_cache_headers ) {
			foreach ( mt_get_nocache_headers() as $header => $header_value ) {
		...
		$request = new MT_REST_Request( $_SERVER['REQUEST_METHOD'], $path );

		$request->set_query_params( mt_unslash( $_GET ) );
		$request->set_body_params( mt_unslash( $_POST ) );
		$request->set_file_params( $_FILES );
		$request->set_headers( $this->get_headers( mt_unslash( $_SERVER ) ) );
		...
		// Normalize to either MT_Error or MT_REST_Response...
		$result = rest_ensure_response( $result );

		// ...then convert MT_Error across.
		if ( is_mt_error( $result ) ) {
		 ...
		 *
		 * @param MT_HTTP_Response $result  Result to send to the client. Usually a `MT_REST_Response`.
		...
		if ( ! $served ) {
			...
			$result = mt_json_encode( $result );

			$json_error_message = $this->get_json_last_error();

			if ( $json_error_message ) {
				$json_error_obj = new MT_Error(
					'rest_encode_error',
					$json_error_message,
					array( 'status' => 500 )
				);
				...
				$result = mt_json_encode( $result->data );
			}
	 ...
	 *
	 * @param MT_REST_Response $response Response object.
	 ...
	 * @since 4.4.0
	 *
	 * @param MT_REST_Response $response Response to extract links from.
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					$request = MT_REST_Request::from_url( $item['href'] );
					if ( ! $request ) {
						$embeds[] = array();
						continue;
					}
	 ...
	 * @return MT_REST_Response New response with wrapped data
	 */
	public function envelope_response( $response, $embed ) {
		 ...
		 * }
		 * @param MT_REST_Response $response Original response data.
		 */
	...
	public function get_routes( $namespace = '' ) {
		...
		if ( $namespace ) {
			$endpoints = mt_list_filter( $endpoints, array( 'namespace' => $namespace ) );
		}
		...
		foreach ( $endpoints as $route => &$handlers ) {
			...
			foreach ( $handlers as $key => &$handler ) {
				...
				$handler = mt_parse_args( $handler, $defaults );
	 ...
	 * @since 4.4.0
	 *
	 * @param MT_REST_Request $request Request to attempt dispatching.
	 * @return MT_REST_Response Response returned by the callback.
	 */
	public function dispatch( $request ) {
		...
		if ( ! is_mt_error( $error ) ) {
			...
			if ( is_mt_error( $check_required ) ) {
				$error = $check_required;
			} else {
				$check_sanitized = $request->sanitize_params();
				if ( is_mt_error( $check_sanitized ) ) {
	 ...
	 * @return array|MT_Error The route and request handler on success or a MT_Error instance if no handler was found.
	 */
	 ...
	 * @param MT_Error|null   $response The current error object if any.
	 *
	...
	protected function respond_to_request( $request, $route, $handler, $response ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_REST_Response|MT_HTTP_Response|MT_Error|mixed $response Result to send to the client.
		 *                                                                   Usually a MT_REST_Response or MT_Error.
		...
		if ( ! is_mt_error( $response ) && ! empty( $handler['permission_callback'] ) ) {
			$permission = call_user_func( $handler['permission_callback'], $request );

			if ( is_mt_error( $permission ) ) {
				$response = $permission;
			} elseif ( false === $permission || null === $permission ) {
				$response = new MT_Error(
					'rest_forbidden',
	 ...
	 * }
	 * @return MT_REST_Response The API root index data.
	 */
	public function get_index( $request ) {
		...
		$response = new MT_REST_Response( $available );
		$response->add_link( 'help', 'http://v2.mt-api.org/' );
		 ...
		 * @since 4.4.0
		 *
		 * @param MT_REST_Response $response Response data.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param MT_REST_Response $response REST API response.
	 */
	protected function add_active_theme_link_to_index( MT_REST_Response $response ) {
		...
		if ( $should_add ) {
			$theme = mt_get_theme();
			$response->add_link( 'https://api.w.org/active-theme', rest_url( 'mt/v2/themes/' . $theme->get_stylesheet() ) );
		}
	}
	 ...
	 * @since 4.4.0
	 *
	 * @param MT_REST_Request $request REST request instance.
	 * @return MT_REST_Response|MT_Error MT_REST_Response instance if the index was found,
	 *                                   MT_Error if the namespace isn't set.
	 */
	public function get_namespace_index( $request ) {
		 ...
		 *
		 * @param MT_REST_Response $response Response data.
		 * @param MT_REST_Request  $request  Request data. The namespace is passed as the 'namespace' parameter.
		 */
	 ...
	 * @since 5.6.0
	 *
	 * @param MT_REST_Request $batch_request The batch request object.
	 * @return MT_REST_Response The generated response object.
	 */
	public function serve_batch_request_v1( MT_REST_Request $batch_request ) {
		$requests = array();

		foreach ( $batch_request['requests'] as $args ) {
			$parsed_url = mt_parse_url( $args['path'] );

			if ( false === $parsed_url ) {
				$requests[] = new MT_Error( 'parse_path_failed', __( 'Could not parse the path.' ), array( 'status' => 400 ) );

				continue;
			}

			$single_request = new MT_REST_Request( isset( $args['method'] ) ? $args['method'] : 'POST', $parsed_url['path'] );

			if ( ! empty( $parsed_url['query'] ) ) {
				$query_args = null; // Satisfy linter.
				mt_parse_str( $parsed_url['query'], $query_args );
		...
		return new MT_REST_Response( array( 'responses' => $responses ), MT_Http::MULTI_STATUS );
	}
