<?php
/**
 * REST API: MT_REST_Themes_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class MT_REST_Themes_Controller extends MT_REST_Controller {
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return bool|MT_Error True if the request has read access for the item, otherwise MT_Error object.
	 */
	public function get_item_permissions_check( $request ) {
		...
		$mt_theme      = mt_get_theme( $request['stylesheet'] );
		$current_theme = mt_get_theme();

		if ( $this->is_same_theme( $mt_theme, $current_theme ) ) {
			return $this->check_read_active_theme_permission();
		}
	 ...
	 * @since 5.7.0
	 *
	 * @return bool|MT_Error Whether the theme can be read.
	 */
	...
	public function get_item( $request ) {
		...
		$data = $this->prepare_item_for_response( $mt_theme, $request );
	...
	public function get_items( $request ) {
		...
		$response->header( 'X-MT-Total', count( $themes ) );
		$response->header( 'X-MT-TotalPages', 1 );
	...
	public function prepare_item_for_response( $theme, $request ) {
		 ...
		 * @param MT_Theme         $theme    Theme object used to create response.
		 * @param MT_REST_Request  $request  Request object.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param MT_Theme $theme Theme data.
	 ...
	 * @since 5.7.0
	 *
	 * @param MT_Theme $theme_a First theme to compare.
	 * @param MT_Theme $theme_b Second theme to compare.
	 *
	 * @return bool
	 */
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				...
				'requires_mt'    => array(
					'description' => __( 'The minimum paCMec version required for the theme to work.' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'screenshot'     => array(
