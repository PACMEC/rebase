<?php
/**
 * REST API: MT_REST_Users_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MT_REST_Users_Controller extends MT_REST_Controller {

	/**
	 * Instance of a user meta fields object.
	 *
	 * @since 4.7.0
	 * @var MT_REST_User_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new MT_REST_User_Meta_Fields();
	}
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has read access, otherwise MT_Error object.
	 */
	...
	public function get_items( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param array           $prepared_args Array of arguments for MT_User_Query.
		 * @param MT_REST_Request $request       The REST API request.
		 */
		$prepared_args = apply_filters( 'rest_user_query', $prepared_args, $request );

		$query = new MT_User_Query( $prepared_args );
		...
		if ( $total_users < 1 ) {
			...
			$count_query = new MT_User_Query( $prepared_args );
			$total_users = $count_query->get_total();
		}

		$response->header( 'X-MT-Total', (int) $total_users );
	 ...
	 * @return MT_User|MT_Error True if ID is valid, MT_Error otherwise.
	 */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has read access for the item, otherwise MT_Error object.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $request['roles'] ) && ! empty( $schema['properties']['roles'] ) ) {
			...
			if ( is_mt_error( $check_permission ) ) {
		...
		if ( is_multisite() ) {
			$ret = mtmu_validate_user_signup( $user->user_login, $user->user_email );

			if ( is_mt_error( $ret['errors'] ) && $ret['errors']->has_errors() ) {
				$error = new MT_Error(
					'rest_invalid_param',
					__( 'Invalid user parameter(s).' ),
					array( 'status' => 400 )
		...
		if ( is_multisite() ) {
			$user_id = mtmu_create_user( $user->user_login, $user->user_pass, $user->user_email );
			...
			$user->ID = $user_id;
			$user_id  = mt_update_user( mt_slash( (array) $user ) );
			...
			$user_id = mt_insert_user( mt_slash( (array) $user ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_User         $user     Inserted or updated user object.
	...
	public function update_item( $request ) {
		...
		/** This action is documented in mt-includes/rest-api/endpoints/class-mt-rest-users-controller.php */
	...
	public function delete_item( $request ) {
		...
		// Include user admin functions to get access to mt_delete_user().
		require_once ABSPATH . 'mt-admin/includes/user.php';

		$result = mt_delete_user( $id, $reassign );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_User          $user     The user data.
	...
	public function prepare_item_for_response( $user, $request ) {
		 ...
		 * @param MT_User          $user     User object used to create response.
		 * @param MT_REST_Request  $request  Request object.
		 */
	 ...
	 * @return true|MT_Error True if the current user is allowed to make the role change,
	 *                       otherwise a MT_Error object.
	 */
	protected function check_role_update( $user_id, $roles ) {
		...
		foreach ( $roles as $role ) {
			...
			if ( ! isset( $mt_roles->role_objects[ $role ] ) ) {
				return new MT_Error(
			...
			$potential_role = $mt_roles->role_objects[ $role ];
	 ...
	 *
	 * Performs a couple of checks like edit_user() in mt-admin/includes/user.php.
	 *
	 * @since 4.7.0
	 *
	 ...
	 * @return string|MT_Error The sanitized username, if valid, otherwise an error.
	 */
	 ...
	 * @param MT_REST_Request $request Full details about the request.
	 * @param string          $param   The parameter name.
	 * @return string|MT_Error The sanitized password, if valid, otherwise an error.
	 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal MT_User_Query parameter.  Use the
		 * `rest_user_query` filter to set MT_User_Query arguments.
		 *
		 * @since 4.7.0
		 *
