<?php
/**
 * REST API: MT_REST_Posts_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see MT_REST_Controller
 */
class MT_REST_Posts_Controller extends MT_REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function __construct( $post_type ) {
		...
		$this->namespace = 'mt/v2';
		$obj             = get_post_type_object( $post_type );
		$this->rest_base = ! empty( $obj->rest_base ) ? $obj->rest_base : $obj->name;

		$this->meta = new MT_REST_Post_Meta_Fields( $this->post_type );
	}
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => MT_REST_Server::READABLE,
					'callback'            => array( $this, 'get_items' ),
					'permission_callback' => array( $this, 'get_items_permissions_check' ),
					'args'                => $this->get_collection_params(),
				),
				array(
					'methods'             => MT_REST_Server::CREATABLE,
					'callback'            => array( $this, 'create_item' ),
					'permission_callback' => array( $this, 'create_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( MT_REST_Server::CREATABLE ),
				),
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => MT_REST_Server::EDITABLE,
					'callback'            => array( $this, 'update_item' ),
					'permission_callback' => array( $this, 'update_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( MT_REST_Server::EDITABLE ),
				),
				array(
					'methods'             => MT_REST_Server::DELETABLE,
	 ...
	 * @since 4.7.0
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has read access, MT_Error object otherwise.
	 */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return MT_REST_Response|MT_Error Response object on success, or MT_Error object on failure.
	 */
	public function get_items( $request ) {
		 ...
		 * values are accepted as-passed, and their internal MT_Query parameter
		 ...
		 */
		foreach ( $parameter_mappings as $api_param => $mt_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$args[ $mt_param ] = $request[ $api_param ];
			}
		}
		...
		if ( isset( $registered['sticky'], $request['sticky'] ) ) {
			...
			if ( $request['sticky'] ) {
				 ...
				 * MT_Query won't return "no posts" for post__in = array()
		...
		/**
		 * Filters MT_Query arguments when querying users via the REST API.
		 *
		...
		$response->header( 'X-MT-Total', (int) $total_posts );
		$response->header( 'X-MT-TotalPages', (int) $max_pages );
	 ...
	 * @return MT_Post|MT_Error Post object if ID is valid, MT_Error otherwise.
	 */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has read access for the item, MT_Error object otherwise.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param MT_Post         $post    Post to check against.
	 * @param MT_REST_Request $request Request data to check.
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to create items, MT_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( is_mt_error( $prepared_post ) ) {
			return $prepared_post;
		}

		$prepared_post->post_type = $this->post_type;

		$post_id = mt_insert_post( mt_slash( (array) $prepared_post ), true, false );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Post         $post     Inserted or updated post object.
		 * @param MT_REST_Request $request  Request object.
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to update the item, MT_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		// Convert the post object to an array, otherwise mt_update_post() will expect non-escaped input.
		$post_id = mt_update_post( mt_slash( (array) $post ), true, false );
		...
		/** This action is documented in mt-includes/rest-api/endpoints/class-mt-rest-posts-controller.php */
		...
		// Filter is fired in MT_REST_Attachments_Controller subclass.
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to delete the item, MT_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param MT_Post $post           The Post object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			// (Note that internally this falls through to `mt_delete_post()`
			// if the Trash is disabled.)
			$result   = mt_trash_post( $id );
			$post     = get_post( $id );
			$response = $this->prepare_item_for_response( $post, $request );
		}
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Post          $post     The deleted or trashed post.
		 * @param MT_REST_Response $response The response data.
		 * @param MT_REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Prepared MT_Query arguments. Default empty array.
	 * @param MT_REST_Request $request       Optional. Full details about the request.
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $prepared_args as $key => $value ) {
			 ...
			 */
			$query_args[ $key ] = apply_filters( "rest_query_var-{$key}", $value ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
		}
		...
		// Map to proper MT_Query orderby param.
	 ...
	 *
	 * @param MT_REST_Request $request Request object.
	 * @return stdClass|MT_Error Post object or MT_Error.
	 */
	 ...
	 * @param MT_REST_Request $request The request object.
	 * @param string          $param   The parameter name.
	 * @return true|MT_Error True if the status is valid, or MT_Error if not.
	 */
	public function check_status( $status, $request, $param ) {
		if ( $request['id'] ) {
			$post = $this->get_post( $request['id'] );

			if ( ! is_mt_error( $post ) && $post->post_status === $status ) {
				return true;
			}
		}
	 ...
	 * @param MT_Post_Type $post_type   Post type.
	 * @return string|MT_Error Post status or MT_Error if lacking the proper permission.
	 */
	 ...
	 * @return bool|MT_Error Whether the post thumbnail was successfully deleted, otherwise MT_Error.
	 */
	 ...
	 * @param MT_REST_Request $request  Request.
	 * @return bool|MT_Error True if template is still valid or if the same as existing value, or false if template not supported.
	 */
	public function check_template( $template, $request ) {
		...
		// If this is a create request, get_post() will return null and mt theme will fallback to the passed post type.
		$allowed_templates = mt_get_theme()->get_page_templates( $post, $this->post_type );
	...
	public function handle_template( $template, $post_id, $validate = false ) {
		...
		if ( $validate && ! array_key_exists( $template, mt_get_theme()->get_page_templates( get_post( $post_id ) ) ) ) {
			$template = '';
		}

		update_post_meta( $post_id, '_mt_page_template', $template );
	}
	 ...
	 * @param MT_REST_Request $request The request object with post and terms data.
	 * @return null|MT_Error MT_Error on an error assigning any of the terms, otherwise null.
	 */
	protected function handle_terms( $post_id, $request ) {
		$taxonomies = mt_list_filter( get_object_taxonomies( $this->post_type, 'objects' ), array( 'show_in_rest' => true ) );
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$result = mt_set_object_terms( $post_id, $request[ $base ], $taxonomy->name );
	 ...
	 * @since 4.7.0
	 *
	 * @param MT_Post_Type|string $post_type Post type name or object.
	 ...
	 * @param MT_Post         $post    Post object.
	 * @param MT_REST_Request $request Request object.
	 * @return MT_REST_Response Response object.
	 */
	public function prepare_item_for_response( $post, $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$data[ $base ] = $terms ? array_values( mt_list_pluck( $terms, 'term_id' ) ) : array();
			}
		}
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_REST_Response $response The response object.
		 * @param MT_Post          $post     Post object.
		 * @param MT_REST_Request  $request  Request object.
		 */
	 ...
	 *
	 * By default, paCMec will show password protected posts with a title of
	...
	protected function prepare_links( $post ) {
		...
		$links = array(
			...
			'about'      => array(
				'href' => rest_url( 'mt/v2/types/' . $this->post_type ),
			),
		);
		...
		if ( in_array( $post->post_type, array( 'post', 'page' ), true ) || post_type_supports( $post->post_type, 'revisions' ) ) {
			$revisions       = mt_get_post_revisions( $post->ID, array( 'fields' => 'ids' ) );
		...
		if ( $featured_media ) {
			$image_url = rest_url( 'mt/v2/media/' . $featured_media );
		...
		if ( ! in_array( $post->post_type, array( 'attachment', 'nav_menu_item', 'revision' ), true ) ) {
			$attachments_url = rest_url( 'mt/v2/media' );
	...
	public function get_collection_params() {
		...
		$query_params['slug'] = array(
			...
			'sanitize_callback' => 'mt_parse_slug_list',
		);
		 ...
		 * collection parameter to an internal MT_Query parameter. Use the
		 * `rest_{$this->post_type}_query` filter to set MT_Query parameters.
		 *
		 * @since 4.7.0
		 *
	 ...
	 * @return array|MT_Error A list of valid statuses, otherwise MT_Error object.
	 */
	public function sanitize_post_statuses( $statuses, $request, $parameter ) {
		$statuses = mt_parse_slug_list( $statuses );

		// The default status is different in MT_REST_Attachments_Controller.
	 ...
	 * @since 5.7.0
	 *
	 * @param array           $args    MT_Query arguments.
	 * @param MT_REST_Request $request Full details about the request.
	 * @return array Updated query arguments.
	 */
	private function prepare_tax_query( array $args, MT_REST_Request $request ) {
