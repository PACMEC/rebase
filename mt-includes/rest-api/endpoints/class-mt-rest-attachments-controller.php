<?php
/**
 * REST API: MT_REST_Attachments_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see MT_REST_Posts_Controller
 */
class MT_REST_Attachments_Controller extends MT_REST_Posts_Controller {
	 ...
	 * prepares for MT_Query.
	 *
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Array of prepared arguments. Default empty array.
	 * @param MT_REST_Request $request       Optional. Request to prepare items for.
	 ...
	 * @return true|MT_Error Boolean true if the attachment may be created, or a MT_Error if not.
	 */
	public function create_item_permissions_check( $request ) {
		$ret = parent::create_item_permissions_check( $request );

		if ( ! $ret || is_mt_error( $ret ) ) {
			return $ret;
		}
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return MT_REST_Response|MT_Error Response object on success, MT_Error object on failure.
	 */
	public function create_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $attachment_id, '_mt_attachment_image_alt', sanitize_text_field( $request['alt_text'] ) );
		}
		 ...
		 * @since 5.0.0
		 *
		 * @param MT_Post         $attachment Inserted or updated attachment object.
		 * @param MT_REST_Request $request    Request object.
		 * @param bool            $creating   True when creating an attachment, false when updating.
		 */
		do_action( 'rest_after_insert_attachment', $attachment, $request, true );

		mt_after_insert_post( $attachment, false, null );
		...
		// Include media and image functions to get access to mt_generate_attachment_metadata().
	...
	protected function insert_attachment( $request ) {
		...
		// Include image functions to get access to mt_read_image_metadata().
		require_once ABSPATH . 'mt-admin/includes/image.php';
		...
		if ( empty( $attachment->post_title ) ) {
			$attachment->post_title = preg_replace( '/\.[^.]+$/', '', mt_basename( $file ) );
		}

		// $post_parent is inherited from $attachment['post_parent'].
		$id = mt_insert_attachment( mt_slash( (array) $attachment ), $file, 0, true, false );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Post         $attachment Inserted or updated attachment
		 *                                    object.
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $data['id'], '_mt_attachment_image_alt', $request['alt_text'] );
		}
		...
		/** This action is documented in mt-includes/rest-api/endpoints/class-mt-rest-attachments-controller.php */
		do_action( 'rest_after_insert_attachment', $attachment, $request, false );

		mt_after_insert_post( $attachment, true, $attachment_before );
	...
	public function post_process_item( $request ) {
		switch ( $request['action'] ) {
			case 'create-image-subsizes':
				require_once ABSPATH . 'mt-admin/includes/image.php';
				mt_update_image_subsizes( $request['id'] );
				break;
		}
	...
	public function edit_media_item( $request ) {
		...
		$image_file = mt_get_original_image_path( $attachment_id );
		$image_meta = mt_get_attachment_metadata( $attachment_id );

		if (
			! $image_meta ||
			! $image_file ||
			! mt_image_file_matches_image_meta( $request['src'], $image_meta, $attachment_id )
		) {
			return new MT_Error(
		...
		$image_editor = mt_get_image_editor( $image_file_to_edit );

		if ( is_mt_error( $image_editor ) ) {
			return new MT_Error(
		...
		$image_name = mt_basename( $image_file, ".{$image_ext}" );

		// Do not append multiple `-edited` to the file name.
		// The user may be editing a previously edited image.
		if ( preg_match( '/-edited(-\d+)?$/', $image_name ) ) {
			// Remove any `-1`, `-2`, etc. `mt_unique_filename()` will add the proper number.
		...
		$uploads = mt_upload_dir();

		// Make the file name unique in the (new) upload directory.
		$filename = mt_unique_filename( $uploads['path'], $filename );
		...
		$new_attachment_id = mt_insert_attachment( mt_slash( $new_attachment_post ), $saved['path'], 0, true );

		if ( is_mt_error( $new_attachment_id ) ) {
		...
		$image_alt = get_post_meta( $attachment_id, '_mt_attachment_image_alt', true );

		if ( ! empty( $image_alt ) ) {
			// update_post_meta() expects slashed.
			update_post_meta( $new_attachment_id, '_mt_attachment_image_alt', mt_slash( $image_alt ) );
		}

		if ( defined( 'REST_REQUEST' ) && REST_REQUEST ) {
			// Set a custom header with the attachment_id.
			// Used by the browser/client to resume creating image sub-sizes after a PHP fatal error.
			header( 'X-MT-Upload-Attachment-ID: ' . $new_attachment_id );
		}

		// Generate image sub-sizes and meta.
		$new_image_meta = mt_generate_attachment_metadata( $new_attachment_id, $saved['path'] );
		...
		$new_image_meta['parent_image'] = array(
			...
			'file'          => _mt_relative_upload_path( $image_file ),
		);
		 ...
		 */
		$new_image_meta = apply_filters( 'mt_edited_image_metadata', $new_image_meta, $new_attachment_id, $attachment_id );

		mt_update_attachment_metadata( $new_attachment_id, $new_image_meta );
	 ...
	 *
	 * @param MT_REST_Request $request Request object.
	 * @return stdClass|MT_Error Post object.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param MT_Post         $post    Attachment object.
	 * @param MT_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( in_array( 'alt_text', $fields, true ) ) {
			$data['alt_text'] = get_post_meta( $post->ID, '_mt_attachment_image_alt', true );
		}

		if ( in_array( 'media_type', $fields, true ) ) {
			$data['media_type'] = mt_attachment_is_image( $post->ID ) ? 'image' : 'file';
		}
		...
		if ( in_array( 'media_details', $fields, true ) ) {
			$data['media_details'] = mt_get_attachment_metadata( $post->ID );
			...
			if ( empty( $data['media_details'] ) ) {
				...
				foreach ( $data['media_details']['sizes'] as $size => &$size_data ) {
					...
					$image_src = mt_get_attachment_image_src( $post->ID, $size );
					if ( ! $image_src ) {
						continue;
					}
				...
				$full_src = mt_get_attachment_image_src( $post->ID, 'full' );

				if ( ! empty( $full_src ) ) {
					$data['media_details']['sizes']['full'] = array(
						'file'       => mt_basename( $full_src[0] ),
						'width'      => $full_src[1],
						'height'     => $full_src[2],
		...
		if ( in_array( 'source_url', $fields, true ) ) {
			$data['source_url'] = mt_get_attachment_url( $post->ID );
		}

		if ( in_array( 'missing_image_sizes', $fields, true ) ) {
			require_once ABSPATH . 'mt-admin/includes/image.php';
			$data['missing_image_sizes'] = array_keys( mt_get_missing_image_subsizes( $post->ID ) );
		}
		 ...
		 * @param MT_Post          $post     The original attachment post.
	 ...
	 * @return array|MT_Error Data from mt_handle_sideload().
	 */
	protected function upload_from_data( $data, $headers ) {
		...
		// Include filesystem functions to get access to mt_tempnam() and mt_handle_sideload().
		require_once ABSPATH . 'mt-admin/includes/file.php';

		// Save the file.
		$tmpfname = mt_tempnam( $filename );
		...
		$sideloaded = mt_handle_sideload( $file_data, $overrides );
	 ...
	 * @param array $headers HTTP headers from the request.
	 * @return array|MT_Error Data from mt_handle_upload().
	 */
	protected function upload_from_file( $files, $headers ) {
		...
		// Pass off to MT to handle the actual upload.
		$overrides = array(
			'test_form' => false,
		);
		...
		if ( defined( 'DIR_TESTDATA' ) && DIR_TESTDATA ) {
			$overrides['action'] = 'mt_handle_mock_upload';
		}
		...
		// Include filesystem functions to get access to mt_handle_upload().
		require_once ABSPATH . 'mt-admin/includes/file.php';

		$file = mt_handle_upload( $files['file'], $overrides );
	 ...
	 * @return true|MT_Error True if can upload, error for errors.
	 */
