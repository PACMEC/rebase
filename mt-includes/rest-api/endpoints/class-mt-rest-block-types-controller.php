<?php
/**
 * REST API: MT_REST_Block_Types_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class MT_REST_Block_Types_Controller extends MT_REST_Controller {

	/**
	 * Instance of MT_Block_Type_Registry.
	 *
	 * @since 5.5.0
	 * @var MT_Block_Type_Registry
	 */
	protected $block_registry;

	/**
	 * Instance of MT_Block_Styles_Registry.
	 *
	 * @since 5.5.0
	 * @var MT_Block_Styles_Registry
	 */
	...
	public function __construct() {
		...
		$this->block_registry = MT_Block_Type_Registry::get_instance();
		$this->style_registry = MT_Block_Styles_Registry::get_instance();
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|MT_Error True if the block type is visible, MT_Error otherwise.
	 */
	protected function check_read_permission() {
		...
		return new MT_Error( 'rest_block_type_cannot_view', __( 'Sorry, you are not allowed to manage block types.' ), array( 'status' => rest_authorization_required_code() ) );
	}
	 ...
	 * @return MT_Block_Type|MT_Error Block type object if name is valid, MT_Error otherwise.
	 */
	protected function get_block( $name ) {
		$block_type = $this->block_registry->get_registered( $name );
		if ( empty( $block_type ) ) {
			return new MT_Error( 'rest_block_type_invalid', __( 'Invalid block type.' ), array( 'status' => 404 ) );
		}
	 ...
	 * @since 5.5.0
	 *
	 * @param MT_Block_Type   $block_type Block type data.
	 * @param MT_REST_Request $request    Full details about the request.
	 * @return MT_REST_Response Block type data.
	 */
	public function prepare_item_for_response( $block_type, $request ) {
		...
		if ( rest_is_field_included( 'styles', $fields ) ) {
			...
			$data['styles'] = mt_parse_args( $styles, $data['styles'] );
		 ...
		 * @param MT_Block_Type    $block_type The original block type object.
	...
	protected function prepare_links( $block_type ) {
		...
		if ( $block_type->is_dynamic() ) {
			$links['https://api.w.org/render-block'] = array(
				'href' => add_query_arg( 'context', 'edit', rest_url( sprintf( '%s/%s/%s', 'mt/v2', 'block-renderer', $block_type->name ) ) ),
			);
		}
