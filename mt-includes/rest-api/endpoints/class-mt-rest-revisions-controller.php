<?php
/**
 * REST API: MT_REST_Revisions_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MT_REST_Revisions_Controller extends MT_REST_Controller {
	...
	public function __construct( $parent_post_type ) {
		...
		if ( ! $this->parent_controller ) {
			$this->parent_controller = new MT_REST_Posts_Controller( $parent_post_type );
		}
	}
	...
	public function get_items( $request ) {
		...
		if ( mt_revisions_enabled( $parent ) ) {
			...
			/** This filter is documented in mt-includes/rest-api/endpoints/class-mt-rest-posts-controller.php */
			$args       = apply_filters( 'rest_revision_query', $args, $request );
			$query_args = $this->prepare_items_query( $args, $request );

			$revisions_query = new MT_Query();
		...
		$response->header( 'X-MT-Total', (int) $total_revisions );
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $response || is_mt_error( $response ) ) {
			return $response;
		}
	...
	public function delete_item( $request ) {
		...
		$result = mt_delete_post( $request['id'], true );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Post|false|null $result The revision object (if it was deleted or moved to the Trash successfully)
