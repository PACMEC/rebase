<?php
/**
 * REST API: MT_REST_Search_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 5.0.0
...
/**
 * Core class to search through all paCMec content via the REST API.
 *
 * @since 5.0.0
 *
 * @see MT_REST_Controller
 */
class MT_REST_Search_Controller extends MT_REST_Controller {
	...
	public function __construct( array $search_handlers ) {
		...
		foreach ( $search_handlers as $search_handler ) {
			if ( ! $search_handler instanceof MT_REST_Search_Handler ) {
				_doing_it_wrong(
					__METHOD__,
					/* translators: %s: PHP class name. */
					sprintf( __( 'REST search handlers must extend the %s class.' ), 'MT_REST_Search_Handler' ),
					'5.0.0'
				);
				continue;
			}
	 ...
	 * @return true|MT_Error True if the request has search access, MT_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( is_mt_error( $handler ) ) {
			return $handler;
		}

		$result = $handler->search_items( $request );

		if ( ! isset( $result[ MT_REST_Search_Handler::RESULT_IDS ] ) || ! is_array( $result[ MT_REST_Search_Handler::RESULT_IDS ] ) || ! isset( $result[ MT_REST_Search_Handler::RESULT_TOTAL ] ) ) {
			return new MT_Error(
		...
		$ids = $result[ MT_REST_Search_Handler::RESULT_IDS ];
		...
		$total     = (int) $result[ MT_REST_Search_Handler::RESULT_TOTAL ];
		...
		$response->header( 'X-MT-Total', $total );
	 ...
	 * @return array|MT_Error List of valid subtypes, or MT_Error object on failure.
	 */
	public function sanitize_subtypes( $subtypes, $request, $parameter ) {
		$subtypes = mt_parse_slug_list( $subtypes );

		$subtypes = rest_parse_request_arg( $subtypes, $request, $parameter );
		if ( is_mt_error( $subtypes ) ) {
			return $subtypes;
		}
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return MT_REST_Search_Handler|MT_Error Search handler for the request type, or MT_Error object on failure.
	 */
