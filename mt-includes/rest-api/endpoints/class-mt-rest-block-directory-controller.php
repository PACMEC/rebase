<?php
/**
 * REST API: MT_REST_Block_Directory_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class MT_REST_Block_Directory_Controller extends MT_REST_Controller {
	 ...
	 *
	 * @return true|MT_Error True if the request has permission, MT_Error object otherwise.
	 */
	...
	public function prepare_item_for_response( $plugin, $request ) {
		...
		$block = array(
			...
			'description'         => mt_trim_words( $plugin['short_description'], 30, '...' ),
			'id'                  => $plugin['slug'],
			...
			'author'              => mt_strip_all_tags( $plugin['author'] ),
		...
		$response = new MT_REST_Response( $block );
	 ...
	 * @since 5.5.0
	 *
	 * @param array $plugin The plugin data from paCMec.org.
	 *
	 * @return array
	 */
	protected function prepare_links( $plugin ) {
		$links = array(
			'https://api.w.org/install-plugin' => array(
				'href' => add_query_arg( 'slug', urlencode( $plugin['slug'] ), rest_url( 'mt/v2/plugins' ) ),
			),
		);
		...
		if ( $plugin_file ) {
			$links['https://api.w.org/plugin'] = array(
				'href'       => rest_url( 'mt/v2/plugins/' . substr( $plugin_file, 0, - 4 ) ),
				'embeddable' => true,
			);
		}
	 ...
	 * @since 5.5.0
	 *
	 * @param string $slug The paCMec.org directory slug for a plugin.
	 *
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'author'              => array(
					'description' => __( 'The paCMec.org username of the block author.' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
				),
				'icon'                => array(
