<?php
/**
 * REST API: MT_REST_Terms_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MT_REST_Terms_Controller extends MT_REST_Controller {
	...
	public function __construct( $taxonomy ) {
		...
		$this->meta = new MT_REST_Term_Meta_Fields( $taxonomy );
	}
	 ...
	 * @return true|MT_Error True if the request has read access, otherwise false or MT_Error object.
	 */
	...
	public function get_items( $request ) {
		...
		if ( ! empty( $prepared_args['post'] ) ) {
			$query_result = mt_get_object_terms( $prepared_args['post'], $this->taxonomy, $prepared_args );

			// Used when calling mt_count_terms() below.
		...
		$total_terms = mt_count_terms( $count_args );

		// mt_count_terms() can return a falsey value when the term has no children.
		if ( ! $total_terms ) {
			$total_terms = 0;
		}
		...
		$response->header( 'X-MT-Total', (int) $total_terms );
	 ...
	 * @return MT_Term|MT_Error Term object if ID is valid, MT_Error otherwise.
	 */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has read access for the item, otherwise false or MT_Error object.
	 */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to create items, false or MT_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		$term = mt_insert_term( mt_slash( $prepared_term->name ), $this->taxonomy, mt_slash( (array) $prepared_term ) );
		if ( is_mt_error( $term ) ) {
			/*
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Term         $term     Inserted or updated term object.
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to update the item, false or MT_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			$update = mt_update_term( $term->term_id, $term->taxonomy, mt_slash( (array) $prepared_term ) );

			if ( is_mt_error( $update ) ) {
				return $update;
			}
		}
		...
		/** This action is documented in mt-includes/rest-api/endpoints/class-mt-rest-terms-controller.php */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to delete the item, otherwise false or MT_Error object.
	 */
	...
	public function delete_item( $request ) {
		...
		$retval = mt_delete_term( $term->term_id, $term->taxonomy );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Term          $term     The deleted term.
	...
	public function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['parent'] ) && ! empty( $schema['properties']['parent'] ) ) {
			...
			if ( $requested_parent ) {
				...
				if ( $parent_term instanceof MT_Term ) {
	 ...
	 * @since 4.7.0
	 *
	 * @param MT_Term         $item    Term object.
	 * @param MT_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param MT_Term           $item      The original term object.
	...
	protected function prepare_links( $term ) {
		...
		$links = array(
			...
			'about'      => array(
				'href' => rest_url( sprintf( 'mt/v2/taxonomies/%s', $this->taxonomy ) ),
			),
		);
		...
		foreach ( $taxonomy_obj->object_type as $type ) {
			...
			$post_type_links[] = array(
				'href' => add_query_arg( $this->rest_base, $term->term_id, rest_url( sprintf( 'mt/v2/%s', $rest_base ) ) ),
			);
		}
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal MT_Term_Query parameter.  Use the
		 * `rest_{$this->taxonomy}_query` filter to set MT_Term_Query parameters.
		 *
		 * @since 4.7.0
		 *
