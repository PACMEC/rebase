<?php
/**
 * REST API: MT_REST_Application_Passwords_Controller class
 *
 * @package    paCMec
 * @subpackage REST_API
 * @since      5.6.0
 ...
 */
class MT_REST_Application_Passwords_Controller extends MT_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$passwords = MT_Application_Passwords::get_user_application_passwords( $user->ID );
		$response  = array();
	...
	public function create_item( $request ) {
		...
		if ( is_mt_error( $prepared ) ) {
			return $prepared;
		}

		$created = MT_Application_Passwords::create_new_application_password( $user->ID, mt_slash( (array) $prepared ) );

		if ( is_mt_error( $created ) ) {
			return $created;
		}

		$password = $created[0];
		$item     = MT_Application_Passwords::get_user_application_password( $user->ID, $created[1]['uuid'] );

		$item['new_password'] = MT_Application_Passwords::chunk_password( $password );
	...
	public function update_item( $request ) {
		...
		$saved = MT_Application_Passwords::update_application_password( $user->ID, $item['uuid'], mt_slash( (array) $prepared ) );
		...
		$item = MT_Application_Passwords::get_user_application_password( $user->ID, $item['uuid'] );

		/** This action is documented in mt-includes/rest-api/endpoints/class-mt-rest-application-passwords-controller.php */
	...
	public function delete_items( $request ) {
		...
		$deleted = MT_Application_Passwords::delete_all_application_passwords( $user->ID );
	...
	public function delete_item( $request ) {
		...
		$deleted  = MT_Application_Passwords::delete_application_password( $user->ID, $password['uuid'] );
	...
	public function get_current_item( $request ) {
		...
		$password = MT_Application_Passwords::get_user_application_password( $user->ID, $uuid );
	 ...
	 *
	 * @param MT_REST_Request $request Request object.
	 * @return object|MT_Error The prepared item, or MT_Error object on failure.
	 */
	 ...
	 * @since 5.6.0
	 *
	 * @param array           $item    paCMec representation of the item.
	 * @param MT_REST_Request $request Request object.
	 * @return MT_REST_Response|MT_Error Response object on success, or MT_Error object on failure.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new MT_REST_Response( $prepared );
	 ...
	 * @since 5.6.0
	 *
	 * @param MT_User $user The requested user.
	 * @param array   $item The application password.
	 * @return array The list of links.
	 */
	protected function prepare_links( MT_User $user, $item ) {
		return array(
			'self' => array(
	 ...
	 *
	 * @param MT_REST_Request $request The request object.
	 * @return MT_User|MT_Error The paCMec user associated with the request, or a MT_Error if none found.
	 */
	protected function get_user( $request ) {
		...
		if ( ! mt_is_application_passwords_available_for_user( $user ) ) {
			return new MT_Error(
	 ...
	 *
	 * @param MT_REST_Request $request The request object.
	 * @return array|MT_Error The application password details if found, a MT_Error otherwise.
	 */
	protected function get_application_password( $request ) {
		...
		$password = MT_Application_Passwords::get_user_application_password( $user->ID, $request['uuid'] );
