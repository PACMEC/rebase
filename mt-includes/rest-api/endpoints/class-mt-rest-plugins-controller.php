<?php
/**
 * REST API: MT_REST_Plugins_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class MT_REST_Plugins_Controller extends MT_REST_Controller {
				...
				array(
					...
					'args'                => array(
						'slug'   => array(
							'type'        => 'string',
							'required'    => true,
							'description' => __( 'paCMec.org plugin directory slug.' ),
							'pattern'     => '[\w\-]+',
						),
						'status' => array(
	...
	public function get_items( $request ) {
		...
		foreach ( get_plugins() as $file => $data ) {
			if ( is_mt_error( $this->check_read_permission( $file ) ) ) {
				continue;
			}
		...
		return new MT_REST_Response( $plugins );
	}
	 ...
	 * @return true|MT_Error True if can read, a MT_Error instance otherwise.
	 */
	protected function check_read_permission( $plugin ) {
		require_once ABSPATH . 'mt-admin/includes/plugin.php';

		if ( ! $this->is_plugin_installed( $plugin ) ) {
			return new MT_Error( 'rest_plugin_not_found', __( 'Plugin not found.' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( is_mt_error( $filesystem_available ) ) {
		...
		if ( is_null( $result ) ) {
			...
			if ( $mt_filesystem instanceof MT_Filesystem_Base && is_mt_error( $mt_filesystem->errors ) && $mt_filesystem->errors->has_errors() ) {
				return new MT_Error( 'unable_to_connect_to_filesystem', $mt_filesystem->errors->get_error_message(), array( 'status' => 500 ) );
			}

			return new MT_Error( 'unable_to_connect_to_filesystem', __( 'Unable to connect to the filesystem. Please confirm your credentials.' ), array( 'status' => 500 ) );
		}

		$file = $upgrader->plugin_info();

		if ( ! $file ) {
			return new MT_Error( 'unable_to_determine_installed_plugin', __( 'Unable to determine what plugin was installed.' ), array( 'status' => 500 ) );
		}

		if ( 'inactive' !== $request['status'] ) {
			$can_change_status = $this->plugin_status_permission_check( $file, $request['status'], 'inactive' );

			if ( is_mt_error( $can_change_status ) ) {
				return $can_change_status;
			}

			$changed_status = $this->handle_plugin_status( $file, $request['status'], 'inactive' );

			if ( is_mt_error( $changed_status ) ) {
		...
		/** This filter is documented in mt-includes/update.php */
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new MT_REST_Response( $data );
	 ...
	 * @return array|MT_Error The plugin data, or a MT_Error if the plugin is not installed.
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param MT_REST_Request $request The request to require the plugin matches against.
	 * @param array           $item    The plugin item.
	 *
	 * @return bool
	 */
	...
	protected function is_plugin_installed( $plugin ) {
		return file_exists( MT_PLUGIN_DIR . '/' . $plugin );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|MT_Error True if filesystem is available, MT_Error otherwise.
	 */
	protected function is_filesystem_available() {
		...
		return new MT_Error( 'fs_unavailable', __( 'The filesystem is currently unavailable for managing plugins.' ), array( 'status' => 500 ) );
	}
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'requires_mt'  => array(
					'description' => __( 'Minimum required version of paCMec.' ),
					'type'        => 'string',
					'readonly'    => true,
					'context'     => array( 'view', 'edit', 'embed' ),
				),
