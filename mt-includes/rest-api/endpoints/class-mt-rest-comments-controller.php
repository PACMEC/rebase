<?php
/**
 * REST API: MT_REST_Comments_Controller class
 *
 * @package paCMec
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MT_REST_Comments_Controller extends MT_REST_Controller {

	/**
	 * Instance of a comment meta fields object.
	 *
	 * @since 4.7.0
	 * @var MT_REST_Comment_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new MT_REST_Comment_Meta_Fields();
	}
	 ...
	 * @return true|MT_Error True if the request has read access, error object otherwise.
	 */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return MT_REST_Response|MT_Error Response object on success, or error object on failure.
	 */
	public function get_items( $request ) {
		...
		foreach ( $parameter_mappings as $api_param => $mt_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$prepared_args[ $mt_param ] = $request[ $api_param ];
			}
		}
		...
		/**
		 * Filters arguments, before passing to MT_Comment_Query, when querying comments via the REST API.
		 *
		 * @since 4.7.0
		 *
		...
		$response->header( 'X-MT-Total', $total_comments );
		$response->header( 'X-MT-TotalPages', $max_pages );
	 ...
	 * @return MT_Comment|MT_Error Comment object if ID is valid, MT_Error otherwise.
	 */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has read access for the item, error object otherwise.
	 */
	public function get_item_permissions_check( $request ) {
		$comment = $this->get_comment( $request['id'] );
		if ( is_mt_error( $comment ) ) {
			return $comment;
		}
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to create items, error object otherwise.
	 */
	public function create_item_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			 ...
			 * @param MT_REST_Request $request Request used to generate the
			 *                                 response.
			 */
	...
	public function create_item( $request ) {
		...
		if ( is_mt_error( $prepared_comment ) ) {
		...
		// Setting remaining values before mt_insert_comment so we can use mt_allow_comment().
		...
		$check_comment_lengths = mt_check_comment_data_max_lengths( $prepared_comment );

		if ( is_mt_error( $check_comment_lengths ) ) {
		...
		$prepared_comment['comment_approved'] = mt_allow_comment( $prepared_comment, true );

		if ( is_mt_error( $prepared_comment['comment_approved'] ) ) {
		 ...
		 *
		 * Allows modification of the comment right before it is inserted via mt_insert_comment().
		 * Returning a MT_Error value from the filter will short-circuit insertion and allow
		 * skipping further processing.
		 *
		 * @since 4.7.0
		 * @since 4.8.0 `$prepared_comment` can now be a MT_Error to short-circuit insertion.
		 *
		 * @param array|MT_Error  $prepared_comment The prepared comment data for mt_insert_comment().
		 * @param MT_REST_Request $request          Request used to insert the comment.
		 */
		...
		$comment_id = mt_insert_comment( mt_filter_comment( mt_slash( (array) $prepared_comment ) ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Comment      $comment  Inserted or updated comment object.
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to update the item, error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( empty( $prepared_args ) && isset( $request['status'] ) ) {
			...
			$check_comment_lengths = mt_check_comment_data_max_lengths( $prepared_args );
			...
			$updated = mt_update_comment( mt_slash( (array) $prepared_args ), true );
		...
		/** This action is documented in mt-includes/rest-api/endpoints/class-mt-rest-comments-controller.php */
	 ...
	 *
	 * @param MT_REST_Request $request Full details about the request.
	 * @return true|MT_Error True if the request has access to delete the item, error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param MT_Comment $comment        The comment object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			$result   = mt_delete_comment( $comment->comment_ID, true );
			...
			$result   = mt_trash_comment( $comment->comment_ID );
		 ...
		 * @since 4.7.0
		 *
		 * @param MT_Comment       $comment  The deleted comment data.
		 * @param MT_REST_Response $response The response returned from the API.
	...
	public function prepare_item_for_response( $comment, $request ) {
		 ...
		 * @param MT_Comment        $comment  The original comment object.
	...
	protected function prepare_links( $comment ) {
		...
		if ( 0 !== (int) $comment->user_id ) {
			$links['author'] = array(
				'href'       => rest_url( 'mt/v2/users/' . $comment->user_id ),
				'embeddable' => true,
			);
		}
	 ...
	 *
	 * @param MT_REST_Request $request Request object.
	 * @return array|MT_Error Prepared comment, otherwise MT_Error object.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['author'] ) ) {
			$user = new MT_User( $request['author'] );
		 ...
		 * @since 4.7.0
		 *
		 * @param array           $prepared_comment The prepared comment data for `mt_insert_comment`.
		 * @param MT_REST_Request $request          The current request.
		 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal MT_Comment_Query parameter. Use the
		 * `rest_comment_query` filter to set MT_Comment_Query parameters.
		 *
		 * @since 4.7.0
		 *
	...
	protected function handle_status_param( $new_status, $comment_id ) {
		$old_status = mt_get_comment_status( $comment_id );
		...
		switch ( $new_status ) {
			...
			case 'approve':
			case '1':
				$changed = mt_set_comment_status( $comment_id, 'approve' );
				break;
			case 'hold':
			case '0':
				$changed = mt_set_comment_status( $comment_id, 'hold' );
				break;
			case 'spam':
				$changed = mt_spam_comment( $comment_id );
				break;
			case 'unspam':
				$changed = mt_unspam_comment( $comment_id );
				break;
			case 'trash':
				$changed = mt_trash_comment( $comment_id );
				break;
			case 'untrash':
				$changed = mt_untrash_comment( $comment_id );
				break;
			default:
				$changed = false;
				break;
	...
	protected function check_read_post_permission( $post, $request ) {
		...
		// Ensure the posts controller is specifically a MT_REST_Posts_Controller instance
		// before using methods specific to that controller.
		if ( ! $posts_controller instanceof MT_REST_Posts_Controller ) {
			$posts_controller = new MT_REST_Posts_Controller( $post->post_type );
		}
	 ...
	 * @return string|MT_Error The sanitized email address, if valid,
	 *                         otherwise an error.
	 */
	...
	protected function check_is_comment_content_allowed( $prepared_comment ) {
		 ...
		 * comment_content. See mt_handle_comment_submission().
		 */
