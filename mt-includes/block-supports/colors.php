 ...
 */
function mt_register_colors_support( $block_type ) {
	$color_support = false;
	if ( property_exists( $block_type, 'supports' ) ) {
		$color_support = _mt_array_get( $block_type->supports, array( '__experimentalColor' ), false );
	}
	$has_text_colors_support       = true === $color_support || ( is_array( $color_support ) && _mt_array_get( $color_support, array( 'text' ), true ) );
	$has_background_colors_support = true === $color_support || ( is_array( $color_support ) && _mt_array_get( $color_support, array( 'background' ), true ) );
	$has_gradients_support         = _mt_array_get( $color_support, array( 'gradients' ), false );
 ...
 */
function mt_apply_colors_support( $block_type, $block_attributes ) {
	$color_support                 = _mt_array_get( $block_type->supports, array( '__experimentalColor' ), false );
	$has_text_colors_support       = true === $color_support || ( is_array( $color_support ) && _mt_array_get( $color_support, array( 'text' ), true ) );
	$has_background_colors_support = true === $color_support || ( is_array( $color_support ) && _mt_array_get( $color_support, array( 'background' ), true ) );
	$has_link_colors_support       = _mt_array_get( $color_support, array( 'linkColor' ), false );
	$has_gradients_support         = _mt_array_get( $color_support, array( 'gradients' ), false );
	$classes                       = array();
	$styles                        = array();
	...
	if ( $has_link_colors_support ) {
		...
		if ( $has_link_color ) {
			...
			if ( strpos( $block_attributes['style']['color']['link'], 'var:preset|color|' ) !== false ) {
				...
				$styles[]        = sprintf( '--mt--style--color--link: var(--mt--preset--color--%s);', $link_color_name );
			} else {
				$styles[] = sprintf( '--mt--style--color--link: %s;', $block_attributes['style']['color']['link'] );
			}
		}
	}
...
MT_Block_Supports::get_instance()->register(
	'colors',
	array(
		'register_attribute' => 'mt_register_colors_support',
		'apply'              => 'mt_apply_colors_support',
	)
);
