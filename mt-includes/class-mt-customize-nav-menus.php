<?php
/**
 * paCMec Customize Nav Menus classes
 *
 * @package paCMec
 ...
 */
final class MT_Customize_Nav_Menus {

	/**
	 * MT_Customize_Manager instance.
	 *
	 * @since 4.3.0
	 * @var MT_Customize_Manager
	 */
	public $manager;
	...
	public function __construct( $manager ) {
		...
		// See https://github.com/xmt/mt-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L469-L499
		...
		add_action( 'mt_ajax_load-available-menu-items-customizer', array( $this, 'ajax_load_available_items' ) );
		add_action( 'mt_ajax_search-available-menu-items-customizer', array( $this, 'ajax_search_available_items' ) );
		add_action( 'mt_ajax_customize-nav-menus-insert-auto-draft', array( $this, 'ajax_insert_auto_draft_post' ) );
	...
	public function filter_nonces( $nonces ) {
		$nonces['customize-menus'] = mt_create_nonce( 'customize-menus' );
		return $nonces;
	}
	...
	public function ajax_load_available_items() {
		...
		if ( isset( $_POST['item_types'] ) && is_array( $_POST['item_types'] ) ) {
			$item_types = mt_unslash( $_POST['item_types'] );
		} elseif ( isset( $_POST['type'] ) && isset( $_POST['object'] ) ) { // Back compat.
			$item_types[] = array(
				'type'   => mt_unslash( $_POST['type'] ),
				'object' => mt_unslash( $_POST['object'] ),
				'page'   => empty( $_POST['page'] ) ? 0 : absint( $_POST['page'] ),
			);
		} else {
			mt_send_json_error( 'nav_menus_missing_type_or_object_parameter' );
		}
		...
		foreach ( $item_types as $item_type ) {
			...
			if ( is_mt_error( $items ) ) {
				mt_send_json_error( $items->get_error_code() );
			}
			$all_items[ $item_type['type'] . ':' . $item_type['object'] ] = $items;
		}

		mt_send_json_success( array( 'items' => $all_items ) );
	}
	 ...
	 * @return array|MT_Error An array of menu items on success, a MT_Error object on failure.
	 */
	public function load_available_items_query( $type = 'post_type', $object = 'page', $page = 0 ) {
		...
		if ( 'post_type' === $type ) {
			...
			if ( ! $post_type ) {
				return new MT_Error( 'nav_menus_invalid_post_type' );
			}
	...
	public function ajax_search_available_items() {
		...
		if ( empty( $_POST['search'] ) ) {
			mt_send_json_error( 'nav_menus_missing_search_parameter' );
		}
		...
		$s     = sanitize_text_field( mt_unslash( $_POST['search'] ) );
		...
		if ( empty( $items ) ) {
			mt_send_json_error( array( 'message' => __( 'No results found.' ) ) );
		} else {
			mt_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 *
	 * Based on MT_Editor::mt_link_query().
	 *
	 * @since 4.3.0
	 *
	...
	public function enqueue_scripts() {
		mt_enqueue_style( 'customize-nav-menus' );
		mt_enqueue_script( 'customize-nav-menus' );

		$temp_nav_menu_setting      = new MT_Customize_Nav_Menu_Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new MT_Customize_Nav_Menu_Item_Setting( $this->manager, 'nav_menu_item[-1]' );
		...
		$data = sprintf( 'var _mtCustomizeNavMenusSettings = %s;', mt_json_encode( $settings ) );
		mt_scripts()->add_data( 'customize-nav-menus', 'data', $data );
		...
		mt_localize_script( 'nav-menu', 'menus', $nav_menus_l10n );
	}
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( MT_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => MT_Customize_Nav_Menu_Setting::TYPE,
				'transport' => 'postMessage',
			);
		} elseif ( preg_match( MT_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => MT_Customize_Nav_Menu_Item_Setting::TYPE,
				'transport' => 'postMessage',
			);
		}
		return $setting_args;
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && MT_Customize_Nav_Menu_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'MT_Customize_Nav_Menu_Setting';
		} elseif ( ! empty( $setting_args['type'] ) && MT_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'MT_Customize_Nav_Menu_Item_Setting';
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( 'MT_Customize_Nav_Menus_Panel' );
		$this->manager->register_control_type( 'MT_Customize_Nav_Menu_Control' );
		$this->manager->register_control_type( 'MT_Customize_Nav_Menu_Name_Control' );
		$this->manager->register_control_type( 'MT_Customize_Nav_Menu_Locations_Control' );
		$this->manager->register_control_type( 'MT_Customize_Nav_Menu_Auto_Add_Control' );
		$this->manager->register_control_type( 'MT_Customize_Nav_Menu_Item_Control' );
		...
		if ( current_theme_supports( 'widgets' ) ) {
			$description .= '<p>' . sprintf(
				...
				"javascript:mt.customize.panel( 'widgets' ).focus();"
			) . '</p>';
		} else {
		...
		/*
		 * Once multiple theme supports are allowed in MT_Customize_Panel,
		 * this panel can be restricted to themes that support menus or widgets.
		 */
		$this->manager->add_panel(
			new MT_Customize_Nav_Menus_Panel(
				$this->manager,
				'nav_menus',
				array(
					'title'       => __( 'Menus' ),
		...
		if ( current_theme_supports( 'widgets' ) ) {
			...
			$description .= '<p>' . sprintf( __( 'If your theme has widget areas, you can also add menus there. Visit the <a href="%s">Widgets panel</a> and add a &#8220;Navigation Menu widget&#8221; to display a menu in a sidebar or footer.' ), "javascript:mt.customize.panel( 'widgets' ).focus();" ) . '</p>';
		}
		...
		foreach ( $menus as $menu ) {
			$choices[ $menu->term_id ] = mt_html_excerpt( $menu->name, 40, '&hellip;' );
		}
		...
		if ( ! $this->manager->is_theme_active() ) {
			...
			$mapped_nav_menu_locations = mt_map_nav_menu_locations( $theme_mods['nav_menu_locations'], $this->original_nav_menu_locations );
		}
		...
		foreach ( $locations as $location => $description ) {
			...
			$this->manager->add_control(
				new MT_Customize_Nav_Menu_Location_Control(
					$this->manager,
					$setting_id,
					array(
						'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section(
				new MT_Customize_Nav_Menu_Section(
					$this->manager,
					$section_id,
					array(
			...
			$this->manager->add_setting(
				new MT_Customize_Nav_Menu_Setting(
					$this->manager,
					$nav_menu_setting_id,
					array(
						'transport' => 'postMessage',
			...
			$menu_items = (array) mt_get_nav_menu_items( $menu_id );
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting(
					new MT_Customize_Nav_Menu_Item_Setting(
						$this->manager,
				...
				$this->manager->add_control(
					new MT_Customize_Nav_Menu_Item_Control(
						$this->manager,
						$menu_item_setting_id,
						array(
							'label'    => $item->title,
	 ...
	 * }
	 * @return MT_Post|MT_Error Inserted auto-draft post object or error.
	 */
	public function insert_auto_draft_post( $postarr ) {
		if ( ! isset( $postarr['post_type'] ) ) {
			return new MT_Error( 'unknown_post_type', __( 'Invalid post type.' ) );
		}
		if ( empty( $postarr['post_title'] ) ) {
			return new MT_Error( 'empty_title', __( 'Empty title.' ) );
		}
		if ( ! empty( $postarr['post_status'] ) ) {
			return new MT_Error( 'status_forbidden', __( 'Status is forbidden.' ) );
		}
		...
		add_filter( 'mt_insert_post_empty_content', '__return_false', 1000 );
		$r = mt_insert_post( mt_slash( $postarr ), true );
		remove_filter( 'mt_insert_post_empty_content', '__return_false', 1000 );
	...
	public function ajax_insert_auto_draft_post() {
		if ( ! check_ajax_referer( 'customize-menus', 'customize-menus-nonce', false ) ) {
			mt_send_json_error( 'bad_nonce', 400 );
		}

		if ( ! current_user_can( 'customize' ) ) {
			mt_send_json_error( 'customize_not_allowed', 403 );
		}

		if ( empty( $_POST['params'] ) || ! is_array( $_POST['params'] ) ) {
			mt_send_json_error( 'missing_params', 400 );
		}

		$params         = mt_unslash( $_POST['params'] );
		$illegal_params = array_diff( array_keys( $params ), array( 'post_type', 'post_title' ) );
		if ( ! empty( $illegal_params ) ) {
			mt_send_json_error( 'illegal_params', 400 );
		}
	 ...
	 *
	 * Templates are imported into the JS use mt.template.
	 *
	 * @since 4.3.0
	 */
				...
				<div class="menu-item-bar">
					<div class="menu-item-handle">
						...
						<span class="item-title" aria-hidden="true">
							<span class="menu-item-title<# if ( ! data.title ) { #> no-title<# } #>">{{ data.title || mt.customize.Menus.data.l10n.untitled }}</span>
						</span>
						<button type="button" class="button-link item-add">
							<span class="screen-reader-text">
							<?php
								/* translators: 1: Title of a menu item, 2: Type of a menu item. */
								printf( __( 'Add to menu: %1$s (%2$s)' ), '{{ data.title || mt.customize.Menus.data.l10n.untitled }}', '{{ data.type_label }}' );
							?>
							</span>
						</button>
					</div>
	...
	public function customize_preview_init() {
		add_action( 'mt_enqueue_scripts', array( $this, 'customize_preview_enqueue_deps' ) );
		add_filter( 'mt_nav_menu_args', array( $this, 'filter_mt_nav_menu_args' ), 1000 );
		add_filter( 'mt_nav_menu', array( $this, 'filter_mt_nav_menu' ), 10, 2 );
		add_filter( 'mt_footer', array( $this, 'export_preview_data' ), 1 );
	...
	public function make_auto_draft_status_previewable() {
		...
		$mt_post_statuses['auto-draft']->protected = true;
	}
	...
	public function sanitize_nav_menus_created_posts( $value ) {
		$post_ids = array();
		foreach ( mt_parse_id_list( $value ) as $post_id ) {
			if ( empty( $post_id ) ) {
				continue;
			}
	 ...
	 * `MT_Customize_Nav_Menu_Items::sanitize_nav_menus_created_posts()` to
	 * remove any post IDs for which the user cannot publish or for which the
	 * post is not an auto-draft.
	 *
	 * @since 4.7.0
	 *
	 * @param MT_Customize_Setting $setting Customizer setting object.
	 */
	public function save_nav_menus_created_posts( $setting ) {
		...
		if ( ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id ) {
				...
				// Note that mt_publish_post() cannot be used because unique slugs need to be assigned.
				mt_update_post( mt_slash( $args ) );
	...
	/**
	 * Keep track of the arguments that are being passed to mt_nav_menu().
	 *
	 * @since 4.3.0
	 *
	 * @see mt_nav_menu()
	 * @see MT_Customize_Widgets::filter_dynamic_sidebar_params()
	 *
	 * @param array $args An array containing mt_nav_menu() arguments.
	 * @return array Arguments.
	 */
	public function filter_mt_nav_menu_args( $args ) {
		/*
		 * The following conditions determine whether or not this instance of
		 * mt_nav_menu() can use selective refreshed. A mt_nav_menu() can be
		 * selective refreshed if...
		 */
		$can_partial_refresh = (
			// ...if mt_nav_menu() is directly echoing out the menu (and thus isn't manipulating the string after generated),
			! empty( $args['echo'] )
			&&
	...
	/**
	 * Prepares mt_nav_menu() calls for partial refresh.
	 *
	 ...
	 * @param object $args             An object containing mt_nav_menu() arguments.
	 * @return string Nav menu HTML with selective refresh attributes added if partial can be refreshed.
	 */
	public function filter_mt_nav_menu( $nav_menu_content, $args ) {
		if ( isset( $args->customize_preview_nav_menus_args['can_partial_refresh'] ) && $args->customize_preview_nav_menus_args['can_partial_refresh'] ) {
			...
			$attributes      .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( mt_json_encode( $args->customize_preview_nav_menus_args ) ) );
	...
	public function customize_preview_enqueue_deps() {
		mt_enqueue_script( 'customize-preview-nav-menus' ); // Note that we have overridden this.
	}
	...
	public function export_preview_data() {
		...
		// Why not mt_localize_script? Because we're not localizing, and it forces values into strings.
		$exports = array(
			'navMenuInstanceArgs' => $this->preview_nav_menu_instance_args,
		);
		printf( '<script>var _mtCustomizePreviewNavMenusExports = %s;</script>', mt_json_encode( $exports ) );
	}

	/**
	 * Export any mt_nav_menu() calls during the rendering of any partials.
	 *
	 * @since 4.5.0
	 *
	...
	/**
	 * Render a specific menu via mt_nav_menu() using the supplied arguments.
	 *
	 * @since 4.3.0
	 *
	 * @see mt_nav_menu()
	 *
	 * @param MT_Customize_Partial $partial       Partial.
