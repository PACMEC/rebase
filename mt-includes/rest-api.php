...
function register_rest_route( $namespace, $route, $args = array(), $override = false ) {
	if ( empty( $namespace ) ) {
		 ...
		 * non-namespaced route, call `MT_REST_Server::register_route` directly.
		 */
...
/**
 * Registers a new field on an existing paCMec object type.
 *
 * @since 4.7.0
 *
 * @global array $mt_rest_additional_fields Holds registered fields, organized
 *                                          by object type.
 *
...
function register_rest_field( $object_type, $attribute, $args = array() ) {
	...
	global $mt_rest_additional_fields;

	$object_types = (array) $object_type;

	foreach ( $object_types as $object_type ) {
		$mt_rest_additional_fields[ $object_type ][ $attribute ] = $args;
	}
}
...
function rest_api_register_rewrites() {
	...
	add_rewrite_rule( '^' . $mt_rewrite->index . '/' . rest_get_url_prefix() . '/?$', 'index.php?rest_route=/', 'top' );
	add_rewrite_rule( '^' . $mt_rewrite->index . '/' . rest_get_url_prefix() . '/(.*)?', 'index.php?rest_route=/$matches[1]', 'top' );
}
...
function create_initial_rest_routes() {
	foreach ( get_post_types( array( 'show_in_rest' => true ), 'objects' ) as $post_type ) {
		...
		if ( post_type_supports( $post_type->name, 'revisions' ) ) {
			$revisions_controller = new MT_REST_Revisions_Controller( $post_type->name );
			$revisions_controller->register_routes();
		}

		if ( 'attachment' !== $post_type->name ) {
			$autosaves_controller = new MT_REST_Autosaves_Controller( $post_type->name );
			$autosaves_controller->register_routes();
		}
	}

	// Post types.
	$controller = new MT_REST_Post_Types_Controller;
	$controller->register_routes();

	// Post statuses.
	$controller = new MT_REST_Post_Statuses_Controller;
	$controller->register_routes();

	// Taxonomies.
	$controller = new MT_REST_Taxonomies_Controller;
	$controller->register_routes();
	...
	// Users.
	$controller = new MT_REST_Users_Controller;
	$controller->register_routes();

	// Application Passwords
	$controller = new MT_REST_Application_Passwords_Controller();
	$controller->register_routes();

	// Comments.
	$controller = new MT_REST_Comments_Controller;
	$controller->register_routes();

	$search_handlers = array(
		new MT_REST_Post_Search_Handler(),
		new MT_REST_Term_Search_Handler(),
		new MT_REST_Post_Format_Search_Handler(),
	);
	 ...
	 *                               handler instance must extend the `MT_REST_Search_Handler` class.
	 *                               Default is only a handler for posts.
	 */
	$search_handlers = apply_filters( 'mt_rest_search_handlers', $search_handlers );

	$controller = new MT_REST_Search_Controller( $search_handlers );
	$controller->register_routes();

	// Block Renderer.
	$controller = new MT_REST_Block_Renderer_Controller;
	$controller->register_routes();

	// Block Types.
	$controller = new MT_REST_Block_Types_Controller();
	$controller->register_routes();

	// Settings.
	$controller = new MT_REST_Settings_Controller;
	$controller->register_routes();

	// Themes.
	$controller = new MT_REST_Themes_Controller;
	$controller->register_routes();

	// Plugins.
	$controller = new MT_REST_Plugins_Controller();
	$controller->register_routes();

	// Block Directory.
	$controller = new MT_REST_Block_Directory_Controller();
	$controller->register_routes();

	// Site Health.
	$site_health = MT_Site_Health::get_instance();
	$controller  = new MT_REST_Site_Health_Controller( $site_health );
	$controller->register_routes();
}
...
function rest_api_loaded() {
	if ( empty( $GLOBALS['mt']->query_vars['rest_route'] ) ) {
		return;
	}
	...
	$route = untrailingslashit( $GLOBALS['mt']->query_vars['rest_route'] );
	if ( empty( $route ) ) {
		$route = '/';
	}
...
function rest_get_url_prefix() {
	 ...
	 * @since 4.4.0
	 *
	 * @param string $prefix URL prefix. Default 'mt-json'.
	 */
	return apply_filters( 'rest_url_prefix', 'mt-json' );
}
...
function get_rest_url( $blog_id = null, $path = '/', $scheme = 'rest' ) {
	...
	if ( is_multisite() && get_blog_option( $blog_id, 'permalink_structure' ) || get_option( 'permalink_structure' ) ) {
		...
		if ( $mt_rewrite->using_index_permalinks() ) {
			$url = get_home_url( $blog_id, $mt_rewrite->index . '/' . rest_get_url_prefix(), $scheme );
		} else {
 ...
 * Do a REST request.
 *
 * Used primarily to route internal requests through MT_REST_Server.
 *
 * @since 4.4.0
 *
 * @param MT_REST_Request|string $request Request.
 * @return MT_REST_Response REST response.
 */
 ...
 * @since 4.5.0
 *
 * @global MT_REST_Server $mt_rest_server REST server instance.
 *
 * @return MT_REST_Server REST server instance.
 */
function rest_get_server() {
	/* @var MT_REST_Server $mt_rest_server */
	global $mt_rest_server;

	if ( empty( $mt_rest_server ) ) {
		/**
		 ...
		 * @since 4.4.0
		 *
		 * @param string $class_name The name of the server class. Default 'MT_REST_Server'.
		 */
		$mt_rest_server_class = apply_filters( 'mt_rest_server_class', 'MT_REST_Server' );
		$mt_rest_server       = new $mt_rest_server_class;
		 ...
		 * @since 4.4.0
		 *
		 * @param MT_REST_Server $mt_rest_server Server object.
		 */
		do_action( 'rest_api_init', $mt_rest_server );
	}

	return $mt_rest_server;
}
 ...
 *
 * @param array|string|MT_REST_Request $request Request to check.
 * @return MT_REST_Request REST request instance.
 */
function rest_ensure_request( $request ) {
	if ( $request instanceof MT_REST_Request ) {
		return $request;
	}

	if ( is_string( $request ) ) {
		return new MT_REST_Request( 'GET', $request );
	}
 ...
 *
 * This implements MT_REST_Response, allowing usage of `set_status`/`header`/etc
 * without needing to double-check the object. Will also allow MT_Error to indicate error
 * responses, so users should immediately check for this value.
 *
 * @since 4.4.0
 *
 * @param MT_REST_Response|MT_Error|MT_HTTP_Response|mixed $response Response to check.
 * @return MT_REST_Response|MT_Error If response generated an error, MT_Error, if response
 *                                   is already an instance, MT_REST_Response, otherwise
 *                                   returns a new MT_REST_Response instance.
 */
function rest_ensure_response( $response ) {
	...
	if ( $response instanceof MT_REST_Response ) {
		return $response;
	}

	// While MT_HTTP_Response is the base class of MT_REST_Response, it doesn't provide
	// all the required methods used in MT_REST_Server::dispatch().
	if ( $response instanceof MT_HTTP_Response ) {
		return new MT_REST_Response(
			$response->get_data(),
			$response->get_status(),
	...
	return new MT_REST_Response( $response );
}
...
function rest_handle_deprecated_function( $function, $replacement, $version ) {
	if ( ! MT_DEBUG || headers_sent() ) {
		return;
	}
	if ( ! empty( $replacement ) ) {
		/* translators: 1: Function name, 2: paCMec version number, 3: New function name. */
		$string = sprintf( __( '%1$s (since %2$s; use %3$s instead)' ), $function, $version, $replacement );
	} else {
		/* translators: 1: Function name, 2: paCMec version number. */
		$string = sprintf( __( '%1$s (since %2$s; no alternative available)' ), $function, $version );
	}

	header( sprintf( 'X-MT-DeprecatedFunction: %s', $string ) );
}
...
function rest_handle_deprecated_argument( $function, $message, $version ) {
	...
	if ( $message ) {
		/* translators: 1: Function name, 2: paCMec version number, 3: Error message. */
	...
	header( sprintf( 'X-MT-DeprecatedParam: %s', $string ) );
}
 ...
 * @param string|null $version  The version of paCMec where the message was added.
 */
function rest_handle_doing_it_wrong( $function, $message, $version ) {
	...
	if ( $version ) {
		/* translators: Developer debugging message. 1: PHP function name, 2: paCMec version number, 3: Explanatory message. */
	...
	header( sprintf( 'X-MT-DoingItWrong: %s', $string ) );
}
 ...
 * @param MT_REST_Server  $handler  ResponseHandler instance (usually MT_REST_Server).
 * @param MT_REST_Request $request  The request that was used to make current response.
 * @return MT_REST_Response Modified response, either response or `null` to indicate pass-through.
 */
 ...
 * @since 4.4.0
 *
 * @param MT_REST_Response $response Current response being served.
 * @param MT_REST_Server   $server   ResponseHandler instance (usually MT_REST_Server).
 * @param MT_REST_Request  $request  The request that was used to make current response.
 * @return MT_REST_Response Response to be served, with "Allow" header if route has allowed methods.
 */
 ...
 * @param MT_REST_Server   $server   ResponseHandler instance (usually MT_REST_Server).
 * @param MT_REST_Request  $request  The request that was used to make current response.
 * @return MT_REST_Response Response to be served, trimmed down to contain a subset of fields.
 */
function rest_filter_response_fields( $response, $server, $request ) {
	...
	$fields = mt_parse_list( $request['_fields'] );
...
/**
 * Adds the REST API URL to the MT RSD endpoint.
 *
 * @since 4.4.0
 *
 * @see get_rest_url()
...
function rest_output_rsd() {
	...
	?>
	<api name="MT-API" blogID="1" preferred="false" apiLink="<?php echo esc_url( $api_root ); ?>" />
	<?php
}
 ...
 * @see get_rest_url()
 */
function rest_output_link_mt_head() {
	$api_root = get_rest_url();
 ...
 *
 * paCMec' built-in cookie authentication is always active
 * for logged in users. However, the API has to check nonces
 * for each request to ensure users are not vulnerable to CSRF.
 *
 * @since 4.4.0
 *
 * @global mixed          $mt_rest_auth_cookie
 *
 * @param MT_Error|mixed $result Error from another authentication handler,
 *                               null if we should handle it, or another value if not.
 * @return MT_Error|mixed|bool MT_Error if the cookie is invalid, the $result, otherwise true.
 */
function rest_cookie_check_errors( $result ) {
	 ...
	 * must have been used).
	 */
	if ( true !== $mt_rest_auth_cookie && is_user_logged_in() ) {
		return $result;
	}
	...
	if ( isset( $_REQUEST['_mtnonce'] ) ) {
		$nonce = $_REQUEST['_mtnonce'];
	} elseif ( isset( $_SERVER['HTTP_X_MT_NONCE'] ) ) {
		$nonce = $_SERVER['HTTP_X_MT_NONCE'];
	}
	...
	// Check the nonce.
	$result = mt_verify_nonce( $nonce, 'mt_rest' );

	if ( ! $result ) {
		return new MT_Error( 'rest_cookie_invalid_nonce', __( 'Cookie nonce is invalid' ), array( 'status' => 403 ) );
	}

	// Send a refreshed nonce in header.
	rest_get_server()->send_header( 'X-MT-Nonce', mt_create_nonce( 'mt_rest' ) );
 ...
 *
 * Collects errors from mt_validate_auth_cookie for use by rest_cookie_check_errors.
 *
 * @since 4.4.0
 *
 * @see current_action()
...
function rest_cookie_collect_status() {
	...
	if ( 'auth_cookie_valid' !== $status_type ) {
		$mt_rest_auth_cookie = substr( $status_type, 12 );
		return;
	}

	$mt_rest_auth_cookie = true;
}
 ...
 *
 * @global MT_User|MT_Error|null $mt_rest_application_password_status
 * @global string|null $mt_rest_application_password_uuid
 *
 * @param MT_Error $user_or_error The authenticated user or error instance.
...
function rest_application_password_collect_status( $user_or_error, $app_password = array() ) {
	global $mt_rest_application_password_status, $mt_rest_application_password_uuid;

	$mt_rest_application_password_status = $user_or_error;

	if ( empty( $app_password['uuid'] ) ) {
		$mt_rest_application_password_uuid = null;
	} else {
		$mt_rest_application_password_uuid = $app_password['uuid'];
	}
}
...
function rest_get_authenticated_app_password() {
	global $mt_rest_application_password_uuid;

	return $mt_rest_application_password_uuid;
}
 ...
 * @global MT_User|MT_Error|null $mt_rest_application_password_status
 *
 * @param MT_Error|null|true $result Error from another authentication handler,
 *                                   null if we should handle it, or another value if not.
 * @return MT_Error|null|true MT_Error if the application password is invalid, the $result, otherwise true.
 */
function rest_application_password_check_errors( $result ) {
	global $mt_rest_application_password_status;

	if ( ! empty( $result ) ) {
		return $result;
	}

	if ( is_mt_error( $mt_rest_application_password_status ) ) {
		$data = $mt_rest_application_password_status->get_error_data();

		if ( ! isset( $data['status'] ) ) {
			$data['status'] = 401;
		}

		$mt_rest_application_password_status->add_data( $data );

		return $mt_rest_application_password_status;
	}

	if ( $mt_rest_application_password_status instanceof MT_User ) {
		return true;
	}
 ...
 * @since 5.6.0
 *
 * @param MT_REST_Response $response The index response object.
 * @return MT_REST_Response
 */
 ...
 *
 * @param mixed           $value
 * @param MT_REST_Request $request
 * @param string          $param
 * @return true|MT_Error
 */
...
function rest_is_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		$maybe_array = mt_parse_list( $maybe_array );
	}

	return mt_is_numeric_array( $maybe_array );
}
...
function rest_sanitize_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		return mt_parse_list( $maybe_array );
	}
...
function rest_handle_multi_type_schema( $value, $args, $param = '' ) {
	...
	if ( $invalid_types ) {
		_doing_it_wrong(
			...
			mt_sprintf( __( 'The "type" schema keyword for %1$s can only contain the built-in types: %2$l.' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
...
/**
 * Formats a combining operation error into a MT_Error object.
 *
 * @since 5.6.0
 *
 ...
 * @return MT_Error      The combining operation error.
 */
function rest_get_combining_operation_error( $value, $param, $errors ) {
	...
	if ( count( $schema_titles ) === count( $errors ) ) {
		...
		return new MT_Error( 'rest_no_matching_schema', mt_sprintf( __( '%1$s is not a valid %2$l.' ), $param, $schema_titles ) );
	}

	/* translators: %s: Parameter. */
	return new MT_Error( 'rest_no_matching_schema', sprintf( __( '%s does not match any of the expected formats.' ), $param ) );
}
 ...
 * @param array  $args    The schema array to use.
 * @param string $param   The parameter name, used in error messages.
 * @return array|MT_Error The matching schema or MT_Error instance if all schemas do not match.
 */
 ...
 * @return array|MT_Error                The matching schema or MT_Error instance if the number of matching schemas is not equal to one.
 */
	...
	if ( count( $matching_schemas ) > 1 ) {
		...
		if ( count( $schema_titles ) === count( $matching_schemas ) ) {
			return new MT_Error(
				...
				/* translators: 1: Parameter, 2: Schema titles. */
				mt_sprintf( __( '%1$s matches %2$l, but should match only one.' ), $param, $schema_titles ),
 ...
 * @param array  $args   The schema array to use.
 * @param string $param  The parameter name, used in error messages.
 * @return true|MT_Error True if the "enum" contains the value or a MT_Error instance otherwise.
 */
function rest_validate_enum( $value, $args, $param ) {
	$sanitized_value = rest_sanitize_value_from_schema( $value, $args, $param );
	if ( is_mt_error( $sanitized_value ) ) {
	...
	foreach ( $args['enum'] as $enum_value ) {
		$encoded_enum_values[] = is_scalar( $enum_value ) ? $enum_value : mt_json_encode( $enum_value );
	}

	if ( count( $encoded_enum_values ) === 1 ) {
		/* translators: 1: Parameter, 2: Valid values. */
		return new MT_Error( 'rest_not_in_enum', mt_sprintf( __( '%1$s is not %2$s.' ), $param, $encoded_enum_values[0] ) );
	}

	/* translators: 1: Parameter, 2: List of valid values. */
	return new MT_Error( 'rest_not_in_enum', mt_sprintf( __( '%1$s is not one of %2$l.' ), $param, $encoded_enum_values ) );
}
...
function rest_validate_value_from_schema( $value, $args, $param = '' ) {
	if ( isset( $args['anyOf'] ) ) {
		...
		if ( is_mt_error( $matching_schema ) ) {
	...
	if ( ! in_array( $args['type'], $allowed_types, true ) ) {
		_doing_it_wrong(
			...
			mt_sprintf( __( 'The "type" schema keyword for %1$s can only be one of the built-in types: %2$l.' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
	...
	if ( ! empty( $args['enum'] ) ) {
		...
		if ( is_mt_error( $enum_contains_value ) ) {
		...
		switch ( $args['format'] ) {
			case 'hex-color':
				if ( ! rest_parse_hex_color( $value ) ) {
					return new MT_Error( 'rest_invalid_hex_color', __( 'Invalid hex color.' ) );
				}
				break;

			case 'date-time':
				if ( ! rest_parse_date( $value ) ) {
					return new MT_Error( 'rest_invalid_date', __( 'Invalid date.' ) );
				}
				break;

			case 'email':
				if ( ! is_email( $value ) ) {
					return new MT_Error( 'rest_invalid_email', __( 'Invalid email address.' ) );
				}
				break;
			case 'ip':
				if ( ! rest_is_ip_address( $value ) ) {
					/* translators: %s: IP address. */
					return new MT_Error( 'rest_invalid_ip', sprintf( __( '%s is not a valid IP address.' ), $param ) );
				}
				break;
			case 'uuid':
				if ( ! mt_is_uuid( $value ) ) {
					/* translators: %s: The name of a JSON field expecting a valid UUID. */
					return new MT_Error( 'rest_invalid_uuid', sprintf( __( '%s is not a valid UUID.' ), $param ) );
				}
				break;
		}
	}
...
function rest_validate_array_value_from_schema( $value, $args, $param ) {
	...
	if ( ! empty( $args['uniqueItems'] ) && ! rest_validate_array_contains_unique_items( $value ) ) {
		/* translators: %s: Parameter. */
		return new MT_Error( 'rest_duplicate_items', sprintf( __( '%s has duplicate items.' ), $param ) );
	}
...
function rest_validate_integer_value_from_schema( $value, $args, $param ) {
	...
	if ( is_mt_error( $is_valid_number ) ) {
 ...
 * @param string $param The parameter name, used in error messages.
 * @return mixed|MT_Error The sanitized value or a MT_Error instance if the value cannot be safely sanitized.
 */
...
function rest_preload_api_request( $memo, $path ) {
	...
	$request = new MT_REST_Request( $method, $path_parts['path'] );
...
function rest_get_route_for_post( $post ) {
	...
	if ( in_array( get_class( $controller ), array( 'MT_REST_Attachments_Controller', 'MT_REST_Posts_Controller' ), true ) ) {
		$namespace = 'mt/v2';
 ...
 * @since 5.5.0
 *
 * @param int|MT_Term $term Term ID or term object.
...
function rest_get_route_for_term( $term ) {
	...
	if ( $controller instanceof MT_REST_Terms_Controller ) {
		$namespace = 'mt/v2';
...
function rest_get_queried_resource_route() {
	if ( is_singular() ) {
		...
		$route = '/mt/v2/users/' . get_queried_object_id();
	} else {
		$route = '';
	}
 ...
 *                       on `EDITABLE` endpoints. Default MT_REST_Server::CREATABLE.
 * @return array The endpoint arguments.
 */
function rest_get_endpoint_args_for_schema( $schema, $method = MT_REST_Server::CREATABLE ) {
	...
	foreach ( $schema_properties as $field_id => $params ) {
		...
		if ( MT_REST_Server::CREATABLE === $method && isset( $params['default'] ) ) {
			$endpoint_args[ $field_id ]['default'] = $params['default'];
		}

		if ( MT_REST_Server::CREATABLE === $method && ! empty( $params['required'] ) ) {
		...
		if ( isset( $params['arg_options'] ) ) {
			...
			if ( MT_REST_Server::CREATABLE !== $method ) {
 ...
 * @since 5.7.0
 *
 * @param MT_Error $error MT_Error instance.
 *
 * @return MT_REST_Response List of associative arrays with code and message keys.
 */
function rest_convert_error_to_response( $error ) {
	...
	return new MT_REST_Response( $data, $status );
}
