 ...
 * form into the sidebar and also by the search widget in paCMec.
 *
 ...
 */
function mt_loginout( $redirect = '', $echo = true ) {
	if ( ! is_user_logged_in() ) {
		$link = '<a href="' . esc_url( mt_login_url( $redirect ) ) . '">' . __( 'Log in' ) . '</a>';
	} else {
		$link = '<a href="' . esc_url( mt_logout_url( $redirect ) ) . '">' . __( 'Log out' ) . '</a>';
	}
	...
	if ( $echo ) {
		...
		/** This filter is documented in mt-includes/general-template.php */
 ...
 * @return string The logout URL. Note: HTML-encoded via esc_html() in mt_nonce_url().
 */
function mt_logout_url( $redirect = '' ) {
	$args = array();
	if ( ! empty( $redirect ) ) {
		$args['redirect_to'] = urlencode( $redirect );
	}

	$logout_url = add_query_arg( $args, site_url( 'mt-login.php?action=logout', 'login' ) );
	$logout_url = mt_nonce_url( $logout_url, 'log-out' );
 ...
 */
function mt_login_url( $redirect = '', $force_reauth = false ) {
	$login_url = site_url( 'mt-login.php', 'login' );
...
function mt_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'mt-login.php?action=register', 'login' ) );
}

/**
 * Provides a simple login form for use anywhere within paCMec.
 *
 ...
 *     @type string $id_submit      ID attribute value for the submit button. Default 'mt-submit'.
 ...
 */
function mt_login_form( $args = array() ) {
	$defaults = array(
		'echo'           => true,
	 ...
	 */
	$args = mt_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );
	...
	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'mt-login.php', 'login_post' ) ) . '" method="post">
			' . $login_form_top . '
			...
			<p class="login-submit">
				<input type="submit" name="mt-submit" id="' . esc_attr( $args['id_submit'] ) . '" class="button button-primary" value="' . esc_attr( $args['label_log_in'] ) . '" />
 ...
 */
function mt_lostpassword_url( $redirect = '' ) {
	$args = array(
		'action' => 'lostpassword',
	);
	...
	if ( is_multisite() ) {
		...
		$mt_login_path = $blog_details->path . 'mt-login.php';
	} else {
		$mt_login_path = 'mt-login.php';
	}

	$lostpassword_url = add_query_arg( $args, network_site_url( $mt_login_path, 'login' ) );
 ...
 *                     if `$echo` is false.
 */
function mt_register( $before = '<li>', $after = '</li>', $echo = true ) {
	if ( ! is_user_logged_in() ) {
		if ( get_option( 'users_can_register' ) ) {
			$link = $before . '<a href="' . esc_url( mt_registration_url() ) . '">' . __( 'Register' ) . '</a>' . $after;
		} else {
			$link = '';
		}
...
/**
 * Theme container function for the 'mt_meta' action.
 *
 * The {@see 'mt_meta'} action can have several purposes, depending on how you use it,
 ...
 * - 'mturl' - The paCMec address (URL) (set in Settings > General)
 * - 'url' - The Site address (URL) (set in Settings > General)
 * - 'admin_email' - Admin email (set in Settings > General)
 * - 'charset' - The "Encoding for pages and feeds"  (set in Settings > Reading)
 * - 'version' - The current paCMec version
...
function get_site_icon_url( $size = 512, $url = '', $blog_id = 0 ) {
	...
	if ( $site_icon_id ) {
		...
		$url = mt_get_attachment_image_url( $site_icon_id, $size_data );
	}
...
function get_custom_logo( $blog_id = 0 ) {
	...
	if ( $custom_logo_id ) {
		...
		if ( $unlink_homepage_logo && is_front_page() && ! is_paged() ) {
			 ...
			 * to the attributes used by mt_get_attachment_image().
			 */
			$image_alt = get_post_meta( $custom_logo_id, '_mt_attachment_image_alt', true );
			if ( empty( $image_alt ) ) {
		 ...
		 * because mt_get_attachment_image() already adds the alt attribute.
		 */
		$image = mt_get_attachment_image( $custom_logo_id, 'full', false, $custom_logo_attr );
 ...
 */
function mt_get_document_title() {

	/**
	 * Filters the document title before it is generated.
	 *
	 * Passing a non-empty value will short-circuit mt_get_document_title(),
 ...
 * @ignore
 * @since 4.1.0
 * @since 4.4.0 Improved title output replaced `mt_title()`.
 * @access private
 */
function _mt_render_title_tag() {
	if ( ! current_theme_supports( 'title-tag' ) ) {
		return;
	}

	echo '<title>' . mt_get_document_title() . '</title>' . "\n";
}
 ...
 */
function mt_title( $sep = '&raquo;', $display = true, $seplocation = '' ) {
	global $mt_locale;
	...
	$t_sep = '%MT_TITLE_SEP%'; // Temporary separator, for accurate flipping, if necessary.
	...
	if ( is_archive() && ! empty( $m ) ) {
		...
		$my_month = $mt_locale->get_month( substr( $m, 4, 2 ) );
	...
	if ( is_archive() && ! empty( $year ) ) {
		$title = $year;
		if ( ! empty( $monthnum ) ) {
			$title .= $t_sep . $mt_locale->get_month( $monthnum );
		}
		if ( ! empty( $day ) ) {
	 ...
	 */
	$title_array = apply_filters( 'mt_title_parts', explode( $t_sep, $title ) );
	 ...
	 */
	$title = apply_filters( 'mt_title', $title, $sep, $seplocation );
...
function single_post_title( $prefix = '', $display = true ) {
	 ...
	 * @param MT_Post $_post       The current post.
	 */
...
function single_month_title( $prefix = '', $display = true ) {
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		$my_year  = $year;
		$my_month = $mt_locale->get_month( $monthnum );
	} elseif ( ! empty( $m ) ) {
		$my_year  = substr( $m, 0, 4 );
		$my_month = $mt_locale->get_month( substr( $m, 4, 2 ) );
	}
...
function get_the_post_type_description() {
	 ...
	 * @param MT_Post_Type $post_type_obj The post type object.
	 */
 ...
 */
function mt_get_archives( $args = '' ) {
	global $mtdb, $mt_locale;
	...
	$sql_where = $mtdb->prepare( "WHERE post_type = %s AND post_status = 'publish'", $parsed_args['post_type'] );
	...
	if ( 'monthly' === $parsed_args['type'] ) {
		$query   = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $mtdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "mt_get_archives:$key:$last_changed";
		$results = mt_cache_get( $key, 'posts' );
		if ( ! $results ) {
			$results = $mtdb->get_results( $query );
			mt_cache_set( $key, $results, 'posts' );
		}
		if ( $results ) {
			...
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%1$s %2$d' ), $mt_locale->get_month( $result->month ), $result->year );
		...
		$query   = "SELECT YEAR(post_date) AS `year`, count(ID) as posts FROM $mtdb->posts $join $where GROUP BY YEAR(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "mt_get_archives:$key:$last_changed";
		$results = mt_cache_get( $key, 'posts' );
		if ( ! $results ) {
		...
		$query   = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth`, count(ID) as posts FROM $mtdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date), DAYOFMONTH(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "mt_get_archives:$key:$last_changed";
		$results = mt_cache_get( $key, 'posts' );
		if ( ! $results ) {
		...
		$week    = _mt_mysql_week( '`post_date`' );
		$query   = "SELECT DISTINCT $week AS `week`, YEAR( `post_date` ) AS `yr`, DATE_FORMAT( `post_date`, '%Y-%m-%d' ) AS `yyyymmdd`, count( `ID` ) AS `posts` FROM `$mtdb->posts` $join $where GROUP BY $week, YEAR( `post_date` ) ORDER BY `post_date` $order $limit";
		$key     = md5( $query );
		$key     = "mt_get_archives:$key:$last_changed";
		$results = mt_cache_get( $key, 'posts' );
		if ( ! $results ) {
		...
		$query   = "SELECT * FROM $mtdb->posts $join $where ORDER BY $orderby $limit";
		$key     = md5( $query );
		$key     = "mt_get_archives:$key:$last_changed";
		$results = mt_cache_get( $key, 'posts' );
		if ( ! $results ) {
...
function get_calendar( $initial = true, $echo = true ) {
	global $mtdb, $m, $monthnum, $year, $mt_locale, $posts;

	$key   = md5( $m . $monthnum . $year );
	$cache = mt_cache_get( 'get_calendar', 'calendar' );
	...
	if ( ! $posts ) {
		$gotsome = $mtdb->get_var( "SELECT 1 as test FROM $mtdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1" );
		if ( ! $gotsome ) {
			$cache[ $key ] = '';
			mt_cache_set( 'get_calendar', $cache, 'calendar' );
			return;
		}
	}
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		...
		$d         = ( ( $w - 1 ) * 7 ) + 6;
		$thismonth = $mtdb->get_var( "SELECT DATE_FORMAT((DATE_ADD('{$thisyear}0101', INTERVAL $d DAY) ), '%m')" );
	} elseif ( ! empty( $m ) ) {
		$thisyear = (int) substr( $m, 0, 4 );
		if ( strlen( $m ) < 6 ) {
			$thismonth = '01';
	...
	$calendar_output  = '<table id="mt-calendar" class="mt-calendar-table">
	<caption>' . sprintf(
		$calendar_caption,
		$mt_locale->get_month( $thismonth ),
		gmdate( 'Y', $unixmonth )
	) . '</caption>
	<thead>
	<tr>';
	...
	for ( $wdcount = 0; $wdcount <= 6; $wdcount++ ) {
		$myweek[] = $mt_locale->get_weekday( ( $wdcount + $week_begins ) % 7 );
	}

	foreach ( $myweek as $wd ) {
		$day_name         = $initial ? $mt_locale->get_weekday_initial( $wd ) : $mt_locale->get_weekday_abbrev( $wd );
		$wd               = esc_attr( $wd );
	...
	// Get days with posts.
	$dayswithposts = $mtdb->get_results(
		"SELECT DISTINCT DAYOFMONTH(post_date)
		FROM $mtdb->posts WHERE post_date >= '{$thisyear}-{$thismonth}-01 00:00:00'
		AND post_type = 'post' AND post_status = 'publish'
	...
	$calendar_output .= '<nav aria-label="' . __( 'Previous and next months' ) . '" class="mt-calendar-nav">';

	if ( $previous ) {
		$calendar_output .= "\n\t\t" . '<span class="mt-calendar-nav-prev"><a href="' . get_month_link( $previous->year, $previous->month ) . '">&laquo; ' .
			$mt_locale->get_month_abbrev( $mt_locale->get_month( $previous->month ) ) .
		'</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="mt-calendar-nav-prev">&nbsp;</span>';
	}

	$calendar_output .= "\n\t\t" . '<span class="pad">&nbsp;</span>';

	if ( $next ) {
		$calendar_output .= "\n\t\t" . '<span class="mt-calendar-nav-next"><a href="' . get_month_link( $next->year, $next->month ) . '">' .
			$mt_locale->get_month_abbrev( $mt_locale->get_month( $next->month ) ) .
		' &raquo;</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="mt-calendar-nav-next">&nbsp;</span>';
	}
...
function delete_get_calendar_cache() {
	mt_cache_delete( 'get_calendar', 'calendar' );
}
 ...
 *
 * @param string      $format Optional. PHP date format. Defaults to the 'date_format' option.
 * @param int|MT_Post $post   Optional. Post ID or MT_Post object. Default current post.
...
function get_the_date( $format = '', $post = null ) {
	 ...
	 * @param string      $format   PHP date format.
	 * @param int|MT_Post $post     The post object or ID.
	 */
...
function get_the_modified_date( $format = '', $post = null ) {
	 ...
	 * @param string           $format   PHP date format.
	 * @param MT_Post|null     $post     MT_Post object or null if no post is found.
	 */
 ...
 *                            was written. Accepts 'G', 'U', or PHP date format.
 *                            Defaults to the 'time_format' option.
 * @param int|MT_Post $post   MT_Post object or ID. Default is global `$post` object.
...
function get_the_time( $format = '', $post = null ) {
	 ...
	 * @param string      $format   Format to use for retrieving the time the post
	 *                              was written. Accepts 'G', 'U', or PHP date format.
	 * @param int|MT_Post $post     MT_Post object or ID.
	 */
...
function get_post_time( $format = 'U', $gmt = false, $post = null, $translate = false ) {
	...
	if ( 'U' === $format || 'G' === $format ) {
		...
		$time = mt_date( $format, $datetime->getTimestamp(), $gmt ? new DateTimeZone( 'UTC' ) : null );
	} else {
		if ( $gmt ) {
 ...
 *
 * The object will be set to the timezone from paCMec settings.
 *
 ...
 * @since 5.3.0
 *
 * @param int|MT_Post $post   Optional. MT_Post object or ID. Default is global `$post` object.
...
function get_post_datetime( $post = null, $field = 'date', $source = 'local' ) {
	...
	return $datetime->setTimezone( $mt_timezone );
}
...
function the_weekday() {
	...
	$the_weekday = $mt_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
function the_weekday_date( $before = '', $after = '' ) {
	global $mt_locale, $currentday, $previousweekday;
	...
	if ( $currentday !== $previousweekday ) {
		...
		$the_weekday_date .= $mt_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
/**
 * Fire the mt_body_open action.
 *
 * See {@see 'mt_body_open'}.
 *
 * @since 5.2.0
 */
function mt_body_open() {
	/**
	 * Triggered after the opening body tag.
	 *
	 * @since 5.2.0
	 */
	do_action( 'mt_body_open' );
}
 ...
 *
 * Typical usage is as a mt_head callback. add_action( 'mt_head', 'mt_strict_cross_origin_referrer' );
 *
 * @since 5.7.0
 */
function mt_strict_cross_origin_referrer() {
	?>
 ...
 * @since 4.6.0
 */
function mt_resource_hints() {
	$hints = array(
		'dns-prefetch' => mt_dependencies_unique_hosts(),
		'preconnect'   => array(),
		'prefetch'     => array(),
		'prerender'    => array(),
	);
	 ...
	 */
	/** This filter is documented in mt-includes/formatting.php */
	...
	foreach ( $hints as $relation_type => $urls ) {
		 ...
		 */
		$urls = apply_filters( 'mt_resource_hints', $urls, $relation_type );
 ...
 */
function mt_dependencies_unique_hosts() {
	global $mt_scripts, $mt_styles;

	$unique_hosts = array();

	foreach ( array( $mt_scripts, $mt_styles ) as $dependencies ) {
		if ( $dependencies instanceof MT_Dependencies && ! empty( $dependencies->queue ) ) {
			foreach ( $dependencies->queue as $handle ) {
				if ( ! isset( $dependencies->registered[ $handle ] ) ) {
					continue;
				}

				/* @var _MT_Dependency $dependency */
				$dependency = $dependencies->registered[ $handle ];
				$parsed     = mt_parse_url( $dependency->src );
 ...
 * @since 2.0.0
 *
 * @global bool $mt_rich_edit Whether the user can access the visual editor.
...
function user_can_richedit() {
	global $mt_rich_edit, $is_gecko, $is_opera, $is_safari, $is_chrome, $is_IE, $is_edge;

	if ( ! isset( $mt_rich_edit ) ) {
		$mt_rich_edit = false;

		if ( 'true' === get_user_option( 'rich_editing' ) || ! is_user_logged_in() ) { // Default to 'true' for logged out users.
			if ( $is_safari ) {
				$mt_rich_edit = ! mt_is_mobile() || ( preg_match( '!AppleWebKit/(\d+)!', $_SERVER['HTTP_USER_AGENT'], $match ) && (int) $match[1] >= 534 );
			} elseif ( $is_IE ) {
				$mt_rich_edit = ( strpos( $_SERVER['HTTP_USER_AGENT'], 'Trident/7.0;' ) !== false );
			} elseif ( $is_gecko || $is_chrome || $is_edge || ( $is_opera && ! mt_is_mobile() ) ) {
				$mt_rich_edit = true;
			}
		}
	}
	 ...
	 * @since 2.1.0
	 *
	 * @param bool $mt_rich_edit Whether the user can access the visual editor.
	 */
	return apply_filters( 'user_can_richedit', $mt_rich_edit );
}
 ...
 */
function mt_default_editor() {
	$r = user_can_richedit() ? 'tinymce' : 'html'; // Defaults.
	if ( mt_get_current_user() ) { // Look for cookie.
	 ...
	 */
	return apply_filters( 'mt_default_editor', $r );
}
 ...
 * @param array  $settings  See _MT_Editors::parse_settings() for description.
 */
function mt_editor( $content, $editor_id, $settings = array() ) {
	if ( ! class_exists( '_MT_Editors', false ) ) {
		require ABSPATH . MTINC . '/class-mt-editor.php';
	}
	_MT_Editors::editor( $content, $editor_id, $settings );
}
 ...
 * See mt.editor.initialize() in mt-admin/js/editor.js for initialization options.
 *
 * @uses _MT_Editors
 * @since 4.8.0
 */
function mt_enqueue_editor() {
	if ( ! class_exists( '_MT_Editors', false ) ) {
		require ABSPATH . MTINC . '/class-mt-editor.php';
	}

	_MT_Editors::enqueue_default_editor();
}
 ...
 * @since 4.9.0
 *
 * @see mt_enqueue_editor()
 * @see mt_get_code_editor_settings();
 * @see _MT_Editors::parse_settings()
 *
 * @param array $args {
 *     Args.
 ...
 *     @type MT_Theme $theme      Theme being edited when on theme editor.
 ...
 */
function mt_enqueue_code_editor( $args ) {
	if ( is_user_logged_in() && 'false' === mt_get_current_user()->syntax_highlighting ) {
		return false;
	}

	$settings = mt_get_code_editor_settings( $args );

	if ( empty( $settings ) || empty( $settings['codemirror'] ) ) {
		return false;
	}

	mt_enqueue_script( 'code-editor' );
	mt_enqueue_style( 'code-editor' );
	...
	if ( isset( $settings['codemirror']['mode'] ) ) {
		...
		if ( ! empty( $settings['codemirror']['lint'] ) ) {
			switch ( $mode['name'] ) {
				...
				case 'text/x-php':
					mt_enqueue_script( 'htmlhint' );
					mt_enqueue_script( 'csslint' );
					mt_enqueue_script( 'jshint' );
					if ( ! current_user_can( 'unfiltered_html' ) ) {
						mt_enqueue_script( 'htmlhint-kses' );
					}
					break;
				case 'javascript':
				...
				case 'application/typescript':
					mt_enqueue_script( 'jshint' );
					mt_enqueue_script( 'jsonlint' );
					break;
			}
		}
	}

	mt_add_inline_script( 'code-editor', sprintf( 'jQuery.extend( mt.codeEditor.defaultSettings, %s );', mt_json_encode( $settings ) ) );
	 ...
	 */
	do_action( 'mt_enqueue_code_editor', $settings );
 ...
 */
function mt_get_code_editor_settings( $args ) {
	$settings = array(
		'codemirror' => array(
			'indentUnit'       => 4,
			'indentWithTabs'   => true,
	...
	if ( isset( $args['type'] ) ) {
		...
		// Supply any types that are not matched by mt_get_mime_types().
		if ( empty( $type ) ) {
			switch ( $extension ) {
				case 'conf':
					$type = 'text/nginx';
	...
	if ( in_array( $type, array( 'text/css', 'text/x-scss', 'text/x-less', 'text/x-sass' ), true ) ) {
		...
		if ( ! current_user_can( 'unfiltered_html' ) ) {
			$settings['htmlhint']['kses'] = mt_kses_allowed_html( 'post' );
		}
	...
	foreach ( mt_array_slice_assoc( $args, array( 'codemirror', 'csslint', 'jshint', 'htmlhint' ) ) as $key => $value ) {
	 ...
	 * }
	 */
	return apply_filters( 'mt_code_editor_settings', $settings, $args );
}

/**
 * Retrieves the contents of the search paCMec query variable.
 *
 ...
 *     @type int    $total              The total amount of pages. Default is the value MT_Query's
 *                                      `max_num_pages` or 1.
...
function paginate_links( $args = '' ) {
	...
	$total   = isset( $mt_query->max_num_pages ) ? $mt_query->max_num_pages : 1;
	...
	$format  = $mt_rewrite->using_index_permalinks() && ! strpos( $pagenum_link, 'index.php' ) ? 'index.php/' : '';
	$format .= $mt_rewrite->using_permalinks() ? user_trailingslashit( $mt_rewrite->pagination_base . '/%#%', 'paged' ) : '?paged=%#%';
	...
	if ( isset( $url_parts[1] ) ) {
		...
		mt_parse_str( $format_query, $format_args );

		// Find the query args of the requested URL.
		mt_parse_str( $url_parts[1], $url_query_args );
 ...
 *
 *     mt_admin_css_color( 'classic', __( 'Classic' ), admin_url( "css/colors-classic.css" ), array(
 ...
 * }
 */
function mt_admin_css_color( $key, $name, $url, $colors = array(), $icons = array() ) {
	global $_mt_admin_css_colors;

	if ( ! isset( $_mt_admin_css_colors ) ) {
		$_mt_admin_css_colors = array();
	}

	$_mt_admin_css_colors[ $key ] = (object) array(
		'name'        => $name,
		'url'         => $url,
		'colors'      => $colors,
		'icon_colors' => $icons,
...
/**
 * Displays the URL of a paCMec admin CSS file.
 *
 * @see MT_Styles::_css_href and its {@see 'style_loader_src'} filter.
 *
 * @since 2.3.0
 *
 * @param string $file file relative to mt-admin/ without its ".css" extension.
 * @return string
 */
function mt_admin_css_uri( $file = 'mt-admin' ) {
	...
	/**
	 * Filters the URI of a paCMec admin CSS file.
	 *
	 * @since 2.3.0
	 *
	 ...
	 */
	return apply_filters( 'mt_admin_css_uri', $_file, $file );
}
 ...
 * {@see 'mt_print_styles'} action has *not* yet been called, the CSS file will be
 * enqueued. If the {@see 'mt_print_styles'} action has been called, the CSS link will
 * be printed. Printing may be forced by passing true as the $force_echo
 * (second) parameter.
 *
 * For backward compatibility with paCMec 2.3 calling method: If the $file
 * (first) parameter does not correspond to a registered CSS file, we assume
 * $file is a file relative to mt-admin/ without its ".css" extension. A
 * stylesheet link to that generated URL is printed.
 *
 * @since 2.3.0
 *
 * @param string $file       Optional. Style handle name or file name (without ".css" extension) relative
 *                           to mt-admin/. Defaults to 'mt-admin'.
 * @param bool   $force_echo Optional. Force the stylesheet link to be printed rather than enqueued.
 */
function mt_admin_css( $file = 'mt-admin', $force_echo = false ) {
	// For backward compatibility.
	$handle = 0 === strpos( $file, 'css/' ) ? substr( $file, 4 ) : $file;

	if ( mt_styles()->query( $handle ) ) {
		if ( $force_echo || did_action( 'mt_print_styles' ) ) {
			// We already printed the style queue. Print this one immediately.
			mt_print_styles( $handle );
		} else {
			// Add to style queue.
			mt_enqueue_style( $handle );
		}
		return;
	}
	 ...
	 *                                relative to mt-admin/. Defaults to 'mt-admin'.
	 */
	echo apply_filters( 'mt_admin_css', $stylesheet_link, $file );

	if ( function_exists( 'is_rtl' ) && is_rtl() ) {
		$rtl_stylesheet_link = sprintf(
			"<link rel='stylesheet' href='%s' type='text/css' />\n",
			esc_url( mt_admin_css_uri( "$file-rtl" ) )
		);

		/** This filter is documented in mt-includes/general-template.php */
		echo apply_filters( 'mt_admin_css', $rtl_stylesheet_link, "$file-rtl" );
	}
}
...
function add_thickbox() {
	mt_enqueue_script( 'thickbox' );
	mt_enqueue_style( 'thickbox' );
...
/**
 * Displays the XHTML generator that is generated on the mt_head hook.
 *
 * See {@see 'mt_head'}.
 *
 * @since 2.5.0
 */
function mt_generator() {
	/**
	 ...
	 */
	the_generator( apply_filters( 'mt_generator_type', 'xhtml' ) );
}
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		...
		case 'comment':
			...
			break;
		case 'export':
			$gen = '<!-- generator="paCMec/' . esc_attr( get_bloginfo_rss( 'version' ) ) . '" created="' . gmdate( 'Y-m-d H:i' ) . '" -->';
			break;
	}
	 ...
	 * @since 2.5.0
	 *
	 * @param string $gen  The HTML markup output to mt_head().
 ...
 * @return string HTML attribute or empty string
 */
function __checked_selected_helper( $helper, $current, $echo, $type ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
 ...
 */
function mt_heartbeat_settings( $settings ) {
	if ( ! is_admin() ) {
		$settings['ajaxurl'] = admin_url( 'admin-ajax.php', 'relative' );
	}

	if ( is_user_logged_in() ) {
		$settings['nonce'] = mt_create_nonce( 'heartbeat-nonce' );
	}
