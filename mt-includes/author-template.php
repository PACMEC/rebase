 ...
 *
 * These functions must be used within the paCMec Loop.
 *
 * @link https://codex.managertechnology.com.co/pacmec/Author_Templates
 *
 * @package paCMec
 * @subpackage Template
 */
 ...
 * @since 1.5.0
 *
 * @global MT_User $authordata The current author's data.
 *
 ...
 *
 * @see get_the_author()
 * @link https://developer.managertechnology.com.co/pacmec/reference/functions/the_author/
 *
 * @param string $deprecated      Deprecated.
 ...
 * author's name.
 *
 * @link https://developer.managertechnology.com.co/pacmec/reference/functions/the_author_link/
 *
 * @since 2.1.0
 */
 ...
 *
 * @link https://developer.managertechnology.com.co/pacmec/reference/functions/the_author_posts/
 * @since 0.71
 */
...
function get_author_posts_url( $author_id, $author_nicename = '' ) {
	...
	$link    = $mt_rewrite->get_author_permastruct();
 ...
 *
 * @link https://developer.managertechnology.com.co/pacmec/reference/functions/mt_list_authors/
 *
 * @since 1.2.0
 *
...
function mt_list_authors( $args = '' ) {
	...
	$query_args           = mt_array_slice_assoc( $args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
	$query_args['fields'] = 'ids';
	$authors              = get_users( $query_args );

	$author_count = array();
	foreach ( (array) $mtdb->get_results( "SELECT DISTINCT post_author, COUNT(ID) AS count FROM $mtdb->posts WHERE " . get_private_posts_cap_sql( 'post' ) . ' GROUP BY post_author' ) as $row ) {
...
function is_multi_author() {
	...
	if ( false === $is_multi_author ) {
		$rows            = (array) $mtdb->get_col( "SELECT DISTINCT post_author FROM $mtdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2" );
 ...
 * @access private
 */
function __clear_multi_author_cache() { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
