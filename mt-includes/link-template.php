<?php
/**
 * paCMec Link Template Functions
 *
 * @package paCMec
 * @subpackage Template
 */
 ...
 *
 * @param int|MT_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|MT_Post $post      Post ID, MT_Post object, or 0. Default 0.
	 */
...
function user_trailingslashit( $string, $type_of_url = '' ) {
	global $mt_rewrite;
	if ( $mt_rewrite->use_trailing_slashes ) {
 ...
 * @since 5.7.0
 *
 * @param MT_Post|int|null $post   Optional. Post ID or post object. Defaults to global $post.
 ...
 */
function mt_force_plain_post_permalink( $post = null, $sample = null ) {
	if (
		null === $sample &&
		is_object( $post ) &&
		isset( $post->filter ) &&
...
function get_permalink( $post = 0, $leavename = false ) {
	 ...
	 * @param MT_Post $post      The post in question.
	...
	if (
		$permalink &&
		! mt_force_plain_post_permalink( $post )
	) {
		...
		if ( strpos( $permalink, '%category%' ) !== false ) {
			...
			if ( $cats ) {
				 ...
				 * @since 3.5.0
				 *
				 * @param MT_Term  $cat  The category to use in the permalink.
				 * @param array    $cats Array of all categories (MT_Term objects) associated with the post.
				 * @param MT_Post  $post The post in question.
				 */
			...
			if ( empty( $category ) ) {
				...
				if ( $default_category && ! is_mt_error( $default_category ) ) {
 ...
 * @global MT_Rewrite $mt_rewrite paCMec rewrite component.
 *
 * @param int|MT_Post $id        Optional. Post ID or post object. Default is the global `$post`.
 * @param bool        $leavename Optional. Whether to keep post name. Default false.
 * @param bool        $sample    Optional. Is it a sample permalink. Default false.
 * @return string|MT_Error The post permalink.
 */
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $mt_rewrite->get_extra_permastruct( $post->post_type );

	$slug = $post->post_name;

	$force_plain_link = mt_force_plain_post_permalink( $post );
 ...
 * @since 1.5.0
 *
 * @param int|MT_Post $post      Optional. Post ID or object. Default uses the global `$post`.
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $mt_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the paCMec Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @global MT_Rewrite $mt_rewrite paCMec rewrite component.
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $force_plain_link || ! $parent_valid ) {
		$link = false;
	} elseif ( $mt_rewrite->using_permalinks() && $parent ) {
	...
	} elseif ( $mt_rewrite->using_permalinks() && ! $leavename ) {
...
function get_year_link( $year ) {
	...
	$yearlink = $mt_rewrite->get_year_permastruct();
	if ( ! empty( $yearlink ) ) {
...
function get_month_link( $year, $month ) {
	...
	$monthlink = $mt_rewrite->get_month_permastruct();
	if ( ! empty( $monthlink ) ) {
...
function get_day_link( $year, $month, $day ) {
	...
	$daylink = $mt_rewrite->get_day_permastruct();
	if ( ! empty( $daylink ) ) {
...
function get_feed_link( $feed = '' ) {
	...
	$permalink = $mt_rewrite->get_feed_permastruct();

	if ( $permalink ) {
		if ( false !== strpos( $feed, 'comments_' ) ) {
			$feed      = str_replace( 'comments_', '', $feed );
			$permalink = $mt_rewrite->get_comment_feed_permastruct();
		}
 ...
 * @param MT_Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
 ...
 * @param string  $before Optional. Display before edit link. Default empty.
 * @param string  $after  Optional. Display after edit link. Default empty.
 * @param MT_Term $term   Optional. Term object. If null, the queried object will be inspected. Default null.
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $mt_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] ) {
			$struct = $mt_rewrite->front . $struct;
		} else {
			$struct = $mt_rewrite->root . $struct;
		}
 ...
 * @since 4.4.0
 *
 * @param int|MT_Post $post         Optional. Post ID or `MT_Post` object. Defaults to global `$post`.
 ...
 *
 * Can be used within the paCMec loop or outside of it. Can be used with
 ...
 *
 * Can be used within the paCMec loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int|MT_Post $id           Optional. Post ID or post object. Default is the global `$post`.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', mt_nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 * @since 2.3.0
 *
 * @param int|MT_Comment $comment_id Optional. Comment ID or MT_Comment object.
 ...
 * @return null|string|MT_Post Post object if successful. Null if global $post is not set. Empty string if no
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		if ( $in_same_term ) {
			$join  .= " INNER JOIN $mtdb->term_relationships AS tr ON p.ID = tr.object_id INNER JOIN $mtdb->term_taxonomy tt ON tr.term_taxonomy_id = tt.term_taxonomy_id";
			$where .= $mtdb->prepare( 'AND tt.taxonomy = %s', $taxonomy );

			if ( ! is_object_in_taxonomy( $post->post_type, $taxonomy ) ) {
				return '';
			}
			$term_array = mt_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );

			// Remove any exclusions from the term array to include.
			$term_array = array_diff( $term_array, (array) $excluded_terms );
			$term_array = array_map( 'intval', $term_array );

			if ( ! $term_array || is_mt_error( $term_array ) ) {
				return '';
			}
		...
		if ( ! empty( $excluded_terms ) ) {
			$where .= " AND p.ID NOT IN ( SELECT tr.object_id FROM $mtdb->term_relationships tr LEFT JOIN $mtdb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.term_id IN (" . implode( ',', array_map( 'intval', $excluded_terms ) ) . ') )';
		}
	}
	...
	if ( is_user_logged_in() ) {
		...
		foreach ( (array) $private_states as $state ) {
			if ( current_user_can( $read_private_cap ) ) {
				$where .= $mtdb->prepare( ' OR p.post_status = %s', $state );
			} else {
				$where .= $mtdb->prepare( ' OR (p.post_author = %d AND p.post_status = %s)', $user_id, $state );
			}
		}
		$where .= ' )';
	} else {
	 ...
	 * @param MT_Post $post           MT_Post object.
	 */
	 ...
	 * @param MT_Post $post           MT_Post object.
	 */
	$where = apply_filters( "get_{$adjacent}_post_where", $mtdb->prepare( "WHERE p.post_date $op %s AND p.post_type = %s $where", $current_post_date, $post->post_type ), $in_same_term, $excluded_terms, $taxonomy, $post );
	...
	$query     = "SELECT p.ID FROM $mtdb->posts AS p $join $where $sort";
	$query_key = 'adjacent_post_' . md5( $query );
	$result    = mt_cache_get( $query_key, 'counts' );
	if ( false !== $result ) {
		if ( $result ) {
	...
	$result = $mtdb->get_var( $query );
	if ( null === $result ) {
		$result = '';
	}

	mt_cache_set( $query_key, $result, 'counts' );
 ...
 *
 * This is meant to be attached to actions like 'mt_head'. Do not call this directly in plugins
 * or theme templates.
 *
 * @since 3.0.0
 ...
 */
function adjacent_posts_rel_link_mt_head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string  $format   Link anchor format.
	 * @param string  $link     Link permalink format.
	 * @param MT_Post $post     The adjacent post.
...
function get_pagenum_link( $pagenum = 1, $escape = true ) {
	...
	if ( ! $mt_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$mt_rewrite->pagination_base/\d+/?$|", '', $request );
		$request = preg_replace( '|^' . preg_quote( $mt_rewrite->index, '|' ) . '|i', '', $request );
		$request = ltrim( $request, '/' );

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $mt_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' !== $request ) ) {
			$base .= $mt_rewrite->index . '/';
		}

		if ( $pagenum > 1 ) {
			$request = ( ( ! empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $mt_rewrite->pagination_base . '/' . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $mt_query;

	if ( ! $max_page ) {
		$max_page = $mt_query->max_num_pages;
	}
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( ! is_singular() ) {
		...
		$max_num_pages = $mt_query->max_num_pages;
...
function get_the_posts_navigation( $args = array() ) {
	...
	if ( $GLOBALS['mt_query']->max_num_pages > 1 ) {
		// Make sure the nav element has an aria-label attribute: fallback to the screen reader text.
		if ( ! empty( $args['screen_reader_text'] ) && empty( $args['aria_label'] ) ) {
			$args['aria_label'] = $args['screen_reader_text'];
		}
	...
	if ( 'newest' === get_option( 'default_comments_page' ) ) {
		if ( $pagenum != $max_page ) {
			if ( $mt_rewrite->using_permalinks() ) {
				$result = user_trailingslashit( trailingslashit( $result ) . $mt_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged' );
			} else {
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty( $max_page ) ) {
		$max_page = $mt_query->max_num_comment_pages;
	}
...
function paginate_comments_links( $args = array() ) {
	...
	if ( $mt_rewrite->using_permalinks() ) {
		$defaults['base'] = user_trailingslashit( trailingslashit( get_permalink() ) . $mt_rewrite->comments_pagination_base . '-%#%', 'commentpaged' );
	}
...
/**
 * Retrieves the URL for the current site where paCMec application files
 * (e.g. mt-blog-header.php or the mt-admin/ folder) are accessible.
 *
...
/**
 * Retrieves the URL for a given site where paCMec application files
 * (e.g. mt-blog-header.php or the mt-admin/ folder) are accessible.
 *
 * Returns the 'site_url' option with the appropriate protocol, 'https' if
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url( $blog_id, 'mt-admin/', $scheme );
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string $path Path relative to the URL to the mt-includes directory. Blank string
...
function content_url( $path = '' ) {
	$url = set_url_scheme( MT_CONTENT_URL );
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$plugin        = mt_normalize_path( $plugin );
	$mu_plugin_dir = mt_normalize_path( MTMU_PLUGIN_DIR );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url( 'mt-admin/network/', $scheme );
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url( 'mt-admin/user/', $scheme );
 ...
 *                      been published yet.
 */
function mt_get_canonical_url( $post = null ) {
	$post = get_post( $post );
 ...
 *
 * @since 2.9.0
 * @since 4.6.0 Adjusted to use `mt_get_canonical_url()`.
 */
 ...
 *                are not enabled.
 */
function mt_get_shortlink( $id = 0, $context = 'post', $allow_slugs = true ) {
	/**
 ...
 *
 * Attached to the {@see 'mt_head'} action.
 *
 * @since 3.0.0
 */
function mt_shortlink_mt_head() {
	$shortlink = mt_get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the {@see 'mt'} action.
 *
 * @since 3.0.0
 */
function mt_shortlink_header() {
	if ( headers_sent() ) {
		return;
	}
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = mt_get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof MT_User ) {
		// User object.
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof MT_Post ) {
		// Post object.
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof MT_Comment ) {
		...
		if ( ( ! $user || is_mt_error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
...
function get_privacy_policy_url() {
	$url            = '';
	$policy_page_id = (int) get_option( 'mt_page_for_privacy_policy' );
