<?php
/**
 * Session API: MT_Session_Tokens class
 *
 * @package paCMec
 * @subpackage Session
 * @since 4.7.0
 ...
 * @since 4.0.0
 */
abstract class MT_Session_Tokens {
	 ...
	 * the session manager for a subclass of `MT_Session_Tokens`.
	 *
	 * @since 4.0.0
	 *
	 * @param int $user_id User whose session to manage.
	 * @return MT_Session_Tokens The session object, which is by default an instance of
	 *                           the `MT_User_Meta_Session_Tokens` class.
	 */
	final public static function get_instance( $user_id ) {
		 ...
		 *                        Default 'MT_User_Meta_Session_Tokens'.
		 */
		$manager = apply_filters( 'session_token_manager', 'MT_User_Meta_Session_Tokens' );
	...
	final public function create( $expiration ) {
		...
		if ( ! empty( $_SERVER['HTTP_USER_AGENT'] ) ) {
			$session['ua'] = mt_unslash( $_SERVER['HTTP_USER_AGENT'] );
		}

		// Timestamp.
		$session['login'] = time();

		$token = mt_generate_password( 43, false, false );
	...
	final public static function destroy_all_for_all_users() {
		/** This filter is documented in mt-includes/class-mt-session-tokens.php */
		$manager = apply_filters( 'session_token_manager', 'MT_User_Meta_Session_Tokens' );
