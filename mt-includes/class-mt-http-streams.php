<?php
/**
 * HTTP API: MT_Http_Streams class
 *
 * @package paCMec
 * @subpackage HTTP
 * @since 4.4.0
 ...
 */
class MT_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see MT_Http::request For default options descriptions.
	 *
	 * @since 2.7.0
	 * @since 3.7.0 Combined with the fsockopen transport and switched to stream_socket_client().
	 *
	 ...
	 * @return array|MT_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A MT_Error instance upon error
	 */
	public function request( $url, $args = array() ) {
		...
		MT_Http::buildCookieHeader( $parsed_args );
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str ) {
				return new MT_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );
			}

			return new MT_Error( 'http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
			...
			while ( ! feof( $handle ) && $keep_reading ) {
				...
				if ( ! $bodyStarted ) {
					...
					if ( strpos( $strResponse, "\r\n\r\n" ) ) {
						$process     = MT_Http::processResponse( $strResponse );
						$bodyStarted = true;
						$block       = $process['body'];
						unset( $strResponse );
						$process['body'] = '';
				...
				if ( $bytes_written_to_file != $this_block_size ) {
					...
					return new MT_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$arrHeaders = MT_Http::processHeaders( $process['headers'], $url );
		...
		// Handle redirects.
		$redirect_response = MT_Http::handle_redirects( $url, $parsed_args, $response );
		...
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] )
			...
			$process['body'] = MT_Http::chunkTransferDecode( $process['body'] );
		}

		if ( true === $parsed_args['decompress'] && true === MT_Http_Encoding::should_decode( $arrHeaders['headers'] ) ) {
			$process['body'] = MT_Http_Encoding::decompress( $process['body'] );
		}
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( MT_Http::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of MT_Http directly through its API.
 *
 * @see MT_HTTP::request
 *
 * @since 2.7.0
 * @deprecated 3.7.0 Please use MT_HTTP::request() directly
 */
class MT_HTTP_Fsockopen extends MT_HTTP_Streams {
