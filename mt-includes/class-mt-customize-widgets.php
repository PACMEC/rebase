<?php
/**
 * paCMec Customize Widgets classes
 *
 * @package paCMec
 ...
 */
final class MT_Customize_Widgets {
	...
	public function __construct( $manager ) {
		...
		// See https://github.com/xmt/mt-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L420-L449
		...
		add_action( 'mt_loaded', array( $this, 'override_sidebars_widgets_for_theme_switch' ) );
	...
	public function get_selective_refreshable_widgets() {
		...
		if ( ! isset( $this->selective_refreshable_widgets ) ) {
			...
			foreach ( $mt_widget_factory->widgets as $mt_widget ) {
				$this->selective_refreshable_widgets[ $mt_widget->id_base ] = ! empty( $mt_widget->widget_options['customize_selective_refresh'] );
			}
		}
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( mt_unslash( $_REQUEST['widget-id'] ) );
		}
	 ...
	 * @since 4.2.0
	 *
	 * @param false|array $args       The arguments to the MT_Customize_Setting constructor.
	...
	public function override_sidebars_widgets_for_theme_switch() {
		...
		$this->old_sidebars_widgets = mt_get_sidebars_widgets();
		...
		// Reset global cache var used by mt_get_sidebars_widgets().
		unset( $GLOBALS['_mt_sidebars_widgets'] );
	}
	 ...
	 * @since 3.9.0
	 *
	 * @see MT_Customize_Widgets::handle_theme_switch()
	 *
	 ...
	 *
	 * Note: these actions are also fired in mt_ajax_update_widget().
	 *
	 * @since 3.9.0
	 */
	 ...
	 *
	 * When in preview, hook to {@see 'customize_register'} for settings after paCMec is loaded
	...
	public function schedule_customize_register() {
		if ( is_admin() ) {
			...
			add_action( 'mt', array( $this, 'customize_register' ) );
		}
	}
	 ...
	 * @global array $mt_registered_sidebars
	 */
	public function customize_register() {
		global $mt_registered_widgets, $mt_registered_widget_controls, $mt_registered_sidebars;

		add_filter( 'sidebars_widgets', array( $this, 'preview_sidebars_widgets' ), 1 );

		$sidebars_widgets = array_merge(
			array( 'mt_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $mt_registered_sidebars ), array() ),
			mt_get_sidebars_widgets()
		);
		 ...
		 */
		foreach ( array_keys( $mt_registered_widgets ) as $widget_id ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_inactive_widgets   = ( 'mt_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title'       => $mt_registered_sidebars[ $sidebar_id ]['name'],
						'description' => $mt_registered_sidebars[ $sidebar_id ]['description'],
						'priority'    => array_search( $sidebar_id, array_keys( $mt_registered_sidebars ), true ),
						'panel'       => 'widgets',
					...
					$section = new MT_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					$control           = new MT_Widget_Area_Customize_Control(
						$this->manager,
						$setting_id,
						array(
							'section'    => $section_id,
			...
			foreach ( $sidebar_widget_ids as $i => $widget_id ) {
				...
				if ( ! $is_active_sidebar || ! isset( $mt_registered_widgets[ $widget_id ] ) ) {
					continue;
				}

				$registered_widget = $mt_registered_widgets[ $widget_id ];
				$setting_id        = $this->get_setting_id( $widget_id );
				$id_base           = $mt_registered_widget_controls[ $widget_id ]['id_base'];

				$control = new MT_Widget_Form_Customize_Control(
					$this->manager,
					$setting_id,
					array(
						...
						'priority'       => $i,
						'width'          => $mt_registered_widget_controls[ $widget_id ]['width'],
						'height'         => $mt_registered_widget_controls[ $widget_id ]['height'],
	 ...
	 * @since 4.4.0
	 *
	 * @see MT_Customize_Panel::$active_callback
	 *
	 * @global array $mt_registered_sidebars
	 * @return bool Active.
	 */
	public function is_panel_active() {
		...
		return ! empty( $mt_registered_sidebars );
	}
	 ...
	 * @return array|MT_Error Array containing a widget's id_base and number components,
	 *                        or a MT_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new MT_Error( 'widget_setting_invalid_id' );
		}
	...
	public function print_styles() {
		...
		do_action( 'admin_print_styles-widgets.php' ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
	...
	public function print_scripts() {
		/** This action is documented in mt-admin/admin-header.php */
		do_action( 'admin_print_scripts-widgets.php' ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
	...
	public function enqueue_scripts() {
		global $mt_scripts, $mt_registered_sidebars, $mt_registered_widgets;

		mt_enqueue_style( 'customize-widgets' );
		mt_enqueue_script( 'customize-widgets' );
		...
		$registered_sidebar_count            = count( $mt_registered_sidebars );
		...
		$settings = array(
			'registeredSidebars'          => array_values( $mt_registered_sidebars ),
			'registeredWidgets'           => $mt_registered_widgets,
		...
		$mt_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _mtCustomizeWidgetsSettings = %s;', mt_json_encode( $settings ) )
		);
	}
	...
	public function print_footer_scripts() {
		...
		do_action( 'admin_print_footer_scripts-widgets.php' ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores

		/** This action is documented in mt-admin/admin-footer.php */
		do_action( 'admin_print_footer_scripts' );

		/** This action is documented in mt-admin/admin-footer.php */
		do_action( 'admin_footer-widgets.php' ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
	}
	...
	public function get_available_widgets() {
		...
		global $mt_registered_widgets, $mt_registered_widget_controls;
		require_once ABSPATH . 'mt-admin/includes/widgets.php'; // For next_widget_id_number().

		$sort = $mt_registered_widgets;
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $mt_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $mt_registered_widget_controls[ $widget['id'] ]['id_base'];
				...
				if ( $sidebar && 'mt_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = mt_list_widget_controls_dynamic_sidebar(
				array(
					0 => $args,
					1 => $widget['params'][0],
				)
			...
			$available_widget = array_merge(
				...
				array(
					...
					'width'        => $mt_registered_widget_controls[ $widget['id'] ]['width'],
					'height'       => $mt_registered_widget_controls[ $widget['id'] ]['height'],
	...
	public function customize_preview_init() {
		add_action( 'mt_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'mt_print_styles', array( $this, 'print_preview_css' ), 1 );
		add_action( 'mt_footer', array( $this, 'export_preview_data' ), 20 );
	}
	...
	public function refresh_nonces( $nonces ) {
		$nonces['update-widget'] = mt_create_nonce( 'update-widget' );
		return $nonces;
	}
	 ...
	 *
	 * Because mt_get_sidebars_widgets() gets called early at {@see 'init' } (via
	 * mt_convert_widget_settings()) and can set global variable `$_mt_sidebars_widgets`
	...
	public function customize_preview_enqueue() {
		mt_enqueue_script( 'customize-preview-widgets' );
	}
	 ...
	 * and at the very end of the mt_footer,
	 *
	 * @since 3.9.0
	 *
	 * @global array $mt_registered_sidebars
	...
	public function export_preview_data() {
		...
		<script type="text/javascript">
			var _mtWidgetCustomizerPreviewSettings = <?php echo mt_json_encode( $settings ); ?>;
		</script>
		<?php
	}
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return mt_hash( $serialized_instance );
	}
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $mt_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 *
	 * @param string $widget_id Widget ID.
	 * @return array|MT_Error Array containing the updated widget information.
	 *                        A MT_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $mt_registered_widget_updates, $mt_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new MT_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new MT_Error( 'widget_setting_unsanitized' );
			}
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new MT_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new MT_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from MT_Customize_Setting::preview() will use this value
	 ...
	 *
	 * Most code here copied from mt_ajax_save_widget().
	 *
	 * @since 3.9.0
	 *
	 * @see mt_ajax_save_widget()
	 */
	public function mt_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			mt_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			mt_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_mt_error( $updated_widget ) ) {
			mt_send_json_error( $updated_widget->get_error_code() );
		}

		$form     = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'] );

		mt_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function selective_refresh_init() {
		...
		add_filter( 'mt_kses_allowed_html', array( $this, 'filter_mt_kses_allowed_data_attributes' ) );
	 ...
	 * }
	 * @see MT_Customize_Nav_Menus::filter_mt_nav_menu_args()
	 *
	 * @return array Params.
	 */
	public function filter_dynamic_sidebar_params( $params ) {
		...
		$attributes                   .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( mt_json_encode( $context ) ) );
	 ...
	 *
	 * This is used in the {@see 'filter_mt_kses_allowed_html'} filter to ensure that the
	 ...
	 *
	 * This is needed in case the `$before_widget` is run through mt_kses() when printed.
	 *
	 * @since 4.5.0
	 *
	 ...
	 */
	public function filter_mt_kses_allowed_data_attributes( $allowed_html ) {
	...
	public function capture_filter_pre_get_option( $value ) {
		...
		if ( isset( $this->_captured_options[ $option_name ] ) ) {
			...
			/** This filter is documented in mt-includes/option.php */
