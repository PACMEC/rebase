 ...
 * @since 3.0.0
 *
 * @param int|string|MT_Term $menu Menu ID, slug, name, or object.
 * @return MT_Term|false Menu object on success, false if $menu param isn't supplied or term does not exist.
 */
function mt_get_nav_menu_object( $menu ) {
	$menu_obj = false;
	...
	if ( ! $menu_obj || is_mt_error( $menu_obj ) ) {
		$menu_obj = false;
	}

	/**
	 * Filters the nav_menu term retrieved for mt_get_nav_menu_object().
	 *
	 * @since 4.3.0
	 *
	 * @param MT_Term|false      $menu_obj Term from nav_menu taxonomy, or false if nothing had been found.
	 * @param int|string|MT_Term $menu     The menu ID, slug, name, or object passed to mt_get_nav_menu_object().
	 */
	return apply_filters( 'mt_get_nav_menu_object', $menu_obj, $menu );
}
 ...
 * @since 3.0.0
 *
 * @param int|string|MT_Term $menu Menu ID, slug, name, or object of menu to check.
...
function is_nav_menu( $menu ) {
	...
	$menu_obj = mt_get_nav_menu_object( $menu );

	if (
		$menu_obj &&
		! is_mt_error( $menu_obj ) &&
 ...
 * @since 3.0.0
 *
 * @global array $_mt_registered_nav_menus
 *
 * @param array $locations Associative array of menu location identifiers (like a slug) and descriptive text.
 */
function register_nav_menus( $locations = array() ) {
	global $_mt_registered_nav_menus;
	...
	$_mt_registered_nav_menus = array_merge( (array) $_mt_registered_nav_menus, $locations );
}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $_mt_registered_nav_menus ) && isset( $_mt_registered_nav_menus[ $location ] ) ) {
		unset( $_mt_registered_nav_menus[ $location ] );
		if ( empty( $_mt_registered_nav_menus ) ) {
...
function get_registered_nav_menus() {
	global $_mt_registered_nav_menus;
	if ( isset( $_mt_registered_nav_menus ) ) {
		return $_mt_registered_nav_menus;
	}
	return array();
}
 ...
 */
function mt_get_nav_menu_name( $location ) {
	$menu_name = '';

	$locations = get_nav_menu_locations();

	if ( isset( $locations[ $location ] ) ) {
		$menu = mt_get_nav_menu_object( $locations[ $location ] );
	 ...
	 * @param string $location  Menu location identifier.
	 */
	return apply_filters( 'mt_get_nav_menu_name', $menu_name, $location );
}
...
function is_nav_menu_item( $menu_item_id = 0 ) {
	return ( ! is_mt_error( $menu_item_id ) && ( 'nav_menu_item' === get_post_type( $menu_item_id ) ) );
}
 ...
 *
 * @param string $menu_name Menu name.
 * @return int|MT_Error Menu ID on success, MT_Error object on failure.
 */
function mt_create_nav_menu( $menu_name ) {
	// expected_slashed ($menu_name)
	return mt_update_nav_menu_object( 0, array( 'menu-name' => $menu_name ) );
}
 ...
 *
 * @param int|string|MT_Term $menu Menu ID, slug, name, or object.
 * @return bool|MT_Error True on success, false or MT_Error object on failure.
 */
function mt_delete_nav_menu( $menu ) {
	$menu = mt_get_nav_menu_object( $menu );
	if ( ! $menu ) {
		return false;
	}
	...
	$result = mt_delete_term( $menu->term_id, 'nav_menu' );
	...
	if ( $result && ! is_mt_error( $result ) ) {
		 ...
		 */
		do_action( 'mt_delete_nav_menu', $menu->term_id );
	}
 ...
 * @return int|MT_Error Menu ID on success, MT_Error object on failure.
 */
function mt_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	// expected_slashed ($menu_data)
	$menu_id = (int) $menu_id;

	$_menu = mt_get_nav_menu_object( $menu_id );
	...
	if (
		$_possible_existing &&
		! is_mt_error( $_possible_existing ) &&
	...
	if ( ! $_menu || is_mt_error( $_menu ) ) {
		...
		$_menu = mt_insert_term( $menu_data['menu-name'], 'nav_menu', $args );

		if ( is_mt_error( $_menu ) ) {
			return $_menu;
		}
		 ...
		 */
		do_action( 'mt_create_nav_menu', $_menu['term_id'], $menu_data );
	...
	$update_response = mt_update_term( $menu_id, 'nav_menu', $args );

	if ( is_mt_error( $update_response ) ) {
	 ...
	 * @param array $menu_data An array of menu data.
	 */
	do_action( 'mt_update_nav_menu', $menu_id, $menu_data );
	return $menu_id;
}
 ...
 * to be pre-slashed since they are passed directly into `mt_insert_post()`.
 *
 * @since 3.0.0
 *
 ...
 * @return int|MT_Error The menu item's database ID or MT_Error object on failure.
 */
function mt_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
	$menu_id         = (int) $menu_id;
	$menu_item_db_id = (int) $menu_item_db_id;

	// Make sure that we don't convert non-nav_menu_item objects into nav_menu_item objects.
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
		return new MT_Error( 'update_nav_menu_item_failed', __( 'The given object ID is not that of a menu item.' ) );
	}

	$menu = mt_get_nav_menu_object( $menu_id );

	if ( ! $menu && 0 !== $menu_id ) {
		return new MT_Error( 'invalid_menu_id', __( 'Invalid menu ID.' ) );
	}
	...
	$args = mt_parse_args( $menu_item_data, $defaults );

	if ( 0 == $menu_id ) {
		$args['menu-item-position'] = 1;
	} elseif ( 0 == (int) $args['menu-item-position'] ) {
		$menu_items                 = 0 == $menu_id ? array() : (array) mt_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
	...
	if ( 'custom' === $args['menu-item-type'] ) {
		...
		if ( mt_unslash( $args['menu-item-title'] ) === mt_specialchars_decode( $original_title ) ) {
			$args['menu-item-title'] = '';
		}

		// Hack to get mt to create a post object when too many properties are empty.
	...
	$post_date = mt_resolve_post_date( $args['menu-item-post-date'], $args['menu-item-post-date-gmt'] );
	if ( $post_date ) {
	...
	if ( ! $update ) {
		...
		$menu_item_db_id     = mt_insert_post( $post );
		if ( ! $menu_item_db_id || is_mt_error( $menu_item_db_id ) ) {
		 ...
		 * @since 4.4.0
		 *
		 * @see mt_update_nav_menu_item()
		 *
		 * @param int   $menu_id         ID of the updated menu.
		 * @param int   $menu_item_db_id ID of the new menu item.
		 * @param array $args            An array of arguments used to update/add the menu item.
		 */
		do_action( 'mt_add_nav_menu_item', $menu_id, $menu_item_db_id, $args );
	}

	// Associate the menu item with the menu term.
	// Only set the menu term if it isn't set to avoid unnecessary mt_get_object_terms().
	if ( $menu_id && ( ! $update || ! is_object_in_term( $menu_item_db_id, 'nav_menu', (int) $menu->term_id ) ) ) {
		mt_set_object_terms( $menu_item_db_id, array( $menu->term_id ), 'nav_menu' );
	}
	 ...
	 */
	do_action( 'mt_update_nav_menu_item', $menu_id, $menu_item_db_id, $args );
 ...
 *                    Default empty array.
 * @return MT_Term[] An array of menu objects.
 */
function mt_get_nav_menus( $args = array() ) {
	$defaults = array(
		'taxonomy'   => 'nav_menu',
		'hide_empty' => false,
		'orderby'    => 'name',
	 ...
	 * @see get_terms()
	 *
	 * @param MT_Term[] $menus An array of menu objects.
	 * @param array     $args  An array of arguments used to retrieve menu objects.
	 */
	return apply_filters( 'mt_get_nav_menus', get_terms( $args ), $args );
}
 ...
 */
function mt_get_nav_menu_items( $menu, $args = array() ) {
	$menu = mt_get_nav_menu_object( $menu );
	...
	if ( empty( $fetched[ $menu->term_id ] ) && ! mt_using_ext_object_cache() ) {
	...
	$items = array_map( 'mt_setup_nav_menu_item', $items );
	 ...
	 */
	return apply_filters( 'mt_get_nav_menu_items', $items, $menu, $args );
}
 ...
 */
function mt_setup_nav_menu_item( $menu_item ) {
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' === $menu_item->post_type ) {
			...
			if ( 'post_type' === $menu_item->type ) {
				...
				if ( $object ) {
					...
					if ( function_exists( 'get_post_states' ) ) {
						...
						if ( $post_states ) {
							$menu_item->type_label = mt_strip_all_tags( implode( ', ', $post_states ) );
						}
					}
				} else {
				...
				$post_content          = mt_trim_words( $menu_item->post_content, 200 );
				...
				if ( $original_object && ! is_mt_error( $original_object ) ) {
			...
			if ( ! isset( $menu_item->description ) ) {
				 ...
				 */
				$menu_item->description = apply_filters( 'nav_menu_description', mt_trim_words( $menu_item->post_content, 200 ) );
			}
			...
			/** This filter is documented in mt-includes/nav-menu.php */
	 ...
	 */
	return apply_filters( 'mt_setup_nav_menu_item', $menu_item );
}
 ...
 */
function mt_get_associated_nav_menu_items( $object_id = 0, $object_type = 'post_type', $taxonomy = '' ) {
 ...
 */
function _mt_delete_post_menu_item( $object_id ) {
	$object_id = (int) $object_id;

	$menu_item_ids = mt_get_associated_nav_menu_items( $object_id, 'post_type' );
 ...
 */
function _mt_delete_tax_menu_item( $object_id, $tt_id, $taxonomy ) {
	$object_id = (int) $object_id;

	$menu_item_ids = mt_get_associated_nav_menu_items( $object_id, 'taxonomy', $taxonomy );
 ...
 * @param MT_Post $post       The post object being transitioned from one status to another.
 */
function _mt_auto_add_pages_to_menu( $new_status, $old_status, $post ) {
	...
	foreach ( $auto_add as $menu_id ) {
		$items = mt_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		if ( ! is_array( $items ) ) {
			continue;
		}
		foreach ( $items as $item ) {
		...
		mt_update_nav_menu_item( $menu_id, 0, $args );
	}
}
 ...
 */
function _mt_delete_customize_changeset_dependent_auto_drafts( $post_id ) {
	...
	remove_action( 'delete_post', '_mt_delete_customize_changeset_dependent_auto_drafts' );
	foreach ( $data['nav_menus_created_posts']['value'] as $stub_post_id ) {
		...
		if ( 'auto-draft' === get_post_status( $stub_post_id ) ) {
			mt_delete_post( $stub_post_id, true );
		} elseif ( 'draft' === get_post_status( $stub_post_id ) ) {
			mt_trash_post( $stub_post_id );
 ...
 * @since 4.9.0
 */
function _mt_menus_changed() {
	$old_nav_menu_locations    = get_option( 'theme_switch_menu_locations', array() );
	$new_nav_menu_locations    = get_nav_menu_locations();
	$mapped_nav_menu_locations = mt_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations );
 ...
 */
function mt_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations ) {
	$registered_nav_menus   = get_registered_nav_menus();
