<?php
/**
 * MT_Application_Passwords class
 *
 * @package paCMec
 * @since   5.6.0
 */
 ...
 * @package paCMec
 */
class MT_Application_Passwords {
	 ...
	 *
	 * @since 5.6.0
	 * @since 5.7.0 Returns MT_Error if application name already exists.
	 *
	 * @param int   $user_id  User ID.
	 * @param array $args     Information about the application password.
	 * @return array|MT_Error The first key in the array is the new password, the second is its detailed information.
	 *                        A MT_Error instance is returned on error.
	 */
	public static function create_new_application_password( $user_id, $args = array() ) {
		...
		if ( empty( $args['name'] ) ) {
			return new MT_Error( 'application_password_empty_name', __( 'An application name is required to create an application password.' ), array( 'status' => 400 ) );
		}

		if ( self::application_name_exists_for_user( $user_id, $args['name'] ) ) {
			return new MT_Error( 'application_password_duplicate_name', __( 'Each application name should be unique.' ), array( 'status' => 409 ) );
		}

		$new_password    = mt_generate_password( static::PW_LENGTH, false );
		$hashed_password = mt_hash_password( $new_password );

		$new_item = array(
			'uuid'      => mt_generate_uuid4(),
		...
		if ( ! $saved ) {
			return new MT_Error( 'db_error', __( 'Could not save application password.' ) );
		}
		 ...
		 */
		do_action( 'mt_create_application_password', $user_id, $new_item, $new_password, $args );
	...
	public static function get_user_application_passwords( $user_id ) {
		...
		foreach ( $passwords as $i => $password ) {
			if ( ! isset( $password['uuid'] ) ) {
				$passwords[ $i ]['uuid'] = mt_generate_uuid4();
				$save                    = true;
			}
		}
	 ...
	 * @return true|MT_Error True if successful, otherwise a MT_Error instance is returned on error.
	 */
	public static function update_application_password( $user_id, $uuid, $update = array() ) {
		...
		foreach ( $passwords as &$item ) {
			 ...
			 */
			do_action( 'mt_update_application_password', $user_id, $item, $update );

			return true;
		}

		return new MT_Error( 'application_password_not_found', __( 'Could not find an application password with that id.' ) );
	}
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's uuid.
	 * @return true|MT_Error True if the usage was recorded, a MT_Error if an error occurs.
	 */
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's uuid.
	 * @return true|MT_Error Whether the password was successfully found and deleted, a MT_Error otherwise.
	 */
	public static function delete_application_password( $user_id, $uuid ) {
		...
		foreach ( $passwords as $key => $item ) {
			if ( $item['uuid'] === $uuid ) {
				...
				if ( ! $saved ) {
					return new MT_Error( 'db_error', __( 'Could not delete application password.' ) );
				}
				 ...
				 */
				do_action( 'mt_delete_application_password', $user_id, $item );
	 ...
	 *
	 * @param int $user_id User ID.
	 * @return int|MT_Error The number of passwords that were deleted or a MT_Error on failure.
	 */
	public static function delete_all_application_passwords( $user_id ) {
		...
		if ( $passwords ) {
			...
			if ( ! $saved ) {
				return new MT_Error( 'db_error', __( 'Could not delete application passwords.' ) );
			}

			foreach ( $passwords as $item ) {
				/** This action is documented in mt-includes/class-mt-application-passwords.php */
				do_action( 'mt_delete_application_password', $user_id, $item );
			}
