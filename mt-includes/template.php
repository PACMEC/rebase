...
function get_post_type_archive_template() {
	...
	if ( ! ( $obj instanceof MT_Post_Type ) || ! $obj->has_archive ) {
		return '';
	}
 ...
 *
 * Searches in the STYLESHEETPATH before TEMPLATEPATH and mt-includes/theme-compat
...
function locate_template( $template_names, $load = false, $require_once = true, $args = array() ) {
	...
	foreach ( (array) $template_names as $template_name ) {
		...
		if ( file_exists( STYLESHEETPATH . '/' . $template_name ) ) {
			...
			break;
		} elseif ( file_exists( ABSPATH . MTINC . '/theme-compat/' . $template_name ) ) {
			$located = ABSPATH . MTINC . '/theme-compat/' . $template_name;
			break;
		}
	}
...
/**
 * Require the template file with paCMec environment.
 *
 * The globals are set up for the template file to ensure that the paCMec
...
function load_template( $_template_file, $require_once = true, $args = array() ) {
	global $posts, $post, $mt_did_header, $mt_query, $mt_rewrite, $mtdb, $mt_version, $mt, $id, $comment, $user_ID;

	if ( is_array( $mt_query->query_vars ) ) {
		/*
		 ...
		 */
		// phpcs:ignore paCMec.PHP.DontExtract.extract_extract
		extract( $mt_query->query_vars, EXTR_SKIP );
	}
