...
/**
 * Returns an array of MT_Theme objects based on the arguments.
 *
 * Despite advances over get_themes(), this function is quite expensive, and grows
 * linearly with additional themes. Stick to mt_get_theme() if possible.
 *
 * @since 3.4.0
 *
 ...
 * }
 * @return MT_Theme[] Array of MT_Theme objects.
 */
function mt_get_themes( $args = array() ) {
	global $mt_theme_directories;
	...
	if ( is_array( $mt_theme_directories ) && count( $mt_theme_directories ) > 1 ) {
		// Make sure the current theme wins out, in case search_theme_directories() picks the wrong
		// one in the case of a conflict. (Normally, last registered theme root wins.)
		$current_theme = get_stylesheet();
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			$root_of_current_theme = get_raw_theme_root( $current_theme );
			if ( ! in_array( $root_of_current_theme, $mt_theme_directories, true ) ) {
				$root_of_current_theme = MT_CONTENT_DIR . $root_of_current_theme;
			}
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, MT_Theme::get_allowed_on_network() );
		} elseif ( 'site' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, MT_Theme::get_allowed_on_site( $args['blog_id'] ) );
		} elseif ( $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, MT_Theme::get_allowed( $args['blog_id'] ) );
		} else {
			$theme_directories = array_diff_key( $theme_directories, MT_Theme::get_allowed( $args['blog_id'] ) );
		}
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) ) {
			...
			$themes[ $theme ] = new MT_Theme( $theme, $theme_root['theme_root'] );
	...
	if ( null !== $args['errors'] ) {
		foreach ( $themes as $theme => $mt_theme ) {
			if ( $mt_theme->errors() != $args['errors'] ) {
				unset( $themes[ $theme ] );
			}
		}
	}
...
/**
 * Gets a MT_Theme object for a theme.
 *
 * @since 3.4.0
 *
 * @global array $mt_theme_directories
 ...
 * @return MT_Theme Theme object. Be sure to check the object's exists() method
 *                  if you need to confirm the theme's existence.
 */
function mt_get_theme( $stylesheet = '', $theme_root = '' ) {
	global $mt_theme_directories;
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root ) {
			$theme_root = MT_CONTENT_DIR . '/themes';
		} elseif ( ! in_array( $theme_root, (array) $mt_theme_directories, true ) ) {
			$theme_root = MT_CONTENT_DIR . $theme_root;
		}
	}

	return new MT_Theme( $stylesheet, $theme_root );
}

/**
 * Clears the cache held by get_theme_roots() and MT_Theme.
 *
 * @since 3.5.0
 * @param bool $clear_update_cache Whether to clear the theme updates cache.
 */
function mt_clean_themes_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_themes' );
	}
	search_theme_directories( true );
	foreach ( mt_get_themes( array( 'errors' => null ) ) as $theme ) {
		$theme->cache_delete();
	}
}
 ...
 * paCMec workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	} elseif ( ! empty( $mt_locale->text_direction ) && file_exists( "$dir/{$mt_locale->text_direction}.css" ) ) {
		$stylesheet_uri = "$stylesheet_dir_uri/{$mt_locale->text_direction}.css";
	} else {
		$stylesheet_uri = '';
	}
	/**
 ...
 *                          or a folder within MT_CONTENT_DIR.
...
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = MT_CONTENT_DIR . '/' . $directory;
	...
	if ( ! is_array( $mt_theme_directories ) ) {
		$mt_theme_directories = array();
	}

	$untrailed = untrailingslashit( $directory );
	if ( ! empty( $untrailed ) && ! in_array( $untrailed, $mt_theme_directories, true ) ) {
		$mt_theme_directories[] = $untrailed;
	}
...
function search_theme_directories( $force = false ) {
	...
	$mt_theme_directories = (array) $mt_theme_directories;
	 ...
	 */
	foreach ( $mt_theme_directories as $theme_root ) {
		if ( 0 === strpos( $theme_root, MT_CONTENT_DIR ) ) {
			$relative_theme_roots[ str_replace( MT_CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		} else {
	 ...
	 */
	$cache_expiration = apply_filters( 'mt_cache_themes_persistently', false, 'search_theme_directories' );
	...
	foreach ( $mt_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				// mt-content/themes/a-single-theme
				// mt-content/themes is $theme_root, a-single-theme is $dir.
				...
				$found_theme = false;
				// mt-content/themes/a-folder-of-themes/*
				// mt-content/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs.
				...
				// Return it; MT_Theme will catch the error.
				if ( ! $found_theme ) {
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
						'theme_root' => $theme_root,
...
function get_theme_root( $stylesheet_or_template = '' ) {
	...
	if ( $stylesheet_or_template ) {
		...
		if ( $theme_root ) {
			// Always prepend MT_CONTENT_DIR unless the root currently registered as a theme directory.
			// This gives relative theme roots the benefit of the doubt when things go haywire.
			if ( ! in_array( $theme_root, (array) $mt_theme_directories, true ) ) {
				$theme_root = MT_CONTENT_DIR . $theme_root;
			}
		}
	}
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $mt_theme_directories, true ) ) {
			...
			if ( 0 === strpos( $theme_root, MT_CONTENT_DIR ) ) {
				$theme_root_uri = content_url( str_replace( MT_CONTENT_DIR, '', $theme_root ) );
			} elseif ( 0 === strpos( $theme_root, ABSPATH ) ) {
				$theme_root_uri = site_url( str_replace( ABSPATH, '', $theme_root ) );
			} elseif ( 0 === strpos( $theme_root, MT_PLUGIN_DIR ) || 0 === strpos( $theme_root, MTMU_PLUGIN_DIR ) ) {
	 ...
	 * @param string $siteurl                paCMec web address which is set in General Options.
...
function switch_theme( $stylesheet ) {
	global $mt_theme_directories, $mt_customize, $sidebars_widgets;

	$requirements = validate_theme_requirements( $stylesheet );
	if ( is_mt_error( $requirements ) ) {
		mt_die( $requirements );
	}

	$_sidebars_widgets = null;
	if ( 'mt_ajax_customize_save' === current_action() ) {
		$old_sidebars_widgets_data_setting = $mt_customize->get_setting( 'old_sidebars_widgets_data' );
		if ( $old_sidebars_widgets_data_setting ) {
			$_sidebars_widgets = $mt_customize->post_value( $old_sidebars_widgets_data_setting );
		}
	...
	$old_theme = mt_get_theme();
	$new_theme = mt_get_theme( $stylesheet );
	$template  = $new_theme->get_template();

	if ( mt_is_recovery_mode() ) {
		$paused_themes = mt_paused_themes();
	...
	if ( count( $mt_theme_directories ) > 1 ) {
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the Customizer when accessing mt-admin/widgets.php.
		 */
		if ( 'mt_ajax_customize_save' === current_action() ) {
	 ...
	 * @param MT_Theme $new_theme MT_Theme instance of the new theme.
	 * @param MT_Theme $old_theme MT_Theme instance of the old theme.
	 */
...
function validate_current_theme() {
	 ...
	 */
	if ( mt_installing() || ! apply_filters( 'validate_current_theme', true ) ) {
		return true;
	}
	...
	$default = mt_get_theme( MT_DEFAULT_THEME );
	if ( $default->exists() ) {
	...
	/**
	 * If we're in an invalid state but MT_DEFAULT_THEME doesn't exist,
	 * switch to the latest core default theme that's installed.
	 *
	 * If it turns out that this latest core default theme is our current
	 * theme, then there's nothing we can do about that, so we have to bail,
	 * rather than going into an infinite loop. (This is why there are
	 * checks against MT_DEFAULT_THEME above, also.) We also can't do anything
	 * if it turns out there is no default theme installed. (That's `false`.)
	 */
	$default = MT_Theme::get_core_default_theme();
...
/**
 * Validates the theme requirements for paCMec version and PHP version.
 *
 ...
 * @return true|MT_Error True if requirements are met, MT_Error on failure.
 */
function validate_theme_requirements( $stylesheet ) {
	$theme = mt_get_theme( $stylesheet );

	$requirements = array(
		'requires'     => ! empty( $theme->get( 'RequiresMT' ) ) ? $theme->get( 'RequiresMT' ) : '',
	...
	$compatible_mt  = is_mt_version_compatible( $requirements['requires'] );
	$compatible_php = is_php_version_compatible( $requirements['requires_php'] );

	if ( ! $compatible_mt && ! $compatible_php ) {
		return new MT_Error(
			'theme_mt_php_incompatible',
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current paCMec and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new MT_Error(
			'theme_mt_incompatible',
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current paCMec version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
...
function get_theme_mods() {
	...
	if ( false === $mods ) {
		...
		if ( false === $theme_name ) {
			$theme_name = mt_get_theme()->get( 'Name' );
		}
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filter is documented in mt-includes/theme.php */
...
function get_header_image_tag( $attr = array() ) {
	...
	if ( empty( $attr['srcset'] ) && ! empty( $header->attachment_id ) ) {
		$image_meta = get_post_meta( $header->attachment_id, '_mt_attachment_metadata', true );
		$size_array = array( $width, $height );

		if ( is_array( $image_meta ) ) {
			$srcset = mt_calculate_image_srcset( $size_array, $header->url, $image_meta, $header->attachment_id );
			$sizes  = ! empty( $attr['sizes'] ) ? $attr['sizes'] : mt_calculate_image_sizes( $size_array, $header->url, $image_meta, $header->attachment_id );
 ...
 * @access private
 *
 * @global array $_mt_default_headers
 *
 * @return object
 */
function _get_random_header_data() {
	static $_mt_random_header = null;

	if ( empty( $_mt_random_header ) ) {
		global $_mt_default_headers;
		$header_image_mod = get_theme_mod( 'header_image', '' );
		$headers          = array();

		if ( 'random-uploaded-image' === $header_image_mod ) {
			$headers = get_uploaded_header_images();
		} elseif ( ! empty( $_mt_default_headers ) ) {
			if ( 'random-default-image' === $header_image_mod ) {
				$headers = $_mt_default_headers;
			} else {
		...
		$_mt_random_header = (object) $headers[ array_rand( $headers ) ];

		$_mt_random_header->url           = sprintf( $_mt_random_header->url, get_template_directory_uri(), get_stylesheet_directory_uri() );
		$_mt_random_header->thumbnail_url = sprintf( $_mt_random_header->thumbnail_url, get_template_directory_uri(), get_stylesheet_directory_uri() );
	}

	return $_mt_random_header;
}
...
function get_uploaded_header_images() {
	...
	$headers = get_posts(
		array(
			...
			'meta_key'   => '_mt_attachment_is_custom_header',
	...
	foreach ( (array) $headers as $header ) {
		$url          = esc_url_raw( mt_get_attachment_url( $header->ID ) );
		$header_data  = mt_get_attachment_metadata( $header->ID );
		...
		$header_images[ $header_index ]['alt_text']          = get_post_meta( $header->ID, '_mt_attachment_image_alt', true );
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			...
			if ( ! empty( $_mt_default_headers ) ) {
				foreach ( (array) $_mt_default_headers as $default_header ) {
	...
	return (object) mt_parse_args( $data, $default );
}
...
function register_default_headers( $headers ) {
	...
	$_mt_default_headers = array_merge( (array) $_mt_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_mt_default_headers[ $header ] ) ) {
		unset( $_mt_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function get_header_video_settings() {
	...
	$video_type = mt_check_filetype( $video_url, mt_get_mime_types() );
...
function get_custom_header_markup() {
	...
	return sprintf(
		'<div id="mt-custom-header" class="mt-custom-header">%s</div>',
...
function the_custom_header_markup() {
	...
	if ( is_header_video_active() && ( has_header_video() || is_customize_preview() ) ) {
		mt_enqueue_script( 'mt-custom-header' );
		mt_localize_script( 'mt-custom-header', '_mtCustomHeaderSettings', get_header_video_settings() );
	}
}
 ...
 * @since 4.7.0
 */
function mt_custom_css_cb() {
	$styles = mt_get_custom_css();
	if ( $styles || is_customize_preview() ) :
		$type_attr = current_theme_supports( 'html5', 'style' ) ? '' : ' type="text/css"';
		?>
		<style<?php echo $type_attr; ?> id="mt-custom-css">
 ...
 * @return MT_Post|null The custom_css post or null if none exists.
 */
function mt_get_custom_css_post( $stylesheet = '' ) {
	if ( empty( $stylesheet ) ) {
		$stylesheet = get_stylesheet();
	}
	...
	if ( get_stylesheet() === $stylesheet ) {
		...
		if ( ! $post && -1 !== $post_id ) {
			$query = new MT_Query( $custom_css_query_vars );
			$post  = $query->post;
			/*
			 * Cache the lookup. See mt_update_custom_css_post().
 ...
 */
function mt_get_custom_css( $stylesheet = '' ) {
	$css = '';
	...
	$post = mt_get_custom_css_post( $stylesheet );
	if ( $post ) {
	 ...
	 */
	$css = apply_filters( 'mt_get_custom_css', $css, $stylesheet );
 ...
 * }
 * @return MT_Post|MT_Error Post on success, error on failure.
 */
function mt_update_custom_css_post( $css, $args = array() ) {
	$args = mt_parse_args(
		$args,
		array(
			'preprocessed' => '',
	 ...
	 *     $post = mt_get_custom_css_post( $setting->stylesheet );
	 ...
	 * }
	 * @param array $args {
	 *     The args passed into `mt_update_custom_css_post()` merged with defaults.
	 *
	...
	$post = mt_get_custom_css_post( $args['stylesheet'] );
	if ( $post ) {
		$post_data['ID'] = $post->ID;
		$r               = mt_update_post( mt_slash( $post_data ), true );
	} else {
		$r = mt_insert_post( mt_slash( $post_data ), true );
		...
		if ( ! is_mt_error( $r ) ) {
			...
			if ( 0 === count( mt_get_post_revisions( $r ) ) ) {
				mt_save_post_revision( $r );
			}
		}
	}
			...
			case 'posts':
				foreach ( $config[ $type ] as $id => $item ) {
					if ( is_array( $item ) ) {
						...
						$content[ $type ][ $id ] = mt_array_slice_assoc(
							$item,
							array(
								'post_type',
								'post_title',
 ...
 *              `editor-styles`, and `mt-block-styles` features were added.
 ...
 *
 * @global array $_mt_theme_features
 *
 ...
 *                        'disable-custom-font-sizes', 'editor-color-palette', 'editor-font-sizes',
 *                        'editor-styles', 'mt-block-styles', and 'core-block-patterns'.
 * @param mixed  ...$args Optional extra arguments to pass along with certain features.
 * @return void|false Void on success, false on failure.
 */
function add_theme_support( $feature, ...$args ) {
	global $_mt_theme_features;
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 * for post thumbnails.
			 */
			if ( isset( $args[0] ) && is_array( $args[0] ) && isset( $_mt_theme_features['post-thumbnails'] ) ) {
				$args[0] = array_unique( array_merge( $_mt_theme_features['post-thumbnails'][0], $args[0] ) );
			}
		...
		case 'html5':
			...
			if ( isset( $_mt_theme_features['html5'] ) ) {
				$args[0] = array_merge( $_mt_theme_features['html5'][0], $args[0] );
			}
			break;
		...
		case 'custom-logo':
			...
			$args[0]  = mt_parse_args( array_intersect_key( $args[0], $defaults ), $defaults );
		...
		case 'custom-header':
			...
			if ( isset( $_mt_theme_features['custom-header'] ) ) {
				$args[0] = mt_parse_args( $_mt_theme_features['custom-header'][0], $args[0] );
			}

			// Load in the defaults at the end, as we need to insure first one wins.
			// This will cause all constants to be defined, as each arg will then be set to the default.
			if ( $jit ) {
				$args[0] = mt_parse_args( $args[0], $defaults );
			}
			 ...
			 * Once we get to mt_loaded (just-in-time), define any constants we haven't already.
		...
		case 'custom-background':
			...
			$defaults = array(
				...
				'default-color'          => '',
				'mt-head-callback'       => '_custom_background_cb',
				'admin-head-callback'    => '',
				'admin-preview-callback' => '',
			);
			...
			if ( isset( $_mt_theme_features['custom-background'] ) ) {
				$args[0] = mt_parse_args( $_mt_theme_features['custom-background'][0], $args[0] );
			}
		...
		case 'title-tag':
			// Can be called in functions.php but must happen before mt_loaded, i.e. not in header.php.
			if ( did_action( 'mt_loaded' ) ) {
				_doing_it_wrong(
					"add_theme_support( 'title-tag' )",
					sprintf(
						/* translators: 1: title-tag, 2: mt_loaded */
	...
	$_mt_theme_features[ $feature ] = $args;
}
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['mt-head-callback'] ) {
			add_action( 'mt_head', $args[0]['mt-head-callback'] );
		}

		if ( is_admin() ) {
			require_once ABSPATH . 'mt-admin/includes/class-custom-image-header.php';
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once ABSPATH . 'mt-admin/includes/class-custom-background.php';
...
function get_theme_support( $feature, ...$args ) {
	global $_mt_theme_features;
	if ( ! isset( $_mt_theme_features[ $feature ] ) ) {
		return false;
	}

	if ( ! $args ) {
		return $_mt_theme_features[ $feature ];
	}
	...
	switch ( $feature ) {
		...
		case 'custom-header':
		case 'custom-background':
			if ( isset( $_mt_theme_features[ $feature ][0][ $args[0] ] ) ) {
				return $_mt_theme_features[ $feature ][0][ $args[0] ];
			}
			return false;
...
function _remove_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'custom-header':
			...
			if ( isset( $support[0]['mt-head-callback'] ) ) {
				remove_action( 'mt_head', $support[0]['mt-head-callback'] );
			}
	...
	unset( $_mt_theme_features[ $feature ] );
...
function current_theme_supports( $feature, ...$args ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 */
			if ( true === $_mt_theme_features[ $feature ] ) {  // Registered for all types.
				return true;
			}
			$content_type = $args[0];
			return in_array( $content_type, $_mt_theme_features[ $feature ][0], true );
		...
		case 'post-formats':
			 ...
			 */
			$type = $args[0];
			return in_array( $type, $_mt_theme_features[ $feature ][0], true );

		case 'custom-logo':
		case 'custom-header':
		case 'custom-background':
			// Specific capabilities can be registered by passing an array to add_theme_support().
			return ( isset( $_mt_theme_features[ $feature ][0][ $args[0] ] ) && $_mt_theme_features[ $feature ][0][ $args[0] ] );
	}
	 ...
	 */
	return apply_filters( "current_theme_supports-{$feature}", true, $args, $_mt_theme_features[ $feature ] ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 * @see add_theme_support()
 *
 * @global array $_mt_registered_theme_features
 *
 ...
 *      }
 * }
 * @return true|MT_Error True if the theme feature was successfully registered, a MT_Error object if not.
 */
function register_theme_feature( $feature, $args = array() ) {
	global $_mt_registered_theme_features;

	if ( ! is_array( $_mt_registered_theme_features ) ) {
		$_mt_registered_theme_features = array();
	}
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		$args['show_in_rest'] = mt_parse_args(
			$args['show_in_rest'],
			array(
				'schema'           => array(),
				'name'             => $feature,
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		...
		$args['show_in_rest']['schema'] = mt_parse_args(
	...
	$_mt_registered_theme_features[ $feature ] = $args;
...
function get_registered_theme_features() {
	...
	return $_mt_registered_theme_features;
}
...
function get_registered_theme_feature( $feature ) {
	...
	return isset( $_mt_registered_theme_features[ $feature ] ) ? $_mt_registered_theme_features[ $feature ] : null;
}
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = mt_get_attachment_url( $id );
...
/**
 * Checks if a theme has been changed and runs 'after_switch_theme' hook on the next MT load.
 *
...
function check_theme_switched() {
	...
	if ( $stylesheet ) {
		$old_theme = mt_get_theme( $stylesheet );

		// Prevent widget & menu mapping from running since Customizer already called it up front.
		if ( get_option( 'theme_switched_via_customizer' ) ) {
			remove_action( 'after_switch_theme', '_mt_menus_changed' );
			remove_action( 'after_switch_theme', '_mt_sidebars_changed' );
			update_option( 'theme_switched_via_customizer', false );
		}

		if ( $old_theme->exists() ) {
			/**
			 * Fires on the first MT load after a theme switch if the old theme still exists.
			 *
			...
			/** This action is documented in mt-includes/theme.php */
...
/**
 * Includes and instantiates the MT_Customize_Manager class.
 *
 * Loads the Customizer at plugins_loaded when accessing the customize.php admin
 * page or when any request includes a mt_customize=on param or a customize_changeset
 * param (a UUID). This param is a signal for whether to bootstrap the Customizer when
 * paCMec is loading, especially in the Customizer preview
 ...
 * @global MT_Customize_Manager $mt_customize
 */
function _mt_customize_include() {

	$is_customize_admin_page = ( is_admin() && 'customize.php' === basename( $_SERVER['PHP_SELF'] ) );
	$should_include          = (
		$is_customize_admin_page
		||
		( isset( $_REQUEST['mt_customize'] ) && 'on' === $_REQUEST['mt_customize'] )
		||
	...
	/*
	 * Note that mt_unslash() is not being used on the input vars because it is
	 * called before mt_magic_quotes() gets called. Besides this fact, none of
	...
	$input_vars = array_merge(
		mt_array_slice_assoc( $_GET, $keys ),
		mt_array_slice_assoc( $_POST, $keys )
	);
	...
	// Note that theme will be sanitized via MT_Theme.
	 ...
	 * here in the case of a customize_save action because this will cause MT to think
	...
	$is_customize_save_action = (
		...
		&&
		'customize_save' === mt_unslash( $_REQUEST['action'] )
	);
	$settings_previewed       = ! $is_customize_save_action;

	require_once ABSPATH . MTINC . '/class-mt-customize-manager.php';
	$GLOBALS['mt_customize'] = new MT_Customize_Manager( compact( 'changeset_uuid', 'theme', 'messenger_channel', 'settings_previewed', 'autosaved', 'branching' ) );
}
 ...
 * @global MT_Customize_Manager $mt_customize Customizer instance.
 *
 * @param string  $new_status     New post status.
 * @param string  $old_status     Old post status.
 * @param MT_Post $changeset_post Changeset post object.
 */
function _mt_customize_publish_changeset( $new_status, $old_status, $changeset_post ) {
	global $mt_customize, $mtdb;
	...
	if ( empty( $mt_customize ) ) {
		require_once ABSPATH . MTINC . '/class-mt-customize-manager.php';
		$mt_customize = new MT_Customize_Manager(
			array(
	...
	if ( ! did_action( 'customize_register' ) ) {
		 ...
		 * will have a $mt_customize with those settings registered since they
		 ...
		 */
		remove_action( 'customize_register', array( $mt_customize, 'register_controls' ) );
		$mt_customize->register_controls();

		/** This filter is documented in /mt-includes/class-mt-customize-manager.php */
		do_action( 'customize_register', $mt_customize );
	}
	$mt_customize->_publish_changeset_values( $changeset_post->ID );
	 ...
	 */
	if ( ! mt_revisions_enabled( $changeset_post ) ) {
		$mt_customize->trash_changeset_post( $changeset_post->ID );
	}
}
 ...
 */
function _mt_customize_changeset_filter_insert_post_data( $post_data, $supplied_post_data ) {
 ...
 * @since 3.4.0
 */
function _mt_customize_loader_settings() {
	...
	$browser = array(
		'mobile' => mt_is_mobile(),
		'ios'    => mt_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] ),
	);
	...
	$script = 'var _mtCustomizeLoaderSettings = ' . mt_json_encode( $settings ) . ';';

	$mt_scripts = mt_scripts();
	$data       = $mt_scripts->get_data( 'customize-loader', 'data' );
	if ( $data ) {
		$script = "$data\n$script";
	}

	$mt_scripts->add_data( 'customize-loader', 'data', $script );
}
 ...
 * @return string
 */
function mt_customize_url( $stylesheet = '' ) {
 ...
 */
function mt_customize_support_script() {
	$admin_origin = parse_url( admin_url() );
	$home_origin  = parse_url( home_url() );
	$cross_domain = ( strtolower( $admin_origin['host'] ) != strtolower( $home_origin['host'] ) );
...
function is_customize_preview() {
	...
	return ( $mt_customize instanceof MT_Customize_Manager ) && $mt_customize->is_preview();
}
 ...
 * garbage-collected at the same time by `mt_delete_auto_drafts()`. Otherwise,
 ...
 */
function _mt_keep_alive_customize_changeset_dependent_auto_drafts( $new_status, $old_status, $post ) {
	global $mtdb;
	unset( $old_status );
	 ...
	 * are then visible in the MT Admin. We cannot wait for the deletion of the changeset in which
	 * _mt_delete_customize_changeset_dependent_auto_drafts() will be called, since they need to be
	...
	if ( 'auto-draft' === $new_status ) {
		 ...
		 */
		$post_args['post_date'] = $post->post_date; // Note mt_delete_auto_drafts() only looks at this date.
	} else {
		/*
...
function create_initial_theme_features() {
	...
	register_theme_feature(
		'mt-block-styles',
		array(
			'description'  => __( 'Whether theme opts in to default paCMec block styles for viewing.' ),
			'show_in_rest' => true,
		)
	);
}
