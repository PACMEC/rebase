...
final class MT_Comment {
	...
	/**
	 * Retrieves a MT_Comment instance.
	 *
	 * @since 4.4.0
	 *
	 ...
	 * @return MT_Comment|false Comment object, otherwise false.
	 */
	public static function get_instance( $id ) {
		...
		$_comment = mt_cache_get( $comment_id, 'comment' );

		if ( ! $_comment ) {
			$_comment = $mtdb->get_row( $mtdb->prepare( "SELECT * FROM $mtdb->comments WHERE comment_ID = %d LIMIT 1", $comment_id ) );

			if ( ! $_comment ) {
				return false;
			}

			mt_cache_add( $_comment->comment_ID, $_comment, 'comment' );
		}

		return new MT_Comment( $_comment );
	}
	 ...
	 * }
	 * @return MT_Comment[] Array of `MT_Comment` objects.
	 */
	public function get_children( $args = array() ) {
		...
		$_args           = mt_parse_args( $args, $defaults );
	 ...
	 *
	 * Used by `MT_Comment_Query` when bulk-filling descendants.
	 *
	 * @since 4.4.0
	 *
	 * @param MT_Comment $child Child comment.
	 */
	public function add_child( MT_Comment $child ) {
	 ...
	 * @return MT_Comment|false Returns the comment object if found, otherwise false.
	 */
