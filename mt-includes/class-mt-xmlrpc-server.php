...
/**
 * paCMec XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional paCMec API for managing comments, pages, posts,
 * options, etc.
 *
 * As of paCMec 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the {@see 'xmlrpc_enabled'} filter found in mt_xmlrpc_server::login().
 *
 * @since 1.5.0
 *
 * @see IXR_Server
 */
class mt_xmlrpc_server extends IXR_Server {
	/**
	 * Methods.
	 *
	 * @var array
	...
	/**
	 * Flags that the user authentication has failed in this instance of mt_xmlrpc_server.
	 *
	 * @var bool
	 */
	...
	public function __construct() {
		$this->methods = array(
			// paCMec API.
			'mt.getUsersBlogs'                 => 'this:mt_getUsersBlogs',
			'mt.newPost'                       => 'this:mt_newPost',
			'mt.editPost'                      => 'this:mt_editPost',
			'mt.deletePost'                    => 'this:mt_deletePost',
			'mt.getPost'                       => 'this:mt_getPost',
			'mt.getPosts'                      => 'this:mt_getPosts',
			'mt.newTerm'                       => 'this:mt_newTerm',
			'mt.editTerm'                      => 'this:mt_editTerm',
			'mt.deleteTerm'                    => 'this:mt_deleteTerm',
			'mt.getTerm'                       => 'this:mt_getTerm',
			'mt.getTerms'                      => 'this:mt_getTerms',
			'mt.getTaxonomy'                   => 'this:mt_getTaxonomy',
			'mt.getTaxonomies'                 => 'this:mt_getTaxonomies',
			'mt.getUser'                       => 'this:mt_getUser',
			'mt.getUsers'                      => 'this:mt_getUsers',
			'mt.getProfile'                    => 'this:mt_getProfile',
			'mt.editProfile'                   => 'this:mt_editProfile',
			'mt.getPage'                       => 'this:mt_getPage',
			'mt.getPages'                      => 'this:mt_getPages',
			'mt.newPage'                       => 'this:mt_newPage',
			'mt.deletePage'                    => 'this:mt_deletePage',
			'mt.editPage'                      => 'this:mt_editPage',
			'mt.getPageList'                   => 'this:mt_getPageList',
			'mt.getAuthors'                    => 'this:mt_getAuthors',
			'mt.getCategories'                 => 'this:mw_getCategories',     // Alias.
			'mt.getTags'                       => 'this:mt_getTags',
			'mt.newCategory'                   => 'this:mt_newCategory',
			'mt.deleteCategory'                => 'this:mt_deleteCategory',
			'mt.suggestCategories'             => 'this:mt_suggestCategories',
			'mt.uploadFile'                    => 'this:mw_newMediaObject',    // Alias.
			'mt.deleteFile'                    => 'this:mt_deletePost',        // Alias.
			'mt.getCommentCount'               => 'this:mt_getCommentCount',
			'mt.getPostStatusList'             => 'this:mt_getPostStatusList',
			'mt.getPageStatusList'             => 'this:mt_getPageStatusList',
			'mt.getPageTemplates'              => 'this:mt_getPageTemplates',
			'mt.getOptions'                    => 'this:mt_getOptions',
			'mt.setOptions'                    => 'this:mt_setOptions',
			'mt.getComment'                    => 'this:mt_getComment',
			'mt.getComments'                   => 'this:mt_getComments',
			'mt.deleteComment'                 => 'this:mt_deleteComment',
			'mt.editComment'                   => 'this:mt_editComment',
			'mt.newComment'                    => 'this:mt_newComment',
			'mt.getCommentStatusList'          => 'this:mt_getCommentStatusList',
			'mt.getMediaItem'                  => 'this:mt_getMediaItem',
			'mt.getMediaLibrary'               => 'this:mt_getMediaLibrary',
			'mt.getPostFormats'                => 'this:mt_getPostFormats',
			'mt.getPostType'                   => 'this:mt_getPostType',
			'mt.getPostTypes'                  => 'this:mt_getPostTypes',
			'mt.getRevisions'                  => 'this:mt_getRevisions',
			'mt.restoreRevision'               => 'this:mt_restoreRevision',
	 ...
	 * @return MT_User|false MT_User object if authentication passed, false otherwise
	 */
	public function login( $username, $password ) {
		...
		if ( $this->auth_failed ) {
			$user = new MT_Error( 'login_prevented' );
		} else {
		...
		if ( is_mt_error( $user ) ) {
			...
			// Flag that authentication has failed once on this mt_xmlrpc_server instance.
			 ...
			 * @param MT_Error  $user  MT_Error object.
			 */
			$this->error = apply_filters( 'xmlrpc_login_error', $this->error, $user );
			return false;
		}

		mt_set_current_user( $user->ID );
		return $user;
	}
	 ...
	 *
	 * @since 1.5.0
	 * @deprecated 2.8.0 Use mt_xmlrpc_server::login()
	 * @see mt_xmlrpc_server::login()
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields( $post_id, $fields ) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset( $meta['id'] ) ) {
				...
				if ( isset( $meta['key'] ) ) {
					$meta['key'] = mt_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key ) {
						continue;
					}
					$meta['value'] = mt_unslash( $meta['value'] );
			...
			} elseif ( current_user_can( 'add_post_meta', $post_id, mt_unslash( $meta['key'] ) ) ) {
	 ...
	 */
	public function mt_getUsersBlogs( $args ) {
		if ( ! $this->minimum_args( $args, 2 ) ) {
			return $this->error;
		}

		// If this isn't on MTMU then just use blogger_getUsersBlogs().
		if ( ! is_multisite() ) {
		 ...
		 * equal to the method's name, e.g., mt.getUsersBlogs, mt.newPost, etc.
		 *
		 * @since 2.5.0
		 ...
		 * @param mt_xmlrpc_server $server The XML-RPC server instance.
		 */
		do_action( 'xmlrpc_call', 'mt.getUsersBlogs', $args, $this );
	 ...
	 *
	 * @param MT_Taxonomy $taxonomy The unprepared taxonomy data.
	...
	protected function _prepare_taxonomy( $taxonomy, $fields ) {
		 ...
		 * @param MT_Taxonomy $taxonomy  Taxonomy object.
		 * @param array       $fields    The subset of taxonomy fields to return.
		 */
	...
	/**
	 * Convert a paCMec date string to an IXR_Date object.
	 *
	...
	/**
	 * Convert a paCMec GMT date string to an IXR_Date object.
	 *
	 * @param string $date_gmt paCMec GMT date string.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields, true ) ) {
			...
			$terms                = mt_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms']       = array();
	 ...
	 *
	 * @param MT_Post_Type $post_type Post type object.
	 ...
	 *
	 * @param MT_Post $media_item     The unprepared media item data.
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => mt_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => mt_get_attachment_metadata( $media_item->ID ),
		 ...
		 * @param MT_Post $media_item     Media item object.
	 ...
	 *
	 * @param MT_Post $page The unprepared page data.
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( mt_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'mt_slug'                => $page->post_name,
			'mt_password'            => $page->post_password,
			'mt_author'              => $author->display_name,
			'mt_page_parent_id'      => $page->post_parent,
			'mt_page_parent_title'   => $parent_title,
			'mt_page_order'          => $page->menu_order,
			'mt_author_id'           => (string) $author->ID,
			'mt_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'mt_page_template'       => $page_template,
		);
	 ...
	 *
	 * @param MT_Comment $comment The unprepared comment data.
	 ...
	 *
	 * @param MT_User $user   The unprepared user object.
	 ...
	 *         Content struct for adding a new post. See mt_insert_post() for information on
	 ...
	 */
	public function mt_newPost( $args ) {
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.newPost', $args, $this );
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			$sticky = mt_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	...
	/**
	 * Helper method for mt_newPost() and mt_editPost(), containing shared logic.
	 *
	 * @since 3.4.0
	 *
	 * @see mt_insert_post()
	 *
	 * @param MT_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = mt_parse_args( array_intersect_key( $content_struct, $defaults ), $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// Create the new term.
							$term_info = mt_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_mt_insert_post_data', $post_data, $content_struct );

		$post_ID = $update ? mt_update_post( $post_data, true ) : mt_insert_post( $post_data, true );
	 ...
	 */
	public function mt_editPost( $args ) {
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.editPost', $args, $this );
	 ...
	 */
	public function mt_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.deletePost', $args, $this );
	 ...
	 *  - 'enclosure'
	 */
	public function mt_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'mt.getPost' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPost', $args, $this );
	 ...
	 * @since 3.4.0
	 *
	 * @see mt_get_recent_posts()
	 * @see mt_getPost() for more on `$fields`
	 ...
	 */
	public function mt_getPosts( $args ) {
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'mt.getPosts' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPosts', $args, $this );
		...
		$posts_list = mt_get_recent_posts( $query );
	 ...
	 */
	public function mt_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.newTerm', $args, $this );
		...
		$term = mt_insert_term( $term_data['name'], $taxonomy['name'], $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function mt_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.editTerm', $args, $this );
		...
		$term = mt_update_term( $term_id, $taxonomy['name'], $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function mt_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.deleteTerm', $args, $this );
		...
		$result = mt_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function mt_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getTerm', $args, $this );
	 ...
	 */
	public function mt_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getTerms', $args, $this );
	 ...
	 */
	public function mt_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'mt.getTaxonomy' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getTaxonomy', $args, $this );
	 ...
	 */
	public function mt_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'mt.getTaxonomies' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getTaxonomies', $args, $this );
	 ...
	 *  - 'roles'
	 */
	public function mt_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mt.getUser' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getUser', $args, $this );
	 ...
	 *
	 * @uses get_users()
	 * @see mt_getUser() for more on $fields and return values
	 *
	 * @param array $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function mt_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mt.getUsers' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getUsers', $args, $this );
	 ...
	 *     @type array  $fields (optional)
	 * }
	 * @return array|IXR_Error (@see mt_getUser)
	 */
	public function mt_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[3] ) ) {
			...
			/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mt.getProfile' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getProfile', $args, $this );
	 ...
	 */
	public function mt_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.editProfile', $args, $this );
		...
		$result = mt_update_user( $user_data );
	 ...
	 */
	public function mt_getPage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPage', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getPages( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPages', $args, $this );
	 ...
	 * @since 2.2.0
	 *
	 * @see mt_xmlrpc_server::mw_newPost()
	 *
	 * @param array $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function mt_newPage( $args ) {
		// Items not escaped here will be escaped in mt_newPost().
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.newPage', $args, $this );
	 ...
	 */
	public function mt_deletePage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.deletePage', $args, $this );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mt_deletePage', $page_id, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_editPage( $args ) {
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.editPage', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getPageList( $args ) {
		global $mtdb;
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPageList', $args, $this );

		// Get list of page IDs and titles.
		$page_list = $mtdb->get_results(
			"
			SELECT ID page_id,
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getAuthors( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getAuthors', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getTags( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getKeywords', $args, $this );
	 ...
	 */
	public function mt_newCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.newCategory', $args, $this );
		...
		// so that paCMec will generate one.
		...
		$cat_id = mt_insert_category( $new_category, true );
		if ( is_mt_error( $cat_id ) ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_mt_newCategory', $cat_id, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * }
	 * @return bool|IXR_Error See mt_delete_term() for return info.
	 */
	public function mt_deleteCategory( $args ) {
		$this->escape( $args );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_mt_deleteCategory', $category_id, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_suggestCategories( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.suggestCategories', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getComment', $args, $this );
	 ...
	 * - 'offset' - Default is 0. See MT_Query::query() for more.
	 *
	 * @since 2.7.0
	 *
	 * @param array $args {
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of comments. See mt_xmlrpc_server::mt_getComment() for a description of each item contents
	 */
	public function mt_getComments( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getComments', $args, $this );
	 ...
	 * See mt_delete_comment() for more information on this behavior.
	 *
	 * @since 2.7.0
	 *
	 * @param array $args {
	 ...
	 *     @type int    $comment_ID
	 * }
	 * @return bool|IXR_Error See mt_delete_comment().
	 */
	public function mt_deleteComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.deleteComment', $args, $this );

		$status = mt_delete_comment( $comment_ID );
		...
		if ( $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_mt_deleteComment', $comment_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
		}
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function mt_editComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.editComment', $args, $this );
		$comment = array(
		...
		$result = mt_update_comment( $comment, true );
		if ( is_mt_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_mt_editComment', $comment_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	 ...
	 *     @type array      $content_struct
	 * }
	 * @return int|IXR_Error See mt_new_comment().
	 */
	public function mt_newComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.newComment', $args, $this );

		$comment_ID = mt_new_comment( $comment, true );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mt_newComment', $comment_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getCommentStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getCommentStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getCommentCount( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getCommentCount', $args, $this );

		$count = mt_count_comments( $post_id );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPostStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPageStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[ $o_name ]['option'], mt_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function mt_getMediaItem( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getMediaItem', $args, $this );
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of media items. See mt_xmlrpc_server::mt_getMediaItem() for a description of each item contents
	 */
	public function mt_getMediaLibrary( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getMediaLibrary', $args, $this );
	 ...
	 */
	public function mt_getPostFormats( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPostFormats', $args, $this );
	 ...
	 *  - 'supports'
	 */
	public function mt_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'mt.getPostType' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPostType', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function mt_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'mt.getPostTypes' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getPostTypes', $args, $this );
	 ...
	 */
	public function mt_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_revision_fields', array( 'post_date', 'post_date_gmt' ), 'mt.getRevisions' );
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.getRevisions', $args, $this );
		...
		if ( ! mt_revisions_enabled( $post ) ) {
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );
		}

		$revisions = mt_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves.
			if ( mt_is_post_autosave( $revision ) ) {
				continue;
			}
	 ...
	 */
	public function mt_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in mt-includes/class-mt-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mt.restoreRevision', $args, $this );

		$revision = mt_get_post_revision( $revision_id );
		if ( ! $revision ) {
			return new IXR_Error( 404, __( 'Invalid post ID.' ) );
		}
		...
		$post = mt_restore_post_revision( $revision_id );
	...
	protected function _multisite_getUsersBlogs( $args ) {
		...
		$rpc->query( 'mt.getUsersBlogs', $args[1], $args[2] );
	...
	public function blogger_getPost( $args ) {
		...
		$categories = implode( ',', mt_get_post_categories( $post_ID ) );

		$content  = '<title>' . mt_unslash( $post_data['post_title'] ) . '</title>';
		$content .= '<category>' . $categories . '</category>';
		$content .= mt_unslash( $post_data['post_content'] );
	...
	public function blogger_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = implode( ',', mt_get_post_categories( $entry['ID'] ) );

			$content  = '<title>' . mt_unslash( $entry['post_title'] ) . '</title>';
			$content .= '<category>' . $categories . '</category>';
			$content .= mt_unslash( $entry['post_content'] );
	...
	public function blogger_newPost( $args ) {
		...
		$post_ID = mt_insert_post( $post_data );
		if ( is_mt_error( $post_ID ) ) {
			return new IXR_Error( 500, $post_ID->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_newPost', $post_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	...
	public function blogger_editPost( $args ) {
		...
		$result = mt_update_post( $postdata );
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_editPost', $post_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	...
	public function blogger_deletePost( $args ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_deletePost', $post_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	...
	public function mw_newPost( $args ) {
		...
		if ( ! empty( $content_struct['post_type'] ) ) {
			if ( 'page' === $content_struct['post_type'] ) {
				...
				$post_type     = 'page';
				if ( ! empty( $content_struct['mt_page_template'] ) ) {
					$page_template = $content_struct['mt_page_template'];
				}
		...
		if ( isset( $content_struct['mt_post_format'] ) ) {
			$content_struct['mt_post_format'] = sanitize_key( $content_struct['mt_post_format'] );
			if ( ! array_key_exists( $content_struct['mt_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format.' ) );
			}
		}

		// Let paCMec generate the 'post_name' (slug) unless
		// one has been provided.
		$post_name = '';
		if ( isset( $content_struct['mt_slug'] ) ) {
			$post_name = $content_struct['mt_slug'];
		}

		// Only use a password if one was given.
		if ( isset( $content_struct['mt_password'] ) ) {
			$post_password = $content_struct['mt_password'];
		} else {
			$post_password = '';
		}

		// Only set a post parent if one was given.
		if ( isset( $content_struct['mt_page_parent_id'] ) ) {
			$post_parent = $content_struct['mt_page_parent_id'];
		} else {
			$post_parent = 0;
		}

		// Only set the 'menu_order' if it was given.
		if ( isset( $content_struct['mt_page_order'] ) ) {
			$menu_order = $content_struct['mt_page_order'];
		} else {
			$menu_order = 0;
		}
		...
		if ( isset( $content_struct['mt_author_id'] ) && ( $user->ID != $content_struct['mt_author_id'] ) ) {
			switch ( $post_type ) {
				case 'post':
			...
			$author = get_userdata( $content_struct['mt_author_id'] );
			if ( ! $author ) {
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			}
			$post_author = $content_struct['mt_author_id'];
		}
		...
		if ( isset( $content_struct['mt_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_ID, $content_struct['mt_post_thumbnail'] ) === false ) {
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
			}

			unset( $content_struct['mt_post_thumbnail'] );
		}
		...
		// in this function.
		if ( isset( $content_struct['mt_post_format'] ) ) {
			set_post_format( $post_ID, $content_struct['mt_post_format'] );
		}

		$post_ID = mt_insert_post( $postdata, true );
		if ( is_mt_error( $post_ID ) ) {
			return new IXR_Error( 500, $post_ID->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_newPost', $post_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	...
	public function attach_uploads( $post_ID, $post_content ) {
		...
		$attachments = $mtdb->get_results( "SELECT ID, guid FROM {$mtdb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( ! empty( $file->guid ) && strpos( $post_content, $file->guid ) !== false ) {
					$mtdb->update( $mtdb->posts, array( 'post_parent' => $post_ID ), array( 'ID' => $file->ID ) );
				}
			}
		}
	}
	...
	public function mw_editPost( $args ) {
		...
		// Use mt.editPost to edit post types other than post and page.
		...
		// Let paCMec manage slug if none was provided.
		...
		$page_template = null;
		if ( ! empty( $content_struct['mt_page_template'] ) && 'page' === $post_type ) {
			$page_template = $content_struct['mt_page_template'];
		}
		...
		// If an author id was provided then use it instead.
		if ( isset( $content_struct['mt_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID != $content_struct['mt_author_id'] || $user->ID != $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
		...
		$nemtost = compact( 'ID', 'post_content', 'post_title', 'post_category', 'post_status', 'post_excerpt', 'comment_status', 'ping_status', 'edit_date', 'post_date', 'post_date_gmt', 'to_ping', 'post_name', 'post_password', 'post_parent', 'menu_order', 'post_author', 'tags_input', 'page_template' );

		$result = mt_update_post( $nemtost, true );
		if ( is_mt_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		...
		if ( isset( $content_struct['mt_post_thumbnail'] ) ) {
			...
			// Empty value deletes, non-empty value adds/updates.
			if ( empty( $content_struct['mt_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_ID );
			} else {
				if ( set_post_thumbnail( $post_ID, $content_struct['mt_post_thumbnail'] ) === false ) {
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_editPost', $post_ID, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	...
	public function mw_getPost( $args ) {
		...
		if ( '' !== $postdata['post_date'] ) {
			...
			$categories = array();
			$catids     = mt_get_post_categories( $post_ID );
			foreach ( $catids as $catid ) {
				$categories[] = get_cat_name( $catid );
			}

			$tagnames = array();
			$tags     = mt_get_post_tags( $post_ID );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$resp = array(
				...
				'mt_more_text'           => $post['more_text'],
				'mt_allow_comments'      => $allow_comments,
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'mt_slug'                => $postdata['post_name'],
				'mt_password'            => $postdata['post_password'],
				'mt_author_id'           => (string) $author->ID,
				'mt_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				'post_status'            => $postdata['post_status'],
				'custom_fields'          => $this->get_custom_fields( $post_ID ),
				'mt_post_format'         => $post_format,
				'sticky'                 => $sticky,
			...
			$resp['mt_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = array();
			$catids     = mt_get_post_categories( $entry['ID'] );
			foreach ( $catids as $catid ) {
				$categories[] = get_cat_name( $catid );
			}

			$tagnames = array();
			$tags     = mt_get_post_tags( $entry['ID'] );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'mt_slug'                => $entry['post_name'],
				'mt_password'            => $entry['post_password'],
				'mt_author_id'           => (string) $author->ID,
				'mt_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				...
				'mt_post_thumbnail'      => get_post_thumbnail_id( $entry['ID'] ),
			);
		}
	...
	public function mw_newMediaObject( $args ) {
		...
		$upload = mt_upload_bits( $name, null, $bits );
		...
		// Save the data.
		$id = mt_insert_attachment( $attachment, $upload['file'], $post_id );
		mt_update_attachment_metadata( $id, mt_generate_attachment_metadata( $id, $upload['file'] ) );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_newMediaObject', $id, $args ); // phpcs:ignore paCMec.NamingConventions.ValidHookName.NotLowercase
	...
	public function mt_getPostCategories( $args ) {
		...
		$categories = array();
		$catids     = mt_get_post_categories( (int) $post_ID );
	...
	public function mt_setPostCategories( $args ) {
		...
		mt_set_post_categories( $post_ID, $catids );
	...
	public function mt_getTrackbackPings( $post_ID ) {
		...
		$comments = $mtdb->get_results( $mtdb->prepare( "SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $mtdb->comments WHERE comment_post_ID = %d", $post_ID ) );
	...
	public function mt_publishPost( $args ) {
		...
		$postdata['post_category'] = mt_get_post_categories( $post_ID );
		$this->escape( $postdata );

		return mt_update_post( $postdata );
	}
	...
	public function pingback_ping( $args ) {
		...
		if ( $post_ID ) {
			...
			if ( (int) $urltest['fragment'] ) {
				...
				$sql     = $mtdb->prepare( "SELECT ID FROM $mtdb->posts WHERE post_title RLIKE %s", $title );
				$post_ID = $mtdb->get_var( $sql );
				if ( ! $post_ID ) {
		...
		if ( $mtdb->get_results( $mtdb->prepare( "SELECT * FROM $mtdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom ) ) ) {
		...
		/** This filter is documented in mt-includes/class-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'paCMec/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $pagelinkedfrom );
		...
		$request                = mt_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$remote_source          = mt_remote_retrieve_body( $request );
		...
		foreach ( $p as $para ) {
			if ( strpos( $para, $pagelinkedto ) !== false ) { // It exists, but is it a link?
				...
				$excerpt = preg_replace( '|\</?mtcontext\>|', '', $para );
				...
				$marker      = '<mtcontext>' . $context[1] . '</mtcontext>';  // Set up our marker.
				$excerpt     = str_replace( $context[0], $marker, $excerpt ); // Swap out the link for our marker.
				$excerpt     = strip_tags( $excerpt, '<mtcontext>' );         // Strip all tags but our context marker.
