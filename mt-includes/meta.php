 ...
 * Core Metadata API
 *
 * Functions for retrieving and manipulating metadata of various paCMec object types. Metadata
...
function add_metadata( $meta_type, $object_id, $meta_key, $meta_value, $unique = false ) {
	...
	$mid = (int) $mtdb->insert_id;

	mt_cache_delete( $object_id, $meta_type . '_meta' );
...
function update_metadata( $meta_type, $object_id, $meta_key, $meta_value, $prev_value = '' ) {
	...
	$meta_ids = $mtdb->get_col( $mtdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) );
	if ( empty( $meta_ids ) ) {
	...
	$result = $mtdb->update( $table, $data, $where );
	if ( ! $result ) {
		return false;
	}
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	$query = $mtdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( ! $delete_all ) {
		$query .= $mtdb->prepare( " AND $type_column = %d", $object_id );
	}

	if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
		$query .= $mtdb->prepare( ' AND meta_value = %s', $meta_value );
	}

	$meta_ids = $mtdb->get_col( $query );
	if ( ! count( $meta_ids ) ) {
		return false;
	}
	...
	if ( $delete_all ) {
		if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
			$object_ids = $mtdb->get_col( $mtdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s AND meta_value = %s", $meta_key, $meta_value ) );
		} else {
			$object_ids = $mtdb->get_col( $mtdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s", $meta_key ) );
		}
	}
	...
	if ( $delete_all ) {
		foreach ( (array) $object_ids as $o_id ) {
			mt_cache_delete( $o_id, $meta_type . '_meta' );
		}
	} else {
		mt_cache_delete( $object_id, $meta_type . '_meta' );
	}
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = mt_cache_get( $object_id, $meta_type . '_meta' );
...
function get_metadata_default( $meta_type, $object_id, $meta_key, $single = false ) {
	...
	if ( ! $single && ! mt_is_numeric_array( $value ) ) {
		$value = array( $value );
	}
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in mt-includes/meta.php */
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $mtdb->get_row( $mtdb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta ) {
		...
		/** This action is documented in mt-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
		...
		$result = $mtdb->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result ) {
			return false;
		}
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta ) {
		...
		$result = (bool) $mtdb->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	$meta_list = $mtdb->get_results( "SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list) ORDER BY $id_column ASC", ARRAY_A );
	...
	foreach ( $non_cached_ids as $id ) {
		...
		mt_cache_add( $id, $cache[ $id ], $cache_key );
	}
 ...
 * @since 4.5.0
 *
 * @return MT_Metadata_Lazyloader Metadata lazyloader queue.
 */
function mt_metadata_lazyloader() {
	static $mt_metadata_lazyloader;

	if ( null === $mt_metadata_lazyloader ) {
		$mt_metadata_lazyloader = new MT_Metadata_Lazyloader();
	}

	return $mt_metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new MT_Meta_Query( $meta_query );
...
function _get_meta_table( $type ) {
	...
	if ( empty( $mtdb->$table_name ) ) {
		return false;
	}

	return $mtdb->$table_name;
}
 ...
 *
 * @since 3.3.0
 * @since 4.6.0 {@link https://core.trac.managertechnology.com.co/pacmec/ticket/35658 Modified
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( ! $has_old_auth_cb && ! $has_old_sanitize_cb ) {
		...
		$mt_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] = $args;
...
function filter_default_metadata( $value, $object_id, $meta_key, $single, $meta_type ) {
	...
	if ( ! is_array( $mt_meta_keys ) || ! isset( $mt_meta_keys[ $meta_type ] ) ) {
		return $value;
	}

	$defaults = array();
	foreach ( $mt_meta_keys[ $meta_type ] as $sub_type => $meta_data ) {
...
function unregister_meta_key( $object_type, $meta_key, $object_subtype = '' ) {
	...
	$args = $mt_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ];
	...
	unset( $mt_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] );

	// Do some clean up.
	if ( empty( $mt_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		unset( $mt_meta_keys[ $object_type ][ $object_subtype ] );
	}
	if ( empty( $mt_meta_keys[ $object_type ] ) ) {
		unset( $mt_meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type, $object_subtype = '' ) {
	...
	if ( ! is_array( $mt_meta_keys ) || ! isset( $mt_meta_keys[ $object_type ] ) || ! isset( $mt_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		return array();
	}

	return $mt_meta_keys[ $object_type ][ $object_subtype ];
}
 ...
 */
function _mt_register_meta_args_allowed_list( $args, $default_args ) {
