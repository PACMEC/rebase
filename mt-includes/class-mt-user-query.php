 ...
 * @since 3.1.0
 *
 * @see MT_User_Query::prepare_query() for information on accepted arguments.
 */
class MT_User_Query {
	 ...
	 * @since 4.4.0
	 *
	 * @param array $args Query vars, as passed to `MT_User_Query`.
	...
	public static function fill_query_vars( $args ) {
		...
		return mt_parse_args( $args, $defaults );
	}
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the MT_User_Query has been parsed.
		 *
		 * The passed MT_User_Query object contains the query variables,
		 * not yet passed into SQL.
		 *
		 * @since 4.0.0
		 *
		 * @param MT_User_Query $query Current instance of MT_User_Query (passed by reference).
		 */
		...
		if ( is_array( $qv['fields'] ) ) {
			...
			foreach ( $qv['fields'] as $field ) {
				...
				$this->query_fields[] = "$mtdb->users.$field";
			}
			$this->query_fields = implode( ',', $this->query_fields );
		} elseif ( 'all' === $qv['fields'] ) {
			$this->query_fields = "$mtdb->users.*";
		} else {
			$this->query_fields = "$mtdb->users.ID";
		}
		...
		if ( ! empty( $qv['include'] ) ) {
			$include = mt_parse_id_list( $qv['include'] );
		} else {
			$include = false;
		}
		...
		if ( $qv['has_published_posts'] && $blog_id ) {
			...
			foreach ( $post_types as &$post_type ) {
				$post_type = $mtdb->prepare( '%s', $post_type );
			}

			$posts_table        = $mtdb->get_blog_prefix( $blog_id ) . 'posts';
			$this->query_where .= " AND $mtdb->users.ID IN ( SELECT DISTINCT $posts_table.post_author FROM $posts_table WHERE $posts_table.post_status = 'publish' AND $posts_table.post_type IN ( " . implode( ', ', $post_types ) . ' ) )';
		}

		// nicename
		if ( '' !== $qv['nicename'] ) {
			$this->query_where .= $mtdb->prepare( ' AND user_nicename = %s', $qv['nicename'] );
		}
		...
		// login
		if ( '' !== $qv['login'] ) {
			$this->query_where .= $mtdb->prepare( ' AND user_login = %s', $qv['login'] );
		}
		...
		if ( isset( $qv['who'] ) && 'authors' === $qv['who'] && $blog_id ) {
			$who_query = array(
				'key'     => $mtdb->get_blog_prefix( $blog_id ) . 'user_level',
				'value'   => 0,
				'compare' => '!=',
			);
		...
		if ( $blog_id && ( ! empty( $roles ) || ! empty( $role__in ) || ! empty( $role__not_in ) || is_multisite() ) ) {
			...
			if ( ! empty( $roles ) ) {
				foreach ( $roles as $role ) {
					$roles_clauses[] = array(
						'key'     => $mtdb->get_blog_prefix( $blog_id ) . 'capabilities',
						'value'   => '"' . $role . '"',
						'compare' => 'LIKE',
					);
				}
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses            = $this->meta_query->get_sql( 'user', $mtdb->users, 'ID', $this );
		...
		if ( isset( $qv['number'] ) && $qv['number'] > 0 ) {
			if ( $qv['offset'] ) {
				$this->query_limit = $mtdb->prepare( 'LIMIT %d, %d', $qv['offset'], $qv['number'] );
			} else {
				$this->query_limit = $mtdb->prepare( 'LIMIT %d, %d', $qv['number'] * ( $qv['paged'] - 1 ), $qv['number'] );
			}
		}
		...
		if ( $search ) {
			...
			if ( ! $search_columns ) {
				...
				} elseif ( preg_match( '|^https?://|', $search ) && ! ( is_multisite() && mt_is_large_network( 'users' ) ) ) {
			...
			/**
			 * Filters the columns to search in a MT_User_Query search.
			 *
			 ...
			 * @param MT_User_Query $query          The current MT_User_Query instance.
			 */
		...
		if ( ! empty( $include ) ) {
			// Sanitized earlier.
			$ids                = implode( ',', $include );
			$this->query_where .= " AND $mtdb->users.ID IN ($ids)";
		} elseif ( ! empty( $qv['exclude'] ) ) {
			$ids                = implode( ',', mt_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $mtdb->users.ID NOT IN ($ids)";
		}

		// Date queries are allowed for the user_registered field.
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query         = new MT_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the MT_User_Query has been parsed, and before
		 * the query is executed.
		 *
		 * The passed MT_User_Query object contains SQL parts formed
	...
	public function query() {
		 ...
		 *
		 * Return a non-null value to bypass paCMec' default user queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `total_users` property of the MT_User_Query object, passed to the filter
		 * by reference. If MT_User_Query does not perform a database query, it will not
		 * have enough information to generate these values itself.
		 *
		 * @since 5.1.0
		 *
		 * @param array|null    $results Return an array of user data to short-circuit MT's user query
		 *                               or null to allow MT to run its normal queries.
		 * @param MT_User_Query $query   The MT_User_Query instance (passed by reference).
		 */
		...
		if ( null === $this->results ) {
			...
			if ( is_array( $qv['fields'] ) || 'all' === $qv['fields'] ) {
				$this->results = $mtdb->get_results( $this->request );
			} else {
				$this->results = $mtdb->get_col( $this->request );
			}

			if ( isset( $qv['count_total'] ) && $qv['count_total'] ) {
				/**
				 * Filters SELECT FOUND_ROWS() query for the current MT_User_Query instance.
				 *
				 * @since 3.2.0
				 ...
				 * @global mtdb $mtdb paCMec database abstraction object.
				 *
				 * @param string        $sql   The SELECT FOUND_ROWS() query for the current MT_User_Query.
				 * @param MT_User_Query $query The current MT_User_Query instance.
				 */
				$found_users_query = apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()', $this );

				$this->total_users = (int) $mtdb->get_var( $found_users_query );
			}
		}
		...
		if ( 'all_with_meta' === $qv['fields'] ) {
			...
			foreach ( $this->results as $userid ) {
				$r[ $userid ] = new MT_User( $userid, '', $qv['blog_id'] );
			}

			$this->results = $r;
		} elseif ( 'all' === $qv['fields'] ) {
			foreach ( $this->results as $key => $user ) {
				$this->results[ $key ] = new MT_User( $user, '', $qv['blog_id'] );
			}
		}
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) {
		...
		$like          = $leading_wild . $mtdb->esc_like( $string ) . $trailing_wild;

		foreach ( $cols as $col ) {
			if ( 'ID' === $col ) {
				$searches[] = $mtdb->prepare( "$col = %s", $string );
			} else {
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ), true ) ) {
			...
			) p ON ({$mtdb->users}.ID = p.post_author)
			";
			$_orderby          = 'post_count';
		} elseif ( 'ID' === $orderby || 'id' === $orderby ) {
			$_orderby = 'ID';
		} elseif ( 'meta_value' === $orderby || $this->get( 'meta_key' ) == $orderby ) {
			$_orderby = "$mtdb->usermeta.meta_value";
		} elseif ( 'meta_value_num' === $orderby ) {
			$_orderby = "$mtdb->usermeta.meta_value+0";
		} elseif ( 'include' === $orderby && ! empty( $this->query_vars['include'] ) ) {
			$include     = mt_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby    = "FIELD( $mtdb->users.ID, $include_sql )";
