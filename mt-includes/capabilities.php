	...
	switch ( $cap ) {
		...
		case 'delete_page':
			...
			if ( $post->post_author && $user_id == $post->post_author ) {
				...
				if ( in_array( $post->post_status, array( 'publish', 'future' ), true ) ) {
					...
					$status = get_post_meta( $post->ID, '_mt_trash_meta_status', true );
		...
		case 'edit_themes':
			...
			if ( defined( 'DISALLOW_FILE_EDIT' ) && DISALLOW_FILE_EDIT ) {
				$caps[] = 'do_not_allow';
			} elseif ( ! mt_is_file_mod_allowed( 'capability_edit_themes' ) ) {
				$caps[] = 'do_not_allow';
			} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
				$caps[] = 'do_not_allow';
			} else {
		...
		case 'update_core':
			...
			if ( ! mt_is_file_mod_allowed( 'capability_update_core' ) ) {
				$caps[] = 'do_not_allow';
			} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
				$caps[] = 'do_not_allow';
 ...
 *              by adding it to the function signature.
 *
 * @param int|MT_User $user       User ID or object.
 * @param string      $capability Capability name.
 * @param mixed       ...$args    Optional further parameters, typically starting with an object ID.
 * @return bool Whether the user has the given capability.
 */
...
/**
 * Retrieves the global MT_Roles instance and instantiates it if necessary.
 *
 * @since 4.3.0
 *
 * @global MT_Roles $mt_roles paCMec role management object.
 *
 * @return MT_Roles MT_Roles global instance if not already instantiated.
 */
function mt_roles() {
	global $mt_roles;
 ...
 * @return MT_Role|null MT_Role object if role is added, null if already exists.
 */
function add_role( $role, $display_name, $capabilities = array() ) {
	if ( empty( $role ) ) {
		return;
	}
	return mt_roles()->add_role( $role, $display_name, $capabilities );
}
 ...
 */
function mt_maybe_grant_install_languages_cap( $allcaps ) {
 ...
 * @return bool[] Filtered array of the user's capabilities.
 */
function mt_maybe_grant_resume_extensions_caps( $allcaps ) {
 ...
 * }
 * @param MT_User  $user    The user object.
 * @return bool[] Filtered array of the user's capabilities.
 */
function mt_maybe_grant_site_health_caps( $allcaps, $caps, $args, $user ) {
