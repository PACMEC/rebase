 ...
 * @since 0.71
 *
 * @global array $mt_cockneyreplace Array of formatted entities for certain common phrases.
 ...
 */
function mttexturize( $text, $reset = false ) {
	global $mt_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		/**
		 * Filters whether to skip running mttexturize().
		 *
		 * Returning false from the filter will effectively short-circuit mttexturize()
		 * and return the original text passed to the function instead.
		 *
		 * The filter runs only once, the first time mttexturize() is called.
		 *
		 * @since 4.0.0
		 *
		 * @see mttexturize()
		 *
		 * @param bool $run_texturize Whether to short-circuit mttexturize().
		 */
		$run_texturize = apply_filters( 'run_mttexturize', $run_texturize );
		...
		if ( isset( $mt_cockneyreplace ) ) {
			$cockney        = array_keys( $mt_cockneyreplace );
			$cockneyreplace = array_values( $mt_cockneyreplace );
		} else {
			/*
	...
	$shortcode_regex  = $found_shortcodes ? _get_mttexturize_shortcode_regex( $tagnames ) : '';
	$regex            = _get_mttexturize_split_regex( $shortcode_regex );

	$textarr = preg_split( $regex, $text, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY );

	foreach ( $textarr as &$curl ) {
		// Only call _mttexturize_pushpop_element if $curl is a delimiter.
		$first = $curl[0];
		if ( '<' === $first ) {
			if ( '<!--' === substr( $curl, 0, 4 ) ) {
				...
				_mttexturize_pushpop_element( $curl, $no_texturize_tags_stack, $no_texturize_tags );
			}
		} elseif ( '' === trim( $curl ) ) {
			...
			if ( '[[' !== substr( $curl, 0, 2 ) && ']]' !== substr( $curl, -2 ) ) {
				...
				_mttexturize_pushpop_element( $curl, $no_texturize_shortcodes_stack, $no_texturize_shortcodes );
			} else {
			...
			if ( false !== strpos( $curl, "'" ) ) {
				...
				$curl = mttexturize_primes( $curl, "'", $prime, $open_sq_flag, $closing_single_quote );
				$curl = str_replace( $apos_flag, $apos, $curl );
				$curl = str_replace( $open_sq_flag, $opening_single_quote, $curl );
			}
			if ( false !== strpos( $curl, '"' ) ) {
				$curl = preg_replace( $dynamic_characters['quote'], $dynamic_replacements['quote'], $curl );
				$curl = mttexturize_primes( $curl, '"', $double_prime, $open_q_flag, $closing_quote );
 ...
 */
function mttexturize_primes( $haystack, $needle, $prime, $open_quote, $close_quote ) {
	$spaces           = mt_spaces_regexp();
	$flag             = '<!--mt-prime-or-quote-->';
 ...
 */
function _mttexturize_pushpop_element( $text, &$stack, $disabled_elements ) {
...
function mtautop( $pee, $br = true ) {
	...
	$pee = mt_replace_in_html_tags( $pee, array( "\n" => ' <!-- mtnl --> ' ) );
	...
	if ( $br ) {
		...
		$pee = str_replace( '<MTPreserveNewline />', "\n", $pee );
	}
	...
	if ( false !== strpos( $pee, '<!-- mtnl -->' ) ) {
		$pee = str_replace( array( ' <!-- mtnl --> ', '<!-- mtnl -->' ), "\n", $pee );
	}
 ...
 * @since 4.4.0
 *
 * @param string $shortcode_regex The result from _get_mttexturize_shortcode_regex(). Optional.
 * @return string The regular expression
 */
function _get_mttexturize_split_regex( $shortcode_regex = '' ) {
	static $html_regex;
	...
	if ( ! isset( $html_regex ) ) {
		...
		$html_regex = // Needs replaced with mt_html_split() per Shortcode API Roadmap.
			'<'                  // Find start of element.
			. '(?(?=!--)'        // Is this a comment?
 ...
 * @return string The regular expression
 */
function _get_mttexturize_shortcode_regex( $tagnames ) {
 ...
 */
function mt_replace_in_html_tags( $haystack, $replace_pairs ) {
	// Find all elements.
	$textarr = mt_html_split( $haystack );
	$changed = false;
...
/**
 * Newline preservation help function for mtautop
 *
 * @since 3.1.0
 * @access private
 *
...
function shortcode_unautop( $pee ) {
	...
	// phpcs:disable Squiz.Strings.ConcatenationSpacing.PaddingFound,paCMec.WhiteSpace.PrecisionAlignment.Found -- don't remove regex indentation
	$pattern =
		'/'
		. '<p>'                              // Opening paragraph.
 ...
 */
function _mt_specialchars( $string, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	...
	// Store the site charset as a static to avoid multiple calls to mt_load_alloptions().
	if ( ! $charset ) {
	...
	if ( ! $double_encode ) {
		...
		$string = mt_kses_normalize_entities( $string, ( $quote_style & ENT_XML1 ) ? 'xml' : 'html' );
	}
 ...
 *                                Also compatible with old _mt_specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function mt_specialchars_decode( $string, $quote_style = ENT_NOQUOTES ) {
	$string = (string) $string;
	...
	// Match the previous behaviour of _mt_specialchars() when the $quote_style is not an accepted value.
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
 ...
 */
function mt_check_invalid_utf8( $string, $strip = false ) {
	$string = (string) $string;
	...
	// phpcs:ignore paCMec.PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $string.
...
function sanitize_file_name( $filename ) {
	...
	if ( false === strpos( $filename, '.' ) ) {
		...
		$filetype   = mt_check_filetype( 'test.' . $filename, $mime_types );
...
function sanitize_user( $username, $strict = false ) {
	...
	$username     = mt_strip_all_tags( $username );
...
function convert_invalid_entities( $content ) {
	...
	if ( strpos( $content, '&#1' ) !== false ) {
		$content = strtr( $content, $mt_htmltranswinuni );
	}
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.FunctionNameInvalid
...
function force_balance_tags( $text ) {
	...
	// MT bug fix for comments - in case you REALLY meant to type '< !--'.
	$text = str_replace( '< !--', '<    !--', $text );
	// MT bug fix for LOVE <3 (and other situations with '<' before a number).
	...
	// MT fix for the bug with HTML comments.
 ...
 */
function mt_rel_callback( $matches, $rel ) {
	$text = $matches[1];
	$atts = mt_kses_hair( $matches[1], mt_allowed_protocols() );

	if ( ! empty( $atts['href'] ) ) {
		if ( in_array( strtolower( mt_parse_url( $atts['href']['value'], PHP_URL_SCHEME ) ), array( 'http', 'https' ), true ) ) {
			if ( strtolower( mt_parse_url( $atts['href']['value'], PHP_URL_HOST ) ) === strtolower( mt_parse_url( home_url(), PHP_URL_HOST ) ) ) {
				return "<a $text>";
			}
		}
	}
 ...
 */
function mt_rel_nofollow( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		function( $matches ) {
			return mt_rel_callback( $matches, 'nofollow' );
		},
		$text
	);
	return mt_slash( $text );
}
 ...
 *
 * @since 2.3.0
 * @deprecated 5.3.0 Use mt_rel_callback()
 *
 * @param array $matches Single match.
 * @return string HTML A Element with `rel="nofollow"`.
 */
function mt_rel_nofollow_callback( $matches ) {
	return mt_rel_callback( $matches, 'nofollow' );
}
 ...
 * @return string Converted content.
 */
function mt_rel_ugc( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		function( $matches ) {
			return mt_rel_callback( $matches, 'nofollow ugc' );
		},
		$text
	);
	return mt_slash( $text );
 ...
 * @return string Converted content.
 */
function mt_targeted_link_rel( $text ) {
	...
	foreach ( $html_parts as &$part ) {
		$part = preg_replace_callback( '|<a\s([^>]*target\s*=[^>]*)>|i', 'mt_targeted_link_rel_callback', $part );
	}
 ...
 */
function mt_targeted_link_rel_callback( $matches ) {
	$link_html          = $matches[1];
	...
	if ( $is_escaped ) {
		// Replace only the quotes so that they are parsable by mt_kses_hair(), leave the rest as is.
		$link_html = preg_replace( '/\\\\([\'"])/', '$1', $link_html );
	}

	$atts = mt_kses_hair( $link_html, mt_allowed_protocols() );
	 ...
	 */
	$rel = apply_filters( 'mt_targeted_link_rel', 'noopener', $link_html );
 ...
 * @since 5.1.0
 */
function mt_init_targeted_link_rel_filters() {
	$filters = array(
		'title_save_pre',
		'content_save_pre',
		'excerpt_save_pre',
	...
	foreach ( $filters as $filter ) {
		add_filter( $filter, 'mt_targeted_link_rel' );
	};
}
 ...
 * @since 5.1.0
 */
function mt_remove_targeted_link_rel_filters() {
	$filters = array(
		'title_save_pre',
		'content_save_pre',
		'excerpt_save_pre',
	...
	foreach ( $filters as $filter ) {
		remove_filter( $filter, 'mt_targeted_link_rel' );
	};
}
 ...
 *
 * Looks up one smiley code in the $mtsmiliestrans global array and returns an
...
function translate_smiley( $matches ) {
	...
	return sprintf( '<img src="%s" alt="%s" class="mt-smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
 ...
 * @since 0.71
 *
 * @global string|array $mt_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $mt_smiliessearch;
	$output = '';
	if ( get_option( 'use_smilies' ) && ! empty( $mt_smiliessearch ) ) {
		...
		for ( $i = 0; $i < $stop; $i++ ) {
			...
			if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] ) {
				$content = preg_replace_callback( $mt_smiliessearch, 'translate_smiley', $content );
			}
 ...
 */
function mt_iso_descrambler( $string ) {
	/* this may only work with iso-8859-1, I'm afraid */
	if ( ! preg_match( '#\=\?(.+)\?Q\?(.+)\?\=#i', $string, $matches ) ) {
		return $string;
	} else {
		$subject = str_replace( '_', ' ', $matches[2] );
		return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_mt_iso_convert', $subject );
	}
}
 ...
 */
function _mt_iso_convert( $match ) {
...
function get_gmt_from_date( $string, $format = 'Y-m-d H:i:s' ) {
	$datetime = date_create( $string, mt_timezone() );
...
function get_date_from_gmt( $string, $format = 'Y-m-d H:i:s' ) {
	...
	return $datetime->setTimezone( mt_timezone() )->format( $format );
}
...
function iso8601_to_datetime( $date_string, $timezone = 'user' ) {
	...
	$datetime    = date_create( $date_string, $mt_timezone ); // Timezone is ignored if input has one.
	...
	if ( 'user' === $timezone ) {
		return $datetime->setTimezone( $mt_timezone )->format( 'Y-m-d H:i:s' );
	}
 ...
 * @param MT_Post|object|int $post Optional. MT_Post instance or Post ID/object. Default null.
 * @return string The excerpt.
 */
function mt_trim_excerpt( $text = '', $post = null ) {
	$raw_excerpt = $text;
	...
	if ( '' === trim( $text ) ) {
		...
		$text         = mt_trim_words( $text, $excerpt_length, $excerpt_more );
	}
	 ...
	 */
	return apply_filters( 'mt_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function mt_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}

	$original_text = $text;
	$text          = mt_strip_all_tags( $text );
	 ...
	 */
	return apply_filters( 'mt_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 *
 * Usually you should prepare queries using mtdb::prepare().
 ...
 *                            Defaults to return value of mt_allowed_protocols().
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	if ( 'display' === $_context ) {
		$url = mt_kses_normalize_entities( $url );
		$url = str_replace( '&amp;', '&#038;', $url );
		$url = str_replace( "'", '&#039;', $url );
	}
	...
	if ( '/' === $url[0] ) {
		...
		if ( ! is_array( $protocols ) ) {
			$protocols = mt_allowed_protocols();
		}
		$good_protocol_url = mt_kses_bad_protocol( $url, $protocols );
...
function esc_js( $text ) {
	$safe_text = mt_check_invalid_utf8( $text );
	$safe_text = _mt_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = mt_check_invalid_utf8( $text );
	$safe_text = _mt_specialchars( $safe_text, ENT_QUOTES );
	/**
	...
	$safe_text = (string) preg_replace_callback(
		...
		static function( $matches ) {
			...
			if ( ! empty( $matches['non_cdata'] ) ) {
				...
				return _mt_specialchars( $matches['non_cdata'], ENT_XML1 );
			}

			// Return the CDATA Section unchanged, escape HTML entities in the rest.
			return _mt_specialchars( $matches['non_cdata_followed_by_cdata'], ENT_XML1 ) . $matches['cdata'];
		},
		$safe_text
	);
 ...
 */
function mt_make_link_relative( $link ) {
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'new_admin_email':
			$value = $mtdb->strip_invalid_text_for_column( $mtdb->options, 'option_value', $value );
			if ( is_mt_error( $value ) ) {
		...
		case 'blogname':
			...
			if ( $value !== $original_value ) {
				$value = $mtdb->strip_invalid_text_for_column( $mtdb->options, 'option_value', mt_encode_emoji( $original_value ) );
			}
		...
		case 'siteurl':
			...
			if ( is_mt_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The paCMec address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
 ...
 */
function mt_parse_str( $string, &$array ) {
	 ...
	 */
	$array = apply_filters( 'mt_parse_str', $array );
}
 ...
 * @return string Converted text.
 */
function mt_pre_kses_less_than( $text ) {
	return preg_replace_callback( '%<[^>]*?((?=<)|>|$)%', 'mt_pre_kses_less_than_callback', $text );
}
 ...
 */
function mt_pre_kses_less_than_callback( $matches ) {
 ...
 */
function mt_pre_kses_block_attributes( $string, $allowed_html, $allowed_protocols ) {
	/*
	 * `filter_block_content` is expected to call `mt_kses`. Temporarily remove
	 * the filter to avoid recursion.
	 */
	remove_filter( 'pre_kses', 'mt_pre_kses_block_attributes', 10 );
...
/**
 * paCMec implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
 ...
 */
function mt_sprintf( $pattern, ...$args ) {
	$len       = strlen( $pattern );
	$start     = 0;
	$result    = '';
	$arg_index = 0;
	while ( $len > $start ) {
		...
		if ( '%' === $pattern[ $start ] ) {
			...
			/**
			 * Filters a fragment from the pattern passed to mt_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'mt_sprintf', $fragment, $arg );
 ...
 */
function mt_sprintf_l( $pattern, $args ) {
	// Not a match.
	...
	/**
	 * Filters the translated delimiters used by mt_sprintf_l().
 ...
 * @return string The excerpt.
 */
function mt_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more ) {
		$more = '';
	}

	$str     = mt_strip_all_tags( $str, true );
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], mt_allowed_protocols(), true ) ?
			$m[3] :
			MT_Http::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
 ...
 * will return 'something'. mt_strip_all_tags will return ''
 *
 * @since 2.9.0
 *
 ...
 */
function mt_strip_all_tags( $string, $remove_breaks = false ) {
 ...
 * @see mt_check_invalid_utf8()
 * @see mt_strip_all_tags()
 *
...
function _sanitize_text_fields( $str, $keep_newlines = false ) {
	...
	$filtered = mt_check_invalid_utf8( $str );

	if ( strpos( $filtered, '<' ) !== false ) {
		$filtered = mt_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = mt_strip_all_tags( $filtered, false );
 ...
 * @return string
 */
function mt_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( array( '%2F', '%5C' ), '/', urlencode( $path ) ), $suffix ) );
}

// phpcs:disable paCMec.MT.CapitalPDangit.Misspelled, paCMec.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'mt_title' === $current_filter ) {
 ...
 */
function mt_slash( $value ) {
	if ( is_array( $value ) ) {
		$value = array_map( 'mt_slash', $value );
	}
...
function mt_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'mt_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			...
			'mtemoji' => apply_filters( 'script_loader_src', includes_url( "js/mt-emoji.js?$version" ), 'mtemoji' ),
			/** This filter is documented in mt-includes/class.mt-scripts.php */
		...
		<script<?php echo $type_attr; ?>>
			window._mtemojiSettings = <?php echo mt_json_encode( $settings ); ?>;
			<?php readfile( ABSPATH . MTINC . '/js/mt-emoji-loader.js' ); ?>
		</script>
		<?php
	} else {
		$settings['source'] = array(
			/** This filter is documented in mt-includes/class.mt-scripts.php */
			'concatemoji' => apply_filters( 'script_loader_src', includes_url( "js/mt-emoji-release.min.js?$version" ), 'concatemoji' ),
		);

		/*
		 * If you're looking at a src version of this file, you'll see an "include"
		 * statement below. This is used by the `npm run build` process to directly
		 * include a minified version of mt-emoji-loader.js, instead of using the
		 * readfile() method from above.
		 *
		 * If you're looking at a build version of this file, you'll see a string of
		 * minified JavaScript. If you need to debug it, please turn on SCRIPT_DEBUG
		 * and edit mt-emoji-loader.js directly.
		 */
		?>
		<script<?php echo $type_attr; ?>>
			window._mtemojiSettings = <?php echo mt_json_encode( $settings ); ?>;
			!function(e,a,t){var n,r,o,i=a.createElement("canvas"),p=i.getContext&&i.getContext("2d");function s(e,t){var a=String.fromCharCode;p.clearRect(0,0,i.width,i.height),p.fillText(a.apply(this,e),0,0);e=i.toDataURL();return p.clearRect(0,0,i.width,i.height),p.fillText(a.apply(this,t),0,0),e===i.toDataURL()}function c(e){var t=a.createElement("script");t.src=e,t.defer=t.type="text/javascript",a.getElementsByTagName("head")[0].appendChild(t)}for(o=Array("flag","emoji"),t.supports={everything:!0,everythingExceptFlag:!0},r=0;r<o.length;r++)t.supports[o[r]]=function(e){if(!p||!p.fillText)return!1;switch(p.textBaseline="top",p.font="600 32px Arial",e){case"flag":return s([127987,65039,8205,9895,65039],[127987,65039,8203,9895,65039])?!1:!s([55356,56826,55356,56819],[55356,56826,8203,55356,56819])&&!s([55356,57332,56128,56423,56128,56418,56128,56421,56128,56430,56128,56423,56128,56447],[55356,57332,8203,56128,56423,8203,56128,56418,8203,56128,56421,8203,56128,56430,8203,56128,56423,8203,56128,56447]);case"emoji":return!s([55357,56424,8205,55356,57212],[55357,56424,8203,55356,57212])}return!1}(o[r]),t.supports.everything=t.supports.everything&&t.supports[o[r]],"flag"!==o[r]&&(t.supports.everythingExceptFlag=t.supports.everythingExceptFlag&&t.supports[o[r]]);t.supports.everythingExceptFlag=t.supports.everythingExceptFlag&&!t.supports.flag,t.DOMReady=!1,t.readyCallback=function(){t.DOMReady=!0},t.supports.everything||(n=function(){t.readyCallback()},a.addEventListener?(a.addEventListener("DOMContentLoaded",n,!1),e.addEventListener("load",n,!1)):(e.attachEvent("onload",n),a.attachEvent("onreadystatechange",function(){"complete"===a.readyState&&t.readyCallback()})),(n=t.source||{}).concatemoji?c(n.concatemoji):n.mtemoji&&n.twemoji&&(c(n.twemoji),c(n.mtemoji)))}(window,document,window._mtemojiSettings);
		</script>
		<?php
	}
}
 ...
 */
function mt_encode_emoji( $content ) {
	$emoji = _mt_emoji_list( 'partials' );
 ...
 * @return string The encoded content.
 */
function mt_staticize_emoji( $text ) {
	if ( false === strpos( $text, '&#x' ) ) {
		if ( ( function_exists( 'mb_check_encoding' ) && mb_check_encoding( $text, 'ASCII' ) ) || ! preg_match( '/[^\x00-\x7F]/', $text ) ) {
			...
			$encoded_text = mt_encode_emoji( $text );
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] && false !== strpos( $content, '&#x' ) ) {
			foreach ( $possible_emoji as $emojum => $emoji_char ) {
				...
				$entity = sprintf( '<img src="%s" alt="%s" class="mt-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $file . $ext, $emoji_char );
 ...
 */
function mt_staticize_emoji_for_email( $mail ) {
	 ...
	 * in mt_mail() - get the Content-Type from the headers, if there is one,
	 * then pass it through the mt_mail_content_type filter, in case a plugin
	...
	if ( 'text/html' === $content_type ) {
		$mail['message'] = mt_staticize_emoji( $mail['message'] );
	}
 ...
 * @return array An array to match all emoji that paCMec recognises.
 */
function _mt_emoji_list( $type = 'entities' ) {
 ...
 *
 * @since 1.2.0
 * @since 4.4.0 Moved to mt-includes/formatting.php from mt-admin/includes/misc.php and added $length param.
 *
