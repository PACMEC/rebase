<?php
/**
 * Canonical API to handle paCMec Redirecting
 *
 ...
 * page/post previews, MT admin, Trackbacks, robots.txt, favicon.ico, searches,
 * or on POST requests.
 *
 * Will also attempt to find the correct link when a user enters a URL that does
 * not exist based on exact paCMec query. Will instead try to parse the URL
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	global $mt_rewrite, $is_IIS, $mt_query, $mtdb, $mt;

	if ( isset( $_SERVER['REQUEST_METHOD'] ) && ! in_array( strtoupper( $_SERVER['REQUEST_METHOD'] ), array( 'GET', 'HEAD' ), true ) ) {
		return;
	}

	// If we're not in mt-admin and the post has been published and preview nonce
	// is non-existent or invalid then no need for preview in query.
	if ( is_preview() && get_query_var( 'p' ) && 'publish' === get_post_status( get_query_var( 'p' ) ) ) {
		if ( ! isset( $_GET['preview_id'] )
			|| ! isset( $_GET['preview_nonce'] )
			|| ! mt_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . (int) $_GET['preview_id'] )
		) {
			$mt_query->is_preview = false;
		}
	}
	...
	if ( is_singular() && $mt_query->post_count < 1 && $post_id ) {

		$vars = $mtdb->get_results( $mtdb->prepare( "SELECT post_type, post_parent FROM $mtdb->posts WHERE ID = %d", $post_id ) );
	...
	// These tests give us a MT-generated permalink.
	if ( is_404() ) {
		...
		if ( $year && $month && $day ) {
			...
			if ( ! mt_checkdate( $month, $day, $year, $date ) ) {
		...
		if ( get_query_var( 'page' ) ) {
			...
			if ( $mt_query->queried_object instanceof MT_Post ) {
				$post_id = $mt_query->queried_object->ID;
			} elseif ( $mt_query->post ) {
				$post_id = $mt_query->post->ID;
			}
	...
	} elseif ( is_object( $mt_rewrite ) && $mt_rewrite->using_permalinks() ) {

		// Rewriting of old ?p=X, ?m=2004, ?m=200401, ?m=20040101.
		if ( is_attachment()
			&& ! array_diff( array_keys( $mt->query_vars ), array( 'attachment', 'attachment_id' ) )
			&& ! $redirect_url
		) {
			...
			$redirect_url = get_permalink( $mt_query->get_queried_object_id() );
			$redirect_obj = get_post( $mt_query->get_queried_object_id() );
			...
			if ( false !== $author
				&& $mtdb->get_var( $mtdb->prepare( "SELECT ID FROM $mtdb->posts WHERE $mtdb->posts.post_author = %d AND $mtdb->posts.post_status = 'publish' LIMIT 1", $author->ID ) )
			) {
			...
			foreach ( $mt_query->tax_query->queried_terms as $tax_query ) {
				$term_count += count( $tax_query['terms'] );
			}

			$obj = $mt_query->get_queried_object();

			if ( $term_count <= 1 && ! empty( $obj->term_id ) ) {
				$tax_url = get_term_link( (int) $obj->term_id, $obj->taxonomy );

				if ( $tax_url && ! is_mt_error( $tax_url ) ) {
					if ( ! empty( $redirect['query'] ) ) {
						...
						$rewrite_vars = array_diff( array_keys( $mt_query->query ), array_keys( $_GET ) );
		...
		} elseif ( is_single() && strpos( $mt_rewrite->permalink_structure, '%category%' ) !== false ) {
			$category_name = get_query_var( 'category_name' );

			if ( $category_name ) {
				$category = get_category_by_path( $category_name );

				if ( ! $category || is_mt_error( $category )
					|| ! has_term( $category->term_id, 'category', $mt_query->get_queried_object_id() )
				) {
					$redirect_url = get_permalink( $mt_query->get_queried_object_id() );
					$redirect_obj = get_post( $mt_query->get_queried_object_id() );
				}
		...
		if ( is_singular() && get_query_var( 'page' ) ) {
			...
			if ( $page > 1 ) {
				...
				if ( is_front_page() ) {
					$redirect_url .= user_trailingslashit( "$mt_rewrite->pagination_base/$page", 'paged' );
				} else {
		...
		if ( get_query_var( 'sitemap' ) ) {
			...
			while ( preg_match( "#/$mt_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] )
				|| preg_match( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+)?$#', $redirect['path'] )
				|| preg_match( "#/{$mt_rewrite->comments_pagination_base}-[0-9]+(/+)?$#", $redirect['path'] )
			) {
				// Strip off any existing paging.
				$redirect['path'] = preg_replace( "#/$mt_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path'] );
				// Strip off feed endings.
				$redirect['path'] = preg_replace( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+|$)#', '/', $redirect['path'] );
				// Strip off any existing comment paging.
				$redirect['path'] = preg_replace( "#/{$mt_rewrite->comments_pagination_base}-[0-9]+?(/+)?$#", '/', $redirect['path'] );
			}

			$addl_path    = '';
			$default_feed = get_default_feed();

			if ( is_feed() && in_array( $feed, $mt_rewrite->feeds, true ) ) {
				...
				$old_feed_files = array(
					'mt-atom.php'         => 'atom',
					'mt-commentsrss2.php' => 'comments_rss2',
					'mt-feed.php'         => $default_feed,
					'mt-rdf.php'          => 'rdf',
					'mt-rss.php'          => 'rss2',
					'mt-rss2.php'         => 'rss2',
				);
			...
			if ( $paged > 0 ) {
				...
				if ( ! is_feed() ) {
					if ( ! is_single() ) {
						...
						if ( $paged > 1 ) {
							$addl_path .= user_trailingslashit( "$mt_rewrite->pagination_base/$paged", 'paged' );
						}
					}
				} elseif ( $paged > 1 ) {
			...
			if ( get_option( 'page_comments' )
				...
				$addl_path  = ( ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '' );
				$addl_path .= user_trailingslashit( $mt_rewrite->comments_pagination_base . '-' . $cpage, 'commentpaged' );

				$redirect['query'] = remove_query_arg( 'cpage', $redirect['query'] );
			}

			// Strip off trailing /index.php/.
			$redirect['path'] = preg_replace( '|/' . preg_quote( $mt_rewrite->index, '|' ) . '/?$|', '/', $redirect['path'] );
			$redirect['path'] = user_trailingslashit( $redirect['path'] );

			if ( ! empty( $addl_path )
				&& $mt_rewrite->using_index_permalinks()
				&& strpos( $redirect['path'], '/' . $mt_rewrite->index . '/' ) === false
			) {
				$redirect['path'] = trailingslashit( $redirect['path'] ) . $mt_rewrite->index . '/';
			}
		...
		if ( 'mt-register.php' === basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in mt-login.php */
				$redirect_url = apply_filters( 'mt_signup_location', network_site_url( 'mt-signup.php' ) );
			} else {
				$redirect_url = mt_registration_url();
			}
	...
	if ( ! $mt_rewrite->using_index_permalinks() ) {
		$redirect['path'] = str_replace( '/' . $mt_rewrite->index . '/', '/', $redirect['path'] );
	}

	// Trailing slashes.
	if ( is_object( $mt_rewrite ) && $mt_rewrite->using_permalinks()
	...
	if ( $redirect_obj instanceof MT_Post ) {
...
function redirect_guess_404_permalink() {
	...
	if ( get_query_var( 'name' ) ) {
		...
		if ( $strict_guess ) {
			$where = $mtdb->prepare( 'post_name = %s', get_query_var( 'name' ) );
		} else {
			$where = $mtdb->prepare( 'post_name LIKE %s', $mtdb->esc_like( get_query_var( 'name' ) ) . '%' );
		}
		...
		if ( get_query_var( 'post_type' ) ) {
			if ( is_array( get_query_var( 'post_type' ) ) ) {
				...
				$where .= $mtdb->prepare( ' AND post_type = %s', get_query_var( 'post_type' ) );
			}
		} else {
		...
		if ( get_query_var( 'year' ) ) {
			$where .= $mtdb->prepare( ' AND YEAR(post_date) = %d', get_query_var( 'year' ) );
		}
		if ( get_query_var( 'monthnum' ) ) {
			$where .= $mtdb->prepare( ' AND MONTH(post_date) = %d', get_query_var( 'monthnum' ) );
		}
		if ( get_query_var( 'day' ) ) {
			$where .= $mtdb->prepare( ' AND DAYOFMONTH(post_date) = %d', get_query_var( 'day' ) );
		}

		// phpcs:ignore paCMec.DB.PreparedSQL.InterpolatedNotPrepared
		$post_id = $mtdb->get_var( "SELECT ID FROM $mtdb->posts WHERE $where AND post_status = 'publish'" );
 ...
 *
 * If a user visits example.com/admin, they'll be redirected to /mt-admin.
 * Visiting /login redirects to /mt-login.php, and so on.
 *
 * @since 3.4.0
 *
 * @global MT_Rewrite $mt_rewrite paCMec rewrite component.
 */
function mt_redirect_admin_locations() {
	global $mt_rewrite;

	if ( ! ( is_404() && $mt_rewrite->using_permalinks() ) ) {
		return;
	}

	$admins = array(
		home_url( 'mt-admin', 'relative' ),
