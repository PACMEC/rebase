 ...
 *                                Passed to the `mt_initialize_site` hook.
 * }
 * @return int|MT_Error The new site's ID on success, or error object on failure.
 */
function mt_insert_site( array $data ) {
	global $mtdb;
	...
	$prepared_data = mt_prepare_site_data( $data, $defaults );
	if ( is_mt_error( $prepared_data ) ) {
		return $prepared_data;
	}

	if ( false === $mtdb->insert( $mtdb->blogs, $prepared_data ) ) {
		return new MT_Error( 'db_insert_error', __( 'Could not insert site into the database.' ), $mtdb->last_error );
	}

	$site_id = (int) $mtdb->insert_id;
	...
	if ( ! $new_site ) {
		return new MT_Error( 'get_site_error', __( 'Could not retrieve site data.' ) );
	}
	 ...
	 * @since 5.1.0
	 *
	 * @param MT_Site $new_site New site object.
	 */
	do_action( 'mt_insert_site', $new_site );
	 ...
	 */
	do_action( 'mt_initialize_site', $new_site, $args );

	// Only compute extra hook parameters if the deprecated hook is actually in use.
	if ( has_action( 'mtmu_new_blog' ) ) {
		$user_id = ! empty( $args['user_id'] ) ? $args['user_id'] : 0;
		$meta    = ! empty( $args['options'] ) ? $args['options'] : array();

		// MTLANG was passed with `$meta` to the `mtmu_new_blog` hook prior to 5.1.0.
		if ( ! array_key_exists( 'MTLANG', $meta ) ) {
			$meta['MTLANG'] = get_network_option( $new_site->network_id, 'MTLANG' );
		}

		// Rebuild the data expected by the `mtmu_new_blog` hook prior to 5.1.0 using allowed keys.
		// The `$allowed_data_fields` matches the one used in `mtmu_create_blog()`.
		 ...
		 *
		 * @since MU (3.0.0)
		 * @deprecated 5.1.0 Use {@see 'mt_insert_site'} instead.
		 *
 ...
 * @param array $data    Site data to update. See {@see mt_insert_site()} for the list of supported keys.
 * @return int|MT_Error The updated site's ID on success, or error object on failure.
 */
function mt_update_site( $site_id, array $data ) {
	global $mtdb;

	if ( empty( $site_id ) ) {
		return new MT_Error( 'site_empty_id', __( 'Site ID must not be empty.' ) );
	}

	$old_site = get_site( $site_id );
	if ( ! $old_site ) {
		return new MT_Error( 'site_not_exist', __( 'Site does not exist.' ) );
	}
	...
	$data = mt_prepare_site_data( $data, $defaults, $old_site );
	if ( is_mt_error( $data ) ) {
		return $data;
	}

	if ( false === $mtdb->update( $mtdb->blogs, $data, array( 'blog_id' => $old_site->id ) ) ) {
		return new MT_Error( 'db_update_error', __( 'Could not update site in the database.' ), $mtdb->last_error );
	}
	 ...
	 *
	 * @param MT_Site $new_site New site object.
	 * @param MT_Site $old_site Old site object.
	 */
	do_action( 'mt_update_site', $new_site, $old_site );
 ...
 * @return MT_Site|MT_Error The deleted site object on success, or error object on failure.
 */
function mt_delete_site( $site_id ) {
	global $mtdb;
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `MT_Error::add()` method. If any errors
	 * are present, the site will not be deleted.
	 *
	 * @since 5.1.0
	 *
	 * @param MT_Error $errors   Error object to add validation errors to.
	 * @param MT_Site  $old_site The site object to be deleted.
	 */
	do_action( 'mt_validate_site_deletion', $errors, $old_site );
	 ...
	 * @since 5.1.0
	 *
	 * @param MT_Site $old_site Deleted site object.
	 */
	do_action( 'mt_uninitialize_site', $old_site );

	if ( is_site_meta_supported() ) {
		$blog_meta_ids = $mtdb->get_col( $mtdb->prepare( "SELECT meta_id FROM $mtdb->blogmeta WHERE blog_id = %d ", $old_site->id ) );
		foreach ( $blog_meta_ids as $mid ) {
			delete_metadata_by_mid( 'blog', $mid );
		}
	}

	if ( false === $mtdb->delete( $mtdb->blogs, array( 'blog_id' => $old_site->id ) ) ) {
		return new MT_Error( 'db_delete_error', __( 'Could not delete site from the database.' ), $mtdb->last_error );
	}
	 ...
	 * @param MT_Site $old_site Deleted site object.
	 */
	do_action( 'mt_delete_site', $old_site );
 ...
 * @since 4.6.0
 *
 * @param MT_Site|int|null $site Optional. Site to retrieve. Default is the current site.
 * @return MT_Site|null The site object or null if not found.
 */
function get_site( $site = null ) {
	...
	if ( $site instanceof MT_Site ) {
		$_site = $site;
	} elseif ( is_object( $site ) ) {
		$_site = new MT_Site( $site );
	} else {
		$_site = MT_Site::get_instance( $site );
	}
...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $mtdb->get_results( sprintf( "SELECT * FROM $mtdb->blogs WHERE blog_id IN (%s)", implode( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore paCMec.DB.PreparedSQL.NotPrepared
...
function update_site_cache( $sites, $update_meta_cache = true ) {
	...
	foreach ( $sites as $site ) {
		...
		mt_cache_add( $site->blog_id, $site, 'sites' );
		mt_cache_add( $site->blog_id . 'short', $site, 'blog-details' );
	}
...
function update_sitemeta_cache( $site_ids ) {
	...
	if ( ! has_filter( 'update_blog_metadata_cache', 'mt_check_site_meta_support_prefilter' ) ) {
		add_filter( 'update_blog_metadata_cache', 'mt_check_site_meta_support_prefilter' );
	}
 ...
 *                                           Default false.
 *     @type array        $date_query        Date query clauses to limit sites by. See MT_Date_Query.
 *                                           Default null.
 ...
 * }
 * @return array|int List of MT_Site objects, a list of site IDs when 'fields' is set to 'ids',
 *                   or the number of sites when 'count' is passed as a query var.
 */
function get_sites( $args = array() ) {
	$query = new MT_Site_Query();
 ...
 *                               See {@see mt_insert_site()} for the possibly included data.
 * @param array        $defaults Site data defaults to parse $data against.
 * @param MT_Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|MT_Error Site data ready for a database transaction, or MT_Error in case a validation
 *                        error occurred.
 */
function mt_prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *                    See {@see mt_insert_site()} for the possibly included data.
	 */
	$data = apply_filters( 'mt_normalize_site_data', $data );

	$allowed_data_fields = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data                = array_intersect_key( mt_parse_args( $data, $defaults ), array_flip( $allowed_data_fields ) );
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `MT_Error::add()` method.
	 *
	 * @since 5.1.0
	 *
	 * @param MT_Error     $errors   Error object to add validation errors to.
	 * @param array        $data     Associative array of complete site data. See {@see mt_insert_site()}
	 *                               for the included data.
	 * @param MT_Site|null $old_site The old site object if the data belongs to a site being updated,
	 *                               or null if it is a new site being inserted.
	 */
	do_action( 'mt_validate_site_data', $errors, $data, $old_site );
 ...
 */
function mt_normalize_site_data( $data ) {
 ...
 * @since 5.1.0
 *
 * @param MT_Error     $errors   Error object, passed by reference. Will contain validation errors if
 *                               any occurred.
 * @param array        $data     Associative array of complete site data. See {@see mt_insert_site()}
 *                               for the included data.
 * @param MT_Site|null $old_site The old site object if the data belongs to a site being updated,
 *                               or null if it is a new site being inserted.
 */
function mt_validate_site_data( $errors, $data, $old_site = null ) {
	...
	foreach ( $date_fields as $date_field ) {
		...
		if ( '0000-00-00 00:00:00' !== $data[ $date_field ] ) {
			...
			$valid_date = mt_checkdate( $month, $day, $year, $data[ $date_field ] );
			if ( ! $valid_date ) {
 ...
 *
 * @param int|MT_Site $site_id Site ID or object.
 * @param array       $args    {
 ...
 *                           Default empty array.
 * }
 * @return true|MT_Error True on success, or error object on failure.
 */
function mt_initialize_site( $site_id, array $args = array() ) {
	global $mtdb, $mt_roles;
	...
	if ( ! $site ) {
		return new MT_Error( 'site_invalid_id', __( 'Site with the ID does not exist.' ) );
	}

	if ( mt_is_site_initialized( $site ) ) {
		return new MT_Error( 'site_already_initialized', __( 'The site appears to be already initialized.' ) );
	}
	 ...
	 * @param MT_Site    $site    Site that is being initialized.
	 * @param MT_Network $network Network that the site belongs to.
	 */
	$args = apply_filters( 'mt_initialize_site_args', $args, $site, $network );

	$orig_installing = mt_installing();
	if ( ! $orig_installing ) {
		mt_installing( true );
	}
	...
	populate_options(
		array_merge(
			array(
				...
				'blogname'    => mt_unslash( $args['title'] ),
				'admin_email' => '',
				...
				'MTLANG'      => get_network_option( $network->id, 'MTLANG' ),
			),
			$args['options']
		)
	);
	...
	populate_roles();
	$mt_roles = new MT_Roles();

	// Populate metadata for the site.
	populate_site_meta( $site->id, $args['meta'] );

	// Remove all permissions that may exist for the site.
	$table_prefix = $mtdb->get_blog_prefix();
	delete_metadata( 'user', 0, $table_prefix . 'user_level', null, true );   // Delete all.
	delete_metadata( 'user', 0, $table_prefix . 'capabilities', null, true ); // Delete all.

	// Install default site content.
	mt_install_defaults( $args['user_id'] );
 ...
 * @return true|MT_Error True on success, or error object on failure.
 */
function mt_uninitialize_site( $site_id ) {
	global $mtdb;
	...
	if ( ! mt_is_site_initialized( $site ) ) {
		return new MT_Error( 'site_already_uninitialized', __( 'The site appears to be already uninitialized.' ) );
	}
	 ...
	 */
	$drop_tables = apply_filters( 'mtmu_drop_tables', $tables, $site->id );

	foreach ( (array) $drop_tables as $table ) {
		$mtdb->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore paCMec.DB.PreparedSQL.NotPrepared
	}
	 ...
	 * @since MU (3.0.0)
	 *
	 * @param string $basedir Uploads path without subdirectory. @see mt_upload_dir()
	 * @param int    $site_id The site ID.
	 */
	$dir     = apply_filters( 'mtmu_delete_blog_upload_dir', $uploads['basedir'], $site->id );
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable paCMec.PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable paCMec.PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
 ...
 */
function mt_is_site_initialized( $site_id ) {
	global $mtdb;
	 ...
	 */
	$pre = apply_filters( 'pre_mt_is_site_initialized', null, $site_id );
	if ( null !== $pre ) {
		return (bool) $pre;
	}
	...
	if ( get_current_blog_id() !== $site_id ) {
		$switch = true;
		remove_action( 'switch_blog', 'mt_switch_roles_and_user', 1 );
		switch_to_blog( $site_id );
	}

	$suppress = $mtdb->suppress_errors();
	$result   = (bool) $mtdb->get_results( "DESCRIBE {$mtdb->posts}" );
	$mtdb->suppress_errors( $suppress );

	if ( $switch ) {
		restore_current_blog();
		add_action( 'switch_blog', 'mt_switch_roles_and_user', 1, 2 );
	}
 ...
 *
 * @param MT_Site|int $blog The site object or ID to be cleared from cache.
 */
function clean_blog_cache( $blog ) {
	...
	if ( ! $blog ) {
		...
		// Make sure a MT_Site object exists even when the site has been deleted.
		$blog = new MT_Site(
			(object) array(
				'blog_id' => $blog_id,
				'domain'  => null,
	...
	mt_cache_delete( $blog_id, 'sites' );
	mt_cache_delete( $blog_id, 'site-details' );
	mt_cache_delete( $blog_id, 'blog-details' );
	mt_cache_delete( $blog_id . 'short', 'blog-details' );
	mt_cache_delete( $domain_path_key, 'blog-lookup' );
	mt_cache_delete( $domain_path_key, 'blog-id-cache' );
	mt_cache_delete( $blog_id, 'blog_meta' );
	 ...
	 * @param MT_Site $blog            Site object.
 ...
 * @since 5.1.0
 *
 * @param MT_Site      $new_site The site object that has been inserted, updated or deleted.
 * @param MT_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function mt_maybe_update_network_site_counts_on_update( $new_site, $old_site = null ) {
	if ( null === $old_site ) {
		mt_maybe_update_network_site_counts( $new_site->network_id );
		return;
	}

	if ( $new_site->network_id != $old_site->network_id ) {
		mt_maybe_update_network_site_counts( $new_site->network_id );
		mt_maybe_update_network_site_counts( $old_site->network_id );
	}
}
 ...
 * @since 5.1.0
 *
 * @param MT_Site      $new_site The site object after the update.
 * @param MT_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function mt_maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	$site_id = $new_site->id;

	// Use the default values for a site if no previous state is given.
	if ( ! $old_site ) {
		$old_site = new MT_Site( new stdClass() );
	}
 ...
 *
 * @param MT_Site $new_site The site object after the update.
 * @param MT_Site $old_site The site obejct prior to the update.
 */
function mt_maybe_clean_new_site_cache_on_update( $new_site, $old_site ) {
 ...
 */
function mt_update_blog_public_option_on_site_update( $site_id, $public ) {

	// Bail if the site's database tables do not exist (yet).
	if ( ! mt_is_site_initialized( $site_id ) ) {
		return;
	}
 ...
 * @since 5.1.0
 */
function mt_cache_set_sites_last_changed() {
	mt_cache_set( 'last_changed', microtime(), 'sites' );
}
 ...
 */
function mt_check_site_meta_support_prefilter( $check ) {
	if ( ! is_site_meta_supported() ) {
		/* translators: %s: Database table name. */
		_doing_it_wrong( __FUNCTION__, sprintf( __( 'The %s table is not installed. Please run the network database upgrade.' ), $GLOBALS['mtdb']->blogmeta ), '5.1.0' );
		return false;
	}
