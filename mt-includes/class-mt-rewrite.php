 ...
 *
 * The paCMec Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * paCMec Query class.
 *
 * The Rewrite along with MT class function as a front controller for paCMec.
...
class MT_Rewrite {
	 ...
	 * `MT_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see MT_Rewrite::init()
	 * @see MT_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to paCMec' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for paCMec into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see MT_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * set to true. Prior to paCMec 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells MT::parse_request() to check if a URL matching the page
	 ...
	 * `MT_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in MT_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see MT_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var array
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see MT_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var array
	 */
	...
	public function page_uri_index() {
		...
		$pages = $mtdb->get_results( "SELECT ID, post_name, post_parent FROM $mtdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'" );
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name.
			$uri         = get_page_uri( $id );
			$attachments = $mtdb->get_results( $mtdb->prepare( "SELECT ID, post_name, post_parent FROM $mtdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ) );
	 ...
	 * @since 1.5.0
	 *
	 * @see MT_Rewrite::$rewritecode
	 * @see MT_Rewrite::$rewritereplace
	 * @see MT_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main MT_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of MT_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see MT_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current MT_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*mt-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*mt-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules.
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*mt-signup.php$']   = $this->index . '?signup=true';
			$registration_pages['.*mt-activate.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*mt-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param MT_Rewrite $this Current MT_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and MT_Rewrite::rewrite_rules() is that
	 ...
	 * @return string[] Array of rewrite rules keyed by their regex pattern.
	 */
	public function mt_rewrite_rules() {
		$this->rules = get_option( 'rewrite_rules' );
		if ( empty( $this->rules ) ) {
			$this->matches = 'matches';
			$this->rewrite_rules();
			if ( ! did_action( 'mt_loaded' ) ) {
				add_action( 'mt_loaded', array( $this, 'flush_rules' ) );
				return $this->rules;
			}
	 ...
	 *
	 * Will add the non_mt_rules property rules to the .htaccess file before
	 * the paCMec rewrite rules one.
	 *
	 * @since 1.5.0
	 *
	 * @return string
	...
	public function mod_rewrite_rules() {
		...
		// Add in the rules that don't redirect to MT's index.php (and thus shouldn't be handled by MT at all).
		foreach ( (array) $this->non_mt_rules as $match => $query ) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="paCMec: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	 ...
	 * query strings. The new permastruct is added to the MT_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by MT_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to MT_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how MT_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `MT_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls MT_Rewrite::mt_rewrite_rules() after removing the 'rewrite_rules' option.
	 ...
	 * MT_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls MT_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls MT_Rewrite::init() after
