/**
 * @output mt-includes/js/mce-view.js
 */
...
( function( window, mt, shortcode, $ ) {
	...
	mt.mce.views = {
		 ...
		 * @param {Object} extend An object to extend mt.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = mt.mce.View.extend( _.extend( extend, { type: type } ) );
		},
		...
		setMarkers: function( content, editor ) {
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: instance.ignore ? text : '<p data-mtview-marker="' + instance.encodedText + '">' + text + '</p>',
							processed: true
						} );
			...
			return content.replace( /<p>\s*<p data-mtview-marker=/g, '<p data-mtview-marker=' ).replace( /<\/p>\s*<\/p>/g, '</p>' );
		},
		 ...
		 *
		 * @return {mt.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {mt.mce.View} The view instance or undefined.
		 */
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-mtview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-mtview-text' ) || '' );
		},
	 ...
	 * @param {Object} options Options.
	 */
	mt.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	mt.mce.View.extend = Backbone.View.extend;

	_.extend( mt.mce.View.prototype, /** @lends mt.mce.View.prototype */{
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-mtview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-mtview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				...
				if ( ! this.loader && $( node ).text() !== tinymce.DOM.decode( this.text ) ) {
					editor.dom.setAttrib( node, 'data-mtview-marker', null );
					return;
				}

				$viewNode = editor.$(
					'<div class="mtview mtview-wrap" data-mtview-text="' + this.encodedText + '" data-mtview-type="' + this.type + '" contenteditable="false"></div>'
				);
			...
			if ( _.isObject( content ) && ( content.sandbox || content.head || content.body.indexOf( '<script' ) !== -1 ) ) {
				...
				this.getNodes( function( editor, node ) {
					...
					editor.undoManager.transact( function() {
						...
						editor.dom.add( node, 'span', { 'class': 'mtview-end' } );
					} );
			...
			this.getNodes( function( editor, node ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/managertechnology/mt-content.css' ) === -1 ) {
				...
				editor.undoManager.transact( function() {
					...
					dom.add( node, 'span', { 'class': 'mtview-end' } );
				} );
						...
						'<head>' +
							...
							'<style>' +
								'html {' +
									...
									'margin: 0;' +
								'}' +
								'body#mtview-iframe-sandbox {' +
									'background: transparent;' +
									'padding: 1px 0 !important;' +
									'margin: -1px 0 0 !important;' +
								'}' +
								'body#mtview-iframe-sandbox:before,' +
								'body#mtview-iframe-sandbox:after {' +
									'display: none;' +
									'content: "";' +
								'}' +
								'iframe {' +
							...
							'</style>' +
						'</head>' +
						'<body id="mtview-iframe-sandbox" class="' + bodyClasses + '">' +
							body +
						'</body>' +
					'</html>'
				);
		...
		setLoader: function( dashicon ) {
			this.setContent(
				'<div class="loading-placeholder">' +
					...
					'<div class="mtview-loading"><ins></ins></div>' +
				'</div>'
			);
		},
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-mtview-text', encodeURIComponent( text ) );
					mt.mce.views.createInstance( type, text, match.options, force ).render();
...
} )( window, window.mt, window.mt.shortcode, window.jQuery );

/*
 * The paCMec core TinyMCE views.
...
( function( window, views, media, $ ) {
	...
	av = _.extend( {}, base, {
		...
		initialize: function() {
			...
			this.getEditors( function( editor ) {
				editor.on( 'mtview-selected', function() {
					self.pausePlayers();
				} );
			} );
		},
		...
		pausePlayers: function() {
			this.getNodes( function( editor, node, content ) {
				var win = $( 'iframe.mtview-sandbox', content ).get( 0 );
...
} )( window, window.mt.mce.views, window.mt.media, window.jQuery );
