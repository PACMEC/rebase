this["mt"] = this["mt"] || {}; this["mt"]["richText"] =
...
(function() { module.exports = window["mt"]["escapeHtml"]; }());
...
function toFormat(_ref) {
  ...
  if (attributes && attributes.class) {
    formatType = Object(external_mt_data_["select"])('core/rich-text').getFormatTypeForClassName(attributes.class);
  ...
  if (!formatType) {
    formatType = Object(external_mt_data_["select"])('core/rich-text').getFormatTypeForBareElement(type);
  }
 ...
 * @param {MTFormat} settings             Format settings.
 *
 * @return {MTFormat|undefined} The format, if it has been successfully
...
function registerFormatType(name, settings) {
  ...
  if (Object(external_mt_data_["select"])(store).getFormatType(settings.name)) {
  ...
  if (settings.className === null) {
    var formatTypeForBareElement = Object(external_mt_data_["select"])(store).getFormatTypeForBareElement(settings.tagName);
    ...
    var formatTypeForClassName = Object(external_mt_data_["select"])(store).getFormatTypeForClassName(settings.className);
  ...
  Object(external_mt_data_["dispatch"])(store).addFormatTypes(settings);
  return settings;
}
...
function createElementHTML(_ref6) {
  ...
  for (var key in attributes) {
    if (!Object(external_mt_escapeHtml_["isValidAttributeName"])(key)) {
      continue;
    }

    attributeString += " ".concat(key, "=\"").concat(Object(external_mt_escapeHtml_["escapeAttribute"])(attributes[key]), "\"");
  }
...
function createChildrenHTML() {
  ...
  return children.map(function (child) {
    return child.text === undefined ? createElementHTML(child) : Object(external_mt_escapeHtml_["escapeEditableHTML"])(child.text);
  }).join('');
}
...
function unregisterFormatType(name) {
  var oldFormat = Object(external_mt_data_["select"])(store).getFormatType(name);
  ...
  Object(external_mt_data_["dispatch"])(store).removeFormatTypes(name);
  return oldFormat;
}
...
function FormatEdit(_ref) {
  ...
  return formatTypes.map(function (settings) {
    ...
    return Object(external_mt_element_["createElement"])(Edit, {
      key: name,
      isActive: isActive,
...
function useFormatTypes(_ref) {
  ...
  var allFormatTypes = Object(external_mt_data_["useSelect"])(formatTypesSelector, []);
  var formatTypes = Object(external_mt_element_["useMemo"])(function () {
  ...
  var keyedSelected = Object(external_mt_data_["useSelect"])(function (select) {
  ...
  var dispatch = Object(external_mt_data_["useDispatch"])();
  var prepareHandlers = [];
  var valueHandlers = [];
  var changeHandlers = [];
  var dependencies = [];
...
function RichText(_ref, ref) {
  ...
  var _value = Object(external_mt_element_["useRef"])(value);

  var record = Object(external_mt_element_["useRef"])(Object(external_mt_element_["useMemo"])(function () {
  ...
  function handlePaste(event) {
    ...
    if (onPaste) {
      var files = Object(external_mt_dom_["getFilesFromDataTransfer"])(clipboardData);
      onPaste({
   ...
   *
   * @param {MTSyntheticEvent} event A synthetic keyboard event.
   */
  ...
  function handleDelete(event) {
    ...
    if (keyCode !== external_mt_keycodes_["DELETE"] && keyCode !== external_mt_keycodes_["BACKSPACE"] && keyCode !== external_mt_keycodes_["ESCAPE"]) {
      return;
    }
    ...
    if (keyCode === external_mt_keycodes_["ESCAPE"]) {
      return;
    }
    ...
    var isReverse = keyCode === external_mt_keycodes_["BACKSPACE"]; // Always handle full content deletion ourselves.
  ...
  function handleSpace(event) {
    ...
    shiftKey || altKey || metaKey || ctrlKey || keyCode !== external_mt_keycodes_["SPACE"] || multilineTag !== 'li') {
      return;
    }
  ...
  function handleHorizontalNavigation(event) {
    ...
    shiftKey || altKey || metaKey || ctrlKey || keyCode !== external_mt_keycodes_["LEFT"] && keyCode !== external_mt_keycodes_["RIGHT"]) {
      return;
    }
    ...
    var reverseKey = direction === 'rtl' ? external_mt_keycodes_["RIGHT"] : external_mt_keycodes_["LEFT"];
  ...
  var lastHistoryValue = Object(external_mt_element_["useRef"])(value);
  ...
  var isComposing = Object(external_mt_element_["useRef"])(false);
  var timeout = Object(external_mt_element_["useRef"])();
  /**
   * Handle input on the next selection change event.
   *
   * @param {MTSyntheticEvent} event Synthetic input event.
   */
  ...
  var didMount = Object(external_mt_element_["useRef"])(false);
  /**
   * Syncs the selection to local state. A callback for the `selectionchange`
   * native events, `keyup`, `mouseup` and `touchend` synthetic events, and
   * animation frames after the `focus` event.
   *
   * @param {Event|MTSyntheticEvent|DOMHighResTimeStamp} event
   */
   ...
   *
   * @param {MTSyntheticEvent} event Synthetic mousedown or touchstart event.
   */
  ...
  var rafId = Object(external_mt_element_["useRef"])();
  /**
  ...
  function handleFocus() {
    ...
    if (setFocusedElement) {
      external_mt_deprecated_default()('mt.blockEditor.RichText setFocusedElement prop', {
  ...
  return Object(external_mt_element_["createElement"])(external_mt_element_["Fragment"], null, isSelected && Object(external_mt_element_["createElement"])(FormatEdit, {
  ...
  }), !children && Object(external_mt_element_["createElement"])(TagName, editableProps));
}
/**
