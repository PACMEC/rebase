this["mt"] = this["mt"] || {}; this["mt"]["apiFetch"] =
...
function createNonceMiddleware(nonce) {
  function middleware(options, next) {
    var _options$headers = options.headers,
        headers = _options$headers === void 0 ? {} : _options$headers; // If an 'X-MT-Nonce' header (or any case-insensitive variation
    // thereof) was specified, no need to add a nonce header.

    for (var headerName in headers) {
      if (headerName.toLowerCase() === 'x-mt-nonce' && headers[headerName] === middleware.nonce) {
        return next(options);
      }
    }
...
var fetch_all_middleware_modifyQuery = function modifyQuery(_ref, queryArgs) {
  ...
  return fetch_all_middleware_objectSpread(fetch_all_middleware_objectSpread({}, options), {}, {
    url: url && Object(external_mt_url_["addQueryArgs"])(url, queryArgs),
    path: path && Object(external_mt_url_["addQueryArgs"])(path, queryArgs)
  });
...
function userLocaleMiddleware(options, next) {
  if (typeof options.url === 'string' && !Object(external_mt_url_["hasQueryArg"])(options.url, '_locale')) {
    options.url = Object(external_mt_url_["addQueryArgs"])(options.url, {
      _locale: 'user'
    });
  }

  if (typeof options.path === 'string' && !Object(external_mt_url_["hasQueryArg"])(options.path, '_locale')) {
    options.path = Object(external_mt_url_["addQueryArgs"])(options.path, {
      _locale: 'user'
    });
  }
...
var response_parseJsonAndNormalizeError = function parseJsonAndNormalizeError(response) {
  var invalidJsonError = {
    code: 'invalid_json',
    message: Object(external_mt_i18n_["__"])('The response is not a valid JSON response.')
  };
...
function parseAndThrowError(response) {
  ...
  return response_parseJsonAndNormalizeError(response).then(function (error) {
    var unknownError = {
      code: 'unknown_error',
      message: Object(external_mt_i18n_["__"])('An unknown error occurred.')
    };
...
function mediaUploadMiddleware(options, next) {
  var isMediaUploadRequest = options.path && options.path.indexOf('/mt/v2/media') !== -1 || options.url && options.url.indexOf('/mt/v2/media') !== -1;
  ...
  var postProcess = function postProcess(attachmentId) {
    retries++;
    return next({
      path: "/mt/v2/media/".concat(attachmentId, "/post-process"),
      method: 'POST',
      data: {
      ...
      next({
        path: "/mt/v2/media/".concat(attachmentId, "?force=true"),
        method: 'DELETE'
      });
  ...
  return next(media_upload_objectSpread(media_upload_objectSpread({}, options), {}, {
    ...
    var attachmentId = response.headers.get('x-mt-upload-attachment-id');
    ...
    if (response.status >= 500 && response.status < 600 && attachmentId) {
      return postProcess(attachmentId).catch(function () {
        if (options.parse !== false) {
          return Promise.reject({
            code: 'post_process',
            message: Object(external_mt_i18n_["__"])('Media upload failed. If this is a photo or a large image, please scale it down and try again.')
          });
        }
...
var build_module_defaultFetchHandler = function defaultFetchHandler(nextOptions) {
  ...
  .then(function (value) {
    ...
    throw {
      code: 'fetch_error',
      message: Object(external_mt_i18n_["__"])('You are probably offline.')
    };
  });
};
