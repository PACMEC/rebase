/**
 * @output mt-includes/js/mt-custom-header.js
 */
...
(function( window, settings ) {
			...
			if ( this.supportsVideo() ) {
				for ( var id in this.handlers ) {
					...
					if ( 'test' in handler && handler.test( settings ) ) {
						...
						trigger( document, 'mt-custom-header-video-loaded' );
						break;
					}
				}
			}
	...
	BaseHandler.prototype = {
		...
		initialize: function( settings ) {
			...
			this.container = document.getElementById( 'mt-custom-header' );
			this.button = button;

			button.setAttribute( 'type', 'button' );
			button.setAttribute( 'id', 'mt-custom-header-video-button' );
			button.setAttribute( 'class', 'mt-custom-header-video-button mt-custom-header-video-play' );
			...
			this.container.addEventListener( 'play', function() {
				button.className = 'mt-custom-header-video-button mt-custom-header-video-play';
				button.innerHTML = settings.l10n.pause;
				if ( 'a11y' in window.mt ) {
					window.mt.a11y.speak( settings.l10n.playSpeak);
				}
			});

			this.container.addEventListener( 'pause', function() {
				button.className = 'mt-custom-header-video-button mt-custom-header-video-pause';
				button.innerHTML = settings.l10n.play;
				if ( 'a11y' in window.mt ) {
					window.mt.a11y.speak( settings.l10n.pauseSpeak);
				}
			});
	 ...
	 * @class
	 */
	NativeHandler = BaseHandler.extend(/** @lends mt.NativeHandler.prototype */{
		/**
		...
		ready: function() {
			...
			video.id = 'mt-custom-header-video';
	 ...
	 * @memberOf mt
	 *
	 * @class mt.YouTubeHandler
	 */
	YouTubeHandler = BaseHandler.extend(/** @lends mt.YouTubeHandler.prototype */{
		/**
	...
	window.mt.customHeader = new CustomHeader();
	document.addEventListener( 'DOMContentLoaded', window.mt.customHeader.initialize.bind( window.mt.customHeader ), false );

	// Selective refresh support in the Customizer.
	if ( 'customize' in window.mt ) {
		window.mt.customize.selectiveRefresh.bind( 'render-partials-response', function( response ) {
			if ( 'custom_header_settings' in response ) {
				settings = response.custom_header_settings;
			}
		});

		window.mt.customize.selectiveRefresh.bind( 'partial-content-rendered', function( placement ) {
			if ( 'custom_header' === placement.partial.id ) {
				window.mt.customHeader.initialize();
			}
		});
	}

})( window, window._mtCustomHeaderSettings || {} );
