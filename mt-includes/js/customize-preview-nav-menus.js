/**
 * @output mt-includes/js/customize-preview-nav-menus.js
 */

/* global _mtCustomizePreviewNavMenusExports */

/** @namespace mt.customize.navMenusPreview */
mt.customize.navMenusPreview = mt.customize.MenusCustomizerPreview = ( function( $, _, mt, api ) {
	'use strict';
	...
	if ( 'undefined' !== typeof _mtCustomizePreviewNavMenusExports ) {
		_.extend( self.data, _mtCustomizePreviewNavMenusExports );
	}
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			api.bind( 'add', function( setting ) {
				 ...
				 * being exported to the frontend by the _is_valid_nav_menu_item filter in mt_get_nav_menu_items(),
			...
			/*
			 * Ensure that mt_nav_menu() instances nested inside of other partials
	...
	if ( api.selectiveRefresh ) {
		...
		/**
		 * Partial representing an invocation of mt_nav_menu().
		 *
		 * @memberOf mt.customize.navMenusPreview
		 * @alias mt.customize.navMenusPreview.NavMenuInstancePartial
		 *
		 * @class
		 * @augments mt.customize.selectiveRefresh.Partial
		 * @since 4.5.0
		 */
		self.NavMenuInstancePartial = api.selectiveRefresh.Partial.extend(/** @lends mt.customize.navMenusPreview.NavMenuInstancePartial.prototype */{
			 ...
			 *
			 * @since 4.5.0
			 * @param {mt.customize.Value|string} setting  - Object or ID.
			 ...
			 *
			 * @inheritdoc
			 * @param {mt.customize.selectiveRefresh.Placement} placement
			 */
			renderContent: function( placement ) {
				...
				if ( api.selectiveRefresh.Partial.prototype.renderContent.call( partial, placement ) ) {
					...
					$( document ).trigger( 'customize-preview-menu-refreshed', [ {
						...
						mtNavArgs: placement.context, // @deprecated
						mtNavMenuArgs: placement.context,
		 ...
		 * @since 4.5.0
		 *
		 * @param {mt.customize.Value} setting
		 * @param {Object}             [options]
		...
		self.onChangeNavMenuLocationsSetting = function() {
			...
			// If there are no mt_nav_menu() instances that refer to the theme location, do full refresh.
