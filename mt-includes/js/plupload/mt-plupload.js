/* global pluploadL10n, plupload, _mtPluploadSettings */
...
( function( exports, $ ) {
	...
	if ( typeof _mtPluploadSettings === 'undefined' ) {
		return;
	}
	 ...
	 * paCMec back end and the paCMec media experience.
	 *
	 * @class
	 * @memberOf mt
	 * @alias mt.Uploader
	...
	Uploader = function( options ) {
		...
		for ( key in elements ) {
			...
			if ( ! this[ key ].prop('id') ) {
				this[ key ].prop( 'id', '__mt-uploader-id-' + Uploader.uuid++ );
			}
		...
		tryAgain = function( message, data, file ) {
			...
			id = data.responseHeaders.match( /x-mt-upload-attachment-id:\s*(\d+)/i );
			...
			if ( times && times > 4 ) {
				...
				$.ajax({
					...
					data: {
						...
						_mtnonce: _mtPluploadSettings.defaults.multipart_params._mtnonce,
						attachment_id: id,
		 ...
		 * and display errors. @see mt.Uploader.errors.
		 *
		 * @param {string}        message Error message.
		 * @param {object}        data    Error data from Plupload.
		 * @param {plupload.File} file    File that was uploaded.
		...
		fileUploaded = function( up, file, response ) {
			...
			mt.media.model.Attachment.get( response.data.id, file.attachment );
		...
		this.uploader.bind( 'init', function( uploader ) {
			...
			if ( ! dragdrop ) {
				return dropzone.unbind('.mt-uploader');
			}

			// 'dragenter' doesn't fire correctly, simulate it with a limited 'dragover'.
			dropzone.on( 'dragover.mt-uploader', function() {
				if ( timer ) {
					clearTimeout( timer );
				}
			...
			dropzone.on('dragleave.mt-uploader, drop.mt-uploader', function() {
				/*
		...
		this.uploader.bind( 'FilesAdded', function( up, files ) {
			_.each( files, function( file ) {
				...
				attributes = _.extend({
					...
					filename:  file.name,
					menuOrder: 0,
					uploadedTo: mt.media.model.settings.post.id
				...
				file.attachment = mt.media.model.Attachment.create( attributes );
	...
	$.extend( Uploader, _mtPluploadSettings );
	...
	$.extend( Uploader.prototype, /** @lends mt.Uploader.prototype */{
		/**
		...
		/**
		 * Make a few internal event callbacks available on the mt.Uploader object
		...
		refresh:  function() {
			...
			if ( this.browser ) {
				 ...
				 */
				if ( ! attached ) {
					id = 'mt-uploader-browser-' + this.uploader.id;

					container = $( '#' + id );
					if ( ! container.length ) {
						container = $('<div class="mt-uploader-browser" />').css({
							position: 'fixed',
							top: '-1000px',
							left: '-1000px',
							height: 0,
							width: 0
						}).attr( 'id', 'mt-uploader-browser-' + this.uploader.id ).appendTo('body');
					}
	...
	Uploader.queue = new mt.media.model.Attachments( [], { query: false });
