/**
 * @output mt-includes/js/admin-bar.js
 */
/**
...
( function( document, window, navigator ) {
	document.addEventListener( 'DOMContentLoaded', function() {
		var adminBar = document.getElementById( 'mtadminbar' ),
			topMenuItems,
			allMenuItems,
			adminBarLogout,
			adminBarSearchForm,
		...
		adminBarLogout = document.getElementById( 'mt-admin-bar-logout' );
		adminBarSearchForm = document.getElementById( 'adminbarsearch' );
		shortlink = document.getElementById( 'mt-admin-bar-get-shortlink' );
	...
	function scrollToTop( event ) {
		// Only scroll when clicking on the mtadminbar, not on menus or submenus.
		if (
			event.target &&
			event.target.id !== 'mtadminbar' &&
			event.target.id !== 'mt-admin-bar-top-secondary'
		) {
			return;
		}
