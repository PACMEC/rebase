/**
 * @output mt-includes/js/mt-pointer.js
 */

/**
 * Initializes the mt-pointer widget using jQuery UI Widget Factory.
 */
(function($){
	var identifier = 0,
		zindex = 9999;

	$.widget('mt.pointer',/** @lends $.widget.mt.pointer.prototype */{
		options: {
			pointerClass: 'mt-pointer',
			pointerWidth: 320,
			content: function() {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var button = $('<a class="close" href="#"></a>').text( mt.i18n.__( 'Dismiss' ) );
		...
		/**
		 * A class that represents a paCMec pointer.
		 *
		 * @since 3.3.0
		 * @private
		 *
		 * @constructs $.widget.mt.pointer
		 */
		_create: function() {
			var positioning,
				family;

			this.content = $('<div class="mt-pointer-content"></div>');
			this.arrow   = $('<div class="mt-pointer-arrow"><div class="mt-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="mt-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /mt-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'mt-pointer-' + edge );
		},
