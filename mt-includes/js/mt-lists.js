/**
 * @output mt-includes/js/mt-lists.js
 */

/* global ajaxurl, mtAjax */
...
mtList = {
	 ...
	 * 4. `_mtnonce` value in element's href attribute.
	 * 5. `_mtnonce` input field that is a descendant of element.
	...
	nonce: function( element, settings ) {
		var url      = mtAjax.unserialize( element.attr( 'href' ) ),
			$element = $( '#' + settings.element );

		return settings.nonce || url._ajax_nonce || $element.find( 'input[name="_ajax_nonce"]' ).val() || url._mtnonce || $element.find( 'input[name="_mtnonce"]' ).val() || 0;
	},

	/**
	 * Extract list item data from a DOM element.
	 *
	 * Example 1: data-mt-lists="delete:the-comment-list:comment-{comment_ID}:66cc66:unspam=1"
	 * Example 2: data-mt-lists="dim:the-comment-list:comment-{comment_ID}:unapproved:e7e7d3:e7e7d3:new=approved"
	 *
	...
	parseData: function( element, type ) {
		var data = [], mtListsData;

		try {
			mtListsData = $( element ).data( 'mt-lists' ) || '';
			mtListsData = mtListsData.match( new RegExp( type + ':[\\S]+' ) );

			if ( mtListsData ) {
				data = mtListsData[0].split( ':' );
			}
		} catch ( error ) {}
	...
	pre: function( list, settings, action ) {
		...
		settings = $.extend( {}, this.mtList.settings, {
			element: null,
			nonce:   0,
			target:  list.get( 0 )
		}, settings || {} );
	...
	ajaxAdd: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mtList.parseData( $element, 'add' ),
			formValues, formData, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = mtList.pre.call( list, $element, settings, 'add' );
		...
		if ( ! $element.is( '[id="' + settings.element + '-submit"]' ) ) {
			return ! mtList.add.call( list, $element, settings );
		}
		...
		settings.nonce  = mtList.nonce( $element, settings );

		if ( ! mtAjax.validateForm( '#' + settings.element ) ) {
			return false;
		}
		...
		}, mtAjax.unserialize( data[4] || '' ) ) );

		formValues = $( '#' + settings.element + ' :input' ).not( '[name="_ajax_nonce"], [name="_mtnonce"], [name="action"]' );
		...
		settings.success = function( response ) {
			parsedResponse   = mtAjax.parseAjaxResponse( response, settings.response, settings.element );
			...
			$.each( parsedResponse.responses, function() {
				mtList.add.call( list, this.data, $.extend( {}, settings, { // this.firstChild.nodevalue
			...
			list.mtList.recolor();
			$( list ).trigger( 'mtListAddEnd', [ settings, list.mtList ] );
			mtList.clear.call( list, '#' + settings.element );
		};
	...
	ajaxDel: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mtList.parseData( $element, 'delete' ),
			$eventTarget, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = mtList.pre.call( list, $element, settings, 'delete' );
		...
		if ( 'none' !== settings.delColor ) {
			$eventTarget.css( 'backgroundColor', settings.delColor ).fadeOut( 350, function() {
				list.mtList.recolor();
				$( list ).trigger( 'mtListDelEnd', [ settings, list.mtList ] );
			} );
		} else {
			list.mtList.recolor();
			$( list ).trigger( 'mtListDelEnd', [ settings, list.mtList ] );
	...
	ajaxDim: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mtList.parseData( $element, 'dim' ),
		...
		settings = settings || {};
		settings = mtList.pre.call( list, $element, settings, 'dim' );
		...
		color        = mtList.getColor( $eventTarget );
			...
			$eventTarget
				...
				.animate( { backgroundColor: color }, {
					complete: function() {
						...
						$( list ).trigger( 'mtListDimEnd', [ settings, list.mtList ] );
					}
				} );
		} else {
			$( list ).trigger( 'mtListDimEnd', [ settings, list.mtList ] );
	...
	add: function( element, settings ) {
		...
		settings = $.extend( { position: 0, id: 0, oldId: null }, this.mtList.settings, settings );
		...
		if ( 'none' !== settings.addColor ) {
			$element.css( 'backgroundColor', settings.addColor ).animate( { backgroundColor: mtList.getColor( $element ) }, {
				complete: function() {
					$( this ).css( 'backgroundColor', '' );
				}
			} );
	...
	clear: function( elementId ) {
		...
		if ( list.mtList && $element.parents( '#' + list.id ).length ) {
			return;
		}
	...
	process: function( elementId ) {
		...
		$element.on( 'submit', 'form[data-mt-lists^="add:' + list.id + ':"]', function() {
			return list.mtList.add( this );
		} );

		$element.on( 'click', 'a[data-mt-lists^="add:' + list.id + ':"], input[data-mt-lists^="add:' + list.id + ':"]', function() {
			return list.mtList.add( this );
		} );

		$element.on( 'click', '[data-mt-lists^="delete:' + list.id + ':"]', function() {
			return list.mtList.del( this );
		} );

		$element.on( 'click', '[data-mt-lists^="dim:' + list.id + ':"]', function() {
			return list.mtList.dim( this );
		} );
	},
	...
	recolor: function() {
		...
		if ( list.mtList.settings.altOffset % 2 ) {
			evenOdd.reverse();
		}

		items.filter( evenOdd[0] ).addClass( list.mtList.settings.alt ).end();
		items.filter( evenOdd[1] ).removeClass( list.mtList.settings.alt );
	},
	...
	init: function() {
		...
		$list.mtList.process = function( element ) {
			$list.each( function() {
				this.mtList.process( element );
			} );
		};

		$list.mtList.recolor = function() {
			$list.each( function() {
				this.mtList.recolor();
			} );
		};
 ...
 *
 * @param {mtList~confirm}   settings.confirm     Callback that's run before a request is made. Default: null.
 * @param {mtList~addBefore} settings.addBefore   Callback that's run before an item gets added to the list.
 *                                                Default: null.
 * @param {mtList~addAfter}  settings.addAfter    Callback that's run after an item got added to the list.
 *                                                Default: null.
 * @param {mtList~delBefore} settings.delBefore   Callback that's run before an item gets deleted from the list.
 *                                                Default: null.
 * @param {mtList~delAfter}  settings.delAfter    Callback that's run after an item got deleted from the list.
 *                                                Default: null.
 * @param {mtList~dimBefore} settings.dimBefore   Callback that's run before an item gets dim'd. Default: null.
 * @param {mtList~dimAfter}  settings.dimAfter    Callback that's run after an item got dim'd. Default: null.
 * @return {$.fn} mtList API function.
 */
$.fn.mtList = function( settings ) {
	this.each( function( index, list ) {
		list.mtList = {
			settings: $.extend( {}, mtList.settings, { what: mtList.parseData( list, 'list' )[1] || '' }, settings )
		};

		$.each( functions, function( func, callback ) {
			list.mtList[func] = function( element, setting ) {
				return mtList[callback].call( list, element, setting );
			};
		} );
	} );

	mtList.init.call( this );
	this.mtList.process();
