...
( function( tinymce ) {
// Set the minimum value for the modals z-index higher than #mtadminbar (100000).
if ( ! tinymce.ui.FloatPanel.zIndex || tinymce.ui.FloatPanel.zIndex < 100100 ) {
	tinymce.ui.FloatPanel.zIndex = 100100;
}

tinymce.PluginManager.add( 'managertechnology', function( editor ) {
	var mtAdvButton, style,
		DOM = tinymce.DOM,
		each = tinymce.each,
		__ = editor.editorManager.i18n.translate,
		$ = window.jQuery,
		mt = window.mt,
		hasWpautop = ( mt && mt.editor && mt.editor.autop && editor.getParam( 'mtautop', true ) ),
		mtTooltips = false;
	...
	function toggleToolbars( state ) {
		...
		if ( ! initial ) {
			if ( state === 'hide' ) {
				...
				mtAdvButton && mtAdvButton.active( false );
			} else {
				setUserSetting( 'hidetb', '1' );
				mtAdvButton && mtAdvButton.active( true );
			}
		}

		editor.fire( 'mt-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'mt_adv', {
		tooltip: 'Toolbar Toggle',
		cmd: 'MT_Adv',
		onPostRender: function() {
			mtAdvButton = this;
			mtAdvButton.active( getUserSetting( 'hidetb' ) === '1' );
		}
	});

	// Hide the toolbars after loading.
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'managertechnology_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		} else {
	...
	editor.addCommand( 'MT_Adv', function() {
		toggleToolbars();
	});

	editor.on( 'focus', function() {
        window.mtActiveEditor = editor.id;
    });
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-mt-more="more" data-mt-more-text="' + moretext + '" ' +
						'class="mt-more-tag mce-mt-more" alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-mt-more="nextpage" class="mt-more-tag mce-mt-nextpage" ' +
						'alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />' );
			}

			if ( event.load && event.format !== 'raw' ) {
				if ( hasWpautop ) {
					event.content = mt.editor.autop( event.content );
				} else {
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-mt-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-mt-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-mt-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-mt-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-mt-preserve' ) !== -1 ) {
					if ( match = image.match( / data-mt-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-mt-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands.
	editor.addCommand( 'MT_More', function( tag ) {
		var parent, html, title,
			classname = 'mt-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode(),
			rootNode = editor.getBody();

		tag = tag || 'more';
		classname += ' mce-mt-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" alt="" title="' + title + '" class="' + classname + '" ' +
			'data-mt-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'MT_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'MT_Page', function() {
		editor.execCommand( 'MT_More', 'nextpage' );
	});

	editor.addCommand( 'MT_Help', function() {
		var access = tinymce.Env.mac ? __( 'Ctrl + Alt + letter:' ) : __( 'Shift + Alt + letter:' ),
			...
			i1 = 0,
			i2 = 0,
			labels = editor.settings.mt_shortcut_labels,
		...
		if ( editor.plugins.mttextpattern && ( ! tinymce.Env.ie || tinymce.Env.ie > 8 ) ) {
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-mt-help' );
	...
	editor.addCommand( 'MT_Medialib', function() {
		if ( mt && mt.media && mt.media.editor ) {
			mt.media.editor.open( editor.id );
		}
	});

	// Register buttons.
	editor.addButton( 'mt_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'MT_More', 'more' );
		}
	});

	editor.addButton( 'mt_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'MT_More', 'nextpage' );
		}
	...
	editor.addButton( 'mt_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'MT_Help'
	});

	editor.addButton( 'mt_code', {
		tooltip: 'Code',
		cmd: 'MT_Code',
		stateSelector: 'code'
	});
	...
	editor.on( 'init', function() {
		...
		dom.setAttrib( doc.documentElement, 'lang', editor.getParam( 'mt_lang_attr' ) );
		...
		if ( editor.getParam( 'mt_paste_filters', true ) ) {
	...
	editor.on( 'SaveContent', function( event ) {
		...
		if ( hasWpautop ) {
			event.content = mt.editor.removep( event.content );
		} else {
			// Restore formatting of block boundaries.
			event.content = event.content.replace( /-->\s*<!-- mt:/g, '-->\n\n<!-- mt:' );
		}
	});
	...
	function getTooltip( tooltip ) {
		...
		if ( mtTooltips.hasOwnProperty( translated ) ) {
			label = mtTooltips[ translated ];
		} else if ( mtTooltips.hasOwnProperty( tooltip ) ) {
			label = mtTooltips[ tooltip ];
		}
		...
		each( editor.theme.panel.find( 'listbox' ), function( listbox ) {
			if ( listbox && listbox.settings.text === 'Paragraph' ) {
				each( listbox.settings.values, function( item ) {
					if ( item.text && mtTooltips.hasOwnProperty( item.text ) ) {
						item.shortcut = '(' + mtTooltips[ item.text ] + ')';
					}
				} );
			}
		} );
	...
	editor.on( 'preinit', function() {
		var Factory = tinymce.ui.Factory,
			...
			mtAdminbar = document.getElementById( 'mtadminbar' ),
			...
			if ( editor.id === 'content' ) {
				mtStatusbar = document.getElementById( 'post-status-info' );
			}
		...
		function create( buttons, bottom ) {
			...
			function reposition() {
				...
				var scrollX = window.pageXOffset || document.documentElement.scrollLeft,
					...
					mtAdminbarBottom = mtAdminbar ? mtAdminbar.getBoundingClientRect().bottom : 0,
					...
					mtStatusbarTop = mtStatusbar ? windowHeight - mtStatusbar.getBoundingClientRect().top : 0,
					blockedTop = Math.max( 0, mtAdminbarBottom, mceToolbarBottom, iframeRect.top ),
					blockedBottom = Math.max( 0, mceStatusbarTop, mtStatusbarTop, windowHeight - iframeRect.bottom ),
		...
		editor.mt = editor.mt || {};
		editor.mt._createToolbar = create;
	}, true );
