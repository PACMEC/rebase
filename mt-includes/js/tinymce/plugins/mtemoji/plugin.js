( function( tinymce ) {
	tinymce.PluginManager.add( 'mtemoji', function( editor ) {
		var typing,
			mt = window.mt,
			settings = window._mtemojiSettings,
			env = tinymce.Env,
		...
		if ( ! mt || ! mt.emoji || settings.supports.everything ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-mt-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			mt.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( window.twemoji && window.twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after mt.emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-mt-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-mt-emoji' ) ) {
