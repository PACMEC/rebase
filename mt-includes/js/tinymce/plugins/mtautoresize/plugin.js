...
// Forked for paCMec so it can be turned on/off after loading.
 ...
 * it's initialized.
 */
tinymce.PluginManager.add( 'mtautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'mt-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'mt-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'mt-autoresize' );
	...
	function off() {
		...
		if ( ! settings.mt_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'mt-autoresize' );
	...
	editor.addCommand( 'mtAutoResize', resize );

	// On/off.
	editor.addCommand( 'mtAutoResizeOn', on );
	editor.addCommand( 'mtAutoResizeOff', off );
});
