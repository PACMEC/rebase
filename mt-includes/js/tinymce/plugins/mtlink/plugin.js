( function( tinymce ) {
	tinymce.ui.Factory.add( 'MTLinkPreview', tinymce.ui.Control.extend( {
		url: '#',
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="mt-link-preview">' +
	...
	tinymce.ui.Factory.add( 'MTLinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="mt-link-input">' +
	...
	tinymce.PluginManager.add( 'mtlink', function( editor ) {
		var toolbar;
		var editToolbar;
		var previewInstance;
		var inputInstance;
		...
		var speak = ( typeof window.mt !== 'undefined' && window.mt.a11y && window.mt.a11y.speak ) ? window.mt.a11y.speak : function() {};
		...
		function removePlaceholders() {
			editor.$( 'a' ).each( function( i, element ) {
				...
				if ( $element.attr( 'href' ) === '_mt_link_placeholder' ) {
					editor.dom.remove( element, true );
				} else if ( $element.attr( 'data-mtlink-edit' ) ) {
					$element.attr( 'data-mtlink-edit', null );
				}
			});
		}
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				if ( tag.indexOf( ' href="_mt_link_placeholder"' ) > -1 ) {
					return text;
				}

				if ( dataAttr ) {
					tag = tag.replace( / data-mtlink-edit="true"/g, '' );
				}

				tag = tag.replace( / data-mtlink-url-error="true"/g, '' );
		...
		editor.on( 'preinit', function() {
			if ( editor.mt && editor.mt._createToolbar ) {
				...
				if ( typeof window.mtLink !== 'undefined' ) {
					editButtons.push( 'mt_link_advanced' );
				}

				editToolbar = editor.mt._createToolbar( editButtons, true );

				editToolbar.on( 'show', function() {
					if ( typeof window.mtLink === 'undefined' || ! window.mtLink.modalOpen ) {
						window.setTimeout( function() {
							...
							if ( element ) {
								if ( ! element.value && selection && typeof window.mtLink !== 'undefined' ) {
									element.value = window.mtLink.getUrlFromSelection( selection );
								}
				...
				editToolbar.on( 'hide', function() {
					if ( ! editToolbar.scrolling ) {
						editor.execCommand( 'mt_link_cancel' );
					}
				} );
			}
		} );

		editor.addCommand( 'MT_Link', function() {
			if ( tinymce.Env.ie && tinymce.Env.ie < 10 && typeof window.mtLink !== 'undefined' ) {
				window.mtLink.open( editor.id );
				return;
			}
			...
			if ( ! linkNode ) {
				removePlaceholders();
				editor.execCommand( 'mceInsertLink', false, { href: '_mt_link_placeholder' } );

				linkNode = editor.$( 'a[href="_mt_link_placeholder"]' )[0];
				editor.nodeChanged();
			}

			editor.dom.setAttribs( linkNode, { 'data-mtlink-edit': true } );
		} );

		editor.addCommand( 'mt_link_apply', function() {
			if ( editToolbar.scrolling ) {
				return;
			}
			...
			if ( linkNode ) {
				...
				editor.dom.setAttribs( linkNode, { href: href, 'data-mtlink-edit': null } );
			...
			if ( typeof window.mtLinkL10n !== 'undefined' && ! hasLinkError ) {
				speak( window.mtLinkL10n.linkInserted );
			}
		} );

		editor.addCommand( 'mt_link_cancel', function() {
			inputInstance.reset();
		...
		editor.addCommand( 'mt_unlink', function() {
			editor.execCommand( 'unlink' );
			editToolbar.tempHide = false;
			editor.execCommand( 'mt_link_cancel' );
		} );

		// MT default shortcuts.
		editor.addShortcut( 'access+a', '', 'MT_Link' );
		editor.addShortcut( 'access+s', '', 'mt_unlink' );
		// The "de-facto standard" shortcut, see #27305.
		editor.addShortcut( 'meta+k', '', 'MT_Link' );
		...
		editor.addButton( 'mt_link_preview', {
			type: 'MTLinkPreview',
		...
		editor.addButton( 'mt_link_input', {
			type: 'MTLinkInput',
			onPostRender: function() {
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-mt-autocomplete-' + ui.item.ID );
							/*
						...
						select: function( event, ui ) {
							...
							if ( 9 === event.keyCode && typeof window.mtLinkL10n !== 'undefined' ) {
								// Audible confirmation message when a link has been selected.
								speak( window.mtLinkL10n.linkSelected );
							}
						...
						var fallbackTitle = ( typeof window.mtLinkL10n !== 'undefined' ) ? window.mtLinkL10n.noTitle : '',
							title = item.title ? item.title : fallbackTitle;

						return $( '<li role="option" id="mce-mt-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="mt-editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
				...
				tinymce.$( input ).on( 'keydown', function( event ) {
					if ( event.keyCode === 13 ) {
						editor.execCommand( 'mt_link_apply' );
						event.preventDefault();
					}
				} );
			}
		...
		editor.on( 'mttoolbar', function( event ) {
			...
			if ( linkNode ) {
				...
				edit = $linkNode.attr( 'data-mtlink-edit' );

				if ( href === '_mt_link_placeholder' || edit ) {
					if ( href !== '_mt_link_placeholder' && ! inputInstance.getURL() ) {
					...
					if ( $linkNode.attr( 'data-mtlink-url-error' ) === 'true' ) {
						toolbar.$el.find( '.mt-link-preview a' ).addClass( 'mtlink-url-error' );
					} else {
						toolbar.$el.find( '.mt-link-preview a' ).removeClass( 'mtlink-url-error' );
						hasLinkError = false;
					}
				}
		...
		editor.addButton( 'mt_link_edit', {
			tooltip: 'Edit|button', // '|button' is not displayed, only used for context.
			icon: 'dashicon dashicons-edit',
			cmd: 'MT_Link'
		} );

		editor.addButton( 'mt_link_remove', {
			tooltip: 'Remove link',
			icon: 'dashicon dashicons-editor-unlink',
			cmd: 'mt_unlink'
		} );

		editor.addButton( 'mt_link_advanced', {
			...
			onclick: function() {
				if ( typeof window.mtLink !== 'undefined' ) {
					...
					window.mtLink.open( editor.id, url, text );
		...
		editor.addButton( 'mt_link_apply', {
			tooltip: 'Apply',
