/**
 * @output mt-includes/js/customize-selective-refresh.js
 */

/* global jQuery, JSON, _customizePartialRefreshExports, console */

/** @namespace mt.customize.selectiveRefresh */
mt.customize.selectiveRefresh = ( function( $, api ) {
	'use strict';
	 ...
	 *
	 * @memberOf mt.customize.selectiveRefresh
	 *
	 * @see PHP class MT_Customize_Partial.
	 *
	 * @class
	 * @augments mt.customize.Class
	 * @since 4.5.0
	 */
	Partial = self.Partial = api.Class.extend(/** @lends mt.customize.SelectiveRefresh.Partial.prototype */{
		 ...
		 * @since 4.5.0
		 *
		 * @param {mt.customize.Value|string} setting  ID or object for setting.
		 ...
		 * @since 4.5.0
		 *
		 * @this {mt.customize.selectiveRefresh.Partial}
		...
		renderContent: function( placement ) {
			...
			try {
				...
				if ( mt.emoji && mt.emoji.parse && ! $.contains( document.head, placement.container[0] ) ) {
					content = mt.emoji.parse( content );
				}
	 ...
	 * @since 4.5.0
	 */
	self.Placement = Placement = api.Class.extend(/** @lends mt.customize.selectiveRefresh.prototype */{

		/**
		 * The partial with which the container is associated.
		 *
		 * @param {mt.customize.selectiveRefresh.Partial}
		 */
		partial: null,
	 ...
	 * @since 4.5.0
	 *
	 * @type {Object.<string, mt.customize.selectiveRefresh.Partial>}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @see mt.customize.previewer.query()
	 *
	 * @return {Object}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @type {Object<string, { deferred: jQuery.Promise, partial: mt.customize.selectiveRefresh.Partial }>}
	 */
	 ...
	 * @since 4.5.0
	 *
	 * @param {mt.customize.selectiveRefresh.Partial} partial
	 * @return {jQuery.Promise}
	 */
	self.requestPartial = function( partial ) {
		...
		self._debouncedTimeoutId = setTimeout(
			function() {
				...
				request = self._currentRequest = mt.ajax.send( null, {
					data: data,
	...
	api.bind( 'preview-ready', function() {
		 ...
		 *
		 * Note this is largely needed because adding a 'change' event handler to mt.customize
		 * will only include the changed setting object as an argument, not including the
		 * new value or the old value.
		 *
		 * @since 4.5.0
		 * @this {mt.customize.Setting}
		 *
		 ...
		 *
		 * @since 4.5.0
		 * @this {mt.customize.Values}
		 *
		 * @param {mt.customize.Setting} setting
		 */
