/**
 * @output mt-includes/js/mt-api.js
 */
...
(function( window, undefined ) {
	...
	/**
	 * Initialise the MT_API.
	 */
	function MT_API() {
		/** @namespace mt.api.models */
		this.models = {};
		/** @namespace mt.api.collections */
		this.collections = {};
		/** @namespace mt.api.views */
		this.views = {};
	}
	...
	window.mt            = window.mt || {};
	/** @namespace mt.api */
	mt.api               = mt.api || new MT_API();
	mt.api.versionString = mt.api.versionString || 'mt/v2/';
...
(function( window, undefined ) {
	...
	/** @namespace mt.api */
	mt.api = mt.api || {};
	/** @namespace mt.api.utils */
	mt.api.utils = mt.api.utils || {};
	 ...
	 */
	mt.api.getModelByRoute = function( route ) {
		return _.find( mt.api.models, function( model ) {
	 ...
	 */
	mt.api.getCollectionByRoute = function( route ) {
		return _.find( mt.api.collections, function( collection ) {
	 ...
	 * @param {Date} date.
	 */
	mt.api.utils.parseISO8601 = function( date ) {
	 ...
	 */
	mt.api.utils.getRootUrl = function() {
	 ...
	 */
	mt.api.utils.capitalize = function( str ) {
	 ...
	 */
	mt.api.utils.capitalizeAndCamelCaseDashes = function( str ) {
		if ( _.isUndefined( str ) ) {
			return str;
		}
		str = mt.api.utils.capitalize( str );

		return mt.api.utils.camelCaseDashes( str );
	};

	/**
	 * Helper function to camel case the letter after dashes, removing the dashes.
	 */
	mt.api.utils.camelCaseDashes = function( str ) {
	 ...
	 * @param {string}  [versionString] Version string, defaults to `mt.api.versionString`.
	 * @param {boolean} [reverse]       Whether to reverse the order when extracting the route part. Optional, default false.
	 */
	mt.api.utils.extractRoutePart = function( route, part, versionString, reverse ) {
		var routeParts;

		part = part || 1;
		versionString = versionString || mt.api.versionString;
	 ...
	 */
	mt.api.utils.extractParentName = function( route ) {
		var name,
	 ...
	 *                                to add the args to.
	 */
	mt.api.utils.decorateFromRoute = function( routeEndpoints, modelInstance ) {
	 ...
	 */
	mt.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. paCMec expects dates in UTC.
				 ...
				 *
				 * paCMec returns 'date' and 'date_modified' in the timezone of the server as well as
				...
				getDate: function( field ) {
					...
					return new Date( mt.api.utils.parseISO8601( theISODate ) );
				}
			},
			...
			buildModelGetter = function( parentModel, modelId, modelName, embedSourcePoint, embedCheckField ) {
				...
				getModel = new mt.api.models[ modelName ]( attributes );
			...
			buildCollectionGetter = function( parentModel, collectionName, embedSourcePoint, embedIndex ) {
				 ...
				 *
				 * @return {Deferred.promise} promise Resolves to a mt.api.collections[ collectionName ]
				 * collection.
				 */
				...
				getObjects = new mt.api.collections[ collectionName ]( properties, classProperties );
			...
			TagsMixin = {
				...
				getTags: function() {
					var tagIds = this.get( 'tags' ),
						tags  = new mt.api.collections.Tags();
				...
				setTags: function( tags ) {
					...
					if ( _.isArray( tags ) ) {
						...
						// Get all the tags.
						allTags = new mt.api.collections.Tags();
						allTags.fetch( {
							data:    { per_page: 100 },
							success: function( alltags ) {
								...
								_.each( tags, function( tag ) {
									newTag = new mt.api.models.Tag( alltags.findWhere( { slug: tag } ) );
								...
								tags = new mt.api.collections.Tags( newTags );
			...
			CategoriesMixin = {
				...
				getCategories: function() {
					var categoryIds = this.get( 'categories' ),
						categories  = new mt.api.collections.Categories();
				...
				setCategories: function( categories ) {
					...
					if ( _.isArray( categories ) ) {
						...
						allCategories = new mt.api.collections.Categories();
						allCategories.fetch( {
							data:    { per_page: 100 },
							success: function( allcats ) {
								...
								_.each( categories, function( category ) {
									newCategory = new mt.api.models.Category( allcats.findWhere( { slug: category } ) );
								...
								categories = new mt.api.collections.Categories( newCategories );
			...
			FeaturedMediaMixin = {
				getFeaturedMedia: function() {
					return buildModelGetter( this, this.get( 'featured_media' ), 'Media', 'mt:featuredmedia', 'source_url' );
				}
			};
...
(function() {
	...
	var mtApiSettings = window.mtApiSettings || {},
	trashableTypes    = [ 'Comment', 'Media', 'Comment', 'Post', 'Page', 'Status', 'Taxonomy', 'Type' ];

	/**
	 * Backbone base model for all models.
	 */
	mt.api.MTApiBaseModel = Backbone.Model.extend(
		/** @lends MTApiBaseModel.prototype  */
		{
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-MT-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-MT-Nonce' );
	 ...
	 */
	mt.api.models.Schema = mt.api.MTApiBaseModel.extend(
		/** @lends Schema.prototype  */
		{
			...
			initialize: function( attributes, options ) {
				...
				mt.api.MTApiBaseModel.prototype.initialize.call( model, attributes, options );

				model.apiRoot = options.apiRoot || mtApiSettings.root;
				model.versionString = options.versionString || mtApiSettings.versionString;
			},
...
( function() {
	 ...
	 */
	mt.api.MTApiBaseCollection = Backbone.Collection.extend(
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-mt-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-mt-total' ), 10 );
						}
...
( function() {
	...
	var Endpoint, initializedDeferreds = {},
		mtApiSettings = window.mtApiSettings || {};
	...
	// If mtApiSettings is unavailable, try the default.
	if ( _.isEmpty( mtApiSettings ) ) {
		mtApiSettings.root = window.location.origin + '/mt-json/';
	}

	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		defaults: {
			apiRoot: mtApiSettings.root,
			versionString: mt.api.versionString,
			nonce: null,
			schema: null,
			models: {},
			collections: {}
		...
		initialize: function() {
			...
			model.schemaModel = new mt.api.models.Schema( null, {
			...
			if ( model.get( 'schema' ) ) {
				...
				( _.isUndefined( mtApiSettings.cacheSchema ) || mtApiSettings.cacheSchema ) &&
				sessionStorage.getItem( 'mt-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'mt-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( mtApiSettings.cacheSchema ) || mtApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'mt-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
		...
		constructFromSchema: function() {
			 ...
			 *
			 * Localizing a variable mtApiSettings.mapping will over-ride the default mapping options.
			 *
			 */
			mapping = mtApiSettings.mapping || {
				models: {
					'Categories':      'Category',
					'Comments':        'Comment',
					'Pages':           'Page',
			...
			modelRoutes      = [];
			collectionRoutes = [];
			schemaRoot       = routeModel.get( 'apiRoot' ).replace( mt.api.utils.getRootUrl(), '' );
			loadingObjects   = {};
			...
			_.each( modelRoutes, function( modelRoute ) {
				...
				var modelClassName,
					routeName  = mt.api.utils.extractRoutePart( modelRoute.index, 2, routeModel.get( 'versionString' ), true ),
					parentName = mt.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), false ),
					routeEnd   = mt.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), true );
				...
				if ( '' !== parentName && parentName !== routeName ) {
					modelClassName = mt.api.utils.capitalizeAndCamelCaseDashes( parentName ) + mt.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = mt.api.MTApiBaseModel.extend( {
					...
					modelClassName = mt.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = mt.api.MTApiBaseModel.extend( {
				...
				mt.api.utils.decorateFromRoute(
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				var collectionClassName, modelClassName,
						...
						parentName = mt.api.utils.extractRoutePart( collectionRoute.index, 1, routeModel.get( 'versionString' ), false );

				// If the collection has a parent in its route, add that to its class name.
				if ( '' !== parentName && parentName !== routeName && routeModel.get( 'versionString' ) !== parentName ) {

					collectionClassName = mt.api.utils.capitalizeAndCamelCaseDashes( parentName ) + mt.api.utils.capitalizeAndCamelCaseDashes( routeName );
					...
					loadingObjects.collections[ collectionClassName ] = mt.api.MTApiBaseCollection.extend( {
					...
					collectionClassName = mt.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName      = mapping.models[ collectionClassName ] || collectionClassName;
					collectionClassName = mapping.collections[ collectionClassName ] || collectionClassName;
					loadingObjects.collections[ collectionClassName ] = mt.api.MTApiBaseCollection.extend( {
				...
				// Add defaults to the new model, pulled form the endpoint.
				mt.api.utils.decorateFromRoute( collectionRoute.route.endpoints, loadingObjects.collections[ collectionClassName ] );
			} );

			// Add mixins and helpers for each of the models.
			_.each( loadingObjects.models, function( model, index ) {
				loadingObjects.models[ index ] = mt.api.utils.addMixinsAndHelpers( model, index, loadingObjects );
			} );
	...
	mt.api.endpoints = new Backbone.Collection();

	/**
	 * Initialize the mt-api, optionally passing the API root.
	 *
	 * @param {Object} [args]
	 * @param {string} [args.nonce] The nonce. Optional, defaults to mtApiSettings.nonce.
	 * @param {string} [args.apiRoot] The api root. Optional, defaults to mtApiSettings.root.
	 * @param {string} [args.versionString] The version string. Optional, defaults to mtApiSettings.root.
	 * @param {Object} [args.schema] The schema. Optional, will be fetched from API if not provided.
	 */
	mt.api.init = function( args ) {
		var endpoint, attributes = {}, deferred, promise;

		args                      = args || {};
		attributes.nonce          = _.isString( args.nonce ) ? args.nonce : ( mtApiSettings.nonce || '' );
		attributes.apiRoot        = args.apiRoot || mtApiSettings.root || '/mt-json';
		attributes.versionString  = args.versionString || mtApiSettings.versionString || 'mt/v2/';
		attributes.schema         = args.schema || null;
		attributes.modelEndpoints = args.modelEndpoints || [ 'me', 'settings' ];
		if ( ! attributes.schema && attributes.apiRoot === mtApiSettings.root && attributes.versionString === mtApiSettings.versionString ) {
			attributes.schema = mtApiSettings.schema;
		}

		if ( ! initializedDeferreds[ attributes.apiRoot + attributes.versionString ] ) {

			// Look for an existing copy of this endpoint.
			endpoint = mt.api.endpoints.findWhere( { 'apiRoot': attributes.apiRoot, 'versionString': attributes.versionString } );
			if ( ! endpoint ) {
			...
			endpoint.schemaConstructed.done( function( resolvedEndpoint ) {
				mt.api.endpoints.add( resolvedEndpoint );

				// Map the default endpoints, extending any already present items (including Schema model).
				mt.api.models      = _.extend( mt.api.models, resolvedEndpoint.get( 'models' ) );
				mt.api.collections = _.extend( mt.api.collections, resolvedEndpoint.get( 'collections' ) );
	...
	// The mt.api.init function returns a promise that will resolve with the endpoint once it is ready.
	mt.api.loadPromise = mt.api.init();
