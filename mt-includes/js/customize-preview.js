 ...
 *
 * @output mt-includes/js/customize-preview.js
 */
(function( exports, $ ){
	 ...
	 */
	api.Preview = api.Messenger.extend(/** @lends mt.customize.Preview.prototype */{
		/**
		...
		handleLinkClick: function( event ) {
			...
			if ( ! api.isLinkPreviewable( link[0] ) ) {
				mt.a11y.speak( api.settings.l10n.linkUnpreviewable );
		...
		handleFormSubmit: function( event ) {
			...
			if ( 'GET' !== form.prop( 'method' ).toUpperCase() || ! api.isLinkPreviewable( urlParser ) ) {
				mt.a11y.speak( api.settings.l10n.formUnpreviewable );
				event.preventDefault();
				return;
			}
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		// Skip mt login and signup pages.
		if ( /\/mt-(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}

		// Allow links to admin ajax as faux frontend URLs.
		if ( /\/mt-admin\/admin-ajax\.php$/.test( element.pathname ) ) {
			return args.allowAdminAjax;
		}

		// Disallow links to admin, includes, and content.
		if ( /\/mt-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		if ( $element.closest( '#mtadminbar' ).length ) {
			return;
		}
		...
		// Ignore links with class mt-playlist-caption.
		if ( $element.hasClass( 'mt-playlist-caption' ) ) {
			return;
		}
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'mt-custom-logo', !! attachmentId );
		},
		...
		background: function() {
			 ...
			 *
			 * See get_body_class() in /mt-includes/post-template.php
			 */
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'mt-customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'mt-customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the mt-custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */
