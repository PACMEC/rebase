...
/**
 * @output mt-includes/js/media-views.js
 */
...
l10n = media.view.l10n = window._mtMediaViewsL10n || {};
...
_.extend( Region.prototype,/** @lends mt.media.controller.Region.prototype */{
	/**
	 * Activate a mode.
	 *
	 * @since 3.5.0
	 ...
	 *
	 * @return {mt.media.controller.Region} Returns itself to allow chaining.
	 */
	mode: function( mode ) {
		if ( ! mode ) {
			return this._mode;
		 ...
		 *
		 * @event mt.media.controller.Region#deactivate
		 */
		 ...
		 *
		 * @event mt.media.controller.Region#activate
		 */
	...
	render: function( mode ) {
		 ...
		 *
		 * @event mt.media.controller.Region#create
		 * @type {object}
		 ...
		 * Region view creation takes place in an event callback on the frame.
		 *
		 * @event mt.media.controller.Region#render
		 * @type {object}
		 */
	 ...
	 * @return {mt.Backbone.Subviews} Subviews is returned to allow chaining.
	 */
	 ...
	 *
	 * @param {string} event
	 * @return {undefined|mt.media.controller.Region} Returns itself to allow chaining.
	 */
...
/**
 * mt.media.controller.StateMachine
 *
...
_.extend( StateMachine.prototype, Backbone.Events,/** @lends mt.media.controller.StateMachine.prototype */{
	/**
	 * Fetch a state.
	 *
	 ...
	 *
	 * @param {string} id
	 * @return {mt.media.controller.State} Returns a State model from
	 ...
	 * @param {string} id
	 *
	 * @fires mt.media.controller.State#deactivate
	 * @fires mt.media.controller.State#activate
	 *
	 * @return {mt.media.controller.StateMachine} Returns itself to allow chaining.
	 */
...
_.each([ 'on', 'off', 'trigger' ], function( method ) {
	/**
	 * @function on
	 * @memberOf mt.media.controller.StateMachine
	 * @instance
	 * @return {mt.media.controller.StateMachine} Returns itself to allow chaining.
	 */
	/**
 ...
 * @augments Backbone.Model
 */
var State = Backbone.Model.extend(/** @lends mt.media.controller.State.prototype */{
	/**
	 * Constructor.
	 *
	 * @since 3.5.0
...
var l10n = mt.media.view.l10n,
	getUserSetting = window.getUserSetting,
	setUserSetting = window.setUserSetting,
	Library;

/**
 * mt.media.controller.Library
 *
 ...
 *
 * @class
 * @augments mt.media.controller.State
 * @augments Backbone.Model
 ...
 * @param {mt.media.model.Attachments}      [attributes.library]                 The attachments collection to browse.
 *                                                                               If one is not supplied, a collection of all attachments will be created.
 * @param {mt.media.model.Selection|object} [attributes.selection]               A collection to contain attachment selections within the state.
 ...
 */
Library = mt.media.controller.State.extend(/** @lends mt.media.controller.Library.prototype */{
	defaults: {
		id:                 'library',
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', mt.media.query() );
		}

		if ( ! ( selection instanceof mt.media.model.Selection ) ) {
			props = selection;
			...
			this.set( 'selection', new mt.media.model.Selection( null, {
	...
	activate: function() {
		...
		mt.Uploader.queue.on( 'add', this.uploading, this );
	...
	deactivate: function() {
		...
		mt.Uploader.queue.off( null, null, this );
	},
	...
	resetDisplays: function() {
		var defaultProps = mt.media.view.settings.defaultProps;
		this._displays = [];
	 ...
	 * @since 3.5.0
	 *
	 * @param {mt.media.model.Attachment} attachment
	...
	canEmbed: function( attachment ) {
		...
		return _.contains( mt.media.view.settings.embedExts, attachment.get('filename').split('.').pop() );
	},
...
_.extend( Library.prototype, mt.media.selectionSync );
...
var State = mt.media.controller.State,
	Library = mt.media.controller.Library,
	l10n = mt.media.view.l10n,
	ImageDetails;

/**
 * mt.media.controller.ImageDetails
 *
 ...
 * @param {mt.media.model.Attachment} attributes.image                   The image's model.
 ...
 */
ImageDetails = State.extend(/** @lends mt.media.controller.ImageDetails.prototype */{
	defaults: _.defaults({
		id:       'image-details',
...
var Library = mt.media.controller.Library,
	l10n = mt.media.view.l10n,
	GalleryEdit;

/**
 * mt.media.controller.GalleryEdit
 *
 ...
 *
 * @class
 * @augments mt.media.controller.Library
 * @augments mt.media.controller.State
 * @augments Backbone.Model
 *
 * @memberOf mt.media.controller
 ...
 * @param {mt.media.model.Attachments} [attributes.library]               The collection of attachments in the gallery.
 ...
 *                                                                        If none supplied, defaults to mt.media.view.Attachment.EditLibrary.
 */
GalleryEdit = Library.extend(/** @lends mt.media.controller.GalleryEdit.prototype */{
	defaults: {
		id:               'gallery-edit',
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', new mt.media.model.Selection() );
		}

		// The single `Attachment` view to be used in the `Attachments` view.
		if ( ! this.get('AttachmentView') ) {
			this.set( 'AttachmentView', mt.media.view.Attachment.EditLibrary );
		}
	...
	activate: function() {
		...
		this.get('library').observe( mt.Uploader.queue );
	...
	deactivate: function() {
		...
		this.get('library').unobserve( mt.Uploader.queue );
	 ...
	 * @since 3.5.0
	 *
	 * @param {mt.media.view.Frame} browser The file browser.
	 *
	 * @return {void}
	 */
	gallerySettings: function( browser ) {
		...
		browser.sidebar.set({
			gallery: new mt.media.view.Settings.Gallery({
				controller: this,
...
var Selection = mt.media.model.Selection,
	Library = mt.media.controller.Library,
	l10n = mt.media.view.l10n,
	GalleryAdd;

/**
 * mt.media.controller.GalleryAdd
 *
 ...
 */
GalleryAdd = Library.extend(/** @lends mt.media.controller.GalleryAdd.prototype */{
	defaults: _.defaults({
		id:            'gallery-library',
	...
	initialize: function() {
		if ( ! this.get('library') ) {
			this.set( 'library', mt.media.query({ type: 'image' }) );
		}
...
/**
 * mt.media.controller.CollectionEdit
 *
 ...
 * @param {mt.media.model.Attachments} [attributes.library]              The attachments collection to edit.
 *                                                                       If one is not supplied, an empty media.model.Selection collection is created.
 * @param {boolean}                    [attributes.multiple=false]       Whether multi-select is enabled.
 ...
 */
CollectionEdit = Library.extend(/** @lends mt.media.controller.CollectionEdit.prototype */{
	defaults: {
		multiple:         false,
		sortable:         true,
		date:             false,
	 ...
	 * @since 3.9.0
	 *
	 * @param {mt.media.view.attachmentsBrowser} The attachments browser view.
	 */
	renderSettings: function( attachmentsBrowserView ) {
		...
		if ( dragInfoText ) {
			attachmentsBrowserView.toolbar.set( 'dragInfo', new mt.media.View({
...
/**
 * mt.media.controller.CollectionAdd
 *
 ...
 * @param {string}                     attributes.collectionType            The collection type. (e.g. 'playlist').
 */
CollectionAdd = Library.extend(/** @lends mt.media.controller.CollectionAdd.prototype */{
	defaults: _.defaults( {
	...
	initialize: function() {
		...
		// If we haven't been provided a `library`, create a `Selection`.
		if ( ! this.get('library') ) {
			this.set( 'library', mt.media.query({ type: this.get('type') }) );
		}
		Library.prototype.initialize.apply( this, arguments );
	},
...
var Attachment = mt.media.model.Attachment,
	Library = mt.media.controller.Library,
	l10n = mt.media.view.l10n,
	FeaturedImage;

/**
 * mt.media.controller.FeaturedImage
 *
 ...
 * @param {boolean}                    [attributes.syncSelection=true]       Whether the Attachments selection should be persisted from the last state.
 */
FeaturedImage = Library.extend(/** @lends mt.media.controller.FeaturedImage.prototype */{
	defaults: _.defaults({
		id:            'featured-image',
	...
	updateSelection: function() {
		var selection = this.get('selection'),
			id = mt.media.view.settings.post.featuredImageId,
			attachment;
...
/**
 * mt.media.controller.ReplaceImage
 *
 ...
 * @param {boolean}                    [attributes.syncSelection=true]      Whether the Attachments selection should be persisted from the last state.
 */
ReplaceImage = Library.extend(/** @lends mt.media.controller.ReplaceImage.prototype */{
	defaults: _.defaults({
		id:            'replace-image',
 ...
 *
 * @param {object}                    attributes                      The attributes hash passed to the state.
 * @param {mt.media.model.Attachment} attributes.model                The attachment.
 ...
 */
EditImage = mt.media.controller.State.extend(/** @lends mt.media.controller.EditImage.prototype */{
	defaults: {
		id:      'edit-image',
	...
	toolbar: function() {
		...
		frame.toolbar.set( new mt.media.view.Toolbar({
			controller: frame,
			items: {
				back: {
					style: 'primary',
...
/**
 * mt.media.controller.MediaLibrary
 *
 * @memberOf mt.media.controller
 *
 * @class
...
MediaLibrary = Library.extend(/** @lends mt.media.controller.MediaLibrary.prototype */{
	defaults: _.defaults({
		// Attachments browser defaults. @see media.view.AttachmentsBrowser
		filterable:      'uploaded',
	...
	initialize: function( options ) {
		...
		this.set( 'library', mt.media.query({ type: this.type }) );
	...
	activate: function() {
		...
		if ( mt.media.frame.lastMime ) {
			this.set( 'library', mt.media.query({ type: mt.media.frame.lastMime }) );
			delete mt.media.frame.lastMime;
		}
		Library.prototype.activate.apply( this, arguments );
	}
});
 ...
 */
Embed = mt.media.controller.State.extend(/** @lends mt.media.controller.Embed.prototype */{
	defaults: {
		id:       'embed',
	 ...
	 *
	 * @fires mt.media.controller.Embed#scan
	 */
	scan: function() {
		var scanners,
			embed = this,
 ...
 * @augments Backbone.Model
 */
Cropper = mt.media.controller.State.extend(/** @lends mt.media.controller.Cropper.prototype */{
	defaults: {
		id:          'cropper',
	...
	createCropContent: function() {
		this.cropperView = new mt.media.view.Cropper({
			controller: this,
	...
	createCropToolbar: function() {
		...
		this.frame.toolbar.set( new mt.media.view.Toolbar(toolbarOptions) );
	},
	...
	doCrop: function( attachment ) {
		return mt.ajax.post( 'custom-header-crop', _.extend(
			{},
...
var Controller = mt.media.controller,
 ...
 * @since 4.3.0
 *
 * @constructs mt.media.controller.CustomizeImageCropper
 * @memberOf mt.media.controller
 * @augments mt.media.controller.CustomizeImageCropper.Cropper
 * @inheritDoc
 */
CustomizeImageCropper = Controller.Cropper.extend(/** @lends mt.media.controller.CustomizeImageCropper.prototype */{
	/**
	...
	doCrop: function( attachment ) {
		...
		return mt.ajax.post( 'crop-image', {
			mt_customize: 'on',
			nonce: attachment.get( 'nonces' ).edit,
			id: attachment.get( 'id' ),
			context: control.id,
...
/**
 * mt.media.controller.SiteIconCropper
 *
 ...
 *
 * @class
 * @augments mt.media.controller.Cropper
 * @augments mt.media.controller.State
 * @augments Backbone.Model
 */
SiteIconCropper = Controller.Cropper.extend(/** @lends mt.media.controller.SiteIconCropper.prototype */{
	activate: function() {
		this.frame.on( 'content:create:crop', this.createCropContent, this );
		this.frame.on( 'close', this.removeCropper, this );
		this.set('selection', new Backbone.Collection(this.frame._selection.single));
	...
	createCropContent: function() {
		this.cropperView = new mt.media.view.SiteIconCropper({
			controller: this,
			attachment: this.get('selection').first()
		});
		this.cropperView.on('image-loaded', this.createCropToolbar, this);
 ...
 */
var View = mt.Backbone.View.extend(/** @lends mt.media.View.prototype */{
	constructor: function( options ) {
		if ( options && options.controller ) {
			this.controller = options.controller;
		}
		mt.Backbone.View.apply( this, arguments );
	},
	/**
	 ...
	 *
	 * @return {mt.media.View} Returns itself to allow chaining.
	 */
	dispose: function() {
		/*
		 * Undelegating events, removing events from the model, and
	...
	remove: function() {
		 ...
		 */
		return mt.Backbone.View.prototype.remove.apply( this, arguments );
	}
});
 ...
 * states.
 *
 * @memberOf mt.media.view
 *
 * @see mt.media.controller.State
 * @see mt.media.controller.Region
 *
 * @class
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 * @mixes mt.media.controller.StateMachine
 */
var Frame = mt.media.View.extend(/** @lends mt.media.view.Frame.prototype */{
	initialize: function() {
	...
	_createRegions: function() {
		...
		_.each( this.regions, function( region ) {
			this[ region ] = new mt.media.controller.Region({
				view:     this,
				id:       region,
	 ...
	 *
	 * @see mt.media.controller.State
	 * @see mt.media.controller.StateMachine
	 *
	 * @fires mt.media.controller.State#ready
	 */
	_createStates: function() {
		// Create the default `states` collection.
		this.states = new Backbone.Collection( null, {
			model: mt.media.controller.State
		});
	 ...
	 *
	 * @return {mt.media.view.Frame} Returns itself to allow chaining.
	 */
	reset: function() {
	...
	deactivateMode: function( mode ) {
		 ...
		 *
		 * @event mt.media.view.Frame#{mode}:deactivate
		 */
...
_.extend( Frame.prototype, mt.media.controller.StateMachine.prototype );
...
var Frame = mt.media.view.Frame,
	l10n = mt.media.view.l10n,
	$ = jQuery,
	MediaFrame;

/**
 * mt.media.view.MediaFrame
 *
 ...
 *
 * @class
 * @augments mt.media.view.Frame
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 * @mixes mt.media.controller.StateMachine
 */
MediaFrame = Frame.extend(/** @lends mt.media.view.MediaFrame.prototype */{
	className: 'media-frame',
	template:  mt.template('media-frame'),
	...
	initialize: function() {
		...
		this.$el.addClass('mt-core-ui');

		// Initialize modal container view.
		if ( this.options.modal ) {
			this.modal = new mt.media.view.Modal({
				controller: this,
		...
		if ( mt.Uploader.limitExceeded || ! mt.Uploader.browser.supported ) {
			this.options.uploader = false;
		}

		// Initialize window-wide uploader.
		if ( this.options.uploader ) {
			this.uploader = new mt.media.view.UploaderWindow({
				controller: this,
				uploader: {
	...
	/**
	 * @return {mt.media.view.MediaFrame} Returns itself to allow chaining.
	 */
	render: function() {
	...
	/**
	 * @param {Object} title
	 * @this mt.media.controller.Region
	 */
	createTitle: function( title ) {
		title.view = new mt.media.View({
			controller: this,
			tagName: 'h1'
		});
	},
	...
	createMenu: function( menu ) {
		menu.view = new mt.media.view.Menu({
			controller: this,
	...
	createToolbar: function( toolbar ) {
		toolbar.view = new mt.media.view.Toolbar({
			controller: this
		});
	},
	/**
	...
	createRouter: function( router ) {
		router.view = new mt.media.view.Router({
			controller: this,
	...
	iframeContent: function( content ) {
		...
		content.view = new mt.media.view.Iframe({
			controller: this
		});
	},
	...
	iframeMenu: function( view ) {
		...
		_.each( mt.media.view.settings.tabs, function( title, id ) {
			views[ 'iframe:' + id ] = {
...
_.each(['open','close','attach','detach','escape'], function( method ) {
	/**
	 * @function open
	 * @memberOf mt.media.view.MediaFrame
	 * @instance
	 *
	 * @return {mt.media.view.MediaFrame} Returns itself to allow chaining.
	 */
...
var MediaFrame = mt.media.view.MediaFrame,
	l10n = mt.media.view.l10n,
	Select;

/**
 * mt.media.view.MediaFrame.Select
 *
 ...
 *
 * @class
 * @augments mt.media.view.MediaFrame
 * @augments mt.media.view.Frame
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 * @mixes mt.media.controller.StateMachine
 */
Select = MediaFrame.extend(/** @lends mt.media.view.MediaFrame.Select.prototype */{
	initialize: function() {
	...
	createSelection: function() {
		...
		if ( ! (selection instanceof mt.media.model.Selection) ) {
			this.options.selection = new mt.media.model.Selection( selection, {
				multiple: this.options.multiple
			});
		}

		this._selection = {
			attachments: new mt.media.model.Attachments(),
			difference: []
		};
	},
	...
	editImageContent: function() {
		var image = this.state().get('image'),
			view = new mt.media.view.EditImage( { model: image, controller: this } ).render();
	...
	createStates: function() {
		...
		this.states.add([
			// Main states.
			new mt.media.controller.Library({
				library:   mt.media.query( options.library ),
				multiple:  options.multiple,
				title:     options.title,
				priority:  20
			}),
			new mt.media.controller.EditImage( { model: options.editImage } )
		]);
	},
	 ...
	 *
	 * @param {mt.media.view.Router} routerView
	 */
	 ...
	 *
	 * @param {mt.media.controller.Region} contentRegion
	 */
	browseContent: function( contentRegion ) {
		...
		contentRegion.view = new mt.media.view.AttachmentsBrowser({
			controller: this,
	...
	uploadContent: function() {
		this.$el.removeClass( 'hide-toolbar' );
		this.content.set( new mt.media.view.UploaderInline({
			controller: this
		}) );
	},
	...
	createSelectToolbar: function( toolbar, options ) {
		...
		toolbar.view = new mt.media.view.Toolbar.Select( options );
	}
});
...
var Select = mt.media.view.MediaFrame.Select,
	Library = mt.media.controller.Library,
	l10n = mt.media.view.l10n,
	Post;

/**
 * mt.media.view.MediaFrame.Post
 *
 ...
 *
 * @class
 * @augments mt.media.view.MediaFrame.Select
 * @augments mt.media.view.MediaFrame
 * @augments mt.media.view.Frame
 * @augments mt.media.View
 * @augments mt.Backbone.View
 ...
 * @mixes mt.media.controller.StateMachine
 */
Post = Select.extend(/** @lends mt.media.view.MediaFrame.Post.prototype */{
	initialize: function() {
		this.counts = {
			audio: {
				count: mt.media.view.settings.attachmentCounts.audio,
				state: 'playlist'
			},
			video: {
				count: mt.media.view.settings.attachmentCounts.video,
	...
	createStates: function() {
		...
		this.states.add([
			...
			new Library({
				...
				library:  mt.media.query( _.defaults({
					type: 'image'
				}, options.library ) )
			}),

			// Embed states.
			new mt.media.controller.Embed( { metadata: options.metadata } ),

			new mt.media.controller.EditImage( { model: options.editImage } ),

			// Gallery states.
			new mt.media.controller.GalleryEdit({
				library: options.selection,
				editing: options.editing,
				menu:    'gallery'
			}),

			new mt.media.controller.GalleryAdd(),
			...
			// Playlist states.
			new mt.media.controller.CollectionEdit({
				type: 'audio',
				collectionType: 'playlist',
				title:          l10n.editPlaylistTitle,
				SettingsView:   mt.media.view.Settings.Playlist,
				library:        options.selection,
				editing:        options.editing,
				menu:           'playlist',
			...
			new mt.media.controller.CollectionAdd({
				type: 'audio',
				collectionType: 'playlist',
		...
		if ( mt.media.view.settings.post.featuredImageId ) {
			this.states.add( new mt.media.controller.FeaturedImage() );
		}
	},
	...
	bindHandlers: function() {
		...
		if ( typeof checkCounts !== 'undefined' ) {
			this.listenTo( mt.media.model.Attachments.all, 'change:type', this.mediaTypeCounts );
		}
	...
	// Menus.
	/**
	 * @param {mt.Backbone.View} view
	 */
	mainMenu: function( view ) {
		view.set({
			'library-separator': new mt.media.View({
				className:  'separator',
				priority:   100,
				attributes: {
					role: 'presentation'
	...
	galleryMenu: function( view ) {
		...
		view.set({
			...
			separateCancel: new mt.media.View({
				className: 'separator',
				priority: 40
			})
		});
	...
	embedContent: function() {
		var view = new mt.media.view.Embed({
			controller: this,
			model:      this.state()
		}).render();
	...
	editSelectionContent: function() {
		...
		view = new mt.media.view.AttachmentsBrowser({
			controller: this,
			...
			AttachmentView: mt.media.view.Attachments.EditSelection
		}).render();
	...
	selectionStatusToolbar: function( view ) {
		...
		view.set( 'selection', new mt.media.view.Selection({
			controller: this,
	...
	mainInsertToolbar: function( view ) {
		...
		view.set( 'insert', {
			 ...
			 * @ignore
			 *
			 * @fires mt.media.controller.State#insert
			 */
			click: function() {
	...
	mainGalleryToolbar: function( view ) {
		...
		view.set( 'gallery', {
			...
			click: function() {
				...
				edit.set( 'library', new mt.media.model.Selection( models, {
	...
	mainEmbedToolbar: function( toolbar ) {
		toolbar.view = new mt.media.view.Toolbar.Embed({
			controller: this
		});
	},
	...
	galleryEditToolbar: function() {
		...
		this.toolbar.set( new mt.media.view.Toolbar({
			controller: this,
			items: {
				insert: {
					style:    'primary',
					...
					/**
					 * @fires mt.media.controller.State#update
					 */
					click: function() {
						var controller = this.controller,
		...
		this.toolbar.set( new mt.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires mt.media.controller.State#reset
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state(),
...
/**
 * mt.media.view.MediaFrame.ImageDetails
 *
 ...
 * @mixes mt.media.controller.StateMachine
 */
ImageDetails = Select.extend(/** @lends mt.media.view.MediaFrame.ImageDetails.prototype */{
	defaults: {
		id:      'image',
		url:     '',
		menu:    'image-details',
	...
	initialize: function( options ) {
		this.image = new mt.media.model.PostImage( options.metadata );
		this.options.selection = new mt.media.model.Selection( this.image.attachment, { multiple: false } );
		Select.prototype.initialize.apply( this, arguments );
	},
	...
	createStates: function() {
		this.states.add([
			new mt.media.controller.ImageDetails({
				image: this.image,
				editable: false
			}),
			new mt.media.controller.ReplaceImage({
				id: 'replace-image',
				library: mt.media.query( { type: 'image' } ),
				image: this.image,
				multiple:  false,
			...
			new mt.media.controller.EditImage( {
				image: this.image,
	...
	imageDetailsContent: function( options ) {
		options.view = new mt.media.view.ImageDetails({
			controller: this,
	...
	editImageContent: function() {
		...
		view = new mt.media.view.EditImage( { model: model, controller: this } ).render();
			...
			items: {
				select: {
					...
					click: function() {
						...
						// Not sure if we want to use mt.media.string.image which will create a shortcode or
						// perhaps mt.html.string to at least to build the <img />.
 ...
 * @augments Backbone.View
 */
Modal = mt.media.View.extend(/** @lends mt.media.view.Modal.prototype */{
	tagName:  'div',
	template: mt.template('media-modal'),
	...
	initialize: function() {
		...
		this.focusManager = new mt.media.view.FocusManager({
			el: this.el
		});
	},
	/**
	...
	/**
	 * @return {mt.media.view.Modal} Returns itself to allow chaining.
	 */
	attach: function() {
 ...
 * @augments Backbone.View
 */
var FocusManager = mt.media.View.extend(/** @lends mt.media.view.FocusManager.prototype */{
	 ...
	 * prevents the `mt.a11y.speak()` ARIA live regions to work as they're outside
 ...
 */
UploaderWindow = mt.media.View.extend(/** @lends mt.media.view.UploaderWindow.prototype */{
	tagName:   'div',
	className: 'uploader-window',
	template:  mt.template('uploader-window'),
	...
	ready: function() {
		var postId = mt.media.view.settings.post.id,
			dropzone;
		...
		this.uploader = new mt.Uploader( this.options.uploader );
	...
	hide: function() {
		...
		mt.media.transition( $el ).done( function() {
...
var View = mt.media.View,
	l10n = mt.media.view.l10n,
	$ = jQuery,
	EditorUploader;

/**
 * Creates a dropzone on MT editor instances (elements with .mt-editor-wrap)
 * and relays drag'n'dropped files to a media workflow.
 *
 * mt.media.view.EditorUploader
 *
 * @memberOf mt.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
EditorUploader = View.extend(/** @lends mt.media.view.EditorUploader.prototype */{
	tagName:   'div',
	className: 'uploader-editor',
	template:  mt.template( 'uploader-editor' ),
	...
	render: function() {
		...
		$( '.mt-editor-wrap' ).each( _.bind( this.attach, this ) );
		return this;
	},
	...
	drop: function( event ) {
		...
		$wrap = $( event.target ).parents( '.mt-editor-wrap' );
		if ( $wrap.length > 0 && $wrap[0].id ) {
			window.mtActiveEditor = $wrap[0].id.slice( 3, -5 );
		}

		if ( ! this.workflow ) {
			this.workflow = mt.media.editor.open( window.mtActiveEditor, {
				frame:    'post',
				state:    'insert',
				title:    l10n.addMedia,
				multiple: true
 ...
 * @augments Backbone.View
 */
UploaderInline = View.extend(/** @lends mt.media.view.UploaderInline.prototype */{
	tagName:   'div',
	className: 'uploader-inline',
	template:  mt.template('uploader-inline'),
	...
	initialize: function() {
		...
		if ( _.isUndefined( this.options.postId ) ) {
			this.options.postId = mt.media.view.settings.post.id;
		}

		if ( this.options.status ) {
			this.views.set( '.upload-inline-status', new mt.media.view.UploaderStatus({
	...
	/**
	 * @return {mt.media.view.UploaderInline} Returns itself to allow chaining.
	 */
	dispose: function() {
		if ( this.disposing ) {
			/**
	...
	/**
	 * @return {mt.media.view.UploaderInline}
	 */
	ready: function() {
 ...
 * @augments Backbone.View
 */
UploaderStatus = View.extend(/** @lends mt.media.view.UploaderStatus.prototype */{
	className: 'media-uploader-status',
	template:  mt.template('uploader-status'),
	...
	initialize: function() {
		this.queue = mt.Uploader.queue;
		this.queue.on( 'add remove reset', this.visibility, this );
		this.queue.on( 'add remove reset change:percent', this.progress, this );
		this.queue.on( 'add remove reset change:uploading', this.info, this );

		this.errors = mt.Uploader.errors;
		this.errors.reset();
		this.errors.on( 'add remove reset', this.visibility, this );
		this.errors.on( 'add', this.error, this );
	},
	/**
	 * @return {mt.media.view.UploaderStatus}
	 */
	dispose: function() {
		mt.Uploader.queue.off( null, null, this );
		/**
	...
	error: function( error ) {
		var statusError = new mt.media.view.UploaderStatusError( {
		...
		_.delay( function() {
			...
			mt.a11y.speak( error.get( 'message' ), 'assertive' );
		}, 1000 );
	},
...
/**
 * mt.media.view.UploaderStatusError
 *
 * @memberOf mt.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
var UploaderStatusError = mt.media.View.extend(/** @lends mt.media.view.UploaderStatusError.prototype */{
	className: 'upload-error',
	template:  mt.template('uploader-status-error')
});
 ...
 * @augments Backbone.View
 */
Toolbar = View.extend(/** @lends mt.media.view.Toolbar.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		...
		this.primary   = new mt.media.view.PriorityList();
		this.secondary = new mt.media.view.PriorityList();
	...
	/**
	 * @return {mt.media.view.Toolbar} Returns itsef to allow chaining
	 */
	dispose: function() {
		if ( this.selection ) {
	 ...
	 * @param {Object} [options={}]
	 * @return {mt.media.view.Toolbar} Returns itself to allow chaining.
	 */
	set: function( id, view, options ) {
		...
		if ( _.isObject( id ) ) {
			...
			if ( ! ( view instanceof Backbone.View ) ) {
				...
				view = new mt.media.view.Button( view ).render();
			}
...
var Toolbar = mt.media.view.Toolbar,
	l10n = mt.media.view.l10n,
	Select;

/**
 * mt.media.view.Toolbar.Select
 *
 * @memberOf mt.media.view.Toolbar
 *
 * @class
 * @augments mt.media.view.Toolbar
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
Select = Toolbar.extend(/** @lends mt.media.view.Toolbar.Select.prototype */{
	initialize: function() {
		var options = this.options;
...
var Select = mt.media.view.Toolbar.Select,
	l10n = mt.media.view.l10n,
	Embed;

/**
 * mt.media.view.Toolbar.Embed
 *
 * @memberOf mt.media.view.Toolbar
 *
 * @class
 * @augments mt.media.view.Toolbar.Select
 * @augments mt.media.view.Toolbar
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
Embed = Select.extend(/** @lends mt.media.view.Toolbar.Embed.prototype */{
	initialize: function() {
		_.defaults( this.options, {
			text: l10n.insertIntoPost,
			requires: false
 ...
 * @augments Backbone.View
 */
var Button = mt.media.View.extend(/** @lends mt.media.view.Button.prototype */{
	tagName:    'button',
	...
	/**
	 * @return {mt.media.view.Button} Returns itself to allow chaining.
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
ButtonGroup = mt.media.View.extend(/** @lends mt.media.view.ButtonGroup.prototype */{
	tagName:   'div',
	className: 'button-group button-large media-button-group',

	initialize: function() {
		/**
		 * @member {mt.media.view.Button[]}
		 */
		this.buttons = _.map( this.options.buttons || [], function( button ) {
			if ( button instanceof Backbone.View ) {
				return button;
			} else {
				return new mt.media.view.Button( button ).render();
			}
		});
	...
	/**
	 * @return {mt.media.view.ButtonGroup}
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
var PriorityList = mt.media.View.extend(/** @lends mt.media.view.PriorityList.prototype */{
	tagName:   'div',
	...
	/**
	 * @param {string} id
	 * @param {mt.media.View|Object} view
	 * @param {Object} options
	 * @return {mt.media.view.PriorityList} Returns itself to allow chaining.
	 */
	set: function( id, view, options ) {
	...
	/**
	 * @param {string} id
	 * @return {mt.media.view.PriorityList}
	 */
	unset: function( id ) {
	...
	toView: function( options ) {
		return new mt.media.View( options );
	}
});
 ...
 * @augments Backbone.View
 */
MenuItem = mt.media.View.extend(/** @lends mt.media.view.MenuItem.prototype */{
	tagName:   'button',
	...
	/**
	 * @return {mt.media.view.MenuItem} returns itself to allow chaining.
	 */
	render: function() {
		var options = this.options,
...
var MenuItem = mt.media.view.MenuItem,
	PriorityList = mt.media.view.PriorityList,
	Menu;
 ...
 *
 * @class
 * @augments mt.media.view.PriorityList
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
Menu = PriorityList.extend(/** @lends mt.media.view.Menu.prototype */{
	tagName:   'div',
...
/**
 * mt.media.view.RouterItem
 *
 * @memberOf mt.media.view
 *
 * @class
 * @augments mt.media.view.MenuItem
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
var RouterItem = mt.media.view.MenuItem.extend(/** @lends mt.media.view.RouterItem.prototype */{
	/**
...
var Menu = mt.media.view.Menu,
	Router;

/**
 * mt.media.view.Router
 *
 * @memberOf mt.media.view
 *
 * @class
 * @augments mt.media.view.Menu
 * @augments mt.media.view.PriorityList
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
Router = Menu.extend(/** @lends mt.media.view.Router.prototype */{
	tagName:   'div',
	className: 'media-router',
	property:  'contentMode',
	ItemView:  mt.media.view.RouterItem,
	region:    'router',
 ...
 * @augments Backbone.View
 */
var Sidebar = mt.media.view.PriorityList.extend(/** @lends mt.media.view.Sidebar.prototype */{
 ...
 * @augments Backbone.View
 */
Attachment = View.extend(/** @lends mt.media.view.Attachment.prototype */{
	tagName:   'li',
	className: 'attachment',
	template:  mt.template('attachment'),
	...
	/**
	 * @return {mt.media.view.Attachment} Returns itself to allow chaining.
	 */
	dispose: function() {
		var selection = this.options.selection;
...
_.each({
	 ...
	 * @memberOf mt.media.view.Attachment
	 * @instance
	 *
	 * @param {Backbone.Model} model
	 * @param {string} value
...
/**
 * mt.media.view.Attachment.Library
 *
 * @memberOf mt.media.view.Attachment
 *
 * @class
 * @augments mt.media.view.Attachment
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
var Library = mt.media.view.Attachment.extend(/** @lends mt.media.view.Attachment.Library.prototype */{
	buttons: {
		check: true
	}
});
...
/**
 * mt.media.view.Attachment.EditLibrary
 *
 * @memberOf mt.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var EditLibrary = mt.media.view.Attachment.extend(/** @lends mt.media.view.Attachment.EditLibrary.prototype */{
	buttons: {
		close: true
	}
});
...
Attachments = View.extend(/** @lends mt.media.view.Attachments.prototype */{
	tagName:   'ul',
	...
	initialize: function() {
		 ...
		 */
		_.defaults( this.options, {
			refreshSensitivity: mt.media.isTouchDevice ? 300 : 200,
			refreshThreshold:   3,
			AttachmentView:     mt.media.view.Attachment,
			sortable:           false,
			resize:             true,
	 ...
	 * @param {mt.media.model.Attachment} attachment
	 *
	 * @return {mt.media.View} The created view.
	 */
 ...
 * @augments Backbone.View
 */
Search = mt.media.View.extend(/** @lends mt.media.view.Search.prototype */{
	tagName:   'input',
	className: 'search',
	...
	/**
	 * @return {mt.media.view.Search} Returns itself to allow chaining.
	 */
	render: function() {
...
/**
 * mt.media.view.AttachmentFilters
 *
 * @memberOf mt.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
AttachmentFilters = mt.media.View.extend(/** @lends mt.media.view.AttachmentFilters.prototype */{
	tagName:   'select',
 ...
 *
 * @memberOf mt.media.view.AttachmentFilters
 *
 * @class
 * @augments mt.media.view.AttachmentFilters
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
DateFilter = mt.media.view.AttachmentFilters.extend(/** @lends mt.media.view.AttachmentFilters.Date.prototype */{
	id: 'media-attachment-date-filters',

	createFilters: function() {
		var filters = {};
		_.each( mt.media.view.settings.months || {}, function( value, index ) {
			filters[ index ] = {
				text: value.text,
				props: {
					year: value.year,
...
/**
 * mt.media.view.AttachmentFilters.Uploaded
 *
 * @memberOf mt.media.view.AttachmentFilters
 *
 * @class
 ...
 * @augments Backbone.View
 */
Uploaded = mt.media.view.AttachmentFilters.extend(/** @lends mt.media.view.AttachmentFilters.Uploaded.prototype */{
	createFilters: function() {
		var type = this.model.get('type'),
			types = mt.media.view.settings.mimeTypes,
		...
		this.filters = {
			...
			uploaded: {
				...
				props: {
					uploadedTo: mt.media.view.settings.post.id,
					orderby: 'menuOrder',
					order:   'ASC',
					author:	 null
				},
...
/**
 * mt.media.view.AttachmentFilters.All
 *
 * @memberOf mt.media.view.AttachmentFilters
 *
 * @class
 ...
 * @augments Backbone.View
 */
All = mt.media.view.AttachmentFilters.extend(/** @lends mt.media.view.AttachmentFilters.All.prototype */{
	createFilters: function() {
		var filters = {},
			uid = window.userSettings ? parseInt( window.userSettings.uid, 10 ) : 0;

		_.each( mt.media.view.settings.mimeTypes || {}, function( text, key ) {
			filters[ key ] = {
				text: text,
				props: {
					status:  null,
		...
		if ( mt.media.view.settings.post.id ) {
			filters.uploaded = {
				text:  l10n.uploadedToThisPost,
				props: {
					status:  null,
		...
		if ( mt.media.view.settings.mediaTrash &&
...
var View = mt.media.View,
	mediaTrash = mt.media.view.settings.mediaTrash,
	l10n = mt.media.view.l10n,
	$ = jQuery,
	AttachmentsBrowser;

/**
 * mt.media.view.AttachmentsBrowser
 *
 * @memberOf mt.media.view
 *
 * @class
 ...
 */
AttachmentsBrowser = View.extend(/** @lends mt.media.view.AttachmentsBrowser.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		_.defaults( this.options, {
			...
			display: false,
			sidebar: true,
			AttachmentView: mt.media.view.Attachment.Library
		});
	...
	/**
	 * Updates the `mt.a11y.speak()` ARIA live region with a message to communicate
	...
	announceSearchResults: _.debounce( function() {
		...
		if ( this.collection.mirroring.args.s ) {
			...
			if ( 0 === count ) {
				mt.a11y.speak( l10n.noMediaTryNewSearch );
				return;
			}

			if ( this.collection.hasMore() ) {
				mt.a11y.speak( l10n.mediaFoundHasMoreResults.replace( '%d', count ) );
				return;
			}

			mt.a11y.speak( l10n.mediaFound.replace( '%d', count ) );
		}
	}, 200 ),
	...
	/**
	 * @return {mt.media.view.AttachmentsBrowser} Returns itself to allow chaining.
	 */
	dispose: function() {
	...
	createToolbar: function() {
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			toolbarOptions.className = 'media-toolbar mt-filter';
		}

		/**
		* @member {mt.media.view.Toolbar}
		*/
		this.toolbar = new mt.media.view.Toolbar( toolbarOptions );

		this.views.add( this.toolbar );

		this.toolbar.set( 'spinner', new mt.media.view.Spinner({
			priority: -20
		}) );
		...
		if ( showFilterByType || this.options.date ) {
			 ...
			 */
			this.toolbar.set( 'filters-heading', new mt.media.view.Heading( {
				priority:   -100,
		...
		if ( showFilterByType ) {
			...
			this.toolbar.set( 'filtersLabel', new mt.media.view.Label({
			...
			if ( 'uploaded' === this.options.filters ) {
				this.toolbar.set( 'filters', new mt.media.view.AttachmentFilters.Uploaded({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				}).render() );
			} else {
				Filters = new mt.media.view.AttachmentFilters.All({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				});
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			LibraryViewSwitcher = View.extend({
				...
				template: mt.template( 'media-library-view-switcher')
			});
			...
			this.toolbar.set( 'dateFilterLabel', new mt.media.view.Label({
			...
			this.toolbar.set( 'dateFilter', new mt.media.view.DateFilter({
				controller: this.controller,
				model:      this.collection.props,
				priority: -75
			}).render() );
			...
			this.toolbar.set( 'selectModeToggleButton', new mt.media.view.SelectModeToggleButton({
				text: l10n.bulkSelect,
				controller: this.controller,
				priority: -70
			}).render() );

			this.toolbar.set( 'deleteSelectedButton', new mt.media.view.DeleteSelectedButton({
				filters: Filters,
				style: 'primary',
				disabled: true,
			...
			if ( mediaTrash ) {
				this.toolbar.set( 'deleteSelectedPermanentlyButton', new mt.media.view.DeleteSelectedPermanentlyButton({
					filters: Filters,
		...
		if ( this.options.search ) {
			...
			this.toolbar.set( 'searchLabel', new mt.media.view.Label({
			...
			this.toolbar.set( 'search', new mt.media.view.Search({
				controller: this.controller,
				model:      this.collection.props,
				priority:   60
			}).render() );
	...
	createUploader: function() {
		this.uploader = new mt.media.view.UploaderInline({
			controller: this.controller,
			status:     false,
	...
	createAttachments: function() {
		this.attachments = new mt.media.view.Attachments({
			controller:           this.controller,
			collection:           this.collection,
			selection:            this.options.selection,
			model:                this.model,
	...
	createAttachmentsHeading: function() {
		this.attachmentsHeading = new mt.media.view.Heading( {
	...
	createSidebar: function() {
		var options = this.options,
			...
			sidebar = this.sidebar = new mt.media.view.Sidebar({
				controller: this.controller
			});
		...
		if ( this.controller.uploader ) {
			sidebar.set( 'uploads', new mt.media.view.UploaderStatus({
				controller: this.controller,
				priority:   40
			}) );
		}
	...
	createSingle: function() {
		...
		sidebar.set( 'details', new mt.media.view.Attachment.Details({
			controller: this.controller,
			model:      single,
			priority:   80
		}) );

		sidebar.set( 'compat', new mt.media.view.AttachmentCompat({
			controller: this.controller,
			model:      single,
			priority:   120
		}) );
		...
		if ( this.options.display ) {
			sidebar.set( 'display', new mt.media.view.Settings.AttachmentDisplay({
				controller:   this.controller,
...
var _n = mt.i18n._n,
	sprintf = mt.i18n.sprintf,
	Selection;

/**
 * mt.media.view.Selection
 *
 * @memberOf mt.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
Selection = mt.media.View.extend(/** @lends mt.media.view.Selection.prototype */{
	tagName:   'div',
	className: 'media-selection',
	template:  mt.template('media-selection'),
	...
	initialize: function() {
		...
		/**
		 * @member {mt.media.view.Attachments.Selection}
		 */
		this.attachments = new mt.media.view.Attachments.Selection({
			controller: this.controller,
			collection: this.collection,
...
/**
 * mt.media.view.Attachment.Selection
 *
 * @memberOf mt.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Selection = mt.media.view.Attachment.extend(/** @lends mt.media.view.Attachment.Selection.prototype */{
...
var Attachments = mt.media.view.Attachments,
	Selection;

/**
 * mt.media.view.Attachments.Selection
 *
 * @memberOf mt.media.view.Attachments
 *
 * @class
 * @augments mt.media.view.Attachments
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
Selection = Attachments.extend(/** @lends mt.media.view.Attachments.Selection.prototype */{
	events: {},
	initialize: function() {
		_.defaults( this.options, {
			sortable:   false,
			...
			// The single `Attachment` view to be used in the `Attachments` view.
			AttachmentView: mt.media.view.Attachment.Selection
		});
		// Call 'initialize' directly on the parent class.
...
/**
 * mt.media.view.Attachment.EditSelection
 *
 * @memberOf mt.media.view.Attachment
 *
 * @class
 * @augments mt.media.view.Attachment.Selection
 * @augments mt.media.view.Attachment
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
var EditSelection = mt.media.view.Attachment.Selection.extend(/** @lends mt.media.view.Attachment.EditSelection.prototype */{
	buttons: {
		close: true
	}
});
 ...
 * @augments Backbone.View
 */
Settings = View.extend(/** @lends mt.media.view.Settings.prototype */{
	events: {
	...
	/**
	 * @return {mt.media.view.Settings} Returns itself to allow chaining.
	 */
	render: function() {
		View.prototype.render.apply( this, arguments );
...
/**
 * mt.media.view.Settings.AttachmentDisplay
 *
 * @memberOf mt.media.view.Settings
 *
 * @class
 * @augments mt.media.view.Settings
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
AttachmentDisplay = Settings.extend(/** @lends mt.media.view.Settings.AttachmentDisplay.prototype */{
	className: 'attachment-display-settings',
	template:  mt.template('attachment-display-settings'),
	...
	/**
	 * @return {mt.media.view.AttachmentDisplay} Returns itself to allow chaining.
	 */
	render: function() {
		var attachment = this.options.attachment;
		if ( attachment ) {
...
/**
 * mt.media.view.Settings.Gallery
 *
 * @memberOf mt.media.view.Settings
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Gallery = mt.media.view.Settings.extend(/** @lends mt.media.view.Settings.Gallery.prototype */{
	className: 'collection-settings gallery-settings',
	template:  mt.template('gallery-settings')
});
...
/**
 * mt.media.view.Settings.Playlist
 *
 * @memberOf mt.media.view.Settings
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Playlist = mt.media.view.Settings.extend(/** @lends mt.media.view.Settings.Playlist.prototype */{
	className: 'collection-settings playlist-settings',
	template:  mt.template('playlist-settings')
});
...
/* global ClipboardJS */
var Attachment = mt.media.view.Attachment,
	l10n = mt.media.view.l10n,
	$ = jQuery,
	Details,
	__ = mt.i18n.__;

Details = Attachment.extend(/** @lends mt.media.view.Attachment.Details.prototype */{
	tagName:   'div',
	className: 'attachment-details',
	template:  mt.template('attachment-details'),
	 ...
	 */
	 copyAttachmentDetailsURLClipboard: function() {
		...
		clipboard.on( 'success', function( event ) {
			...
			mt.a11y.speak( __( 'The file URL has been copied to your clipboard' ) );
		} );
	 },
	 ...
	 * @since 3.5.0
	 *
	 * @constructs mt.media.view.Attachment.Details
	 * @augments mt.media.view.Attachment
	 *
	 * @return {void}
	 */
	 ...
	 * @since 4.0.0
	 *
	 * @fires mt.media.controller.MediaLibrary#attachment:details:shift-tab
	 * @fires mt.media.controller.MediaLibrary#attachment:keydown:arrow
	 *
	...
	render: function() {
		...
		mt.media.mixin.removeAllPlayers();
		this.$( 'audio, video' ).each( function (i, elem) {
			var el = mt.media.view.MediaDetails.prepareSrc( elem );
			new window.MediaElementPlayer( el, mt.media.mixin.mejsSettings );
		} );
	}
});
...
/**
 * mt.media.view.AttachmentCompat
 *
 ...
 * @augments Backbone.View
 */
AttachmentCompat = View.extend(/** @lends mt.media.view.AttachmentCompat.prototype */{
	tagName:   'form',
	...
	/**
	 * @return {mt.media.view.AttachmentCompat} Returns itself to allow chaining.
	 */
	dispose: function() {
 ...
 * @augments Backbone.View
 */
var Iframe = mt.media.View.extend(/** @lends mt.media.view.Iframe.prototype */{
	className: 'media-iframe',
	/**
	 * @return {mt.media.view.Iframe} Returns itself to allow chaining.
	 */
	render: function() {
		this.views.detach();
 ...
 * @augments Backbone.View
 */
var Embed = mt.media.View.extend(/** @lends mt.media.view.Ember.prototype */{
	className: 'media-embed',

	initialize: function() {
		/**
		 * @member {mt.media.view.EmbedUrl}
		 */
		this.url = new mt.media.view.EmbedUrl({
			controller: this.controller,
	...
	refresh: function() {
		...
		if ( 'image' === type ) {
			constructor = mt.media.view.EmbedImage;
		} else if ( 'link' === type ) {
			constructor = mt.media.view.EmbedLink;
		} else {
			return;
		}
 ...
 * @augments Backbone.View
 */
var Label = mt.media.View.extend(/** @lends mt.media.view.Label.prototype */{
	tagName: 'label',
 ...
 * @augments Backbone.View
 */
EmbedUrl = View.extend(/** @lends mt.media.view.EmbedUrl.prototype */{
	tagName:   'span',
	className: 'embed-url',
	...
	/**
	 * @return {mt.media.view.EmbedUrl} Returns itself to allow chaining.
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
EmbedLink = mt.media.view.Settings.extend(/** @lends mt.media.view.EmbedLink.prototype */{
	className: 'embed-link-settings',
	template:  mt.template('embed-link-settings'),
	...
	updateoEmbed: _.debounce( function() {
		...
		this.fetch();
	}, mt.media.controller.Embed.sensitivity ),
	...
	fetch: function() {
		...
		this.dfd = mt.apiRequest({
			url: mt.media.view.settings.oEmbedProxyUrl,
			data: {
				url: url,
...
var AttachmentDisplay = mt.media.view.Settings.AttachmentDisplay,
	EmbedImage;

/**
 * mt.media.view.EmbedImage
 *
 * @memberOf mt.media.view
 *
 * @class
 * @augments mt.media.view.Settings.AttachmentDisplay
 * @augments mt.media.view.Settings
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
EmbedImage = AttachmentDisplay.extend(/** @lends mt.media.view.EmbedImage.prototype */{
	className: 'embed-media-settings',
	template:  mt.template('embed-image-settings'),
 ...
 * @augments Backbone.View
 */
ImageDetails = AttachmentDisplay.extend(/** @lends mt.media.view.ImageDetails.prototype */{
	className: 'image-details',
	template:  mt.template('image-details'),
...
var View = mt.media.View,
	UploaderStatus = mt.media.view.UploaderStatus,
	l10n = mt.media.view.l10n,
	$ = jQuery,
	Cropper;

/**
 * mt.media.view.Cropper
 *
 * Uses the imgAreaSelect plugin to allow a user to crop an image.
 *
 * Takes imgAreaSelect options from
 * mt.customize.HeaderControl.calculateImageSelectOptions via
 * mt.customize.HeaderControl.openMM.
 *
 * @memberOf mt.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
Cropper = View.extend(/** @lends mt.media.view.Cropper.prototype */{
	className: 'crop-content',
	template: mt.template('crop-content'),
	initialize: function() {
	...
	onError: function() {
		...
		this.views.add( '.upload-errors', new mt.media.view.UploaderStatusError({
			filename: UploaderStatus.prototype.filename(filename),
			message: window._mtMediaViewsL10n.cropError
		}), { at: 0 });
	}
});
...
/**
 * mt.media.view.SiteIconCropper
 *
 * Uses the imgAreaSelect plugin to allow a user to crop a Site Icon.
 *
 * Takes imgAreaSelect options from
 * mt.customize.SiteIconControl.calculateImageSelectOptions.
 *
 * @memberOf mt.media.view
 *
 * @class
 * @augments mt.media.view.Cropper
 * @augments mt.media.View
 * @augments mt.Backbone.View
 * @augments Backbone.View
 */
SiteIconCropper = View.Cropper.extend(/** @lends mt.media.view.SiteIconCropper.prototype */{
	...
	addSidebar: function() {
		this.sidebar = new mt.media.view.Sidebar({
			controller: this.controller
		});

		this.sidebar.set( 'preview', new mt.media.view.SiteIconPreview({
			controller: this.controller,
			attachment: this.options.attachment
		}) );
 ...
 * @augments Backbone.View
 */
SiteIconPreview = View.extend(/** @lends mt.media.view.SiteIconPreview.prototype */{
	className: 'site-icon-preview',
	template: mt.template( 'site-icon-preview' ),
 ...
 * @augments Backbone.View
 */
EditImage = View.extend(/** @lends mt.media.view.EditImage.prototype */{
	className: 'image-editor',
	template: mt.template('image-editor'),
 ...
 * @augments Backbone.View
 */
var Spinner = mt.media.View.extend(/** @lends mt.media.view.Spinner.prototype */{
	tagName:   'span',
	className: 'spinner',
	 ...
	 * @since 3.9.0
	 *
	 * @return {mt.media.view.Spinner} The spinner.
	 */
	show: function() {
 ...
 * @augments Backbone.View
 */
var Heading = mt.media.View.extend( {
	tagName: function() {
