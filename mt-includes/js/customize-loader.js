/**
 * @output mt-includes/js/customize-loader.js
 */

/* global _mtCustomizeLoaderSettings */
...
(function( exports, $ ){
	 ...
	 *
	 *     e.g. <a class="load-customize" href="<?php echo mt_customize_url(); ?>">Open Customizer</a>
	 *
	 * @memberOf mt.customize
	 *
	 * @class
	 * @augments mt.customize.Events
	 */
	Loader = $.extend( {}, api.Events,/** @lends mt.customize.Loader.prototype */{
		/**
		...
		initialize: function() {
			...
			// the Customizer.
			$('#mtbody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function() {
			...
			if ( hash && 0 === hash.indexOf( 'mt_customize=on' ) ) {
		...
		pushState: function ( src ) {
			...
			if ( $.support.history && window.location.href !== src ) {
				...
				window.location.hash = 'mt_customize=on&' + hash;
			}
	...
	$( function() {
		Loader.settings = _mtCustomizeLoaderSettings;
		Loader.initialize();
	});

	// Expose the API publicly on window.mt.customize.Loader.
	api.Loader = Loader;
})( mt, jQuery );
