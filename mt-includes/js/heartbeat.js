 ...
 * @see mt_ajax_nopriv_heartbeat(), mt_ajax_heartbeat()
 *
 ...
 *
 * @since 3.6.0
 * @output mt-includes/js/heartbeat.js
 */
...
( function( $, window, undefined ) {
	...
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.mt-heartbeat', function() {
			...
			$(window).on( 'unload.mt-heartbeat', function() {
		...
		function setErrorState( error, status ) {
			...
			if ( error ) {
				...
				if ( trigger && ! hasConnectionError() ) {
					...
					mt.hooks.doAction( 'heartbeat.connection-lost', error, status );
				}
			}
		}
		...
		function clearErrorState() {
			...
			if ( hasConnectionError() ) {
				...
				mt.hooks.doAction( 'heartbeat.connection-restored' );
			}
		}
		...
		function connect() {
			...
			mt.hooks.doAction( 'heartbeat.send', heartbeatData );
			...
			settings.xhr = $.ajax({
				...
				if ( response.nonces_expired ) {
					...
					mt.hooks.doAction( 'heartbeat.nonces-expired' );
				}
				...
				// Update the Rest API nonce if set and mt-api loaded.
				if ( response.rest_nonce && typeof window.mtApiSettings === 'object' ) {
					window.mtApiSettings.nonce = response.rest_nonce;
					// This nonce is required for api-fetch through heartbeat.tick.
					// delete response.rest_nonce;
				}

				$document.trigger( 'heartbeat-tick', [response, textStatus, jqXHR] );
				mt.hooks.doAction( 'heartbeat.tick', response, textStatus, jqXHR );
				...
				mt.hooks.doAction( 'heartbeat.error', jqXHR, textStatus, error );
			});
		}
		...
		function userIsActive() {
			...
			$document.off( '.mt-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.mt-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.mt-heartbeat-active keyup.mt-heartbeat-active touchend.mt-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.mt-heartbeat-active keyup.mt-heartbeat-active touchend.mt-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
		 ...
		 *
		 * @since 3.6.0
		 * @memberOf mt.heartbeat.prototype
		 *
		 ...
		 * the third argument is 'true'. Use `mt.heartbeat.isQueued('handle')` to see if
	...
	/**
	 * Ensure the global `mt` object exists.
	 *
	 * @namespace mt
	 */
	window.mt = window.mt || {};
	 ...
	 *
	 * @namespace mt.heartbeat
	 * @type {Heartbeat}
	 */
	window.mt.heartbeat = new Heartbeat();
