...
class MT_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to MT_Ajax_Response::add().
	 *
	 * @since 2.1.0
	 *
	 * @see MT_Ajax_Response::add()
	 *
	 ...
	 *     @type int|MT_Error    $id           The response ID, used as the response type `id` attribute. Also
	 *                                         accepts a `MT_Error` object if the ID does not exist. Default 0.
	 ...
	 *     @type string|MT_Error $data         The response content/message. Also accepts a MT_Error object if the
	...
	public function add( $args = '' ) {
		...
		if ( is_mt_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response  .= "<mt_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . ']]></mt_error>';
				...
				$response .= "<mt_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><mt_ajax>";
