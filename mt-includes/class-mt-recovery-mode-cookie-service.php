<?php
/**
 * Error Protection API: MT_Recovery_Mode_Cookie_Service class
 *
 * @package paCMec
 * @since 5.2.0
 */
 ...
 * @since 5.2.0
 */
final class MT_Recovery_Mode_Cookie_Service {
	 ...
	 * @return true|MT_Error True on success, error object on failure.
	 */
	public function validate_cookie( $cookie = '' ) {

		if ( ! $cookie ) {
			if ( empty( $_COOKIE[ RECOVERY_MODE_COOKIE ] ) ) {
				return new MT_Error( 'no_cookie', __( 'No cookie present.' ) );
			}
		...
		if ( ! ctype_digit( $created_at ) ) {
			return new MT_Error( 'invalid_created_at', __( 'Invalid cookie format.' ) );
		}

		/** This filter is documented in mt-includes/class-mt-recovery-mode-cookie-service.php */
		$length = apply_filters( 'recovery_mode_cookie_length', WEEK_IN_SECONDS );

		if ( time() > $created_at + $length ) {
			return new MT_Error( 'expired', __( 'Cookie expired.' ) );
		}
		...
		if ( ! hash_equals( $signature, $hashed ) ) {
			return new MT_Error( 'signature_mismatch', __( 'Invalid cookie.' ) );
		}
	 ...
	 * @param string $cookie Optionally specify the cookie string.
	 *                       If omitted, it will be retrieved from the super global.
	 * @return string|MT_Error Session ID on success, or error object on failure.
	 */
	 ...
	 * @return array|MT_Error Cookie parts array, or error object on failure.
	 */
	private function parse_cookie( $cookie ) {
		...
		if ( 4 !== count( $parts ) ) {
			return new MT_Error( 'invalid_format', __( 'Invalid cookie format.' ) );
		}
	...
	private function generate_cookie() {
		$to_sign = sprintf( 'recovery_mode|%s|%s', time(), mt_generate_password( 20, false ) );
	...
	/**
	 * Gets a form of `mt_hash()` specific to Recovery Mode.
	 *
	 * We cannot use `mt_hash()` because it is defined in `pluggable.php` which is not loaded until after plugins are loaded,
	...
	private function recovery_mode_hash( $data ) {
		if ( ! defined( 'AUTH_KEY' ) || AUTH_KEY === 'put your unique phrase here' ) {
			...
			if ( ! $auth_key ) {
				...
				$auth_key = mt_generate_password( 64, true, true );
		...
		if ( ! defined( 'AUTH_SALT' ) || AUTH_SALT === 'put your unique phrase here' || AUTH_SALT === $auth_key ) {
			...
			if ( ! $auth_salt ) {
				...
				$auth_salt = mt_generate_password( 64, true, true );
