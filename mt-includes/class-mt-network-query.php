<?php
/**
 * Network API: MT_Network_Query class
 *
 * @package paCMec
 ...
 * @since 4.6.0
 *
 * @see MT_Network_Query::__construct() for accepted arguments.
 */
class MT_Network_Query {
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query MT_Network_Query arguments. See MT_Network_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param MT_Network_Query $this The MT_Network_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_network_query', array( &$this ) );
	}

	/**
	 * Sets up the paCMec query for retrieving networks.
	 *
	 * @since 4.6.0
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 * @return array|int List of MT_Network objects, a list of network IDs when 'fields' is set to 'ids',
	...
	public function get_networks() {
		 ...
		 * @since 4.6.0
		 *
		 * @param MT_Network_Query $this Current instance of MT_Network_Query (passed by reference).
		 */
		 ...
		 *
		 * Return a non-null value to bypass paCMec' default network queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of MT_Network objects.
		 *
		 * Note that if the filter returns an array of network data, it will be assigned
		 * to the `networks` property of the current MT_Network_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_networks` and `max_num_pages` properties of the MT_Network_Query object,
		 * passed to the filter by reference. If MT_Network_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.2.0
		 * @since 5.6.0 The returned array of network data is assigned to the `networks` property
		 *              of the current MT_Network_Query instance.
		 *
		 * @param array|int|null   $network_data Return an array of network data to short-circuit MT's network query,
		 *                                       the network count as an integer if `$this->query_vars['count']` is set,
		 *                                       or null to allow MT to run its normal queries.
		 * @param MT_Network_Query $query        The MT_Network_Query instance, passed by reference.
		 */
		...
		$last_changed = mt_cache_get_last_changed( 'networks' );

		$cache_key   = "get_network_ids:$key:$last_changed";
		$cache_value = mt_cache_get( $cache_key, 'networks' );
		...
		if ( false === $cache_value ) {
			...
			mt_cache_add( $cache_key, $cache_value, 'networks' );
		} else {
		 ...
		 * @since 4.6.0
		 *
		 * @param MT_Network[]     $_networks An array of MT_Network objects.
		 * @param MT_Network_Query $query     Current instance of MT_Network_Query (passed by reference).
		 */
		$_networks = apply_filters_ref_array( 'the_networks', array( $_networks, &$this ) );

		// Convert to MT_Network instances.
	...
	protected function get_network_ids() {
		...
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = "$mtdb->site.id IN ( " . implode( ',', mt_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = "$mtdb->site.id NOT IN ( " . implode( ',', mt_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}

		if ( ! empty( $this->query_vars['domain'] ) ) {
			$this->sql_clauses['where']['domain'] = $mtdb->prepare( "$mtdb->site.domain = %s", $this->query_vars['domain'] );
		}

		// Parse network domain for an IN clause.
		if ( is_array( $this->query_vars['domain__in'] ) ) {
			$this->sql_clauses['where']['domain__in'] = "$mtdb->site.domain IN ( '" . implode( "', '", $mtdb->_escape( $this->query_vars['domain__in'] ) ) . "' )";
		}

		// Parse network domain for a NOT IN clause.
		if ( is_array( $this->query_vars['domain__not_in'] ) ) {
			$this->sql_clauses['where']['domain__not_in'] = "$mtdb->site.domain NOT IN ( '" . implode( "', '", $mtdb->_escape( $this->query_vars['domain__not_in'] ) ) . "' )";
		}

		if ( ! empty( $this->query_vars['path'] ) ) {
			$this->sql_clauses['where']['path'] = $mtdb->prepare( "$mtdb->site.path = %s", $this->query_vars['path'] );
		}

		// Parse network path for an IN clause.
		if ( is_array( $this->query_vars['path__in'] ) ) {
			$this->sql_clauses['where']['path__in'] = "$mtdb->site.path IN ( '" . implode( "', '", $mtdb->_escape( $this->query_vars['path__in'] ) ) . "' )";
		}

		// Parse network path for a NOT IN clause.
		if ( is_array( $this->query_vars['path__not_in'] ) ) {
			$this->sql_clauses['where']['path__not_in'] = "$mtdb->site.path NOT IN ( '" . implode( "', '", $mtdb->_escape( $this->query_vars['path__not_in'] ) ) . "' )";
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			$this->sql_clauses['where']['search'] = $this->get_search_sql(
				...
				array( "$mtdb->site.domain", "$mtdb->site.path" )
			);
		}
		...
		$this->sql_clauses['from']    = "FROM $mtdb->site $join";
		...
		$network_ids = $mtdb->get_col( $this->request );
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param MT_Network_Query $network_query        The `MT_Network_Query` instance.
			 */
			$found_networks_query = apply_filters( 'found_networks_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_networks = (int) $mtdb->get_var( $found_networks_query );
		}
	}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( 'network__in' === $orderby ) {
			...
			$parsed      = "FIELD( {$mtdb->site}.id, $network__in )";
		} elseif ( 'domain_length' === $orderby || 'path_length' === $orderby ) {
			$field  = substr( $orderby, 0, -7 );
			$parsed = "CHAR_LENGTH($mtdb->site.$field)";
		} elseif ( in_array( $orderby, $allowed_keys, true ) ) {
			$parsed = "$mtdb->site.$orderby";
		}
