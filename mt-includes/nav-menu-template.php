...
require_once ABSPATH . MTINC . '/class-walker-nav-menu.php';
 ...
 *
 *     @type int|string|MT_Term $menu                 Desired menu. Accepts a menu ID, slug, name, or object.
 *                                                    Default empty.
 ...
 *                                                    Default is 'mt_page_menu'. Set to false for no fallback.
 ...
 */
function mt_nav_menu( $args = array() ) {
	 ...
	 * @see mt_nav_menu()
	 *
	 * @param array $args Array of mt_nav_menu() arguments.
	 */
	$args = apply_filters( 'mt_nav_menu_args', $args );
	$args = (object) $args;

	/**
	 * Filters whether to short-circuit the mt_nav_menu() output.
	 *
	 * Returning a non-null value from the filter will short-circuit mt_nav_menu(),
	 ...
	 * @param stdClass    $args   An object containing mt_nav_menu() arguments.
	 */
	$nav_menu = apply_filters( 'pre_mt_nav_menu', null, $args );
	...
	$menu = mt_get_nav_menu_object( $args->menu );

	// Get the nav menu based on the theme_location.
	$locations = get_nav_menu_locations();
	if ( ! $menu && $args->theme_location && $locations && isset( $locations[ $args->theme_location ] ) ) {
		$menu = mt_get_nav_menu_object( $locations[ $args->theme_location ] );
	}
	...
	if ( ! $menu && ! $args->theme_location ) {
		...
		foreach ( $menus as $menu_maybe ) {
			$menu_items = mt_get_nav_menu_items( $menu_maybe->term_id, array( 'update_post_term_cache' => false ) );
			if ( $menu_items ) {
				$menu = $menu_maybe;
				break;
			}
	...
	if ( $menu && ! is_mt_error( $menu ) && ! isset( $menu_items ) ) {
		$menu_items = mt_get_nav_menu_items( $menu->term_id, array( 'update_post_term_cache' => false ) );
	}
	 ...
	 *  - Otherwise, bail.
	 */
	if ( ( ! $menu || is_mt_error( $menu ) || ( isset( $menu_items ) && empty( $menu_items ) && ! $args->theme_location ) )
		&& isset( $args->fallback_cb ) && $args->fallback_cb && is_callable( $args->fallback_cb ) ) {
			return call_user_func( $args->fallback_cb, (array) $args );
	}

	if ( ! $menu || is_mt_error( $menu ) ) {
		return false;
	}
	...
	if ( $args->container ) {
		 ...
		 */
		$allowed_tags = apply_filters( 'mt_nav_menu_container_allowedtags', array( 'div', 'nav' ) );
	...
	_mt_menu_item_classes_by_context( $menu_items );
	 ...
	 * @param stdClass $args              An object containing mt_nav_menu() arguments.
	 */
	$sorted_menu_items = apply_filters( 'mt_nav_menu_objects', $sorted_menu_items, $args );
	 ...
	 * @param stdClass $args  An object containing mt_nav_menu() arguments.
	 */
	$items = apply_filters( 'mt_nav_menu_items', $items, $args );
	/**
	 ...
	 * @param stdClass $args  An object containing mt_nav_menu() arguments.
	 */
	$items = apply_filters( "mt_nav_menu_{$menu->slug}_items", $items, $args );
	 ...
	 * @param stdClass $args     An object containing mt_nav_menu() arguments.
	 */
	$nav_menu = apply_filters( 'mt_nav_menu', $nav_menu, $args );
 ...
 */
function _mt_menu_item_classes_by_context( &$menu_items ) {
	global $mt_query, $mt_rewrite;

	$queried_object    = $mt_query->get_queried_object();
	$queried_object_id = (int) $mt_query->queried_object_id;
	...
	if ( $mt_query->is_singular && ! empty( $queried_object->post_type ) && ! is_post_type_hierarchical( $queried_object->post_type ) ) {
		foreach ( (array) get_object_taxonomies( $queried_object->post_type ) as $taxonomy ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				$term_hierarchy = _get_term_hierarchy( $taxonomy );
				$terms          = mt_get_object_terms( $queried_object_id, $taxonomy, array( 'fields' => 'ids' ) );
				if ( is_array( $terms ) ) {
	...
	foreach ( (array) $menu_items as $key => $menu_item ) {
		...
		if ( $mt_query->is_singular && 'taxonomy' === $menu_item->type
			...
			&& (
				( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
					&& $mt_query->is_home && $home_page_id == $menu_item->object_id )
				|| ( 'post_type' === $menu_item->type && $mt_query->is_singular )
				|| ( 'taxonomy' === $menu_item->type
					&& ( $mt_query->is_category || $mt_query->is_tag || $mt_query->is_tax )
			...
			if ( 'post_type' === $menu_item->type && 'page' === $menu_item->object ) {
				// Back compat classes for pages to match mt_page_menu().
				$classes[] = 'page_item';
			...
			$_indexless_current = untrailingslashit( preg_replace( '/' . preg_quote( $mt_rewrite->index, '/' ) . '$/', '', $current_url ) );
			...
			if ( $raw_item_url && in_array( $item_url, $matches, true ) ) {
				...
				if ( in_array( home_url(), array( untrailingslashit( $current_url ), untrailingslashit( $_indexless_current ) ), true ) ) {
					// Back compat for home link to match mt_page_menu().
					$classes[] = 'current_page_item';
				}
				$active_parent_item_ids[]   = (int) $menu_item->menu_item_parent;
				$active_parent_object_ids[] = (int) $menu_item->post_parent;
		...
		// Back-compat with mt_page_menu(): add "current_page_parent" to static home page link for any non-page query.
		if ( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
			&& empty( $mt_query->is_page ) && $home_page_id == $menu_item->object_id
		) {
 ...
 * @param stdClass $r     An object containing mt_nav_menu() arguments.
