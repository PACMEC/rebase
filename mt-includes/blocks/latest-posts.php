...
function render_block_core_latest_posts( $attributes ) {
	...
	foreach ( $recent_posts as $post ) {
		...
		if ( $attributes['displayFeaturedImage'] && has_post_thumbnail( $post ) ) {
			...
			$image_classes = 'mt-block-latest-posts__featured-image';
		...
		if ( isset( $attributes['displayAuthor'] ) && $attributes['displayAuthor'] ) {
			...
			if ( ! empty( $author_display_name ) ) {
				$list_items_markup .= sprintf(
					'<div class="mt-block-latest-posts__post-author">%1$s</div>',
					esc_html( $byline )
				);
			}
		}
		...
		if ( isset( $attributes['displayPostDate'] ) && $attributes['displayPostDate'] ) {
			$list_items_markup .= sprintf(
				'<time datetime="%1$s" class="mt-block-latest-posts__post-date">%2$s</time>',
		...
		if ( isset( $attributes['displayPostContent'] ) && $attributes['displayPostContent']
			...
			$list_items_markup .= sprintf(
				'<div class="mt-block-latest-posts__post-excerpt">%1$s</div>',
				$trimmed_excerpt
			);
		}
		...
		if ( isset( $attributes['displayPostContent'] ) && $attributes['displayPostContent']
			...
			$list_items_markup .= sprintf(
				'<div class="mt-block-latest-posts__post-full-content">%1$s</div>',
				mt_kses_post( html_entity_decode( $post->post_content, ENT_QUOTES, get_option( 'blog_charset' ) ) )
			);
		}
	...
	$class = 'mt-block-latest-posts__list';
