...
if ( ! class_exists( 'Requests' ) ) {
	require ABSPATH . MTINC . '/class-requests.php';

	Requests::register_autoloader();
	Requests::set_certificate_path( ABSPATH . MTINC . '/certificates/ca-bundle.crt' );
}
...
class MT_Http {
	 ...
	 *                                             Default 'paCMec/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ).
	 *     @type bool         $reject_unsafe_urls  Whether to pass URLs through mt_http_validate_url().
	 *                                             Default false.
	 ...
	 *                                             Default ABSPATH . MTINC . '/certificates/ca-bundle.crt'.
	 *     @type bool         $stream              Whether to stream to a file. If set to true and no filename was
	 *                                             given, it will be droped it in the MT temp dir and its name will
	 ...
	 *
	 * }
	 * @return array|MT_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A MT_Error instance upon error.
	 */
	public function request( $url, $args = array() ) {
		$defaults = array(
			 ...
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param string $user_agent paCMec user agent string.
			 * @param string $url        The request URL.
			 */
			'user-agent'          => apply_filters( 'http_headers_useragent', 'paCMec/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $url ),
			/**
			 * Filters whether to pass URLs through mt_http_validate_url() in an HTTP request.
			 *
			 * @since 3.6.0
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param bool   $pass_url Whether to pass URLs through mt_http_validate_url(). Default false.
			 * @param string $url      The request URL.
			 */
			...
			'decompress'          => true,
			'sslverify'           => true,
			'sslcertificates'     => ABSPATH . MTINC . '/certificates/ca-bundle.crt',
			'stream'              => false,
			'filename'            => null,
		 ...
		 * @since 2.9.0
		 *
		 * @param false|array|MT_Error $preempt     A preemptive return value of an HTTP request. Default false.
		...
		if ( function_exists( 'mt_kses_bad_protocol' ) ) {
			if ( $parsed_args['reject_unsafe_urls'] ) {
				$url = mt_http_validate_url( $url );
			}
			if ( $url ) {
				$url = mt_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		if ( empty( $url ) || empty( $arrURL['scheme'] ) ) {
			$response = new MT_Error( 'http_request_failed', __( 'A valid URL was not provided.' ) );
			/** This action is documented in mt-includes/class-http.php */
			do_action( 'http_api_debug', $response, 'response', 'Requests', $parsed_args, $url );
			return $response;
		}

		if ( $this->block_request( $url ) ) {
			$response = new MT_Error( 'http_request_not_executed', __( 'User has blocked requests through HTTP.' ) );
			/** This action is documented in mt-includes/class-http.php */
			do_action( 'http_api_debug', $response, 'response', 'Requests', $parsed_args, $url );
			return $response;
		}

		// If we are streaming to a file but no filename was given drop it in the MT temp dir
		...
		if ( $parsed_args['stream'] ) {
			...
			if ( ! mt_is_writable( dirname( $parsed_args['filename'] ) ) ) {
				$response = new MT_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
				/** This action is documented in mt-includes/class-http.php */
				do_action( 'http_api_debug', $response, 'response', 'Requests', $parsed_args, $url );
				return $response;
			}
		...
		// MT allows passing in headers as a string, weirdly.
		if ( ! is_array( $parsed_args['headers'] ) ) {
			$processedHeaders       = MT_Http::processHeaders( $parsed_args['headers'] );
		...
		$options = array(
			...
			'hooks'     => new MT_HTTP_Requests_Hooks( $url, $parsed_args ),
		);
		...
		if ( function_exists( 'mt_kses_bad_protocol' ) && $parsed_args['reject_unsafe_urls'] ) {
		...
		if ( ! empty( $parsed_args['cookies'] ) ) {
			$options['cookies'] = MT_Http::normalize_cookies( $parsed_args['cookies'] );
		}
		...
		try {
			...
			$http_response = new MT_HTTP_Requests_Response( $requests_response, $parsed_args['filename'] );
			$response      = $http_response->to_array();

			// Add the original object to the array.
			$response['http_response'] = $http_response;
		} catch ( Requests_Exception $e ) {
			$response = new MT_Error( 'http_request_failed', $e->getMessage() );
		}
		 ...
		 * @since 2.8.0
		 *
		 * @param array|MT_Error $response    HTTP response or MT_Error object.
	...
	public static function normalize_cookies( $cookies ) {
		...
		foreach ( $cookies as $name => $value ) {
			if ( $value instanceof MT_Http_Cookie ) {
	...
	public static function validate_redirects( $location ) {
		if ( ! mt_http_validate_url( $location ) ) {
			throw new Requests_Exception( __( 'A valid URL was not provided.' ), 'mt_http.redirect_failed_validation' );
		}
	}
	 ...
	 *
	 * @since 3.2.0
	 * @deprecated 5.1.0 Use MT_Http::request()
	 * @see MT_Http::request()
	 *
	 * @param string $url  URL to request.
	 * @param array  $args Request arguments.
	...
	private function _dispatch_request( $url, $args ) {
		...
		if ( ! $class ) {
			return new MT_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		}
	 ...
	 * }
	 */
	public static function processResponse( $strResponse ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.MethodNameInvalid
		$res = explode( "\r\n\r\n", $strResponse, 2 );
	 ...
	 *     @type MT_Http_Cookie[] $cookies    If the original headers contain the 'Set-Cookie' key,
	 *                                        an array containing `MT_Http_Cookie` objects is returned.
	 * }
	 */
	public static function processHeaders( $headers, $url = '' ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.MethodNameInvalid
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' === $key ) {
				$cookies[] = new MT_Http_Cookie( $value, $url );
			}
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to MT_Http_Cookie instances,
	 ...
	 */
	public static function buildCookieHeader( &$r ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.MethodNameInvalid
		if ( ! empty( $r['cookies'] ) ) {
			// Upgrade any name => value cookie pairs to MT_HTTP_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) ) {
					$r['cookies'][ $name ] = new MT_Http_Cookie(
						array(
							'name'  => $name,
							'value' => $value,
						)
	 ...
	 */
	public static function chunkTransferDecode( $body ) { // phpcs:ignore paCMec.NamingConventions.ValidFunctionName.MethodNameInvalid
	...
	public function block_request( $uri ) {
		...
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split( '|,\s*|', MT_ACCESSIBLE_HOSTS );

			if ( false !== strpos( MT_ACCESSIBLE_HOSTS, '*' ) ) {
	 ...
	 *
	 * @deprecated 4.4.0 Use mt_parse_url()
	 * @see mt_parse_url()
	 *
	...
	protected static function parse_url( $url ) {
		_deprecated_function( __METHOD__, '4.4.0', 'mt_parse_url()' );
		return mt_parse_url( $url );
	}
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		$url_parts = mt_parse_url( $url );
		if ( ! $url_parts ) {
			return $maybe_relative_path;
		}

		$relative_url_parts = mt_parse_url( $maybe_relative_path );
	 ...
	 * @return array|false|MT_Error An HTTP API response array if the redirect is successfully followed,
	 *                              false if no redirect is present, or a MT_Error object if there's an error.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 ) {
			return new MT_Error( 'http_request_failed', __( 'Too many redirects.' ) );
		}
		...
		$redirect_location = MT_Http::make_absolute_url( $redirect_location, $url );
		...
		return mt_remote_request( $redirect_location, $args );
	}
