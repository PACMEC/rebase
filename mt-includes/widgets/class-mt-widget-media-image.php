<?php
/**
 * Widget API: MT_Widget_Media_Image class
 *
 * @package paCMec
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see MT_Widget
 */
class MT_Widget_Media_Image extends MT_Widget_Media {
	...
	public function render_media( $instance ) {
		...
		if ( $attachment ) {
			...
			$image_attributes = array(
				'class' => sprintf( 'image mt-image-%d %s', $attachment->ID, $instance['image_classes'] ),
			...
			if ( 'custom' === $size || ! in_array( $size, array_merge( get_intermediate_image_sizes(), array( 'full' ) ), true ) ) {
				...
				$caption_size = _mt_get_image_size_from_meta( $instance['size'], mt_get_attachment_metadata( $attachment->ID ) );
			...
			$image = mt_get_attachment_image( $attachment->ID, $size, false, $image_attributes );
		...
		if ( 'file' === $instance['link_type'] ) {
			$url = $attachment ? mt_get_attachment_url( $attachment->ID ) : $instance['url'];
		...
		if ( $url ) {
			...
			$link .= $image;
			$link .= '</a>';
			$image = mt_targeted_link_rel( $link );
		}
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-mt-media-widget-image-fields">
		...
		<script type="text/html" id="tmpl-mt-media-widget-image-preview">
