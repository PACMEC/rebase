<?php
/**
 * Widget API: MT_Media_Widget class
 *
 * @package paCMec
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see MT_Widget
 */
abstract class MT_Widget_Media extends MT_Widget {
	 ...
	 * @param array  $control_options Optional. Widget control options. See mt_register_widget_control()
	 *                                for information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		$widget_opts = mt_parse_args(
			$widget_options,
			array(
		...
		$control_opts = mt_parse_args( $control_options, array() );
	...
	public function get_instance_schema() {
		 ...
		 * @param MT_Widget_Media $widget Widget object.
		 */
	 ...
	 * @since 4.8.0
	 *
	 * @param int|MT_Post $attachment Attachment post ID or object.
	...
	public function is_attachment_with_mime_type( $attachment, $mime_type ) {
		...
		return mt_attachment_is( $mime_type, $attachment );
	}
	 ...
	 * @since 4.8.0
	 *
	 * @see MT_Widget::widget()
	 *
	 * @param array $args     Display arguments including before_title, after_title, before_widget, and after_widget.
	 * @param array $instance Saved setting from the database.
	 */
	public function widget( $args, $instance ) {
		$instance = mt_parse_args( $instance, mt_list_pluck( $this->get_instance_schema(), 'default' ) );
	...
	public function update( $new_instance, $instance ) {
		...
		foreach ( $schema as $field => $field_schema ) {
			...
			if ( is_mt_error( $value ) ) {
				continue; // Handle case when rest_sanitize_value_from_schema() ever returns MT_Error as its phpdoc @return tag indicates.
			}
	 ...
	 * @since 4.8.0
	 *
	 * @see \MT_Widget_Media::render_control_template_scripts() Where the JS template is located.
	 *
	 * @param array $instance Current settings.
	 */
	final public function form( $instance ) {
		$instance_schema = $this->get_instance_schema();
		$instance        = mt_array_slice_assoc(
			mt_parse_args( (array) $instance, mt_list_pluck( $instance_schema, 'default' ) ),
		...
		foreach ( $instance as $name => $value ) : ?>
			<input
				...
				id="<?php echo esc_attr( $this->get_field_id( $name ) ); // Needed specifically by mtWidgets.appendTitle(). ?>"
	...
	public function enqueue_admin_scripts() {
		mt_enqueue_media();
		mt_enqueue_script( 'media-widgets' );
	}
