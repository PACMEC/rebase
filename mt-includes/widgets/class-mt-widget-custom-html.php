<?php
/**
 * Widget API: MT_Widget_Custom_HTML class
 *
 * @package paCMec
 * @subpackage Widgets
 * @since 4.8.1
 ...
 * @see MT_Widget
 */
class MT_Widget_Custom_HTML extends MT_Widget {
	...
	public function _register_one( $number = -1 ) {
		...
		mt_add_inline_script( 'custom-html-widgets', sprintf( 'mt.customHtmlWidgets.idBases.push( %s );', mt_json_encode( $this->id_base ) ) );

		// Note that the widgets component in the customizer will also do
		// the 'admin_print_scripts-widgets.php' action in MT_Customize_Widgets::print_scripts().
		add_action( 'admin_print_scripts-widgets.php', array( $this, 'enqueue_admin_scripts' ) );

		// Note that the widgets component in the customizer will also do
		// the 'admin_footer-widgets.php' action in MT_Customize_Widgets::print_footer_scripts().
		add_action( 'admin_footer-widgets.php', array( 'MT_Widget_Custom_HTML', 'render_control_template_scripts' ) );

		// Note this action is used to ensure the help text is added to the end.
		add_action( 'admin_head-widgets.php', array( 'MT_Widget_Custom_HTML', 'add_help_text' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		$simulated_text_widget_instance = array_merge(
			...
			array(
				...
				'filter' => false, // Because mtautop is not applied.
				'visual' => false, // Because it wasn't created in TinyMCE.
			)
		);
		unset( $simulated_text_widget_instance['content'] ); // Was moved to 'text' prop.

		/** This filter is documented in mt-includes/widgets/class-mt-widget-text.php */
		$content = apply_filters( 'widget_text', $instance['content'], $simulated_text_widget_instance, $this );

		// Adds 'noopener' relationship, without duplicating values, to all HTML A elements that have a target.
		$content = mt_targeted_link_rel( $content );
		 ...
		 * @param MT_Widget_Custom_HTML $widget   Current Custom HTML widget instance.
		 */
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			...
			$instance['content'] = mt_kses_post( $new_instance['content'] );
		}
		return $instance;
	}
	...
	public function enqueue_admin_scripts() {
		$settings = mt_enqueue_code_editor(
			array(
				'type'       => 'text/html',
				'codemirror' => array(
					'indentUnit' => 2,
		...
		mt_enqueue_script( 'custom-html-widgets' );
		if ( empty( $settings ) ) {
			$settings = array(
				'disabled' => true,
			);
		}
		mt_add_inline_script( 'custom-html-widgets', sprintf( 'mt.customHtmlWidgets.init( %s );', mt_json_encode( $settings ) ), 'after' );
		...
		mt_add_inline_script( 'custom-html-widgets', sprintf( 'jQuery.extend( mt.customHtmlWidgets.l10n, %s );', mt_json_encode( $l10n ) ), 'after' );
	}
	 ...
	 *
	 * @since 4.8.1
	 * @since 4.9.0 The form contains only hidden sync inputs. For the control UI, see `MT_Widget_Custom_HTML::render_control_template_scripts()`.
	 *
	 * @see MT_Widget_Custom_HTML::render_control_template_scripts()
	 *
	 * @param array $instance Current instance.
	 */
	public function form( $instance ) {
		$instance = mt_parse_args( (array) $instance, $this->default_instance );
		?>
