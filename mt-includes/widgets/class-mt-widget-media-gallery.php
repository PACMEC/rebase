<?php
/**
 * Widget API: MT_Widget_Media_Gallery class
 *
 * @package paCMec
 * @subpackage Widgets
 * @since 4.9.0
 ...
 * @see MT_Widget
 */
class MT_Widget_Media_Gallery extends MT_Widget_Media {
	...
	public function get_instance_schema() {
		$schema = array(
			...
			'ids'            => array(
				...
				'default'           => array(),
				'sanitize_callback' => 'mt_parse_id_list',
			),
			'columns'        => array(
				'type'    => 'integer',
				'default' => 3,
		...
		/** This filter is documented in mt-includes/widgets/class-mt-widget-media.php */
	...
	public function enqueue_admin_scripts() {
		...
		foreach ( $this->get_instance_schema() as $field => $field_schema ) {
			$exported_schema[ $field ] = mt_array_slice_assoc( $field_schema, array( 'type', 'default', 'enum', 'minimum', 'format', 'media_prop', 'should_preview_update', 'items' ) );
		}
		...
		mt_add_inline_script(
			...
			sprintf(
				'
					...
					_.extend( mt.mediaWidgets.controlConstructors[ %1$s ].prototype.l10n, %3$s );
				',
				mt_json_encode( $this->id_base ),
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-mt-media-widget-gallery-preview">
			<#
	...
	protected function has_content( $instance ) {
		if ( ! empty( $instance['ids'] ) ) {
			$attachments = mt_parse_id_list( $instance['ids'] );
