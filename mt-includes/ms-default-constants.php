 ...
 * mt-includes/ms-files.php (mt-content/blogs.php in MU).
 *
 * @since 3.0.0
 */
function ms_upload_constants() {
	...
	if ( ! defined( 'UPLOADBLOGSDIR' ) ) {
		define( 'UPLOADBLOGSDIR', 'mt-content/blogs.dir' );
	}

	// Note, the main site in a post-MU network uses mt-content/uploads.
	// This is handled in mt_upload_dir() by ignoring UPLOADS for this case.
	if ( ! defined( 'UPLOADS' ) ) {
		...
		if ( 'mt-content/blogs.dir' === UPLOADBLOGSDIR && ! defined( 'BLOGUPLOADDIR' ) ) {
			define( 'BLOGUPLOADDIR', MT_CONTENT_DIR . '/blogs.dir/' . $site_id . '/files/' );
		}
	}
}
...
function ms_file_constants() {
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'MTMU_SENDFILE' ) ) {
		define( 'MTMU_SENDFILE', false );
	}
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'MTMU_ACCEL_REDIRECT' ) ) {
		define( 'MTMU_ACCEL_REDIRECT', false );
	}
}
...
function ms_subdomain_constants() {
	...
	if ( $subdomain_error ) {
		$vhost_deprecated = sprintf(
			/* translators: 1: VHOST, 2: SUBDOMAIN_INSTALL, 3: mt-config.php, 4: is_subdomain_install() */
